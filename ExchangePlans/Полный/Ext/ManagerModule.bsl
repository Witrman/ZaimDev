...
Процедура ПриПолученииНастроек(НастройкиПланаОбмена) Экспорт
	...
	НастройкиПланаОбмена.НазначениеПланаОбмена = "РИБ";
	
	НастройкиПланаОбмена.Алгоритмы.ПриПолученииОписанияВариантаНастройки = Истина;
	НастройкиПланаОбмена.Алгоритмы.ОписаниеОграниченийПередачиДанных     = Ложь;
...
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	ОписаниеВарианта.ЗаголовокКомандыДляСозданияНовогоОбменаДанными = НСтр("ru = 'Полный (Распределенная информационная база)'");
...
Процедура ЗарегистрироватьОбъектыПодсистемыСвойства() Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Запрос = Новый Запрос();
		Запрос.Параметры.Вставить("ЭтотУзел", ПланыОбмена.Полный.ЭтотУзел());
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ПланОбменаРИБ.Ссылка
		|ИЗ
		|	ПланОбмена.Полный КАК ПланОбменаРИБ
		|ГДЕ
		...
		Попытка
			...
			ПланыОбмена.ЗарегистрироватьИзменения(МассивУзлов, Метаданные.РегистрыСведений.ДополнительныеСведения);
		...
		Исключение
			...
			Комментарий = НСтр(
			"ru = 'При регистрации объектов плана обмена ""Полный"" произошла ошибка:
			|%1'");
			
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Комментарий, ПодробноеПредставлениеОшибки);
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление ИБ. Регистрация объектов плана обмена ""Полный""'"), УровеньЖурналаРегистрации.Ошибка, , , Комментарий);
