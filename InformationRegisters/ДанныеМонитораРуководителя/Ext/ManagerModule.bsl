...
Процедура ОбновитьДанныеМонитора(Организация, Дата = Неопределено, ИнтервалОбновления = 0, РазделыМонитора = Неопределено, ПолучатьПрошлыйПериод = Ложь) Экспорт
	...
	Если НЕ ИнтервалОбновления Тогда
	...
	Если ПолучатьПрошлыйПериод И НЕ ДанныеПрошлогоПериодаАктуальны(Организация, Дата, ИнтервалОбновления) Тогда
	...
	Попытка
		...
		СтруктураПараметровБлокировки.Вставить("ИмяТаблицы", "ДанныеМонитораРуководителя");
		
		ЗначенияБлокировки = Новый Соответствие;
		ЗначенияБлокировки.Вставить("Организация", Организация);
		
		//ОбщегоНазначенияБКВызовСервера.УстановитьУправляемуюБлокировку(СтруктураПараметровБлокировки, ЗначенияБлокировки);
