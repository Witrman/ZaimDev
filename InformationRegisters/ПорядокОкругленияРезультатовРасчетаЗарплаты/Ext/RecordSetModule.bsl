...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаОкругления Из ЭтотОбъект Цикл
        ...
        ИначеЕсли СтрокаОкругления.ВидРасчета = Справочники.НалогиСборыОтчисления.ЕдиныйПлатеж Тогда
            
            СообщениеОбОшибке = НСтр("ru = 'Установить порядок округления для ""Единого платежа"" невозможно.'");
            ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
            Отказ = Истина;
