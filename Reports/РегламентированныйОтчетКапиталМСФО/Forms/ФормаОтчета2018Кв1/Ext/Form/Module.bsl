...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""об изменениях в собственном капитале (мсфо)"" (заполнение)";
...
Процедура УстановитьПериодОтчета()
	...
	Если СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Год Тогда
		...
		ПериодОтчета = НСтр("kz = '%1 жылғы 31 желтоқсанда аяқталатын жыл ішінде'; ru = 'за год, заканчивающийся 31 декабря %1 года'", ЯзыкФормирования);
	...
	ОтчетныйПериодНаКазахском	= Строка(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)) + " жыл" + " есепті кезең";
...
Функция ПолучитьСписокСчетовКапитала()
	...
	СписокСчетовКапитала.Добавить(ПредопределенноеЗначение("ПланСчетов.Типовой.Резервы")); 									// Счет5500
	СписокСчетовКапитала.Добавить(ПредопределенноеЗначение("ПланСчетов.Типовой.НераспределеннаяПрибыльНепокрытыйУбыток"));	// Счет5600
...
Процедура РассчитатьИтогПоСтроке(ИмяСтроки, Таблицаданных)
	...
	Для СчетчикСтраниц = 1 По ВсегоСтраниц Цикл
		Если ЭтотОбъект["ФормаОтчетаПолеТабличногоДокументаСтраница" + СчетчикСтраниц].Области.Найти(ИмяСтроки + "_10") <> Неопределено Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,ИмяСтроки+ "_10", ИтогоКапитал, ЭтотОбъект["ФормаОтчетаПолеТабличногоДокументаСтраница" + СчетчикСтраниц]);			
			Возврат;
		КонецЕсли;
	КонецЦикла;	    
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	Для НомерКолонки = 3 По 10 Цикл
		...
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б100_" + НомерКолонки)
				...
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б319_" + НомерКолонки);
		...
		Для Счетчик  = 621 по 629 Цикл
			...
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерКолонки);
		КонецЦикла;	
        РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"Б620_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница3);
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"Б600_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница3);		
		...
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б500_" + НомерКолонки)
				...
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б719_" + НомерКолонки);
	...
	РассчитатьИтогПоСтроке("Б220", ТаблицаДанных);
	...
	РассчитатьИтогПоСтроке("Б319", ТаблицаДанных);
	...
	РассчитатьИтогПоСтроке("Б710", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б710а", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б710б", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б710с", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б711", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б712", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б713", ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б714", ТаблицаДанных);
	...
	РассчитатьИтогПоСтроке("Б719", ТаблицаДанных);
...
Функция ФормаРассчитатьДанныеФормы(ДатаНачалаПериода, ДатаОкончанияПериода) 		
	...
	Запрос.УстановитьПараметр("Счет5500", 				ПланыСчетов.Типовой.Резервы);
...
Функция ФормаРассчитатьСальдо(ДатаНачалаПериода, ДатаОкончанияПериода) 
	...
	МассивСчетовКапитала.Добавить(ПланыСчетов.Типовой.Резервы); 								// Счет5500
	МассивСчетовКапитала.Добавить(ПланыСчетов.Типовой.НераспределеннаяПрибыльНепокрытыйУбыток); // Счет5600
	...
	Запрос.УстановитьПараметр("ДатаОкончания", 			ДатаОкончанияПериода);
	Запрос.УстановитьПараметр("Счет5500", 				ПланыСчетов.Типовой.Резервы);
	Запрос.УстановитьПараметр("Счет5600", 				ПланыСчетов.Типовой.НераспределеннаяПрибыльНепокрытыйУбыток);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ТОГДА 2
	               |			КОГДА ТиповойОстаткиИОбороты.Счет В ИЕРАРХИИ (&Счет5600)
	               |				ТОГДА 3
	               |			ИНАЧЕ 1
	               |		КОНЕЦ КАК Капитал,
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Счет5400 = ПС.ДополнительноОплаченныйКапитал;
	Счет5500 = ПС.Резервы;
	Счет5600 = ПС.НераспределеннаяПрибыльНепокрытыйУбыток;
	...
	МассивСчетов.Добавить(Счет5600);
	...
	Запрос.УстановитьПараметр("Счет5600", Счет5600);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	(ВЫБРАТЬ
	               |		ВЫБОР
				   |			КОГДА ТиповойОбороты.Счет В ИЕРАРХИИ (&Счет5600)
	               |				ТОГДА 7
	               |			КОГДА ТиповойОбороты.Счет В ИЕРАРХИИ (&Счет5500)
	               |				ТОГДА 6
				   ...
				   |			КОГДА ТиповойОбороты.Субконто2 ССЫЛКА Перечисление.ВидыИзмененийВСобственномКапитале
				   |				ТОГДА ТиповойОбороты.Субконто2 " + ?(ЕстьЭлементПеречисленияВМетаданных("Дивиденды"),"
	               ...
	               |		ЕСТЬNULL(ТиповойОбороты.СуммаОборотКт, 0) КАК СуммаОборотКт
	               |	ИЗ
	               //|		РегистрБухгалтерии.Типовой.Обороты(&НачПериод, &КонПериод, , Счет В ИЕРАРХИИ (&МассивСчетов), ЗНАЧЕНИЕ(ПланВИдовХарактеристик.ВидыСубконтоТиповые.ВидыИзмененийВСобственномКапитале), Организация В (&СписокСтруктурныхЕдиниц), , ) КАК ТиповойОбороты) КАК ТаблицаКапитал
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Капитал = 7 Тогда		 
			// по счета 5600 не ведется аналитика по Видам изменения в капитале (исключение составляют дивиденды)
		...
		ИначеЕсли  ЕстьЭлементПеречисленияВМетаданных("ПереоценкаДолговыхФинансовыхИнструментовОцениваемыхПоСправедливойСтоимости")
			И Субконто = Перечисления.ВидыИзмененийВСобственномКапитале.ПереоценкаДолговыхФинансовыхИнструментовОцениваемыхПоСправедливойСтоимости Тогда
			
			ИмяЯчейки = "Б221";
			
		ИначеЕсли  ЕстьЭлементПеречисленияВМетаданных("ПереоценкаДолевыхФинансовыхИнструментовОцениваемыхПоСправедливойСтоимости")
			И Субконто = Перечисления.ВидыИзмененийВСобственномКапитале.ПереоценкаДолевыхФинансовыхИнструментовОцениваемыхПоСправедливойСтоимости Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Капитал = 7 Тогда		
			...
			Иначе	
				...
				// по счета 5600 не ведется аналитика по Видам изменения в капитале
				// помещаем в строкуприбыли убытка за год
				ИмяЯчейки = "Б610"; 				
			КонецЕсли;
		ИначеЕсли  Субконто = Перечисления.ВидыИзмененийВСобственномКапитале.ИзмененияУчетнойПолитики Тогда
	...
	УстановитьЗначениеПоказателя("Б010_6", СНК(Счет5500,РезультатЗапроса) - СНД(Счет5500,РезультатЗапроса), ТаблицаДанных);
	УстановитьЗначениеПоказателя("Б010_7", СНК(Счет5600,РезультатЗапроса) - СНД(Счет5600,РезультатЗапроса), ТаблицаДанных);
	УстановитьЗначениеПоказателя("Б010_8", СНК(Счет5400,РезультатЗапроса) - СНД(Счет5400,РезультатЗапроса), ТаблицаДанных);
	...
	УстановитьЗначениеПоказателя("Б400_6", СКК(Счет5500,РезультатЗапроса) - СКД(Счет5500,РезультатЗапроса), ТаблицаДанных);
	УстановитьЗначениеПоказателя("Б400_7", СКК(Счет5600,РезультатЗапроса) - СКД(Счет5600,РезультатЗапроса), ТаблицаДанных);
	УстановитьЗначениеПоказателя("Б400_8", СКК(Счет5400,РезультатЗапроса) - СКД(Счет5400,РезультатЗапроса), ТаблицаДанных);
	РассчитатьИтогПоСтроке("Б400", ТаблицаДанных);
