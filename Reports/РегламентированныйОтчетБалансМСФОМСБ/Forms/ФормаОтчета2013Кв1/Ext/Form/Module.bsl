...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеАктивы 		= "08";
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеОбязательства 	= "29";
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеРезервы 		= "41";
	
	СтруктураРеквизитовФормы.мКодНачалаРазделаОбязательств 	= 22; // Счета до этого кода выводятся по Дт остатку, после - по Кт остатку.
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""баланс (мсфо мсб)"" (заполнение)";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("мКодСтрокиПрочиеРезервы");	
...
Функция ПолучитьКодСтрокиБаланса(Счет, СчетПредопределенный = Неопределено, СчетВидСчета = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(КодСтроки) Тогда
		...
		ИначеЕсли Счет.ПринадлежитЭлементу(ПланыСчетов.Типовой.Резервы) Тогда
			КодСтроки =  СтруктураРеквизитовФормы.мКодСтрокиПрочиеРезервы; 		// прочие резервы
		Иначе	
...
Процедура УстановитьЗначенияПоказателей(ТаблицаДанных, КодБаланса, СуммаНачальныйОстатокДт, СуммаНачальныйОстатокКт, СуммаКонечныйОстатокДт, СуммаКонечныйОстатокКт)
	...
	КодСтрокиОтчета = "s_" + СокрЛП(КодБаланса); // код строки в макете
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, КодСтрокиОтчета + "_1", СуммаКонечныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,КодСтрокиОтчета + "_2", СуммаНачальныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница1);
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	// 01. Итого краткосрочные активы (сумма строк с 02 по 08)																				
	Для НомерПериода = 1 по 2 Цикл
		...
		КонецЦикла;	
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_01_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	
	// 09. Итого долгосрочные активы (сумма строк с 10 по 20)
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 10 по 20 Цикл
			...
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_09_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	
	// 23. Итого краткосрочные обязательства(сумма строк с 24 по 29)																				
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 24 по 29 Цикл
			НомерСтроки = "s_" + Формат(Счетчик,"ЧЦ=2; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_23_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;		
	// 30. Итого долгосрочные обязательства(сумма строк с 31 по 35)
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 31 по 35 Цикл
			НомерСтроки = "s_" + Формат(Счетчик,"ЧЦ=2; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
	...
	КонецЦикла;		
	// 36. Итого капитал(сумма строк с 37 по 43)
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 37 по 43 Цикл
			НомерСтроки = "s_" + Формат(Счетчик,"ЧЦ=2; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_36_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	
	// 21. Баланс активов (стр.01 + стр.09)								
	Для НомерПериода = 1 по 2 Цикл
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_01_" + НомерПериода)
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_09_" + НомерПериода);
		
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_21_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	
	// 22. Итого обязательства (стр.23 + стр.30)								
	Для НомерПериода = 1 по 2 Цикл
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_23_" + НомерПериода)
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_30_" + НомерПериода);
		
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_22_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	

	// 44. Итого капитал и обязательства (стр.22 + стр.36)								
	Для НомерПериода = 1 по 2 Цикл
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_22_" + НомерПериода)
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_36_" + НомерПериода);
		
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_44_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;	
