...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "ФормаОС112017Кв4";
...
Процедура ФормаОС11ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	|	СУММА(ЗапросДляРасчета.ДоходОтРеализации) КАК ДоходОтРеализации
	...
	//Средняя стоимость основных средств
	ЗначениеСр = (Значение("100", "1",, ФормаОтчетаПолеТабличногоДокументаСтраница1) + Значение("100", "10",, ФормаОтчетаПолеТабличногоДокументаСтраница1)) / 2;
	Значение("074", , ЗначениеСр, ФормаОтчетаПолеТабличногоДокументаСтраница5);
		...
		Если ПустаяСтрока(КодыСтрок) Тогда			
			...
			Если Не Ремонт Тогда 					
				Для Каждого СтрокаОС Из СтрокаТЗ.Строки Цикл
					...
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СтрокаОС.ОС); 					
				КонецЦикла;
			Иначе 
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СтрокаТЗ.Наименование);
			КонецЕсли;				
...
Процедура ЗаполнитьСтрокуЗначений5(СтрокаТЗ) // для раздела 5
	...
	Если СтрокаТЗ <> Неопределено Тогда
		...
		Если ПустаяСтрока(КодыСтрок) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СтрокаТЗ.НМА); 
			Возврат;
		КонецЕсли;
