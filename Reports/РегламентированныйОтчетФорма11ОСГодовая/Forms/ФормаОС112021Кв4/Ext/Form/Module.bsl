...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "ФормаОС112021Кв4";
	...
	Макет = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("КлассификаторОсновныхФондов_2016");
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""форма 11 ос годовая"" (заполнение)";
...
Процедура ФормаОС11ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	Если Перезаполнить Тогда	
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Расчет формы № 11 годовая'"));
	КонецЕсли;	
	...
	|	ПереоценкаВнеоборотныхАктивовОбороты.ВнеоборотныйАктив КАК ВнеоборотныйАктив
	|ПОМЕСТИТЬ ПереоценкаВнеоборотныхАктивовОбороты
	|ИЗ
	|	РегистрНакопления.ПереоценкаВнеоборотныхАктивов.Обороты(&ДатаНач, &ДатаКон, Регистратор, Организация В (&СписокОрганизаций)) КАК ПереоценкаВнеоборотныхАктивовОбороты
	|ГДЕ
	...
	|	0 КАК ПоступилоПрочее,
	|	0 КАК ВыбылоПрочее
	|ПОМЕСТИТЬ ВТ_ЗапросДляРасчета
	|ИЗ
	...
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстаткиИОборотыОС
	...
	|				,
	|				,
	|				Счет В ИЕРАРХИИ (&СписокСчетовАмортизации),
	|				,
	|				Организация В (&СписокОрганизаций)
	|					И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстаткиИОборотыАморт
	...
	|				&ДатаКон,
	|				,
	|				СчетДт В ИЕРАРХИИ (&СписокСчетовАмортизации),
	|				,
	|				СчетКт В ИЕРАРХИИ (&СписокСчетов),
	|				,
	|				Организация В (&СписокОрганизаций)
	|					И СтруктурноеПодразделениеДт В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОборотыДтКтАморт
	|		ПО ТиповойОстаткиИОборотыОС.Субконто1 = ТиповойОборотыДтКтАморт.СубконтоКт1
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТиповойОборотыПеремещениеОС.Субконто1,
	|	ТиповойОборотыПеремещениеОС.Субконто1.КодКОФ,
	|	0,
	|	-ТиповойОборотыПеремещениеОС.СуммаОборотДт,
	|	-ТиповойОборотыПеремещениеОС.СуммаОборотКт,
	|	0,
	|	0,
	|	0,
	|	0,
	...
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)
	|				И СтруктурноеПодразделениеКор В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетов),
	|			) КАК ТиповойОборотыПеремещениеОС
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|				И Субконто1 = КорСубконто1
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)
	|				И НЕ СтруктурноеПодразделениеКор В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетов),
	|			) КАК ТиповойОборотыПеремещениеОС
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТиповойОборотыПеремещениеАморт.Субконто1,
	|	ТиповойОборотыПеремещениеАморт.Субконто1.КодКОФ,
	|	0,
	|	0,
	|	0,
	|	0,
	|	0,
	|	0,
	|	-ТиповойОборотыПеремещениеАморт.СуммаОборотКт,
	|	0,
	|	0,
	|	0,
	|	0
	...
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)
	|				И СтруктурноеПодразделениеКор В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетовАмортизации),
	|			) КАК ТиповойОборотыПеремещениеАморт
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТиповойОбороты.Субконто1,
	|	ТиповойОбороты.Субконто1.КодКОФ,
	|	0,
	...
	|			Организация В (&СписокОрганизаций)
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетовДоходов),
	|			) КАК ТиповойОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|			Организация В (&СписокОрганизаций)
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетовКапитал),
	|			) КАК ТиповойОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|			Организация В (&СписокОрганизаций)
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетовЗадолженностьКтСП),
	|			) КАК ТиповойОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|			Организация В (&СписокОрганизаций)
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц),
	|			КорСчет В ИЕРАРХИИ (&СписокСчетовЗадолженностьДтСП),
	|			) КАК ТиповойОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	СУММА(ЗапросДляРасчета.ОстатокНаНачало) КАК ОстатокНаНачало,
	|	СУММА(ЗапросДляРасчета.Поступило) КАК Поступило,
	|	СУММА(ЗапросДляРасчета.Выбыло) КАК Выбыло,
	|	СУММА(ЗапросДляРасчета.ОстатокНаКонец) КАК ОстатокНаКонец,
	|	СУММА(ЗапросДляРасчета.Дооценка) КАК Дооценка,
	|	СУММА(ЗапросДляРасчета.Уценка) КАК Уценка,
	|	СУММА(ЗапросДляРасчета.Амортизация) КАК Амортизация,
	|	СУММА(ЗапросДляРасчета.АмортизацияНаКонец) КАК АмортизацияНаКонец,
	|	СУММА(ЗапросДляРасчета.АмортизацияСписано) КАК АмортизацияСписано,
	|	СУММА(ЗапросДляРасчета.ПоступилоПрочее) КАК ПоступилоПрочее,
	|	СУММА(ЗапросДляРасчета.ВыбылоПрочее) КАК ВыбылоПрочее
	|ПОМЕСТИТЬ ВТ_ЗапросДляРасчетаСгруппированный
	|ИЗ
	|	ВТ_ЗапросДляРасчета КАК ЗапросДляРасчета
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗапросДляРасчета.ОС,
	|	ЗапросДляРасчета.КодКоф
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ЗапросДляРасчета.ОстатокНаКонец = 0
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Списано,
	|	ЗапросДляРасчета.ВыбылоПрочее + ВЫБОР
	|		КОГДА ЗапросДляРасчета.ОстатокНаКонец <> 0
	|			ТОГДА ЗапросДляРасчета.Выбыло - ВЫБОР
	|					КОГДА ЗапросДляРасчета.Уценка ЕСТЬ NULL
	|						ТОГДА 0
	...
	|	ВЫБОР
	|		КОГДА ЗапросДляРасчета.ОстатокНаНачало = 0
	...
	|	ЗапросДляРасчета.ПоступилоПрочее + ВЫБОР
	|		КОГДА ЗапросДляРасчета.ОстатокНаНачало <> 0
	|			ТОГДА ЗапросДляРасчета.Поступило - ВЫБОР
	|					КОГДА ЗапросДляРасчета.Дооценка ЕСТЬ NULL
	|						ТОГДА 0
	...
	|ПОМЕСТИТЬ ВТ_ЗапросДляГруппировки
	|ИЗ
	|	ВТ_ЗапросДляРасчетаСгруппированный КАК ЗапросДляРасчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ЗапросДляГруппировки.ОС КАК ОС
	|ИЗ
	|	ВТ_ЗапросДляГруппировки КАК ЗапросДляГруппировки
	|ИТОГИ
	...
	Для Каждого СтруктурнаяЕдиница Из мСписокСтруктурныхЕдиниц Цикл
		Если ТипЗнч(СтруктурнаяЕдиница.Значение) = Тип("СправочникСсылка.Организации") Тогда
			СписокОрганизаций.Добавить(СтруктурнаяЕдиница.Значение);
			Если СписокСтруктурныхЕдиниц.НайтиПоЗначению(ПустоеПодразделение) = Неопределено Тогда
				СписокСтруктурныхЕдиниц.Добавить(ПустоеПодразделение);
			КонецЕсли;
		КонецЕсли;
		Если ТипЗнч(СтруктурнаяЕдиница.Значение) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
			СписокСтруктурныхЕдиниц.Добавить(СтруктурнаяЕдиница.Значение);
		КонецЕсли;
	КонецЦикла;
	Если СписокОрганизаций.Количество() = 0 Тогда
		СписокОрганизаций.Добавить(Налогоплательщик);
	КонецЕсли;
	...
	СписокСчетовКапитал = Новый СписокЗначений;
	СписокСчетовКапитал.Добавить(ПланыСчетов.Типовой.УставныйКапитал);  // 5000
	Запрос.УстановитьПараметр("СписокСчетовКапитал", СписокСчетовКапитал);
	
	СписокСчетовЗадолженностьДтСП = Новый СписокЗначений;
	СписокСчетовЗадолженностьДтСП.Добавить(ПланыСчетов.Типовой.КраткосрочнаяДебиторскаяЗадолженностьФилиаловИСтруктурныхПодразделений);  // 1240
	Запрос.УстановитьПараметр("СписокСчетовЗадолженностьДтСП", СписокСчетовЗадолженностьДтСП);
	
	СписокСчетовЗадолженностьКтСП = Новый СписокЗначений;
	СписокСчетовЗадолженностьКтСП.Добавить(ПланыСчетов.Типовой.КраткосрочнаяКредиторскаяЗадолженностьФилиаламИСтруктурнымПодразделениям);  // 3340
	Запрос.УстановитьПараметр("СписокСчетовЗадолженностьКтСП", СписокСчетовЗадолженностьКтСП);
	...
	|			&ДатаЗапроса,
	|			Счет = &СчетНезавершенноеСтроительство,
	|			,
	|			Организация В (&СписокОрганизаций)
	|				И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстатки");
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |				И СтруктурноеПодразделение В ИЕРАРХИИ (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстаткиОС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Типовой.Остатки(
	               |				&КонПер,
	               |				Счет В ИЕРАРХИИ (&СписокСчетовАмортизации),
	               |				,
	               |				Организация В (&СписокОрганизаций)
	               |					И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстаткиАмортизацияОС
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Типовой.Остатки(&КонПер, Счет В ИЕРАРХИИ (&Счет), , Организация В (&СписокОрганизаций) И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстатки
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаРезультатов.Следующий() Цикл 
			Сумма = Сумма + ВыборкаРезультатов.Сумма;		
		КонецЦикла;
		Значение("_52", "1", Сумма, ФормаОтчетаПолеТабличногоДокументаСтраница5, 5);
	КонецЕсли;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаРезультатов.Следующий() Цикл 
			Сумма = Сумма + ВыборкаРезультатов.Сумма;		
		КонецЦикла;
		Значение("_52", "11", Сумма, ФормаОтчетаПолеТабличногоДокументаСтраница5, 5);
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Типовой.Обороты(&НачПер, &КонПер, , Счет = &Счет, , Организация В (&СписокОрганизаций) И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц), , ) КАК ТиповойОбороты";
	...
	//Запрос.УстановитьПараметр("СписокСчетов", СписокСчетов);
	//Запрос.УстановитьПараметр("СписокОрганизаций", СписокСтруктурныхЕдиниц);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Типовой.ОборотыДтКт(&НачПер, &КонПер, , СчетДт В ИЕРАРХИИ (&СчетДт), , СчетКт В ИЕРАРХИИ (&СчетКт), , Организация В (&СписокОрганизаций) И СтруктурноеПодразделениеДт В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОборотыДтКт";
	Запрос.УстановитьПараметр("НачПер",                  мДатаНачалаПериодаОтчета);
	Запрос.УстановитьПараметр("КонПер",                  Новый Граница(КонецДня(мДатаКонцаПериодаОтчета), ВидГраницы.Включая));
	...
	|			Счет = &СчетРемонтов,
	|			,
	|			Организация В (&СписокОрганизаций) И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)
	...
	ЗаполнитьНМА(СписокОрганизаций, СписокСтруктурныхЕдиниц);  	
	Расчет1();
	
	//Средняя стоимость основных средств
	ЗначениеСр = (Значение("1000", "1",, ФормаОтчетаПолеТабличногоДокументаСтраница1) + Значение("1000", "10",, ФормаОтчетаПолеТабличногоДокументаСтраница1)) / 2;
	Значение("074", , ЗначениеСр, ФормаОтчетаПолеТабличногоДокументаСтраница5);
...
//
Процедура ЗаполнитьНМА(СписокОрганизаций, СписокСтруктурныхЕдиниц) // для раздела 5
	...
	|					,
	|					Организация В (&СписокОрганизаций) И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстаткиИОбороты
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|			0
	|		ИЗ
	|			РегистрБухгалтерии.Типовой.Обороты(&ДатаНач, &ДатаКон, , Счет = &СчетАмортизацияПрочиеНМА, , Организация В (&СписокОрганизаций) И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц), , ) КАК ТиповойОбороты
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|		ИЗ
	|			РегистрБухгалтерии.Типовой.Остатки(&ДатаКон, Счет = &СчетАмортизацияПрочиеНМА, , Организация В (&СписокОрганизаций) И СтруктурноеПодразделение В (&СписокСтруктурныхЕдиниц)) КАК ТиповойОстатки) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
...
Процедура Расчет1() // для раздела 1
	...
	РасчетИтоговСтрок("39", "327,332",,, 1);
	РасчетИтоговСтрок("39", "327,332",,, 2);
	
	//строка 15 = сумме строк 16, 21, 39 для каждой графы;
	РасчетИтоговСтрок("3", "31,32,39",,, 1);
	РасчетИтоговСтрок("3", "31,32,39",,, 2);
	...
	//Итоги по столбцам. графа 10 = сумме граф 1, 2, 3, 4 – сумма граф 5, 7, 8 для каждой строки;
	РасчетИтоговСтолбцов("10", "1,2,3,4,-5,-7,-8", "0,1,2,31,32,71,321,3211,40,5,7", 3);	
	
	//строка 4 = сумме строк 5, 6, 7 для каждой графы;
	РасчетИтоговСтрок("3", "31,32,71",,, 3);
	РасчетИтоговСтрок("3", "31,32,71",,, 4);
