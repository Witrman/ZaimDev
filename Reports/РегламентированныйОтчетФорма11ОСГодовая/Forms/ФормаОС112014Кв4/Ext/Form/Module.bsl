...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "ФормаОС112014Кв4";
	...
	Макет = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("КлассификаторОсновныхФондов_2014");
...
Процедура ФормаОС11ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	|ГДЕ
	|	НЕ ПереоценкаВнеоборотныхАктивовОбороты.Регистратор ССЫЛКА Документ.ЗакрытиеМесяца
	|
	|СГРУППИРОВАТЬ ПО
	|	ПереоценкаВнеоборотныхАктивовОбороты.ВнеоборотныйАктив
	|;
