...
Процедура СформироватьОтчет(Команда)
	...
	УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""списки сотрудников организаций"" (формирование)");
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ТекстОрганизация = БухгалтерскиеОтчетыКлиентСервер.ВыгрузитьСписокВСтроку(Форма.СписокСтруктурныхЕдиниц);
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("ДополнительныеПоля") И ЗаполняемыеНастройки.ДополнительныеПоля Тогда
		...
		НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
		НоваяСтрока.Поле           = "Сотрудник.КИ_14";
		НоваяСтрока.Представление  = НСтр("ru = 'Адрес электронной почты'");
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("КонецПериода"                 , Отчет.Период);
...
Функция ПолучитьПараметрыВыбораЗначенияОтбора() Экспорт
	...
	СписокПараметров.Вставить("Дата"       , Отчет.Период);
...
Процедура ОткрытьФормуВариантыОтчета()
	...
	ОткрытьФорму("Отчет.СпискиРаботниковОрганизаций.ФормаОбъекта", ПараметрыФормы, ВладелецФормы);
