...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма4002022Кв1";
	КодФормы 		= "Форма400"; 		
	...
	мВыбраннаяФорма = "Форма4002022Кв1"; // могла восстановиться из данных сохраненной ранее формы
...
Процедура Форма400_02ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф400_02_") > 0) Тогда
		...
		ИначеЕсли (Найти(Ячейка, "Ф400_02_002__") > 0) ИЛИ (Найти(Ячейка, "Ф400_02_003__") > 0) 
			...
			ИЛИ (Найти(Ячейка, "Ф400_02_008__") > 0) ИЛИ (Найти(Ячейка, "Ф400_02_009__") > 0) ИЛИ (Найти(Ячейка, "Ф400_02_010__") > 0) Тогда
...
Процедура Форма400_03ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф400_03_") > 0) Тогда
		...
		ИначеЕсли (Найти(Ячейка, "Ф400_03_001__") > 0) ИЛИ (Найти(Ячейка, "Ф400_03_002__") > 0) 
			...
			ИЛИ (Найти(Ячейка, "Ф400_03_009__") > 0) ИЛИ (Найти(Ячейка, "Ф400_03_010__") > 0)
			ИЛИ (Найти(Ячейка, "Ф400_03_011_1__") > 0) ИЛИ (Найти(Ячейка, "Ф400_03_011_2__") > 0) Тогда
			ПараметрыСтраницы.Вставить("Расчет", Истина);		
			
			ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
			ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Введите налоговую базу'"), 12, 0) ;
			
		ИначеЕсли Найти(Ячейка, "Ф400_03_011_A_") > 0 Тогда	
			ПараметрыСтраницы.Вставить("Расчет", Истина);		

			ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
			ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Количество марок'"), 9, 0) ;
						
		ИначеЕсли Найти(Ячейка, "Ф400_03_011_B_") > 0 Тогда
			
			ПараметрыСтраницы.Вставить("Расчет", Истина);		

			ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
			ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Количество штук, кг в пачке'"), 5, 3) ;
		...
		ИначеЕсли (Найти(Ячейка, "Ф400_03_015__") > 0) Тогда
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЧНОГО ПОЛЯ   Форма400_05ПолеТабличногоДокументаСтраница4

&НаКлиенте
Процедура Форма400_05ПолеТабличногоДокументаСтраница4ПриИзмененииСодержимогоОбласти(Элемент, Область)
...
&НаКлиенте
Процедура Форма400_05ПолеТабличногоДокументаСтраница4Выбор(Элемент, Область, СтандартнаяОбработка)

	Ячейка = Область.Имя;	
	
	ЯчейкаКБК = Ложь;
	Для Индекс = 1 По 12 Цикл
		Если (Найти(Ячейка, "s_400_05_010_" + Индекс + "_a") > 0) Тогда
			ЯчейкаКБК = Истина;
			Прервать;
		КонецЕсли; 		
	КонецЦикла; 
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""%1 декларация по акцизу"" (заполнение)";
		КлючеваяОперация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(КлючеваяОперация,?(ЗначениеЗаполнено(КодФормы), НРег(КодФормы), "форма 400")); 
...
Процедура ЗаписатьЗамер(ВремяНачала)
	...
	Если ВремяНачала <> Неопределено 
		...
		КлючеваяОперация = "Регламентированный отчет ""форма 400 декларация по акцизу"" (заполнение)";
...
Процедура ВыборРасчета(ТекФорма, Индекс = Неопределено, Показатель = Неопределено) Экспорт
	...
	ИначеЕсли ТекФорма = "Форма400_06" Тогда		
        Форма400_06РасчетНаСервере(Индекс, Показатель, ТекФорма);
...
Процедура ЗаполнитьАвтоЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли КодФормы = "Форма400_06" Тогда 		
		Форма400_06ЗаполнитьАвто(Перезаполнить);
...
Процедура Форма400РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы)
	...
	//Рассчитаем ячейку 400.00.010
	//s_400_00_010 = сумма с s_400_00_001 по s_400_00_009
	ИтогЗначение = 0;
	Для Н = 1 По 9 Цикл
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,ТекИмяФормы,"s_400_00_010", ИтогЗначение, Истина);
			                            	
	//Рассчитаем ячейку 400.00.012
	// s_400_00_012 = s_400_00_010 - s_400_00_011
	s_400_00_012 = ИтогЗначение
	             - РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_400_00_011");
				 
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,ТекИмяФормы,"s_400_00_012", s_400_00_012, Истина);
			
	//s_400_00_015
	//Рассчитаем ячейку 400.00.015
	//s_400_00_015 = сумма с s_400_00_015_1 по s_400_00_015_15
	ИтогЗначение = 0;
	Для Н = 1 По 15 Цикл
		ИмяГрафы = "s_400_00_015_" + Н;
		ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
	КонецЦикла;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,ТекИмяФормы,"s_400_00_015", ИтогЗначение, Истина);
				...
				Если ИмяГруппы = "ЛистA" Тогда
					...
					Пока ВыборкаПоВидамОпераций.Следующий() Цикл
						...
						Иначе
							...
							ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КодСтр));						
...
Функция Форма400_02РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	СписокКодовСтрок.Добавить("400.02.010");
	Запрос.УстановитьПараметр("СписокКодовСтрок", СписокКодовСтрок);
...
Процедура Форма400_02ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	Если ТаблицаОтчета.Количество() <> 0 Тогда
		...
		Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
			...
			ТаблицаСтрокГруппы = ЭлементСтруктуры.Значение.Строки;
			ОбщиеЭлементыГруппы = ЭлементСтруктуры.Значение.ШапкаИПодвал;                    				
			ОбластьФ400_02_014 = ОбщиеЭлементыГруппы.Найти("Ф400_02_014", "ИмяОбласти"); 
			ТаблицаСтрокГруппы.Очистить();
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				Для Каждого СтрокаОтчета Из ТаблицаОтчета Цикл
					...
					Если СтрокаОтчета.Выводить Тогда
						Для Н = 2 По 10 Цикл
							СтрН = Формат(Н, "ЧЦ=2; ЧВН=");
							ИмяГрафы = "400.02.0" + СтрН;
							//Помещаем данные в форму
							Если КодСтр = ИмяГрафы Тогда
								Элемент["Ф400_02_0" + СтрН] = Окр(?(СтрокаОтчета.Количество = NULL , 0, СтрокаОтчета.Количество),2);
				...
				Элемент.Ф400_02_012 = ИтоговоеКоличество;
				Элемент.Ф400_02_014 = Окр(ИтоговаяСуммаАкциза);
				Если Не ИтоговоеКоличество = 0 Тогда
					...
					Если Ставка >=1000 ИЛИ Ставка <=-1000 Тогда
						...
						ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Ставка));												
						Ставка = 999.99;
					КонецЕсли;
					Элемент.Ф400_02_013 = Ставка;
				КонецЕсли;		
		...
		//заполняем данные для передачи итогов в "Шапку и подвал"
		Если  Не ОбластьФ400_02_014 = Неопределено Тогда
			ОбластьФ400_02_014.Значение = ТаблицаСтрокГруппы.Итог("Ф400_02_014");			
		КонецЕсли; 
...
Процедура Форма400_02РасчетНаСервере(Инд,Колонка, ТекИмяФормы)
	...
	A1 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_A1 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_A1);
	B1 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_B1 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_B1);
	
	A2 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_A2 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_A2);
	B2 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_B2 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_B2);
	
	A3 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_A3 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_A3);
	B3 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_B3 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_B3);
	
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_C1 = Окр(A1 * B1,2);
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_C2 = Окр(A2 * B2,2);
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_011_C3 = Окр(A3 * B3,2);
	
	Форма400_02ПолеТабличногоДокументаЛистA.Области["Ф400_02_011_C1__"+ Строка(Инд)].Значение = Окр(A1 * B1,2);
	Форма400_02ПолеТабличногоДокументаЛистA.Области["Ф400_02_011_C2__"+ Строка(Инд)].Значение = Окр(A2 * B2,2);
	Форма400_02ПолеТабличногоДокументаЛистA.Области["Ф400_02_011_C3__"+ Строка(Инд)].Значение = Окр(A3 * B3,2);
	...
	//рассчитаем ячейку 400.02.012
	ИтогЗначение =  0;
	Для Н = 2 По 11 Цикл
		ИмяГрафы = "Ф400_02_0" + Формат(Н, "ЧЦ=2; ЧВН=");
		ИтогЗначение = ИтогЗначение + ?(ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы] = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы]);
	КонецЦикла;
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_012 = ИтогЗначение;
	Форма400_02ПолеТабличногоДокументаЛистA.Области["Ф400_02_012__"+ Строка(Инд)].Значение = ИтогЗначение;
	
	//рассчитаем ячейку 400.02.014 и подсчитаем итог для данной строки
	Знач012 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_012 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_012);
	Знач013 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_013 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_013);
	Знач014 = Окр(Знач012 * Знач013);
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_02_014 = Знач014;
	Форма400_02ПолеТабличногоДокументаЛистA.Области["Ф400_02_014__"+ Строка(Инд)].Значение = Знач014;
...
Процедура Форма400_03ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	Пока ВыборкаПоВидамТМЗ.Следующий() Цикл
		Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
			...
			ТаблицаСтрокГруппы = ЭлементСтруктуры.Значение.Строки;
			ОбщиеЭлементыГруппы = ЭлементСтруктуры.Значение.ШапкаИПодвал;                    				
			ОбластьФ400_03_015 = ОбщиеЭлементыГруппы.Найти("Ф400_03_015", "ИмяОбласти");
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				Элемент.Ф400_03_015 = Окр(ИтоговаяСуммаАкциза);
	...
	//заполняем данные для передачи итогов в "Шапку и подвал"
	Если Не ОбластьФ400_03_015 = Неопределено Тогда
		ОбластьФ400_03_015.Значение = ТаблицаСтрокГруппы.Итог("Ф400_03_015");			
	КонецЕсли; 
...
Процедура Форма400_03РасчетНаСервере(Инд, Колонка, ТекИмяФормы)
	...
	Знач011_a_1 	= ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_011_a_1 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_011_a_1);
	Знач011_b_1	= ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_011_b_1 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_011_b_1);
	Знач011_c_1	= Окр(Знач011_a_1 * Знач011_b_1);
	
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_011_1 = Знач011_c_1;
	Форма400_03ПолеТабличногоДокументаЛистA.Области["Ф400_03_011_1__"+ Строка(Инд)].Значение = Знач011_c_1;
	...
	Для Н = 2 По 10 Цикл
		...
		ИтогЗначение = ИтогЗначение + ?(ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы] = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы]);				 
	КонецЦикла;
	ИтогЗначение = ИтогЗначение + ?(ТаблицаСтрокГруппы.Получить(Индекс)["Ф400_03_011_1"] = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс)["Ф400_03_011_1"]);
	...
	//рассчитаем ячейку 400.04.015
	Знач014 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_014 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_014);
	
	Знач015 = Знач014;
		
	ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_015 = Знач015;
	Форма400_03ПолеТабличногоДокументаЛистA.Области["Ф400_03_015__"+ Строка(Инд)].Значение = Знач015;	
...
Функция Форма400_05РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	Запрос.УстановитьПараметр("ЭкоТопливо", 			Перечисления.ВидыПодакцизныхТМЗ.НефрасЭкоТопливоСмеси);
	...
	СписокКодовСтрокЭкоТопливо = Новый СписокЗначений;
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.007.I");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.007.II");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.007.III");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.007.IV");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.007.V");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.I");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.II");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.III");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.IV");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.V");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.VI");
	СписокКодовСтрокЭкоТопливо.Добавить("400.05.008.VII");
	Запрос.УстановитьПараметр("СписокКодовСтрокЭкоТопливо", СписокКодовСтрокЭкоТопливо);
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |			ТОГДА ИСТИНА
	               |		КОГДА ОсновнаяТаблица.ВидТМЗ = &ЭкоТопливо
	               |				И ОсновнаяТаблица.КодСтроки В (&СписокКодовСтрокЭкоТопливо)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ КАК Выводить,
	               |	ВЫБОР
	               ...
	               |							ИЛИ ТМЗ.ВидПодакцизногоТМЗ = &ДизТопливо
	               |							ИЛИ ТМЗ.ВидПодакцизногоТМЗ = &ЭкоТопливо)) КАК АкцизОбороты
	               |				ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |					КодыСтрокДекларацииПоАкцизуСрезПоследних.ВидТМЗ КАК ВидТМЗ,
	               |					КодыСтрокДекларацииПоАкцизуСрезПоследних.ВидОперации КАК ВидОперации,
	               |					КодыСтрокДекларацииПоАкцизуСрезПоследних.КодСтроки КАК КодСтроки
...
Процедура Форма400_05ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)                        
	...
	Для Каждого СтрокаОтчета Из ТаблицаОтчета Цикл
		...
		Если СтрокаОтчета.Выводить Тогда
			...
			Если Вид = "БЕНЗИН" Тогда
				Номер1 = 1;
				Номер2 = 2;
			ИначеЕсли Вид = "ДИЗТОПЛИВО" Тогда
				Номер1 = 4;
				Номер2 = 5;
			Иначе
				Номер1 = 7;
	...
	// Выводим общую сумму акциза для КБК в строке 400.05.010
	Если ЗначениеЗаполнено(ЗначСуммаАкциза) Тогда
		Если Не ПустаяСтрока(КБК) Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_010_1_a", КБК);  			
		КонецЕсли;
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_010_1_b", ЗначСуммаАкциза);  	
...
Процедура Форма400_05РасчетНаСервере(Изменено = Истина,ТекИмяФормы)
	...
	//Расчитываем ячейку s_400_05_001, s_400_05_004, s_400_05_007 (Оптовая реализация)
	ИтогГрафыА001 = 0;
	ИтогГрафыС001 = 0;
	ИтогГрафыА004 = 0;
	ИтогГрафыС004 = 0;
	...
	Для Н = 1 По 5 Цикл
		...
		Если Изменено Тогда
			// ячейки 400.05.007
			ЗначГрафыA007 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_007_" + Н + "_a");
			ЗначГрафыB007 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_007_" + Н + "_b");
			ЗначГрафыC007 = Окр(ЗначГрафыA007 * ЗначГрафыB007);			
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_007_" + Н + "_c", ЗначГрафыC007,Форма400_05ПолеТабличногоДокументаСтраница3);									
		КонецЕсли;												
		ИтогГрафыА007 = ИтогГрафыА007 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_007_" + Н + "_a");
		ИтогГрафыС007 = ИтогГрафыС007 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_007_" + Н + "_c");
	КонецЦикла;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_007_a", ИтогГрафыА007, Форма400_05ПолеТабличногоДокументаСтраница3);				
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_007_c", ИтогГрафыС007, Форма400_05ПолеТабличногоДокументаСтраница3);				
		
	//Расчитываем ячейку s_400_05_002, s_400_05_005, s_400_05_008 (Розничная реализация)
	ИтогГрафыА002 = 0;
	ИтогГрафыС002 = 0;
	ИтогГрафыА005 = 0;
	ИтогГрафыС005 = 0;
	...
	Для Н = 1 По 7 Цикл
		...
		Если Изменено Тогда
			// ячейки 400.05.008
			ЗначГрафыA008 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_008_" + Н + "_a");
			ЗначГрафыB008 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_008_" + Н + "_b");
			ЗначГрафыC008 = Окр(ЗначГрафыA008 * ЗначГрафыB008);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_008_" + Н + "_c", ЗначГрафыC008,Форма400_05ПолеТабличногоДокументаСтраница3);													
		КонецЕсли;
		ИтогГрафыА008 = ИтогГрафыА008 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_008_" + Н + "_a");
		ИтогГрафыС008 = ИтогГрафыС008 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_008_" + Н + "_c");
	КонецЦикла;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_008_a", ИтогГрафыА008, Форма400_05ПолеТабличногоДокументаСтраница3);										
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_008_c", ИтогГрафыС008, Форма400_05ПолеТабличногоДокументаСтраница3);											
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_006_a", Знач400_05_006_a, Форма400_05ПолеТабличногоДокументаСтраница2);										
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_006_c", Знач400_05_006_c, Форма400_05ПолеТабличногоДокументаСтраница2);	
			
	//Расчитываем ячейку s_400_05_009
	Знач400_05_009_a = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_007_a")
					 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_008_a");
	Знач400_05_009_c = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_007_c")
					 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_400_05_008_c");
					 
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_009_a", Знач400_05_009_a, Форма400_05ПолеТабличногоДокументаСтраница3);										
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_009_c", Знач400_05_009_c, Форма400_05ПолеТабличногоДокументаСтраница3);	
...
Процедура Форма400_06РасчетНаСервере(Инд,Колонка,ТекИмяФормы)
	...
	ТаблицаСтрокГруппы = СтруктураГруппы.Строки;
	ОбщиеЭлементыГруппы = СтруктураГруппы.ШапкаИПодвал;
	ОбластьФ400_06_E = ОбщиеЭлементыГруппы.Найти("Ф400_06_E", "ИмяОбласти"); 

	// Рассчитаем сумму в зависимости от ставки и объема
	Попытка
		ИмяГрафы 	= Лев(Колонка,7); 
		Объем  		= ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы + "_C"]; 
		Ставка  	= ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы + "_D"]; 
		
		СуммаВычета = Окр(Объем * Ставка);
		ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы + "_E"] = СуммаВычета;
		Форма400_06ПолеТабличногоДокументаЛистA.Области[ИмяГрафы + "_E__"+ Строка(Инд)].Значение = СуммаВычета;
	Исключение		
	КонецПопытки;
	
	КоличествоСтрок = ТаблицаСтрокГруппы.Количество();
	
	ИтогЗначение =  0;
	Для Н = 1 По КоличествоСтрок Цикл
		ИмяГрафы = "Ф400_06_E";  
		ИтогЗначение = ИтогЗначение + ?(Форма400_06ПолеТабличногоДокументаЛистA.Области[ИмяГрафы + "__" + Строка(Н)] = Неопределено, 0, Форма400_06ПолеТабличногоДокументаЛистA.Области[ИмяГрафы + "__" + Строка(Н)].Значение);
	КонецЦикла; 

	Форма400_06ПолеТабличногоДокументаЛистA.Области["Ф400_06_E"].Значение = ИтогЗначение;
	
	//заполняем данные для передачи итогов в "Шапку и подвал"
	Если  Не ОбластьФ400_06_E = Неопределено Тогда
		ОбластьФ400_06_E.Значение = ТаблицаСтрокГруппы.Итог("Ф400_06_E");			
	КонецЕсли; 
...
&НаКлиенте
Процедура Форма400_06ЗаполнитьАвто(Перезаполнить = Ложь)   
	
	Состояние("Расчет формы 400.06");		
	Форма400_06ЗаполнитьАвтоНаСервере(Перезаполнить);	
	Состояние("");
...
Процедура Форма400_07РасчетНаСервере(Изменено = Ложь, ТекИмяФормы)
	...
	Если Изменено Тогда
		Форма400_07ПолеТабличногоДокументаСтраница1.Области["s_400_07_004_2"].Значение = ИтогЗначение;		
	КонецЕсли;	
