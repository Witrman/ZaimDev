...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма4002014Кв1";
	КодФормы 		= "Форма400"; 		
...
Процедура Форма400_01ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф400_01_") > 0) Тогда
		...
		Если (Найти(Ячейка, "Ф400_01_001_A__")> 0) Тогда		
			...
			ОткрытьФорму("Справочник.Номенклатура.Форма.ФормаВыбора",,ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца) ;
		...
		Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				Если Не ИтоговоеКоличество = 0 Тогда
					Ставка = Окр(Окр(ИтоговаяСуммаАкциза)/ИтоговоеКоличество,3);
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				Пока ВыборкаПоВидамОпераций.Следующий() Цикл
					...
					Если ВыборкаПоВидамОпераций.Выводить И СписокГрафФормы.НайтиПоЗначению(КодСтр)<> Неопределено Тогда   
						...
						Если Найти(КодСтр, "400.03.008.B") = 0 Тогда
...
Процедура Форма400_03РасчетНаСервере(Инд,Колонка, ТекИмяФормы)
	...
	Для Н = 2 По 11 Цикл
		...
		ИтогЗначение = ИтогЗначение + ?(ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы] = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс)[ИмяГрафы]);				 
	КонецЦикла;
	// 400.03.012 Итого ((сумма строк с 400.03.002 по 400.03.011)-400.03.008В))
	// отнимаем строку 008B
	ИтогЗначение = ИтогЗначение - Знач008_b;
	...
	Знач009 = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_009 = Неопределено, 0, ТаблицаСтрокГруппы.Получить(Индекс).Ф400_03_009);
	
	Знач014 = Окр(Знач012 * Знач013) - Знач009;
	...
	Для Каждого СтрокаОтчета Из ТаблицаОтчета Цикл
		...
		Если СтрокаОтчета.Выводить Тогда
			...
			ИначеЕсли КодСтр = "400.05.00" + Номер2 +".I" Тогда
				...
				РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_00" + Номер1 +"_6_a", ЗначКолВо);
				РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_00" + Номер1 +"_6_b", ЗначСтавка);
				РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_400_05_00" + Номер1 +"_6_c", ЗначСумма);
...
Процедура Форма400_07РасчетНаСервере(Изменено = Ложь, ТекИмяФормы)
	...
	Для Н = 1 По 2 Цикл
		...
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_400_07_001" + ?(Н=1,"_1","_2") + "_" + ИмяЯчейки;
			s_400_07_001 = s_400_07_001 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы); 
		КонецЦикла;
		
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_400_07_001" + ?(Н=1,"_1","_2"), s_400_07_001, Ложь);
		Если Изменено Тогда
			Форма400_07ПолеТабличногоДокументаСтраница1.Области["s_400_07_001" + ?(Н=1,"_1","_2")].Значение = s_400_07_001;		
		КонецЕсли;	
	КонецЦикла;
