...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2002020Кв1";
	КодФормы 		= "Форма200";
...
Процедура Форма200_05ПолеТабличногоДокументаСтраница1Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(Ячейка, "Ф200_05ФИОГПХ__")> 0 ИЛИ Найти(Ячейка, "Ф200_05ИИНГПХ__")> 0 Тогда
		НачЗначение = Область.Значение;
		...
		//может быть не заполнена расшировка ячейки при восстановления данных старых форм
		Если Не ЗначениеЗаполнено(ЗначениеРасшифровки) Тогда
			ИмяЯчейкиРасшифровки = "Ф200_05ФИОГПХ" + Сред(Ячейка,Найти(Ячейка,"__"));
		...
		ОповещениеВыбора = Новый ОписаниеОповещения("ПослеВыбораЭлемента", ЭтаФорма, ПараметрыСтраницы); 		 
		СписокВыбора.ПоказатьВыборЭлемента(ОповещениеВыбора, "Выберите расшифровку", СписокВыбора[0]);				 
	ИначеЕсли (Найти(Ячейка, "Ф200_05_D__") > 0) Тогда
		НачЗначение = Область.Значение;
...
Процедура Форма200_05ПолеТабличногоДокументаСтраница2Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф200_05_E__") > 0) 
	 ИЛИ (Найти(Ячейка, "Ф200_05_F__") > 0) 
	 ИЛИ (Найти(Ячейка, "Ф200_05_G__") > 0) 
	 ИЛИ (Найти(Ячейка, "Ф200_05_H__") > 0) Тогда
		НачЗначение = Область.Значение;
		Если (Найти(Ячейка, "Ф200_05_E__") > 0) Тогда 
		...
		ИначеЕсли (Найти(Ячейка, "Ф200_05_F__") > 0) Тогда			
			...
			ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
			ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Введите сумму налоговых стандартных вычетов'"), 12,0) ;
				
		ИначеЕсли (Найти(Ячейка, "Ф200_05_G__") > 0) Тогда
			
			ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
			ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Введите сумму ИПН, к начислению'"), 12,0) ;
			
		ИначеЕсли (Найти(Ячейка, "Ф200_05_H__") > 0) Тогда
	...
	Иначе
		...
		Если   СписокСохранения.Свойство("СтруктураДополнительныхРеквизитовФормы") Тогда
			...
			Если ТаблицаСтрокГруппыФорма200_05Страница1.Колонки.Найти("Ф200_05ИНН")<> Неопределено И ТаблицаСтрокГруппыФорма200_05Страница1.Колонки.Найти("Ф200_05ФИО")<> Неопределено Тогда
				ТаблицаСтрокГруппыФорма200_05Страница1.Колонки["Ф200_05ИНН"].Имя = "Ф200_05ИИНГПХ";
				ТаблицаСтрокГруппыФорма200_05Страница1.Колонки["Ф200_05ИНН_Расшифровка"].Имя = "Ф200_05ИИНГПХ_Расшифровка";
				ТаблицаСтрокГруппыФорма200_05Страница1.Колонки["Ф200_05ФИО"].Имя = "Ф200_05ФИОГПХ";
				ТаблицаСтрокГруппыФорма200_05Страница1.Колонки["Ф200_05ФИО_Расшифровка"].Имя = "Ф200_05ФИОГПХ_Расшифровка";
...
Процедура ВыгрузитьПриложение(ИмяФайла, Многострочность = Ложь, ВыгружатьВсеПриложения)
	...
	мВыбраннаяФорма = "Форма2002019Пр3";
...
Процедура Форма200Расшифровка(ИмяЯчейки)
	...
	Если ПолучитьЗначениеРеквизита(Налогоплательщик, "ЮрФизЛицо") = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо") Тогда
		...
		ИначеЕсли Лев(ИмяЯчейки,12) <> "s_200_00_007" 
				И Лев(ИмяЯчейки,12) <> "s_200_00_011" Тогда	 
			СтруктураПоФизЛицу = Новый Структура;
			СтруктураПоФизЛицу.Вставить("ФизЛицо", ПолучитьЗначениеРеквизита(Налогоплательщик, "ИндивидуальныйПредприниматель"));
...
Процедура Форма200_01Расшифровка(ИмяЯчейки)
	...
	// Если в строках по самому ИП, то установим отбор по физ лицу	
	Если ПолучитьЗначениеРеквизита(Налогоплательщик, "ЮрФизЛицо") = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо") Тогда
		Если Лев(ИмяЯчейки,12) <> "s_200_01_009" И Лев(ИмяЯчейки,12) <> "s_200_01_015" И Лев(ИмяЯчейки,12) <> "s_200_01_017" Тогда
			СтруктураПоФизЛицу = Новый Структура;
			СтруктураПоФизЛицу.Вставить("ФизЛицо", ПолучитьЗначениеРеквизита(Налогоплательщик, "ИндивидуальныйПредприниматель"));
			СтруктураПоФизЛицу.Вставить("ВидСравнения", ВидСравненияКомпоновкиДанных.НеРавно);	
		ИначеЕсли Лев(ИмяЯчейки,12) <> "s_200_01_017" Тогда
...
Процедура Форма200_02РасчетНаСервере(Инд,Колонка)
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтаФорма,"Страница7", "Ф200_02_Y", ТаблицаСтрокГруппы.Итог("Ф200_02_Y"), Ложь); 
	Модифицированность = Истина;
...
Функция Форма200_05РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	|	НАЧАЛОПЕРИОДА(ИПНСведенияОДоходах.Период, МЕСЯЦ) КАК МесяцНалоговогоПериода,
	|	СУММА(ИПНСведенияОДоходах.СуммаДоходаОборот) КАК НачисленныеДоходы,
	|	СУММА(ИПНСведенияОДоходах.СуммаВычетаОборот) КАК НеПодлежащиеНалогооблажению,
	|	0 КАК НалоговыеВычеты,
	|	0 КАК ИПНКНачислению,
	|	0 КАК ОПВ
	|ПОМЕСТИТЬ вт_Данные
	|ИЗ
	|	РегистрНакопления.ИПНСведенияОДоходах.Обороты(
	...
	|						ТОГДА НЕ ФизическоеЛицо.ИндивидуальныйПредпринимательАдвокатЧастныйНотариус
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК СведенияОбИсчисленииВычетовИПНОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(СведенияОбИсчисленииВычетовИПНОбороты.Период, МЕСЯЦ),
	|	СведенияОбИсчисленииВычетовИПНОбороты.ФизическоеЛицо
	...
	|	ВЫБОР
	|		КОГДА ФИОФизЛиц.ФизЛицо ЕСТЬ NULL
	|			ТОГДА Контрагенты.ФизЛицо.Наименование
	|		ИНАЧЕ ФИОФизЛиц.Фамилия + ВЫБОР
	|				КОГДА ФИОФизЛиц.Имя <> """"
	|					ТОГДА "" "" + ФИОФизЛиц.Имя
	|				ИНАЧЕ """"
	...
	|	ВЫРАЗИТЬ (СУММА(вт_Данные.ИПНКНачислению) КАК ЧИСЛО(19, 0)) КАК ИПНКНачислению,
	|	ВЫРАЗИТЬ (СУММА(вт_Данные.ОПВ) КАК ЧИСЛО(19, 0)) КАК ОПВ,
	|	Контрагенты.Ссылка КАК ФизЛицо
	|ИЗ
	|	вт_Данные КАК вт_Данные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО вт_Данные.ФизЛицо = Контрагенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&парамПериодПо, ) КАК ФИОФизЛиц
	|		ПО (Контрагенты.ФизЛицо = ФИОФизЛиц.ФизЛицо)
	|
	|СГРУППИРОВАТЬ ПО
	|	Контрагенты.Ссылка,
	...
	Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
		...
		Пока Выборка.Следующий() Цикл  			
			...
			Если ИмяГруппы = "Страница1" Тогда
				...
				Элемент.Ф200_05ФИОГПХ = Выборка.ФИОФизЛица;
				
				Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Выборка.ИНН) Тогда  					
					 СтрокаИИН = Выборка.ИНН;
				Иначе
					 СтрокаИИН = "";
				...
				Элемент.Ф200_05ИИНГПХ = СтрокаИИН;
				Элемент.Ф200_05_D  = Выборка.НачисленныеДоходы;
				РегламентированнаяОтчетность.ДобавитьРасшифровкуСтрокиМногострочнойФормы(ТаблицаСтрокГруппы,Элемент, "Ф200_05ФИОГПХ",        Выборка.ФизЛицо);
				РегламентированнаяОтчетность.ДобавитьРасшифровкуСтрокиМногострочнойФормы(ТаблицаСтрокГруппы,Элемент, "Ф200_05ИИНГПХ",        Выборка.ФизЛицо);
			 
			ИначеЕсли ИмяГруппы = "Страница2" Тогда
				
				Элемент.Ф200_05_A2 = Формат(НомерСтроки, мФорматНомераСтроки);
				Элемент.Ф200_05_E  = Выборка.НеПодлежащиеНалогооблажению;
				Элемент.Ф200_05_F  = Выборка.НалоговыеВычеты;
				Элемент.Ф200_05_G  = Выборка.ИПНКНачислению;
				Элемент.Ф200_05_H  = Выборка.ОПВ;
...
Процедура Форма200_05ОбработкаПодбораНаСервере(Расшифровка, ИмяЯчейки, ИмяГруппы, НомерСтроки) Экспорт   
	...
	Если ИмяЯчейки = "Ф200_05ФИОГПХ" Тогда
		СсылкаФизЛица = Расшифровка;
		
		ОбластьНаименование = "Ф200_05ФИОГПХ__" + НомерСтроки;		
		ОбластьИИН = "Ф200_05ИИНГПХ__" + НомерСтроки;
...
Процедура Форма200_05Расшифровка(ИмяЯчейки)
	...
	Если Найти(ИмяЯчейки,"__") > 0 Тогда		
		...
		ОбластьРасшифровки = Форма200_05ПолеТабличногоДокументаСтраница1.Области.Найти("Ф200_05ФИОГПХ__" +  НомерЯчейки);		
		Если ОбластьРасшифровки <> Неопределено Тогда
			Если ЗначениеЗаполнено(ОбластьРасшифровки.Расшифровка) Тогда
				СтруктураПоФизЛицу.Вставить("ФизЛицо", ОбластьРасшифровки.Расшифровка);
				СтруктураПоФизЛицу.Вставить("ВидСравнения", ВидСравненияКомпоновкиДанных.Равно);					
