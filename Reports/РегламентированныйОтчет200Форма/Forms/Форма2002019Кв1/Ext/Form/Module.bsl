...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2002019Кв1";
	КодФормы 		= "Форма200";
...
Процедура ПоказатьРасшифровкуВВидеРегистраНУпоИПНиСН(ДатаНачала, ДатаОкончания, СтруктурнаяЕдиницаДляОтбора = Неопределено, СтруктураФизЛицоДляОтбора = Неопределено, Режим = "БезДетализации", ПризнакНеРезиденства = Ложь, Приложение200_02 = Ложь, Приложение200_05 = Ложь)
	...
	Если Приложение200_02 Тогда 
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, "ИндивидуальныйПредпринимательАдвокатЧастныйНотариус", ПризнакИП, ВидСравненияКомпоновкиДанных.Равно);
	Иначе
