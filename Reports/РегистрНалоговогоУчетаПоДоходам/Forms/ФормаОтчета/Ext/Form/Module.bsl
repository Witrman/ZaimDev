...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Отчет.РежимРасшифровки Тогда
		...
		Иначе
			Отчет.ВидОтчета = "НалоговыйРегистр";
		КонецЕсли;
		Если ЭтаФорма.Параметры.ПараметрНомерПриложения = "Приложение20" Тогда
			Отчет.НомерПриложения = "Приложение20";
		Иначе
			Отчет.НомерПриложения = "Приложение12";
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьОтчет(Команда)
	...
	//УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""отчет типовой (бух.)"" (формирование)"); 
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.НомерПриложения.Видимость = Отчет.ВидОтчета = "НалоговыйРегистр";
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = ?(Отчет.ВидОтчета = "НалоговыйРегистр", НСтр("ru = 'Налоговый регистр по учету доходов'"),
	НСтр("ru = 'Отчет по доходам'"));
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	//ДеревоЗнач = РеквизитФормыВЗначение("ДеревоСтруктурныхЕдиниц");
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("НомерПриложения"             			 , Отчет.НомерПриложения);
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	//Если Не ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПолучитьСвойстваСчета(Отчет.Счет).УчетПоПодразделениям Тогда
	//	СписокПолей.Добавить("Подразделение");
	//КонецЕсли;
	
	Если Режим = "Группировка" Тогда
		СписокПолей.Добавить("СуммаДохода");
		СписокПолей.Добавить("СуммаКорректировки");
		СписокПолей.Добавить("ВидДохода");
...
Процедура ИнициализацияКомпоновщикаНастроек(ПользовательскиеНастройки = Неопределено)
	...
	Если ЭтаФорма.Параметры.Свойство("ПараметрНомерПриложения")И ЭтаФорма.Параметры.ПараметрНомерПриложения <> ""  Тогда
		Отчет.НомерПриложения = ЭтаФорма.Параметры.ПараметрНомерПриложения;
	КонецЕсли;
	
	ВыводитьЗаголовок = Отчет.ВидОтчета = "НалоговыйРегистр";
	ВыводитьПодписи = Отчет.ВидОтчета = "НалоговыйРегистр";
	
	Схема = Отчеты[БухгалтерскиеОтчетыКлиентСервер.ПолучитьИдентификаторОбъекта(ЭтаФорма)].ПолучитьМакет("МакетРегистраДоходов");
