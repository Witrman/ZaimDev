...
Процедура ПриВыводеЗаголовка(ПараметрыОтчета,Результат) Экспорт
	...
	Областьзаголовок.Параметры.НомерПриложения = ?(ПараметрыОтчета.НомерПриложения = "Приложение12", "12", "20");
	...
	ОбластьЗаголовокТаблицы = Макет.ПолучитьОбласть("ЗаголовокТаблицы_Общий");
	ОбластьЗаголовокТаблицы.Параметры.ЗаголовокТаблицы = ПолучитьТекстЗаголовкаТаблицы(ПараметрыОтчета);
...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт
	...
	Если ПараметрыОтчета.НомерПриложения = "Приложение20" Тогда
		ЗаголовокОтчета = НСтр("ru = 'Налоговый регистр'") + Символы.ПС + 
			НСтр("ru = 'для индивидуальных предпринимателей, применяющих специальный налоговый'") + Символы.ПС + 
			НСтр("ru = 'режим с использованием фиксированного вычета, по учету доходов'");
	Иначе
		ЗаголовокОтчета = НСтр("ru = 'Налоговый регистр'") + Символы.ПС + 
			НСтр("ru = 'по учету доходов, в том числе полученных путем безналичных расчетов'");
	КонецЕсли;
...
Функция ПолучитьТекстЗаголовкаТаблицы(ПараметрыОтчета) Экспорт
	Если ПараметрыОтчета.НомерПриложения = "Приложение20" Тогда
		ЗаголовокОтчета = НСтр("ru = 'Учет операций по доходам индивидуальных предпринимателей, применяющих'") + Символы.ПС + 
			НСтр("ru = 'специальный налоговый режим с использованием фиксированного вычета'");
	Иначе
		ЗаголовокОтчета = НСтр("ru = 'Учет операций по доходам, за исключением операций по'") + Символы.ПС + 
			НСтр("ru = 'реализации или передаче в уставный капитал имущества, являющегося'") + Символы.ПС + 
			НСтр("ru = 'основным средством индивидуального предпринимателя'");
	КонецЕсли;
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Для Каждого ПолеВыбраннойГруппировки Из ПараметрыОтчета.Группировка Цикл 
		Если ПолеВыбраннойГруппировки.Использование Тогда
			...
			Если ПараметрыОтчета.ВидОтчета = "НалоговыйРегистр" и ПолеВыбраннойГруппировки.Поле = "Период" Тогда
				ПолеВыбора = Структура.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
				ПолеВыбора.Заголовок = "№ п/п";
				ПолеВыбора.Поле = Новый ПолеКомпоновкиДанных("СистемныеПоля.НомерПоПорядку");;
			КонецЕсли;
	...
	//Для произвольного отчета включаем детальные записи
	Если ПараметрыОтчета.РежимРасшифровки ИЛИ ПараметрыОтчета.ВидОтчета = "ПроизвольныйОтчет" Тогда
		Структура = Структура.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
		ПолеГруппировки = Структура.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
		ПолеГруппировки.Использование  = Истина;
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	ДополнительныеСвойства.Вставить("НомерПриложения",   ДанныеОтчета.Объект.НомерПриложения);
	...
	СписокПунктовМеню.Добавить("РегистрНалоговогоУчетаПоДоходам", НСтр("ru = 'Регистр налогового учета по доходам'"));
	ИДРасшифровки = "РегистрНалоговогоУчетаПоДоходам";
