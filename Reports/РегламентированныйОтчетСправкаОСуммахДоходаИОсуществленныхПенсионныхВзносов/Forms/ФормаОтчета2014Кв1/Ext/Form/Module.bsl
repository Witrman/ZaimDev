...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "ФормаОтчета2014Кв1";
...
Процедура ВывестиВДокумент(СписокСохранения)
	...
	Макет = Отчет.ПолучитьМакет("МакетФормаОтчета2014Кв1");
...
Процедура СохранитьДанныеНаСервере()
	...
	СписокСохранения.Вставить("ВерсияФормы", "01/10/2014");
...
Функция ФормаРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь)
    ...
    Если РасчетНаРегистрах Тогда
		...
		| 		И ОсновныеНачисления.ВидРасчета.ОблагаетсяОПВ <> ЗНАЧЕНИЕ(Справочник.СпособыНалогообложенияДоходов.НеОблагаетсяЦеликом)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		И ДополнительныеНачисления.ВидРасчета.ОблагаетсяОПВ <> ЗНАЧЕНИЕ(Справочник.СпособыНалогообложенияДоходов.НеОблагаетсяЦеликом)
		|";
		...
		Если Выборка.Период <> ТекущийПериод Тогда
			...
			Если НомерСтроки > 0 Тогда
				...
				Если Выборка.Год <> ТекущийГод Тогда
					...
					СписокСохранения.Вставить("ИтогоПериод_"         + СтрГод, НСтр("ru = 'Итого за '") + СтрГод +  НСтр("ru = ' г.'"));
