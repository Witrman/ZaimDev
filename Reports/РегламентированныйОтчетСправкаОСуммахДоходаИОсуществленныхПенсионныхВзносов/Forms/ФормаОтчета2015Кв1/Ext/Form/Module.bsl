...
Процедура ВывестиВДокумент(СписокСохранения)
	...
	Макет = Отчет.ПолучитьМакет("МакетФормаОтчета2015Кв1");
...
Процедура СохранитьДанныеНаСервере()
	...
	СписокСохранения.Вставить("ВерсияФормы", "01/10/2015");
...
Функция ФормаРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь)
	...
	|ПОМЕСТИТЬ ВТ_ППИ
	|ИЗ
	| 	РегистрНакопления.ОПВРасчетыСФондами.Обороты(&парамПериодС, &парамПериодПо, Регистратор, ФизЛицо = &парамФизЛицо) КАК ОПВРасчетыСФондамиОбороты
