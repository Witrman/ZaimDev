...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИсключатьВнутренниеОбороты       = Ложь;
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(Настройки)
	...
	СохраняемыеРеквизитыФормы.Добавить("ИсключатьВнутренниеОбороты");
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	ЗагружаемыеРеквизитыФормы.Добавить("ИсключатьВнутренниеОбороты");
...
&НаКлиенте
Процедура ИсключатьВнутренниеОборотыПриИзменении(Элемент)
...
Процедура СформироватьОтчет(Команда)
	...
	УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""ведомость амортизации ос за период (бух.)"" (формирование)");
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Показатели") И ЗаполняемыеНастройки.Показатели Тогда
		...
		СписокПоказателей.Добавить("ПервоначальнаяСтоимость",           НСтр("ru = 'Первоначальная cтоимость'"));
		СписокПоказателей.Добавить("СтоимостьДляВычисленияАмортизации", НСтр("ru = 'Стоимость для вычисления амортизации'"));
		СписокПоказателей.Добавить("СтоимостьНачальныйОстаток",         НСтр("ru = 'Стоимость на начало периода'"));
		СписокПоказателей.Добавить("АмортизацияНачальныйОстаток",       НСтр("ru = 'Амортизация на начало периода'"));
		СписокПоказателей.Добавить("ОстаточнаяСтоимостьНач",            НСтр("ru = 'Остаточная стоимость на начало периода'"));
		СписокПоказателей.Добавить("СтоимостьУвеличение",               НСтр("ru = 'Увеличение стоимости'"));
		СписокПоказателей.Добавить("АмортизацияНачисление",             НСтр("ru = 'Амортизация начисление'"));
		СписокПоказателей.Добавить("АмортизацияСписание",               НСтр("ru = 'Амортизация списание'"));
		СписокПоказателей.Добавить("СтоимостьУменьшение",               НСтр("ru = 'Уменьшение стоимости'"));
		СписокПоказателей.Добавить("СтоимостьКонечныйОстаток",          НСтр("ru = 'Стоимость на конец периода'"));
		СписокПоказателей.Добавить("АмортизацияКонечныйОстаток",        НСтр("ru = 'Амортизация на конец периода'"));
		СписокПоказателей.Добавить("ОстаточнаяСтоимость",               НСтр("ru = 'Остаточная стоимость'"));
		СписокПоказателей.Добавить("ЛиквидационнаяСтоимость",           НСтр("ru = 'Ликвидационная стоимость'"));
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка") И ЗаполняемыеНастройки.Группировка Тогда
		...
		Если ВестиУчетОСВРазрезеПодразделений Тогда
			...
			НоваяСтрока.Представление  = НСтр("ru = 'Подразделение (БУ)'");
		...
		Если ВестиУчетОСВРазрезеМОЛ Тогда
			...
			НоваяСтрока.Использование  = Истина;
			НоваяСтрока.Представление  = НСтр("ru = 'МОЛ (БУ)'");
			НоваяСтрока.ТипГруппировки = Перечисления.ТипДетализацииСтандартныхОтчетов.Элементы;
		КонецЕсли;
		
		Если НЕ ВестиУчетОСВРазрезеПодразделений И НЕ ВестиУчетОСВРазрезеМОЛ Тогда
			НоваяСтрока = Отчет.Группировка.Добавить();
			НоваяСтрока.Поле           = "МестонахождениеКон";
			НоваяСтрока.Использование  = Истина;
			НоваяСтрока.Представление  = НСтр("ru = 'Подразделение (упр., кон. периода)'");
			НоваяСтрока.ТипГруппировки = Перечисления.ТипДетализацииСтандартныхОтчетов.Элементы;
			
			НоваяСтрока = Отчет.Группировка.Добавить();
			НоваяСтрока.Поле           = "МОЛКон";
			НоваяСтрока.Использование  = Истина;
			НоваяСтрока.Представление  = НСтр("ru = 'МОЛ (упр., кон. периода)'");
			НоваяСтрока.ТипГруппировки = Перечисления.ТипДетализацииСтандартныхОтчетов.Элементы;
		КонецЕсли;
		...
		НоваяСтрока.Использование  = Истина;
		НоваяСтрока.Представление  = НСтр("ru = 'Основное средство'");
		НоваяСтрока.ТипГруппировки = Перечисления.ТипДетализацииСтандартныхОтчетов.Элементы;
	...
	Если ЗаполняемыеНастройки.Свойство("ДополнительныеПоля") И ЗаполняемыеНастройки.ДополнительныеПоля Тогда
		...
		НоваяСтрока.Представление  = НСтр("ru = 'Дата принятия к учету'");
		НоваяСтрока.Использование  = Ложь;
		
		НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
		НоваяСтрока.Поле           = "ОсновноеСредство.ИнвентарныйНомер";
		НоваяСтрока.Представление  = НСтр("ru = 'Инвентарный номер'");
		НоваяСтрока.Использование  = Ложь;
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ВестиУчетОСВРазрезеМОЛ"                , ВестиУчетОСВРазрезеМОЛ);
	ПараметрыОтчета.Вставить("ВестиУчетОСВРазрезеПодразделений"      , ВестиУчетОСВРазрезеПодразделений);
	ПараметрыОтчета.Вставить("ИсключатьВнутренниеОбороты"            , ИсключатьВнутренниеОбороты);
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	Если Режим = "Выбор" ИЛИ Режим = "Группировка" ИЛИ Режим = "Отбор" Тогда
		...
		КонецЦикла;
		СписокПолей.Добавить("ОбъемПродукцииРаботДляВычисленияАмортизации");
		СписокПолей.Добавить("СтоимостьКонечныйОстаток");
		СписокПолей.Добавить("Выработка");
	КонецЕсли;
...
Процедура ИнициализироватьОтчет()
	...
	ПоляНабораДанных = СКД.НаборыДанных.ОсновнойНаборДанных.Поля;
...
Процедура ПроверитьНастройки()
	...
	НайденныеПоказатели = Отчет.Показатели.НайтиСтроки(Новый Структура("Поле", "ОстаточнаяСтоимостьНач"));
	Если НайденныеПоказатели.Количество() = 0 Тогда
		ДобавленныйПоказатель               = Отчет.Показатели.Вставить(4);
		ДобавленныйПоказатель.Поле          = "ОстаточнаяСтоимостьНач";
		ДобавленныйПоказатель.Представление = НСтр("ru = 'Остаточная стоимость на начало периода'");
		ДобавленныйПоказатель.Использование = Истина;
	КонецЕсли;
