...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма9102021Кв1";
	КодФормы 		= "Форма910";
...
Процедура Форма910Расшифровка(НомерЯчейки, МесяцЯчейки, ИмяЯчейки)
	...
	Иначе	
		...
		Если НомерЯчейки = 17 ИЛИ НомерЯчейки = 18 Тогда
			// граждане РК		
		...
		ИначеЕсли НомерЯчейки = 19 Тогда
			// иностранцы			
			ОтборПоСтране				= ПредопределенноеЗначение("Справочник.КлассификаторСтранМира.Казахстан");
...
//
Процедура ПоказатьРасшифровкуВВидеРегистраНУпоИПНиСН(ДатаНачала, ДатаОкончания, СтруктурнаяЕдиницаДляОтбора = Неопределено, СтруктураФизЛицоДляОтбора = Неопределено, Режим = "БезДетализации", ПризнакНеРезиденства = Ложь, Приложение910_01 = Ложь, Приложение910_02 = Ложь)
...
Процедура ЗаполнитьПанель(ПризнакОсновнойФормы, ПризнакМногострочности)
	...
	Элементы.ФормаРасшифровка.Видимость  = (КодФормы <> "Форма910_01" И КодФормы <> "Форма910_02");	
	Элементы.Расшифровка910_01.Видимость = (КодФормы = "Форма910_01" ИЛИ КодФормы = "Форма910_02");	
...
Процедура Форма910_02ПолеТабличногоДокументаСтраница3Выбор(Элемент, Область, СтандартнаяОбработка)
    ...
    Если Найти(Ячейка, "Ф910_02КодДохРезидента__")> 0 Тогда
        ...
        ПараметрыФормы.Вставить("РежимВыбора"		  , Истина);
        ПараметрыФормы.Вставить("ИмяМакета"			  , "КодыВидов");
        ПараметрыФормы.Вставить("ИмяСекции"			  ,	"КодыВидовДоходов2021");
        ПараметрыФормы.Вставить("ПолучатьПолныеДанные", Истина);
        ПараметрыФормы.Вставить("ИмяОтчета",          "РегламентированныйОтчет910Форма");
...
Процедура Расшифровка(Команда)
	...
	ИначеЕсли НомерЯчейки = 2 или НомерЯчейки = 3 или 
		...
		НомерЯчейки = 16 или ИмяЯчейки = "s_910_00_001_A1" или ИмяЯчейки = "s_910_00_001_B1" Тогда
...
Функция Форма910РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
    ...
    Если Метаданные.РегистрыБухгалтерии.Найти("Типовой") <> Неопределено Тогда
		...
		СтруктураРезультата.Вставить("s_910_00_001_B", 0);   
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли Выборка.КодСтроки = "910.00.001.В" ИЛИ Выборка.КодСтроки = "910.00.001.B" Тогда 
				СтруктураРезультата.s_910_00_001_B = СтруктураРезультата.s_910_00_001_B + (Выборка.СуммаОборотКт - Выборка.СуммаОборотДт);
			Иначе
				СтруктураРезультата.s_910_00_001_B = СтруктураРезультата.s_910_00_001_B + (Выборка.СуммаОборотКт - Выборка.СуммаОборотДт);
		...
		СтруктураРезультата.s_910_00_001 = СтруктураРезультата.s_910_00_001_A + СтруктураРезультата.s_910_00_001_B;
	...
	|			   КОНЕЦ)
	|			 И ВТ_Гражданство.Страна = ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.Казахстан)
	...
	СтруктураРезультата.Вставить("s_910_00_017_A", 0);
	СтруктураРезультата.Вставить("s_910_00_017_B", 0);
	СтруктураРезультата.Вставить("s_910_00_017_C", 0);
	СтруктураРезультата.Вставить("s_910_00_017_D", 0);
	СтруктураРезультата.Вставить("s_910_00_017_E", 0);
	НачисленныеДоходы = 0;
	Номер = 1;
	Пока Выборка.Следующий() Цикл
		НачисленныеДоходы = НачисленныеДоходы + Окр(Выборка.НачисленныеДоходы);
		СтруктураРезультата.Вставить("s_910_00_017_" + Номер, Окр(Выборка.НачисленныеДоходы));    
		Номер = Номер + 1;
		...
		Пока ВыборкаКодСтроки.Следующий() Цикл
			...
			Если (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "A") > 0)
				...
				СтруктураРезультата.s_910_00_017_A = СтруктураРезультата.s_910_00_017_A + ВыборкаКодСтроки.НачисленныеДоходы;

			ИначеЕсли (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "B") > 0)
				Или (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "В") > 0) Тогда
				
				СтруктураРезультата.s_910_00_017_B = СтруктураРезультата.s_910_00_017_B + ВыборкаКодСтроки.НачисленныеДоходы;

			ИначеЕсли (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "С") > 0)
				ИЛИ (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "C") > 0) Тогда
				
				СтруктураРезультата.s_910_00_017_C = СтруктураРезультата.s_910_00_017_C + ВыборкаКодСтроки.НачисленныеДоходы;

			ИначеЕсли (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "D") > 0) Тогда
				
				СтруктураРезультата.s_910_00_017_D = СтруктураРезультата.s_910_00_017_D + ВыборкаКодСтроки.НачисленныеДоходы;

			ИначеЕсли (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "Е") > 0)
				ИЛИ (Найти(СокрЛП(ВыборкаКодСтроки.КодСтроки), "E") > 0) Тогда
				
				СтруктураРезультата.s_910_00_017_E = СтруктураРезультата.s_910_00_017_E + ВыборкаКодСтроки.НачисленныеДоходы;
	...
	СтруктураРезультата.Вставить("s_910_00_017_7", НачисленныеДоходы);
	СтруктураРезультата.s_910_00_017_A = Окр(СтруктураРезультата.s_910_00_017_A);
	СтруктураРезультата.s_910_00_017_B = Окр(СтруктураРезультата.s_910_00_017_B);
	СтруктураРезультата.s_910_00_017_C = Окр(СтруктураРезультата.s_910_00_017_C);
	СтруктураРезультата.s_910_00_017_D = Окр(СтруктураРезультата.s_910_00_017_D);
	СтруктураРезультата.s_910_00_017_E = Окр(СтруктураРезультата.s_910_00_017_E);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("s_910_00_018_" + Номер, Окр(Выборка.СуммаИПНПодлежащегоПеречислениюГражданРК));    
		СтруктураРезультата.Вставить("s_910_00_019_" + Номер, Окр(Выборка.СуммаИПНПодлежащегоПеречислениюИностранцевРК));
	...
	Номер = 1;
	Пока Выборка.Следующий() Цикл		
		СтруктураРезультата.Вставить("s_910_00_020_" + Номер, Окр(Выборка.ОблагаемаяБазаСО));    
		ОблагаемаяБазаСО = ОблагаемаяБазаСО + Окр(Выборка.ОблагаемаяБазаСО);
			
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_020_7", ОблагаемаяБазаСО);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("s_910_00_022_" + Номер, Окр(Выборка.ОблагаемаяБазаОПВ));    
					
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_022_7", ОблагаемаяБазаОПВ);
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаОПВПодлежащихПеречислению = СуммаОПВПодлежащихПеречислению + Окр(Выборка.СуммаОПВПодлежащихПеречислению);
		СтруктураРезультата.Вставить("s_910_00_023_" + Номер, Окр(Выборка.СуммаОПВПодлежащихПеречислению));    
							
		Номер = Номер + 1;
	КонецЦикла;
	
	// сумма ОПВ, подлежащих перечислению
	СтруктураРезультата.Вставить("s_910_00_023_7", СуммаОПВПодлежащихПеречислению);
	...
	Пока Выборка.Следующий() Цикл	
		...
		СтруктураРезультата.Вставить("s_910_00_024_" + Номер, Окр(Выборка.ОблагаемаяБазаОППВ));    
						
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_024_7", ОблагаемаяБазаОППВ);			
	...
	Пока Выборка.Следующий() Цикл	
		...
		СтруктураРезультата.Вставить("s_910_00_025_" + Номер, Окр(Выборка.ОППВ));    
						
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_025_7", СуммаОППВПодлежащихПеречислению);			
	...
	|							ТОГДА НЕ ФизЛицо.ИндивидуальныйПредпринимательАдвокатЧастныйНотариус
	|						ИНАЧЕ ИСТИНА
	|					КОНЕЦ) КАК ВОСМССведенияОДоходахОбороты
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("s_910_00_026_" + Номер, Окр(Выборка.СуммаОСМС));    
		
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_026_7", ОблагаемаяБазаООСМС);		
	...
	СтруктураРезультата.Вставить("s_910_00_027_1", 0);
	СтруктураРезультата.Вставить("s_910_00_027_2", 0);
	СтруктураРезультата.Вставить("s_910_00_027_3", 0);
	СтруктураРезультата.Вставить("s_910_00_027_4", 0);
	СтруктураРезультата.Вставить("s_910_00_027_5", 0);
	СтруктураРезультата.Вставить("s_910_00_027_6", 0);
	Номер = 1;
	Пока Выборка.Следующий() Цикл	
		СуммаООСМСПодлежащихПеречислению = СуммаООСМСПодлежащихПеречислению + Окр(Выборка.Сумма);
		СтруктураРезультата.Вставить("s_910_00_027_" + Номер, Окр(Выборка.Сумма));    				
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_027_7", СуммаООСМСПодлежащихПеречислению);
...
Процедура Форма910Расчет(Изменено = Ложь, ИмяОбласти = Неопределено)
	...
	s_910_00_001_A = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_001_A");
	s_910_00_001_B = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_001_B");
	...
	s_910_00_008 = ?(НеРассчитыватьСуммыКУплате, 0, Окр(s_910_00_007 * 0.5));
	...
	s_910_00_011 =  Макс(s_910_00_008 - s_910_00_010, 0);
	...
	Если СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП  Тогда
		Если НЕ СтруктураРеквизитовФормы.мЕжемесячныйРасчетВзносовИОтчисленийЗаИП Тогда
			...
			s_910_00_013_7 = Окр(s_910_00_013_1) + Окр(s_910_00_013_2) + Окр(s_910_00_013_3) + Окр(s_910_00_013_4) + Окр(s_910_00_013_5) + Окр(s_910_00_013_6);
			
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_013_1", Окр(s_910_00_013_1));
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_013_2", Окр(s_910_00_013_2));
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_013_3", Окр(s_910_00_013_3));
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_013_4", Окр(s_910_00_013_4));
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_013_5", Окр(s_910_00_013_5));
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_013_6", Окр(s_910_00_013_6));
	...
	Если СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП Тогда
		Если НЕ СтруктураРеквизитовФормы.мЕжемесячныйРасчетВзносовИОтчисленийЗаИП Тогда
			...
			Если СтруктураРеквизитовФормы.мИППенсионер Тогда
				...
				Для Мес = 1 По 6 Цикл
					...
					КонецЕсли;
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_016_" + Мес, ВОСМСЗаМесяц);
				КонецЦикла;
				РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_016_7", ВОСМСЗаПериод);
			Иначе
		...
		Иначе
			...
			ОбъектИсчисленияВОСМС = s_910_00_016_7 / СтавкаВОСМС * 100;
	...
	Если НеРассчитыватьСуммыКУплате Тогда
		s_910_00_009 = 0;
	Иначе
		...
		s_910_00_009 = Окр(Макс(s_910_00_007 * 0.5 - Окр(s_910_00_013_7) - Окр(s_910_00_021_7), 0));
	КонецЕсли;
	...
	s_910_00_027_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_027_1");
	s_910_00_027_2 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_027_2");
	s_910_00_027_3 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_027_3");
	s_910_00_027_4 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_027_4");
	s_910_00_027_5 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_027_5");
	s_910_00_027_6 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_027_6");
	
	s_910_00_027_7 = s_910_00_027_1 + s_910_00_027_2 + s_910_00_027_3 + s_910_00_027_4 + s_910_00_027_5 + s_910_00_027_6;
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_027_7", s_910_00_027_7);
...
Процедура ПоказатьРасшифроку910АнализСубконто(ИмяЯчейки)	
	...
	ИначеЕсли ИмяЯчейки = "s_910_00_001_B" Тогда
	...
	Если ИмяЯчейки = "Ф910_02ФИОРезидента" Тогда
		...
		Если ТипЗнч(Расшифровка) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				КодСтраныГражданства = ?(Выборка.Страна = Справочники.КлассификаторСтранМира.Казахстан, "", Выборка.КодСтраныГражданства);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ИмяГруппы = "Страница1" Тогда
					...
					Если Выборка.Категория5 <> "" Тогда
					    Категория.Добавить(Выборка.Категория5);
					КонецЕсли;
					Если Выборка.Категория1 <> "" ИЛИ Выборка.Категория3 <> "" ИЛИ Выборка.Категория6 <> "" Тогда
						Категория.Добавить("7");
	                КонецЕсли;
	                Если Выборка.Категория2 <> "" Тогда
	                    Категория.Добавить(Выборка.Категория2);
	                КонецЕсли;
            ...
            Пока ВыборкаПоСтруктурному.Следующий() Цикл                   	
                ...
                Пока Выборка.Следующий() Цикл
                    ...
                    Если ИмяГруппы = "Страница1" Тогда
                        ...
                        Элемент.Ф910_02КодСтраныГражданства = Выборка.КодСтраныГражданства;
...
Процедура Форма910_02ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
    ...
    Если ВРег(ИсточникВыбора.ИмяФормы) = ?(СтрНайти(ЭтаФорма.ИмяФормы, "ВнешнийОтчет.") > 0, ВРег("ВнешнийОтчет.РегламентированныйОтчет910Форма.Форма.ФормаВыбораИзКлассификатора"), 
        ВРег("Отчет.РегламентированныйОтчет910Форма.Форма.ФормаВыбораИзКлассификатора")) Тогда
        Если ВРег(ИсточникВыбора.ИмяСекции) = ВРег("КодыВидовДоходов2021")  
