...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма9102015Кв1";
	КодФормы 		= "Форма910";
...
Процедура Форма910Расчет(Изменено = Ложь, ИмяОбласти = Неопределено)
	...
	Если СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП Тогда
		// для ИП предельная сумма дохода с 2007 года - 10 млн. - пп.1) п.2 ст. 376 НК РК
		// с 2010 года - Статья 433
		ПредельныйДоход = 1400 *СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения ;
	Иначе
		// для ЮЛ предельная сумма дохода с 2007 года - 25 млн. - пп.2) п.2 ст. 376 НК РК
		// с 2010 года - Статья 433
		ПредельныйДоход = 2800 *СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения;
	КонецЕсли;
	
	ДоходВПределахНорматива = Мин(s_910_00_001, ПредельныйДоход);
	...
	Если СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП Тогда
		// Статья 433. Для ИП предельная численность с 2007 года - 25 человек, включая самого ИП		
		ЧисленностьВПределахНорматива = Мин(s_910_00_003, 25);
	Иначе
		// Статья 433. Для ЮЛ предельная численность с 2007 года - 50 человек		
		ЧисленностьВПределахНорматива = Мин(s_910_00_003, 50);
	КонецЕсли;	
	...
	Если (СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП и s_910_00_004 >= 2 * СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения)
		...
		s_910_00_006 =  ДоходВПределахНорматива * 0.03 * ЧисленностьВПределахНорматива * 0.015;
	КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП Тогда
		...
		Если s_910_00_012 > 0 Тогда
			...
			s_910_00_013 = Окр(Мин(s_910_00_012, 75 * СтруктураРеквизитовФормы.мРазмерМЗП * ?(НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.мКоличествоМесяцев), 6, СтруктураРеквизитовФормы.мКоличествоМесяцев)) * СтруктураРеквизитовФормы.мСтавкаОПВ / 100);
		КонецЕсли;
	КонецЕсли;
