...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма9102018Кв1";
	КодФормы 		= "Форма910";
...
&НаКлиенте
Процедура Форма910Расшифровка(НомерЯчейки, МесяцЯчейки)
	...
	Иначе	
		...
		ИначеЕсли НомерЯчейки = 16 Тогда
			// иностранцы			
			ОтборПоСтране				= ПредопределенноеЗначение("Справочник.КлассификаторСтранМира.Казахстан");
...
Процедура Расшифровка(Команда)
	...
	ИначеЕсли НомерЯчейки = 2 или НомерЯчейки = 3 или 
		...
		НомерЯчейки = 12 или НомерЯчейки = 13 или НомерЯчейки = 14 Тогда
		...
		Возврат;	
	КонецЕсли;	
		Форма910Расшифровка(НомерЯчейки, МесяцЯчейки);
...
Процедура ВосстановитьСохраненныеДанные()	
	...
	Если НЕ СтруктураРеквизитовФормы.Свойство("мСтавкаВОСМС") Тогда
		СтруктураРеквизитовФормы.Вставить("мСтавкаВОСМС", ПроцедурыНалоговогоУчета.ПолучитьСтавкуОСМС(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "Взносы", Перечисления.ЮрФизЛицо.ЮрЛицо));
	КонецЕсли; 
...
Функция Форма910РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
 ...
 //   Запрос.УстановитьПараметр("НачПериода", 			СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("s_910_00_015_" + Номер, Окр(Выборка.СуммаИПНПодлежащегоПеречислениюГражданРК));    
		СтруктураРезультата.Вставить("s_910_00_016_" + Номер, Окр(Выборка.СуммаИПНПодлежащегоПеречислениюИностранцевРК));
	...
	Если СтруктураРеквизитовФормы.мЕжемесячныйРасчетВзносовИОтчисленийЗаИП Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда
			...
			СтруктураРезультата.Вставить("s_910_00_010_7", СоциальныеОтчисления);
		КонецЕсли;
		...
		Если СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				СтруктураРезультата.Вставить("s_910_00_012_" + Номер, Окр(Выборка.ОблагаемаяБазаОПВ));    
				
				Номер = Номер + 1;	
			КонецЦикла;
			СтруктураРезультата.Вставить("s_910_00_012_7", ОблагаемаяБазаОПВ);
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				СтруктураРезультата.Вставить("s_910_00_012_" + Номер, Окр(Выборка.ИсчисленныеОПВ));    
				
				Номер = Номер + 1;	
			КонецЦикла;
			СтруктураРезультата.Вставить("s_910_00_012_7", ИсчисленныеОПВ);
		КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда	
		...
		Номер = 1;
		Пока Выборка.Следующий() Цикл		
			СтруктураРезультата.Вставить("s_910_00_017_" + Номер, Окр(Выборка.ОблагаемаяБазаСО));    
			ОблагаемаяБазаСО = ОблагаемаяБазаСО + Окр(Выборка.ОблагаемаяБазаСО);
				
			Номер = Номер + 1;	
		КонецЦикла;
		СтруктураРезультата.Вставить("s_910_00_017_7", ОблагаемаяБазаСО);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл		
		СоциальныеОтчисления = СоциальныеОтчисления + Окр(Выборка.СоциальныеОтчисления);	
		СтруктураРезультата.Вставить("s_910_00_018_" + Номер, Окр(Выборка.СоциальныеОтчисления));    
		
		Если НЕ СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда
		СтруктураРезультата.Вставить("s_910_00_017_" + Номер, Окр(Выборка.СоциальныеОтчисления));	
		КонецЕсли;			
	...
	Если НЕ СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда
		// для расчета потом доходов физлиц, с которых исчисляются социальные отчисления
		СтруктураРезультата.Вставить("s_910_00_017_7", СоциальныеОтчисления);
	КонецЕсли;
	
	СтруктураРезультата.Вставить("s_910_00_018_7", СоциальныеОтчисления);
	...
	Если СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			СтруктураРезультата.Вставить("s_910_00_019_" + Номер, Окр(Выборка.ОблагаемаяБазаОПВ));    
						
			Номер = Номер + 1;	
		КонецЦикла;
		СтруктураРезультата.Вставить("s_910_00_019_7", ОблагаемаяБазаОПВ);
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			// для расчета строки 910.01.019 начисленные доходы, с которых удерживаются (начисляются) ОПВ
			ИсчисленныеОПВ = ИсчисленныеОПВ + Окр(Выборка.ИсчисленныеОПВ);	
			СтруктураРезультата.Вставить("s_910_00_019_" + Номер, Окр(Выборка.ИсчисленныеОПВ));    
							
			Номер = Номер + 1;	
		КонецЦикла;
		СтруктураРезультата.Вставить("s_910_00_019_7", ИсчисленныеОПВ);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаОПВПодлежащихПеречислению = СуммаОПВПодлежащихПеречислению + Окр(Выборка.СуммаОПВПодлежащихПеречислению);
		СтруктураРезультата.Вставить("s_910_00_020_" + Номер, Окр(Выборка.СуммаОПВПодлежащихПеречислению));    
							
		Номер = Номер + 1;
	КонецЦикла;
	
	// сумма ОПВ, подлежащих перечислению
	СтруктураРезультата.Вставить("s_910_00_020_7", СуммаОПВПодлежащихПеречислению);
	...
	Если СтруктураРеквизитовФормы.ВедетсяУчетОППВ Тогда
		...
		Пока Выборка.Следующий() Цикл	
			...
			СтруктураРезультата.Вставить("s_910_00_021_" + Номер, Окр(Выборка.ОблагаемаяБазаОППВ));    
							
			Номер = Номер + 1;	
		КонецЦикла;
		СтруктураРезультата.Вставить("s_910_00_021_7", ОблагаемаяБазаОППВ);			
		...
		Пока Выборка.Следующий() Цикл	
			...
			СтруктураРезультата.Вставить("s_910_00_022_" + Номер, Окр(Выборка.ОППВ));    
							
			Номер = Номер + 1;	
		КонецЦикла;
		СтруктураРезультата.Вставить("s_910_00_022_7", СуммаОППВПодлежащихПеречислению);			
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл	
		...
		СтруктураРезультата.Вставить("s_910_00_023_" + Номер, Окр(Выборка.СуммаОСМС));    
		
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_023_7", ОблагаемаяБазаООСМС);		
	...
	Пока Выборка.Следующий() Цикл	
		...
		СтруктураРезультата.Вставить("s_910_00_024_" + Номер, Окр(Выборка.Сумма));    				
		Номер = Номер + 1;	
	КонецЦикла;
	СтруктураРезультата.Вставить("s_910_00_024_7", СуммаООСМСПодлежащихПеречислению);
	...
	Если НЕ СтруктураРеквизитовФормы.РасчетПоВидамДоходовИОблагаемойБазе Тогда
		...
		ДоходыОблагаемыеОПВ1мес = СтруктураРезультата.s_910_00_019_1* 100 / ПроцентОПВ;
		ДоходыОблагаемыеОПВ2мес = СтруктураРезультата.s_910_00_019_2* 100 / ПроцентОПВ;
		ДоходыОблагаемыеОПВ3мес = СтруктураРезультата.s_910_00_019_3* 100 / ПроцентОПВ;
		ДоходыОблагаемыеОПВ4мес = СтруктураРезультата.s_910_00_019_4* 100 / ПроцентОПВ;
		ДоходыОблагаемыеОПВ5мес = СтруктураРезультата.s_910_00_019_5* 100 / ПроцентОПВ;
		ДоходыОблагаемыеОПВ6мес = СтруктураРезультата.s_910_00_019_6* 100 / ПроцентОПВ;
		...
		ДоходыОблагаемыеСО1мес = СтруктураРезультата.s_910_00_017_1* 100 / ПроцентСО;
		ДоходыОблагаемыеСО2мес = СтруктураРезультата.s_910_00_017_2* 100 / ПроцентСО;
		ДоходыОблагаемыеСО3мес = СтруктураРезультата.s_910_00_017_3* 100 / ПроцентСО;
		ДоходыОблагаемыеСО4мес = СтруктураРезультата.s_910_00_017_4* 100 / ПроцентСО;
		ДоходыОблагаемыеСО5мес = СтруктураРезультата.s_910_00_017_5* 100 / ПроцентСО;
		ДоходыОблагаемыеСО6мес = СтруктураРезультата.s_910_00_017_6* 100 / ПроцентСО;
		
		СтруктураРезультата.Вставить("s_910_00_019_7", ДоходыОблагаемыеОПВ);
		СтруктураРезультата.Вставить("s_910_00_019_1", ДоходыОблагаемыеОПВ1мес);
		СтруктураРезультата.Вставить("s_910_00_019_2", ДоходыОблагаемыеОПВ2мес);
		СтруктураРезультата.Вставить("s_910_00_019_3", ДоходыОблагаемыеОПВ3мес);
		СтруктураРезультата.Вставить("s_910_00_019_4", ДоходыОблагаемыеОПВ4мес);
		СтруктураРезультата.Вставить("s_910_00_019_5", ДоходыОблагаемыеОПВ5мес);
		СтруктураРезультата.Вставить("s_910_00_019_6", ДоходыОблагаемыеОПВ6мес);
		СтруктураРезультата.Вставить("s_910_00_017_7", ДоходыОблагаемыеСО);
		СтруктураРезультата.Вставить("s_910_00_017_1", ДоходыОблагаемыеСО1мес);
		СтруктураРезультата.Вставить("s_910_00_017_2", ДоходыОблагаемыеСО2мес);
		СтруктураРезультата.Вставить("s_910_00_017_3", ДоходыОблагаемыеСО3мес);
		СтруктураРезультата.Вставить("s_910_00_017_4", ДоходыОблагаемыеСО4мес);
		СтруктураРезультата.Вставить("s_910_00_017_5", ДоходыОблагаемыеСО5мес);
		СтруктураРезультата.Вставить("s_910_00_017_6", ДоходыОблагаемыеСО6мес);
...
Процедура Форма910Расчет(Изменено = Ложь, ИмяОбласти = Неопределено)
		...
		Если НЕ СтруктураРеквизитовФормы.мЕжемесячныйРасчетВзносовИОтчисленийЗаИП Тогда
			...
			Если ИмяОбласти = "s_910_00_001" Или ИмяОбласти = Неопределено Тогда
				...
				Если СтруктураРеквизитовФормы.мМесяцЗаполнения = 1 Тогда
					...
					s_910_00_010_1 = Мин(s_910_00_010_1, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_010_2 = Мин(s_910_00_010_2, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_010_3 = Мин(s_910_00_010_3, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_010_4 = Мин(s_910_00_010_4, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_010_5 = Мин(s_910_00_010_5, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_5", s_910_00_010_5);
					s_910_00_010_6 = Макс(ДоходЗаМесяц, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					s_910_00_010_6 = Мин(s_910_00_010_6, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
			...
			ИначеЕсли ИмяОбласти = "s_910_00_010_1" 
				...
				Если СтруктураРеквизитовФормы.мМесяцЗаполнения = 1 Тогда
					...
					s_910_00_010_1 = ?(s_910_00_010_1 = 0, 0, Мин(s_910_00_010_1, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_1", s_910_00_010_1);
					s_910_00_010_2 = ?(s_910_00_010_2 = 0, 0, Макс(s_910_00_010_2, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_010_2 = ?(s_910_00_010_2 = 0, 0, Мин(s_910_00_010_2, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_2", s_910_00_010_2);
					s_910_00_010_3 = ?(s_910_00_010_3 = 0, 0, Макс(s_910_00_010_3, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_010_3 = ?(s_910_00_010_3 = 0, 0, Мин(s_910_00_010_3, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_3", s_910_00_010_3);
					s_910_00_010_4 = ?(s_910_00_010_4 = 0, 0, Макс(s_910_00_010_4, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_010_4 = ?(s_910_00_010_4 = 0, 0, Мин(s_910_00_010_4, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_4", s_910_00_010_4);
					s_910_00_010_5 = ?(s_910_00_010_5 = 0, 0, Макс(s_910_00_010_5, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_010_5 = ?(s_910_00_010_5 = 0, 0, Мин(s_910_00_010_5, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_5", s_910_00_010_5);
					s_910_00_010_6 = ?(s_910_00_010_6 = 0, 0, Макс(s_910_00_010_6, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_010_6 = ?(s_910_00_010_6 = 0, 0, Мин(s_910_00_010_6, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_010_6", s_910_00_010_6);
				ИначеЕсли СтруктураРеквизитовФормы.мМесяцЗаполнения = 2 Тогда
					s_910_00_010_2 = ?(s_910_00_010_2 = 0, 0, Макс(s_910_00_010_2, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_010_2 = ?(s_910_00_010_2 = 0, 0, Мин(s_910_00_010_2, 10*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
			...
			КонецЕсли;	
				...
				Если s_910_00_010_1 > 0 Тогда
					s_910_00_011_1 = Окр(Мин(s_910_00_010_1, 10 * СтруктураРеквизитовФормы.мРазмерМЗП) * СтруктураРеквизитовФормы.мСтавкаСО / 100);
				КонецЕсли;
				
				Если s_910_00_010_2 > 0 Тогда
					s_910_00_011_2 = Окр(Мин(s_910_00_010_2, 10 * СтруктураРеквизитовФормы.мРазмерМЗП) * СтруктураРеквизитовФормы.мСтавкаСО / 100);
				КонецЕсли;
				
				Если s_910_00_010_3 > 0 Тогда
					s_910_00_011_3 = Окр(Мин(s_910_00_010_3, 10 * СтруктураРеквизитовФормы.мРазмерМЗП) * СтруктураРеквизитовФормы.мСтавкаСО / 100);
				КонецЕсли;
				
				Если s_910_00_010_4 > 0 Тогда
					s_910_00_011_4 = Окр(Мин(s_910_00_010_4, 10 * СтруктураРеквизитовФормы.мРазмерМЗП) * СтруктураРеквизитовФормы.мСтавкаСО / 100);
				КонецЕсли;
				
				Если s_910_00_010_5 > 0 Тогда
					s_910_00_011_5 = Окр(Мин(s_910_00_010_5, 10 * СтруктураРеквизитовФормы.мРазмерМЗП) * СтруктураРеквизитовФормы.мСтавкаСО / 100);
				КонецЕсли;
				
				Если s_910_00_010_6 > 0 Тогда
					s_910_00_011_6 = Окр(Мин(s_910_00_010_6, 10 * СтруктураРеквизитовФормы.мРазмерМЗП) * СтруктураРеквизитовФормы.мСтавкаСО / 100);
				КонецЕсли;
				
				s_910_00_011_7 = s_910_00_011_1 + s_910_00_011_2 + s_910_00_011_3 + s_910_00_011_4 + s_910_00_011_5 + s_910_00_011_6;
				
			
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_011_1", s_910_00_011_1);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_011_2", s_910_00_011_2);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_011_3", s_910_00_011_3);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_011_4", s_910_00_011_4);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_011_5", s_910_00_011_5);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_011_6", s_910_00_011_6);
	...
	s_910_00_018_7 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_018_7"); // сумма социальных отчислений за наемных работников
	s_910_00_011_7 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_011_7");
	
	s_910_00_009 = Окр(Макс(s_910_00_007 * 0.5 - Окр(s_910_00_011_7) - Окр(s_910_00_018_7), 0));
		...
		Если НЕ СтруктураРеквизитовФормы.мЕжемесячныйРасчетВзносовИОтчисленийЗаИП Тогда
			Если ИмяОбласти = "s_910_00_001" Или ИмяОбласти = Неопределено Тогда
				...
				Если СтруктураРеквизитовФормы.мМесяцЗаполнения = 1 Тогда
					...
					s_910_00_012_1 = Мин(s_910_00_012_1, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_012_2 = Мин(s_910_00_012_2, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_012_3 = Мин(s_910_00_012_3, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_012_4 = Мин(s_910_00_012_4, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					...
					s_910_00_012_5 = Мин(s_910_00_012_5, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_5", s_910_00_012_5);
					s_910_00_012_6 = Макс(ДоходЗаМесяц, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
					s_910_00_012_6 = Мин(s_910_00_012_6, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения);
			...
			ИначеЕсли ИмяОбласти = "s_910_00_012_1" 
				...
				Если СтруктураРеквизитовФормы.мМесяцЗаполнения = 1 Тогда
					...
					s_910_00_012_1 = ?(s_910_00_012_1 = 0, 0, Мин(s_910_00_012_1, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_1", s_910_00_012_1);
					s_910_00_012_2 = ?(s_910_00_012_2 = 0, 0, Макс(s_910_00_012_2, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_012_2 = ?(s_910_00_012_2 = 0, 0, Мин(s_910_00_012_2, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_2", s_910_00_012_2);
					s_910_00_012_3 = ?(s_910_00_012_3 = 0, 0, Макс(s_910_00_012_3, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_012_3 = ?(s_910_00_012_3 = 0, 0, Мин(s_910_00_012_3, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_3", s_910_00_012_3);
					s_910_00_012_4 = ?(s_910_00_012_4 = 0, 0, Макс(s_910_00_012_4, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_012_4 = ?(s_910_00_012_4 = 0, 0, Мин(s_910_00_012_4, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_4", s_910_00_012_4);
					s_910_00_012_5 = ?(s_910_00_012_5 = 0, 0, Макс(s_910_00_012_5, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_012_5 = ?(s_910_00_012_5 = 0, 0, Мин(s_910_00_012_5, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_5", s_910_00_012_5);
					s_910_00_012_6 = ?(s_910_00_012_6 = 0, 0, Макс(s_910_00_012_6, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_012_6 = ?(s_910_00_012_6 = 0, 0, Мин(s_910_00_012_6, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_012_6", s_910_00_012_6);
				ИначеЕсли СтруктураРеквизитовФормы.мМесяцЗаполнения = 2 Тогда
					s_910_00_012_2 = ?(s_910_00_012_2 = 0, 0, Макс(s_910_00_012_2, СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
					s_910_00_012_2 = ?(s_910_00_012_2 = 0, 0, Мин(s_910_00_012_2, 75*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения));
	...
	s_910_00_014_6 = 0;
	s_910_00_014_7 = 0;
	ОбъектИсчисленияВОСМС = 0;
	
	Если СтруктураРеквизитовФормы.мНалогоплательщикЯвляетсяИП И Дата(2019,12,31)< НачалоДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) Тогда
		// В 2018 году ежемесячная сумма взносов ИП составляет 2 446 тенге (24 459 * 2 * 0,05)
		ВОСМС = Окр(2*СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения*СтруктураРеквизитовФормы.мСтавкаВОСМС/100);
		
		Если СтруктураРеквизитовФормы.мМесяцЗаполнения = 1 Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_1", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_2", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_3", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_4", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_5", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_6", ВОСМС);
		ИначеЕсли СтруктураРеквизитовФормы.мМесяцЗаполнения = 2 Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_2", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_3", ВОСМС);
			РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_910_00_014_4", ВОСМС);
		...
		//исчисления ВОСМС
		ОбъектИсчисленияВОСМС = 12 * СтруктураРеквизитовФормы.мРазмерМЗПДляЦелейНалогообложения;
	КонецЕсли;
	
	s_910_00_014_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_910_00_014_1");
...
Процедура ИнициализироватьВспомогательныеПеременные(ОтчетОбъект, СтруктураРеквизитовФормы) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.Свойство("мСтавкаВОСМС") Тогда
		СтруктураРеквизитовФормы.Вставить("мСтавкаВОСМС", ПроцедурыНалоговогоУчета.ПолучитьСтавкуОСМС(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "Взносы", Перечисления.ЮрФизЛицо.ЮрЛицо));
	Иначе
		СтруктураРеквизитовФормы.мСтавкаВОСМС = ПроцедурыНалоговогоУчета.ПолучитьСтавкуОСМС(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "Взносы", Перечисления.ЮрФизЛицо.ЮрЛицо);
	КонецЕсли;
