...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма9102020Кв2";
	КодФормы 		= "Форма910";
...
//
Процедура ПоказатьРасшифровкуВВидеРегистраНУпоИПНиСН(ДатаНачала, ДатаОкончания, СтруктурнаяЕдиницаДляОтбора = Неопределено, СтруктураФизЛицоДляОтбора = Неопределено, Режим = "БезДетализации", ПризнакНеРезиденства = Ложь, Приложение910_01 = Ложь)
...
Процедура ЗаполнитьПанель(ПризнакОсновнойФормы, ПризнакМногострочности)
	...
	Элементы.ФормаРасшифровка.Видимость = (КодФормы <> "Форма910_01");	
	Элементы.Расшифровка910_01.Видимость = (КодФормы = "Форма910_01");	
...
Процедура Форма910_01ПолеТабличногоДокументаСтраница1Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Найти(Ячейка, "Ф910_01Статус__") > 0) Тогда
		...
		ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Введите статус физического лица'"), 1,0);
		
	ИначеЕсли (Найти(Ячейка, "Ф910_01Категория__") > 0) Тогда
		НачЗначение = Область.Значение;
		
		ПараметрыСтраницы.Вставить("ПроверитьКатегориюФизЛица", Истина);	
		ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
...
Процедура Форма910_01ПолеТабличногоДокументаСтраница4Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф910_01_O__") > 0) 
		...
		ИначеЕсли (Найти(Ячейка, "Ф910_01_R__") > 0) Тогда
			...
			ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
			ПоказатьВводЧисла(ОповещениеВводаЗначений,НачЗначение, НСтр("ru='Введите сумму ОПВР'"), 12,0) ;
...
Процедура ПослеВводаЗначения(Значение, Параметры) Экспорт
	...
	Если НЕ Значение = Неопределено Тогда         	
		...
		//Если Параметры.Свойство("ПроверитьКатегориюФизЛица" , ПроверитьСтатусФизЛица) И ПроверитьСтатусФизЛица Тогда
		//	Если Значение > 3 Тогда 
		//		ПоказатьПредупреждение(, НСтр("ru = 'Введено неправильное значение категории. Категория может принимать значение от 1 до 3.'"));
		//		Значение = "";
		//	КонецЕсли;	
		//КонецЕсли;
...
Процедура Форма910_01ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	Выборка = Форма910_01РассчитатьДанныеФормы(Истина).Выбрать();	
	...
	Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
		...
		Пока Выборка.Следующий() Цикл  			
			...
			Если ИмяГруппы = "Страница1" Тогда
				...
				КонецЕсли;
				Если Выборка.Категория3 <> "" ИЛИ Выборка.Категория4 <> "" Тогда
					Категория = Категория + ?(Категория <> "", ";" + "3", "3");
				КонецЕсли;
				
				Элемент.Ф910_01Категория  = Категория;

				Элемент.Ф910_01_F  = Выборка.НачисленныеДоходы;

			ИначеЕсли ИмяГруппы = "Страница2" Тогда
				
				Элемент.Ф910_01_A2 = Формат(НомерСтроки, мФорматНомераСтроки);
				Элемент.Ф910_01_G  = Выборка.НеПодлежащиеНалогооблажению;
				Элемент.Ф910_01_H  = Выборка.ОПВ;
				Элемент.Ф910_01_I  = Выборка.ВОСМС;
				Элемент.Ф910_01_J  = Выборка.НалоговыеВычеты;
				
			ИначеЕсли ИмяГруппы = "Страница3" Тогда
				
				Элемент.Ф910_01_A3 = Формат(НомерСтроки, мФорматНомераСтроки);
				Элемент.Ф910_01_K  = Выборка.ПрочиеНалоговыеВычеты;
				Элемент.Ф910_01_L  = Выборка.ОблагаемыйДоходИПН;
				Элемент.Ф910_01_M  = Выборка.ИПНИсчисленный;
				Элемент.Ф910_01_N  = Выборка.ВыплаченоДоходов;
				
			ИначеЕсли ИмяГруппы = "Страница4" Тогда
				
				Элемент.Ф910_01_A4 = Формат(НомерСтроки, мФорматНомераСтроки);
				Элемент.Ф910_01_O  = Выборка.ИПНПодлежащийУплате;
				Элемент.Ф910_01_P  = Выборка.СоциальныеОтчисления;
				Элемент.Ф910_01_Q  = Выборка.ОППВ;
				Элемент.Ф910_01_R  = 0;
				
			ИначеЕсли ИмяГруппы = "Страница5" Тогда
				
				Элемент.Ф910_01_A5 = Формат(НомерСтроки, мФорматНомераСтроки);
				Элемент.Ф910_01_S  = Выборка.ООСМС;
