...
Процедура СформироватьОтчет(Команда)
	...
	УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""обороты счета (бух.)"" (формирование)");
...
Процедура ИзменениеСхемыКомпоновкиДанныхНаСервере() Экспорт
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		...
		ОсновнойНабор = Схема.НаборыДанных.ОсновнойНаборДанных;
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru = 'Обороты счета %1 %2'");
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка") Тогда
		Если ЗаполняемыеНастройки.Группировка Тогда
			...
			// По умолчанию включаем только n субконто
...
&НаСервере
Процедура ПроверитьКорректностьНастроек()
	
	ПараметрыОтбора = Новый Структура("Поле", "Регистратор");
	ГруппировкиДляУдаления = Отчет.Группировка.НайтиСтроки(ПараметрыОтбора);
	
	Для Каждого СтрокаГруппировка Из ГруппировкиДляУдаления Цикл
		Отчет.Группировка.Удалить(СтрокаГруппировка);
	КонецЦикла;
