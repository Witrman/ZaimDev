...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КлючТекущегоВарианта = "СчетФактураВыданный" ИЛИ КлючТекущегоВарианта = "СчетФактураПолученный" Тогда
		Отчет.ВидСчетаФактуры = КлючТекущегоВарианта;
	Иначе
		Отчет.ВидСчетаФактуры = "СчетФактураПолученный";
	КонецЕсли;
	
	СпособФормированияРеестра = ?(Отчет.ФормированиеПоДатеВыписки, "ПоДатеВыписки", "ПоДатеОборота");
	Отчет.ПредставлениеНомераДокумента 	= Константы.ПредставлениеНомераДокументов.Получить();
...
Процедура ПриОткрытии(Отказ)
	...
	ВидСФПоДатеОборота = Элементы.СпособФормированияРеестра.СписокВыбора.НайтиПоЗначению("ПоДатеОборота");
	Если ВидСФПоДатеОборота <> Неопределено Тогда
		ВидСФПоДатеОборота.Представление = ?(ВидСчетаФактуры, НСтр("ru = 'По дате отнесения в зачет'"), НСтр("ru = 'По дате оборота'"));
	КонецЕсли;
...
Процедура ВидСчетаФактурыПриИзменении(Элемент)
	...
	Если ВидСчетаФактуры Тогда
		Отчет.ВидСчетаФактуры = "СчетФактураПолученный";
		КлючТекущегоВарианта  = "СчетФактураПолученный";
	Иначе
		Отчет.ВидСчетаФактуры = "СчетФактураВыданный";
		КлючТекущегоВарианта  = "СчетФактураВыданный";
	КонецЕсли;
	
	Отчет.ФормированиеПоДатеВыписки = ?(СпособФормированияРеестра = "ПоДатеВыписки", Истина, Ложь);
...
&НаКлиенте
Процедура СпособФормированияРеестраПриИзменении(Элемент)
	Если НЕ ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(Элементы.Результат, "НеАктуальность");
	КонецЕсли;
	Отчет.ФормированиеПоДатеВыписки = ?(СпособФормированияРеестра = "ПоДатеВыписки", Истина, Ложь);
...
Процедура ПредставлениеСпискаОрганизацийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСДиалогамиКлиент.НачалоВыбораСпискаСтруктурныхЕдиниц(
		СписокСтруктурныхЕдиниц, ЭтаФорма, СтандартнаяОбработка, "ПоНалогоплательщику", Отчет.Налогоплательщик, , ПредопределенноеЗначение("Перечисление.РазделыНалоговогоУчета.НДС"), );
...
Процедура СформироватьОтчет(Команда)
	...
	КлючеваяОперация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Отчет ""регистр налогового учета по счетам-фактурам (%1)"" (формирование)"
			,?(Отчет.ВидСчетаФактуры = "СчетФактураВыданный", "выданным", "полученным"));
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ЗаголовокОтчета, ?(Форма.ВидСчетаФактуры, НСтр("ru = 'полученным'"), НСтр("ru = 'выданным'")),
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ПредставлениеНомераДокумента", Отчет.ПредставлениеНомераДокумента);
	...
	ПараметрыОтчета.Вставить("ФормированиеПоДатеВыписки"   , Отчет.ФормированиеПоДатеВыписки);
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	СписокПолей.Добавить("Налогоплательщик");
	СписокПолей.Добавить("Организация");
	СписокПолей.Добавить("СтруктурнаяЕдиница");
	СписокПолей.Добавить("ДокументПоступления");
	СписокПолей.Добавить("ДокументРеализации");
...
Процедура ИнициализацияКомпоновщикаНастроек(ПользовательскиеНастройки = Неопределено)
	...
	Схема = Отчеты[БухгалтерскиеОтчетыКлиентСервер.ПолучитьИдентификаторОбъекта(ЭтаФорма)].ПолучитьМакет(Отчет.ВидСчетаФактуры);
	Настройки = Схема.ВариантыНастроек[Отчет.ВидСчетаФактуры].Настройки;
	
	Если Настройки.ПараметрыДанных.Элементы.Найти("ФормированиеПоДатеВыписки") <> Неопределено Тогда
		Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ФормированиеПоДатеВыписки", Отчет.ФормированиеПоДатеВыписки);
	КонецЕсли;	
...
Процедура ОткрытьФормуВариантыОтчета()
	...
	ПараметрыФормы = Новый Структура("КлючВарианта", Отчет.ВидСчетаФактуры);
	ОткрытьФорму("Отчет.РегистрНалоговогоУчетаПоСчетамФактурам.ФормаОбъекта", ПараметрыФормы, ВладелецФормы);
