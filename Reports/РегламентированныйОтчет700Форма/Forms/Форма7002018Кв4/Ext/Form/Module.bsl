...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма7002018Кв4";
	КодФормы 		= "Форма700";
...
Процедура СохранитьДанныеСервере()
	...
	ПодготовитьТаблицуДляОтражениеВУчете();
...
Процедура ПодготовитьТаблицуДляОтражениеВУчете(ВнешнийОтчет = Ложь) Экспорт
	...
	Для Каждого ДобавленноеПриложение ИЗ ТаблицаДобавленныхПриложений_ Цикл
		...
		Если СтрНайти(ДобавленноеПриложение.КодФормы, "Форма700_02") <> 0 Тогда
			//Земельный налог
			//Строки 1 и 2 области макета отражения
			Для Ном = ОбластьМакета.Верх По ОбластьМакета.Верх + 1 Цикл
...
Функция ПолучитьПостфикс(Ном, ОбластьМакета, ТаблицаШапкаИПодвал, ТекИмяФормыБезПрефикса)
	...
	Если Ном = ОбластьМакета.Верх Тогда
		//Считываем строку макета для формы 700.02 (КБК 104309).
		Постфикс = "S3"
	ИначеЕсли Ном = ОбластьМакета.Верх + 1 Тогда
		//Считываем строку макета для формы 700.02 (КБК 104302).
		Постфикс = "P3"
...
Процедура Форма700_01ЗаполнитьАвто(Перезаполнить)
	...
	Для Каждого СтрокаОтчета Из мТаблицаРасшифровкиНалогаНаТранспорт Цикл		
		...
		Если СтрокаОтчета.СтруктурнаяЕдиницаОшибка <> NULL Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, СтрокаОтчета.СтруктурнаяЕдиницаОшибка);
		КонецЕсли;
...
Функция СформироватьСтуктуруСтавокНалогаНаТранспортНаСервере() Экспорт
	...
	Область = МакетСтавок.Области["Строка2018"];
	...
	Если ТаблицаОтчета.Количество() <> 0 Тогда
		...
		Для Каждого СтрокаБюджеты Из МестныеБюджеты Цикл
			...
			Для Каждого СтрокаОтчета Из ТаблицаОтчетаПоМестнымБюджетам Цикл
				...
				СтавкаСУчетомКорректировки = Окр(Ставка * КорректировкаБазСтавкиПоРешениюМестныхОрганов * КорректировкаБазСтавки * КорректировкаБазСтавкиНаТерриторияхСЭЗ, 2);
				...
				Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
					...
					ИначеЕсли ИмяГруппы = "ЛистB" Тогда
						...
						Элемент["Ф" + ИмяФормыБезПрефикса + "_M"] = ?(СтрокаОтчета.КБКЗемельногоНалога = NULL, Неопределено, СокрЛП(СтрокаОтчета.КБКЗемельногоНалога));
						
						РегламентированнаяОтчетность.ДобавитьРасшифровкуСтрокиМногострочнойФормы(ТаблицаСтрокГруппы, Элемент, "Ф" + ИмяФормыБезПрефикса + "_M",  ?(СтрокаОтчета.КатегорияЗемельВидЗемельногоНалога = NULL, Неопределено, СтрокаОтчета.КатегорияЗемельВидЗемельногоНалога));
						
					ИначеЕсли ИмяГруппы = "ЛистC" Тогда
						
						Элемент["Ф" + ИмяФормыБезПрефикса + "_A3"] = Формат(НомерСтроки, мФорматНомераСтроки);
						
						O = Окр(СтрокаОтчета.ПлощадьОблагаемая * СтавкаСУчетомКорректировки/12*?(КоличествоМесяцевИспользования = 0, 1, КоличествоМесяцевИспользования));
						
						Элемент["Ф" + ИмяФормыБезПрефикса + "_O"] = O;
...
Процедура Форма700_02Расчет(знач Инд, знач Колонка, ТекИмяФормы)
	...
	Ф700_02_O = "Ф" + ТекИмяФормыБезПрефикса + "_O";
	...
	Если Инд <> 0 Тогда
		...
		ТаблицаСтрокГруппыЛистC.Получить(Индекс)[Ф700_02_O] = O;
		
		ЭтотОбъект[ТекИмяФормы + "ПолеТабличногоДокументаЛистC"].Области[Ф700_02_O + "__" + Строка(Инд)].Значение = O;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_O + "__" + Строка(Инд), O, Ложь, O);		
		
	КонецЕсли;
	
	Ф700_02_M = "Ф" + ТекИмяФормыБезПрефикса + "_M";
	
	Если Инд <> 0 Тогда
		M = ?(ТаблицаСтрокГруппыЛистB.Получить(Индекс)[Ф700_02_M] = Неопределено, 0, ТаблицаСтрокГруппыЛистB.Получить(Индекс)[Ф700_02_M]);
	Иначе
		M = ?(ОбщиеЭлементыГруппыЛистB.Найти(Ф700_02_M, "ИмяОбласти") = Неопределено, 0, ОбщиеЭлементыГруппыЛистB.Найти(Ф700_02_M, "ИмяОбласти").Значение);	 
	КонецЕсли;
...
Процедура РасчетИтоговыхСтрок_ЛистC(мМногострочнаяСтруктура)
	...
	Ф700_02_M  = "Ф" + ТекИмяФормыБезПрефикса + "_M";
	Ф700_02_O  = "Ф" + ТекИмяФормыБезПрефикса + "_O";
	Ф700_02_P1 = "Ф" + ТекИмяФормыБезПрефикса + "_P1";
	Ф700_02_S1 = "Ф" + ТекИмяФормыБезПрефикса + "_S1";
	Ф700_02_P2 = "Ф" + ТекИмяФормыБезПрефикса + "_P2";
	Ф700_02_S2 = "Ф" + ТекИмяФормыБезПрефикса + "_S2";
	Ф700_02_P3 = "Ф" + ТекИмяФормыБезПрефикса + "_P3";
	Ф700_02_S3 = "Ф" + ТекИмяФормыБезПрефикса + "_S3";
	Ф700_02_P4 = "Ф" + ТекИмяФормыБезПрефикса + "_P4";
	Ф700_02_S4 = "Ф" + ТекИмяФормыБезПрефикса + "_S4";
	...
	Для Каждого Страница Из ЭтаФорма.Элементы[ИмяПанелиФормы].ПодчиненныеЭлементы Цикл
		...
		Если ИмяГруппы = "ЛистB" Тогда
			Для Каждого Строка Из ТаблицаСтрокГруппы Цикл
				...
				ДанныеСтрокиЛистB.Вставить("КБК", СокрЛП(Строка[Ф700_02_M]));
		...
		Если ИмяГруппы = "ЛистC" Тогда
			Для Каждого Строка Из ТаблицаСтрокГруппы Цикл
				...
				ДанныеСтрокиЛистC.Вставить("СуммаНалога", Строка[Ф700_02_O]);
	...
	Для Каждого СтрокаЛистB Из МассивЗначенийЛистB Цикл
		...
		Для Каждого СтрокаЛистC Из МассивЗначенийЛистC Цикл
			...
			Если СтрокаЛистC.НомерСтроки = НомерСтрокиЛистB Тогда
				СуммаПоСтроке = СтрокаЛистC.СуммаНалога;
				Прервать;
			КонецЕсли;
			
		КонецЦикла;
		
		Если СтрокаЛистB.КБК = "104302" Тогда 
			СуммаПоКБК_104302 = СуммаПоКБК_104302 + СуммаПоСтроке;
		ИначеЕсли СтрокаЛистB.КБК = "104309" Тогда
			СуммаПоКБК_104309 = СуммаПоКБК_104309 + СуммаПоСтроке;
		КонецЕсли;
	...
	P1 = СуммаПоКБК_104302;
	S1 = СуммаПоКБК_104309;
	
	ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_P1, "ИмяОбласти").Значение = P1;
	ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_S1, "ИмяОбласти").Значение = S1;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_P1, P1, Ложь, P1);
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_S1, S1, Ложь, S1);
	
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, "Форма700", "s_700_00_002_I", P1, Истина);
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, "Форма700", "s_700_00_002_II", S1, Истина);
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, "Форма700", "s_700_00_002", P1 + S1, Истина);
	
	P2 = ?(ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_P2, "ИмяОбласти").Значение = Неопределено, 0, ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_P2, "ИмяОбласти").Значение);	 
	S2 = ?(ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_S2, "ИмяОбласти").Значение = Неопределено, 0, ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_S2, "ИмяОбласти").Значение);
	...
	ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_P3, "ИмяОбласти").Значение = P3;
	ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_S3, "ИмяОбласти").Значение = S3;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_P3, P3, Ложь, P3);
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_S3, S3, Ложь, S3);
	
	ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_P4, "ИмяОбласти").Значение = P4;
	ОбщиеЭлементыГруппыЛистC.Найти(Ф700_02_S4, "ИмяОбласти").Значение = S4;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_P4, P4, Ложь, P4);
	РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистC", Ф700_02_S4, S4, Ложь, S4);
...
Процедура Форма700_02ПолеТабличногоДокументаЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Найти(Ячейка, "Ф" + КодФормыБезПрефикса + "_K__") > 0 Тогда                                                           			
		...
		ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, НСтр("ru='Введите ставку налога с учетом корректировки'"), 6, 2);
	...
	ИначеЕсли Найти(Ячейка, "Ф" + КодФормыБезПрефикса + "_M__") > 0 Тогда
			...
			ИмяЯчейкиРасшифровки = "Ф" + КодФормыБезПрефикса + "_M" + Сред(Ячейка,Найти(Ячейка,"__"));
			ОбластьРасширофвки = Форма700_02ПолеТабличногоДокументаЛистB.Области.Найти(ИмяЯчейкиРасшифровки);
			
			ПараметрыВыбора = Новый Структура("КодФормы, ИмяЛиста, ИмяЯчейки, ЭлементВыбора, ПризнакРасчета, НачЗначение", ТекИмяФормы, ИмяЛиста, Ячейка, "НалогиСборыОтчисления", Истина, НачЗначение);
			ОписаниеОповещения = Новый ОписаниеОповещения("Форма700_02ОбработкаВыбораЗначений", ЭтотОбъект, ПараметрыВыбора);
			ОткрытьФорму("Справочник.НалогиСборыОтчисления.ФормаВыбора",,ЭтотОбъект,,,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			
			Если ОбластьРасширофвки <> Неопределено Тогда 
				ЗначениеРасшифровки = ОбластьРасширофвки.Расшифровка;				 
				Форма700_02ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, Ячейка, НачЗначение, ЗначениеРасшифровки, Ложь);
			КонецЕсли;
...
Процедура Форма700_02ПолеТабличногоДокументаЛистCВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(Ячейка, "Ф" + КодФормыБезПрефикса + "_P2") > 0 Тогда                                                           			
		
		ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, НСтр("ru='Сумма текущих платежей по земельному налогу КБК 104302, всего:'"), 9, 0);
		
	КонецЕсли;
	
	Если Найти(Ячейка, "Ф" + КодФормыБезПрефикса + "_S2") > 0 Тогда                                                           			
		
		ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, НСтр("ru='Сумма текущих платежей по земельному налогу КБК 104309, всего:'"), 9, 0);
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	...
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма7002018Кв4");
...
Процедура Форма700_01СформироватьРасшифровку()
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "Форма7002018Кв4", "ФормаРасшифровки"), ПараметрыФормы);
...
Функция ПолучитьМестныеБюджеты(МассивСтруктурныхЕдиниц)
	...
	Для Каждого СтрокаМестныйБюджет Из ТаблицаМестныхБюджетов Цикл
		...
		Если СокрЛП(СтрокаМестныйБюджет.ИдентификационныйКодЛичности) = "" Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, СтрокаМестныйБюджет.МестныйБюджет);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьСтруктуруМногострочныхФорм()
	...
	СтрокиДереваСтруктураМногострочныхРазделов = ДеревоСтруктураМногострочныхРазделов.Строки;
	СтрокаПриложения = СтрокиДереваСтруктураМногострочныхРазделов.Найти("Форма700_02", "КодФормы");
	Если СтрокаПриложения <> Неопределено Тогда
		Для Каждого СтраницаПриложения Из СтрокаПриложения.Строки Цикл
			НовыйОбщийЭлемент = СтраницаПриложения.ОбщиеЭлементыГруппы.Добавить();
			НовыйОбщийЭлемент.ИмяПараметра = "Стр";
			НовыйОбщийЭлемент.Значение     = "001";
			Если СтраницаПриложения.ИмяГруппы = "ЛистA" Тогда
				НовыйОбщийЭлемент.ИмяОбласти = "Ф700_02Стр1";
			ИначеЕсли СтраницаПриложения.ИмяГруппы = "ЛистB" Тогда
				НовыйОбщийЭлемент.ИмяОбласти = "Ф700_02Стр2";
			ИначеЕсли СтраницаПриложения.ИмяГруппы = "ЛистC" Тогда
				НовыйОбщийЭлемент.ИмяОбласти = "Ф700_02Стр3";
			Иначе
				НовыйОбщийЭлемент.ИмяОбласти = "Ф700_02Стр";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
