...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма7002015Кв4";
	КодФормы 		= "Форма700";
...
Процедура Форма700Расчет(Изменено = Ложь)
	...
	ИтогЗначение = Значение700_00_012_1 - Значение700_00_013_1;
	Если ИтогЗначение > 0  Тогда
		// заполняем 700.00.013 
		ИтогСтрокаФормы = "s_700_00_014_1";
		ОчищаемаяСтрокаФормы = "s_700_00_015_1";  
	Иначе
		// заполняем 700.00.014 
		ИтогСтрокаФормы = "s_700_00_015_1";
	...
	Значение700_00_012_3 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_012_3");
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_014_2", Значение700_00_012_3, Ложь);
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_014_2"].Значение = Значение700_00_012_3;			
	КонецЕсли;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_013", Значение700_00_013_1, Ложь);
	
	Значение700_00_014_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_014_1");
	Значение700_00_014_2 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_014_2");
	
	Значение700_00_014 = Значение700_00_014_1 + Значение700_00_014_2;
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы,"s_700_00_014", Значение700_00_014, Ложь);
	
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_012"].Значение = Значение700_00_012;			
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_013"].Значение = Значение700_00_013_1;			
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_014"].Значение = Значение700_00_014;					
	КонецЕсли;
...
Функция СформироватьСтуктуруСтавокНалогаНаТранспортНаСервере() Экспорт
	...
	Область = МакетСтавок.Области["Строка2015"];
...
Процедура Форма700_02ПолеТабличногоДокументаЛистАВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф70002_009__1") > 0) Тогда                                                           			
		ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	...
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма7002015Кв4");
...
Процедура Форма700_01СформироватьРасшифровку()
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "Форма7002015Кв4", "ФормаРасшифровки"), ПараметрыФормы);
