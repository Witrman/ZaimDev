...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма7002023Кв1";
	КодФормы 		= "Форма700";
...
Процедура ЗаполнитьСведенияОНалогоплательщикеНаСервере(ТекущееИмяФормы)
	...
	ОргСведения = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОНалогоплательщике(Налогоплательщик, ДатаПодачи, НалоговыйКомитет);
...
Функция ВыгрузитьВXMLНаСервере(ВыдаватьСообщение = Ложь)
	...
	АдресВременногоХранилища = РегламентированнаяОтчетность.ВыгрузитьОтчетВXMLДляСОНО(ЭтотОбъект, ВыдаватьСообщение);
...
Функция СформироватьСтуктуруСтавокНалогаНаТранспортНаСервере() Экспорт
	...
	Область = МакетСтавок.Области["Строка2023"];
...
Процедура Форма700_02ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(Ячейка, "Ф" + КодФормыБезПрефикса + "ИИН_БИНГоловного_01") > 0 Тогда
		...
		ПоказатьВводСтроки(ОписаниеОповещения, НачЗначение, НСтр("ru='Введите БИН'"), 12);
	...
	Если Найти(Ячейка, "Ф" + КодФормыБезПрефикса + "ИИН_БИНАкимата_01") > 0 Тогда 
		...
		ПоказатьВводСтроки(ОписаниеОповещения, НачЗначение, НСтр("ru='Введите БИН аппарата акима'"), 12);
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	...
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма7002023Кв1");
...
Процедура Форма700_01СформироватьРасшифровку()
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "Форма7002023Кв1", "ФормаРасшифровки"), ПараметрыФормы);
