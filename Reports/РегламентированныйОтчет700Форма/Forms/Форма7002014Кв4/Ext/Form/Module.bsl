...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма7002014Кв4";
	КодФормы 		= "Форма700";
...
Процедура Расшифровка(Команда)
	...
	Если КодФормы = "Форма700" Тогда
		...
		Если ИмяЯчейки = "s_700_00_011_1" Тогда
...
Процедура Форма700ЗаполнитьАвто(Перезаполнить = Ложь)
	...
	СуммаНалогаНаИмущество = Форма700_00РассчитатьСуммуНалогаНаИмущество();
	...
	Если Найти(КБКНалогаНаИмущество, "104102") > 0 Тогда
		// КБК ФЛ
		СтрокаНалогаНаИмущество = "s_700_00_011_3";
	Иначе
		// КБК ЮЛ
		СтрокаНалогаНаИмущество = "s_700_00_011_1";
	КонецЕсли;
...
Процедура Форма700Расчет(Изменено = Ложь)
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Сумма исчисленного земельного налога: Сумма 700.00.007.1 ...  700.00.007.6		
	ИтогЗначение007 = 0;
	ИтогЗначение008 = 0;
	ИтогЗначение009 = 0;
	ИтогЗначение010 = 0;
	Для Счетчик = 1 по 8 Цикл
		...
		// в 700.00.008 и 700.00.010 нет строк 1,2
		Если Счетчик > 2 Тогда
			ИтогЗначение008 = ИтогЗначение008 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_008_" + Счетчик);
			
			// расчет налога к уменьшению или к увеличению
			ИтогЗначение = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_007_" + Счетчик)
						 - РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_008_" + Счетчик);
		...
		Иначе
			// по строкам 1, 2 нет текущих платежей, поэтому всегда заполянем сумму к начислению 700.00.009 из 700.00.007
			ИтогСтрокаФормы = "s_700_00_009";
			ОчищаемаяСтрокаФормы = "";  // нет очищаемой подстроки			
			ИтогЗначение = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_007_" + Счетчик);
		КонецЕсли;
		
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, ИтогСтрокаФормы + "_" + Счетчик, ИтогЗначение, Ложь);
		Если Изменено Тогда 
			Форма700ПолеТабличногоДокументаСтраница2.Области[ИтогСтрокаФормы + "_" + Счетчик].Значение = ИтогЗначение;
		КонецЕсли;
			
		// для первых двух подстрок в 008 и 010 нет данных для очистки. Поэтому необходима проверка на необходимость очистки строки.
		Если ЗначениеЗаполнено(ОчищаемаяСтрокаФормы) Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, ОчищаемаяСтрокаФормы + "_" + Счетчик, 0, Ложь);
			Если Изменено Тогда 
				Форма700ПолеТабличногоДокументаСтраница2.Области[ОчищаемаяСтрокаФормы + "_" + Счетчик].Значение = 0;
			КонецЕсли;
		КонецЕсли;
		
		ТаблицаДанных = РегламентированнаяОтчетность.ПолучитьТаблицуДанныхОбычнойФормы(ЭтаФорма, КодЭтойФормы);
		
		// Расчет Итогов по 009 и 010
		ИтогЗначение009 = ИтогЗначение009 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_009_" + Счетчик);
		Если Счетчик > 2 Тогда
			ИтогЗначение010 = ИтогЗначение010 + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_010_" + Счетчик);
		КонецЕсли;
	КонецЦикла;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_008", ИтогЗначение008, Ложь);	
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница2.Области["s_700_00_008"].Значение = ИтогЗначение008;			
	КонецЕсли;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_009", ИтогЗначение009, Ложь);	
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница2.Области["s_700_00_009"].Значение = ИтогЗначение009;			
	КонецЕсли;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_010", ИтогЗначение010, Ложь);	
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница2.Области["s_700_00_010"].Значение = ИтогЗначение010;			
	КонецЕсли;
			
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Сумма исчисленного имущественного налога	
	Значение700_00_011_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_011_1");
	Значение700_00_011_3 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_011_3");	
	...
	ИтогЗначение = Значение700_00_011_1 - Значение700_00_012_1;
	Если ИтогЗначение > 0  Тогда
		// заполняем 700.00.013 
		ИтогСтрокаФормы = "s_700_00_013_1";
		ОчищаемаяСтрокаФормы = "s_700_00_014_1";  
	Иначе
		// заполняем 700.00.014 
		ИтогСтрокаФормы = "s_700_00_014_1";
		ОчищаемаяСтрокаФормы = "s_700_00_013_1";  
		// меняем знак
		ИтогЗначение = - ИтогЗначение;
	КонецЕсли;
	...
	// по БК 10102 нет авансовых платежей, поэтому вся начисленная сумма в 700.00.011_3 относится на сумму к начислению: 700.00.013_2
	Значение700_00_011_3 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_011_3");
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_013_2", Значение700_00_011_3, Ложь);
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_013_2"].Значение = Значение700_00_011_3;			
	КонецЕсли;
		
	// Итоговые строки
	Значение700_00_011 = Значение700_00_011_1 + Значение700_00_011_3;
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_011", Значение700_00_011,   Ложь);
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы, "s_700_00_012", Значение700_00_012_1, Ложь);
	
	Значение700_00_013_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_013_1");
	Значение700_00_013_2 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_700_00_013_2");
	
	Значение700_00_013 = Значение700_00_013_1 + Значение700_00_013_2;
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодЭтойФормы,"s_700_00_013", Значение700_00_013, Ложь);
	
	Если Изменено Тогда 
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_011"].Значение = Значение700_00_011;			
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_012"].Значение = Значение700_00_012_1;			
		Форма700ПолеТабличногоДокументаСтраница3.Области["s_700_00_013"].Значение = Значение700_00_013;					
	КонецЕсли;
...
Процедура Форма700_01ЗаполнитьАвто(КодФормы, Перезаполнить)
	...
	Для Каждого СтрокаОтчета Из мТаблицаРасшифровкиНалогаНаТранспорт Цикл		
		...
		ИмяЯчейкиВидаТС = ПолучитьИмяЯчейкиВидаТСНаСервере(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, СтрокаОтчета.ВидТранспортногоСредства, ЗначениеПоказателя, СтрокаОтчета);
...
Функция СформироватьСтуктуруСтавокНалогаНаТранспортНаСервере() Экспорт
	...
	МакетСтавок = УправлениеПечатью.МакетПечатнойФормы("Отчет.РегламентированныйОтчет700Форма.СтавкиНалогаНаТранспорт");	
	...
	Область = МакетСтавок.Области["Строка2014"];
...
//
Функция ПолучитьИмяЯчейкиВидаТСНаСервере(мДатаКонцаПериодаОтчета, ВидТС, Категория = 0, СтрокаОтчета = Неопределено)
		...
		Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
			...
			Для Каждого СтрокаОтчета Из ТаблицаОтчета Цикл  			
				...
				Если ИмяГруппы = "ЛистА" Тогда
					...
					Элемент.Ф70002_000 = "00" + Строка(НомерСтраницы);
					...
					Элемент.Ф70002_008 = СтавкаСУчетомКорректировки;
				 	//вычислим сумму земельного налога
				 	Элемент.Ф70002_009_1 = Окр(Элемент.Ф70002_003_2 * СтавкаСУчетомКорректировки/12*?(СтрокаОтчета.КоличествоМесяцевИспользования = 0, 1, СтрокаОтчета.КоличествоМесяцевИспользования));
				 	
				 	Элемент.Ф70002_010 = ?(СтрокаОтчета.КБКЗемельногоНалога = NULL, Неопределено, СокрЛП(СтрокаОтчета.КБКЗемельногоНалога));	 
					...
					РегламентированнаяОтчетность.ДобавитьРасшифровкуСтрокиМногострочнойФормы(ТаблицаСтрокГруппы, Элемент, "Ф70002_010",  ?(СтрокаОтчета.КатегорияЗемельВидЗемельногоНалога = NULL, Неопределено, СтрокаОтчета.КатегорияЗемельВидЗемельногоНалога));
										
					// Сумма налога по КБК 104301 ... 104308					
					КБК			= ?(Элемент.Ф70002_010   = Неопределено, "", Элемент.Ф70002_010);
					СуммаНалога = ?(Элемент.Ф70002_009_1 = Неопределено, 0,  Элемент.Ф70002_009_1);
					КБККорректен = Ложь;
					Для Счетчик = 1 по 8 Цикл
						Элемент["Ф70002_10430" + Счетчик] = 0; 
						Если Найти(КБК, "10430" + Счетчик) > 0  Тогда
							Элемент["Ф70002_10430" + Счетчик] = СуммаНалога; 
							КБККорректен = Истина;
						КонецЕсли;	
					КонецЦикла;				
...
Процедура Форма700_02Расчет(Инд, Колонка)
	...
	// Сумма налога по КБК 104301 ... 104308					
	КБК			= ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф70002_010   = Неопределено, "", ТаблицаСтрокГруппы.Получить(Индекс).Ф70002_010);
	СуммаНалога = ?(ТаблицаСтрокГруппы.Получить(Индекс).Ф70002_009_1 = Неопределено,  0, ТаблицаСтрокГруппы.Получить(Индекс).Ф70002_009_1);
	
	КБККорректен = Ложь;

	Для Счетчик = 1 по 8 Цикл
		ТаблицаСтрокГруппы.Получить(Индекс)["Ф70002_10430" + Счетчик] = 0; 
		Если Найти(КБК, "10430" + Счетчик) > 0  Тогда
			ТаблицаСтрокГруппы.Получить(Индекс)["Ф70002_10430" + Счетчик] = СуммаНалога; 
			КБККорректен = Истина;
		КонецЕсли;	
	КонецЦикла;
...
Процедура Форма700_02ПолеТабличногоДокументаЛистАВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф70002_003_G__") > 0) Тогда	
		Форма700_02ВыборНаСервере(Ячейка, "Ф70002_003_M__" + СтрЗаменить(Ячейка, "Ф70002_003_G__", ""));
		Возврат;
	ИначеЕсли (Найти(Ячейка, "Ф70002_003_M__") > 0) Тогда 
		Форма700_02ВыборНаСервере(Ячейка, "Ф70002_003_G__" + СтрЗаменить(Ячейка, "Ф70002_003_M__", ""));
		Возврат;
	КонецЕсли;	
	...
	Если (Найти(Ячейка, "Ф70002_008__") > 0) Тогда                                                           			
		ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
		ПоказатьВводЧисла(ОповещениеВводаЗначений, НачЗначение, НСтр("ru='Введите ставку налога с учетом корректировки'"), 8, 2);
		Возврат;
	КонецЕсли;
	
	Если (Найти(Ячейка, "Ф70002_009_1__") > 0) Тогда                                                           			
		ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
		ПоказатьВводЧисла(ОповещениеВводаЗначений, НачЗначение, НСтр("ru='Введите сумму налога'"), 9, 0);
		Возврат;
	КонецЕсли;
	
	Если (Найти(Ячейка, "Ф70002_009_2__") > 0) Тогда                                                           			
		ОповещениеВводаЗначений = Новый ОписаниеОповещения("ПослеВводаЗначения", ЭтаФорма, ПараметрыСтраницы); 		 
		ПоказатьВводЧисла(ОповещениеВводаЗначений, НачЗначение, НСтр("ru='Введите сумму налога в соответствии со статьей 451 Налогового кодекса'"), 9, 0);
		Возврат;
	КонецЕсли;
	...
	Если (Найти(Ячейка, "Ф70002_010__") > 0) Тогда
...
&НаСервере
Процедура Форма700_02ВыборНаСервере(Ячейка, ИмяЯчейки, Значение = Неопределено)
...
Процедура Форма700_02ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.НалогиСборыОтчисления") Тогда
		...
		Форма700_02ОбработкаПодбораНаСервере("Ф70002_010__" + НомерСтроки, ВыбранноеЗначение); 	
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	...
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма7002014Кв4");
...
Процедура Форма700РасшифровкаИмуществоСКД(ПользовательскиеНастройки)
	...
	Если ВключатьВРасчетСчет2933 Тогда
		СписокСчетов.Добавить(Вычислить("ПредопределенноеЗначение(""ПланыСчетов.Типовой.КапитальныйРемонтОС"")"));
	КонецЕсли;
...
Процедура Форма700_01СформироватьРасшифровку()
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "Форма7002014Кв4", "ФормаРасшифровки"), ПараметрыФормы);
