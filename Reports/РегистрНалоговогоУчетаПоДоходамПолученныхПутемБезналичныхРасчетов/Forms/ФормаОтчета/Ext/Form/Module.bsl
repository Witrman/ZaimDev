...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = ?(Отчет.ВидОтчета = "НалоговыйРегистр", НСтр("ru = 'Налоговый регистр по учету доходов, в том числе полученных путем безналичных расчетов'"),
	НСтр("ru = 'Отчет по доходам,в том числе полученным путем безналичных расчетов'"));
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	Область = Результат.НайтиТекст("Регистрационный номер расчета стоимости патента");	
	Если НЕ ТипЗнч(Область) = Тип("Неопределено") Тогда
		
		ИмяОбластиНачальная = Область.Имя;
	    ИмяОбластиКонечная = СтрЗаменить(ИмяОбластиНачальная,"R1","R10000");
		
		ОбластьБезЗащиты =  Результат.Область(Область.Имя +":"+ ИмяОбластиКонечная ); 
...
Процедура ИнициализацияКомпоновщикаНастроек(ПользовательскиеНастройки = Неопределено)
	...
	Схема = Отчеты[БухгалтерскиеОтчетыКлиентСервер.ПолучитьИдентификаторОбъекта(ЭтаФорма)].ПолучитьМакет("МакетРегистраДоходовБезналичныеРасчеты");
