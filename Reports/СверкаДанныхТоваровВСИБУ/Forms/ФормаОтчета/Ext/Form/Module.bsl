...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки, ИспользуютсяСтандартныеНастройки)
	...
	ИнициализироватьСВосстановлениемОтбора();
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВыбранноеДействие <> ДействиеОбработкиРасшифровкиКомпоновкиДанных.Нет Тогда
		...
		Иначе
			...
			ОткрытьФорму("Отчет.СверкаДанныхТоваровВСИБУ.Форма", ПараметрыФормы, , Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Показатели") И ЗаполняемыеНастройки.Показатели Тогда
		...
		Если Отчет.ВидОтчета = "БезУчетаРезерва" Тогда
		...
		Иначе
			...
			СписокПоказателей = Новый СписокЗначений;
			СписокПоказателей.Добавить("СвободныйНачальныйОстаток", НСтр("ru = 'Нач. остаток (своб)'"));
			СписокПоказателей.Добавить("КоличествоПриходСУчетомРезерва"          , НСтр("ru = 'Приход (с рез)'"));
			СписокПоказателей.Добавить("КоличествоРасходСУчетомРезерва"          , НСтр("ru = 'Расход (с рез)'"));
			СписокПоказателей.Добавить("СвободныйКонечныйОстаток" , НСтр("ru = 'Кон. остаток (своб)'"));
	...
	Если ЗаполняемыеНастройки.Свойство("Реквизиты") И ЗаполняемыеНастройки.Реквизиты Тогда
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ПоИсточникамПроисхождения"             , Отчет.ПоИсточникамПроисхождения);
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	Если Режим = "Группировка" Тогда
		...
		КонецЕсли;
		Если Отчет.ПоИсточникамПроисхождения Тогда
			СписокПолей.Добавить("ИсточникПроисхождения");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВидОтчетаПриИзменении(Элемент)
	...
	Если ПереключательВидаОтчета Тогда
		КлючТекущегоВарианта = "БезУчетаРезерва";
	Иначе
		КлючТекущегоВарианта = "СУчетомРезерва";
	КонецЕсли;
	
	Отчет.ВидОтчета = КлючТекущегоВарианта;
...
Процедура ВидОтчетаПриИзмененииНаСервере()
	...
	ХранилищеСистемныхНастроек.Сохранить("Отчет.СверкаДанныхТоваровВСИБУ/КлючТекущегоВарианта", , КлючТекущегоВарианта)
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидОтчета()
	
	Если Не ЗначениеЗаполнено(КлючТекущегоВарианта) Или Не (КлючТекущегоВарианта = "БезУчетаРезерва" Или КлючТекущегоВарианта = "СУчетомРезерва") Тогда
		Если ЗначениеЗаполнено(Отчет.ВидОтчета) И (Отчет.ВидОтчета = "БезУчетаРезерва" Или Отчет.ВидОтчета = "СУчетомРезерва") Тогда
			КлючТекущегоВарианта = Отчет.ВидОтчета;
		Иначе
			КлючТекущегоВарианта = "БезУчетаРезерва";
		КонецЕсли;
	КонецЕсли;
	...
	Отчет.ВидОтчета = КлючТекущегоВарианта;
	ПереключательВидаОтчета = КлючТекущегоВарианта = "БезУчетаРезерва";
...
Процедура НастройкиПоУмолчанию(Команда)
	...
	ЗаполняемыеНастройки = Новый Структура("Показатели, Группировка, Отбор, ДополнительныеПоля, Реквизиты",
...
&НаСервере
Процедура ИнициализироватьСВосстановлениемОтбора()
	
	СтруктураСохраняемогоОтбора = Новый ТаблицаЗначений;
	СтруктураСохраняемогоОтбора.Колонки.Добавить("ЛевоеЗначение", Новый ОписаниеТипов("ПолеКомпоновкиДанных"));
	СтруктураСохраняемогоОтбора.Колонки.Добавить("ПравоеЗначение");
	СтруктураСохраняемогоОтбора.Колонки.Добавить("ВидСравнения", Новый ОписаниеТипов("ВидСравненияКомпоновкиДанных"));
	СтруктураСохраняемогоОтбора.Колонки.Добавить("Использование", Новый ОписаниеТипов("Булево"));
	
	Для Каждого Элемент Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		СтрокаОтбора = СтруктураСохраняемогоОтбора.Добавить();
		СтрокаОтбора.ЛевоеЗначение = Элемент.ЛевоеЗначение;
		СтрокаОтбора.ПравоеЗначение = Элемент.ПравоеЗначение;
	...
	Для Каждого Строка Из СтруктураСохраняемогоОтбора Цикл
		ДобавляемаяСтрокаОтбора = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		
		ДобавляемаяСтрокаОтбора.ЛевоеЗначение = Строка.ЛевоеЗначение;
		ДобавляемаяСтрокаОтбора.ПравоеЗначение = Строка.ПравоеЗначение;
		ДобавляемаяСтрокаОтбора.ВидСравнения = Строка.ВидСравнения;
		ДобавляемаяСтрокаОтбора.Использование = Строка.Использование;
	КонецЦикла;
