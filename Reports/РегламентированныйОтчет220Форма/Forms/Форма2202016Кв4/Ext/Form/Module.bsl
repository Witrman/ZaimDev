...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2202016Кв4";
	КодФормы 		= "Форма220";
...
Процедура Форма220РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	s_220_00_003 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_003")
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела, ТекстЗапроса = "")
	...
	Если КодФормы = "Форма220" Тогда
		...
		ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма2202016Кв4");
...
Функция ЗаполнитьВФонеНаСервере(Перезаполнить)
	...
	Иначе
		...
		ИДРедакцииОтчета = "Форма2202016Кв4";
