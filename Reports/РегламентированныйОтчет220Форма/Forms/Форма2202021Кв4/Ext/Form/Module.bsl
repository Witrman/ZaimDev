...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2202021Кв4";
	КодФормы 		= "Форма220";
...
Процедура Форма220РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	////220.00.010  
	////(220.00.010 I + 220.00.010 II + 220.00.010 III + 220.00.010 IV + 220.00.010 V)
	//Если Изменено Тогда
	//	Префикс = "s_220_00_010";
	//	СтруктураА = Новый Структура;
	//	Для НомерСтроки = 1 по 5 Цикл
	//		СтруктураА.Вставить(Префикс + "__" + НомерСтроки);
	//	КонецЦикла;
	...
	////220.00.012 Корректировка совокупного годового дохода в соответствии с пунктом 1 статьи 241 Налогового кодекса, в том числе  
	////(220.00.012 I + 220.00.012 II)
	//Если Изменено Тогда
	//	Префикс = "s_220_00_012";
	//	СтруктураА = Новый Структура;
	//	Для НомерСтроки = 1 по 2 Цикл
	//		СтруктураА.Вставить(Префикс + "__" + НомерСтроки);
	//	КонецЦикла;
	...
	// 220.00.043 ИТОГО НАЛОГООБЛАГАЕМОГО ДОХОДА (УБЫТКА)(220.00.041 + 220.00.042)
	ВсегоОблагаемыйДоходСКИК = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_041")
	+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_042");
	...
	// 220.00.045 Убыток, подлежащий переносу (220.00.043 + 220.00.044 + 220.02.008I)
	// в строке 220.00.045 указывается убыток, подлежащий переносу в соответствии с пунктом 1 статьи 300 Налогового кодекса. 
	// Если строка 220.00.043 имеет отрицательное значение, строка 220.00.045 определяется как сумма модуля строки 220.00.043 и строки 220.02.008 I. 
	// Если строка 220.00.043 имеет положительное значение, в строку 220.00.045 переносится строка 220.02.008 I;
	...
	Если Строка043 < 0 Тогда
		Строка043 	= - Строка043;
	Иначе
		Строка043 	= 0;
	КонецЕсли;
	УбытокКПереносу = Строка043 + Строка044 + Строка008_1;
	...
	//УменьшениеНОД = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_046_a")
	//+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_046_b");
	//
	//РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_220_00_046", Окр(УменьшениеНОД), ?(Изменено, Форма220ПолеТабличногоДокументаСтраница4, Неопределено));			
	//
	
	// 220.00.047	Налогооблагаемый доход с учетом уменьшения (220.00.043 - 220.00.046)																																																			
	НОДСУчетомУменьшения = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_043")
	...
	* РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_050") / 100;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_220_00_052", Окр(ИПНИсчисленный), ?(Изменено, Форма220ПолеТабличногоДокументаСтраница4, Неопределено));			
	#КонецОбласти
...
Функция Форма220ПолучитьСписокКодовСтрокВключаемыхВРасшифровкуСложныеСтроки(ИмяЯчейки)
	...
	ИначеЕсли ИмяЯчейки = "220.00.032" Тогда
		...
		мМакетКодовСтрок = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("КодыСтрокНалоговыхДеклараций");
		ДополнитьСписокКодовСтрокИзКлассификатора("220.00.032.II"  , КодыСтрокРасшифровки, мМакетКодовСтрок);
		ДополнитьСписокКодовСтрокИзКлассификатора("220.00.032.III"  , КодыСтрокРасшифровки, мМакетКодовСтрок);
	ИначеЕсли ИмяЯчейки = "220.00.046" Тогда
		КодыСтрокРасшифровки.Добавить("220.00.046");
		мМакетКодовСтрок = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("КодыСтрокНалоговыхДеклараций");
		ДополнитьСписокКодовСтрокИзКлассификатора("220.00.046.A"  , КодыСтрокРасшифровки, мМакетКодовСтрок);
		ДополнитьСписокКодовСтрокИзКлассификатора("220.00.046.B"  , КодыСтрокРасшифровки, мМакетКодовСтрок);
	КонецЕсли;
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела, ТекстЗапроса = "")
	...
	Если КодФормы = "Форма220" Тогда
		...
		ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма2202021Кв4");
...
Процедура Форма220РасшифровкаСКД(ИмяЯчейки, МетодРасшифровки)
	...
	ИначеЕсли СтрНайти(ИмяЯчейкиВходящее, "s_220_00_012__") ИЛИ СтрНайти(ИмяЯчейкиВходящее, "s_220_00_014__") Тогда
		...
		КонецЦикла;
		КодПодстрокиПолный = СокрЛП(КодыПодстроки);
	ИначеЕсли СтрНайти(ИмяЯчейкиВходящее, "s_220_00_032_2") Тогда
		КодыСтрокРасшифровки = Новый СписокЗначений;
		КодыПодстроки = Форма220ПолеТабличногоДокументаСтраница3.Область("s_220_00_032_2_dic__1").Значение;
		МассивКодовПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КодыПодстроки, ";");
		Для Каждого КодПодстроки Из МассивКодовПодстрок Цикл
			КодыСтрокРасшифровки.Добавить(Форма220ПолучитьСписокКодовСтрокВключаемыхВРасшифровку("220.00.032.II." + СокрЛП(КодПодстроки)));
			
		КонецЦикла;
		КодПодстрокиПолный = СокрЛП(КодыПодстроки);
	ИначеЕсли СтрНайти(ИмяЯчейкиВходящее, "s_220_00_032_3") Тогда
		КодыСтрокРасшифровки = Новый СписокЗначений;
		КодыПодстроки = Форма220ПолеТабличногоДокументаСтраница3.Область("s_220_00_032_3_dic__1").Значение;
		МассивКодовПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КодыПодстроки, ";");
		Для Каждого КодПодстроки Из МассивКодовПодстрок Цикл
			КодыСтрокРасшифровки.Добавить(Форма220ПолучитьСписокКодовСтрокВключаемыхВРасшифровку("220.00.032.III." + СокрЛП(КодПодстроки)));
			
		КонецЦикла;
		КодПодстрокиПолный = СокрЛП(КодыПодстроки);
	ИначеЕсли СтрНайти(ИмяЯчейкиВходящее, "s_220_00_045_a") Тогда
		КодыСтрокРасшифровки = Новый СписокЗначений;
		КодыПодстроки = Форма220ПолеТабличногоДокументаСтраница4.Область("s_220_00_045_a_dic__1").Значение;
		МассивКодовПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КодыПодстроки, ";");
		Для Каждого КодПодстроки Из МассивКодовПодстрок Цикл
			КодыСтрокРасшифровки.Добавить(Форма220ПолучитьСписокКодовСтрокВключаемыхВРасшифровку("220.00.045.A." + СокрЛП(КодПодстроки)));
			
		КонецЦикла;
		КодПодстрокиПолный = СокрЛП(КодыПодстроки);
	ИначеЕсли СтрНайти(ИмяЯчейкиВходящее, "s_220_00_046_a") Тогда
		КодыСтрокРасшифровки = Новый СписокЗначений;
		КодыПодстроки = Форма220ПолеТабличногоДокументаСтраница4.Область("s_220_00_046_a_dic__1").Значение;
		МассивКодовПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КодыПодстроки, ";");
		Для Каждого КодПодстроки Из МассивКодовПодстрок Цикл
			КодыСтрокРасшифровки.Добавить(Форма220ПолучитьСписокКодовСтрокВключаемыхВРасшифровку("220.00.046.A." + СокрЛП(КодПодстроки)));
			
		КонецЦикла;
		КодПодстрокиПолный = СокрЛП(КодыПодстроки);
	ИначеЕсли СтрНайти(ИмяЯчейкиВходящее, "s_220_00_046_b") Тогда
		КодыСтрокРасшифровки = Новый СписокЗначений;
		КодыПодстроки = Форма220ПолеТабличногоДокументаСтраница4.Область("s_220_00_046_b_dic__1").Значение;
		МассивКодовПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КодыПодстроки, ";");
		Для Каждого КодПодстроки Из МассивКодовПодстрок Цикл
			КодыСтрокРасшифровки.Добавить(Форма220ПолучитьСписокКодовСтрокВключаемыхВРасшифровку("220.00.046.B." + СокрЛП(КодПодстроки)));
	...
	ИначеЕсли СокрЛП(ИмяЯчейки)	 = "220.00.032.II" Тогда
	...
	Если СокрЛП(ИмяЯчейки)	 = "220.00.032.II" Тогда
		
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ПользовательскиеПоля, "ВидВычета", "Вид вычета");
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ПользовательскиеПоля, "РазмерМЗП", "Размер");
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ПользовательскиеПоля, "КоличествоМЗП", "Количество");
...
Функция ЗаполнитьВФонеНаСервере(Перезаполнить)
	...
	Иначе
		...
		ИДРедакцииОтчета = "Форма2202021Кв4";
...
Процедура Форма220ПолеТабличногоДокументаСтраницаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ПоложениеПризнакаКлассификатора <> 0 Тогда
		...
		Если (Найти(Ячейка, "s_220_00_045_a") > 0) 
			ИЛИ (Найти(Ячейка, "s_220_00_032_2") > 0)
			ИЛИ (Найти(Ячейка, "s_220_00_032_3") > 0)Тогда
			РазделДоходыРасходы = "НалогНаПрибыль_Расходы_";
		Иначе
			РазделДоходыРасходы = "НалогНаПрибыль_Доходы_";
		КонецЕсли;
		
		ИмяРаздела = Сред(Ячейка, 3, ПоложениеПризнакаКлассификатора - 3);
		Если (Найти(Ячейка, "s_220_00_032_2") > 0) Тогда
			ИмяРаздела = "220_00_032_II";
		ИначеЕсли (Найти(Ячейка, "s_220_00_032_3") > 0) Тогда
			ИмяРаздела = "220_00_032_III";
		КонецЕсли;
