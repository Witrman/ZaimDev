...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2202013Кв4";
	КодФормы 		= "Форма220";
...
Процедура Форма220_01ПолеТабличногоДокументаСтраница1Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф220_01_B__")> 0) ИЛИ (Найти(Ячейка, "Ф220_01_C__")> 0) ИЛИ (Найти(Ячейка, "Ф220_01_D__")> 0) Тогда	
...
Процедура ВыгрузитьВXML(Команда)
	...
	Если 2013 < ГодФормирования Тогда 
...
Процедура РасшифровкаПоСтроке(Команда)
	...
	//ИначеЕсли (Найти(ИмяЯчейки , "Ф220_01_") > 0) И Не ИмяЯчейки = "Ф220_01_G" Тогда
	//	МетодРасшифровки = Ложь; 
...
Процедура ЗаполнитьПараметры(ТекущееИмяФормы) Экспорт
	...
	Если Форма.Многострочность = 1 Тогда // заполнение структуры шапки(подвала) для многострочных форм
		...
		Если НЕ ((Налогоплательщик  = Неопределено) Или (Налогоплательщик = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.Организации"))) Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "РНННалогоплательщика", Налогоплательщик.РНН, Ложь);
		КонецЕсли;                                                  
	...
	Иначе //обычная форма
		...
		Если НЕ ((Налогоплательщик  = Неопределено) Или (Налогоплательщик = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.Организации"))) Тогда
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "РНННалогоплательщика", Налогоплательщик.РНН);
		КонецЕсли;
...
Процедура ЗаполнитьСведенияОНалогоплательщикеНаСервере(ТекущееИмяФормы)
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "РНННалогоплательщика", РННЮЛ);
...
Процедура ЗаполнитьПанель(ПризнакОсновнойФормы,ПризнакМногострочности)
	...
	Элементы.ФормаРасшифровка.Видимость   = ПризнакОсновнойФормы; 
...
Процедура Форма220РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	// 220.00.09  ОБЛАГАЕМЫЙ ДОХОД (УБЫТОК)  ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ (220.00.008 – 220.00.025 + 220.00.026 + 220.00.027 + 220.00.028)
...
Процедура Форма220_01ОбработкаПодбораНаСервере(Расшифровка, ИмяЯчейки, ИмяГруппы, НомерСтроки) Экспорт   
	...
	Если ИмяЯчейки = "Ф220_01_B" Тогда
		...
		ОбластьРНН 			 	= "Ф220_01_B__" + НомерСтроки;
		ОбластьИИН_БИН 		 	= "Ф220_01_C__" + НомерСтроки;
		ОблостьКодСтраны		= "Ф220_01_D__" + НомерСтроки;
		ОбластьНомерРегистрации = "Ф220_01_E__" + НомерСтроки;
		...
		Если Лев(Расшифровка.КБЕ,1) = "2" Тогда
			...
			СтрокаРННКонтрагента = Расшифровка.РНН; 			
		Иначе 
			Если СокрЛП(Расшифровка.РНН) = "" Тогда 
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'РННконтрагента %1 не заполнен'"),
								Строка(Расшифровка.Наименование)); 				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,Расшифровка); 
				
				СтрокаРННКонтрагента = "";                                                                             						
			ИначеЕсли НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Расшифровка.РНН)  ТОгда
		...
		Форма220_01ПолеТабличногоДокументаСтраница1.Области[ОбластьРНН].Значение 			= СтрокаРННКонтрагента; 
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтаФорма, "Страница1", ОбластьРНН, 		СтрокаРННКонтрагента,,Расшифровка); 
		...
		Форма220_01ПолеТабличногоДокументаСтраница1.Области[ОбластьРНН].Расшифровка 		      = Расшифровка; 
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма2202013Кв4");
...
Функция ЗаполнитьВФонеНаСервере(Перезаполнить)
	...
	Иначе
		...
		ИДРедакцииОтчета = "Форма2202013Кв4";
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	//	Если РеквизитФормы.Путь = "" И РеквизитФормы.Имя <> "ОтчетОбъект" Тогда
	//		Если РеквизитФормы.ТипЗначения = ОписаниеТипов_ТаблицаЗначений Тогда
	//			ЗначениеВДанныеФормы(ФормаСтруктура[РеквизитФормы.Имя], ЭтотОбъект[РеквизитФормы.Имя]);
	//		ИначеЕсли РеквизитФормы.ТипЗначения = ОписаниеТипов_ДеревоЗначений Тогда
	//			ЗначениеВДанныеФормы(ФормаСтруктура[РеквизитФормы.Имя], ЭтотОбъект[РеквизитФормы.Имя]);
	//		Иначе
	//			ЭтотОбъект[РеквизитФормы.Имя] = ФормаСтруктура[РеквизитФормы.Имя];
	//		КонецЕсли;
	//	КонецЕсли;
	//КонецЦикла;
	...
	Если ФормаСтруктура.Свойство("СтруктураФормыФорма220_01") Тогда   // было заполнено приложение 220.01
