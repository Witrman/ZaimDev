...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2202022Кв1";
	КодФормы 		= "Форма220";
...
Процедура Форма220РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	//Если строка 220.00.044 имеет отрицательное значение, строка 220.00.045 определяется как сумма модуля строки 220.00.041, и строк 220.00.044, 220.02.008 I. 
	//Если строка 220.00.041 имеет положительное значение, в строку 220.00.044 переносится строка сумма строк 220.00.043 и 220.02.008 I;
	...
	* РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_220_00_050") / 100;
	...
	// 220.00.052	Исчисленная сумма ИПН (220.00.051 - 220.00.052 I  - 220.00.052 II - 220.00.052 III - 220.00.052 IV - 220.00.052 V)
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела, ТекстЗапроса = "", ИмяЯчейки = "")
	...
	Если КодФормы = "Форма220" Тогда
		...
		ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма2202022Кв1");
...
Функция ЗаполнитьВФонеНаСервере(Перезаполнить)
	...
	Иначе
		...
		ИДРедакцииОтчета = "Форма2202022Кв1";
