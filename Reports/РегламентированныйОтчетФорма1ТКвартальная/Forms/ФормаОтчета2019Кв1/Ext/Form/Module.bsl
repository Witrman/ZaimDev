...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""форма № 1-т квартальная"" (заполнение)";
...
Процедура ЗаполнитьСведенияОНалогоплательщикеНаСервере(ТекущееИмяФормы)
	...
	// Код КАТО
	Если ЗначениеЗаполнено(КодКАТО) Тогда
...
Процедура ЗаполнитьПараметры(ТекущееИмяФормы) Экспорт
	...
	тКвартал = Формат(СтруктураРеквизитовФормы.мКвартал, "ДФ=к");
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "Квартал1","0");
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "Квартал2",тКвартал);
	
	тГод = Формат(СтруктураРеквизитовФормы.мГод, "ДФ=гггг");
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "Год1",Сред(тГод, 1, 1));
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "Год2",Сред(тГод, 2, 1));
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "Год3",Сред(тГод, 3, 1));
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, ТекущееИмяФормы, "Год4",Сред(тГод, 4, 1));
...
Процедура Форма1ТКвартальнаяЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)               
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаНалоговыйУчет.СрезПоследних(&парамПериодПо, Организация = &парамОрганизация) КАК УчетнаяПолитикаНУ
	|";
	...
	|			КОГДА ПериодыНазначений.ДатаНачала > Периоды.НачалоМесяца ТОГДА ПериодыНазначений.ДатаНачала
	|			ИНАЧЕ Периоды.НачалоМесяца
	|		КОНЕЦ) + 1) * (ВЫБОР КОГДА  ФизическиеЛица.Пол = ЗНАЧЕНИЕ(Перечисление.ПолФизическихЛиц.Женский) 
	| ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК ДнейРаботыВМесяцеЖенщины,
	...
	|	ПО ПериодыНазначений.Физлицо = ФизическиеЛица.Ссылка
	|";
	...
	|						КОГДА ПериодыНазначений.ВидЗанятости <> &парамСовместительство
	|								И ПериодыНазначений.ВидЗанятости <> &парамВнутреннееСовместительство
	|							ТОГДА ПериодыНазначений.ДнейРаботыВМесяцеЖенщины
	|						ИНАЧЕ 0
	|				   КОНЕЦ) / ДЕНЬ(ПериодыНазначений.КонецМесяца) КАК ЧИСЛО(10, 0)) КАК СреднесписочнаяЖенщины,
	|	ВЫРАЗИТЬ(СУММА(ВЫБОР 
	|						КОГДА ПериодыНазначений.Состояние НЕ В(&парамСписокИсключаемыхСостояний)
	|                             	И ПериодыНазначений.ВидЗанятости <> &парамСовместительство
	|								И ПериодыНазначений.ВидЗанятости <> &парамВнутреннееСовместительство
	|							ТОГДА ПериодыНазначений.ДнейРаботыВМесяцеЖенщины
	|						ИНАЧЕ 0
	|				   КОНЕЦ) / ДЕНЬ(ПериодыНазначений.КонецМесяца) КАК ЧИСЛО(10, 0)) КАК ФактическаяЖенщины
	|
	...
	|	СУММА(ВЫБОР 
	|				КОГДА Численность.НачалоМесяца >= &парамПериодС ТОГДА Численность.СреднесписочнаяЖенщины 
	|				ИНАЧЕ 0 
	|		  КОНЕЦ / 3) КАК СреднесписочнаяЗаКварталЖенщины,
	|	СУММА(Численность.СреднесписочнаяЖенщины / МЕСЯЦ(&парамПериодПо) ) КАК СреднесписочнаяСНачалаГодаЖенщины,
	|
	|	СУММА(ВЫБОР 
	|				КОГДА Численность.НачалоМесяца >= &парамПериодС ТОГДА Численность.ФактическаяЖенщины 
	|				ИНАЧЕ 0 
	|		  КОНЕЦ / 3) КАК ФактическаяЗаКварталЖенщины,
	|	СУММА(Численность.ФактическаяЖенщины / МЕСЯЦ(&парамПериодПо) ) КАК ФактическаяСНачалаГодаЖенщины
	| ПОМЕСТИТЬ ВТ_Численность1
	|ИЗ
	...
	|ВЫБРАТЬ 
	|	ВЫБОР 
	|		КОГДА Численность.СреднесписочнаяЗаКвартал > 0 И Численность.СреднесписочнаяЗаКвартал < 1 ТОГДА 1 
	|			ИНАЧЕ Численность.СреднесписочнаяЗаКвартал 
	|	КОНЕЦ КАК СреднесписочнаяЗаКвартал,
	|	ВЫБОР КОГДА Численность.СреднесписочнаяСНачалаГода > 0 И Численность.СреднесписочнаяСНачалаГода < 1 ТОГДА 1 
	|			ИНАЧЕ Численность.СреднесписочнаяСНачалаГода 
	|	КОНЕЦ КАК СреднесписочнаяСНачалаГода,
	|	Численность.ФактическаяЗаКвартал,
	|	Численность.ФактическаяСНачалаГода,
	|	ВЫБОР КОГДА Численность.СреднесписочнаяЗаКварталЖенщины > 0 И Численность.СреднесписочнаяЗаКварталЖенщины < 1 ТОГДА 1 
	|			ИНАЧЕ Численность.СреднесписочнаяЗаКварталЖенщины 
	|	КОНЕЦ КАК СреднесписочнаяЗаКварталЖенщины,
	|	ВЫБОР КОГДА Численность.СреднесписочнаяСНачалаГодаЖенщины > 0 И Численность.СреднесписочнаяСНачалаГодаЖенщины < 1 ТОГДА 1 
	|			ИНАЧЕ Численность.СреднесписочнаяСНачалаГодаЖенщины 
	|	КОНЕЦ КАК СреднесписочнаяСНачалаГодаЖенщины,
	|	Численность.ФактическаяЗаКварталЖенщины,
	|	Численность.ФактическаяСНачалаГодаЖенщины
	|
	|ИЗ
	|	ВТ_Численность1 КАК Численность";
	...
	Если Выборка.Следующий() Тогда
		...
		Стр210_1 = ?(ЗначениеЗаполнено(Выборка.СреднесписочнаяЗаКварталЖенщины), Выборка.СреднесписочнаяЗаКварталЖенщины, 0);
		Стр210_2 = ?(ЗначениеЗаполнено(Выборка.СреднесписочнаяСНачалаГодаЖенщины), Выборка.СреднесписочнаяСНачалаГодаЖенщины, 0);

		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр210_1", Стр210_1);
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр210_2", Стр210_2);

		// Фактическая численность
		Стр211_1 = ?(ЗначениеЗаполнено(Выборка.ФактическаяЗаКварталЖенщины), Выборка.ФактическаяЗаКварталЖенщины, 0);
		Стр211_2 = ?(ЗначениеЗаполнено(Выборка.ФактическаяСНачалаГодаЖенщины), Выборка.ФактическаяСНачалаГодаЖенщины, 0);

		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр211_1",Стр211_1);
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр211_2",Стр211_2);
	...
	|			ТОГДА " + ?(ОтражениеПоПериодуРегистрации, "НАЧАЛОПЕРИОДА(ОсновныеНачисления.Ссылка.ПериодРегистрации, МЕСЯЦ)",  "НАЧАЛОПЕРИОДА(ОсновныеНачисления.ДатаНачала, МЕСЯЦ)") + "
	...
	|			ИЛИ " + ?(ОтражениеПоПериодуРегистрации, "ОсновныеНачисления.Ссылка.ПериодРегистрации МЕЖДУ &парамНачГода И &парамПериодПо", "НАЧАЛОПЕРИОДА(ОсновныеНачисления.ДатаНачала, МЕСЯЦ) МЕЖДУ &парамНачГода И &парамПериодПо") + "
	|				И ОсновныеНачисления.ВидРасчета.СпособРасчета = &парамПоСреднемуЗаработкуДляОтпуска
	...
	|СУММА(ВЫБОР
	|		КОГДА ФЗП.НачалоМесяца >= &парамПериодС
	|			ТОГДА ФЗП.ЧислоСовместителей * (Выбор Когда ФизическиеЛица.Пол = Значение(Перечисление.ПолФизическихЛиц.Женский) Тогда 1 Иначе 0 Конец)
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК ЧислоСовместителейЗаКварталЖенщины,
	|СУММА(ФЗП.ЧислоСовместителей * (Выбор Когда ФизическиеЛица.Пол = Значение(Перечисление.ПолФизическихЛиц.Женский) Тогда 1 Иначе 0 Конец)) КАК ЧислоСовместителейЗаНачалоГодаЖенщины,
	...
	|	ВТ_ФЗП КАК ФЗП
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
	|	ПО (ФЗП.ФизЛицо = ФизическиеЛица.Ссылка)	
	|";
	...
	Если Выборка.Следующий() Тогда
		...
		// добавим к фактической численности женщин
		Стр211_1 = Стр211_1 + ?(ЗначениеЗаполнено(Выборка.ЧислоСовместителейЗаКварталЖенщины), Выборка.ЧислоСовместителейЗаКварталЖенщины / 3, 0);
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр211_1",Стр211_1);
		
		// добавим к фактической численности женщин
		Стр211_2 = Стр211_2 + ?(ЗначениеЗаполнено(Выборка.ЧислоСовместителейЗаНачалоГодаЖенщины), Выборка.ЧислоСовместителейЗаНачалоГодаЖенщины / Месяц(мДатаКонцаПериодаОтчета), 0);
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр211_2",Стр211_2);
	...
	// Раздел IV данные о численности лиц, выполняющих работы по договорам гражданско-правового характера, 
	...
	|	ТекущееНазначение.ПричинаИзмененияСостояния <> &парамУвольнение	
	|	И ТекущееНазначение.ОбособленноеПодразделение В (&парамСписокСтруктурныхЕдиниц)
	|	И ТекущееНазначение.ВидЗанятости = &парамСовместительство   
	|СГРУППИРОВАТЬ ПО
	|	ТекущееНазначение.ФизЛицо,
	|	ТекущееНазначение.Сотрудник,
	|	ТекущееНазначение.Организация,
	...
	|ВЫБРАТЬ 
	|	ПериодыНазначений.НачалоМесяца,
	|	ВЫРАЗИТЬ(СУММА(ЕстьNULL(ПериодыНазначений.ДнейРаботыВМесяце,0))/ ДЕНЬ(ПериодыНазначений.КонецМесяца) КАК ЧИСЛО(10, 0)) КАК Среднесписочная
	|
	|ПОМЕСТИТЬ ВТ_Численность
	|
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ 
	|	ВЫБОР КОГДА Численность.СреднесписочнаяЗаКвартал > 0 И Численность.СреднесписочнаяЗаКвартал < 1 Тогда 1 
	|			ИНАЧЕ Численность.СреднесписочнаяЗаКвартал 
	|	КОНЕЦ КАК СреднесписочнаяЗаКвартал,
	|	ВЫБОР КОГДА Численность.СреднесписочнаяСНачалаГода > 0 И Численность.СреднесписочнаяСНачалаГода < 1 Тогда 1
	|			ИНАЧЕ Численность.СреднесписочнаяСНачалаГода 
	...
	Если Выборка.Следующий() Тогда
		...
		Стр401_1 = ?(ЗначениеЗаполнено(Выборка.СреднесписочнаяЗаКвартал), Выборка.СреднесписочнаяЗаКвартал, 0);
		Стр401_2 = ?(ЗначениеЗаполнено(Выборка.СреднесписочнаяСНачалаГода), Выборка.СреднесписочнаяСНачалаГода, 0);
		
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр401_1",Стр401_1);
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Стр401_2",Стр401_2);
	КонецЕсли;
...
Процедура Форма1ТКвартальнаяРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы)
	...
	Стр207_1 = ?(ФормаОтчетаПолеТабличногоДокументаСтраница2.Области["Стр203_1"].Значение <> 0, 
				ФормаОтчетаПолеТабличногоДокументаСтраница3.Области["Стр205_1"].Значение * 1000 / ФормаОтчетаПолеТабличногоДокументаСтраница2.Области["Стр203_1"].Значение / 3,
				0);
	...
	Стр207_2 = ?(ФормаОтчетаПолеТабличногоДокументаСтраница2.Области["Стр203_2"].Значение <> 0,
				ФормаОтчетаПолеТабличногоДокументаСтраница3.Области["Стр205_2"].Значение * 1000 / ФормаОтчетаПолеТабличногоДокументаСтраница2.Области["Стр203_2"].Значение / Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),
				0);
