...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма2102015Кв3";
	КодФормы 		= "Форма210";
...
Функция ВыгрузитьОтчетВXMLНаСервере(ИмяФайла, ВыдаватьСообщение = Ложь, ВыгружатьВсеПриложения = Ложь)
	...
	мВыбраннаяФорма = "Форма2102015Кв3Пр2";
...
Процедура ВыгрузитьОтчетВXMLДляСОНОПриложение(ИмяФайла, Объект, СоответствиеФайлов,  ВыдаватьСообщение = Истина, ВыгружатьВсеПриложения = Истина) Экспорт
	...
	//проверка на длину префикса, в связи с тем что добавлена область макета ВыгрузкаВXML - "Выгрузка2015Кв3Пр2" с нестандартной длинной
	Если СтрДлина(мВыбраннаяФорма) > 15 Тогда 
		
		Префикс = Сред(мВыбраннаяФорма,9,СтрДлина(мВыбраннаяФорма)- 8);
		...
		Для ШагФорм = ШагФорм По СтруктураРеквизитовФормы.ВсегоСтр Цикл 
			...
			Для Шаг = НачальнаяСтрока По КонечнаяСтрока Цикл  
				...
				Если ОдинШаг Тогда 
					...
					ЗаписатьЗначениеЭлементаПриложение(Запись, СтрокаВыгрузкиТек, МногСтруктура, НомерЛиста);
