...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеАктивы 		= "09";
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеОбязательства 	= "15";
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеРезервы 		= "20";	
	СтруктураРеквизитовФормы.мКодНачалаРазделаОбязательств 	= 11; // Счета до этого кода выводятся по Дт остатку, после - по Кт остатку.
...
Процедура Расшифровка(Команда)
	...
	ЦветРасшифровки = ФормаОтчетаПолеТабличногоДокументаСтраница1.Области.s_01_1.ЦветФона;
	
	Если ТекущийЭлемент.ТекущаяОбласть.ЦветФона = ЦветРасшифровки Тогда 
		
		ФормаОтчетаРасшифровкаСКД(ИмяЯчейки);			
	Иначе	
		ПоказатьПредупреждение(, "Расшифровка данной ячейки не поддерживается."+Символы.ПС+"Расшифровку содержат ячейки формы, выделенные голубым цветом.");
	КонецЕсли;	
КонецПроцедуры
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""баланс (нсфо)"" (заполнение)";
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	// 10. Итого активы (сумма строк с 01 по 09)																				
	...
	КонецЦикла;	
	// 16. Итого обязательства (сумма строк с 11 по 15)																				
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 11 по 15 Цикл
			...
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_16_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);		
	КонецЦикла;	
	// 21. Итого капитал(сумма строк с 17 по 20)
	Для НомерПериода = 1 по 2 Цикл
		Итого = 0;
		Для Счетчик  = 17 по 20 Цикл
			НомерСтроки = "s_" + Формат(Счетчик,"ЧЦ=2; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
	...
	// 23. Итого капитал и обязательства (стр.16 + стр.21)								
	Для НомерПериода = 1 по 2 Цикл
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_16_" + НомерПериода)
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_21_" + НомерПериода);
