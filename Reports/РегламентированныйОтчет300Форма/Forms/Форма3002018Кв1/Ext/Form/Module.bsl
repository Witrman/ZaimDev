...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Форма300_09НоваяТаблицаКодовИПрефиксовЧленовТС();
	
	СформироватьСтруктуруРеквизитовФормы();
	
	мВыбраннаяФорма = "Форма3002018Кв1";
	КодФормы 		= "Форма300";
	...
	УстановитьДоступностьПодтверждениеРеализацииВЕАЭС(ЭтаФорма);
...
Процедура ЗаполнитьАвтоЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Найти(КодФормы, "Форма300_09") <> 0 Тогда 
		КодЧленаТС = Форма300_09КодЧленаТСПоИмениФормы(КодФормы, ТаблицаКодовИПрефиксовЧленовТС);
		Форма300_09ЗаполнитьАвто(Перезаполнить, КодЧленаТС);								
	КонецЕсли;	
КонецПроцедуры
...
Процедура ДобавитьСтрокуНаСервере()
	...
	Если КодФормы = "Форма300_05" Тогда
		ДобавитьСтрокуМногострочнойФормы(ЭтотОбъект);
	Иначе
...
Процедура ДобавитьСтрокуЯчейкиНаСервере()
	...
	//ИмяОбласти = СтрЗаменить(мВыбраннаяОбласть, ТекИмяФормы , "");
...
Процедура УдалитьСтрокуНаСервере(УдаляетсяПодстрока = Ложь)
	...
	ИначеЕсли КодФормы = "Форма300_03" 
			ИЛИ КодФормы = "Форма300_05"Тогда
		УдалитьСтрокуМногострочнойФормы(ЭтотОбъект, 1);
	Иначе
...
// Процедура перенесена временно до выхода новой версии конфигурации.
// Процедура удаляет строку из многострочной части.
//
// Параметры:
//  Форма - УправляемаяФорма - Форма, в которой необходимо удалить строку.
...
Процедура УдалитьСтрокуМногострочнойФормы(Форма, Знач КоличествоСтрокШапки = 1) Экспорт
	...
	Перем СтруктураГруппы, СтруктураГруппыЛистC;
	...
	ТекущаяПанель 	= Форма.Элементы["Панель" + ТекущееИмяФормы];	
	ТекСтраница  	= ТекущаяПанель.ТекущаяСтраница;
		
	мМногострочнаяСтруктура = РегламентированнаяОтчетность.ПолучитьМногострочнуюСтруктуру(Форма);	
	ТаблицаРасшифровки 		= РегламентированнаяОтчетность.ПолучитьТаблицуРасшифровки(Форма); 
	...
	Если мКоличествоНаЛисте > 1 Тогда
		ИмяПоля = ТекущееИмяФормы + "ПолеТабличногоДокумента" + ИмяСтраницыБезПрефиксаПанели;
		...
		Если Найти(ТекОбласть.Имя, "__") <> 0 Тогда
			...
			Для Каждого Страница Из ТекущаяПанель.ПодчиненныеЭлементы Цикл                 		 				
				...
				Если ТекущееИмяФормы = "Форма300_05" И ИмяСтраницыБезПрефиксаПанели = "ЛистA" Тогда
				...
				Иначе	
					...
					Если ТаблицаСтрокГруппы.Количество() = 0 Тогда
						...
						ВсегоСтраниц = 0;
					КонецЕсли;
					РегламентированнаяОтчетность.ВывестиРазделВТабличныйДокументФормы(Форма, Форма.ТекущаяСтраница, ИмяСтраницыБезПрефиксаПанели, СтруктураГруппы, ТекущееИмяФормы, ВсегоСтраниц);
	...
	Если ТекущееИмяФормы = "Форма300_05" Тогда
 		...
 		мМногострочнаяСтруктура.Свойство("ЛистC", СтруктураГруппыЛистC);
		ОбщиеЭлементыГруппыЛистC = СтруктураГруппыЛистC.ШапкаИПодвал; 
		
		
		МассивСтрокЛистC = ОбщиеЭлементыГруппыЛистC.НайтиСтроки(Новый Структура("ИмяОбласти", "Ф300_05_F"));
		Если МассивСтрокЛистC.Количество() = 0 Тогда
			Ф300_05_F = 0;
		Иначе
			Ф300_05_F = МассивСтрокЛистC[0].Значение;	
		КонецЕсли;	
		Форма.Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_001"].Значение  = Ф300_05_F;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(Форма, "ЛистA", "Ф300_05_001", Ф300_05_F, Истина);
		
		МассивСтрокЛистC = ОбщиеЭлементыГруппыЛистC.НайтиСтроки(Новый Структура("ИмяОбласти", "Ф300_05_G"));
		Если МассивСтрокЛистC.Количество() = 0 Тогда
			Ф300_05_G = 0;
		Иначе
			Ф300_05_G = МассивСтрокЛистC[0].Значение;	
		КонецЕсли;	
		Форма.Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_002"].Значение  = Ф300_05_G;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(Форма, "ЛистA", "Ф300_05_002", Ф300_05_G, Истина);
		
		МассивСтрокЛистC = ОбщиеЭлементыГруппыЛистC.НайтиСтроки(Новый Структура("ИмяОбласти", "Ф300_05_H"));
		Если МассивСтрокЛистC.Количество() = 0 Тогда
			Ф300_05_H = 0;
		Иначе
			Ф300_05_H = МассивСтрокЛистC[0].Значение;	
		КонецЕсли;	
		Форма.Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_003"].Значение  = Ф300_05_H;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(Форма, "ЛистA", "Ф300_05_003", Ф300_05_H, Истина);
		
		РегламентированнаяОтчетность.СформироватьПризнакиФормыИПередатьИтоги(Форма, ТекущееИмяФормы);
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""%1 декларация по ндс"" (заполнение)";
		КлючеваяОперация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(КлючеваяОперация,?(ЗначениеЗаполнено(КодФормы), НРег(КодФормы), "форма 300")); 
...
Процедура ЗаписатьЗамер(ВремяНачала)
	...
	Если ВремяНачала <> Неопределено 
		...
		КлючеваяОперация = "Регламентированный отчет ""форма 300 декларация по ндс"" (заполнение)";
...
Процедура ЗаполнитьПараметры(ТекущееИмяФормы) Экспорт
	...
	Если Форма.Многострочность = 1 Тогда // заполнение структуры шапки(подвала) для многострочных форм
		...
		Если Найти(ТекущееИмяФормы, "Форма300_09") Тогда
			
			КодЧленаТС = Форма300_09КодЧленаТСПоИмениФормы(ТекущееИмяФормы, ТаблицаКодовИПрефиксовЧленовТС);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекущееИмяФормы, "КодЧленаТС", КодЧленаТС, Ложь);
...
Функция ВыгрузитьОтчетВXMLДляСОНО300(Объект, ВыдаватьСообщение = Истина) Экспорт
	...
	Форма300_09УдалитьПустыеПриложенияИзТаблицыВыгрузки(ТаблицаВыгрузки);
	...
	// Заменить в именах элементов "form_300_09_0X" на "form_300_09".
	XMLФорма300 = ТекстДок.ПолучитьТекст();
	Форма300_09УдалитьПрефиксыЧленовТСИзXML(XMLФорма300, ТаблицаКодовИПрефиксовЧленовТС);
	ТекстДок.УстановитьТекст(XMLФорма300);
...
&НаСервере
Процедура Форма300_09УдалитьПустыеПриложенияИзТаблицыВыгрузки(ТаблицаВыгрузки)
	
	//////////////////////////////////////////////////////////////////////////////
	// Определить какие формы 300_09_0X заполнены, а какие пустые.
	
	МассивПрефиксовЗаполненныхФорм300_09_0X = Новый Массив;
	
	ДеревоМногострочныхРазделов = РеквизитФормыВЗначение("мСтруктураМногострочныхРазделов");
	
	Для Каждого СтрокаКодовИПрефиксовЧленовТС Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		
		КодФормыВключаяПрефиксЧленаТС = Форма300_09ИмяФормыВключаяПрефиксЧленаТС(СтрокаКодовИПрефиксовЧленовТС.ПрефиксЧленаТС);
		ДанныеФормы300_09_0X = ДеревоМногострочныхРазделов.Строки.Найти(КодФормыВключаяПрефиксЧленаТС, "КодФормы", Ложь); 
		
		Если ДанныеФормы300_09_0X = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось найти форму 300.09 по коду формы.'");
		КонецЕсли;
			
		ИмяТаблицыСтрок300_09_0X = ДанныеФормы300_09_0X.Строки[0].ИмяРеквизитаТаблицыСтрокГруппы;
		
		Если ЭтаФорма[ИмяТаблицыСтрок300_09_0X].Количество() <> 0 Тогда
			МассивПрефиксовЗаполненныхФорм300_09_0X.Добавить(СтрокаКодовИПрефиксовЧленовТС.ПрефиксЧленаТС);
		КонецЕсли;
		
	КонецЦикла;
	
	//////////////////////////////////////////////////////////////////////////////
	// Удалить пустые формы 300_09_0X.
	
	Если МассивПрефиксовЗаполненныхФорм300_09_0X.Количество() = 0 Тогда
		
		// В форме 300 всегда должен оставаться один раздел 300.09,
		// поэтому удалим все разделы 300_09_0X, кроме 300_09_01.
		
		Для Каждого СтрокаКодовИПрефиксовЧленовТС Из ТаблицаКодовИПрефиксовЧленовТС Цикл
			
			Если СтрокаКодовИПрефиксовЧленовТС.ПрефиксЧленаТС <> "01" Тогда
				
				ПараметрыОтбора = Новый Структура;
				ПараметрыОтбора.Вставить("Форма", "form_300_09_" + СтрокаКодовИПрефиксовЧленовТС.ПрефиксЧленаТС);   
				МассивНайденныхСтрок = ТаблицаВыгрузки.НайтиСтроки(ПараметрыОтбора);
				
				Для Каждого НайденнаяСтрока Из МассивНайденныхСтрок Цикл
					ТаблицаВыгрузки.Удалить(НайденнаяСтрока); 	
				КонецЦикла;
	...
	Иначе
		...
		Для Каждого СтрокаКодовИПрефиксовЧленовТС Из ТаблицаКодовИПрефиксовЧленовТС Цикл
			...
			Форма300_09_0XПустая = ?(МассивПрефиксовЗаполненныхФорм300_09_0X.Найти(СтрокаКодовИПрефиксовЧленовТС.ПрефиксЧленаТС) = Неопределено, Истина, Ложь); 
			
			Если Форма300_09_0XПустая Тогда
...
Процедура Форма300ПолеТабличногоДокументаВыборНаСервере(Ячейка, Значение, ПризнакРасчета)	
	...
	Если (Найти(Ячейка, "Условие_A") > 0)
		...
		ИЛИ (Найти(Ячейка, "ОтказОтВозврата") > 0) 
...
Процедура Форма300РасчетНаСервере(знач ИмяЯчейки, знач ЗначениеЯчейки)
	...
	//s_300_00_006_1. Общий облогаемый оборот (300.00.001А+300.00.002А+ 300.00.003А)
	...
	Если a_s_300_00_006 <> 0 Тогда	
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_300_00_007_b", СтруктураРеквизитовФормы.УдВес, Форма300ПолеТабличногоДокументаСтраница1);		
	Иначе
	...
	// (300.00.013A + 300.00.014А + 300.00.015 + 300.00.016АI + 300.00.016АII + 300.00.017 + 300.00.018 + 300.00.020А + 300.00.026А)
	...
	//Убрана строка s_300_00_018_a
	//Новый Структура("s_300_00_013_a, s_300_00_014_a, s_300_00_015_a, s_300_00_016_1_a, s_300_00_016_2_a, s_300_00_017_a, s_300_00_018_a, s_300_00_020_a, s_300_00_026_a"),, Форма300ПолеТабличногоДокументаСтраница2);
	...
	ИначеЕсли СтруктураРеквизитовФормы.мМетодРасчета = Перечисления.МетодыОтнесенияНДСВЗачет.Пропорциональный Тогда
		...
		s_300_00_007_b = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_300_00_007_b");
		
		Если s_300_00_007_b = 0 Тогда
			b_s_300_00_025_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_300_00_023_b"); 
		Иначе	
			b_s_300_00_025_1 = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_300_00_023_b") * s_300_00_007_b/100; 
		КонецЕсли;	
	...
	Иначе	
		...
		Если ПризнакОдновременногоПрименения Тогда
		    ...
		    //300.00.026 III. Сумма не разрешенного к отнесению в зачет НДС при пропорциональном и раздельном методе (300.00.023 - 300.00.025III)
			s_300_00_026_3_b = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_300_00_023_b")
								- b_s_300_00_025_3;		
		КонецЕсли;
	КонецЕсли;	
...
//	(300.00.012-300.00.025I-300.00.027I) x70% 
//или (300.00.012-300.00.025II+300.00.027 I)х70%
//или (300.00.012-300.00.025III+300.00.027 I) х70%
...
Функция Форма300РассчитатьНачислениеНДС(ВыдаватьСообщения = Ложь) 
	...
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА вт_ДействующиеСФ.Ссылка.СпособВыставления = ЗНАЧЕНИЕ(Перечисление.СпособыОбменаДокументами.Электронно)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	|	КОНЕЦ КАК ЕстьЭСФ
...
Процедура Форма300ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь) 
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		Форма300_09ЗаполнитьАвтоНаСервере(, СтрокаТаблицы.КодЧленаТС);	
	КонецЦикла;
...
Процедура ЗаполнитьПоле300_00_11()
	...
	Для Каждого ПриложениеФормы300 Из мСписокФормБезИерархии.ПолучитьЭлементы() Цикл
		...
		Если Найти(ПриложениеФормы300.КодФормы, "Форма300_09") = 0 
			И ПриложениеФормы300.КодФормы <> "Форма300" Тогда
	...
	// Приложение 300.09 считается заполненным, если заполнено хотябы одно из приложений 300.09.
	ЗаполненоПриложение300_09 = Ложь;
	Для Каждого ПриложениеФормы300 Из мСписокФормБезИерархии.ПолучитьЭлементы() Цикл
		Если Найти(ПриложениеФормы300.КодФормы, "Форма300_09") <> 0 Тогда
			Если ПриложениеФормы300.Выгружать Тогда
				ЗаполненоПриложение300_09 = Истина;	
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗаполненоПриложение300_09 Тогда
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы300_00, ИмяОбласти, "Х", Ложь);
	Иначе
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы300_00, ИмяОбласти, "", Ложь);
	КонецЕсли;
...
Функция Форма300_02РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
  ...
  Запрос = Новый Запрос;
	...
	|			,
	|			Налогоплательщик = &Налогоплательщик
	|				И СтавкаНДС.ДляОсвобожденногоОборота = ИСТИНА
	|				И НЕ ВидНалогаНДС = ЗНАЧЕНИЕ(Перечисление.ВидыНДС.НДСОблагаемыйИмпорт)) КАК НДСОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КодыСтрокДекларацииПоНДС.СрезПоследних(&КонПериода, ) КАК КодыСтрокДекларацииПоНДС
	|		ПО НДСОбороты.ВидОперацииРеализации = КодыСтрокДекларацииПоНДС.ВидОперации
	|ГДЕ
...
Процедура Форма300_05РасчетНаСервере(знач ИмяЯчейки, знач ЗначениеЯчейки)
	...
	s_300_05_007 = Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_003"].Значение + Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_006"].Значение;
	Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_007"].Значение  = s_300_05_007;
	
	ИмяОбласти = "Ф300_05_007";
	МассивСтрок = ОбщиеЭлементыГруппы.НайтиСтроки(Новый Структура("ИмяОбласти", ИмяОбласти));
	Если МассивСтрок.Количество() = 0 Тогда
		Элемент = ОбщиеЭлементыГруппы.Добавить();   			 
		Элемент.ИмяОбласти 	= ИмяОбласти;
	Иначе
		Элемент = МассивСтрок[0];
	КонецЕсли;
	Элемент.Значение = s_300_05_007;
	СтруктураРеквизитовФормы.СтруктураИтоговыхПолей300_05.Вставить(ИмяОбласти, Элемент.Значение); 
	...
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);									
	//	s_300_05_007 = Окр(s_300_05_007 * СтруктураРеквизитовФормы.УдВес / 100);
	//	
	//КонецЕсли;
	Форма300_05ПолеТабличногоДокументаЛистA.Области["Знач_300_05_007"].Значение  = s_300_05_007;

	ИмяОбласти = "Знач_300_05_007";
	МассивСтрок = ОбщиеЭлементыГруппы.НайтиСтроки(Новый Структура("ИмяОбласти", ИмяОбласти));
	Если МассивСтрок.Количество() = 0 Тогда
		Элемент = ОбщиеЭлементыГруппы.Добавить();   			 
		Элемент.ИмяОбласти 	= ИмяОбласти;
...
Функция Форма300_05_002РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	НДСЗаНерезидентаОстаткиИОбороты.СуммаБезНДСПриход КАК ОблогаемыОборот,
	...
	|	НДСЗаНерезидентаОстаткиИОбороты.ДокументПоступления.Контрагент КАК Контрагент
	|ИЗ
	...
	РезультатЗапроса = Запрос.Выполнить();          	
	Если РезультатЗапроса.Пустой() И ВыдаватьСообщения Тогда
		ТекстСообщения = НСтр("ru = 'Данные для заполнения строк 300.05.001, 300.05.002, 300.05.003 формы 300.05 за отчетный период отсутствуют...'");		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);													 
	КонецЕсли;
...
Функция Форма300_05_005РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СУММА(0) КАК Знач300_05_006,
	|	СУММА(ЕСТЬNULL(НДСЗаНерезидентаОстатки.СуммаНДСОстаток, 0)) КАК Знач300_05_005,
	|	СУММА(ЕСТЬNULL(НДСЗаНерезидентаОстатки.СуммаБезНДСОстаток, 0)) КАК Знач300_05_004
	...
	|
	|ВЫБРАТЬ
	|	СУММА(ЕСТЬNULL(НДСЗаНерезидентаОбороты.СуммаНДСРасход, 0)),
	|	ЕСТЬNULL(НДСЗаНерезидентаОбороты.СуммаНДСПриход, 0),
	|	ЕСТЬNULL(НДСЗаНерезидентаОбороты.СуммаБезНДСПриход, 0)
	|ИЗ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	НДСЗаНерезидентаОбороты.Номенклатура,
	|	ЕСТЬNULL(НДСЗаНерезидентаОбороты.СуммаНДСПриход, 0),
	|	ЕСТЬNULL(НДСЗаНерезидентаОбороты.СуммаБезНДСПриход, 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(вт_НеСгруппированыеДанные.Знач300_05_006) КАК Знач300_05_006,
	|	СУММА(вт_НеСгруппированыеДанные.Знач300_05_005) КАК Знач300_05_005,
	|	СУММА(вт_НеСгруппированыеДанные.Знач300_05_004) КАК Знач300_05_004
	|ИЗ
	|	вт_НеСгруппированыеДанные КАК вт_НеСгруппированыеДанные";
	
	РезультатЗапроса = Запрос.Выполнить();          		
	Если РезультатЗапроса.Пустой() И ВыдаватьСообщения Тогда
		ТекстСообщения = НСтр("ru = 'Данные для заполнения строк 300.05.004, 300.05.005, 300.05.006 формы 300.05 за отчетный период отсутствуют...'");		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);													 
	КонецЕсли;
...
Процедура Форма300_05ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	ЗначФ300_05 = Новый Соответствие;
	...
	Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
		...
		Выборка002.Сбросить();
	 	...
	 	Пока Выборка002.Следующий() Цикл
			...
			Если ИмяГруппы = "ЛистB" Тогда
				...
				Контрагент 			= Выборка002.Контрагент; 
				
				Элемент.Ф300_05_A   = Формат(НомерСтроки, мФорматНомераСтроки);
				...
				Если ЗначениеЗаполнено(Контрагент) Тогда
					...
					Элемент.Ф300_05_C   = СтранаРезидентства.Код;
					
					Элемент.Ф300_05_D   = Контрагент.НомерНалоговойРегистрацииВСтранеРезидентства;
					
				КонецЕсли;
				
				Элемент.Ф300_05_E   = Выборка002.ДатаОборота;
						 
			ИначеЕсли ИмяГруппы = "ЛистC" Тогда
				
				Элемент.Ф300_05_A2 = Формат(НомерСтроки, мФорматНомераСтроки);
				
				Элемент.Ф300_05_F   = Окр(?(Выборка002.ОблогаемыОборот = NULL, 0, Выборка002.ОблогаемыОборот));
				
				Элемент.Ф300_05_G   = Окр(?(Выборка002.СуммаНДСКУплате = NULL, 0, Выборка002.СуммаНДСКУплате));
				
				Элемент.Ф300_05_H   = Окр(?(Выборка002.СуммаНДСУплаченного = NULL, 0, Выборка002.СуммаНДСУплаченного));
				
				ЗначФ300_05_001 	= ЗначФ300_05_001 + Элемент.Ф300_05_F;
				ЗначФ300_05_002 	= ЗначФ300_05_002 + Элемент.Ф300_05_G;
				ЗначФ300_05_003 	= ЗначФ300_05_003 + Элемент.Ф300_05_H;
			КонецЕсли; 
	...
	ЗначФ300_05.Вставить("ЗначФ300_05_001", ЗначФ300_05_001);
	ЗначФ300_05.Вставить("ЗначФ300_05_002", ЗначФ300_05_002);
	ЗначФ300_05.Вставить("ЗначФ300_05_003", ЗначФ300_05_003);
	
	Если Выборка005.Следующий() Тогда
		ЗначФ300_05_004  = Окр(?(Выборка005.Знач300_05_004=NULL, 0, Выборка005.Знач300_05_004)); 
		ЗначФ300_05_005  = Окр(?(Выборка005.Знач300_05_005=NULL, 0, Выборка005.Знач300_05_005)); 
		ЗначФ300_05_006  = Окр(?(Выборка005.Знач300_05_006=NULL, 0, Выборка005.Знач300_05_006)); 
		
		Если ЗначФ300_05_004 <> 0 
				ИЛИ ЗначФ300_05_005 <> 0 
				ИЛИ ЗначФ300_05_006 <> 0 Тогда
			ВыводитьПервуюСтраницу = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	ЗначФ300_05_007 = ЗначФ300_05_003 + ЗначФ300_05_006;
	
	ЗначФ300_05.Вставить("ЗначФ300_05_004", ЗначФ300_05_004);
	ЗначФ300_05.Вставить("ЗначФ300_05_005", ЗначФ300_05_005);
	ЗначФ300_05.Вставить("ЗначФ300_05_006", ЗначФ300_05_006);
	ЗначФ300_05.Вставить("ЗначФ300_05_007", ЗначФ300_05_007);
	...
	Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл	
		...
		Если ИмяГруппы = "ЛистA" Тогда
			...
			ТаблицаСтрокГруппы 	= ЭлементСтруктуры.Значение.Строки;
			//Если ТаблицаСтрокГруппы.Количество() <> 0 ИЛИ ВыводитьПервуюСтраницу Тогда
			Если ВыводитьПервуюСтраницу Тогда
				ТаблицаСтрокГруппы.Очистить();
				ЭлементСтроки  		= ТаблицаСтрокГруппы.Добавить();
				Для i =1 По 7 Цикл
					ИмяОбласти = "Ф300_05_00" + Строка(i);
					...
					Иначе
						Элемент = МассивСтрок[0];
					КонецЕсли;
					ИмяЗначения = "ЗначФ300_05_00" + Строка(i);
					Элемент.Значение = ЗначФ300_05.Получить(ИмяЗначения);
					СтруктураРеквизитовФормы.СтруктураИтоговыхПолей300_05.Вставить(ИмяЗначения, Элемент.Значение);
				КонецЦикла;
				...
				Иначе
					Элемент = МассивСтрок[0];
				КонецЕсли;
				ИмяЗначения 	= "ЗначФ300_05_007";
				s_300_05_007 	= ЗначФ300_05.Получить(ИмяЗначения);
				Если СтруктураРеквизитовФормы.мМетодРасчета = Перечисления.МетодыОтнесенияНДСВЗачет.Пропорциональный ИЛИ СтруктураРеквизитовФормы.мПропорцИРаздел Тогда
				...
				Иначе
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СтруктураРеквизитовФормы.УдВес);			
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);									
					s_300_05_007 = Форма300_05РассчитатьДанныеДляЗачетаНДС();
				КонецЕсли;
				Элемент.Значение = s_300_05_007;
				СтруктураРеквизитовФормы.СтруктураИтоговыхПолей300_05.Вставить(ИмяОбласти, Элемент.Значение); 
			КонецЕсли;	
...
Процедура Форма300_05ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф300_05") > 0) Тогда
		...
		Расчет = Истина;
		Если (Найти(Ячейка, "Ф300_05_004") > 0) Тогда
			НачЗначение = Элемент.ТекущаяОбласть.Значение;			
			ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, "Введите облагаемый оборот в предыдущие налоговые периоды", 12, 0); 
			 
		ИначеЕсли (Найти(Ячейка, "Ф300_05_005") > 0) Тогда						
			НачЗначение = Элемент.ТекущаяОбласть.Значение;
			ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, "Сумма НДС, подлежащего уплате в предыдущие налоговые периоды", 12, 0); 						 
		ИначеЕсли (Найти(Ячейка, "Ф300_05_006") > 0) Тогда			
			НачЗначение = Элемент.ТекущаяОбласть.Значение;
			ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, "Сумма НДС, уплаченного в бюджет в предыдущие налоговые периоды", 12, 0);
		Иначе 
			Возврат;			 
		КонецЕсли;
	КонецЕсли;
...
Процедура Форма300_05ПолеТабличногоДокументаЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф300_05_B") > 0) 
		...
		Если Не ЗначениеЗаполнено(ЗначениеРасшифровки) Тогда
			 ...
			 ОбластьРасширофвки = Форма300_05ПолеТабличногоДокументаЛистB.Области.Найти(ИмяЯчейкиРасшифровки);
	...
	ИначеЕсли  (Найти(Ячейка, "Ф300_05_E") > 0) Тогда 
		ПараметрыВыбора = Новый Структура("КодФормы, ИмяЛиста, ИмяЯчейки, ЭлементВыбора, ПризнакРасчета", ТекИмяФормы, ИмяЛиста, Ячейка, "Дата оборота", ПризнакРасчета);
		ОписаниеОповещения = Новый ОписаниеОповещения("Форма300_05ОбработкаВыбораЗначений", ЭтотОбъект, ПараметрыВыбора);

		СтандартнаяОбработка = Ложь;
		НачЗначение = Элемент.ТекущаяОбласть.Значение;
		ПоказатьВводДаты(ОписаниеОповещения, НачЗначение, "Дата совершения оборота по реализации", ЧастиДаты.Дата); 
		
		Форма300_05ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, Ячейка, НачЗначение,, Ложь);		
	КонецЕсли;
...
Процедура Форма300_05ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, ИмяЯчейки, Значение, ЗначениеРасшифровки, ПризнакРасчета)	
	...
	МногострочнаяСтруктураЛистC = РегламентированнаяОтчетность.ПолучитьМногострочнуюСтруктуру(ЭтотОбъект, "Форма300_05");					
	МногострочнаяСтруктураЛистC.Свойство("ЛистC", СтруктураГруппыЛистC);
	ОбщиеЭлементыГруппыЛистC = СтруктураГруппыЛистC.ШапкаИПодвал; 
	
	Если (Найти(ИмяЯчейки, "Ф300_05_F") > 0) Тогда
		МассивСтрокЛистC = ОбщиеЭлементыГруппыЛистC.НайтиСтроки(Новый Структура("ИмяОбласти", "Ф300_05_F"));
		Если МассивСтрокЛистC.Количество() = 0 Тогда
			Ф300_05_F = 0;
		Иначе
			Ф300_05_F = МассивСтрокЛистC[0].Значение;	
		КонецЕсли;	
		Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_001"].Значение  = Ф300_05_F;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистA", "Ф300_05_001", Ф300_05_F, ПризнакРасчета,ЗначениеРасшифровки);
		РегламентированнаяОтчетность.СформироватьПризнакиФормыИПередатьИтоги(ЭтотОбъект, ЭтотОбъект.КодФормы);
	ИначеЕсли (Найти(ИмяЯчейки, "Ф300_05_G") > 0) Тогда
		МассивСтрокЛистC = ОбщиеЭлементыГруппыЛистC.НайтиСтроки(Новый Структура("ИмяОбласти", "Ф300_05_G"));
		Если МассивСтрокЛистC.Количество() = 0 Тогда
			Ф300_05_G = 0;
		Иначе
			Ф300_05_G = МассивСтрокЛистC[0].Значение;	
		КонецЕсли;	
		Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_002"].Значение  = Ф300_05_G;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистA", "Ф300_05_002", Ф300_05_G, ПризнакРасчета,ЗначениеРасшифровки);
		РегламентированнаяОтчетность.СформироватьПризнакиФормыИПередатьИтоги(ЭтотОбъект, ЭтотОбъект.КодФормы);
	ИначеЕсли (Найти(ИмяЯчейки, "Ф300_05_H") > 0) Тогда
		МассивСтрокЛистC = ОбщиеЭлементыГруппыЛистC.НайтиСтроки(Новый Структура("ИмяОбласти", "Ф300_05_H"));
		Если МассивСтрокЛистC.Количество() = 0 Тогда
			Ф300_05_H = 0;
		Иначе
			Ф300_05_H = МассивСтрокЛистC[0].Значение;	
		КонецЕсли;	
		Форма300_05ПолеТабличногоДокументаЛистA.Области["Ф300_05_003"].Значение  = Ф300_05_H;
		РегламентированнаяОтчетность.ПередатьЗначениеВМногострочнуюСтруктуру(ЭтотОбъект, "ЛистA", "Ф300_05_003", Ф300_05_H, ПризнакРасчета,ЗначениеРасшифровки);
		РегламентированнаяОтчетность.СформироватьПризнакиФормыИПередатьИтоги(ЭтотОбъект, ЭтотОбъект.КодФормы);
	КонецЕсли;
...
Процедура  Форма300_05ОбработкаВыбораЗначенийНаСервере(Результат, ПараметрыВыбора)
	...
	Если ПараметрыВыбора.ЭлементВыбора = "Контрагент" И ЗначениеЗаполнено(Результат) Тогда
		...
		Форма300_05ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, ИмяЯчейкиКодСтраныРезидентства, Результат.СтранаРезидентства.Код,  Результат, ПараметрыВыбора.ПризнакРасчета);	
...
&НаКлиенте
Процедура Форма300_07ПолеТабличногоДокументаЛистDВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Ячейка, "Ф300_07") > 0) Тогда
		...
		Если (Найти(Ячейка, "Ф300_07_K__") > 0) Тогда
			НачЗначение = Элемент.ТекущаяОбласть.Значение;			
		...
		ИначеЕсли (Найти(Ячейка, "Ф300_07_M__") > 0) Тогда			
			НачЗначение = Элемент.ТекущаяОбласть.Значение;
			...
			ВыборкаОтчета.Сбросить();
		 	...
		 	Пока ВыборкаОтчета.Следующий() Цикл
				...
				Если ИмяГруппы = "ЛистA" Тогда
					...
					Элемент.Ф300_07_D_Расшифровка = ВыборкаОтчета.Контрагент;											 
				...
				ИначеЕсли ИмяГруппы = "ЛистB" Тогда
					...
					Элемент.Ф300_07_F_Расшифровка = ВыборкаОтчета.СФ;
				...
				ИначеЕсли ИмяГруппы = "ЛистD" Тогда
					
				 	Элемент.Ф300_07_A4 = Формат(НомерСтроки, мФорматНомераСтроки);	
...
Функция Форма300_07ПолучитьТаблицуДанныхРеестраExcel(ТекКодФормы)
	...
	ТаблицаДанныхA.Колонки.Добавить("РННПокупателя", , 		"РНН покупателя"); 
	...
	ТаблицаДанныхB = Новый ТаблицаЗначений;
	ТаблицаДанныхB.Колонки.Добавить("НомерСФ");
	ТаблицаДанныхB.Колонки.Добавить("ДатаСФ");
	ТаблицаДанныхB.Колонки.Добавить("ВидСФЛев");
	ТаблицаДанныхB.Колонки.Добавить("ВидСФПрав");
	
	ТаблицаДанныхC = Новый ТаблицаЗначений;
	ТаблицаДанныхC.Колонки.Добавить("СтоимостьБезНДС");
	ТаблицаДанныхC.Колонки.Добавить("СуммаНДССФ");
	ТаблицаДанныхC.Колонки.Добавить("СуммаНДСНачисленного");
	
	ТаблицаДанныхD = Новый ТаблицаЗначений;
	ТаблицаДанныхD.Колонки.Добавить("ВидПолезногоИскопаемого");
	ТаблицаДанныхD.Колонки.Добавить("ОбъемПолезногоИскопаемого");
	ТаблицаДанныхD.Колонки.Добавить("ЕдиницаИзмерения");	
		...
		Для Каждого Элемент Из ТаблицаСтрокГруппы Цикл
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				Если ЗначениеЗаполнено(Элемент.Ф300_07_C) Тогда 
					НоваяСтрокаДанных.РННПокупателя    = Элемент.Ф300_07_C;
				КонецЕсли;
				Если ЗначениеЗаполнено(Элемент.Ф300_07_D) Тогда 
					НоваяСтрокаДанных.ИННБИНПокупателя = Элемент.Ф300_07_D;
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистB" Тогда
				...
				НоваяСтрокаДанных.НомерСФ 		   = Элемент.Ф300_07_E;
				Если ЗначениеЗаполнено(Элемент.Ф300_07_F) Тогда
					НоваяСтрокаДанных.ДатаСФ	   = НачалоДня(Элемент.Ф300_07_F);
				КонецЕсли;
												
				Если ЗначениеЗаполнено(Элемент.Ф300_07_G) Тогда
					НоваяСтрокаДанных.ВидСФПрав	   = Элемент.Ф300_07_G;
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистC" Тогда
				...
				НоваяСтрокаДанных.СтоимостьБезНДС	   = Элемент.Ф300_07_H;
				НоваяСтрокаДанных.СуммаНДССФ 		   = Элемент.Ф300_07_I;
				НоваяСтрокаДанных.СуммаНДСНачисленного = Элемент.Ф300_07_J;
				
			ИначеЕсли ИмяГруппы = "ЛистD" Тогда
				
				НоваяСтрокаДанных = ТаблицаДанныхD.Добавить();	

				НоваяСтрокаДанных.ВидПолезногоИскопаемого	= Элемент.Ф300_07_K;
				НоваяСтрокаДанных.ОбъемПолезногоИскопаемого = Элемент.Ф300_07_L;
				НоваяСтрокаДанных.ЕдиницаИзмерения          = Элемент.Ф300_07_M;
	...
	// сводная таблица
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("НомерСФ"), 		"НомерСФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ДатаСФ"), 			"ДатаСФ");	
    ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ВидСФПрав"),		"ВидСФПрав");
    ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СтоимостьБезНДС"),	"СтоимостьБезНДС");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СуммаНДССФ"), 		"СуммаНДССФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СуммаНДСНачисленного"),"СуммаНДСНачисленного");
    ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхD.ВыгрузитьКолонку("ВидПолезногоИскопаемого"),	  "ВидПолезногоИскопаемого");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхD.ВыгрузитьКолонку("ОбъемПолезногоИскопаемого"), "ОбъемПолезногоИскопаемого");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхD.ВыгрузитьКолонку("ЕдиницаИзмерения"),          "ЕдиницаИзмерения");
...
&НаКлиенте
Процедура Форма300_08ПолеТабличногоДокументаЛистDВыбор(Элемент, Область, СтандартнаяОбработка)
	Ячейка = Элемент.ТекущаяОбласть.Имя;
	Если (Найти(Ячейка, "Ф300_08") = 0) Тогда
		Возврат;
	КонецЕсли;
	...
	Если (Найти(Ячейка, "Ф300_08") > 0) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("Форма300_08ОбработкаВыбораЗначений", ЭтотОбъект, ПараметрыВыбора);
		Расчет = Истина;
		Если (Найти(Ячейка, "Ф300_08_K__") > 0) Тогда
			НачЗначение = Элемент.ТекущаяОбласть.Значение;			
			ПоказатьВводСтроки(ОписаниеОповещения, НачЗначение, "Вид полезного ископаемого", 50); 
		...
		ИначеЕсли (Найти(Ячейка, "Ф300_08_M__") > 0) Тогда			
			НачЗначение = Элемент.ТекущаяОбласть.Значение;
			ПоказатьВводСтроки(ОписаниеОповещения, НачЗначение, "Единица измерения", 12);
					...
					Пока СтрокаОтчетаПоНомеру.Следующий() Цикл						
						...
						Пока СтрокаОтчета.Следующий() Цикл
								...
								Если ИмяГруппы = "ЛистA" Тогда
									...
									Элемент.Ф300_08_D_Расшифровка = СтрокаОтчета.Контрагент;
								...
								ИначеЕсли ИмяГруппы = "ЛистD" Тогда
									Элемент.Ф300_08_A4 = Формат(НомерСтроки, мФорматНомераСтроки);
...
Функция Форма300_08ПолучитьТаблицуДанныхРеестраExcel(ТекКодФормы)	
	...
	ТаблицаДанныхA.Колонки.Добавить("НомерСтроки",,               "Номер строки");
	ТаблицаДанныхA.Колонки.Добавить("СтатусПоставщика",,          "Статус поставщика");
	ТаблицаДанныхA.Колонки.Добавить("РННПоставщика",,             "РНН поставщика");
	...
	ТаблицаДанныхC.Колонки.Добавить("СтоимостьБезНДС");
	ТаблицаДанныхC.Колонки.Добавить("СуммаНДССФ");
	ТаблицаДанныхC.Колонки.Добавить("СуммаНДСВЗачет");
		...
		Для Каждого Элемент Из ТаблицаСтрокГруппы Цикл
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				Если ЗначениеЗаполнено(Элемент.Ф300_08_C) Тогда 
					НоваяСтрокаДанных.РННПоставщика    = Элемент.Ф300_08_C;
				КонецЕсли;
				Если ЗначениеЗаполнено(Элемент.Ф300_08_D) Тогда 
					НоваяСтрокаДанных.ИННБИНПоставщика = Элемент.Ф300_08_D;
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистB" Тогда
				...
				НоваяСтрокаДанных.НомерСФ 		   = Элемент.Ф300_08_E;
				Если ЗначениеЗаполнено(Элемент.Ф300_08_F) Тогда
					НоваяСтрокаДанных.ДатаСФ	   = НачалоДня(Элемент.Ф300_08_F);
				КонецЕсли;
				Если ЗначениеЗаполнено(Элемент.Ф300_08_G) Тогда
					НоваяСтрокаДанных.ВидСФПрав	   = Элемент.Ф300_08_G;
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистC" Тогда
				...
				НоваяСтрокаДанных.СтоимостьБезНДС = Элемент.Ф300_08_H;
				НоваяСтрокаДанных.СуммаНДССФ 	  = Элемент.Ф300_08_I;
				НоваяСтрокаДанных.СуммаНДСВЗачет  = Элемент.Ф300_08_J;
				
			ИначеЕсли ИмяГруппы = "ЛистD" Тогда
				
				НоваяСтрокаДанных = ТаблицаДанныхD.Добавить();	

				НоваяСтрокаДанных.ВидПолезногоИскопаемого	= Элемент.Ф300_08_K;
				НоваяСтрокаДанных.ОбъемПолезногоИскопаемого = Элемент.Ф300_08_L;
				НоваяСтрокаДанных.ЕдиницаИзмерения          = Элемент.Ф300_08_M;
	...
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СтоимостьБезНДС"),           "СтоимостьБезНДС");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СуммаНДССФ"),                "СуммаНДССФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СуммаНДСВЗачет"),            "СуммаНДСВЗачет");
    ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхD.ВыгрузитьКолонку("ВидПолезногоИскопаемого"),	  "ВидПолезногоИскопаемого");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхD.ВыгрузитьКолонку("ОбъемПолезногоИскопаемого"), "ОбъемПолезногоИскопаемого");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхD.ВыгрузитьКолонку("ЕдиницаИзмерения"),          "ЕдиницаИзмерения");
...
&НаКлиенте
Процедура Форма300_09ЛистAВыбор(Элемент, Область, СтандартнаяОбработка)	
	...
	ИмяЯчейкиБезПрефиксаЧленаТС = Форма300_09ИмяЯчейкиБезПрефиксаЧленаТС(Ячейка, ТаблицаКодовИПрефиксовЧленовТС);
		
	Если (Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_B__") > 0) Тогда
		
		СтандартнаяОбработка = Ложь;
		НачЗначение = Элемент.ТекущаяОбласть.Значение;
		ПоказатьВводСтроки(ОписаниеОповещения, НачЗначение, "Номер отметки налогового органа в Заявлении"); 
		
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, Ячейка, НачЗначение,, Расчет);
		
	ИначеЕсли (Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_C__") > 0) Тогда			
		
		СтандартнаяОбработка = Ложь;
		НачЗначение = Элемент.ТекущаяОбласть.Значение;
		ПоказатьВводДаты(ОписаниеОповещения, НачЗначение, "Дата отметки налогового органа в Заявлении", ЧастиДаты.Дата); 
		
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, Ячейка, НачЗначение,, Расчет);		
				
	ИначеЕсли (Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_D__") > 0) Тогда
		...
		//
		//ИмяЯчейкиДляРасшифорвки = Лев(Ячейка, Найти(Ячейка, "__") - 1);
		//ОткрытьРасшифровкуЯчейки(Ячейка, ИмяЯчейкиДляРасшифорвки, ТаблицаРасшифровки, ФормаВыбора);
...
&НаКлиенте
Процедура Форма300_09ЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_E__") > 0 
		ИЛИ Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_F__") > 0 Тогда		
...
&НаКлиенте
Процедура Форма300_09ЛистCВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_G__") > 0 Тогда		
	...
	ИначеЕсли (Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_H__") > 0) Тогда
		...
		РасшифровкаГ = НСтр(
		"ru = 'Счет-фактура выписан резиденту государства, не являющегося членом Таможенного союза, 
		|а грузополучателем товара на территории государства-члена Таможенного союза 
		|является резидент этого государства-члена Таможенного союза.'");
		
		РасшифровкаР = НСтр(
		"ru = 'Счет-фактура выписан по работам по переработке давальческого сырья,
		|ввезенного на территорию Республики Казахстан с территории 
		|другого государства-члена Таможенного союза с последующим вывозом 
		|продуктов переработки на территорию другого государства'");
		
		СписокВыбора = Новый СписокЗначений;
		СписокВыбора.Добавить("", "Пустое значение");
		СписокВыбора.Добавить("Э", "Счет-фактура выписан по экспортированным в государства-члены Таможенного союза товарам.");
		СписокВыбора.Добавить("Г", РасшифровкаГ);
		СписокВыбора.Добавить("Р", РасшифровкаР);
		
		СписокВыбора.ПоказатьВыборЭлемента(ОписаниеОповещения, "Выберите вид оборота, облагаемого по нулевой ставке");
		
	ИначеЕсли (Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_I__") > 0) Тогда
		
		НачЗначение = Элемент.ТекущаяОбласть.Значение;			
		ПоказатьВводЧисла(ОписаниеОповещения, НачЗначение, "Всего размер облагаемого оборота по счету-фактуре", 12, 0); 
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, Ячейка, НачЗначение,, Расчет);
...
&НаКлиенте
Процедура Форма300_09_01ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)	
	Форма300_09ЛистAВыбор(Элемент, Область, СтандартнаяОбработка);	
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_01ПолеТабличногоДокументаЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистBВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_01ПолеТабличногоДокументаЛистCВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистCВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_02ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистAВыбор(Элемент, Область, СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_02ПолеТабличногоДокументаЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистBВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_02ПолеТабличногоДокументаЛистCВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистCВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_03ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистAВыбор(Элемент, Область, СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_03ПолеТабличногоДокументаЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистBВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_03ПолеТабличногоДокументаЛистCВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистCВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_04ПолеТабличногоДокументаЛистAВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистAВыбор(Элемент, Область, СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_04ПолеТабличногоДокументаЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистBВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры

&НаКлиенте
Процедура Форма300_09_04ПолеТабличногоДокументаЛистCВыбор(Элемент, Область, СтандартнаяОбработка)
	Форма300_09ЛистCВыбор(Элемент, Область, СтандартнаяОбработка);			
КонецПроцедуры
...
Процедура Форма300_09ОбработкаВыбораЗначенийНаСервере(Результат, ПараметрыВыбора)
	...
	Если ПараметрыВыбора.ЭлементВыбора = "Контрагент" И ЗначениеЗаполнено(Результат) Тогда
		...
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ПараметрыВыбора.ИмяГруппы, ПараметрыВыбора.Ячейка, Результат.НомерНалоговойРегистрацииВСтранеРезидентства, Результат, ПараметрыВыбора.ПризнакРасчета);
		
	ИначеЕсли ПараметрыВыбора.ЭлементВыбора = "СчетФактура" И ЗначениеЗаполнено(Результат) Тогда	
		
		// Номер СФ
		ИмяЯчейки = Лев(ПараметрыВыбора.Ячейка, 10) + "_E__" + НомерЯчейки;
		НомерСФ = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Результат.Номер, Результат, ПредставлениеНомераДокументов);
		НомерСФ = ИсключитьЗапрещенныеСимволыИзНомераСФ(НомерСФ, ИсключатьНецифровыеСимволы300_09, ИсключатьЛидирующиеНули300_09);		
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ПараметрыВыбора.ИмяГруппы, ИмяЯчейки, НомерСФ, Результат, Ложь);			
		
		// Дата СФ
		ИмяЯчейки = Лев(ПараметрыВыбора.Ячейка, 10) + "_F__" + НомерЯчейки;
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ПараметрыВыбора.ИмяГруппы, ИмяЯчейки, Результат.Дата, Результат, Ложь);
		
		// Дата оборота СФ
		ИмяЯчейки = Лев(ПараметрыВыбора.Ячейка, 10) + "_G__" + НомерЯчейки;
		Форма300_09ПолеТабличногоДокументаВыборНаСервере(ПараметрыВыбора.ИмяГруппы, ИмяЯчейки, Результат.ДатаСовершенияОборотаПоРеализации, Результат, Ложь);
...
&НаСервере
Функция Форма300_09РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь, Знач КодЧленаТС) 
    ...
    ЗапросПоСтранам = Новый Запрос;
	ЗапросПоСтранам.УстановитьПараметр("Код", КодЧленаТС);
	...
	Запрос.УстановитьПараметр("ПустаяДата", Дата(1,1,1));
	Запрос.УстановитьПараметр("ПустаяСтрока", "");
	Запрос.УстановитьПараметр("ДатаПодтвержденияРеализацииВЕАЭС", ДатаПодтвержденияРеализацииВЕАЭС300_09);
	...
	ТекстЗапроса = " ВЫБРАТЬ
	               ...
	               |	СчетФактура
	               |; 
				   |" + ТекстЗапросаСчетаФактурыВыданныеИДокументыОснования(Ложь) + Символы.ПС + ТекстЗапросаПоДаннымСчетовФактур + "
				   | 					  				   
				   |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ДанныеСчетовФактур.Номер,
	               |	ДанныеСчетовФактур.Дата,
				   |	ПодтверждениеРеализацииВЕАЭС.ДатаПодтверждения КАК ДатаОтметки,
				   |	ПодтверждениеРеализацииВЕАЭС.НомерПодтверждения КАК НомерОтметки,
	               |	ДанныеСчетовФактур.Покупатель КАК Контрагент,
	               |	ДанныеСчетовФактур.Ссылка КАК СФ,
	               |	ДанныеСчетовФактур.Дополнительная,
	               |	ДанныеСчетовФактур.НомерНалоговойРегистрацииВСтранеРезидентства,
	               |	ВЫБОР
	               |		КОГДА ДанныеСчетовФактур.Покупатель.СтранаРезидентства <> ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.Казахстан) И ДанныеСчетовФактур.Покупатель.СтранаРезидентства.УчастникТаможенногоСоюза = ЗНАЧЕНИЕ(Справочник.СтраныУчастникиТаможенногоСоюза.ПустаяСсылка)
	               |			ТОГДА ""Г""				   
	               |		КОГДА ТИПЗНАЧЕНИЯ(ДокументОснованиеВТ.Ссылка) = ТИП(Документ.РеализацияУслугПоПереработке)
	               |			ТОГДА ""Р""
	               |		ИНАЧЕ ""Э""
	               |	КОНЕЦ КАК ВидОборота,
				   |	МИНИМУМ(ДанныеСчетовФактур.ОборотПоРеализации) КАК СуммаОборот,
	               |	СУММА(ЕстьNull(ДокументОснованиеВТ.СуммаОборот, 0)) КАК СуммаОборотПоРегистру,
				   |	ДанныеСчетовФактур.УчастникСовместнойДеятельности КАК УчастникСовместнойДеятельности,
				   |	ЕСТЬNULL(АктуальныеЭСФ.ЭСФ.РегистрационныйНомер, &ПустаяСтрока) Как РегистрационныйНомерЭСФ,
				   |	НАЧАЛОПЕРИОДА(ДанныеСчетовФактур.Ссылка.ДатаСовершенияОборотаПоРеализации) КАК ДатаСовершенияОборота
	               |ИЗ
	               |	ДанныеСчетовФактур КАК ДанныеСчетовФактур
				   ...
				   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПодтверждениеРеализацииВЕАЭС КАК ПодтверждениеРеализацииВЕАЭС
				   |		ПО ДанныеСчетовФактур.Ссылка = ПодтверждениеРеализацииВЕАЭС.ДокументРеализации
				   |		И ПодтверждениеРеализацииВЕАЭС.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРеализацийВЕАЭС.ПодтвержденаРеализация)
				   |		И ПодтверждениеРеализацииВЕАЭС.ДатаПодтверждения <= &ДатаПодписи
				   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АктуальныеЭСФ КАК АктуальныеЭСФ
				   |		ПО ДанныеСчетовФактур.Ссылка = АктуальныеЭСФ.СчетФактура
	               |ГДЕ
	               |	ДанныеСчетовФактур.Ссылка.Проведен = ИСТИНА";
				   				    	 
	Если ПризнакПокупателя300_09 Тогда
		ТекстЗапроса = ТекстЗапроса + "
				   |	И НЕ ДанныеСчетовФактур.НомерНалоговойРегистрацииВСтранеРезидентства  = &ПустаяСтрока";
	КонецЕсли;

	Если ОтображатьТолькоПодтвержденныеСчетаФактуры300_09 Тогда
		ТекстЗапроса = ТекстЗапроса + "
				   |	И НЕ ПодтверждениеРеализацииВЕАЭС.ДокументРеализации ЕСТЬ NULL";
	КонецЕсли;

	Если ОтображатьСчетаФактурыПодтвержденныеПосле300_09 Тогда
		ТекстЗапроса = ТекстЗапроса + "
				   |	И ПодтверждениеРеализацииВЕАЭС.ДатаПодтверждения >= &ДатаПодтвержденияРеализацииВЕАЭС";
	КонецЕсли;
	...
	ТекстЗапроса = ТекстЗапроса + " 		   
	               ...
	               |	КОНЕЦ,
				   |	ДанныеСчетовФактур.УчастникСовместнойДеятельности, 
				    |	АктуальныеЭСФ.ЭСФ.РегистрационныйНомер,
				   |	НАЧАЛОПЕРИОДА(ДанныеСчетовФактур.Ссылка.ДатаСовершенияОборотаПоРеализации)
				   |
				   | УПОРЯДОЧИТЬ ПО
	 			   |	ДанныеСчетовФактур.Дата ";  
...
&НаКлиенте
Процедура Форма300_09ЗаполнитьАвто(Перезаполнить = Ложь, Знач КодЧленаТС)
	
	Состояние("Расчет формы 300.09");		
	
	Форма300_09ЗаполнитьАвтоНаСервере(Перезаполнить, КодЧленаТС);		
...
&НаСервере
Процедура Форма300_09ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь, Знач КодЧленаТС)	
	
	ТекИмяФормы = Форма300_09ИмяФормыПоКодуЧленаТС(КодЧленаТС, ТаблицаКодовИПрефиксовЧленовТС);		
	...
	ВыборкаОтчета = Форма300_09РассчитатьДанныеФормы(Истина, КодЧленаТС);
	
	Если ВыборкаОтчета = Неопределено Тогда 	
		Возврат
	КонецЕсли;
	
	// Создать префикс ячейки, например, для КодЧленаТС = "643" префикс ячейки будет "Ф300_09_01_".
	ПрефиксЯчейки = "";
	Для Каждого СтрокаТаблицаКодовИПрефиксовЧленовТС Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		Если СтрокаТаблицаКодовИПрефиксовЧленовТС.КодЧленаТС = КодЧленаТС Тогда
			ПрефиксЯчейки = "Ф300_09_" + СтрокаТаблицаКодовИПрефиксовЧленовТС.ПрефиксЧленаТС + "_";  
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если ПрефиксЯчейки = "" Тогда
		ВызватьИсключение НСтр("ru = 'Внутренняя ошибка. Не удалось сформировать имя ячейки при заполнении приложения 300.09.'");
	КонецЕсли;
			...
			ВыборкаОтчета.Сбросить();
		 	...
		 	Пока ВыборкаОтчета.Следующий() Цикл
				...
				Если ИмяГруппы = "ЛистA" Тогда
					...
					Элемент[ПрефиксЯчейки + "A"] = Формат(НомерСтроки, мФорматНомераСтроки);
					Элемент[ПрефиксЯчейки + "B"] = СокрЛП(ВыборкаОтчета.НомерОтметки);
					Элемент[ПрефиксЯчейки + "C"] = СокрЛП(ВыборкаОтчета.ДатаОтметки);
					Элемент[ПрефиксЯчейки + "D"] = ВыборкаОтчета.НомерНалоговойРегистрацииВСтранеРезидентства;
					Элемент[ПрефиксЯчейки + "D_Расшифровка"] =  ВыборкаОтчета.Контрагент;
					
				ИначеЕсли ИмяГруппы = "ЛистB" Тогда
					Если УказыватьРерНомерЭСФ300_09 и ЗначениеЗаполнено(ВыборкаОтчета.РегистрационныйНомерЭСФ) 
					Тогда
					    НомерСФ = ВыборкаОтчета.РегистрационныйНомерЭСФ;
					Иначе
						НомерСФ =  ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаОтчета.Номер, ВыборкаОтчета.СФ, ПредставлениеНомераДокументов);
						НомерСФ = ИсключитьЗапрещенныеСимволыИзНомераСФ(НомерСФ, ИсключатьНецифровыеСимволы300_09, ИсключатьЛидирующиеНули300_09);		
					КонецЕсли;
					Элемент[ПрефиксЯчейки + "A2"] = Формат(НомерСтроки, мФорматНомераСтроки);
					
					Элемент[ПрефиксЯчейки + "E"] = НомерСФ;
					Элемент[ПрефиксЯчейки + "E_Расшифровка"] = ВыборкаОтчета.СФ;
					
					Элемент[ПрефиксЯчейки + "F"] = ВыборкаОтчета.Дата;
					Элемент[ПрефиксЯчейки + "F_Расшифровка"] = ВыборкаОтчета.СФ;
					
				ИначеЕсли ИмяГруппы = "ЛистC" Тогда
					Элемент[ПрефиксЯчейки + "A3"] = Формат(НомерСтроки, мФорматНомераСтроки);
					
					Элемент[ПрефиксЯчейки + "G"] = ВыборкаОтчета.ДатаСовершенияОборота;	
					Элемент[ПрефиксЯчейки + "G_Расшифровка"] = ВыборкаОтчета.СФ;
					
					Элемент[ПрефиксЯчейки + "H"] = ВыборкаОтчета.ВидОборота;
				 	Элемент[ПрефиксЯчейки + "I"] = Окр(ВыборкаОтчета.СуммаОборот);
...
&НаКлиенте
Процедура ОтображатьТолькоПодтвержденныеСчетаФактурыПриИзменении(Элемент)
	Если НЕ ОтображатьТолькоПодтвержденныеСчетаФактуры300_09 Тогда
		ОтображатьСчетаФактурыПодтвержденныеПосле300_09 = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтображатьСчетаФактурыПодтвержденныеПослеПриИзменении(Элемент)
	УстановитьДоступностьПодтверждениеРеализацииВЕАЭС(ЭтаФорма);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьПодтверждениеРеализацииВЕАЭС(Форма)
	Форма.Элементы.ОтображатьСчетаФактурыПодтвержденныеПосле300_09_01.Доступность = Форма.ОтображатьТолькоПодтвержденныеСчетаФактуры300_09;
	Форма.Элементы.ОтображатьСчетаФактурыПодтвержденныеПосле300_09_02.Доступность = Форма.ОтображатьТолькоПодтвержденныеСчетаФактуры300_09;
	Форма.Элементы.ОтображатьСчетаФактурыПодтвержденныеПосле300_09_03.Доступность = Форма.ОтображатьТолькоПодтвержденныеСчетаФактуры300_09;
	Форма.Элементы.ОтображатьСчетаФактурыПодтвержденныеПосле300_09_04.Доступность = Форма.ОтображатьТолькоПодтвержденныеСчетаФактуры300_09;
	
	Форма.Элементы.ДатаПодтвержденияРеализацииВЕАЭС300_09_01.Доступность = Форма.ОтображатьСчетаФактурыПодтвержденныеПосле300_09;
	Форма.Элементы.ДатаПодтвержденияРеализацииВЕАЭС300_09_02.Доступность = Форма.ОтображатьСчетаФактурыПодтвержденныеПосле300_09;
	Форма.Элементы.ДатаПодтвержденияРеализацииВЕАЭС300_09_03.Доступность = Форма.ОтображатьСчетаФактурыПодтвержденныеПосле300_09;
	Форма.Элементы.ДатаПодтвержденияРеализацииВЕАЭС300_09_04.Доступность = Форма.ОтображатьСчетаФактурыПодтвержденныеПосле300_09;	
КонецПроцедуры

///////////////////////////////////////////////////////////////////////////////
// Функции для работы с префиксами "_0X" в именами форм, элементов и т.д. формы 300.09.

&НаСервере
Функция Форма300_09НоваяТаблицаКодовИПрефиксовЧленовТС()
	
	ТабЗнач = ТаблицаКодовИПрефиксовЧленовТС;
	
	ТабЗнач.Очистить();
	
	// Россия.
	СтрокаТабЗнач =  ТабЗнач.Добавить();
	СтрокаТабЗнач.КодЧленаТС = "643";
	СтрокаТабЗнач.ПрефиксЧленаТС = "01"; 
	
	// Беларусь.
	СтрокаТабЗнач =  ТабЗнач.Добавить();
	СтрокаТабЗнач.КодЧленаТС = "112";
	СтрокаТабЗнач.ПрефиксЧленаТС = "02";
	
	// Армения.
	СтрокаТабЗнач =  ТабЗнач.Добавить();
	СтрокаТабЗнач.КодЧленаТС = "051";
	СтрокаТабЗнач.ПрефиксЧленаТС = "03";
	
	// Киргизия.
	СтрокаТабЗнач =  ТабЗнач.Добавить();
	СтрокаТабЗнач.КодЧленаТС = "417";
	СтрокаТабЗнач.ПрефиксЧленаТС = "04";
...
&НаКлиентеНаСервереБезКонтекста
Функция Форма300_09КодЧленаТСПоИмениФормы(Знач ПараметрИмяФормы, Знач ТаблицаКодовИПрефиксовЧленовТС)
	
	НайденныйКодЧленаТС = Неопределено;
		
	Для Каждого СтрокаТаблицы Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		Если ПараметрИмяФормы = Форма300_09ИмяФормыВключаяПрефиксЧленаТС(СтрокаТаблицы.ПрефиксЧленаТС) Тогда 
			НайденныйКодЧленаТС = СтрокаТаблицы.КодЧленаТС;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если НайденныйКодЧленаТС = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось найти код государства-члена Таможенного союза по имени формы.'");
	КонецЕсли;
	
	Возврат НайденныйКодЧленаТС;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция Форма300_09ИмяФормыПоКодуЧленаТС(Знач ПараметрКодЧленаТС, Знач ТаблицаКодовИПрефиксовЧленовТС)
	
	НайденноеИмяФормы = Неопределено;
	
	Для Каждого СтрокаТаблицы Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		Если СтрокаТаблицы.КодЧленаТС = ПараметрКодЧленаТС Тогда
			НайденноеИмяФормы = Форма300_09ИмяФормыВключаяПрефиксЧленаТС(СтрокаТаблицы.ПрефиксЧленаТС);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если НайденноеИмяФормы = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось найти имя формы по коду государства-члена Таможенного союза.'");
	КонецЕсли;
	
	Возврат НайденноеИмяФормы;
	
КонецФункции

// Заменяет имена узлов и атрибутов с "form_300_09_0X" на "form_300_09".
//
&НаКлиентеНаСервереБезКонтекста
Процедура Форма300_09УдалитьПрефиксыЧленовТСИзXML(XMLФорма300, Знач ТаблицаКодовИПрефиксовЧленовТС)
	
	Для Каждого СтрокаТаблицы Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		ПрефиксУзлаВключаяПрефиксЧленаТС = "form_300_09_" + СтрокаТаблицы.ПрефиксЧленаТС;
		XMLФорма300 = СтрЗаменить(XMLФорма300, ПрефиксУзлаВключаяПрефиксЧленаТС, "form_300_09");
	КонецЦикла;
	
КонецПроцедуры

// Исключение префиксов из имен и добавление префиксов в имена.

// Для "Ф300_09_0X_A__1" возвращает "Ф300_09_A__1".
&НаКлиентеНаСервереБезКонтекста
Функция Форма300_09ИмяЯчейкиБезПрефиксаЧленаТС(Знач ИмяЯчейки, Знач ТаблицаКодовИПрефиксовЧленовТС)
	
	ИмяЯчейкиБезПрефиксаЧленаТС = Неопределено;
	
	Для Каждого СтрокаТаблицы Из ТаблицаКодовИПрефиксовЧленовТС Цикл
		ПрефиксЯчейкиВключаяПрефиксЧленаТС = Форма300_09ПрефиксЯчейкиВключаяПрефиксЧленаТС(СтрокаТаблицы.ПрефиксЧленаТС);
		Если Найти(ИмяЯчейки, ПрефиксЯчейкиВключаяПрефиксЧленаТС) Тогда
			ИмяЯчейкиБезПрефиксаЧленаТС = СтрЗаменить(ИмяЯчейки, ПрефиксЯчейкиВключаяПрефиксЧленаТС, "Ф300_09");
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ИмяЯчейкиБезПрефиксаЧленаТС = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось найти имя ячейки с общим префиксом по имени текущей ячейки. Возможно, текущая ячейка имеет некорректное имя.'");
	КонецЕсли;
	
	Возврат ИмяЯчейкиБезПрефиксаЧленаТС;	
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция Форма300_09ИмяФормыВключаяПрефиксЧленаТС(Знач ПрефиксЧленаТС)
	
	Возврат "Форма300_09_" + ПрефиксЧленаТС;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция Форма300_09ПрефиксЯчейкиВключаяПрефиксЧленаТС(Знач ПрефиксЧленаТС)
	
	Возврат "Ф300_09_" + ПрефиксЧленаТС;
	
КонецФункции

&НаСервере
Функция Форма300_09ПолучитьТаблицуДанныхРеестраExcel(ТекКодФормы)
	СтруктураФормы = РегламентированнаяОтчетность.ПолучитьМногострочнуюСтруктуру(ЭтаФорма, ТекКодФормы);
	
	ТаблицаДанныхA = Новый ТаблицаЗначений;              
	Если ТекКодФормы = "Форма300_09_01" Тогда
		ВариантФормы = "01";
		СтранаРеализации = "РОССИЯ";
	ИначеЕсли ТекКодФормы = "Форма300_09_02" Тогда
		ВариантФормы = "02";
		СтранаРеализации = "БЕЛАРУСЬ";
	ИначеЕсли ТекКодФормы = "Форма300_09_03" Тогда
		ВариантФормы = "03";
		СтранаРеализации = "АРМЕНИЯ";
	ИначеЕсли ТекКодФормы = "Форма300_09_04" Тогда
		ВариантФормы = "04";
		СтранаРеализации = "КИРГИЗИЯ";		
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	ТаблицаДанныхA = Новый ТаблицаЗначений;              
	ТаблицаДанныхA.Колонки.Добавить("НомерСтроки",,				"Номер строки");
	ТаблицаДанныхA.Колонки.Добавить("НомерОтметки",,			"№ отметки налогового органа в Заявлении");
	ТаблицаДанныхA.Колонки.Добавить("ДатаОтметки",,             "Дата отметки налогового органа в Заявлении");
	ТаблицаДанныхA.Колонки.Добавить("ИдентификаторПокупателя",, "Идентификационный код (номер) покупателя/получателя государства-члена Таможенного союза");
	ТаблицаДанныхA.Колонки.Добавить("НомерСФ",,					"Номер счета-фактуры");
	ТаблицаДанныхA.Колонки.Добавить("ДатаСФ",,					"Дата выписки счета - фактуры");
	ТаблицаДанныхA.Колонки.Добавить("ДатаОборотаПоРеализации",, "Дата совершения оборота по реализации");
	ТаблицаДанныхA.Колонки.Добавить("ВидОборота",,				"Вид оборота");	
	ТаблицаДанныхA.Колонки.Добавить("СуммаНДССФ",,				"Всего размер облагаемого оборота по счету-фактуре");
	
	ТаблицаДанныхB = Новый ТаблицаЗначений;
	ТаблицаДанныхB.Колонки.Добавить("НомерСФ",,					"Номер счета-фактуры");
	ТаблицаДанныхB.Колонки.Добавить("ДатаСФ",,					"Дата выписки счета - фактуры");
	
	ТаблицаДанныхC = Новый ТаблицаЗначений;
	ТаблицаДанныхC.Колонки.Добавить("ДатаОборотаПоРеализации",,	"Дата совершения оборота по реализации");
	ТаблицаДанныхC.Колонки.Добавить("ВидОборота",,				"Вид оборота");	
	ТаблицаДанныхC.Колонки.Добавить("СуммаНДССФ",,				"Всего размер облагаемого оборота по счету-фактуре");
	...
	Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
		...
		Для Каждого Элемент Из ТаблицаСтрокГруппы Цикл
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				//Так должны заполнять по отдельности значения выборки 
				//для каждого листа
				НоваяСтрокаДанных.НомерСтроки = Элемент["Ф300_09_" + ВариантФормы + "_A"];
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_B"]) Тогда
					НоваяСтрокаДанных.НомерОтметки = Элемент["Ф300_09_" + ВариантФормы + "_B"];
				КонецЕсли;
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_C"]) Тогда
					НоваяСтрокаДанных.ДатаОтметки = НачалоДня(Элемент["Ф300_09_" + ВариантФормы + "_C"]);
				КонецЕсли;
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_D"]) Тогда
					НоваяСтрокаДанных.ИдентификаторПокупателя = Элемент["Ф300_09_" + ВариантФормы + "_D"];
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистB" Тогда
				...
				НоваяСтрокаДанных.НомерСФ = Элемент["Ф300_09_" + ВариантФормы + "_E"];				
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_F"]) Тогда
					НоваяСтрокаДанных.ДатаСФ = НачалоДня(Элемент["Ф300_09_" + ВариантФормы + "_F"]);
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистC" Тогда
				...
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_G"]) Тогда
					НоваяСтрокаДанных.ДатаОборотаПоРеализации = НачалоДня(Элемент["Ф300_09_" + ВариантФормы + "_G"])
				КонецЕсли;
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_H"]) Тогда
					НоваяСтрокаДанных.ВидОборота = Элемент["Ф300_09_" + ВариантФормы + "_H"];
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Элемент["Ф300_09_" + ВариантФормы + "_I"]) Тогда
					НоваяСтрокаДанных.СуммаНДССФ = Элемент["Ф300_09_" + ВариантФормы + "_I"];
				КонецЕсли;
	...
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("НомерСФ"),                 "НомерСФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ДатаСФ"),                  "ДатаСФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("ДатаОборотаПоРеализации"),	"ДатаОборотаПоРеализации");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("ВидОборота"),              "ВидОборота");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("СуммаНДССФ"),              "СуммаНДССФ");
...
&НаСервере
Функция Форма300_09ПолучитьСтруктуруСоответствияПолейРеестраExcel()
	
	СтруктураСоответствияПолей = Новый Структура();
	
	СтруктураСоответствияПолей.Вставить("НомерСтроки", 				1);
	СтруктураСоответствияПолей.Вставить("НомерОтметки", 			2);
	СтруктураСоответствияПолей.Вставить("ДатаОтметки",  			3);
	СтруктураСоответствияПолей.Вставить("ИдентификаторПокупателя",  4);
	СтруктураСоответствияПолей.Вставить("НомерСФ", 					5);
	СтруктураСоответствияПолей.Вставить("ДатаСФ", 					6);
	СтруктураСоответствияПолей.Вставить("ДатаОборотаПоРеализации", 	7);
	СтруктураСоответствияПолей.Вставить("ВидОборота", 				8);
	СтруктураСоответствияПолей.Вставить("СуммаНДССФ", 				9);
...
&НаСервере
Функция Форма300_09ПолучитьСтруктуруИтоговыхПолейРеестраExcel()
	СтруктураИтоговыхПолей = Новый Структура;
	СтруктураИтоговыхПолей.Вставить("СуммаНДССФ", 9);	
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002018Кв1");
...
Процедура Форма300РасшифровкаСКД(ИмяЯчейки)
	...
	Если (Найти(ИмяЯчейки, "300.00") > 0 И СокращенныйКод >= 13 И СокращенныйКод <= 29)
	...
	ИначеЕсли Найти(ИмяЯчейки, "300.00.018") >0 Тогда	
		ОтображатьПоказательСуммаНДС 			= Ложь;
		ОтображатьПоказательСуммаВДекларации 	= Ложь;	
	//ИначеЕсли Найти(ИмяЯчейки, "300.00.029") >0 Тогда	
...
Функция ПолучитьТаблицуДанныхРеестраExcel(ИмяФормы)
	...
	ИначеЕсли Найти(ИмяФормы, "Форма300_09")>0 Тогда
		Возврат Форма300_09ПолучитьТаблицуДанныхРеестраExcel(ИмяФормы);			
	КонецЕсли;	
КонецФункции	
...
Функция ПолучитьСтруктуруСоответствияПолейРеестраExcel(ИмяФормы)
	...
	ИначеЕсли Найти(ИмяФормы, "Форма300_09")>0 Тогда
		Возврат Форма300_09ПолучитьСтруктуруСоответствияПолейРеестраExcel();	
	КонецЕсли;	
КонецФункции
...
Функция ПолучитьСтруктуруИтоговыхПолейРеестраExcel(ИмяФормы)
	...
	ИначеЕсли Найти(ИмяФормы, "Форма300_09")>0 Тогда
		Возврат Форма300_09ПолучитьСтруктуруИтоговыхПолейРеестраExcel();			
	КонецЕсли;	
КонецФункции
...
Функция ПолучитьПараметрыВыгрузкиВExcel(ИмяФормы)
	...
	ИначеЕсли Найти(ИмяФормы, "Форма300_09")>0 Тогда
		ПараметрыВыгрузки.ИмяФайлаШаблона = "SONO_300_09_2016.xls";
		ПараметрыВыгрузки.ИмяЛистаШаблона = "300.09.Data1";
		ПараметрыВыгрузки.НомерПервойСтроки = 2;
		Если ИмяФормы = "Форма300_09_01" Тогда		
			СтранаРеализации = "РОССИЯ";
		ИначеЕсли ИмяФормы = "Форма300_09_02" Тогда			
			СтранаРеализации = "БЕЛАРУСЬ";
		ИначеЕсли ИмяФормы = "Форма300_09_03" Тогда			
			СтранаРеализации = "АРМЕНИЯ";
		ИначеЕсли ИмяФормы = "Форма300_09_04" Тогда			
			СтранаРеализации = "КИРГИЗИЯ";			
		Иначе
			СтранаРеализации = "";
		КонецЕсли;
		ПараметрыВыгрузки.ЗаголовокРеестра = "" + СтранаРеализации + ". ПЕРЕЧЕНЬ ЗАЯВЛЕНИЙ О ВВОЗЕ ТОВАРОВ И УПЛАТЕ КОСВЕННЫХ НАЛОГОВ";
	КонецЕсли;	
...
//
&НаСервереБезКонтекста
Процедура ДобавитьСтрокуМногострочнойФормы(Форма) 
	...
	СтруктураМногострочныхРазделов = Форма.РеквизитФормыВЗначение("мСтруктураМногострочныхРазделов");	
	СтрокаФормы = СтруктураМногострочныхРазделов.Строки.Найти(ТекущееИмяФормы,"КодФормы");	
	    	
	ИмяПанелиФормы = "Панель" + ТекущееИмяФормы;
	СписокФормБезИерархии = Форма.РеквизитФормыВЗначение("мСписокФормБезИерархии");                 
	
	ТекФормыВСпискеБезИерархии = СписокФормБезИерархии.Строки.Найти(ТекущееИмяФормы,"КодФормы");
	...
	Для Каждого Страница Из Форма.Элементы[ИмяПанелиФормы].ПодчиненныеЭлементы Цикл
		...
		Если ТекущееИмяФормы = "Форма300_05" И ИмяГруппы = "ЛистA" Тогда
			Если ТаблицаСтрокГруппы.Количество() = 0 Тогда
		...
		РегламентированнаяОтчетность.ВывестиРазделВТабличныйДокументФормы(Форма, ТекФормыВСпискеБезИерархии.ВсегоСтраниц, ИмяГруппы, Новый Структура("ШапкаИПодвал, Строки", ОбщиеЭлементыГруппы, ТаблицаСтрокГруппы), ТекущееИмяФормы, ТекФормыВСпискеБезИерархии.ВсегоСтраниц);
		Если ТекущееИмяФормы = "Форма300_05" И ИмяГруппы = "ЛистA" Тогда
	...
	Форма.ЗначениеВРеквизитФормы(СтруктураМногострочныхРазделов, "мСтруктураМногострочныхРазделов");
	РегламентированнаяОтчетность.ПоказатьСтраницыМногострочнойФормы(Форма);
	Форма.Модифицированность = Истина;	
