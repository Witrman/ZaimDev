...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма3002017Кв1";
	КодФормы 		= "Форма300";
...
Процедура УдалитьСтрокуНаСервере()
	...
	Если КодФормы = "Форма300_03" Тогда
		УдалитьСтрокуМногострочнойФормы(ЭтотОбъект, 2);
	Иначе
...
Процедура Форма300_03ПолеТабличногоДокументаВыборНаСервере(ИмяЛиста, Ячейка, Значение, ЗначениеРасшифровки, ПризнакРасчета)
	...
	Если  Найти(Ячейка, "Ф300_03_J") > 0 Тогда
		Если ЭтотОбъект["Форма300_03ПолеТабличногоДокумента" + ИмяЛиста].Области.Найти("Ф300_03_J1") <> Неопределено 
			И ЭтотОбъект["Форма300_03ПолеТабличногоДокумента" + ИмяЛиста].Области.Найти("Ф300_03_J2") <> Неопределено Тогда 
...
Функция Форма300_03ПолучитьТаблицуДанныхРеестраExcel(ТекКодФормы)	
	...
	ТаблицаДанныхA.Колонки.Добавить("КодВидаИмпорта",,                "Код вида импорта");
	...
	ТаблицаДанныхA.Колонки.Добавить("ПогашеноНДСВОтчетномПериоде",,   "Погашено НДС в отчетном налоговом периоде");
	ТаблицаДанныхA.Колонки.Добавить("ПогашеноНДСВПредыдущемПериоде",, "Погашено НДС в предыдущие налоговые периоды");
	ТаблицаДанныхA.Колонки.Добавить("СуммаНДСНепогашенная",,          "Сумма НДС, непогашенная взаимозачетом в течение 3-х месячного периода (D-(F+G))");
	ТаблицаДанныхA.Колонки.Добавить("КодБюджетнойКлассификации",,     "Код бюджетной классификации");
	...
	ТаблицаДанныхA.Колонки.Добавить("ЗадолженностьНДС",,              "Задолженность по НДС (D-(F+G+J+K))");
	
	ТаблицаДанныхB = Новый ТаблицаЗначений;
	ТаблицаДанныхB.Колонки.Добавить("ПогашеноНДСВОтчетномПериоде");
	ТаблицаДанныхB.Колонки.Добавить("ПогашеноНДСВПредыдущемПериоде");
	ТаблицаДанныхB.Колонки.Добавить("СуммаНДСНепогашенная");
	
	ТаблицаДанныхC = Новый ТаблицаЗначений;
	ТаблицаДанныхC.Колонки.Добавить("КодБюджетнойКлассификации");
	ТаблицаДанныхC.Колонки.Добавить("УплаченоНДСВОтчетномПериоде");
	ТаблицаДанныхC.Колонки.Добавить("УплаченоНДСВПредыдущемПериоде");
	ТаблицаДанныхC.Колонки.Добавить("ЗадолженностьНДС");
	...
	Для Каждого ЭлементСтруктуры Из СтруктураФормы Цикл
		...
		Для Каждого Элемент Из ТаблицаСтрокГруппы Цикл
			...
			Если ИмяГруппы = "ЛистA" Тогда
				...
				НоваяСтрокаДанных.КодВидаИмпорта = Элемент.Ф300_03_B;
				Если ЗначениеЗаполнено(Элемент.Ф300_03_C) Тогда 
					НоваяСтрокаДанных.НомерДекларацииЗаявления = Элемент.Ф300_03_C;
				КонецЕсли;
				НоваяСтрокаДанных.СуммаНДСНачисленного = Элемент.Ф300_03_D;
				Если ЗначениеЗаполнено(Элемент.Ф300_03_E) Тогда 
					НоваяСтрокаДанных.СрокПлатежа = НачалоДня(Элемент.Ф300_03_E);
				КонецЕсли;
			...
			ИначеЕсли ИмяГруппы = "ЛистB" Тогда
				...
				НоваяСтрокаДанных.ПогашеноНДСВОтчетномПериоде   = Элемент.Ф300_03_F;
				НоваяСтрокаДанных.ПогашеноНДСВПредыдущемПериоде = Элемент.Ф300_03_G;
				НоваяСтрокаДанных.СуммаНДСНепогашенная          = Элемент.Ф300_03_H;
				
			ИначеЕсли ИмяГруппы = "ЛистC" Тогда

				НоваяСтрокаДанных = ТаблицаДанныхC.Добавить();	

				НоваяСтрокаДанных.КодБюджетнойКлассификации 	= Элемент.Ф300_03_I;
				НоваяСтрокаДанных.УплаченоНДСВОтчетномПериоде   = Элемент.Ф300_03_J;
				НоваяСтрокаДанных.УплаченоНДСВПредыдущемПериоде = Элемент.Ф300_03_K;
				НоваяСтрокаДанных.ЗадолженностьНДС              = Элемент.Ф300_03_L;
	...
	// сводная таблица
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ПогашеноНДСВОтчетномПериоде"),   "ПогашеноНДСВОтчетномПериоде");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ПогашеноНДСВПредыдущемПериоде"), "ПогашеноНДСВПредыдущемПериоде");	
    ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("СуммаНДСНепогашенная"),          "СуммаНДСНепогашенная");
    ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("КодБюджетнойКлассификации"),     "КодБюджетнойКлассификации");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("УплаченоНДСВОтчетномПериоде"),   "УплаченоНДСВОтчетномПериоде");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("УплаченоНДСВПредыдущемПериоде"), "УплаченоНДСВПредыдущемПериоде");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхC.ВыгрузитьКолонку("ЗадолженностьНДС"),              "ЗадолженностьНДС");
...
Функция Форма300_05_002РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	НДСЗаНерезидентаОстаткиИОбороты.СуммаБезНДСПриход КАК Знач300_05_001,
...
Функция Форма300_05_005РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	|	СУММА(НДСЗаНерезидентаОстатки.СуммаБезНДСОстаток) КАК Знач300_05_004
	|ИЗ
	|	РегистрНакопления.НДСЗаНерезидента.Остатки(&НачПериода, Налогоплательщик = &Налогоплательщик) КАК НДСЗаНерезидентаОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			СУММА(НДСЗаНерезидентаОбороты.СуммаНДСРасход) КАК СуммаНДСРасходУплачено,
	...
	|		ГДЕ
	|			НДСЗаНерезидентаОбороты.Регистратор.Дата >= НДСЗаНерезидентаОбороты.ДокументПоступления.Дата
	|		
	|		СГРУППИРОВАТЬ ПО
	|			НДСЗаНерезидентаОбороты.Номенклатура) КАК УплаченныйНДС
...
Процедура Форма300_05ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь)
	...
	Если Выборка002.Следующий() Тогда
		Знач300_05_001 = ?(Выборка002.Знач300_05_001=NULL, 0, Выборка002.Знач300_05_001); 
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_300_05_001", Знач300_05_001, Форма300_05ПолеТабличногоДокументаСтраница1);
	...
	Если Выборка005.Следующий() Тогда
		Знач300_05_004 = ?(Выборка005.Знач300_05_004=NULL, 0, Выборка005.Знач300_05_004); 
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_300_05_004",Знач300_05_004, Форма300_05ПолеТабличногоДокументаСтраница1);
...
&НаСервере
Функция ТекстЗапросаСчетаФактурыВыданныеИДокументыОснования()
...
Функция Форма300_09РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь, Знач КодЧленаТС) 
	...
	ТекстЗапроса = " ВЫБРАТЬ
	               ...
	               |	СчетФактура
	               |; 
				   |" + ТекстЗапросаСчетаФактурыВыданныеИДокументыОснования() + Символы.ПС + ТекстЗапросаПоДаннымСчетовФактур + "
				   | 					  				   
				   |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ДанныеСчетовФактур.Номер,
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002017Кв1");
