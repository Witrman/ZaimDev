...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма3002020Кв2";
	КодФормы 		= "Форма300";
		...
		Выборка05.Сбросить();
	 	...
	 	Пока Выборка05.Следующий() Цикл
			...
			ИначеЕсли ИмяГруппы = "ЛистC" Тогда
				...
				Элемент.Ф300_05_A3 = Формат(НомерСтроки, мФорматНомераСтроки);
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002020Кв2");
