...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма3002015Кв3";
	КодФормы 		= "Форма300";
		...
		Для Каждого ЭлементСтруктуры Из МногострочнаяСтруктура Цикл
			...
			Для Каждого СтрокаОтчета из ТаблицаОтчета Цикл  			
				...
				Если ИмяГруппы = "ЛистA" Тогда
					...
					Если СтрДлина(СокрЛП(СтрокаОтчета.НомерГТД)) < 20 Тогда 
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002015Кв3");
