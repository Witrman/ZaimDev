...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма3002023Кв1";
	КодФормы 		= "Форма300";
...
Процедура Форма300ПолеТабличногоДокументаСтраница3Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если СтрНайти(Ячейка, "D") = 0 Тогда
...
Процедура Форма300ПолеТабличногоДокументаВыборНаСервере(Ячейка, Значение, ПризнакРасчета)	
	...
	Если (Найти(Ячейка, "Условие_A") > 0)
		...
		ИЛИ (Найти(Ячейка, "ТребованиеОВозврате_") > 0) 
...
Функция Форма300РассчитатьЗачетНДС(ВыдаватьСообщения = Ложь) 
	...
	|	НДСКВозмещению.УплаченныйНДСОборот КАК УплаченныйНДСОборот,
	...
	|	КорректировкаНДСКВозмещению.УплаченныйНДСОборот КАК УплаченныйНДСОборот,
	...
	|	СУММА(вт_НДСКВозмещениюОбороты.УплаченныйНДСОборот) КАК СуммаУплаченныйНДСОборот,
	...
	|	СУММА(ВозмещениеВТ.СуммаУплаченныйНДСОборот) КАК СуммаУплаченныйНДСОборот,
	|	СУММА(ВЫБОР
	|			КОГДА вт_ДействующиеСФ.Ссылка ЕСТЬ NULL 
	...
	|	СУММА(ВозмещениеВТ.СуммаУплаченныйНДСОборот),
	|	СУММА(ВЫБОР
	|			КОГДА вт_ДействующиеСФ.Ссылка ЕСТЬ NULL 
	|				ТОГДА ВозмещениеВТ.СуммаБезНДСОборот
	|			ИНАЧЕ 0
...
Процедура Форма300ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь) 
	...
	Для Каждого Строка Из ТаблицаРезультата Цикл
		...
		ИначеЕсли ОбластьНачало = "s_300_00_018" ИЛИ ОбластьНачало = "s_300_00_015" ИЛИ ОбластьНачало = "s_300_00_016" ИЛИ ОбластьНачало = "s_300_00_022" Тогда			
			СуммаБезНДС = Окр(?(Строка.СуммаБезНДСОборот <> NULL, Строка.СуммаБезНДСОборот, 0));
			СуммаНДС    = Окр(?(Строка.СуммаНДСОборот <> NULL, Строка.СуммаНДСОборот, 0));							
			СуммаУплаченныйНДС    = Окр(?(Строка.СуммаУплаченныйНДСОборот <> NULL, Строка.СуммаУплаченныйНДСОборот, 0));
			...
			Если Область <> Неопределено Тогда
				Если ОбластьНачало = "s_300_00_015" Тогда
					Область.Значение = Область.Значение + СуммаУплаченныйНДС;
				ИначеЕсли СтруктураРеквизитовФормы.мМетодРасчета = Перечисления.МетодыОтнесенияНДСВЗачет.Пропорциональный Тогда
					Область.Значение = Область.Значение + СуммаНДС;
				Иначе
					Если Строка.ВидОборота = Перечисления.ВидыОборотовПоРеализации.Облагаемый Тогда
...
Процедура Форма300_01ПолеТабличногоДокументаСтраница1Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если СтрНайти(Ячейка, "text__") <> 0 Тогда
		...
		ПараметрыФормы.Вставить("ПустойКодСтрокиДекларации" , Истина);
...
Функция Форма300_07ПолучитьСтруктуруСоответствияПолейРеестраExcel()
	...
	СтруктураСоответствияПолей.Вставить("ИННБИНПокупателя",           3);
	СтруктураСоответствияПолей.Вставить("НомерСФ", 			          4);
	СтруктураСоответствияПолей.Вставить("ДатаСФ", 			          5);
	СтруктураСоответствияПолей.Вставить("ВидСФПрав", 		          6);
	СтруктураСоответствияПолей.Вставить("СтоимостьБезНДС", 	          7);
	СтруктураСоответствияПолей.Вставить("СуммаНДССФ", 		          8);
	СтруктураСоответствияПолей.Вставить("СуммаНДСНачисленного",       9);
	СтруктураСоответствияПолей.Вставить("ВидПолезногоИскопаемого", 	 10);
	СтруктураСоответствияПолей.Вставить("ОбъемПолезногоИскопаемого", 11);
	СтруктураСоответствияПолей.Вставить("ЕдиницаИзмерения",          12);
...
Функция Форма300_07ПолучитьСтруктуруИтоговыхПолейРеестраExcel()
	СтруктураИтоговыхПолей = Новый Структура;
	СтруктураИтоговыхПолей.Вставить("СтоимостьБезНДС", 		7);
	СтруктураИтоговыхПолей.Вставить("СуммаНДССФ", 			8);
	СтруктураИтоговыхПолей.Вставить("СуммаНДСНачисленного", 9);
...
Функция Форма300_08ПолучитьСтруктуруСоответствияПолейРеестраExcel()
	...
	СтруктураСоответствияПолей.Вставить("НомерСтроки",                1);
	СтруктураСоответствияПолей.Вставить("СтатусПоставщика",           2);
	СтруктураСоответствияПолей.Вставить("ИННБИНПоставщика",           3);
	СтруктураСоответствияПолей.Вставить("НомерСФ",                    4);
	СтруктураСоответствияПолей.Вставить("ДатаСФ",                     5);	
	СтруктураСоответствияПолей.Вставить("ВидСФПрав",                  6);
	СтруктураСоответствияПолей.Вставить("СтоимостьБезНДС",            7);
	СтруктураСоответствияПолей.Вставить("СуммаНДССФ",                 8);
	СтруктураСоответствияПолей.Вставить("СуммаНДСВЗачет",             9);
	СтруктураСоответствияПолей.Вставить("ВидПолезногоИскопаемого", 	 10);
	СтруктураСоответствияПолей.Вставить("ОбъемПолезногоИскопаемого", 11);
	СтруктураСоответствияПолей.Вставить("ЕдиницаИзмерения",          12);
...
Функция Форма300_08ПолучитьСтруктуруИтоговыхПолейРеестраExcel()
	...
	СтруктураИтоговыхПолей.Вставить("СтоимостьБезНДС", 7);
	СтруктураИтоговыхПолей.Вставить("СуммаНДССФ",      8);
	СтруктураИтоговыхПолей.Вставить("СуммаНДСВЗачет",  9);
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002023Кв1");
...
Процедура Форма300РасшифровкаСКД(ИмяЯчейки)
	...
	ОтображатьПоказательСуммаУплаченныйНДС  = Ложь;
	...
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "Строка15", Ложь);
	...
	ИначеЕсли Найти(ИмяЯчейки, "300.00.015") >0 Тогда	
		ОтображатьПоказательСуммаНДС 			= Ложь;
		ОтображатьПоказательСуммаУплаченныйНДС 	= Истина;
		ОтображатьПоказательСуммаВДекларации 	= Истина;
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "Строка15", Истина);
	...
	КонецЕсли;
	Если ОтображатьПоказательСуммаУплаченныйНДС Тогда
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ПользовательскиеПоля, "СуммаУплаченныйНДС", "Уплаченный НДС");
	КонецЕсли;
