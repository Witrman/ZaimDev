...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма3002016Кв1";
	КодФормы 		= "Форма300";
...
Функция ПолучитьТекстЗапросаСчетовФактурВыданныхПоДокументам()
	...
	|			СчетФактураВыданныйТовары.ОборотПоРеализации
	|		ИЗ
	...
	|	Контрагенты.ИдентификационныйКодЛичности,
	|	Контрагенты.НомерНалоговойРегистрацииВСтранеРезидентства, 
	|	ВЫБОР КОГДА ДанныеСчетовФактур.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовФактур.Дополнительный) ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ,
...
Процедура Форма300_09ЛистBВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_E__") > 0 
		ИЛИ Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_F__") > 0 
		ИЛИ Найти(ИмяЯчейкиБезПрефиксаЧленаТС, "Ф300_09_G__") > 0 Тогда		
...
Функция Форма300_09ПолучитьТаблицуДанныхРеестраExcel(ТекКодФормы)
	...
	ТаблицаДанныхB.Колонки.Добавить("ДатаОборотаПоРеализации",,	"Дата совершения оборота по реализации");
	ТаблицаДанныхB.Колонки.Добавить("ВидОборота",,				"Вид оборота");	
	ТаблицаДанныхB.Колонки.Добавить("СуммаНДССФ",,				"Всего размер облагаемого оборота по счету-фактуре");
	...
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("НомерСФ"),                 "НомерСФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ДатаСФ"),                  "ДатаСФ");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ДатаОборотаПоРеализации"),	"ДатаОборотаПоРеализации");
	ТаблицаДанныхA.ЗагрузитьКолонку(ТаблицаДанныхB.ВыгрузитьКолонку("ВидОборота"),              "ВидОборота");
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002016Кв1");
...
Функция ВыгрузитьДанныеРеестраВExcelНаКлиенте(ИмяФормы, АдресФайлаШаблона, АдресРеестра, ИмяЛиста, НомерНачальнойСтрокиExcel, КоличествоЗаписей)
	...
	Для НомерСтроки = 2 По КоличествоЗаписей+1 Цикл
		...
		Для Каждого КлючИЗначение Из СтруктураСоответствияПолей Цикл
			...
			Попытка
				РабочийЛистКниги.Cells(НомерТекущейСтроки,КлючИЗначение.Значение).Value = Реестр.Область("R" + НомерСтроки + "C" + СтруктураСоответствияКолонок[КлючИЗначение.Ключ]).Текст;
			Исключение
...
Функция ВыгрузитьДанныеРеестраНаСервере(ИмяФормы, Заголовок)
	...
	Макет = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетРеестра");
	...
	ОбластьЗаголовок.Параметры.РНН 				= СведенияОНалогоплательщике.РНН;
