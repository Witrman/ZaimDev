...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма3002019Кв2";
	КодФормы 		= "Форма300";
...
Процедура УдалитьСтрокуМногострочнойФормы(Форма, Знач КоличествоСтрокШапки = 1) Экспорт
	...
	ИмяСтраницыБезПрефиксаПанели =  СтрЗаменить(ТекСтраница.Имя, ТекущаяПанель.Имя, ""); // ИмяСтраницыБезПерфиксаПанели
	СтруктураГруппы = СтрокаФормы.Строки.Найти(ИмяСтраницыБезПрефиксаПанели);
			...
			Для Каждого Страница Из ТекущаяПанель.ПодчиненныеЭлементы Цикл                 		 				
				...
				Иначе	
					...
					Если ТаблицаСтрокГруппы.Количество() = 0 Тогда
						...
						ВсегоСтраниц = 0;
					КонецЕсли;
					РегламентированнаяОтчетность.ВывестиРазделВТабличныйДокументФормы(
						Форма, Форма.ТекущаяСтраница, ИмяСтраницыБезПрефиксаПанели, Новый Структура("ШапкаИПодвал, Строки", ОбщиеЭлементыГруппы, ТаблицаСтрокГруппы), ТекущееИмяФормы, ВсегоСтраниц);
	...
	Если ТекущееИмяФормы = "Форма300_05" Тогда
		...
		СтруктураГруппыЛистC     = СтрокаФормы.Строки.Найти("ЛистC");
		ОбщиеЭлементыГруппыЛистC = СтруктураГруппыЛистC.ОбщиеЭлементыГруппы;
		СтруктураГруппыЛистC     = СтрокаФормы.Строки.Найти("ЛистC");
...
Процедура Форма300РасчетНаСервере(знач ИмяЯчейки, знач ЗначениеЯчейки)
	...
	ИначеЕсли СтруктураРеквизитовФормы.мМетодРасчета = Перечисления.МетодыОтнесенияНДСВЗачет.Пропорциональный Тогда
		...
		Если s_300_00_006_a <> 0 И s_300_00_007_b = 0 Тогда	
			b_s_300_00_025_1 = 0; 
		ИначеЕсли s_300_00_007_b = 0 Тогда
			...
			Если ИмяГруппы = "ЛистB" Тогда
				...
				Если ЗначениеЗаполнено(Контрагент) Тогда
					...
					Если НЕ ЗначениеЗаполнено(СтранаРезидентства) Тогда
						...
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
					Иначе
						Если ПустаяСтрока(СтранаРезидентства.КодАльфа3) Тогда
							ТекстСообщения = НСтр("ru = 'Форма 300.05: У страны <%1> - не заполнен код ""Альфа-3"".
						...
						Иначе
							СтранаРезидентстваКод = СтранаРезидентства.КодАльфа3;
						КонецЕсли;
...
Процедура  Форма300_05ОбработкаВыбораЗначенийНаСервере(Результат, ПараметрыВыбора)
	...
	Если ПараметрыВыбора.ЭлементВыбора = "Контрагент" И ЗначениеЗаполнено(Результат) Тогда
		...
		Иначе
			Если ПустаяСтрока(СтранаРезидентства.КодАльфа3) Тогда
				ТекстСообщения = НСтр("ru = 'У страны <%1> - не заполнен код ""Альфа-3"".
				                            |При заполнении формы применяется цифровой код страны.'");								
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СтранаРезидентства);			
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				СтранаРезидентстваКод = СтранаРезидентства.Код;
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма3002019Кв2");
