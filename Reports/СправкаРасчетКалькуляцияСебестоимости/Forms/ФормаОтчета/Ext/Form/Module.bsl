...
&НаКлиенте
Процедура ГруппироватьОтчетПриИзменении(Элемент)
...
Процедура СформироватьОтчет(Команда)
	...
	УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""справка-расчет ""калькуляция себестоимости"""" (формирование)"); 
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru = 'Справка-расчет калькуляции себестоимости продукции %1'");
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	Если ИБФайловая Тогда
		...
		Отчеты.СправкаРасчетКалькуляцияСебестоимости.СформироватьОтчет(ПараметрыОтчета, АдресХранилища);
		РезультатВыполнения = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор,
			"Отчеты.СправкаРасчетКалькуляцияСебестоимости.СформироватьОтчет",
			ПараметрыОтчета,
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ГруппироватьОтчет"                     , Отчет.ГруппироватьОтчет);
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	Для Каждого ДоступноеПоле Из Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.Элементы Цикл
		
		ПолеНаименование = Строка(ДоступноеПоле.Поле);
		
		Окончание = Сред(ПолеНаименование, СтрДлина(ПолеНаименование) - 1, 2);
		
		ПризнакНУ = Окончание = "НУ";
		
		Если Отчет.НаборПоказателейОтчета = 1 И ПризнакНУ Тогда
			СписокПолей.Добавить(ПолеНаименование);
		КонецЕсли;
		
		Если Отчет.НаборПоказателейОтчета <> 1 И НЕ ПризнакНУ Тогда
			СписокПолей.Добавить(ПолеНаименование);
		КонецЕсли;
