...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мКодыСтрокПрочиеПоступления.Вставить(Перечисления.РазрезыДеятельности.Инвестиционная, 	"051");
	...
	мКодыСтрокПрочиеВыбытия.Вставить(Перечисления.РазрезыДеятельности.Инвестиционная, 	"071");
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	Для НомерПериода = 3 по 4 Цикл
		...
		// 50. 2. Выбытие денежных средств, всего (сумма строк с 61 по 71)			
		Итого = 0;
		Для Счетчик  = 61 по 71 Цикл
			НомерСтроки = "Б" + Формат(Счетчик,"ЧЦ=3; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;	
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"Б080_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		...
		// 130.	Итого:            Увеличение +/- уменьшение денежных средств (стр. 030 +/- стр. 080 +/- стр. 110 +/- стр. 120)
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"Б130_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница2);	
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Для НомерПериода = 3 по 4 Цикл
		...
		Пока ВыборкаОтчета.Следующий() Цикл					
			...
			Если Число(КодСтрокиотчета)> 80 Тогда
		...
		УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "Б140_" + НомерПериода, ВыборкаОтчета.СуммаНачальныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница2);
		УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "Б150_" + НомерПериода, ВыборкаОтчета.СуммаКонечныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница2);
...
Процедура ФормаОтчетаРасшифровкаСКД(ПолноеИмяЯчейки)
	...
	РасшифровкаОстатков = ?(Найти(ПолноеИмяЯчейки, "Б140") <> 0 ИЛИ Найти(ПолноеИмяЯчейки, "Б150") <> 0, Истина, Ложь);
