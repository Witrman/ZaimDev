...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "ФормаОтчета2009Кв1";
...
Процедура СохранитьДанныеНаСервере()
	...
	СписокСохранения.Вставить("ВерсияФормы", "01/01/2009");
...
Функция ФормаРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь)
	...
	|	СУММА(ДоходыИНалоги.ОблагаемыеСНДоходыЗаМесяц) КАК ОблагаемыеСНДоходыЗаМесяц,
	...
	|		КОНЕЦ + ИПНСведенияОДоходах.ПримененнаяЛьготаОборот КАК НеоблагаемыеИПНДоходыЗаМесяц,
	...
	"ДоходыИНалоги.ДоходыЗаМесяц - ДоходыИНалоги.НеоблагаемыеИПНДоходыЗаМесяц - ДоходыИНалоги.ВычетыЗаМесяц";
	...
	|	ДоходыИНалоги.ОблагаемыеСНДоходыЗаМесяц,
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "5",  Выборка.ОПВЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "6",  Выборка.ОПВСНачалаГода);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр0136",        Выборка.ОПВСНачалаГода);

		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "7",  Выборка.ВычетыЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр0137",        Выборка.ВычетыСНачалаГода);

		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "8",  Выборка.ОблагаемыеИПНДоходыЗаМесяц);
		
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "9",  Выборка.ИПНЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр0139",        Выборка.ИПНСНачалаГода);

		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "10", Выборка.НеоблагаемыеСНДоходыЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "11", Выборка.НеоблагаемыеСНДоходыСНачалаГода);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр01311",       Выборка.НеоблагаемыеСНДоходыСНачалаГода);
		
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "12", Выборка.ОблагаемыеСНДоходыЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "13", Выборка.ОблагаемыеСНДоходыСНачалаГода);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр01313",       Выборка.ОблагаемыеСНДоходыСНачалаГода);

		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "14", Выборка.СНСРасчетнойСуммы);
		
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "15", Выборка.ОблагаемыеСОДоходыЗаМесяц);
		
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "16", Выборка.СОЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр01316",       Выборка.СОСНачалаГода);
		
		УстановитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "17", Выборка.СНЗаМесяц);
		УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр01317",       Выборка.СНСНачалаГода);
	...
	Для НомерСтроки = 1 По 12 Цикл
		...
		Стр0135 = Стр0135 + ПолучитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "5", 0);
		Стр0138 = Стр0138 + ПолучитьЗначениеОбласти(ТекТабличноеПоле, ИмяСтроки + "8", 0);
	КонецЦикла;
	
	УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр0132", Стр0132);
	УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр0135", Стр0135);
	УстановитьЗначениеОбласти(ТекТабличноеПоле, "Стр0138", Стр0138);
...
Процедура ВывестиПроцентыНалоговСборовОтчислений(ГодПериода)
    ...
    ТекстОбласти = НСтр("ru = 'Сумма индивидуального подоходного налога'") + Символы.ПС + НСтр("ru = '(гр. 8 * '") + ПроцентИПН + "%)";
	УстановитьЗначениеОбласти(ТекТабличныйДокумент, "Текст9", ТекстОбласти);
    ...
    ТекстОбласти = НСтр("ru = 'Сумма социального налога с расчетной суммы дохода, облагаемого социальным налогом'") +
	               Символы.ПС + НСтр("ru = '(гр. 12 * '") + ПроцентСН + "%)";
	УстановитьЗначениеОбласти(ТекТабличныйДокумент, "Текст14", ТекстОбласти);
    ...
    ТекстОбласти = НСтр("ru = 'Сумма социальных отчислений'") + Символы.ПС + НСтр("ru = '(гр. 15 * '") + ПроцентСО + "%)";
	УстановитьЗначениеОбласти(ТекТабличныйДокумент, "Текст16", ТекстОбласти);
