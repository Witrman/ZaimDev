...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеДоходы 		= "016";
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеРасходы 		= "015";
...
Процедура УстановитьПериодОтчета()
	...
	Иначе
		...
		ПериодОтчетаНаРусском 	= ПредставлениеПериода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ФП=Истина");
		ПериодОтчетаНаКазахском	= ПредставлениеПериода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "Л=kk_KZ; ФП=Истина");	
	...
	ОтчетныйПериодНаРусском 	= "ОТЧЕТ О ПРИБЫЛЯХ И УБЫТКАХ ОТЧЕТНЫЙ ПЕРИОД " + Строка(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)) + " г.";
	ОтчетныйПериодНаКазахском	= "ПАЙДАЛАР МЕН ЗАЛАЛДАР ТУРАЛЫ ЕСЕП " + Строка(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)) + " ж." + " ЕСЕПТІ КЕЗЕҢ";
...
Функция ФормаРассчитатьДанныеФормы(ДатаНачалаПериода, ДатаОкончанияПериода) 		
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Типовой.Обороты(&ДатаНачала, &ДатаОкончания, , Счет = Значение(ПланСчетов.Типовой.ИтоговаяПрибыльИтоговыйУбыток), , Организация В (&СписокОрганизаций), КорСчет В ИЕРАРХИИ (&КорСчет), ) КАК ТиповойОбороты
	               |ИТОГИ
	               |	СУММА(СуммаОборотДт),
	               |	СУММА(СуммаОборотКт)
	               |ПО
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Для НомерПериода = 3 по 4 Цикл
		...
		Пока ВыборкаОтчета.Следующий() Цикл					
			...
			Сумма = 0;
			Если КодСтрокиОтчета = "010" ИЛИ КодСтрокиОтчета = "016" ИЛИ КодСтрокиОтчета = "021" 
				ИЛИ КодСтрокиОтчета = "023" ИЛИ КодСтрокиОтчета = "201" Тогда
