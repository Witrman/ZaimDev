...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеДоходы  = "024";
	СтруктураРеквизитовФормы.мКодСтрокиПрочиеРасходы = "025";
...
Процедура Расшифровка(Команда)
	...
	ЦветРасшифровки = ФормаОтчетаПолеТабличногоДокументаСтраница1.Области.Б010_3.ЦветФона;
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""о доходах и расходах (мсфо)"" (заполнение)";
...
Процедура УстановитьПериодОтчета()
	...
	Иначе
		...
		ПериодОтчетаНаРусском 	= СтрЗаменить(ПредставлениеПериода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ФП=Истина"), "г.", "год");
		ПериодОтчетаНаКазахском	= СтрЗаменить(ПредставлениеПериода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "Л=kk_KZ; ФП=Истина"), "ж.", "жыл");	
	...
	ОтчетныйПериодНаРусском 	= "Отчет о прибылях и убытках";
	ПериодНаРусском             = "отчетный период " + Формат(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),"ЧГ=0") + " год";
	ОтчетныйПериодНаКазахском	= "Пайдалар мен залалдар туралы есеп";
	ПериодНаКазахском           = Формат(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),"ЧГ=0") + " жыл" + " есепті кезең";
	
	ОтчетныйПериод = НСТР("ru = " + """" + ОтчетныйПериодНаРусском + """; kz = " + """" + ОтчетныйПериодНаКазахском + """", ЯзыкФормирования);
	Период         = НСТР("ru = " + """" + ПериодНаРусском + """; kz = " + """" + ПериодНаКазахском + """", ЯзыкФормирования);
	
	//РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "ОтчетныйПериод", ОтчетныйПериод);
	РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтотОбъект, КодФормы, "Период", Период);
...
Функция ПолучитьКодСтрокиОтчета(Счет, СчетПредопределенный = Неопределено, СчетВидСчета = Неопределено)
	...
	Если СчетПредопределенный = Истина Тогда
		// поиск счета в Структуре соответствия
		ИмяПредопределенногоЭлемента = ПланыСчетов.Типовой.ПолучитьИмяПредопределенного(Счет);
		Если СтруктураРеквизитовФормы.СтруктураСоответствияСчетов.Свойство(ИмяПредопределенногоЭлемента) Тогда
			КодСтроки  = СтруктураРеквизитовФормы.СтруктураСоответствияСчетов[ИмяПредопределенногоЭлемента];
		КонецЕсли;	
	КонецЕсли;
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	Для НомерПериода = 3 по 4 Цикл
		...
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б010_" + НомерПериода) 
				- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б011_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "Б012_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		
				
		//020 Итого операционная прибыль (убыток) (+/- строки с 012 - (013+...016) + 016 (отнимаем расходы прибавляем доходы))
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б012_" + НомерПериода);
		Для Счетчик  = 13 по 14 Цикл
		...
		КонецЦикла;		
		//Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б016_" + НомерПериода);
		...
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б020_" + НомерПериода)
		        + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б021_" + НомерПериода)
				- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б022_" + НомерПериода)
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б023_" + НомерПериода)
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б024_" + НомерПериода)
				- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б025_" + НомерПериода);
		...
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б200_" + НомерПериода) 
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б201_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "Б300_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		
		// 420 - Прочий совокупный доход (сумма строк с 410 по 418)
		Итого = 0;
		Для Счетчик  = 410 по 418 Цикл
			НомерСтроки = "Б" + Формат(Счетчик,"ЧЦ=3; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;		
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "Б420_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);

		// 440 - Итого прочий совокупный доход не подлежащий реклассификации в доходы или расходы в последующие периоды (за вычетом налога на прибыль) (сумма строк с 431 по 435)
		Итого = 0;
		Для Счетчик  = 431 по 435 Цикл
			НомерСтроки = "Б" + Формат(Счетчик,"ЧЦ=3; ЧДЦ=0; ЧВН=") + "_";			
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,НомерСтроки + НомерПериода);
		КонецЦикла;		
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "Б440_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);

		
		// 400 - Прочая совокупная прибыль, всего (сумма строк с 420 по 440)
		Итого = 0;
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б420_" + НомерПериода) 
				+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"Б440_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "Б400_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		...
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "Б500_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);					
...
&НаСервере
Функция ФормаРассчитатьДанныеФормы(ДатаНачалаПериода, ДатаОкончанияПериода, МассивИсключенныхСчетов = Неопределено) 		
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			&ДатаОкончания,
	               |			,
	               |			Счет = ЗНАЧЕНИЕ(ПланСчетов.Типовой.ИтоговаяПрибыльИтоговыйУбыток),
	               |			,
	               |			Организация В (&СписокОрганизаций),
	               |			КорСчет В ИЕРАРХИИ (&КорСчет)
	               |				И НЕ КорСчет В ИЕРАРХИИ (&МассивИсключенныхСчетов),
	               |			) КАК ТиповойОбороты
	               |ИТОГИ
	               |	СУММА(СуммаОборотДт),
	               |	СУММА(СуммаОборотКт)
	...
	//кор.счета, относящиеся к доходам/убыткам организации	
	МассивСчетов = РегламентированнаяОтчетностьПереопределяемый.СформироватьСписокСчетовДоходовРасходов(Истина, Истина);
	Запрос.УстановитьПараметр("КорСчет", МассивСчетов);	
	Если МассивИсключенныхСчетов = Неопределено Тогда
		МассивИсключенныхСчетов = Новый Массив;
	КонецЕсли;
	Запрос.УстановитьПараметр("МассивИсключенныхСчетов", МассивИсключенныхСчетов);
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	МассивСчетов.Добавить(ПредопределенноеЗначение("ПланСчетов.Типовой.ПрочиеДоходы_"));
	МассивСчетов.Добавить(ПредопределенноеЗначение("ПланСчетов.Типовой.ПрочиеРасходы_"));
	...
	Для НомерПериода = 3 по 4 Цикл
		...
		ВыборкаОтчета = ВыборкаРезультата.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
		Пока ВыборкаОтчета.Следующий() Цикл
			
			Если МассивСчетов.Найти(ВыборкаОтчета.КорСчет) <> Неопределено Тогда
				
				КодСтрокиОтчета = ПолучитьКодСтрокиОтчета(ВыборкаОтчета.КорСчет, ВыборкаОтчета.КорСчет.Предопределенный);			
				ВыборкаПрочие = ВыборкаОтчета.Выбрать();
				
				Пока ВыборкаПрочие.Следующий() Цикл

					КодСтрокиПрочие = ПолучитьКодСтрокиОтчета(ВыборкаПрочие.КорСчет, ВыборкаПрочие.КорСчет.Предопределенный);
					Если Не ЗначениеЗаполнено(КодСтрокиПрочие) Тогда
						КодСтрокиПрочие = КодСтрокиОтчета;
					КонецЕсли;
					Если Не ЗначениеЗаполнено(КодСтрокиПрочие) Тогда
						Продолжить;
					КонецЕсли;
					...
					Сумма = 0;
					Если КодСтрокиПрочие = "021" ИЛИ КодСтрокиПрочие = "024" Тогда
						Сумма = ВыборкаПрочие.СуммаОборотКт - ВыборкаПрочие.СуммаОборотДт;
					ИначеЕсли КодСтрокиПрочие = "022" ИЛИ КодСтрокиПрочие = "025" Тогда
						Сумма = ВыборкаПрочие.СуммаОборотДт - ВыборкаПрочие.СуммаОборотКт;
					КонецЕсли;
					
					// сумма с учетом уже имеющихся данных
					УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "Б" + КодСтрокиПрочие + "_" + НомерПериода, Сумма);
			...
			КодСтрокиОтчета = ПолучитьКодСтрокиОтчета(ВыборкаОтчета.КорСчет, ВыборкаОтчета.КорСчет.Предопределенный);			
			Если Не ЗначениеЗаполнено(КодСтрокиОтчета) Тогда
				Продолжить;
			КонецЕсли;	
			
			Сумма = 0;
			Если КодСтрокиОтчета = "010" ИЛИ КодСтрокиОтчета = "021" 
				ИЛИ КодСтрокиОтчета = "023" ИЛИ КодСтрокиОтчета = "201" ИЛИ КодСтрокиОтчета = "101" Тогда
			...
			// сумма с учетом уже имеющихся данных
			УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "Б" + КодСтрокиОтчета + "_" + НомерПериода, Сумма);
...
Процедура ФормаОтчетаРасшифровкаСКД(ИмяЯчейки)
	...
	Если ИмяЯчейки = СтруктураРеквизитовФормы.мКодСтрокиПрочиеДоходы Тогда
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, "Счет", ПредопределенноеЗначение("ПланСчетов.Типовой.ДоходыОтВосстановленияУбыткаОтОбесцененияПоФинансовымАктивам"), ВидСравненияКомпоновкиДанных.НеВИерархии);		
	КонецЕсли;
	
	Если ИмяЯчейки = СтруктураРеквизитовФормы.мКодСтрокиПрочиеРасходы Тогда
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, "Счет", ПредопределенноеЗначение("ПланСчетов.Типовой.РасходыОтОбесцененияФинансовыхИнструментов"), ВидСравненияКомпоновкиДанных.НеВИерархии);		
	КонецЕсли;
	...
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ПользовательскиеПоля, "СуммаОборотКт", "Оборот Кт");
		
	// Прочие настройки
	ТекстЗаголовка = НСТР("ru = 'Регистр бухгалтерского учета к строке %1 Отчета о прибылях и убытках'");	
...
//Обход ситуации, когда значения очищались в клиент-сервер
&НаКлиенте
