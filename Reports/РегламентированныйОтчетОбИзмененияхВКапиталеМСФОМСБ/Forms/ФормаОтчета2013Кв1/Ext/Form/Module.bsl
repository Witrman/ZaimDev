...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""об изменениях в капитале (МСФО МСБ)"" (заполнение)";
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	// вертикальные итоги
	Для НомерСтроки = 1 по 200 Цикл
		ИмяОбласти = "s_"+ Формат(НомерСтроки, "ЧЦ=3; ЧДЦ=; ЧВН=") + "_";
		СтрокаОбласти = ТаблицаДанных.Найти(ИмяОбласти + "6","ИмяОбласти");
		Если СтрокаОбласти = Неопределено Тогда
			Продолжить;
		КонецЕсли;	  
		Итого = 0;
		 // Если такая строка есть, то рассчитываем итоги
		Для НомерКолонки = 1 по 3 Цикл
			Итого = Итого + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,ИмяОбласти + НомерКолонки);
		КонецЦикла;	
		// Найдем ПТД строки
		ВсегоСтраниц = 2;
		ПТД = Неопределено;
		Для НомерСтраницы = 1  По ВсегоСтраниц Цикл
			ПТД = ЭтотОбъект["ФормаОтчетаПолеТабличногоДокументаСтраница" + НомерСтраницы];
			Если ПТД.Области.Найти(ИмяОбласти  + "4")<> Неопределено Тогда
				РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,ИмяОбласти + "4", Итого, ПТД);
			КонецЕсли;	
			Если ПТД.Области.Найти(ИмяОбласти  + "6")<> Неопределено Тогда
				РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,ИмяОбласти + "6", Итого, ПТД);
			КонецЕсли;	
		КонецЦикла;	       		
	...
	Для НомерКолонки = 1 по 6 Цикл
		// ОТЧЕТНЫЙ ГОД
		// 030. Пересчитанное сальдо   (стр.010+/-стр. 020)		
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_010_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_020_" + НомерКолонки);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_030_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);		
		
		// 040. Прибыль/убыток,	признанная/ый непосредственно  в  самом  капитале (стр. 031+/-стр. 032+/- стр.033)
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_031_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_032_" + НомерКолонки) +
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_033_" + НомерКолонки);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_040_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);			
		
		// 060. Всего прибыль/убыток за период(стр. 040+/-стр. 050)		
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_040_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_050_" + НомерКолонки);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_060_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);			
		
		// 100. Сальдо на 31 декабря отчетного года (стр.030 + стр. 060 + стр. 070 + стр. 080 + стр. 090)		
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_030_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_060_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_070_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_080_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_090_" + НомерКолонки);
				
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_100_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);			
		
		// ПРОШЛЫЙ ГОД
		// 130. Пересчитанное сальдо   (стр.110+/-стр. 120)		
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_110_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_120_" + НомерКолонки);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_130_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);		
		
		// 140. Прибыль/убыток,	признанная/ый непосредственно  в  самом  капитале (стр. 131+/-стр. 132+/- стр.133)
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_131_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_132_" + НомерКолонки) +
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_133_" + НомерКолонки);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_140_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница2);			
		
		// 160. Всего прибыль/убыток за период(стр. 140+/-стр. 150)		
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_140_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_150_" + НомерКолонки);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_160_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница2);			
		
		// 200. Сальдо на 31 декабря отчетного года (стр.130 + стр. 160 + стр. 170 + стр. 180 + стр. 190)		
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_130_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_160_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_170_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_180_" + НомерКолонки) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_190_" + НомерКолонки);
				
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных,"s_200_" + НомерКолонки ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница2);			
	КонецЦикла;	   	
	...
	ТаблицаДанных = РегламентированнаяОтчетность.ПолучитьТаблицуДанныхОбычнойФормы(ЭтаФорма, КодФормы);	
		...
		Для НомерПериода = 1 по 2 Цикл
			Если НомерПериода = 1 Тогда
				ВыборкаРезультата 		= ФормаРассчитатьДанныеФормы(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));		
			Иначе
				ВыборкаРезультата 		= ФормаРассчитатьДанныеФормы(СтруктураРеквизитовФормы.мДатаНачалаСравнительногоПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаСравнительногоПериодаОтчета));			
			КонецЕсли;
			
			ВыборкаОтчета = ВыборкаРезультата.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
			Пока ВыборкаОтчета.Следующий() Цикл					
				КодСтрокиОтчета = ПолучитьКодСтрокиОтчета(ВыборкаОтчета.Субконто1, НомерПериода);			
				Если Не ЗначениеЗаполнено(КодСтрокиОтчета) Тогда
					Продолжить;
				КонецЕсли;	
				
				Сумма = ВыборкаОтчета.СуммаОборотКт - ВыборкаОтчета.СуммаОборотДт;
				Если КодСтрокиОтчета >="133" Тогда
					ПолеТабличногоДокумента = ФормаОтчетаПолеТабличногоДокументаСтраница2;
				Иначе
					ПолеТабличногоДокумента = ФормаОтчетаПолеТабличногоДокументаСтраница1;
				КонецЕсли;
				// сумма с учетом уже имеющихся данных
				УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "s_" + КодСтрокиОтчета + "_" + ВыборкаОтчета.Капитал, Сумма, ПолеТабличногоДокумента);				
			КонецЦикла;		
		КонецЦикла; 
		
		// Данные о сальдо
		// прошлого периода
		ВыборкаРезультатаИтогов	= ФормаРассчитатьСальдо(СтруктураРеквизитовФормы.мДатаНачалаСравнительногоПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаСравнительногоПериодаОтчета);
		ВыборкаИтогов = ВыборкаРезультатаИтогов.Выбрать();
		Пока ВыборкаИтогов.Следующий() Цикл			
			УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "s_110_" + ВыборкаИтогов.Капитал, ВыборкаИтогов.СуммаНачальныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница1);				
			УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "s_200_" + ВыборкаИтогов.Капитал, ВыборкаИтогов.СуммаКонечныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница2);				
		КонецЦикла;		
		
		// текущего периода
		ВыборкаРезультатаИтогов	= ФормаРассчитатьСальдо(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ВыборкаИтогов = ВыборкаРезультатаИтогов.Выбрать();
		Пока ВыборкаИтогов.Следующий() Цикл			
			УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "s_010_" + ВыборкаИтогов.Капитал, ВыборкаИтогов.СуммаНачальныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница1);				
			УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "s_100_" + ВыборкаИтогов.Капитал, ВыборкаИтогов.СуммаКонечныйОстаток, ФормаОтчетаПолеТабличногоДокументаСтраница2);				
		КонецЦикла;		
...
Процедура ФормаОтчетаРасшифровкаСКД(ПолноеИмяЯчейки)
	...
	ПараметрыВиртуальнойТаблицы = "({&НачалоПериода}, {&КонецПериода}, Авто, , &ВидыСубконто)";
    ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("ПараметрыВиртуальнойТаблицы", ПараметрыВиртуальнойТаблицы);
    // Параметры     
    МассивПараметровДанных = Новый Массив();     
    ВидыСубконто = Новый Массив;
    ВидыСубконто.Добавить(ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыСубконтоТиповые.ВидыИзмененийВСобственномКапитале"));    
	
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "ВидыСубконто", ВидыСубконто);
	...
	НовыйЭлементПорядка.Поле  = Новый ПолеКомпоновкиДанных("Субконто1");
	...
	// Прочие настройки
	ТекстЗаголовка = НСТР("ru = 'Регистр бухгалтерского учета к строке %1 Отчета об изменении в капитале'");	
...
//Обход проблемы с очисткой значением в клиент-сервер
&НаКлиенте
