...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Отчет.ВидОтчета = "НаличиеДокументовРеализации";
...
Процедура ПриОткрытии(Отказ)
	...
	ВидОтчета = Отчет.ВидОтчета = "НаличиеДокументовРеализации";
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	Если Настройки.ДополнительныеСвойства.Свойство("ДанныеОтчета") Тогда
		...
		Если НЕ ДанныеОтчета.Свойство("ВидОтчета") Тогда
			Отчет.ВидОтчета = "НаличиеСчетовФактурВыданных";
		КонецЕсли;
		Если ДанныеОтчета.Показатели.Найти("СФСуммаНДСРегл", "Поле") = Неопределено Тогда
			НовыйПоказатель = Отчет.Показатели.Добавить();
			НовыйПоказатель.Поле          = "СФСуммаНДСРегл";
			НовыйПоказатель.Представление = НСтр("ru = 'Сумма НДС регл. (счет-фактура)'");
			НовыйПоказатель.Использование = Ложь;
		КонецЕсли;
		Если ДанныеОтчета.Показатели.Найти("СФОборотПоРеализацииРегл", "Поле") = Неопределено Тогда
			НовыйПоказатель = Отчет.Показатели.Добавить();
			НовыйПоказатель.Поле          = "СФОборотПоРеализацииРегл";
			НовыйПоказатель.Представление = НСтр("ru = 'Оборот по реализации регл. (счет-фактура)'");
			НовыйПоказатель.Использование = Ложь;
		КонецЕсли;
...
Процедура ВидОтчетаПриИзменении(Элемент)
	...
	Если ВидОтчета Тогда
		Отчет.ВидОтчета = "НаличиеДокументовРеализации";
		КлючТекущегоВарианта  = "НаличиеДокументовРеализации";
	Иначе
		Отчет.ВидОтчета = "НаличиеСчетовФактурВыданных";
		КлючТекущегоВарианта  = "НаличиеСчетовФактурВыданных";
	КонецЕсли;
...
Процедура СформироватьОтчет(Команда)
	...
	УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""отчет по наличию счетов-фактур выданных"" (формирование)." + Отчет.ВидОтчета); 
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru = 'Отчет по наличию счетов-фактур выданных %1'");
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Показатели") И ЗаполняемыеНастройки.Показатели Тогда
		...
		СписокПоказателей.Добавить("СФСуммаНДСРегл", НСтр("ru = 'Сумма НДС регл. (счет-фактура)'"));
		СписокПоказателей.Добавить("СФОборотПоРеализацииРегл", НСтр("ru = 'Оборот по реализации регл. (счет-фактура)'"));
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка") И ЗаполняемыеНастройки.Группировка Тогда
		...
		Если Отчет.ВидОтчета = "НаличиеДокументовРеализации" Тогда
			НоваяСтрока = Отчет.Группировка.Добавить();
			НоваяСтрока.Поле           = "СчетФактураРеализации";
			НоваяСтрока.Использование  = Истина;
	...
	Если ЗаполняемыеНастройки.Свойство("ДополнительныеПоля") И ЗаполняемыеНастройки.ДополнительныеПоля Тогда
		...
		НоваяСтрока.Использование  = НЕ Отчет.ВидОтчета = "НаличиеДокументовРеализации";
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭСФЧерезAPI")
			...
			НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
			НоваяСтрока.Поле           = "СпособВыставления";
		...
		НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
		НоваяСтрока.Поле           = "ДатаОборота";
		НоваяСтрока.Представление  = НСтр("ru = 'Дата оборота'");
		НоваяСтрока.Использование  = Отчет.ВидОтчета = "НаличиеСчетовФактурВыданных";    
		
		НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
		НоваяСтрока.Поле           = "КрайнийСрокВыпискиЭСФ";
		НоваяСтрока.Представление  = НСтр("ru = 'Крайняя дата выписки ЭСФ'");
		НоваяСтрока.Использование  = Отчет.ВидОтчета = "НаличиеСчетовФактурВыданных";    
	...
	Если ЗаполняемыеНастройки.Свойство("Отбор") И ЗаполняемыеНастройки.Отбор Тогда
		...
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(Отчет.КомпоновщикНастроек, "СчетФактураРеализации.ВалютаДокумента", Справочники.Валюты.ПустаяСсылка(),, Ложь);
		
		Если НЕ Отчет.ВидОтчета = "НаличиеДокументовРеализации" Тогда
			БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(Отчет.КомпоновщикНастроек, "ОсталосьДней", , , Ложь);
		КонецЕсли;
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	Если Режим = "Выбор" ИЛИ Режим = "Группировка" Тогда
		...
		КонецЦикла;
		СписокПолей.Добавить("ОборотПоРеализацииОборот");
		СписокПолей.Добавить("СФСуммаНДСРегл");
		СписокПолей.Добавить("СФОборотПоРеализацииРегл");
		СписокПолей.Добавить("СФСуммаРегл");
		СписокПолей.Добавить("СФСуммаНДСВВалютеДоговора");
		СписокПолей.Добавить("СФСуммаВВалютеДоговора");
		СписокПолей.Добавить("СФСуммаАкцизаРегл");
		СписокПолей.Добавить("СФОборотПоРеализацииВВалютеДоговора");
	КонецЕсли;
...
Процедура ИнициализацияКомпоновщикаНастроек(ПользовательскиеНастройки = Неопределено)
	...
	Если СпособФормирования = "ПоПервичнымДокументам" Тогда
		Если Отчет.ВидОтчета = "НаличиеСчетовФактурВыданных" Тогда
			ИмяМакета = "НаличиеСчетовФактурВыданныхПоДокументам";
		Иначе
			ИмяМакета = "НаличиеДокументовРеализацииПоДокументам";
		КонецЕсли;
	Иначе
