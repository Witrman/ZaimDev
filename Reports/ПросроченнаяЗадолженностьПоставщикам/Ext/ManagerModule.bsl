...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт
	...
	ЗаголовокОтчета = НСтр("ru = 'Просроченная задолженность поставщикам на %1'");
...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	УстановитьТекстЗапроса(Запрос, ПараметрыОтчета, ПараметрыОтчета.Дата2);
	
	ПросроченнаяЗадолженность2 = Запрос.Выполнить();
	
	УстановитьТекстЗапроса(Запрос, ПараметрыОтчета, ПараметрыОтчета.Дата3);
...
Функция ПустыеПараметрыКомпоновкиОтчета() Экспорт
	...
	ПараметрыОтчета.Вставить("ТипЗадолженности" , 2);
...
Процедура УстановитьТекстЗапроса(Запрос, ПараметрыОтчета, КонДата)
	...
	Запрос.УстановитьПараметр("СчетаДтЗадолженности", БухгалтерскиеОтчетыВызовСервера.ВозвратитьМассивСчетовДтКтЗадолженности(Истина, Истина, Истина));
	...
	|			ТиповыеВидыСубконто.Ссылка КАК Ссылка
	|		ИЗ
	|			ПланСчетов.Типовой.ВидыСубконто КАК ТиповыеВидыСубконто
	|		ГДЕ
	|			ТиповыеВидыСубконто.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоТиповые.Договоры)) КАК СчетаДоговоров
