...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "ФормаОтчета2006Кв4";
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""справка о перечислении пенсионных взносов"" (заполнение)";
...
Процедура ВывестиВДокумент(СписокСохранения)
	...
	Макет = Отчет.ПолучитьМакет("МакетФормаОтчета2006Кв4");

	// Шапка состоит из нескольких областей
	ОбластьОрганизацияРеквизиты = Макет.ПолучитьОбласть("ОрганизацияРеквизиты");
	Если ВыводитьРНН Тогда
		ОбластьОрганизацияРНН_БИН = Макет.ПолучитьОбласть("ОрганизацияРНН");
		ОбластьРеквизитыРаботника = Макет.ПолучитьОбласть("РеквизитыРНН_СИКРаботника");
	Иначе 
		ОбластьОрганизацияРНН_БИН = Макет.ПолучитьОбласть("ОрганизацияБИН");
		ОбластьРеквизитыРаботника = Макет.ПолучитьОбласть("РеквизитыИИНРаботника");
	КонецЕсли;		
	...
	ТекТабличныйДокумент.Вывести(ОбластьОрганизацияРеквизиты);
	ТекТабличныйДокумент.Вывести(ОбластьОрганизацияРНН_БИН);
	ТекТабличныйДокумент.Вывести(ОбластьШапка);
	ТекТабличныйДокумент.Вывести(ОбластьРеквизитыРаботника);
	ТекТабличныйДокумент.Вывести(ОбластьПериод);
	...
	Для Каждого КлючЗначение Из СписокСохранения Цикл
		...
		Если Позиция <> 0 Тогда
			...
			Если (ПрефиксИмени = "Период_") 
				 Или (ПрефиксИмени = "НомерДокумента_") 
				 Или (ПрефиксИмени = "ДатаДокумента_") 				 
				 Или (ПрефиксИмени = "СуммаДокумента_")
				 Или (ПрефиксИмени = "Сумма_") Тогда
...
Процедура СохранитьДанныеНаСервере()
	...
	СписокСохранения.Вставить("ВерсияФормы", "01/10/2006");
	...
	КлючеваяОперация = "Документ ""регламентированный отчет справка о перечислении пенсионных взносов"" (запись)'";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока Стр < ВсегоСтраниц + 1  Цикл
		...
		Если Стр = 1 Тогда 
			НаименованиеДляЗаписи = НСтр("ru = 'Справка о ППВ с '") + ТабДок.Области.ПериодС.Текст + НСтр("ru = ' по '") + ТабДок.Области.ПериодПО.Текст;
		КонецЕсли;	
...
Функция ФормаРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь)
	...
	МассивВидовСтрок = Новый Массив();
	МассивВидовСтрок.Добавить(Перечисления.РасчетыСБюджетомФондамиВидСтроки.Перечисление);
	МассивВидовСтрок.Добавить(Перечисления.РасчетыСБюджетомФондамиВидСтроки.Возврат);
	...
	Запрос.УстановитьПараметр("парамВидыСтрок",				  	   МассивВидовСтрок);
	Запрос.УстановитьПараметр("парамВидСтрокиВозврат",			   Перечисления.РасчетыСБюджетомФондамиВидСтроки.Возврат);
	Запрос.УстановитьПараметр("парамВнутреннееСовместительство",   Перечисления.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство);
	...
	|		ПО ДанныеОФизЛице.Ссылка = ФИОФизЛица.ФизЛицо
	|ГДЕ
	...
	|	ОПВРасчетыСФондами.Регистратор.Номер КАК НомерДокумента,
	|	ОПВРасчетыСФондами.Регистратор.Дата КАК ДатаДокумента,
	|	МАКСИМУМ(ИтогиПоРегистратору.СуммаДокумента) КАК СуммаДокумента,    
	|	СУММА(ВЫБОР КОГДА ОПВРасчетыСФондами.ВидСтроки = &парамВидСтрокиВозврат ТОГДА -1 ИНАЧЕ 1 КОНЕЦ * ОПВРасчетыСФондами.Взнос) КАК Сумма
	|ИЗ
	|	РегистрНакопления.ОПВРасчетыСФондами КАК ОПВРасчетыСФондами
	|	ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|							ОПВРасчетыСФондами.Регистратор,
	|							СУММА(ВЫБОР КОГДА ОПВРасчетыСФондами.ВидСтроки = &парамВидСтрокиВозврат ТОГДА -1 ИНАЧЕ 1 КОНЕЦ * ОПВРасчетыСФондами.Взнос) КАК СуммаДокумента
	|						ИЗ
	|							РегистрНакопления.ОПВРасчетыСФондами КАК ОПВРасчетыСФондами
	|						ГДЕ
	|							ОПВРасчетыСФондами.Период МЕЖДУ &парамПериодС И &парамПериодПо
	|							И ОПВРасчетыСФондами.Организация В (&парамСписокСтруктурныхЕдиниц)
	|							И ОПВРасчетыСФондами.ВидСтроки В (&парамВидыСтрок)
	|						СГРУППИРОВАТЬ ПО
	|							ОПВРасчетыСФондами.Регистратор) КАК ИтогиПоРегистратору
	|		ПО ОПВРасчетыСФондами.Регистратор = ИтогиПоРегистратору.Регистратор
	|	
	|ГДЕ
	|	ОПВРасчетыСФондами.Период МЕЖДУ &парамПериодС И &парамПериодПо
	|	И ОПВРасчетыСФондами.Организация В (&парамСписокСтруктурныхЕдиниц)
	|	И ОПВРасчетыСФондами.ВидСтроки В (&парамВидыСтрок)
	|	И ОПВРасчетыСФондами.ФизЛицо = &парамФизЛицо
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(ОПВРасчетыСФондами.Период, МЕСЯЦ),
	|	ОПВРасчетыСФондами.Регистратор,
	|	ОПВРасчетыСФондами.Регистратор.Номер,
	|	ОПВРасчетыСФондами.Регистратор.Дата
	|";	
	...
	|	ОПВРасчетыСФондами.НомерДокумента,
	|	ОПВРасчетыСФондами.ДатаДокумента,
	|	ОПВРасчетыСФондами.СуммаДокумента,
	|	ОПВРасчетыСФондами.Сумма
	|
	|ИЗ
	|	(" + ОПВРасчетыСФондамиТекст + ") КАК ОПВРасчетыСФондами
	|
	...
	|		ПО КонтактнаяИнформацияТелефон.Объект = &парамОрганизация 
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НомерСтроки = 1 Тогда
			...
			СписокСохранения.Вставить("ОргНазв",  Выборка.НазваниеОрганизации);
			
			РеквизитыОрганизации = "";
			Если ЗначениеЗаполнено(Выборка.ЮрАдресОрганизации) Тогда
				РеквизитыОрганизации = Выборка.ЮрАдресОрганизации;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Выборка.ТелефонОрганизации) Тогда
				...
				КонецЕсли;
				РеквизитыОрганизации = РеквизитыОрганизации + НСтр("ru = 'тел. '") + Выборка.ТелефонОрганизации;
			КонецЕсли;
			
			СписокСохранения.Вставить("РеквизитыОрганизации", РеквизитыОрганизации);
			Если ВыводитьРНН Тогда
				СписокСохранения.Вставить("РННОрганизации",   Выборка.РННОрганизации);
			Иначе 
				СписокСохранения.Вставить("БИНОрганизации",   Выборка.БИНОрганизации);
			КонецЕсли;
			
			СписокСохранения.Вставить("ДатаПодписи",  ДатаПодписи);

			// Реквизиты физлица
			СписокСохранения.Вставить("ФИОРаботника", Выборка.ФИОРаботника); 
			СписокСохранения.Вставить("ДатаРождения", Выборка.ДатаРождения); 
		...
		СписокСохранения.Вставить("Период_" + СтрНомерСтроки, Выборка.Период);
		
		// Заполняем колонки отчета
		СписокСохранения.Вставить("НомерДокумента_" + СтрНомерСтроки, Выборка.НомерДокумента);
		СписокСохранения.Вставить("ДатаДокумента_"  + СтрНомерСтроки, Выборка.ДатаДокумента);
		СписокСохранения.Вставить("СуммаДокумента_" + СтрНомерСтроки, Выборка.СуммаДокумента);
		
		ИтогоСумма = ИтогоСумма + Выборка.Сумма;
		СписокСохранения.Вставить("Сумма_" + СтрНомерСтроки, Выборка.Сумма);
		
	КонецЦикла;	
	
	// Итоги отчета
	СписокСохранения.Вставить("ИтогоСумма", ИтогоСумма);
