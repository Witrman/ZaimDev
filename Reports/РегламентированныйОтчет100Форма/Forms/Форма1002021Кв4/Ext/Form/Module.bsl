...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма1002021Кв4";
	КодФормы 		= "Форма100";
...
Функция Форма100РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	ДополнитьСписокКодовСтрокИзКлассификатора("100.00.052.A", СписокДохКодовСтрок, мМакетКодовСтрок);
	...
	ДополнитьСписокКодовСтрокИзКлассификатора("100.00.052.B", СписокРасКодовСтрок, мМакетКодовСтрок);
...
Процедура Форма100РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	Если Изменено Тогда
		...
		s_100_00_037 = АвтоматическийРасчетПоСтрокам(ТекИмяФормы, ТаблицаДанных, "s_100_00_037", СтруктураА, Форма100ПолеТабличногоДокументаСтраница3, Ложь);		
	КонецЕсли;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_049", ИтогоНОД, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница4, Неопределено));
	...
	Если Изменено Тогда
		...
		s_100_00_052_a = АвтоматическийРасчетПоСтрокам(ТекИмяФормы, ТаблицаДанных, "s_100_00_052_a", СтруктураА, Форма100ПолеТабличногоДокументаСтраница4, Ложь);		
	КонецЕсли;
	...
	Если Изменено Тогда
		...
		s_100_00_052 = АвтоматическийРасчетПоСтрокам(ТекИмяФормы, ТаблицаДанных, "s_100_00_052", СтруктураА, Форма100ПолеТабличногоДокументаСтраница4, Истина);		
	КонецЕсли;
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_060", ЧистыйДоход, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница5, Неопределено));
...
Функция Форма100ПолучитьСписокКодовДоходов()
	...
	СписокДохКодовСтрок.Добавить("100.00.052.A");
...
Функция Форма100ПолучитьСписокКодовРасходов()
	...
	СписокРасКодовСтрок.Добавить("100.00.052.B");
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела, ТекстЗапроса = "", СписокСчетов = Неопределено, СписокКодовСтрок = Неопределено)
	...
	Если КодФормы = "Форма100" Тогда 
		ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма1002021Кв4");
