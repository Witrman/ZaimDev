...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма1002016Кв4";
	КодФормы 		= "Форма100";
...
Процедура Форма100РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	// Рассчитаем строку 100.00.038. Чистый доход. 
	// Определяется как 100.00.033 - 100.00.035.
	ЧистыйДоход = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_033")
				- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_035");
					
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_038", ЧистыйДоход, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница4, Неопределено));
...
Функция Форма100_01СтрокаПрочееРассчитатьДанныеФормы(ПериодФормирования, ВыдаватьСообщения = Ложь) 
	...
	ТекстЗапроса = "ВЫБРАТЬ
				   ...
				   |
	               |УПОРЯДОЧИТЬ ПО
	               //|	КонечныеИтоговыеДанные.Контрагент.ИдентификационныйКодЛичности"+УсловиеСортировкиДляИИН_БИН+", КонечныеИтоговыеДанные.ПризнакВидаУчета";
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела, ТекстЗапроса = "", СписокСчетов = Неопределено, СписокКодовСтрок = Неопределено)
	...
	Если КодФормы = "Форма100" Тогда 
		ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма1002016Кв4");
