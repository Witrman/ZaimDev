...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма1002019Кв1";
	КодФормы 		= "Форма100";
...
Процедура Форма100ЗаполнитьАвтоНаСервере(Перезаполнить = Ложь) 
	...
	СписокКодовОбластей.Добавить("s_100_00_029");
	СписокКодовОбластей.Добавить("s_100_00_030");
	СписокКодовОбластей.Добавить("s_100_00_031");
	
	СписокКодовОбластей.Добавить("s_100_00_033_1");
	СписокКодовОбластей.Добавить("s_100_00_033_2");
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_037", Окр(?(СтавкаКПН <> NULL, СтавкаКПН, 0)), Форма100ПолеТабличногоДокументаСтраница3);
	...
	мСтруктураЗначений.Вставить("s_100_00_003_1", РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_003_1"));	
...
Функция Форма100РассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) 
	...
	Запрос.УстановитьПараметр("КодСтрокиКоррДоходов", 	"100.00.022.I");
	Запрос.УстановитьПараметр("КодСтрокиКоррВычетов", 	"100.00.022.II");
	Запрос.УстановитьПараметр("КодСтроки31",            "100.00.031");
	...
	КодыДляИсключенияИзОсновногоЗапроса.Добавить("100.00.031");
	...
	|			И (ВТ_СведенияОкодахСтрокДекларации.КодСтроки = &КодСтроки31)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СведенияОкодахСтрокДекларации КАК ВТ_СведенияОкодахСтрокДекларации
	|			ПО НалоговыйОстаткиИОбороты.Субконто1 = ВТ_СведенияОкодахСтрокДекларации.ДоходРасход
	|				И (ВТ_СведенияОкодахСтрокДекларации.КодСтроки = &КодСтроки31)) КАК ЗапросПоСтроке30VII
	|ИТОГИ
	|	СУММА(Сумма)
	|ПО
	|	КодСтроки";
...
Процедура Форма100РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	//Рассчитаем ячейку s_100_00_004. Чтобы не происходило занижения доходов, учтем в ней сумму строки s_100_00_006
	СтруктураА = Новый Структура;
	ИмяОбластиА = "s_100_00_006";
	СтруктураА.Вставить(ИмяОбластиА);			
	s_100_00_004 = АвтоматическийРасчетПоСтрокам(ТекИмяФормы, ТаблицаДанных, "s_100_00_004", СтруктураА, Форма100ПолеТабличногоДокументаСтраница1, Ложь);		
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_004", s_100_00_004, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница1, Неопределено));
	...
	//Рассчитаем ячейку s_100_00_019, с учетом входящей в нее s_100_00_019_1
	Если Изменено Тогда
		СтруктураА = Новый Структура;
		ИмяОбластиА = "s_100_00_019_1";
		СтруктураА.Вставить(ИмяОбластиА);			
		s_100_00_019 = АвтоматическийРасчетПоСтрокам(ТекИмяФормы, ТаблицаДанных, "s_100_00_019", СтруктураА, Форма100ПолеТабличногоДокументаСтраница2, Ложь);		
	КонецЕсли;
	
	//Рассчитаем ячейку 100.00.021I. Вычеты
	Вычеты	= 0;
	Для Н = 9 По 20 Цикл
		ИмяГрафы = "s_100_00_0" + Формат(Н, "ЧЦ=2; ЧВН=");
	...
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_021_1", Вычеты, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница3, Неопределено));
	
	//Рассчитаем ячейку 100.00.021   Всего вычетов 
	
	ВсегоВычетов = 0;
	Значение_21II = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_021_2"); 
	Значение_021III = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_021_3");
	...
	Если Значение_021III > 0 И ПостоянноеУчреждениеРК Тогда
		ВсегоВычетов = Значение_021III; 
	ИначеЕсли Значение_21II > 0 Тогда
		ВсегоВычетов = Значение_21II;
	Иначе
		ВсегоВычетов = Вычеты;
	КонецЕсли;
	...
	// Рассчитаем строку 100.00.028. Всего налогооблагаемого дохода/убытка (100.00.025 - 100.00.027)
	...
	// Рассчитаем строку 100.00.030 Итого налогооблагаемого дохода/убытка (100.00.028 + 100.00.29)
	ИтогоНОД = НОДСУчетомИностранногоДохода 
	+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_029");
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_030", ИтогоНОД, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница3, Неопределено));
	
	// Рассчитаем строку 100.00.032. Убыток, подлежащий переносу (100.00.030 + 100.00.031 + 100.02.008_1). строку 030 и 031 учитываем, только если они отрицательная
	Убыток = ?(ИтогоНОД < 0, -ИтогоНОД, 0) 
	+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_031")
	+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "Строка008_1");
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_032", Убыток, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница3, Неопределено));
	
	//// Рассчитаем строку 100.00.033. Уменьшение НОД по п. 1, 2 и 4 ст. 288
	Если Изменено Тогда
		СтруктураА = Новый Структура;
		СтруктураА.Вставить("s_100_00_033_1");
		СтруктураА.Вставить("s_100_00_033_2");
		s_100_00_033 = АвтоматическийРасчетПоСтрокам(ТекИмяФормы, ТаблицаДанных, "s_100_00_033", СтруктураА, Форма100ПолеТабличногоДокументаСтраница3, Истина);		
	КонецЕсли;
	
	// Рассчитаем строку 100.00.034. НОД  с учетом уменьшения: s_100_00_030 - s_100_00_033 
	// В случае если строка 100.00.033 больше строки 100.00.030, в строке 100.00.034 указывается ноль
	НОДсУчетомУменьшения = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_030")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_033");
	
	Если НОДсУчетомУменьшения < 0 Тогда
		НОДсУчетомУменьшения = 0		
	КонецЕсли;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_034", НОДсУчетомУменьшения, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница3, Неопределено));
	
	// Рассчитаем строку 100.00.036. НОД  с учетом перенесенных убытков: s_100_00_034 - s_100_00_035 
	// Если строка 100.00.035 больше строки 100.00.034, в строке 100.00.036 указывается ноль.
	НОДсУчетомУбытков = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_034")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_035");
	
	Если НОДсУчетомУбытков < 0 Тогда
		НОДсУчетомУбытков = 0		
	КонецЕсли;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_036", НОДсУчетомУбытков, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница3, Неопределено));
	
	// Рассчитаем строку 100.00.037. КПН: s_100_00_036 х s_100_00_037 
	КПН = Окр(РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_036")
	* РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_037") / 100);
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_038", КПН, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница3, Неопределено));
	
	// Рассчитаем строку 100.00.039. Исчисленный КПН: s_100_00_038  - s_100_00_039_1- s_100_00_039_2 .. - s_100_00_039_7 
	// Если полученная разница меньше ноля, то в строке 100.00.036 указывается ноль;
	ИсчисленныйКПН = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_038")
	...
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_039_3")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_039_4")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_039_5")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_039_6")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_039_7");
	
	Если ИсчисленныйКПН < 0 Тогда
		ИсчисленныйКПН = 0		
	КонецЕсли;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_039", ИсчисленныйКПН, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница4, Неопределено));
	
	// Рассчитаем строку 100.00.040.Сумма исчисленного корпоративного подоходного налога за налоговый период с учетом уменьшения налогового обязательства. 
	// Определяемая как разница строк (100.00.039 - 100.00.040):
	ИсчисленныйКПНСУчетомУменьшения = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_039")
	- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_040_1");
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_040", ИсчисленныйКПНСУчетомУменьшения, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница4, Неопределено));
	
	// Рассчитаем строку 100.00.041. Чистый доход. 
	// Определяется как 100.00.036 - 100.00.038.
	...
	Если НерезидентРК Тогда
		ЧистыйДоход = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_036")
		- РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_038");
	Иначе
		ЧистыйДоход = 0;
	КонецЕсли;
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_041", ЧистыйДоход, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница4, Неопределено));
	...
	ИтоговыйКПН = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_040")
	+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_042_1")
	+ РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, "s_100_00_042_2_2");
	
	РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_100_00_043", ИтоговыйКПН, ?(Изменено, Форма100ПолеТабличногоДокументаСтраница4, Неопределено));
...
Функция Форма100ПолучитьСписокКодовДоходов()
	...
	СписокДохКодовСтрок.Добавить("100.00.031");
	СписокДохКодовСтрок.Добавить("100.00.027");	 		
	СписокДохКодовСтрок.Добавить("100.00.033.I"); 	
	СписокДохКодовСтрок.Добавить("100.00.033.II"); 	
...
Функция Форма100ПолучитьСписокКодовРасходов()
	...
	СписокРасКодовСтрок.Добавить("100.00.031");
	
	СписокРасКодовСтрок.Добавить("100.00.033.I"); 	
	СписокРасКодовСтрок.Добавить("100.00.033.II"); 	
...
Функция Форма100ПолучитьСписокКодовСтрокБезРасшифровки()
	...
	СписокКодовДляИсключения.Добавить("s_100_00_033"); // Итоговая. Арифметически расчитываемая
	СписокКодовДляИсключения.Добавить("s_100_00_033_1"); 	
	СписокКодовДляИсключения.Добавить("s_100_00_033_2"); 	
	...
	СписокКодовДляИсключения.Добавить("s_100_00_029"); // КИК 
	СписокКодовДляИсключения.Добавить("s_100_00_030"); // ИТОГО, арифметически рассчитываемая
	СписокКодовДляИсключения.Добавить("s_100_00_032"); // убыток, подлежащий переносу, автоматически расчитываемая
	СписокКодовДляИсключения.Добавить("s_100_00_039_1"); 
	СписокКодовДляИсключения.Добавить("s_100_00_039_2"); 
	СписокКодовДляИсключения.Добавить("s_100_00_039_3"); 
	СписокКодовДляИсключения.Добавить("s_100_00_039_4"); 
	СписокКодовДляИсключения.Добавить("s_100_00_039_5"); 
	СписокКодовДляИсключения.Добавить("s_100_00_039_6");
	СписокКодовДляИсключения.Добавить("s_100_00_039_7");
	СписокКодовДляИсключения.Добавить("s_100_00_040"); 
	СписокКодовДляИсключения.Добавить("s_100_00_040_1"); 
	
	СписокКодовДляИсключения.Добавить("s_100_00_041");     //чистый доход, арифметически расчитываемая
	СписокКодовДляИсключения.Добавить("s_100_00_043");     //ВСЕГО КПН, арифметически расчитываемая
	...
	СписокКодовДляИсключения.Добавить("s_100_00_042_1"); 
	СписокКодовДляИсключения.Добавить("s_100_00_042_2_2"); 
	СписокКодовДляИсключения.Добавить("s_100_00_042_3"); 
	СписокКодовДляИсключения.Добавить("s_100_00_042_4_1"); 
	СписокКодовДляИсключения.Добавить("s_100_00_042_5"); 
...
Функция Форма100ПолучитьСписокКодовСтрокВключаемыхВРасшифровку(ИмяЯчейки)
	...
	ИначеЕсли ИмяЯчейки = "100.00.004" ИЛИ ИмяЯчейки = "100.00.020" Тогда
		...
		Если ИмяЯчейки = "100.00.004" Тогда
			КодыСтрокРасшифровки.Добавить("100.00.006");
		КонецЕсли;
		
	ИначеЕсли ИмяЯчейки = "100.00.003" Тогда
		КодыСтрокРасшифровки.Добавить("100.00.003");
		КодыСтрокРасшифровки.Добавить("100.00.003.I");
		
	ИначеЕсли ИмяЯчейки = "100.00.019" Тогда
		КодыСтрокРасшифровки.Добавить("100.00.019");
		КодыСтрокРасшифровки.Добавить("100.00.019.I");
...
Функция ПолучитьАдресМакетаСКД(Знач ИмяРаздела, ТекстЗапроса = "", СписокСчетов = Неопределено, СписокКодовСтрок = Неопределено)
	...
	Если КодФормы = "Форма100" Тогда 
		ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма1002019Кв1");
...
Процедура Форма100РасшифровкаСКД(ИмяЯчейки, МетодРасшифровки)                          	
	...
	ИначеЕсли СокрЛП(ИмяЯчейки)	 = "100.00.031" Тогда
	...
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "КодСтроки", 					"100.00%");
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "КодСтроки31", 					"100.00.031");
	...
	Если ИмяЯчейки = "100.00.009.IIIA" Тогда 
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "КодСтрокиКоррВычетов", "100.00.022.II");
	КонецЕсли;
