...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
        ...
        ВыводитьЗаголовок                      = Параметры.ВыводитьЗаголовок;
        ВыводитьПодписи                        = Параметры.ВыводитьПодписи;   
        ВыводитьПодписиРуководителей           = Параметры.ВыводитьПодписиРуководителей;
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	Если Параметры.Свойство("РежимВариантаОтчета") Тогда
		Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("РежимВариантаОтчета", Параметры.РежимВариантаОтчета);
		Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("РежимРасшифровки", Истина);
		
		ЗаполняемыеРеквизиты = "СписокСтруктурныхЕдиниц, СписокПодразделений, СписокВладельцевГоловныхПодразделений,
								|ВыводитьЗаголовок, ВыводитьПодписи, ВыводитьПодписиРуководителей";
		ЗаполняемыеРеквизитыОтчета = "НачалоПериода, КонецПериода";
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры, ЗаполняемыеРеквизиты);
		ЗаполнитьЗначенияСвойств(Отчет, Параметры, ЗаполняемыеРеквизитыОтчета);
		...
		Отчет.Группировка.Очистить();
		Для Каждого ГруппировкаРасшифровки Из Параметры.Группировка Цикл
			НоваяГруппировка = Отчет.Группировка.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяГруппировка, ГруппировкаРасшифровки);
		КонецЦикла;
		...
		Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если ОтборДляУдаления <> Неопределено Тогда
			Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Удалить(ОтборДляУдаления);
		КонецЕсли;
		
		Отчет.РежимРасшифровки = Истина;
		
		ПараметрыОтчета = Новый Структура(ЗаполняемыеРеквизитыОтчета + "," + ЗаполняемыеРеквизиты);
		ЗаполнитьЗначенияСвойств(ПараметрыОтчета, Параметры, ЗаполняемыеРеквизитыОтчета + "," + ЗаполняемыеРеквизиты);
		ПараметрыОтчета.Вставить("ИдентификаторОтчета", БухгалтерскиеОтчетыКлиентСервер.ПолучитьИдентификаторОбъекта(ЭтаФорма));
		Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("ПараметрыОтчета", ПараметрыОтчета);
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВыбранноеДействие <> ДействиеОбработкиРасшифровкиКомпоновкиДанных.Нет Тогда
		...
		Иначе
			...
			ПараметрыФормы.Вставить("РежимВариантаОтчета"                   , РежимВариантаОтчета);
			ПараметрыФормы.Вставить("ВыводитьЗаголовок"                     , ВыводитьЗаголовок);
			ПараметрыФормы.Вставить("ВыводитьПодписи"                       , ВыводитьПодписи);
			ПараметрыФормы.Вставить("ВыводитьПодписиРуководителей"          , ВыводитьПодписиРуководителей);
			...
			Если ПараметрыВыбранногоДействия.Структура.Количество() <> 0 Тогда
				...
				СтрокаГруппировки = Новый Структура("Использование, Поле, Представление, ТипГруппировки");
				СтрокаГруппировки.Использование = Истина;
				СтрокаГруппировки.Поле = Строка(ПараметрыВыбранногоДействия.Структура[0].ПоляГруппировки.Элементы[0].Поле);
				СтрокаГруппировки.ТипГруппировки = ПредопределенноеЗначение("Перечисление.ТипДетализацииСтандартныхОтчетов.Элементы");
				Группировки.Добавить(СтрокаГруппировки);
			КонецЕсли;
			ПараметрыФормы.Вставить("Группировка", Группировки);
			
			ОткрытьФорму("Отчет.АнализНачисленийРаботникамОрганизаций.Форма.ФормаОтчета", ПараметрыФормы, , Истина);
...
Процедура ГруппировкаПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		Элемент.ПодчиненныеЭлементы.ГруппировкаТипГруппировки.Доступность = НЕ Элемент.ТекущиеДанные.Поле = "ПериодРегистрации"
		                                                                  И НЕ Элемент.ТекущиеДанные.Поле = "ПериодВключенияВФондЗаработнойПлаты"
		                                                                  И НЕ Элемент.ТекущиеДанные.Поле = "ПериодДействия"
		                                                                  И НЕ Элемент.ТекущиеДанные.Поле = "Регистратор";
	КонецЕсли;
...
Процедура СформироватьОтчет(Команда)
	...
	УИДЗамера = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Ложь, "Отчет ""анализ начислений сотрудникам организаций"" (формирование)");
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.РежимВариантаОтчета.Доступность = НЕ Форма.Отчет.РежимРасшифровки И НЕ Форма.ОткрытИзРассылки;
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка") И ЗаполняемыеНастройки.Группировка Тогда
		...
		НоваяСтрока.Использование  = Истина;
		НоваяСтрока.Представление  = НСтр("ru = 'Подразделение организации'");
		НоваяСтрока.ТипГруппировки = Перечисления.ТипДетализацииСтандартныхОтчетов.Элементы;	
	
		НоваяСтрока = Отчет.Группировка.Добавить();
		НоваяСтрока.Поле           = "ВидРасчета";
		НоваяСтрока.Использование  = Истина;
		НоваяСтрока.Представление  = НСтр("ru = 'Начисление'");
		НоваяСтрока.ТипГруппировки = Перечисления.ТипДетализацииСтандартныхОтчетов.Элементы;
	...
	Если ЗаполняемыеНастройки.Свойство("Показатели") И ЗаполняемыеНастройки.Показатели Тогда
		...
		СписокПоказателей.Добавить("УчитываемыеДни"     , НСтр("ru = 'Учитываемые дни'"));
		СписокПоказателей.Добавить("УчитываемыеЧасы"    , НСтр("ru = 'Учитываемые часы'"));
		СписокПоказателей.Добавить("Начислено"          , НСтр("ru = 'Начислено'"));
		СписокПоказателей.Добавить("ФондЗаработнойПлаты", НСтр("ru = 'Фонд заработной платы'"));
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("РежимВариантаОтчета"                   , РежимВариантаОтчета);
...
Процедура ОткрытьФормуВариантыОтчета()
	...
	ОткрытьФорму("Отчет.АнализНачисленийРаботникамОрганизаций.ФормаОбъекта", ПараметрыФормы, ВладелецФормы);
