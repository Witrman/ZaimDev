...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма701012018Кв1";
	КодФормы 		= "Форма70101";
...
Функция Форма70101ЗемельныйНалогРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) Экспорт 
	...
	Если СтруктураРеквизитовФормы.ВидОтчета = "Очередной" Тогда
		...
		ТекстЗапроса = "ВЫБРАТЬ
					   ...
					   |	ВЫБОР
 					   |		КОГДА НЕ Организации.УчетПоМестнымБюджетам ЕСТЬ NULL И Организации.УчетПоМестнымБюджетам
					   |			ТОГДА Организации.МестныйБюджет
					   |		ИНАЧЕ ВЫБОР
					   |			КОГДА НЕ ПодразделенияОрганизаций.УчетПоМестнымБюджетам ЕСТЬ NULL И ПодразделенияОрганизаций.УчетПоМестнымБюджетам
	...
	Иначе
		...
		ТекстЗапроса = "ВЫБРАТЬ
					   ...
					   |	ВЫРАЗИТЬ((Расчет.СуммаЗемельногоНалога * Расчет.КоличествоМесяцев)/12 КАК Число(12,0)) - 
					   |	(ВЫРАЗИТЬ((ВЫРАЗИТЬ((Расчет.СуммаЗемельногоНалога * Расчет.КоличествоМесяцев)/12 КАК Число(12,0))) / &КоличествоОставшихсяПериодовПлатежа КАК Число(12,0)))*(&КоличествоОставшихсяПериодовПлатежа-1) КАК СуммаЗемельногоНалогаОст
					   |ИЗ
					   |	ВТ_Расчет КАК Расчет
					   |ИТОГИ
					   |	Сумма(СуммаЗемельногоНалога),
	...
	Иначе
		...
		Пока ВыборкаЗемля.Следующий() Цикл
			...
			Пока ВыборкаСтрока.Следующий() Цикл	
				...
				СуммаНалога = ВыборкаСтрока.СуммаЗемельногоНалога; 
				...
				Если Не ЗначениеЗаполнено(КБК) Или СокрЛП(КБК) = "104303" Или СокрЛП(КБК) = "104304" 
					...
					ОбщаяСумма104309Окр = ОбщаяСумма104309Окр + ВыборкаСтрока.СуммаЗемельногоНалогаОкр;
					ОбщаяСумма104309Ост = ОбщаяСумма104309Ост + ВыборкаСтрока.СуммаЗемельногоНалогаОст;

		
				ИначеЕсли СокрЛП(КБК) = "104302" Тогда
					
					ОбщаяСумма104302 = ОбщаяСумма104302 + СуммаНалога;
					ОбщаяСумма104302Окр = ОбщаяСумма104302Окр + ВыборкаСтрока.СуммаЗемельногоНалогаОкр;
					ОбщаяСумма104302Ост = ОбщаяСумма104302Ост + ВыборкаСтрока.СуммаЗемельногоНалогаОст;
