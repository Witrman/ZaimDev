...
Процедура ИзменитьПериод(Шаг)
	...
	Если ВидОтчета = "Очередной" Тогда  // годовой
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.мВыбраннаяФорма) Тогда
		мПараметрыПрежнейФормы = Новый Структура("мВыбраннаяФорма, мСохраненныйДок, Налогоплательщик, НалоговыйКомитет,ВидОтчета, мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета",
												Параметры.мВыбраннаяФорма, Параметры.мСохраненныйДок, Параметры.Налогоплательщик, Параметры.НалоговыйКомитет, Параметры.ВидОтчета, Параметры.мДатаНачалаПериодаОтчета, Параметры.мДатаКонцаПериодаОтчета);
	КонецЕсли;	
	...
	Элементы.ВидОтчета.СписокВыбора.Добавить("Очередной");
	Элементы.ВидОтчета.СписокВыбора.Добавить("Дополнительный");
	...
	//при первичном открытии всегда "Очередной"
...
Процедура ВидОтчетаПриИзменении(Элемент)
	...
	Если ВидОтчета = "Очередной" Тогда  // ежеквартально
	...
	УстановитьПериодичностьНаСервере();
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Если мПараметрыПрежнейФормы <> Неопределено Тогда
		...
		Если мПараметрыПрежнейФормы.ВидОтчета <> ВидОтчета Тогда
			ТекстИзменений = ТекстИзменений + ?(ЕстьИзменения, НСТР("ru=' и '"), "") + НСТР("ru = 'вид отчета'");
			ЕстьИзменения = Истина;
			НеобходимоОчиститьФорму = Истина;
		КонецЕсли;	
...
//		
//	ВыбраннаяФорма = РегламентированнаяОтчетностьКлиент.ВыбратьФормуОтчетаИзДействующегоСписка(ЭтаФорма);
