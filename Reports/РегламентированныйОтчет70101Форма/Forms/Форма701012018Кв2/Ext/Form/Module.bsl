...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма701012018Кв2";
	КодФормы 		= "Форма70101";
...
Процедура Форма70101ЗаполнитьЗемельныйНалог(КодФормы, Перезаполнить)               
	...
	Если СтруктураРеквизитовФормы.ВидОтчета = "Очередной" Тогда
		...
		ВыборкаЗемля = МассивРезультатов[1].Выбрать();
		// РАСЧЕТ ЗЕМЕЛЬНОГО НАЛОГА		
		ОбщаяСумма104309 = 0;
		ОбщаяСумма104302 = 0;
		
		ОбщаяСумма104309Окр = 0;
		ОбщаяСумма104302Окр = 0;
		
		ОбщаяСумма104309Ост = 0;
		ОбщаяСумма104302Ост = 0;
		
		ОбщаяСуммаКОтражению104309 = 0;
		ОбщаяСуммаКОтражению104309Окр = 0;
		ОбщаяСуммаКОтражению104309Ост = 0;
		...
		Пока ВыборкаЗемля.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(КБК) Или СокрЛП(КБК) = "104303" Или СокрЛП(КБК) = "104304" 
				...
				Если ЗначениеЗаполнено(ВыборкаЗемля.ОбъектНалогообложения) Тогда
					Если Не ЗначениеЗаполнено(ВыборкаЗемля.МестныйБюджет) Тогда
						
						ОбщаяСуммаКОтражению104309 	  = ОбщаяСуммаКОтражению104309 	   + ВыборкаЗемля.СуммаЗемельногоНалога;
						ОбщаяСуммаКОтражению104309Окр = ОбщаяСуммаКОтражению104309Окр  + ВыборкаЗемля.СуммаЗемельногоНалогаОкр; 
						ОбщаяСуммаКОтражению104309Ост = ОбщаяСуммаКОтражению104309Ост  + ВыборкаЗемля.СуммаЗемельногоНалогаОст; 
				...
				Иначе
					...
					ОбщаяСумма104309Окр = ОбщаяСумма104309Окр + ВыборкаЗемля.СуммаЗемельногоНалогаОкр; 
					ОбщаяСумма104309Ост = ОбщаяСумма104309Ост + ВыборкаЗемля.СуммаЗемельногоНалогаОст; 
					
				КонецЕсли;
				
				// если КБК заполнен и отличается от перечисленных в форме
			ИначеЕсли СокрЛП(КБК) = "104302" Тогда
				
				Если ЗначениеЗаполнено(ВыборкаЗемля.ОбъектНалогообложения) Тогда 
					Если Не ЗначениеЗаполнено(ВыборкаЗемля.МестныйБюджет) Тогда
						
						ОбщаяСуммаКОтражению104302 	  = ОбщаяСуммаКОтражению104302 	   + ВыборкаЗемля.СуммаЗемельногоНалога;
						ОбщаяСуммаКОтражению104302Окр = ОбщаяСуммаКОтражению104302Окр  + ВыборкаЗемля.СуммаЗемельногоНалогаОкр; 
						ОбщаяСуммаКОтражению104302Ост = ОбщаяСуммаКОтражению104302Ост  + ВыборкаЗемля.СуммаЗемельногоНалогаОст; 
		...
		МассивКБК.Добавить("2");
		МассивКБК.Добавить("9");
		
		Для Каждого Н Из МассивКБК Цикл
			
			Если Н = "2" Тогда
				
				ОбщаяСумма = ОбщаяСумма104302;
				СуммаВМесяц = ОбщаяСумма104302Окр;
				СуммаВПоследнийМесяц = ОбщаяСумма104302Ост;
				
				ОбщаяСуммаКОтражению = ОбщаяСуммаКОтражению104302;
				СуммаКОтражениюВМесяц = ОбщаяСуммаКОтражению104302Окр;
				СуммаКОтражениюВПоследнийМесяц = ОбщаяСуммаКОтражению104302Ост;
				
			Иначе
				
				ОбщаяСумма = ОбщаяСумма104309;
				СуммаВМесяц = ОбщаяСумма104309Окр;
				СуммаВПоследнийМесяц = ОбщаяСумма104309Ост;
				
				ОбщаяСуммаКОтражению = ОбщаяСуммаКОтражению104309;
				СуммаКОтражениюВМесяц = ОбщаяСуммаКОтражению104309Окр;
				СуммаКОтражениюВПоследнийМесяц = ОбщаяСуммаКОтражению104309Ост;
			...
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_C", СуммаКОтражениюВМесяц, Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_D", СуммаКОтражениюВМесяц, Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_E", СуммаКОтражениюВМесяц, Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_F", СуммаКОтражениюВПоследнийМесяц, Истина);			
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_G", ОбщаяСуммаКОтражению, Истина);
				...
				Если ИмяГруппы = "Страница1" Тогда
					...
					Если СокрЛП(ВыборкаАкиматы.БИНАкимата) = "" ТОгда
						Если ЗначениеЗаполнено(ВыборкаАкиматы.МестныйБюджет) Тогда
							...
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ВыборкаАкиматы.МестныйБюджет); 
						КонецЕсли;
	...
	Иначе
		...
		//Заполнение 701.01
		ВыборкаЗемля = РезультатКБК.Выбрать();	
		Пока ВыборкаЗемля.Следующий() Цикл
		...
		Для Каждого Н Из МассивКБК Цикл
			...
			Если Н = "2" Тогда
				...
				ОбщаяСумма = ОбщаяСумма104302;
				ОбщаяСуммаОкр = ОбщаяСумма104302Окр;
				ОбщаяСуммаОст = ОбщаяСумма104302Ост;
				
				ОбщаяСуммаКОтражению = ОбщаяСуммаКОтражению104302;
				СуммаКОтражениюОкр = ОбщаяСуммаКОтражению104302Окр;
				СуммаКОтражениюОст = ОбщаяСуммаКОтражению104302Ост;
				
			Иначе
				
				ОбщаяСумма = ОбщаяСумма104309;
				ОбщаяСуммаОкр = ОбщаяСумма104309Окр;
				ОбщаяСуммаОст = ОбщаяСумма104309Ост;
				
				ОбщаяСуммаКОтражению = ОбщаяСуммаКОтражению104309;
				СуммаКОтражениюОкр = ОбщаяСуммаКОтражению104309Окр;
				СуммаКОтражениюОст = ОбщаяСуммаКОтражению104309Ост;
			...
			Если НомерТекущегоКвартала = 1 Тогда
				РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "s_10430" + Н + "_C", ОбщаяСуммаОкр, Истина);
				РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_C", СуммаКОтражениюОкр, Истина);
			КонецЕсли;
			
			Если НомерТекущегоКвартала <= 2 Тогда
				РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "s_10430" + Н + "_D", ОбщаяСуммаОкр, Истина);
				РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_D", СуммаКОтражениюОкр, Истина);
			КонецЕсли;                           
			
			Если НомерТекущегоКвартала <= 3 Тогда
				РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "s_10430" + Н + "_E", ОбщаяСуммаОкр, Истина);
				РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_E", СуммаКОтражениюОкр, Истина);
			КонецЕсли;                           
			
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "s_10430" + Н + "_F", ОбщаяСуммаОст, Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, КодФормы, "otr_s_10430" + Н + "_F", СуммаКОтражениюОст, Истина);
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма70101018Кв1");
