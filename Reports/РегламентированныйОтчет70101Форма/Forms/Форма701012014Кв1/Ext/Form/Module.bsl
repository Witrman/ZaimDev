...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма701012014Кв1";
	КодФормы 		= "Форма70101";
...
Функция Форма70101НалогНаИмуществоРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) Экспорт 
	...
	Если СтруктураРеквизитовФормы.ВидОтчета = "Очередной" Тогда
		...
		Если Результат.Пустой() И ВыдаватьСообщения Тогда
			Сообщить("Данные для заполнения данных по налогу на имущетсво формы 701.01 за отчетный период отсутствуют...", СтатусСообщения.Информация);	
		КонецЕсли;	
	...
	Иначе
		...
		Иначе
			Если ВыдаватьСообщения Тогда
				Сообщить("Данные для заполнения данных по налогу на имущество формы 701.01 за отчетный период отсутствуют...", СтатусСообщения.Информация);	
			КонецЕсли;
		КонецЕсли;	
...
Функция СформироватьОсновнойЗапросРасчетаНалогаНаИмущество()
	...
	ТекстЗапросаПоОбъектамИмущественногоНалога = "ВЫБРАТЬ
												 ...
												 |ГДЕ ОбъектыИмущественногоНалога.СтруктурнаяЕдиница В (&СписокСтруктурныхЕдиниц)
	                                             |;
	                                             |
												 |
												 |////////////////////////////////////////////////////////////////////////////////
												 ...
												 |			ВЫБОР
												 |				КОГДА ЕСТЬNULL(ТиповойОстаткиНаНачалоГода.СуммаОстаток, 0) = 0 И ЕСТЬNULL(ТиповойОстаткиНаКонецОтчетногоПериода.СуммаОстаток,0) = 0 
												 |					ТОГДА ЕСТЬNULL(ПриобретениеЗаГод.ПриобретениеВТечениеГода,0) // выбыл по стоимости приобретения
												 |				КОГДА ЕстьNull(ТиповойОстаткиНаКонецОтчетногоПериода.СуммаОстаток,0) = 0  
												 |					ТОГДА ЕСТЬNULL(ТиповойОстаткиНаНачалоГода.СуммаОстаток, 0) // выбыл по балансовой стоимости
												 |               ИНАЧЕ ЕСТЬNULL(ТиповойОбороты.СуммаОборотКт,0)  // частичное списание
												 ...
												 |		ГДЕ ТиповойОбороты.СуммаОборотКт <>0
												 |		
												 |		ОБЪЕДИНИТЬ ВСЕ
												 |		
												 |		ВЫБРАТЬ
												 ...
												 |			ОбъектыИмущественногоНалога.ОбъектНалогообложения,
												 |			ОбъектыИмущественногоНалога.Организация,
												 |			ТаблицаПервоначальнаяСтоимость.ПервоначальнаяСтоимость,
												 |			0
												 |		ИЗ
												 |			ОбъектыИмущественногоНалога КАК ОбъектыИмущественногоНалога
												 ...
												 |					ИЗ
												 |						РегистрБухгалтерии.Типовой.Обороты(&ДатаНач, &ДатаКон, Месяц, Счет В ИЕРАРХИИ (&СписокСчетов), , , , ) КАК ТиповойОбороты) КАК ТаблицаПервоначальнаяСтоимость
												 ...
												 |		ТаблицаПервоначальнаяСтоимость.ПервоначальнаяСтоимость <> 0) КАК ИтоговаяТаблица
												 |	
												 |	СГРУППИРОВАТЬ ПО
												 |		ИтоговаяТаблица.ОбъектНалогообложения,
												 |		ИтоговаяТаблица.Организация,
												 |		ИтоговаяТаблица.ИзменениеСтоимостиПоступившихАктивов,
												 |		ИтоговаяТаблица.ИзменениеСтоимостиВыбывшихАктивов) КАК КонИтоговаяТаблица
												 |ГДЕ
												 |(КонИтоговаяТаблица.ИзменениеСтоимостиПоступившихАктивов <> 0 ИЛИ КонИтоговаяТаблица.ИзменениеСтоимостиВыбывшихАктивов <> 0)
												 |
												 |СГРУППИРОВАТЬ ПО
...
Функция Форма70101ЗемельныйНалогРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь) Экспорт 
	...
	Если СтруктураРеквизитовФормы.ВидОтчета = "Очередной" Тогда
		...
		Если Результат.Пустой() И ВыдаватьСообщения Тогда
			Сообщить("Данные для заполнения данных по земельному налогу формы 701.01 за отчетный период отсутствуют...", СтатусСообщения.Информация);	
		КонецЕсли;	
...
Процедура Форма70101ЗаполнитьЗемельныйНалог(КодФормы, Перезаполнить)               
	...
	Если СтруктураРеквизитовФормы.ВидОтчета = "Очередной" Тогда
		...
		Пока ВыборкаЗемля.Следующий() Цикл
			...
			Если СокрЛП(КБК) = "104303" Тогда
				ОбщаяСумма104303 = ОбщаяСумма104303 + Окр(ВыборкаЗемля.СуммаЗемельногоНалога); 
			ИначеЕсли СокрЛП(КБК) = "104304" Тогда
				ОбщаяСумма104304 = ОбщаяСумма104304 + Окр(ВыборкаЗемля.СуммаЗемельногоНалога); 
			ИначеЕсли СокрЛП(КБК) = "104305" Тогда
				ОбщаяСумма104305 = ОбщаяСумма104305 + Окр(ВыборкаЗемля.СуммаЗемельногоНалога); 
			ИначеЕсли СокрЛП(КБК) = "104306" Тогда
				ОбщаяСумма104306 = ОбщаяСумма104306 + Окр(ВыборкаЗемля.СуммаЗемельногоНалога); 
			ИначеЕсли СокрЛП(КБК) = "104307" Тогда
				ОбщаяСумма104307 = ОбщаяСумма104307 + Окр(ВыборкаЗемля.СуммаЗемельногоНалога); 
			// в параметрах расчета земельного налога не заполнен КБК, 
			// то помещаем рассчитанный налог в строку 104308
			ИначеЕсли КБК = NULL ИЛИ СокрЛП(КБК) = "104308" Тогда
				ОбщаяСумма104308 = ОбщаяСумма104308 + Окр(ВыборкаЗемля.СуммаЗемельногоНалога);
				Если КБК = NULL Тогда
					ВыборкаСтрока = ВыборкаЗемля.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					Пока ВыборкаСтрока.Следующий() Цикл
						ТекстСообщения = "";
						ТекстСообщения = "Для объекта земельного налога """ + ВыборкаСтрока.ОбъектНалогообложения + """ в параметрах расчета не заполнены: " + Символы.ПС;
						Если НЕ ЗначениеЗаполнено(ВыборкаСтрока.КатегорияЗемель) Тогда
							 ТекстСообщения = ТекстСообщения + "   ""Категория земель"", в которой для вида земельного налога нужно указать ""Код бюджетной классификации"".";
						Иначе
							Если НЕ ЗначениеЗаполнено(ВыборкаСтрока.ВидЗемельногоНалога) Тогда
								ТекстСообщения = ТекстСообщения + "   для категории земель ""Вид земельного налога"", в котором нужно указать ""Код бюджетной классификации"".";
							Иначе
							    ТекстСообщения = ТекстСообщения + "   ""Код бюджетной классификации"", который указывается для вида земельного налога.";
							КонецЕсли;
						КонецЕсли;
						ТекстСообщения = ТекстСообщения + Символы.ПС + "   Рассчитанная сумма земельного налога отражена в строке с Кодом бюджетной классификации ""104308"".";
			...
			Иначе
			    ВыборкаСтрока = ВыборкаЗемля.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ВыборкаСтрока.Следующий() Цикл 
					 Сообщить("Для объекта земельного налога """ + ВыборкаСтрока.ОбъектНалогообложения + """ указан Код бюджетной классификации """ 
					 			+ СокрЛП(КБК) + """. " + Символы.ПС + "Сумма рассчитанного земельного налога для данного объекта налогообложения не отражена в форме.", СтатусСообщения.Важное);
				КонецЦикла;			
			КонецЕсли;  	
		...
		Для Н = 3 По 8 Цикл			
			Если Н = 3 Тогда
				ОбщаяСумма = ОбщаяСумма104303;
			ИначеЕсли Н = 4 Тогда
				ОбщаяСумма = ОбщаяСумма104304;
			ИначеЕсли Н = 5 Тогда
				ОбщаяСумма = ОбщаяСумма104305;
			ИначеЕсли Н = 6 Тогда
				ОбщаяСумма = ОбщаяСумма104306;
			ИначеЕсли Н = 7 Тогда
				ОбщаяСумма = ОбщаяСумма104307;
			ИначеЕсли Н = 8 Тогда
				ОбщаяСумма = ОбщаяСумма104308;
			КонецЕсли;			
			
			СуммаВМесяц = Окр(ОбщаяСумма/4,0);
			СуммаВПоследнийМесяц = ОбщаяСумма - СуммаВМесяц*3;
					
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,КодФормы,"s_10430" + Н + "_C",СуммаВМесяц,Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,КодФормы,"s_10430" + Н + "_D",СуммаВМесяц,Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,КодФормы,"s_10430" + Н + "_E",СуммаВМесяц,Истина);
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,КодФормы,"s_10430" + Н + "_F",СуммаВПоследнийМесяц,Истина);
				
			// Итоговая сумма текущих платежей
			РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма,КодФормы,"s_10430" + Н + "_G",ОбщаяСумма,Истина);
		КонецЦикла;				
		...
		Пока ВыборкаЗемля.Следующий() Цикл
			...
			Пока ВыборкаСтрока.Следующий() Цикл							
				Если СокрЛП(КБК) = "104303" Тогда
					...
					ОбщаяСумма104303 = ОбщаяСумма104303 + СуммаНалога; 
				ИначеЕсли СокрЛП(КБК) = "104304" Тогда
					СуммаНалога = Окр(ВыборкаСтрока.СуммаЗемельногоНалога * ВыборкаСтрока.КоличествоМесяцев / 12);
					ОбщаяСумма104304 = ОбщаяСумма104304 + СуммаНалога; 
				ИначеЕсли СокрЛП(КБК) = "104305" Тогда
					СуммаНалога = Окр(ВыборкаСтрока.СуммаЗемельногоНалога * ВыборкаСтрока.КоличествоМесяцев / 12);
					ОбщаяСумма104305 = ОбщаяСумма104305 + СуммаНалога; 
				ИначеЕсли СокрЛП(КБК) = "104306" Тогда
					СуммаНалога = Окр(ВыборкаСтрока.СуммаЗемельногоНалога * ВыборкаСтрока.КоличествоМесяцев / 12);
					ОбщаяСумма104306 = ОбщаяСумма104306 + СуммаНалога; 
				ИначеЕсли СокрЛП(КБК) = "104307" Тогда
					СуммаНалога = Окр(ВыборкаСтрока.СуммаЗемельногоНалога * ВыборкаСтрока.КоличествоМесяцев / 12);
					ОбщаяСумма104307 = ОбщаяСумма104307 + СуммаНалога; 
				// в параметрах расчета земельного налога не заполнен КБК, 
				// то помещаем рассчитанный налог в строку 104308
				ИначеЕсли КБК = NULL ИЛИ СокрЛП(КБК) = "104308" Тогда
					СуммаНалога = Окр(ВыборкаСтрока.СуммаЗемельногоНалога * ВыборкаСтрока.КоличествоМесяцев / 12);
					ОбщаяСумма104308 = ОбщаяСумма104308 + СуммаНалога; 			
					Если КБК = NULL Тогда						
						ТекстСообщения = "";
						ТекстСообщения = "Для объекта земельного налога """ + ВыборкаСтрока.ОбъектНалогообложения + """ в параметрах расчета не заполнены: " + Символы.ПС;
						Если НЕ ЗначениеЗаполнено(ВыборкаСтрока.КатегорияЗемель) Тогда
...
Процедура Форма70101РасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки, ТекИмяФормы, Изменено = Истина)
	...
	Если Изменено Тогда
		...
		//Рассчитаем итоговую ячейку по КБК 104303
		ИтогЗначение = 0 ;
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_104303_" + Символ(ИмяЯчейки);		
			ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
		КонецЦикла;
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_104303_G", ИтогЗначение, Истина);
		
		//Рассчитаем итоговую ячейку по КБК 104304
		ИтогЗначение = 0 ;
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_104304_" + Символ(ИмяЯчейки);		
			ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
		КонецЦикла;
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_104304_G", ИтогЗначение, Истина);
		
		//Рассчитаем итоговую ячейку по КБК 104305
		ИтогЗначение = 0 ;
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_104305_" + Символ(ИмяЯчейки);		
			ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
		КонецЦикла;
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_104305_G", ИтогЗначение, Истина);
	
		//Рассчитаем итоговую ячейку по КБК 104306
		ИтогЗначение = 0 ;
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_104306_" + Символ(ИмяЯчейки);		
			ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
		КонецЦикла;
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_104306_G", ИтогЗначение, Истина);
		
		//Рассчитаем итоговую ячейку по КБК 104307
		ИтогЗначение = 0 ;
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_104307_" + Символ(ИмяЯчейки);		
			ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
		КонецЦикла;
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_104307_G", ИтогЗначение);
		
		//Рассчитаем итоговую ячейку по КБК 104308
		ИтогЗначение = 0 ;
		Для ИмяЯчейки = НомерНач  По НомерКон Цикл
			ИмяГрафы = "s_104308_" + Символ(ИмяЯчейки);		
			ИтогЗначение = ИтогЗначение + РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных, ИмяГрафы);
		КонецЦикла;
		РегламентированнаяОтчетность.ПередатьЗначениеВСтруктуру(ЭтаФорма, ТекИмяФормы, "s_104308_G", ИтогЗначение, Истина);
...
Функция ПолучитьАдресМакетаСКД(знач ИмяРаздела)
	ОбщаяСКД = РеквизитФормыВЗначение("ОтчетОбъект").ПолучитьМакет("МакетСКДФорма70101014Кв1");
...
Процедура Форма70101РасшифровкаЗемельныйДополнительныйСКД(ПользовательскиеНастройки)
	...
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "ДатаНач", ПолучитьДатуНачалаНаКлиенте());
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "ДатКон", ПолучитьДатуОкончанияНаКлиенте());
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "СписокСтруктурныхЕдиниц", мСписокСтруктурныхЕдиниц);	
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьПараметр(МассивПараметровДанных, "Налогоплательщик", Налогоплательщик);
