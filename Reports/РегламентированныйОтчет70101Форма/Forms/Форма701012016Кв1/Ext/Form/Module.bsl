...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма701012016Кв1";
	КодФормы 		= "Форма70101";
			...
			ИначеЕсли КБК = NULL ИЛИ СокрЛП(КБК) = "104308" Тогда
				...
				Если КБК = NULL Тогда
					...
					Пока ВыборкаСтрока.Следующий() Цикл
						...
						Если ЗначениеЗаполнено(ВыборкаСтрока.СуммаЗемельногоНалога) Тогда
							
							Если НЕ ЗначениеЗаполнено(ВыборкаСтрока.КатегорияЗемель) Тогда
								 ПараметрТекстСообщения = НСтр("ru = ' ""Категория земель"", в которой для вида земельного налога нужно указать ""Код бюджетной классификации"".'");
							Иначе
								Если НЕ ЗначениеЗаполнено(ВыборкаСтрока.ВидЗемельногоНалога) Тогда
									ПараметрТекстСообщения = НСтр("ru = ' для категории земель ""Вид земельного налога"", в котором нужно указать ""Код бюджетной классификации"".'");
								Иначе
								    ПараметрТекстСообщения = НСтр("ru = ' ""Код бюджетной классификации"", который указывается для вида земельного налога.'");
								КонецЕсли;
							КонецЕсли;

							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для объекта земельного налога ""%1"" в параметрах расчета не заполнены: %2 %3
																											|Рассчитанная сумма земельного налога отражена в строке с Кодом бюджетной классификации ""104308"".'")
																											, ВыборкаСтрока.ОбъектНалогообложения
																											, Символы.ПС
																											, ПараметрТекстСообщения);
						Иначе
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для объекта земельного налога ""%1"" не рассчитана сумма налога. Проверьте параметры расчета.'"), ВыборкаСтрока.ОбъектНалогообложения);
						КонецЕсли;	
