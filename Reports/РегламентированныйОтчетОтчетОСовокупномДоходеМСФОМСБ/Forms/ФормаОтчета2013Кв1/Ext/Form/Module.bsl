...
Процедура Расшифровка(Команда)
	...
	ЦветРасшифровки = ФормаОтчетаПолеТабличногоДокументаСтраница1.Области.s_10_1.ЦветФона;
...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Регламентированный отчет ""о совокупном доходе, методом одного отчета(мсфо мсб)"" (заполнение)";
...
Процедура ФормаОтчетаРасчетНаСервере(ИмяЯчейки, ЗначениеЯчейки )
	...
	Для НомерПериода = 1 по 2 Цикл
		...
		// 30. Валовая прибыль (стр. 010 - стр. 020)																						
		...
		//110. Прибыль (убыток) за  период  от  продолжаемой деятельности (стр. 030+стр. 040+стр. 050-стр.060 – стр. 070 - стр.080 - стр. 090+/- стр. 100)
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_30_" + НомерПериода) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_40_" + НомерПериода) + 
				...
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_70_" + НомерПериода) -
				...
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_100_" + НомерПериода);
				;
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_110_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		
		// 130.  Прибыль (убыток) до налогообложения  (стр.110+/-стр. 120)																				
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_110_" + НомерПериода) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_120_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_130_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		
		// 150. Чистая прибыль (убыток) за период (стр. 130 - стр.140) до вычета доли меньшинства							
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_130_" + НомерПериода) - 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_140_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_150_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		
		// 200.00 Итоговая прибыль (итоговый убыток) за период (стр. 150-стр. 160)																				
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_150_" + НомерПериода) - 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_160_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_200_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
		
		// 240.00 Итоговая прибыль (итоговый убыток) за период (стр. 200 + стр. 220 + стр. 230)			
		Итого = РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_200_" + НомерПериода) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_220_" + НомерПериода) + 
				РегламентированнаяОтчетность.ЗначениеТаблицыДанныхОбычнойФормы(ТаблицаДанных,"s_230_" + НомерПериода);
		РегламентированнаяОтчетность.ПередатьЗначениеВТаблицуДанных(ТаблицаДанных, "s_240_" + НомерПериода ,Итого, ФормаОтчетаПолеТабличногоДокументаСтраница1);
	КонецЦикла;		
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Для НомерПериода = 1 по 2 Цикл
		...
		Пока ВыборкаОтчета.Следующий() Цикл					
			...
			Сумма = 0;
			Если КодСтрокиОтчета = "10" ИЛИ КодСтрокиОтчета = "40" ИЛИ КодСтрокиОтчета = "50" 
				ИЛИ КодСтрокиОтчета = "100" ИЛИ КодСтрокиОтчета = "120" Тогда
			...
			УстановитьЗначенияПоказателей(КодФормы, ТаблицаДанных, "s_" + КодСтрокиОтчета + "_" + НомерПериода, Сумма);				
		КонецЦикла;					
	КонецЦикла; 
