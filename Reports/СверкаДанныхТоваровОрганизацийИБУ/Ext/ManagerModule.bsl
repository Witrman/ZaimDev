...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ПараметрыОтчета.ПоСчетам ИЛИ ПараметрыОтчета.ПоНомерамГТД Тогда
	
		Структура = Структура.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
		
		Если ПараметрыОтчета.ПоНомерамГТД Тогда
			ПолеГруппировки = Структура.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
			ПолеГруппировки.Использование  = Истина;
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("НомерГТД");
			ПолеГруппировки.ТипГруппировки = ТипГруппировкиКомпоновкиДанных.Элементы;
		КонецЕсли;
	...
	ГруппаТоварыОрганизаций = КомпоновщикНастроек.Настройки.Выбор.Элементы.Добавить(Тип("ГруппаВыбранныхПолейКомпоновкиДанных"));
	ГруппаТоварыОрганизаций.Заголовок     = НСтр("ru = 'Товары организаций (БУ)'");
	ГруппаТоварыОрганизаций.Использование = Истина;
	ГруппаТоварыОрганизаций.Расположение  = РасположениеПоляКомпоновкиДанных.Горизонтально;
	Если ПараметрыОтчета.ПоНомерамГТД Тогда
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаТоварыОрганизаций, "НомерГТД");
	КонецЕсли;
	...
	Для Каждого Показатель Из ПараметрыОтчета.Показатели Цикл
		Если Показатель.Использование Тогда
			Если Показатель.Поле = "КоличествоНачальныйОстаток" Тогда
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаТоварыОрганизаций, "КоличествоНачальныйОстаток");
			ИначеЕсли Показатель.Поле = "КоличествоПриход" Тогда
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаТоварыОрганизаций, "КоличествоПриход");
			ИначеЕсли Показатель.Поле = "КоличествоРасход" Тогда
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаТоварыОрганизаций, "КоличествоРасход");
			ИначеЕсли Показатель.Поле = "КоличествоКонечныйОстаток" Тогда
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаТоварыОрганизаций, "КоличествоКонечныйОстаток");
