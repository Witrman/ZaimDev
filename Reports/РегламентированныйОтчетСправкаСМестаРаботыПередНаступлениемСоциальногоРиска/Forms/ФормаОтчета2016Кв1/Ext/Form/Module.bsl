...
Функция НачатьЗамерВремени()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		КлючеваяОперация = "Отчет ""справка с места работы о доходах за последние двенадцать календарных месяцев перед наступлением социального риска""";
...
Процедура ВывестиВДокумент(СписокСохранения)
	...
	ОбластьЗаголовокПриложение4	= Макет.ПолучитьОбласть("ЗаголовокПриложение4");
	...
	ТекТабличныйДокумент.Вывести(ОбластьЗаголовокПриложение4);	
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Иначе // новый документ
			...
			ОтчетОбъект.ПоказыватьЗаголовок = "Приложение7";
...
Процедура СохранитьДанныеНаСервере()
	...
	СписокСохранения.Вставить("ВерсияФормы", "01/03/2016");
	...
	КлючеваяОперация = "Документ ""справка с места работы о доходах за последние двенадцать календарных месяцев перед наступлением социального риска"" (запись)'";
...
Процедура ФормаОтчетаРасчетНаСервере()
	...
	Для Каждого Область Из СканируемыеОбласти Цикл
		...
		// сумма заработка
		Если Найти(Область.Имя, "ОбъектИсчисления_") <> 0 Тогда
			ОбъектИсчисления 		= Область.Значение;
			ИтогоОбъектИсчисления	= ИтогоОбъектИсчисления + ОбъектИсчисления;
		КонецЕсли;
	...
	УстановитьЗначениеОбласти(ТекТабличныйДокумент, "ИтогоОбъектИсчисления", ИтогоОбъектИсчисления);
	...
	// Сумма прописью
	УстановитьЗначениеОбласти(ТекТабличныйДокумент, "ИтогоСумма", ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(ИтогоОбъектИсчисления, Константы.ВалютаРегламентированногоУчета.Получить()));
...
Функция ФормаРассчитатьДанныеФормы(ВыдаватьСообщения = Ложь)
	...
	|ПОМЕСТИТЬ ВТ_ППИ
	|ИЗ
	| 	РегистрНакопления.СОРасчетыСФондами.Обороты(
	|												,
	|												,
	|												Регистратор,
	|												ФизЛицо = &парамФизЛицо 
	|													И Организация В (&парамСписокСтруктурныхЕдиниц) 
	|													И (МесяцНалоговогоПериода МЕЖДУ &парамПериодС И &парамПериодПо)) КАК СОРасчетыСФондамиОбороты
	...
	|  		ПО СОРасчетыСФондамиОбороты.Регистратор = ПлатежноеПоручениеИсходящееПеречислениеСоциальныхОтчислений.Ссылка
	|
	|;
	|
	|/////////////////////////////////////////
	...
	|		ПО СОРасчетыСФондамиОбороты.Регистратор = ПлатежноеПоручениеВходящееВозвратСоциальныхОтчислений.Ссылка
	|;
	|
	|/////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_Регистратор.Регистратор.Номер,
	|	ВТ_Регистратор.Регистратор.Дата,
	|	ВТ_Регистратор.Регистратор
	|ПОМЕСТИТЬ ВТ_Регистратор
	|ИЗ
	|	ВТ_ППИ КАК ВТ_Регистратор
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ППВ КАК ВТ_ППВ
	|		ПО ВТ_Регистратор.Ведомость = ВТ_ППВ.Ведомость.ДокументОснование
	|ГДЕ
	...
	|	СУММА(ДоходыИВзносы.ОбъектИсчисления) КАК ОбъектИсчисления,
	...
	|		СОСведенияОДоходах.ОблагаемаяБаза КАК ОбъектИсчисления,
	...
	|	ГДЕ
	|		СОСведенияОДоходах.ФизЛицо = &парамФизЛицо
	|		И СОСведенияОДоходах.Организация В(&парамСписокСтруктурныхЕдиниц)
	|		И СОСведенияОДоходах.Период МЕЖДУ &парамПериодС И &парамПериодПо
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|			ПО СОРасчетыСФондами.Регистратор = ВТ_Регистратор.Регистратор
	|	ГДЕ
	|		СОРасчетыСФондами.ФизЛицо = &парамФизЛицо
	|		И СОРасчетыСФондами.Организация В(&парамСписокСтруктурныхЕдиниц)
	...
	|	ДоходыИВзносы.ОбъектИсчисления,
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	Пока ТекущийПериод <= КонецМесяца(мДатаКонцаПериодаОтчета) Цикл 
		...
		Для Каждого СтрокаВыборки Из НайденныеСтроки Цикл 
			ОбъектИсчисления = ОбъектИсчисления + ?(СтрокаВыборки.ОбъектИсчисления <> NULL, СтрокаВыборки.ОбъектИсчисления, 0);
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			СписокСохранения.Вставить("ОбъектИсчисления_" + СтрНомерСтроки, ОбъектИсчисления);
	...
	СписокСохранения.Вставить("ИтогоОбъектИсчисления", 	ИтогоОбъектИсчисления);
	...
	// Сумма прописью
	СписокСохранения.Вставить("ИтогоСумма", Формат(ИтогоОбъектИсчисления, "ЧЦ=15; ЧДЦ=2; ЧРД=,") + " (" + ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(ИтогоОбъектИсчисления, Константы.ВалютаРегламентированногоУчета.Получить()) + ")");
...
Процедура ВыводЗаголовка(ТабДок)
	...
	ПоказыватьЗаголовокПриложение4 = Ложь;
	ПоказыватьЗаголовокПриложение7 = Ложь;
	
	Если ОтчетОбъект.ПоказыватьЗаголовок = "Приложение4" Тогда
		ПоказыватьЗаголовокПриложение4 = Истина;		
	КонецЕсли;	
	
	Если ОтчетОбъект.ПоказыватьЗаголовок = "Приложение7" Тогда
		ПоказыватьЗаголовокПриложение7 = Истина;		
	КонецЕсли;	
	
	Если ТабДок.Области.Найти("Заголовок") <> Неопределено Тогда
		ТабДок.Область("Заголовок").Видимость = ПоказыватьЗаголовокПриложение7;
	КонецЕсли;
	
	Если ТабДок.Области.Найти("ЗаголовокПриложение4") <> Неопределено Тогда
		ТабДок.Область("ЗаголовокПриложение4").Видимость = ПоказыватьЗаголовокПриложение4;
	КонецЕсли;   
