...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	ПредставлениеПериода = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода);
	
	Если ПараметрыОтчета.Свойство("РежимРасшифровки") И ПараметрыОтчета.РежимРасшифровки Тогда
		ЗаголовокОтчета = НСтр("ru = 'Расшифровка оборачиваемости товаров%1'");
	Иначе
		ЗаголовокОтчета = НСтр("ru = 'Оборачиваемость товаров%1'");
	КонецЕсли;
	
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОтчета, ПредставлениеПериода);
...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Если Не ПараметрыОтчета.Свойство("РежимРасшифровки") Или Не ПараметрыОтчета.РежимРасшифровки Тогда
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ПериодичностьОтчета = ?(ЗначениеЗаполнено(ПараметрыОтчета.Периодичность), ПараметрыОтчета.Периодичность, ПолучитьПериодичность(НачалоПериода, КонецПериода));
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "Периодичность", ПериодичностьОтчета);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ПериодичностьЦифрой", ПериодичностьЦифрой(ПериодичностьОтчета));
...
//
Функция ПредыдущийПериод(ДатаНачала, ДатаКонца) Экспорт
...
Функция ПериодичностьЦифрой(ПериодичностьСтрокой) Экспорт
...
Функция СчетаУчетаТоваров() Экспорт 
	...
	УсловияОтбора.Вставить("СчетаИсключения", ПланыСчетов.Типовой.МатериалыПереданныеВПереработку);
	
	Возврат ПроцедурыБухгалтерскогоУчета.СформироватьМассивСубсчетовПоОтбору(СписокСчетов, УсловияОтбора);
...
Функция ПустыеПараметрыКомпоновкиОтчета() Экспорт
	...
	ПараметрыОтчета.Вставить("ПериодОборачиваемости"          , Ложь);
	ПараметрыОтчета.Вставить("КоэффициентОборачиваемости"     , Ложь);
	ПараметрыОтчета.Вставить("ИзменениеПрошлогоПериода"       , Ложь);
	ПараметрыОтчета.Вставить("БазаРасчета"                    , "");
	ПараметрыОтчета.Вставить("Периодичность"                  , "");
	ПараметрыОтчета.Вставить("ПериодичностьЦифрой"            , 0);
	
	Возврат ПараметрыОтчета;
	
КонецФункции

Функция ПолучитьПериодичность(НачалоПериода = Неопределено, КонецПериода = Неопределено) Экспорт
	
	Периодичность = "День";
	
	Если ЗначениеЗаполнено(НачалоПериода) И ЗначениеЗаполнено(КонецПериода) Тогда
		ЧислоДнейПериода = (КонецДня(КонецПериода) + 1 - НачалоПериода)/(60*60*24);
		ЧислоДнейВГоду = 365;
