...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мВыбраннаяФорма = "Форма7012022Кв1";
	КодФормы 		= "Форма701";
...
Процедура ЗаполнитьСведенияОНалогоплательщикеНаСервере(ТекущееИмяФормы)
	...
	ОргСведения = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОНалогоплательщике(Налогоплательщик, ДатаПодписи, НалоговыйКомитет, Истина);
...
Процедура Форма701ЗаполнитьАвто(Перезаполнить = Ложь)               
	...
	СтруктураСтавокНалога = СформироватьСтуктуруСтавокНалогаНаТранспорт("2022");
...
Процедура ЗаполнитьПараметрыФормы(ТекФорма, Параметры)
	...
	ТекФорма.парамМРП = ПолучитьВеличинуМРП(ТекФорма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
...
Процедура Расшифровка(Команда)
	...
	Если ТекущийЭлемент.ТекущаяОбласть.ЦветФона = ЦветРасшифровки Тогда 
		...
		ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "Форма7012022Кв1", "ФормаРасшифровки"), ПараметрыФормы);
	Иначе	
...
Функция ПолучитьАдресТаблицыРасшифровки(НомерПакета, ОтборПоМестнымБюджетам)
	...
	Для Каждого СтрокаОтчета из ТаблицаОтчета Цикл  			
		...
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапроса;
		Запрос.УстановитьПараметр("ДатаКон", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ВеличинаМРП = Выборка.МРП;
		Иначе
