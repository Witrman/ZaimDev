...
Процедура ОткрытьФормуВыбораДокументаОснования(Знач ФормаЭДВС, Знач ИмяФормыВыбора) Экспорт
	...
	Если ИмяФормыВыбора = "Документ.РеализацияТоваровУслуг.ФормаВыбора" Тогда
		Отбор.Вставить("ВидОперации", ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализацияТоваров.ПередачаСтруктурномуПодразделению"));
	КонецЕсли;	
...
Процедура ВыполнитьКомандуСоздатьЭДВС(ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Структура") Тогда		
		...
		ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ЭСФ") И ПараметрКоманды.Свойство("ЭтоВводОстатковИзВходящегоЭСФ") Тогда
			ЭтоВводОстатковИзВходящегоЭСФ = ПараметрКоманды.ЭтоВводОстатковИзВходящегоЭСФ;
		ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ЭСФ") И ПараметрКоманды.Свойство("ЭтоВводСписаниеИзВходящегоЭСФ") Тогда
			ЭтоВводСписаниеИзВходящегоЭСФ = ПараметрКоманды.ЭтоВводСписаниеИзВходящегоЭСФ;
	...
	ДополнительныеПараметры.Вставить("ЭтоВводСписаниеИзВходящегоЭСФ", ЭтоВводСписаниеИзВходящегоЭСФ);
	...
	Если ПараметрКоманды.Количество() = 1 И ОткрыватьФормуНовогоДокумента Тогда
		...
		Иначе
			СоздатьЭДВС(МассивДокументовБезЭДВС[0], НомерДетализируемойСтроки,ДетализироватьОС, ЭтоВводОстатковИзВходящегоЭСФ, ЭтоВводНаПеремещениеИзВходящегоСНТ, ЭтоВводСписаниеИзВходящегоЭСФ);
		КонецЕсли;	
	Иначе
		Если МассивДокументовСЭДВС.Количество() = 0 Тогда
...
Процедура СоздатьЭДВСЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ЭтоВводСписаниеИзВходящегоЭСФ = ДополнительныеПараметры.ЭтоВводСписаниеИзВходящегоЭСФ;
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Если МассивДокументов.Количество() = 1 И ДополнительныеПараметры.ОткрыватьФормуНовогоДокумента Тогда
			СоздатьЭДВС(МассивДокументов[0], НомерДетализируемойСтроки, ДетализироватьОС, ЭтоВводОстатковИзВходящегоЭСФ, ЭтоВводНаПеремещениеИзВходящегоСНТ, ЭтоВводСписаниеИзВходящегоЭСФ);
		Иначе
...
Процедура СоздатьСписокЭДВСЗавершение(Ответ, МассивДокументов) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ПараметрыСоздания.Вставить("ЭтоВводСписаниеИзВходящегоЭСФ"		, Ложь);
...
Процедура СоздатьЭДВС(ДокументОснование,НомерДетализируемойСтроки = Неопределено,ДетализироватьОС = Ложь,ЭтоВводОстатковИзВходящегоЭСФ = Ложь,ЭтоВводНаПеремещениеИзВходящегоСНТ = Ложь,ЭтоВводСписаниеИзВходящегоЭСФ = Ложь)
	...
	ЗначенияЗаполнения.Вставить("ЭтоВводСписаниеИзВходящегоЭСФ"		, ЭтоВводСписаниеИзВходящегоЭСФ);
...
#Область КомандаРазбитьНаНесколькоДокументов

Процедура РазбитьДокументЭДВС(ДокументЭДВС) Экспорт
	ТекстВопроса = НСтр(
	"ru = 'Данный документ будет разбит на несколько электронных документов ВС.
	|Продолжить?'");
	
	ОписаниеОповещения = Новый ОписаниеОповещения("РазбитьДокументЭДВСЗавершение",ЭтотОбъект , ДокументЭДВС);
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
КонецПроцедуры	

Процедура РазбитьДокументЭДВСЗавершение(Ответ, ДокументЭДВС) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		
		МассивИсходныхДокументовЭДВС = Новый Массив;
		МассивИсходныхДокументовЭДВС.Добавить(ДокументЭДВС);
		
		ПараметрыСоздания = Новый Структура;
		ПараметрыСоздания.Вставить("МассивДокументовЭДВС" , МассивИсходныхДокументовЭДВС);
		
		СписокЭДВС = ВСВызовСервера.РазбитьЭлектронныеДокументыВС(ПараметрыСоздания);
		
		Если СписокЭДВС <> Неопределено И СписокЭДВС.Количество() > 0 Тогда
			
			ЭСФКлиент.ОповеститьФормы(ВСКлиентСервер.ИмяСобытияЗаписьЭДВС(), СписокЭДВС);
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("Отбор", Новый Структура("Ссылка", Новый ФиксированныйМассив(СписокЭДВС)));
			ОткрытьФорму("Документ.ЭлектронныйДокументВС.Форма.ФормаСписка", ПараметрыФормы,, Истина);
