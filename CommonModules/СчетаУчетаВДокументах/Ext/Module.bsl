...
Процедура ДобавитьПравилоЗаполнения(Правила, ТабличнаяЧасть, Реквизит, Роль, ЗаполнитьПоУмолчанию = Истина) Экспорт
	...
	Правило.ПолноеИмяРеквизита = ОбщегоНазначенияБККлиентСервер.ПолноеИмяРеквизита(Реквизит, ТабличнаяЧасть);
	Правило.Роль               = Роль;
	Правило.Владелец           = -1;
...
Процедура ЗаполнитьПередОтображениемПользователю(Объект) Экспорт
	...
	Если (ТипЗнч(Объект) = Тип("ДанныеФормыСтруктура")
		ИЛИ ТипЗнч(Объект) = Тип("ДанныеФормыСтруктураСКоллекцией"))
		И НЕ Объект.Свойство("ДополнительныеСвойства") Тогда
		ДополнительныеСвойства = Новый Структура;
	Иначе
		ДополнительныеСвойства = Объект.ДополнительныеСвойства;
	КонецЕсли;
	
	Если ЗаполнятьПередЗаписью(ДополнительныеСвойства) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПравоПросмотраСчетовУчета() Экспорт
	...
	Возврат ПравоДоступа("Просмотр", Метаданные.ПланыСчетов.Типовой);
...
Функция КодФункцииНачатьЗаполнениеСчетовУчета(МенеджерОбъекта) Экспорт
	...
	Для Каждого Связь Из Связи Цикл
		...
		ПолноеИмя = ОбщегоНазначенияБККлиентСервер.ПолноеИмяРеквизита(Связь.РеквизитИсточникСвязи, Связь.ТабличнаяЧастьИсточникСвязи);
	...
	Для Каждого Причина Из Причины Цикл
		...
		Для Каждого ЗависимыйСчет Из Связи Цикл
			...
			ПолноеИмяЗависимого = ОбщегоНазначенияБККлиентСервер.ПолноеИмяРеквизита(
	...
	Для Каждого Причина Из Причины Цикл
		...
		Для Каждого ЗависимыйСчет Из Связи Цикл
			...
			ПолноеИмяЗаполняемого = ОбщегоНазначенияБККлиентСервер.ПолноеИмяРеквизита(
				ЗависимыйСчет.РеквизитПриемникСвязи,
				ЗависимыйСчет.ТабличнаяЧастьПриемникСвязи);
			...
			Для Каждого ИсточникДанных Из Связи Цикл
				...
				ПолноеИмяИсточникаДанных = ОбщегоНазначенияБККлиентСервер.ПолноеИмяРеквизита(
...
Процедура ДобавитьВПравилоПоУмолчанию(Правила, Роль)
	...
	Если Роль = "ПредъявленныйНДС_Запасы"
		...
		Или Роль = "ПредъявленныйНДС_НематериальныеАктивы"
		Или Роль = "РеализацияНДС" Тогда
		
		ДобавитьВПравилоУсловиеРазрешено(Правила, "УчитыватьНДС");
	...
	Если Роль = "ПоступлениеНДС"
		Или Роль = "РеализацияНДС"
		Или Роль = "ОтложенныйНДС"
		Или Роль = "ОтложенныйНДСНУ" Тогда
		
		ДобавитьВПравилоУсловиеПроверкиЗаполнения(Правила, "ЗаполненаСуммаНДС");
		
	КонецЕсли;
	
	Если Роль = "Акциз" Тогда
		
		ДобавитьВПравилоУсловиеПроверкиЗаполнения(Правила, "ЗаполненаСуммаАкциза");
...
Функция ФункцияВключена(Роль)
	...
	Если ЭтоСубконтоСтатьяДвиженияДенежныхСредств(Роль) Тогда
		Возврат Истина; //!!!ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиДвиженияДенежныхСредств");
	КонецЕсли;
...
Функция МетодЗаполнения(Правило)
	...
	ИначеЕсли Роль = "ВидОборотаНДСПредъявленного" Тогда
		Возврат "ВидОборотаНДС";
	ИначеЕсли Роль = "Затраты"
		Или Роль = "ЗатратыНУ"
		Или Роль = "РасходыНаПродажу"
		Или Роль = "ИспользованиеМатериалов"
		Или Роль = "ПрочиеРасходы"
		Или Роль = "ПрочиеРасходыНУ"
		Или Роль = "РасходыУслугиБанка"
		Или Роль = "РасходыУслугиФакторинга"
		Или Роль = "СебестоимостьРеализации"
		Или Роль = "СебестоимостьРеализацииНУ"
		Или Роль = "ЗатратыНДС"
		Или Роль = "СебестоимостьРеализации_ПередачаСтруктурномуПодразделению"
		Или Роль = "СебестоимостьРеализации_ПередачаСтруктурномуПодразделениюНУ" Тогда
		Возврат "Затраты";
	ИначеЕсли Роль = "СубконтоЗатрат1" Или Роль = "СубконтоЗатрат2" Или Роль = "СубконтоЗатрат3" Тогда
		Возврат "СубконтоЗатрат";
	ИначеЕсли Роль = "СубконтоДоходов1" Или Роль = "СубконтоДоходов2" Или Роль = "СубконтоДоходов3" Тогда
	...
	ИначеЕсли ЭтоСубконтоДенежныеСредства(Роль) Тогда
		Возврат "СубконтоДенежныеСредства";
	ИначеЕсли Роль = "ВидНДС_НДС" 
		Или Роль = "ВидНДС_НДСОблагаемыйИмпорт" 
		Или Роль = "ВидНДС_НДСМетодомЗачета"
		Или Роль = "ВидНДС_ИмпортНоменклатура" Тогда
		Возврат "ВидНДС";
	ИначеЕсли Роль = "ЗаймыКредитыКПолучению"
		Или Роль = "ЗаймыКредитыКПолучениюНУ"
		Или Роль = "ЗаймыКредитыКВыплате"
		Или Роль = "ЗаймыКредитыКВыплатеНУ" Тогда
		Возврат "ЗаймыКредиты";
	Иначе
		Возврат Роль;
	КонецЕсли;
...
Функция ИсточникиДанныхПравила(Правило)
	...
	Если МетодЗаполнения = "Выручка"
	 Или МетодЗаполнения = "Запасы"
	 ...
	 Или МетодЗаполнения = "СубконтоДоходов" Тогда
		ИсточникиДанных.Добавить("Основание");
	КонецЕсли;
	
	Если МетодЗаполнения = "СубконтоЗатрат" ИЛИ МетодЗаполнения = "СубконтоДоходов" Тогда
	...
	Если МетодЗаполнения = "Деньги" Тогда
		...
		ИсточникиДанных.Добавить("БанковскийСчет");
		ИсточникиДанных.Добавить("Касса");
	КонецЕсли;
	...
	Если МетодЗаполнения = "СубконтоДенежныеСредства" Тогда
		ИсточникиДанных.Добавить("СчетОрганизации");
		ИсточникиДанных.Добавить("Касса");
	КонецЕсли;
...
Функция НастройкиДляЗаполнения(Правило)
	...
	ИначеЕсли МетодЗаполнения = "ПредъявленныйНДС" ИЛИ МетодЗаполнения = "РеализацияНДС" Тогда
	...
	ИначеЕсли МетодЗаполнения = "ВидОборотаНДС" Тогда
		
		Настройки.Добавить("МетодОтнесенияНДСВЗачет");
	...
	ИначеЕсли МетодЗаполнения = "Выручка" Тогда
		...
		Настройки.Добавить("СчетаУчетаНоменклатуры");
		Настройки.Добавить("СчетаУчетаНМА");
		Настройки.Добавить("СчетаУчетаОС");
		
	ИначеЕсли МетодЗаполнения = "СубконтоДоходов"
		Или МетодЗаполнения = "Доход" Тогда
	...
	ИначеЕсли МетодЗаполнения = "Расчеты" Тогда
		...
		//Если Правило.Роль = "АвансыПокупателя" Или Правило.Роль = "АвансыПоставщику" Тогда
			Настройки.Добавить("АвтозаполнятьСчетаВзаиморасчетов");
		//КонецЕсли;
	...
	ИначеЕсли МетодЗаполнения = "ОсновныеСредстваНУ" Тогда
		
		Настройки.Добавить("СчетаУчетаОС");
		
	ИначеЕсли МетодЗаполнения = "НематериальныеАктивы" Тогда
		
		Настройки.Добавить("СчетаУчетаНМА");
		
	ИначеЕсли МетодЗаполнения = "НематериальныеАктивыНУ" Тогда
		
		Настройки.Добавить("СчетаУчетаНМА");
		
	ИначеЕсли МетодЗаполнения = "Деньги" Тогда
		
		Настройки.Добавить("СчетаУчетаДенежныхСредств");
		
	ИначеЕсли МетодЗаполнения = "ЗаймыКредиты" Тогда
...
Функция НастройкиДляПроверкиЗаполнения(Правило)
	...
	Если МетодЗаполнения = "Расчеты"
		Или МетодЗаполнения = "РасчетыПоЛизингу" Тогда
...
Функция ИсточникиДанныхНастройки(ИмяНастройки)
	...
	ИначеЕсли ИмяНастройки = "СчетаУчетаОС" Тогда
		ИсточникиДанных.Вставить("ОсновноеСредство");
		ИсточникиДанных.Вставить("Организация");
		ИсточникиДанных.Вставить("Дата");
	ИначеЕсли ИмяНастройки = "СчетаУчетаНМА" Тогда
		ИсточникиДанных.Вставить("НематериальныйАктив");
		ИсточникиДанных.Вставить("Организация");
		ИсточникиДанных.Вставить("Дата");
	...
	ИначеЕсли ИмяНастройки = "СчетаУчетаДенежныхСредств" Тогда
		ИсточникиДанных.Вставить("Касса");
		ИсточникиДанных.Вставить("СчетОрганизации");
	ИначеЕсли ЭтоНастройкаУчетнойПолитики(ИмяНастройки) Тогда
		ИсточникиДанных.Вставить("Организация");
		ИсточникиДанных.Вставить("Дата");
	ИначеЕсли ИмяНастройки = "АвтозаполнятьСчетаВзаиморасчетов" Тогда
		ИсточникиДанных.Вставить("АвтозаполнятьСчетаВзаиморасчетов");
	КонецЕсли;
...
Функция ПроверитьУсловиеПоФункциональнымОпциям(Условие) // очередность, сложность - 0
	...
	Если Условие = "ТоварыВЦенахПродажи" 
...
Функция ПроверитьУсловиеПоДаннымЗаполнения(ИмяУсловия, ДанныеЗаполнения) // очередность, сложность - 1
	...
	ИначеЕсли ИмяУсловия = "АвтозаполнятьСчетаВзаиморасчетов" Тогда
	...
	ИначеЕсли ИмяУсловия = "ИспользоватьПереводыВПути" Тогда
		...
		Возврат УчетнаяПолитикаСервер.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(ДанныеЗаполнения.Организация, ДанныеЗаполнения.Дата);
...
Функция ПроверитьУсловиеПоНастройкам(ИмяУсловия, ПланВыполнения, НастройкиЭлемента) // очередность, сложность - 2
	...
	Если ИмяУсловия = "ДоговорСПокупателем" Тогда
	...
	ИначеЕсли ИмяУсловия = "ПредъявленныйНДСУчитываетсяНаОтдельномСчете" Тогда
		...
		Если ДанныеНастройкиЭлементаКоллекции("НалоговыйАгентПоНДС", ПланВыполнения, НастройкиЭлемента) = Истина Тогда
			Возврат Истина;
...
Функция ПроверитьУсловиеПоКоллекции(ИмяУсловия, Коллекция, ОписаниеРеквизитовКоллекции) // очередность, сложность - 3
	...
	Если ИмяУсловия = "ЗаполненаСуммаНДС" Тогда
		
		Результат = ЗаполненРеквизитКоллекции(Коллекция, ОписаниеРеквизитовКоллекции, "СуммаНДС");
		
	ИначеЕсли ИмяУсловия = "ЗаполненаСуммаАкциза" Тогда
		
		Результат = ЗаполненРеквизитКоллекции(Коллекция, ОписаниеРеквизитовКоллекции, "СуммаАкциза");
	...
	ИначеЕсли ИмяУсловия = "ВидНДСМетодомЗачета" Тогда
		
		Результат = Новый Массив;
		Для Каждого ЭлементКоллекции Из Коллекция Цикл
			ВидНДС = ЗначениеРеквизитаЭлементаКоллекции(ЭлементКоллекции, ОписаниеРеквизитовКоллекции, "ВидНДС");
			Результат.Добавить(ВидНДС = Перечисления.ВидыНДС.НДСМетодомЗачета);
		КонецЦикла;
...
Функция НастройкиДляПроверкиУсловия(ИмяУсловия)
	...
	Если ИмяУсловия = "ДоговорСПокупателем" 
...
Функция ИсточникиДанныхДляПроверкиУсловия(ИмяУсловия)
	...
	ИначеЕсли ИмяУсловия = "ИспользоватьПереводыВПути" Тогда
		...
		ИсточникиДанных.Вставить("Дата");
	ИначеЕсли ИмяУсловия = "ВидНДСМетодомЗачета" Тогда
		ИсточникиДанных.Вставить("ВидНДС");
	КонецЕсли;
...
Функция ДанныеНастройки(ИмяНастройки, ПредметыНастройки)
	...
	ИначеЕсли ИмяНастройки = "СчетаУчетаОС" Тогда
		...
		Возврат ДанныеНастройкиСчетаУчетаОС(ПредметыНастройки);
		
	ИначеЕсли ИмяНастройки = "СчетаУчетаНМА" Тогда
		
		Возврат ДанныеНастройкиСчетаУчетаНМА(ПредметыНастройки);
	...
	ИначеЕсли ИмяНастройки = "АвтозаполнятьСчетаВзаиморасчетов" Тогда
		...
		Возврат ДанныеНастройкиАвтозаполнятьСчетаВзаиморасчетов(ПредметыНастройки);
	...
	ИначеЕсли ИмяНастройки = "ЧужиеЗапасы" Тогда
		...
		Возврат ЗначенияРеквизитаПредметовНастройки(ПредметыНастройки, "СчетЗапасов", "Забалансовый", Тип("ПланСчетовСсылка.Типовой"));
	...
	ИначеЕсли ИмяНастройки = "СчетаУчетаДенежныхСредств" Тогда
		...
		Возврат ДанныеНастройкиСчетаУчетаДенежныхСредств(ПредметыНастройки);
...
Процедура ЗаполнитьРеквизит(Параметры)
	...
	ИначеЕсли МетодЗаполнения = "ОтложенныйНДС" ИЛИ МетодЗаполнения = "ОтложенныйНДСНУ" Тогда
		
		ЗаполнитьОтложенныйНДС(Параметры);
		
	ИначеЕсли МетодЗаполнения = "ВидПоступленияНДСПредъявленного"
		Или МетодЗаполнения = "ВидПоступленияНДСПредъявленного_Расходы"
		Или МетодЗаполнения = "ВидПоступленияНДСПредъявленного_ОсновныеСредства"
		Или МетодЗаполнения = "ВидПоступленияНДСПредъявленного_НематериальныеАктивы"
		Или МетодЗаполнения = "ВидПоступленияНДС_Импорт"
		Или МетодЗаполнения = "ВидПоступленияНДС_ВозвратТМЗ"
		Или МетодЗаполнения = "ВидПоступленияНДС_СписаниеТМЗ" Тогда
		
		ЗаполнитьВидПоступленияНДС(Параметры);
		
	ИначеЕсли МетодЗаполнения = "НДСВидОперацииРеализации"
		Или МетодЗаполнения = "НДСВидОперацииРеализации_ПрочийОблагаемыйИмпорт"
		Или МетодЗаполнения = "НДСВидОперацииРеализации_ВозвратТМЗ" Тогда
		
		ЗаполнитьВидРеализацииНДС(Параметры);
		
	ИначеЕсли МетодЗаполнения = "ВидОборотаНДС" Тогда
		
		ЗаполнитьВидОборотаПоНДС(Параметры);
		
	ИначеЕсли МетодЗаполнения = "ВидНДС" Тогда
		
		ЗаполнитьВидНДС(Параметры);
		
	ИначеЕсли МетодЗаполнения = "СтавкаНДС_Номенклатура" Тогда
		
		ЗаполнитьСтавкуНДС(Параметры);
		
	ИначеЕсли МетодЗаполнения = "АкцизВидОперацииРеализации" Тогда
		
		ЗаполнитьВидРеализацииАкциз(Параметры);
		
	ИначеЕсли МетодЗаполнения = "Затраты" ИЛИ МетодЗаполнения = "КомандировочныеРасходы" Тогда
	...
	ИначеЕсли МетодЗаполнения = "СубконтоДоходов" Тогда
		
		ЗаполнитьСубконтоДоходов(Параметры);
	...
	ИначеЕсли МетодЗаполнения = "СубконтоДенежныеСредства" Тогда
		
		ЗаполнитьСубконтоДенежныеСредства(Параметры);
	...
	ИначеЕсли МетодЗаполнения = "ОсновныеСредства" Тогда
		...
		ЗаполнитьОсновныеСредства(Параметры);
	...
	ИначеЕсли МетодЗаполнения = "ОсновныеСредстваАмортизация" Тогда
		...
		Параметры.Значение = ПланыСчетов.Типовой.АмортизацияОсновныхСредств;
	...
	ИначеЕсли МетодЗаполнения = "ЗаймыКредиты" Тогда
		...
		ЗаполнитьЗаймыКредиты(Параметры);
		
	Иначе
		
		Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Типовой.ОсновнойСчет(Параметры.Правило.Роль);
		
		Если СтрЗаканчиваетсяНа(Параметры.Правило.Роль, "НУ")
			И ТипЗнч(Параметры.ЗначениеПоУмолчанию) = Тип("ПланСчетовСсылка.Типовой") Тогда
			Параметры.ЗначениеПоУмолчанию = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПреобразоватьСчетаБУвСчетНУ(
				Новый Структура("СчетБУ", Параметры.ЗначениеПоУмолчанию));
		КонецЕсли;
...
Функция ОшибкаЗаполнения(ПараметрыПроверкиЗаполнения)
	...
	//Если ТипЗнч(Значение) = Тип("ПланСчетовСсылка.Типовой") Тогда
...
Функция ДанныеНастройкиСчетаУчетаНоменклатуры(ПредметыНастройки)
	...
	Пока ВыбратьСледующийКластер(Кластеры) Цикл
		...
		НастройкиПоКлючу = ПроцедурыБухгалтерскогоУчета.ПолучитьСчетаУчетаСпискаНоменклатуры(
...
Функция НастройкиПоИсточникуДанных()
	...
	НастройкиПоИсточникуДанных.Добавить("СчетаУчетаНМА");
	НастройкиПоИсточникуДанных.Добавить("СчетаУчетаОС");
...
#Область СчетаУчетаОС

Функция ДанныеНастройкиСчетаУчетаОС(ПредметыНастройки)
	
	Результат = НовыйДанныеНастройки(ПредметыНастройки);
	
	// Счета можно оптимально получать для списка номенклатуры
	КлючевоеПоле     = "ОсновноеСредство";
	ТипКлючевогоПоля = Тип("СправочникСсылка.ОсновныеСредства");
	Кластеры         = РазбитьНаКластеры(ПредметыНастройки, Новый Структура(КлючевоеПоле));
	...
	Пока ВыбратьСледующийКластер(Кластеры) Цикл
		...
		ОсновноеСредство = ВыгрузитьЗначения(ДанныеКластера, КлючевоеПоле, ТипКлючевогоПоля);
		
		НастройкиПоКлючу = УправлениеВнеоборотнымиАктивамиСервер.ПолучитьСчетаУчетаСпискаОС(
			КлючКластера.Организация,
			ОсновноеСредство,
			КлючКластера.Дата);
...
#Область СчетаУчетаНМА

Функция ДанныеНастройкиСчетаУчетаНМА(ПредметыНастройки)
	
	Результат = НовыйДанныеНастройки(ПредметыНастройки);
	
	// Счета можно оптимально получать для списка номенклатуры
	КлючевоеПоле     = "НематериальныйАктив";
	ТипКлючевогоПоля = Тип("СправочникСсылка.НематериальныеАктивы");
	Кластеры         = РазбитьНаКластеры(ПредметыНастройки, Новый Структура(КлючевоеПоле));
	...
	Пока ВыбратьСледующийКластер(Кластеры) Цикл
		...
		НематериальныйАктив = ВыгрузитьЗначения(ДанныеКластера, КлючевоеПоле, ТипКлючевогоПоля);
		
		НастройкиПоКлючу = УправлениеВнеоборотнымиАктивамиСервер.ПолучитьСчетаУчетаСпискаНМА(
			КлючКластера.Организация,
			НематериальныйАктив,
			КлючКластера.Дата);
...
Функция ДанныеНастройкиСчетаУчетаДенежныхСредств(ПредметыНастройки)
	
	Настройки = Новый Массив;
	
	Для Каждого ПредметНастройки Из ПредметыНастройки Цикл
		
		Если ТипЗнч(ПредметНастройки.Касса) = Тип("СправочникСсылка.Кассы") Тогда
			Настройка = ПроцедурыБухгалтерскогоУчета.ПолучитьСчетУчетаДенежныхСредств(ПредметНастройки.Касса, Истина);
		ИначеЕсли ТипЗнч(ПредметНастройки.СчетОрганизации) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
			Настройка = ПроцедурыБухгалтерскогоУчета.ПолучитьСчетУчетаДенежныхСредств(ПредметНастройки.СчетОрганизации, Истина);
		Иначе
...
Функция РесурсНастройкиЗапасы(Роль)
	...
	Если Роль      = "Запасы"
		...
		Или Роль   = "ЗапасыКромеПринятыхВПереработку" // Все, кроме принятых в переработку. Наличие такой роли обусловлено тем, что алгоритмы проведения не могут использовать отдельные забалансовые счета
	...
	ИначеЕсли Роль = "ЗапасыНУ"
		Или Роль   = "ЗапасыКромеЗабалансовыхНУ"
		Или Роль   = "ЗапасыКромеПринятыхВПереработкуНУ" // Все, кроме принятых на комиссию или в переработку. Наличие такой роли обусловлено тем, что алгоритмы проведения не могут использовать отдельные забалансовые счета
		Или Роль   = "ЗапасыВЦенахПродажиНУ"
		Или Роль   = "ЗапасыЗатратыНУ" Тогда
		
		Возврат "СчетУчетаНУ";
		
	ИначеЕсли Роль = "МатериалыПереданныеВПереработку" Тогда
		
		Возврат "СчетПередачиБУ";
		
	ИначеЕсли Роль = "МатериалыПереданныеВПереработкуНУ" Тогда
		 
		Возврат "СчетПередачиНУ";
	...
	ИначеЕсли Роль = "Акциз" Тогда
		
		Возврат "СчетУчетаАкцизаПоРеализации";
...
Процедура ЗаполнитьЗапасы(Параметры)
	...
	Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Типовой.ОсновнойСчет("Запасы");
	Если СтрЗаканчиваетсяНа(Роль, "НУ") Тогда
		Параметры.ЗначениеПоУмолчанию = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПреобразоватьСчетаБУвСчетНУ(
			Новый Структура("СчетБУ", Параметры.ЗначениеПоУмолчанию));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Параметры.Значение) Тогда
		Если Роль = "ЗапасыОтгруженные" Тогда
			...
			Если ЗначениеЗаполнено(СчетВладелец) И СчетВладелец.Забалансовый Тогда
				...
				КонецЕсли;
			ИначеЕсли ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.СчетВИерархии(Параметры.Значение, ПланыСчетов.Типовой.МатериалыПереданныеВПереработку) Тогда
				Параметры.Значение = Неопределено;
			КонецЕсли;
		ИначеЕсли Роль = "ЗапасыКромеПринятыхВПереработку" 
			И СчетЗапрещенРольюЗапасыКромеПринятыхВПереработку(Параметры.Значение) Тогда
			Параметры.Значение = Неопределено;
		КонецЕсли;
	КонецЕсли;	
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		...
		Если Роль = "ЗапасыОтгруженные" Тогда
			...
			КонецЕсли;
			Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет(Роль); // Заглушка
		ИначеЕсли Роль = "ЗапасыКромеЗабалансовых" Или Роль = "ЗапасыКромеПринятыхВПереработку" Тогда
			Параметры.Значение = Параметры.ЗначениеПоУмолчанию;
		Иначе
			Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет(Роль);
		КонецЕсли;
		
	КонецЕсли;
	
	Если СтрЗаканчиваетсяНа(Роль, "НУ")
		И ТипЗнч(Параметры.Значение) = Тип("ПланСчетовСсылка.Типовой") Тогда
		Параметры.Значение = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ", Параметры.Значение));
	КонецЕсли;
...
Функция ТребованиеБалансовостиСчетаЗапасов(Роль)
	...
	ИначеЕсли Роль = "МатериалыПринятыеВПереработку"
		Или Роль = "МатериалыПринятыеВПереработкуВПроизводстве" Тогда
...
Функция СчетЗапрещенРольюЗапасыКромеПринятыхВПереработку(Счет)
	
	Возврат ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.СчетВИерархии(Счет, ПланыСчетов.Типовой.МатериалыПринятыеВПереработку_);
...
Функция СчетЗапасовИзНастройки(Объект, Роль, ВыполнениеПравила, НастройкиЭлемента)
	...
	Если ТребованиеБалансовостиСчета = Неопределено Тогда
		...
		СчетПодходит = Истина;
	Иначе
		Если ИмяРесурсаНастройки = "СчетУчетаБУ" Тогда
...
Функция ДанныеНастройкиУчетнойПолитики(ИмяНастройки, ПредметыНастройки)
	...
	Для Каждого ПредметНастройки Из ПредметыНастройки Цикл
		...
		Если ИмяНастройки = "ПлательщикНалогаНаПрибыль" Тогда
			ЗначениеНастройки = УчетнаяПолитикаСервер.ПлательщикНалогаНаПрибыль(ПредметНастройки.Организация, ПредметНастройки.Дата);
		ИначеЕсли ИмяНастройки = "ПлательщикНДС" Тогда
			ЗначениеНастройки = ПроцедурыНалоговогоУчета.ПолучитьПризнакПлательщикаНДС(ПредметНастройки.Организация, ПредметНастройки.Дата);
		ИначеЕсли ИмяНастройки = "ОсновнойСчетЗатрат" Тогда
			ЗначениеНастройки = УчетнаяПолитикаСервер.ОсновнойСчетУчетаЗатрат(ПредметНастройки.Организация, ПредметНастройки.Дата);
		ИначеЕсли ИмяНастройки = "МетодОтнесенияНДСВЗачет" Тогда
			ЗначениеНастройки = УчетнаяПолитикаСервер.ПолучитьМетодОтнесенияНДСВЗачет(ПредметНастройки.Организация, ПредметНастройки.Дата);
		КонецЕсли;
...
Функция ЭтоНастройкаУчетнойПолитики(ИмяНастройки)
	...
	Возврат ИмяНастройки = "ПлательщикНалогаНаПрибыль"
		Или ИмяНастройки = "ПлательщикНДС"
		Или ИмяНастройки = "ОсновнойСчетЗатрат"
		Или ИмяНастройки = "МетодОтнесенияНДСВЗачет"
...
Процедура ЗаполнитьПредъявленныйНДС(Параметры)
	...
	Если Параметры.Правило.Роль = "ПредъявленныйНДС_НематериальныеАктивы" Тогда
		Параметры.Значение = Параметры.ЗначениеПоУмолчанию;
		
	ИначеЕсли Не АналитикаЗатратОпределяетсяНоменклатуройУслуг(СвойствоВладельцаПравила(Параметры.Правило, "Роль"))
		...
		Если Настройка <> Неопределено Тогда
			Параметры.Значение = Настройка.СчетУчетаНДСПоПриобретению;
		КонецЕсли;
...
Процедура ЗаполнитьОтложенныйНДС(Параметры)
...
Процедура ЗаполнитьВидПоступленияНДС(Параметры)
	
	Роль = Параметры.Правило.Роль;
	
	Если Роль = "ВидПоступленияНДС_ВозвратТМЗ" Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыПоступления.ВозвратТМЗ;
	ИначеЕсли Роль = "ВидПоступленияНДС_СписаниеТМЗ" Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыПоступления.СписаниеТМЗ;	
	ИначеЕсли Роль = "ВидПоступленияНДС_Импорт" Тогда			
		Параметры.ЗначениеПоУмолчанию = ПользователиБКВызовСервераПовтИсп.ПолучитьЗначениеПоУмолчанию(, "ОсновнойВидПоступленияИмпорт");
		Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеПоУмолчанию) Тогда
			Параметры.ЗначениеПоУмолчанию = Справочники.ВидыПоступления.ИмпортТМЗМетодомЗачета;
		КонецЕсли;
	Иначе
		Параметры.ЗначениеПоУмолчанию = ПользователиБКВызовСервераПовтИсп.ПолучитьЗначениеПоУмолчанию(, "ОсновнойВидПоступления");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеПоУмолчанию) Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыПоступления.ОсновнойВидПоступления(Роль);
	КонецЕсли;
		
КонецПроцедуры

Процедура ЗаполнитьВидРеализацииНДС(Параметры)

	Роль = Параметры.Правило.Роль;
	
	Если Роль = "НДСВидОперацииРеализации_ВозвратТМЗ" Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыРеализации.ВозвратТМЗ;
	ИначеЕсли Роль = "НДСВидОперацииРеализации_ПрочийОблагаемыйИмпорт" Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыРеализации.ПрочийОблагаемыйИмпорт;
	Иначе
		Параметры.ЗначениеПоУмолчанию = ПользователиБКВызовСервераПовтИсп.ПолучитьЗначениеПоУмолчанию(, "ОсновнойВидОперацииРеализации");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеПоУмолчанию) Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыРеализации.ОсновнойВидРеализации(Роль);
	КонецЕсли;
		
КонецПроцедуры

Процедура ЗаполнитьВидРеализацииАкциз(Параметры)

	Параметры.ЗначениеПоУмолчанию = ПользователиБКВызовСервераПовтИсп.ПолучитьЗначениеПоУмолчанию(, "ОсновнойВидОперацииОблагаемойАкцизом");
	
	Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеПоУмолчанию) Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ВидыОперацийОблагаемыхАкцизом.Реализация;
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьВидОборотаПоНДС(Параметры)
	
	Параметры.ЗначениеПоУмолчанию = Перечисления.ВидыОборотовПоРеализации.Общий;
	
	Настройка = ДанныеНастройкиЭлементаКоллекции("МетодОтнесенияНДСВЗачет", Параметры.ПланВыполнения, Параметры.НастройкиЭлемента);
	
	Если Настройка = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Настройка = Перечисления.МетодыОтнесенияНДСВЗачет.Раздельный Тогда
		Параметры.Значение = Перечисления.ВидыОборотовПоРеализации.Облагаемый;
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьВидНДС(Параметры)
	
	Роль = Параметры.Правило.Роль;
	
	Если Роль = "ВидНДС_НДСМетодомЗачета" Тогда
		Параметры.ЗначениеПоУмолчанию = Перечисления.ВидыНДС.НДСМетодомЗачета;
	ИначеЕсли Роль = "ВидНДС_НДСОблагаемыйИмпорт" Тогда
		Параметры.ЗначениеПоУмолчанию = Перечисления.ВидыНДС.НДСОблагаемыйИмпорт;
	ИначеЕсли Роль = "ВидНДС_ИмпортНоменклатура" Тогда
		Номенклатура = ЗначениеРеквизитаЭлементаКоллекции(
			Параметры.Элемент, 
			Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта,
			"Номенклатура");
		Если Номенклатура <> Неопределено Тогда
			ВидНДСПриИмпорте = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "ВидНДСПриИмпорте");
			Если ЗначениеЗаполнено(ВидНДСПриИмпорте) Тогда
				Параметры.ЗначениеПоУмолчанию = ВидНДСПриИмпорте;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеПоУмолчанию) Тогда
		Параметры.ЗначениеПоУмолчанию = Перечисления.ВидыНДС.НДС;
	КонецЕсли;
		
КонецПроцедуры

Процедура ЗаполнитьСтавкуНДС(Параметры)
	
	Роль = Параметры.Правило.Роль;
	
	Если Роль = "СтавкаНДС_Номенклатура" Тогда
		Номенклатура = ЗначениеРеквизитаЭлементаКоллекции(
		Параметры.Элемент, 
		Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта,
		"Номенклатура");
		Если Номенклатура <> Неопределено Тогда
			Параметры.ЗначениеПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "СтавкаНДС");
		КонецЕсли;
	Конецесли;
...
Функция АналитикаЗатратОпределяетсяНоменклатуройУслуг(Роль)
	Возврат РесурсНастройкиЗатраты(Роль) <> Неопределено;
КонецФункции

Процедура ЗаполнитьЗатраты(Параметры)
	
	Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Типовой.ОсновнойСчет("Затраты");
	
	Если ЗаполнитьПоОснованию(Параметры) Тогда
		Возврат;
	КонецЕсли;
	
	СчетЗатратОпределенНоменклатурой = Ложь;
	СчетЗатратОпределенВидомОперацииПередачаСтруктурномуПодразделению = Ложь;
	
	Если РесурсНастройкиЗатраты(Параметры.Правило.Роль) <> Неопределено Тогда
		...
		ИмяРесурсаНастройки = РесурсНастройкиЗатраты(Параметры.Правило.Роль);
		Если Настройка <> Неопределено И ИмяРесурсаНастройки <> Неопределено Тогда
			Параметры.Значение = Настройка[ИмяРесурсаНастройки];
		КонецЕсли;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Параметры.Значение) Тогда
		СчетЗатратОпределенНоменклатурой = Истина;
		Если Параметры.Правило.Роль = "СебестоимостьРеализации_ПередачаСтруктурномуПодразделению"
			ИЛИ Параметры.Правило.Роль = "СебестоимостьРеализации_ПередачаСтруктурномуПодразделениюНУ" Тогда
			СчетЗатратОпределенВидомОперацииПередачаСтруктурномуПодразделению = Истина;
		КонецЕсли;
	ИначеЕсли Параметры.Правило.Роль = "РасходыНаПродажу" Тогда
		Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет("РасходыНаПродажу");
	ИначеЕсли Параметры.Правило.Роль = "РасходыУслугиБанка"
		ИЛИ Параметры.Правило.Роль = "РасходыУслугиБанкаНУ" Тогда
		Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет("РасходыУслугиБанка");
	ИначеЕсли Параметры.Правило.Роль = "РасходыУслугиФакторинга" Тогда
		Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет("ПрочиеРасходы");
	ИначеЕсли Параметры.Правило.Роль = "ПрочиеРасходы"
		ИЛИ Параметры.Правило.Роль = "ПрочиеРасходыНУ" Тогда
		Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет("ПрочиеРасходы");
	ИначеЕсли Параметры.Правило.Роль = "СебестоимостьРеализации"
		ИЛИ Параметры.Правило.Роль = "СебестоимостьРеализацииНУ" Тогда
		Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет("СебестоимостьРеализации");
	Иначе
	...
	Параметры.ИсходящееСообщение.Вставить("СчетЗатратОпределенНоменклатурой",  СчетЗатратОпределенНоменклатурой);
	Параметры.ИсходящееСообщение.Вставить("СчетЗатратОпределенВидомОперацииПередачаСтруктурномуПодразделению",  СчетЗатратОпределенВидомОперацииПередачаСтруктурномуПодразделению);
	
	Если СтрЗаканчиваетсяНа(Параметры.Правило.Роль, "НУ")
		И ТипЗнч(Параметры.Значение) = Тип("ПланСчетовСсылка.Типовой") Тогда
		Параметры.Значение = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПреобразоватьСчетаБУвСчетНУ(
			Новый Структура("СчетБУ", Параметры.Значение));
	КонецЕсли;
...
Функция СтатьяЗатратОпределяетсяНоменклатуройЗапасов(Роль)
	Возврат (Роль = "ИспользованиеМатериалов");
КонецФункции
...
Процедура ЗаполнитьСубконтоЗатрат(Параметры)
	...
	Если Параметры.ВходящееСообщение.Свойство("СчетЗатратОпределенНоменклатурой") 
		И Параметры.ВходящееСообщение.СчетЗатратОпределенНоменклатурой Тогда
		
		Настройка  = ДанныеНастройкиЭлементаКоллекции("СчетаУчетаНоменклатуры", Параметры);
		
		Если Параметры.ВходящееСообщение.Свойство("СчетЗатратОпределенВидомОперацииПередачаСтруктурномуПодразделению") 
			И Параметры.ВходящееСообщение.СчетЗатратОпределенВидомОперацииПередачаСтруктурномуПодразделению Тогда
			Если ТипЗнч(СвойстваСчета.Ссылка) = Тип("ПланСчетовСсылка.Налоговый") Тогда
				ИмяРесурсаНастройки = "СубконтоОтрицательныхОтклоненийНУ";
			Иначе
				ИмяРесурсаНастройки = "СубконтоОтрицательныхОтклоненийБУ";
			КонецЕсли;
		Иначе
			Если ТипЗнч(СвойстваСчета.Ссылка) = Тип("ПланСчетовСсылка.Налоговый") Тогда
				ИмяРесурсаНастройки = "СубконтоСебестоимостиНУ";
			Иначе
				ИмяРесурсаНастройки = "СубконтоСебестоимостиБУ";
			КонецЕсли;
		КонецЕсли;
		
		Если Настройка <> Неопределено Тогда
			Параметры.Значение = ТипЗначения.ПривестиЗначение(Настройка[ИмяРесурсаНастройки + НомерСубконто]);
		КонецЕсли;
		
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.СтатьиЗатрат Тогда
			ЗаполнитьЗначениеСубконтоСтатьяЗатрат(Параметры);
...
Процедура ЗаполнитьЗначениеСубконтоСтатьяЗатрат(Параметры)
	...
	ИначеЕсли РольВладельца = "РасходыУслугиБанка" ИЛИ РольВладельца = "РасходыУслугиБанкаНУ" Тогда
		
		Параметры.Значение = Справочники.СтатьиЗатрат.СтатьяЗатратПоНазначению("КомиссияБанка");
	...
	ЭтоПростаяВерсияКонфигурации = ОбщееЗначение(Параметры.КешОбщихЗначений, "ЭтоПростаяВерсияКонфигурации");
	...
	ИначеЕсли ЭтоПростаяВерсияКонфигурации Тогда
		Параметры.Значение = Справочники.СтатьиЗатрат.ОсновнаяСтатьяЗатрат();
	КонецЕсли;
			
КонецПроцедуры

Процедура ЗаполнитьЗначениеСубконтоДоход(Параметры)
	
	Параметры.ЗначениеПоУмолчанию = Справочники.Доходы.ПустаяСсылка();
	...
	// Проверим, подойдет ли текущее значение
	ТекущееЗначение = Параметры.Элемент[Параметры.Правило.Реквизит];
	Если ЗначениеЗаполнено(ТекущееЗначение) И ТипЗнч(ТекущееЗначение) = Тип("СправочникСсылка.Доходы") Тогда
		Параметры.Значение = ТекущееЗначение;
	ИначеЕсли ЭтоПростаяВерсияКонфигурации Тогда
		Параметры.Значение = Справочники.Доходы.ДоходыОтРеализации;
	КонецЕсли;
...
Процедура ЗаполнитьПодразделение(Параметры)
	...
	Роль = Параметры.Правило.Роль;
	РольСубконтоЗатрат = Роль = "СубконтоЗатрат1" ИЛИ Роль = "СубконтоЗатрат2" ИЛИ Роль = "СубконтоЗатрат3";
	РольСубконтоДоходов = Роль = "СубконтоДоходов1" ИЛИ Роль = "СубконтоДоходов2" ИЛИ Роль = "СубконтоДоходов3";
	
	Параметры.ЗначениеПоУмолчанию = ОбщегоНазначенияБКВызовСервераПовтИсп.ПодразделениеПоУмолчанию(Параметры.ПланВыполнения.ДанныеЗаполнения.Организация);
	...
	ИначеЕсли РольСубконтоЗатрат ИЛИ РольСубконтоДоходов Тогда
...
Функция РесурсНастройкиЗатраты(Роль)
	
	Если Роль = "Затраты"
		Или Роль = "РасходыНаПродажу"
		Или Роль = "ЗапасыЗатраты"
		Или Роль = "ПрочиеРасходы"
		Или Роль = "СебестоимостьРеализации"Тогда
		Возврат "СчетСебестоимостиПриРеализацииБУ";
	ИначеЕсли Роль = "ЗатратыНУ"
		Или Роль = "РасходыНаПродажуНУ"
		Или Роль = "ЗапасыЗатратыНУ"
		Или Роль = "ПрочиеРасходыНУ"
		Или Роль = "СебестоимостьРеализацииНУ"Тогда
		Возврат "СчетСебестоимостиПриРеализацииНУ";
	ИначеЕсли Роль = "СебестоимостьРеализации_ПередачаСтруктурномуПодразделению" Тогда
		Возврат "СчетОтрицательныхОтклоненийБУ";
	ИначеЕсли Роль = "СебестоимостьРеализации_ПередачаСтруктурномуПодразделениюНУ" Тогда
		Возврат "СчетОтрицательныхОтклоненийНУ";
	КонецЕсли;
...
Функция РесурсНастройкиВыручка(Роль)
	...
	Если Роль = "Выручка" Или Роль = "ПрочиеДоходы" Тогда
		...
		Возврат "СчетДоходовОтРеализацииБУ";
		
	ИначеЕсли Роль = "ВыручкаНУ" Или Роль = "ПрочиеДоходыНУ" Тогда
		
		Возврат "СчетДоходовОтРеализацииНУ";
		
	ИначеЕсли Роль = "ПоступлениеНДС" ИЛИ Роль = "ПеречислениеНДС" Тогда
		
		Возврат "СчетУчетаНДСПоПриобретению";
		
	ИначеЕсли Роль = "РеализацияНДС" Тогда
		
		Возврат "СчетУчетаНДСПоРеализации";
		
	ИначеЕсли Роль = "Расходы" Тогда
		
		Возврат "СчетРасходов";
		
	ИначеЕсли Роль = "ВыручкаВозврат" Тогда
		
		Возврат "СчетДоходовПриВозвратеБУ";
		
	ИначеЕсли Роль = "ВыручкаВозвратНУ" Тогда
		
		Возврат "СчетДоходовПриВозвратеНУ";
		
	ИначеЕсли Роль = "Выручка_ПередачаСтруктурномуПодразделению" Тогда
		
		Возврат "СчетПоложительныхОтклоненийБУ";
		
	ИначеЕсли Роль = "Выручка_ПередачаСтруктурномуПодразделениюНУ" Тогда
		
		Возврат "СчетПоложительныхОтклоненийНУ";
...
Процедура ЗаполнитьВыручка(Параметры)
	...
	СчетДоходовОпределенНоменклатурой = Ложь;
	...
	ИначеЕсли Роль = "Выручка" ИЛИ Роль = "ВыручкаВозврат" Тогда
	...
	Если НастройкаСчетов <> Неопределено Тогда
		Настройка = ДанныеНастройкиЭлементаКоллекции("СчетаУчетаНоменклатуры", Параметры);
		ИмяРесурсаНастройки = РесурсНастройкиВыручка(Роль);
		Если ИмяРесурсаНастройки <> Неопределено Тогда
			Параметры.Значение = Настройка[ИмяРесурсаНастройки];
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Параметры.Значение) Тогда
		...
		СчетДоходовОпределенНоменклатурой = Истина;
				
	ИначеЕсли Параметры.Правило.Роль = "ПрочиеДоходы"
		ИЛИ Параметры.Правило.Роль = "ПрочиеДоходыНУ" Тогда
		
		Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет("ПрочиеДоходы");
	...
	Параметры.ИсходящееСообщение.Вставить("СчетДоходовОпределенНоменклатурой", СчетДоходовОпределенНоменклатурой);
...
Процедура ЗаполнитьВидДеятельности(Параметры)
	...
	Если СвойстваСчета.ВидСубконто1 = ПланыВидовХарактеристик.ВидыСубконтоТиповые.НоменклатурныеГруппы Тогда
		
		// Попробуем заполнить единственной номенклатурной группой
		Параметры.Значение = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ОсновнаяНоменклатурнаяГруппа();
		Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
...
Процедура ЗаполнитьСубконтоДоходов(Параметры)
	
	КоличествоСубконто = 3;
	
	// Определим НомерСубконто
	Для НомерСубконто = 1 По КоличествоСубконто + 1 Цикл
		Если Параметры.Правило.Роль = "СубконтоДоходов" + НомерСубконто Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	// Значение из контекста
	Если Параметры.ВходящееСообщение.Свойство("СчетДоходовОпределенНоменклатурой") 
		И Параметры.ВходящееСообщение.СчетДоходовОпределенНоменклатурой Тогда
		
		Настройка  = ДанныеНастройкиЭлементаКоллекции("СчетаУчетаНоменклатуры", Параметры);
		Если РольВладельца = "ВыручкаВозврат" ИЛИ РольВладельца = "ВыручкаВозвратНУ" Тогда
			Если ТипЗнч(СвойстваСчета.Ссылка) = Тип("ПланСчетовСсылка.Налоговый") Тогда
				ИмяРесурсаНастройки = "СубконтоДоходовПриВозвратеНУ";
			Иначе
				ИмяРесурсаНастройки = "СубконтоДоходовПриВозвратеБУ";
			КонецЕсли;
			
		ИначеЕсли  РольВладельца = "Выручка_ПередачаСтруктурномуПодразделению" ИЛИ РольВладельца = "Выручка_ПередачаСтруктурномуПодразделениюНУ" Тогда
			Если ТипЗнч(СвойстваСчета.Ссылка) = Тип("ПланСчетовСсылка.Налоговый") Тогда
				ИмяРесурсаНастройки = "СубконтоПоложительныхОтклоненийНУ";
			Иначе
				ИмяРесурсаНастройки = "СубконтоПоложительныхОтклоненийБУ";
			КонецЕсли;
			
		Иначе
			Если ТипЗнч(СвойстваСчета.Ссылка) = Тип("ПланСчетовСсылка.Налоговый") Тогда
				ИмяРесурсаНастройки = "СубконтоДоходовНУ";
			Иначе
				ИмяРесурсаНастройки = "СубконтоДоходовБУ";
			КонецЕсли;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Доходы Тогда
			ЗаполнитьЗначениеСубконтоДоход(Параметры);
		ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.НоменклатурныеГруппы Тогда
			ЗаполнитьЗначениеСубконтоНоменклатурнаяГруппа(Параметры);
		ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Подразделения Тогда
			ЗаполнитьПодразделение(Параметры);
		ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Номенклатура Тогда
			ТекущееЗначение = ЗначениеРеквизитаЭлементаКоллекции(Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "Номенклатура");
			Если ЗначениеЗаполнено(ТекущееЗначение) И ТипЗнч(ТекущееЗначение) = Тип("СправочникСсылка.Номенклатура") Тогда
				Параметры.Значение = ТекущееЗначение;
			КонецЕсли;
		Иначе
			// Проверим, подойдет ли текущее значение
...
Функция РесурсНастройкиРасчеты(Роль)
	...
	Если Роль = "РасчетыСПоставщиком" 
		Или Роль = "РасчетыСПоставщикомНУ"
		Или Роль = "РасчетыДругие" Тогда
		
		Возврат "СчетРасчетов";
		
	ИначеЕсли Роль = "РасчетыСПоставщикомПоВозвратам" Тогда
		
		Возврат "СчетВозвратов";
		
	ИначеЕсли Роль = "РасчетыСПокупателем"
		Или Роль = "РасчетыСПокупателемНУ" Тогда
	...
	ИначеЕсли Роль = "РасчетыПоПретензиям"
		Или Роль = "РасчетыПоПретензиямНУ" Тогда
			
		Возврат "СчетВозвратовПокупателя";
...
Процедура ЗаполнитьРасчеты(Параметры)
	...
	Если Не ЗначениеЗаполнено(ВидРасчетов) Тогда
		...
		Если УсловияРасчетов <> Неопределено Тогда
			ВидРасчетов = УправлениеВзаиморасчетамиСервер.ВидРасчетовПоУсловиямРасчетов(УсловияРасчетов.ВалютаВзаиморасчетов);
		КонецЕсли;
		// Обработаем ситуацию, когда по условиям расчетов определить вид расчетов не удалось
		Если Не ЗначениеЗаполнено(ВидРасчетов) Тогда
			ВидРасчетов = УправлениеВзаиморасчетамиСервер.ВидРасчетовПоУсловиямРасчетов();
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		АвтозаполнятьСчетаВзаиморасчетов = ДанныеНастройкиЭлементаКоллекции("АвтозаполнятьСчетаВзаиморасчетов", Параметры).АвтозаполнятьСчетаВзаиморасчетов;
		Если АвтозаполнятьСчетаВзаиморасчетов Тогда
			Параметры.Значение = УправлениеВзаиморасчетамиСервер.ОсновнойСчетРасчетов(ВидРасчетов, Роль);
		Иначе
			Параметры.Очистить = Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеНастройкиСчетаРасчетов(ПредметыНастройки)
	...
	Для Каждого ПредметНастройки Из ПредметыНастройки Цикл
		...
		Если ТипЗнч(ПредметНастройки.Организация) <> Тип("СправочникСсылка.Организации")
			...
			Настройка = Неопределено;
		Иначе
			Настройка = УправлениеВзаиморасчетамиСервер.ПолучитьСчетаРасчетовСКонтрагентом(
...
Функция ДанныеНастройкиУсловияРасчетов(ПредметыНастройки)
	...
	Если Не УчетнаяПолитикаСервер.ИспользоватьВалютныйУчет() Тогда
	...
	ИменаПолей = "ВалютаВзаиморасчетов"; // Совпадают с элементами Условия
...
Функция ДанныеНастройкиАвтозаполнятьСчетаВзаиморасчетов(ПредметыНастройки)
	
	Результат = НовыйДанныеНастройки(ПредметыНастройки);
	
	АвтозаполнятьСчетаВзаиморасчетов = Новый Структура;
	АвтозаполнятьСчетаВзаиморасчетов.Вставить("АвтозаполнятьСчетаВзаиморасчетов", Константы.АвтозаполнятьСчетаВзаиморасчетов.Получить());
	
	Для Индекс = 0 По Результат.ВГраница() Цикл
		Результат[Индекс] = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(АвтозаполнятьСчетаВзаиморасчетов);
	КонецЦикла;
...
Процедура ЗаполнитьДеньги(Параметры)
	...
	Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Типовой.ОсновнойСчет(Роль);
	...
	Если Роль = "Подотчет" ИЛИ Роль = "ПодотчетНУ"
			ИЛИ Роль = "Инкассация" ИЛИ Роль = "ИнкассацияНУ" Тогда
		НастройкаСчетов = Неопределено;
	Иначе
		НастройкаСчетов = ДанныеНастройкиЭлементаКоллекции("СчетаУчетаДенежныхСредств", Параметры);
	КонецЕсли;
	
	Если НастройкаСчетов <> Неопределено Тогда
		Если СтрЗаканчиваетсяНа(Роль, "НУ") Тогда
			ИмяРесурсаНастройки = "СчетУчетаНУ";
		Иначе
			ИмяРесурсаНастройки = "СчетУчетаБУ";
		КонецЕсли;
		Параметры.Значение = НастройкаСчетов[ИмяРесурсаНастройки];
	КонецЕсли;
	...
	Если ОбщееЗначение(Параметры.КешОбщихЗначений, "ИспользоватьВалютныйУчет") Тогда
		...
		Если ОплатаВВалюте Тогда
			Параметры.Значение = ПланыСчетов.Типовой.ОсновнойСчет(Роль + "ВВалюте");
		Иначе
			Параметры.Значение = Параметры.ЗначениеПоУмолчанию;
		КонецЕсли;
...
Функция ЭтоСубконтоДенежныеСредства(Роль)
	
	Возврат Роль = "СубконтоДенежныеСредства1" 
		Или Роль = "СубконтоДенежныеСредства2" 
		Или Роль = "СубконтоДенежныеСредства3";
...
Процедура ЗаполнитьСубконтоСтатьяДвиженияДенежныхСредств(Параметры)
	...
	// Значение из основания
	Если ВидСубконто <> ПланыВидовХарактеристик.ВидыСубконтоТиповые.СтатьиДвиженияДенежныхСредств Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьЗначениеСубконтоНоменклатурнаяГруппа(Параметры)
	...
	// определим НоменклатурнуюГруппу из реквизита Номенклатуры
	ТекущаяНоменклатура = ЗначениеРеквизитаЭлементаКоллекции(Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "Номенклатура");
	Если ЗначениеЗаполнено(ТекущаяНоменклатура) И ТипЗнч(ТекущаяНоменклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		Параметры.Значение = ТекущаяНоменклатура.НоменклатурнаяГруппа;
	КонецЕсли;
...
Процедура ЗаполнитьСубконтоДенежныеСредства(Параметры)
	
	КоличествоСубконто = 3;
	
	// Определим НомерСубконто
	Для НомерСубконто = 1 По КоличествоСубконто + 1 Цикл
		Если Параметры.Правило.Роль = "СубконтоДенежныеСредства" + НомерСубконто Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	// Значение из основания
	Если ВидСубконто <> ПланыВидовХарактеристик.ВидыСубконтоТиповые.ДенежныеСредства Тогда
		Возврат;
	КонецЕсли;
	...
	// Проверим, подойдет ли переданное значение. Оно может содержаться как в "общих" данных заполнения, так и в реквизитах элемента коллекции
	ПереданноеЗначение = ЗначениеРеквизитаЭлементаКоллекции(Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "СчетОрганизации");
	Если ПереданноеЗначение = Неопределено Тогда
		ПереданноеЗначение = Параметры.ПланВыполнения.ДанныеЗаполнения.СчетОрганизации;
	КонецЕсли;
	ДенежноеСредство = ТипЗначения.ПривестиЗначение(ПереданноеЗначение);
	Если ДенежноеСредство = ПереданноеЗначение Тогда
		// Подходит
		Параметры.Значение = ДенежноеСредство;
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСубконтоНалогов(Параметры)
	...
	Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.ВидыПлатежейВБюджетИФонды Тогда
		
		Параметры.ЗначениеПоУмолчанию = Перечисления.ВидыПлатежейВБюджетИФонды.Налог;
		Параметры.Очистить = Истина;
		...
		ТекущееЗначение = Перечисления.ВидыПлатежейВБюджетИФонды.Налог; // Заглушка
...
Процедура ЗаполнитьОсновныеСредства(Параметры)
	
	Роль = Параметры.Правило.Роль;
	
	Если СтрЗаканчиваетсяНа(Роль, "НУ") Тогда
		Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Налоговый.ВАВключенныеВСтоимостнойБаланс;
	Иначе
		Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Типовой.ОсновныеСредства;
	КонецЕсли;
	...
	Параметры.Значение = СчетУчетаОСИзНастройки(Параметры.Элемент, Роль, Параметры.ПланВыполнения, Параметры.НастройкиЭлемента);
	
	Если СтрЗаканчиваетсяНа(Роль, "НУ")
		И ТипЗнч(Параметры.Значение) = Тип("ПланСчетовСсылка.Типовой") Тогда
		ПараметрыПреобразования = Новый Структура("СчетБУ", Параметры.Значение);
		Параметры.Значение = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПреобразоватьСчетаБУвСчетНУ(ПараметрыПреобразования);
	КонецЕсли;
	
КонецПроцедуры

Функция СчетУчетаОСИзНастройки(Объект, Роль, ВыполнениеПравила, НастройкиЭлемента)
	
	Настройка = ДанныеНастройкиЭлементаКоллекции("СчетаУчетаОС", ВыполнениеПравила, НастройкиЭлемента);
	...
	// Определим имя ресурса настройки, по версии ПолучитьСчетаУчетаСпискаНоменклатуры()
	ИмяРесурсаНастройки = РесурсНастройкиОсновныеСредства(Роль);
...
Функция РесурсНастройкиОсновныеСредства(Роль)
...
Процедура ЗаполнитьЗаймыКредиты(Параметры)
...
Функция НовыйПараметрыЗаполненияРеквизита()
	...
	Параметры.Вставить("ЗначениеПоУмолчанию", ПланыСчетов.Типовой.ПустаяСсылка()); 
...
Функция ЗначенияДоИзменения(КЗаполнению, ЭлементКоллекции, ИмяКоллекции)
	...
	Для Каждого КлючИЗначение Из КЗаполнению Цикл
		ИмяРеквизитаДетально = ОбщегоНазначенияБККлиентСервер.РазложитьПолноеИмяРеквизита(КлючИЗначение.Ключ);
...
Процедура ЗаполнитьРасчетыПоЛизингу(Параметры)
	...
	Параметры.Значение = УправлениеВзаиморасчетамиСервер.ОсновнойСчетРасчетов(ВидРасчетов, Параметры.Правило.Роль);
...
Функция ОбщееЗначение(КешОбщихЗначений, ИмяЗначения)
	...
	Если ИмяЗначения = "ИспользоватьВалютныйУчет" Тогда
		Значение = УчетнаяПолитикаСервер.ИспользоватьВалютныйУчет();
	ИначеЕсли ИмяЗначения = "ВалютаРегламентированногоУчета" Тогда
		Значение = ОбщегоНазначенияБКВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	ИначеЕсли ИмяЗначения = "ЭтоПростаяВерсияКонфигурации" Тогда
		Значение = ОбщегоНазначенияБК.ЭтоПростаяВерсияКонфигурации();
