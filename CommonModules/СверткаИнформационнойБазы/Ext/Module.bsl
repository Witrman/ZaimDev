...
Процедура ОбновитьДеревоПоМетаданным(Объект) Экспорт
	...
	Если ВосстановитьРекомендуемыеНастройки Тогда
		...
		МакетРекомендуемыеПараметрыСверткиРегистров = Обработки.СверткаИнформационнойБазы.ПолучитьМакет("РекомендуемыеНастройкиРегистровБК30");
		
		Для НомерСтроки = 1 По МакетРекомендуемыеПараметрыСверткиРегистров.ВысотаТаблицы Цикл
			
			УчетПоПодразделениям = МакетРекомендуемыеПараметрыСверткиРегистров.Область(НомерСтроки, 4).Текст;
			
			Если Не ПустаяСтрока(УчетПоПодразделениям) И Не ВестиУчетПоПодразделениям Тогда
				Продолжить;
			КонецЕсли;
