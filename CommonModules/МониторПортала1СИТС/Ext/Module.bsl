...
Функция ДанныеМонитора() Экспорт
	...
	Если Результат.Данные.updateInfo <> Неопределено Тогда
		Результат.Данные.ДоступноОбновление = (Результат.Данные.ДоступноОбновление
			Или Результат.Данные.updateInfo.updateAvailable);
	КонецЕсли;
	Если Результат.Данные.patchesInfo <> Неопределено
		И Не ПолучениеОбновленийПрограммы.АвтоматическаяЗагрузкаИсправленийВключена() Тогда
		Результат.Данные.ДоступноОбновление = (Результат.Данные.ДоступноОбновление
			Или Результат.Данные.patchesInfo.patchesAvailable);
	КонецЕсли;
...
Функция НовыйТелоЗапросаСодержимоеМонитора(
	...
	Если ПараметрыОбновления <> Неопределено Тогда
		...
		Если ПараметрыОбновления.ПолучатьИсправления Тогда
			...
			ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("additionalModulesInfo");
			ЗаписьДанныхСообщения.ЗаписатьНачалоМассива();
			...
			Для Каждого ДополнительныйМодуль Из ДополнительныеПодсистемы Цикл
				...
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ДополнительныйМодуль.ИдентификаторИнтернетПоддержки);
				ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("programVersion");
				ЗаписьДанныхСообщения.ЗаписатьЗначение(ДополнительныйМодуль.Версия);
