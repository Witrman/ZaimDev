	...
	Для Каждого СтрокаДанных Из ТаблицаФормированияПроводок Цикл
		...
		Если СтрокаДанных.Сумма <> 0 Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДанных.Ведомость) Тогда
				Проводка.Содержание  = "Выплата з/п по ведомости №" + СтрокаДанных.Номер + " от " + Формат(СтрокаДанных.Дата, "ДФ='дд.ММ.гг'");				
			Иначе 
				Проводка.Содержание = "Выплата заработной платы";
			КонецЕсли;
		...
		Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
			...
			Если СуммаКПроводке <> 0 Тогда
				...
				// Реквизиты
				Если ЗначениеЗаполнено(СтрокаДанных.Ведомость) Тогда
					ПроводкаНУ.Содержание  = "Выплата з/п по ведомости №" + СтрокаДанных.Номер + " от " + Формат(СтрокаДанных.Дата, "ДФ='дд.ММ.гг'");				
				Иначе 
					ПроводкаНУ.Содержание = "Выплата заработной платы";
				КонецЕсли;
...
Процедура СформироватьПроводкиВозвратОПВ(Реквизиты, ТаблицаВозвратОПВ, Движения, Отказ) 
	...
	СчетКтНУ = ПроцедурыБухгалтерскогоУчета.ЗаполнитьСчетНалоговогоУчетаПоУмолчанию(СчетКт, Реквизиты.Дата, ПланыСчетов.Налоговый.ОбязательстваПоПенсионнымОтчислениям);			
	...
	Для Каждого СтрокаДанных Из ТаблицаФормированияПроводок Цикл
		...
		// Реквизиты
		Проводка.Содержание		= "Возврат " + ТекстПроводки + " за " + Формат(СтрокаДанных.МесяцИсчисления, "ДФ='ММММ гггг'");
		...
		Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
			...
			// Реквизиты
			ПроводкаНУ.Содержание		= "Возврат " + ТекстПроводки + " за " + Формат(СтрокаДанных.МесяцИсчисления, "ДФ='ММММ гггг'");
...
Процедура СформироватьПроводкиПеречислениеСоциальногоСтрахования(Реквизиты, ТаблицаПеречислениеСО, ВидОперации, Движения, Отказ)
	...
	Для Каждого СтрокаДанных Из ТаблицаФормированияПроводок Цикл
		...
		// Реквизиты
		Проводка.Содержание = ВидВзноса + " за " + Формат(СтрокаДанных.МесяцИсчисления, "ДФ='ММММ гггг'") + " г.";		 
		...
		Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
			...
			// Реквизиты
			ПроводкаНУ.Содержание		=  ВидВзноса + " за " + Формат(СтрокаДанных.МесяцИсчисления, "ДФ='ММММ гггг'") + " г.";
...
Процедура СформироватьПроводкиВозвратСоциальногоСтрахования(Реквизиты, ТаблицаВозвратСО, ВидОперации, Движения, Отказ)
	...
	Для Каждого СтрокаДанных Из ТаблицаФормированияПроводок Цикл
		...
		// Реквизиты
		Проводка.Содержание		= "Возврат " + ВидВзноса + " за " + Формат(СтрокаДанных.МесяцИсчисления, "ДФ='ММММ гггг'");
...
Процедура СформироватьПроводкиПеречислениеПоИЛ(Реквизиты, ТаблицаПеречислениеПоИЛ, Движения, Отказ)
	...
	Для Каждого СтрокаДанных ИЗ ТаблицаПеречислениеПоИЛ Цикл
		...
		Если СтрокаДанных.Сумма <> 0 Тогда
			...
			Проводка.Содержание  = "ИЛ " + СокрЛП(СтрокаДанных.НомерИЛ) + " от " + Формат(СтрокаДанных.ДатаИЛ, "ДФ='дд.ММ.гг'") + " за " + Формат(СтрокаДанных.ПериодВзаиморасчетов, "ДФ='МММ гг'") + " от " + СокрЛП(СтрокаДанных.ФИОРаботника);
			...
			Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
				...
				ПроводкаНУ.Содержание  = "ИЛ " + СокрЛП(СтрокаДанных.НомерИЛ) + " от " + Формат(СтрокаДанных.ДатаИЛ, "ДФ='дд.ММ.гг'") + " за " + Формат(СтрокаДанных.ПериодВзаиморасчетов, "ДФ='МММ гг'") + " от " + СокрЛП(СтрокаДанных.ФИОРаботника);
...
Процедура СформиоватьПроводкиДепонированиеЗП(Реквизиты, ТаблицаДепонированиеЗП, Движения, Отказ)
	...
	Для Каждого СтрокаДанных Из ТаблицаФормированияПроводок Цикл
		...
		Проводка.Содержание  = "Депонир. з/п по ведомости №" + СтрокаДанных.Номер + " от " + Формат(СтрокаДанных.Дата, "ДФ='дд.ММ.гг'");
