...
Функция ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, Режим = "ОтправкаПолучение", УзелОбмена = Неопределено) Экспорт
	...
	ОбменДаннымиПереопределяемый.ПолучитьВерсииФорматаОбмена(УзелОбмена, НастройкиПланаОбмена.ВерсииФорматаОбмена);
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Попытка
		Если НомерЭтапа = 1 Тогда
			...
			//Для обмена с УНФ через универсальный обмен.
...
Функция ВерсииФорматаОбмена(Знач УзелИнформационнойБазы)
	...
	//Для обмена данными с УНФ через универсальный формат используется ОМ МенеджерОбменаЧерезУниверсальныйФормат_УНФ 
	ОбменДаннымиПереопределяемый.ПолучитьВерсииФорматаОбмена(УзелИнформационнойБазы, Результат);
