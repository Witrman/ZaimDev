...
Функция ПолучитьНомерНаПечать(Знач НомерОбъекта, Источник = Неопределено, ПредставлениеНомераОбъекта = Неопределено, УдалитьПрефиксИнформационнойБазы = Ложь, УдалитьПользовательскийПрефикс = Истина, СписокПрефиксовУзлов = Неопределено) Экспорт
	...
	ПрефиксацияОбъектовКлиентСерверПереопределяемый.ПриПолученииНомераНаПечать(НомерОбъекта, Источник, СтандартнаяОбработка, ПредставлениеНомераОбъекта, СписокПрефиксовУзлов);
	
	Если СтандартнаяОбработка = Ложь Тогда
		Возврат НомерОбъекта;
	КонецЕсли;
	
	//Вызов БСП удален, не используется нумерация из БСП
	//Если НЕ НомерСодержитСтандартныйПрефикс(НомерОбъекта) Тогда 
	ИсходныйНомер = НомерОбъекта;
		ПрефиксацияОбъектовБКВызовСервера.ПриПолученииНомераНаПечать(НомерОбъекта, Источник, СтандартнаяОбработка, ПредставлениеНомераОбъекта, СписокПрефиксовУзлов);
	//КонецЕсли;
	
	Если СтрНайти(НомерОбъекта, "-") > 0 И СтрНайти(НомерОбъекта, "-") <= 5 Тогда
		НомерОбъекта = ИсходныйНомер;
		СтандартнаяОбработка = Истина;
	КонецЕсли;
	...
	//Удаление префиксов объектов, полученных при обменах и содержащих дефис 
	ПрефиксацияОбъектовБКВызовСервера.ПриПолученииНомераНаПечатьСДефисом(НомерОбъекта, Источник, СтандартнаяОбработка, ПредставлениеНомераОбъекта, СписокПрефиксовУзлов, Истина, Истина, Истина);
...
Функция НомерСодержитСтандартныйПрефикс(Знач НомерОбъекта) Экспорт
...
Функция НомерСодержитПятизначныйПрефикс(Знач НомерОбъекта) Экспорт
