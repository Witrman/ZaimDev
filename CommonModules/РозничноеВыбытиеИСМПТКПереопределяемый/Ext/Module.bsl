...
Функция ПолучитьНоменклатуруПоШтрихкодуВРозничномКонтуре(ШтрихКод, НужноВернутьМассив = Ложь) Экспорт
	...
	//|	КодыТоваровSKU.Номенклатура.ЕдиницаИзмерения
	//|ИЗ
	//|	РегистрСведений.КодыТоваровSKU КАК КодыТоваровSKU
	//|ГДЕ
	//|	КодыТоваровSKU.SKU = &Штрихкод";
	...
	Если Не Результат.Пустой() Тогда 
		...
		Если Выборка.Количество() = 1 Тогда
			...
			УпаковкаНеШтучная = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.УпаковкаНеШтучная(Выборка.Номенклатура, Выборка.Упаковка);
			...
			ДанныеНоменклатуры.Вставить("ИспользуютсяУпаковки",    ?(ЗначениеЗаполнено(Выборка.Упаковка), Истина, Ложь));  
			ДанныеНоменклатуры.Вставить("УпаковкаНеШтучная",	   УпаковкаНеШтучная);
			ДанныеНоменклатуры.Вставить("Упаковка",                Выборка.Упаковка);
			ДанныеНоменклатуры.Вставить("БазоваяЕдиницаИзмерения", Выборка.БазоваяЕдиницаИзмерения);

			Возврат ДанныеНоменклатуры;
		Иначе		
			//Для конфигураций, поддерживающих регистрацию неуникальных штрихкодов номенклатуры.
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииИСМПТК") Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВестиУчетМаркируемогоТабакаИСМПТК") Тогда
		Форма.ВестиУчетМаркируемогоТабакаИСМПТК = ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемогоТабакаИСМПТК");
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВестиУчетМаркируемойОбувиИСМПТК") Тогда
		Форма.ВестиУчетМаркируемойОбувиИСМПТК = ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойОбувиИСМПТК");
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВестиУчетМаркируемыхЛекарствИСМПТК") Тогда
...
Процедура МодификацияФормы(Форма)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииИСМПТК") Тогда
		Возврат;
	КонецЕсли;
...
Процедура МодифицироватьИнициализироватьФормуРМК(Форма) Экспорт
	...
	Если ПроверитьМодификацияФормыВыполнена(Форма) Тогда
		...
		НаФормеЕстьФарма = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВестиУчетМаркируемойОбувиИСМПТК");
		НаФормеЕстьТабак = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВестиУчетМаркируемогоТабакаИСМПТК");
		НаФормеЕстьОбувь = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВестиУчетМаркируемыхЛекарствИСМПТК");
...
#Область Элементы
	...
	//ПЕРЕОПРЕДЕЛЕНИЕ//
	//ЕРП, КА, УТ
	Если Не СтрНайти(Форма.ИмяФормы, "ЧекККМ.") = 0 Тогда 
		ТоварыГруппаНоменклатура = ЭлементыФормы.ТоварыГруппаНоменклатура;
	Иначе
		ТоварыГруппаНоменклатура = ЭлементыФормы.ТоварыГруппаНоменклатуры;
	КонецЕсли;
...
#Область Команды
	...
	//Розница
	//Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокумента" Тогда //Только для формы документа, в РМК не нужно
	///////////////////
		...
		Если Константы.ВестиУчетМаркируемойПродукцииИСМПТК.Получить() Тогда
			...
			//ПЕРЕОПРЕДЕЛЕНИЕ//
			//ЕРП, КА, УТ
			Кнопка = ЭлементыФормы.Добавить(ИмяКомандыПроверкаКМ, Тип("КнопкаФормы"), ЭлементыФормы.ГруппаНижняяКоманднаяПанельКонтекстныеКоманды4);
			Кнопка.ИмяКоманды      = ИмяКомандыПроверкаКМ;
			Кнопка.ТолькоВоВсехДействиях = Ложь;
			Кнопка.ЦветФона  	   = Новый Цвет(255, 255, 255);
			Кнопка.Ширина 		   = 12;
			Кнопка.Высота 		   = 3;
...
#Область Инициализация
	...
	ЭлементыФормы.ТоварыСтатусПроверкиИСМПТК.Видимость = РозничноеВыбытиеИСМПТКВызовСервера.ЕстьПравоЧтенияОбработкаРозничноеВыбытиеИСМПТК()
															И Форма.ИспользоватьАкцизныеМаркиИСМПТК;
	ЭлементыФормы.ПроверитьКодыМаркировки.Видимость = Форма.ИспользоватьАкцизныеМаркиИСМПТК И РозничноеВыбытиеИСМПТКВызовСервера.ЕстьПравоЧтенияОбработкаРозничноеВыбытиеИСМПТК();
	ЭлементыФормы.ГруппаНижняяКоманднаяПанельКонтекстныеКоманды4.Видимость = Форма.ИспользоватьАкцизныеМаркиИСМПТК;
...
Процедура ДополнитьТаблицуТоваров(Форма, ПараметрыИнтеграции, ПутьКРеквизиту, ДобавляемыеРеквизиты) Экспорт
	...
	Колонка = Новый РеквизитФормы("МаркируемаяПродукцияИСМПТК", Новый ОписаниеТипов("Булево"), ПутьКРеквизиту, НСтр("ru = 'Маркируемая продукция'"));
	ДобавляемыеРеквизиты.Добавить(Колонка);
	Колонка = Новый РеквизитФормы("ВидПродукцииИСМПТК", 	    Новый ОписаниеТипов("ПеречислениеСсылка.ВидыПродукцииИСМПТК"), ПутьКРеквизиту, НСтр("ru = 'Вид продукции'"));
	ДобавляемыеРеквизиты.Добавить(Колонка);
	Колонка = Новый РеквизитФормы("СтатусПроверкиИСМПТК", 	    Новый ОписаниеТипов("Число"),  ПутьКРеквизиту, НСтр("ru = 'Статус проверки подбора'"));
	ДобавляемыеРеквизиты.Добавить(Колонка);
	
	Колонка = Новый РеквизитФормы("НеобходимостьВводаКодаМаркировкиИСМПТК", Новый ОписаниеТипов("Булево"),  ПутьКРеквизиту, НСтр("ru = 'Необходимость ввода кода маркировки'"));
	ДобавляемыеРеквизиты.Добавить(Колонка);
...
Процедура ЗаполнитьДанныеМаркировки(Форма) Экспорт 
	...
	Для Каждого СтрокаТаблицы Из ТабличнаяЧастьТовары Цикл 
		...
		Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "МаркируемаяПродукцияИСМПТК") Тогда
			Возврат;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеМаркировкиВСтроке(Форма, ТекущаяСтрока) Экспорт 
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "НеобходимостьВводаКодаМаркировкиИСМПТК") Тогда
...
Функция СерииИспользуются(Использование = Ложь, Номенклатура = Неопределено) Экспорт
	...
	//	"ВЫБРАТЬ
	//	|	ВЫБОР
	//	|		КОГДА Не Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ЛекарственныеПрепараты)
	//	|			ТОГДА ЛОЖЬ
	//	|		КОГДА Не Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция)
	//	|			ТОГДА ЛОЖЬ
	//	|		КОГДА Не Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция)
	//	|			ТОГДА ЛОЖЬ
...
Процедура УстановитьДоступностьЭлементаФормы(ИмяЭлемента, Доступность, Элементы)
	...
	Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ПроверитьНаличиеЭлементаФормы(Элементы, ИмяЭлемента) Тогда
...
Процедура ЗаполнитьПризнакКодаМаркировкиВТоварах(Форма) Экспорт
	...
	СтруктураПоискаМарки  = Новый Структура("Номенклатура, Характеристика, ЕдиницаИзмерения");
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		Если СтрокаТовары.МаркируемаяПродукцияИСМПТК Тогда
			
			ВидПродукции = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(СтрокаТовары.Номенклатура);
			Если ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.ЛекарственныеПрепараты") Тогда 
				СтрокаТовары.СтатусПроверкиИСМПТК = 3; //специфика фармы
				Продолжить;
			КонецЕсли;
			
			ЗаполнитьЗначенияСвойств(СтруктураПоискаТовары, СтрокаТовары);
			ЗаполнитьЗначенияСвойств(СтруктураПоискаМарки,  СтрокаТовары);
			//В ТЧ кодов маркировки реквизит ЕдиницаИзмерения соответствует реквизиту Упаковка в товарах.
			//При этом в товарах он пустой, если упаковка не используется, а в марках заполнен базовой единицей номенклатуры, 
			//поэтому в структуру отбора по маркам добавляем значение с проверкой.
			БазоваяЕИНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТовары.Номенклатура, "ЕдиницаИзмерения");
			СтруктураПоискаМарки.ЕдиницаИзмерения = ?(ЗначениеЗаполнено(СтрокаТовары.Упаковка), СтрокаТовары.Упаковка, БазоваяЕИНоменклатуры);
			...
			Если КоличествоПоТоварам = КоличествоМарокПоТовару Тогда
				Если КоличествоМарокПоТовару = 0 Тогда
					СтрокаТовары.СтатусПроверкиИСМПТК = 3;	
				Иначе
					//Кол-во связанных со строкой марок равно кол-ву товаров в строке, поэтому иконка марки - зеленая.
					СтрокаТовары.СтатусПроверкиИСМПТК = 1; //марка в наличии
				КонецЕсли;
			Иначе
				//Количество не совпадает, марок или больше нужного, или меньше, поэтому иконка красная.
				СтрокаТовары.СтатусПроверкиИСМПТК = 2; //требуется марка
			КонецЕсли;
		Иначе
			//Если товар не маркируемый, иконку марки в строке не отображаем.
...
Функция ПроверитьТипШтрихкодаВФормеВводаКМ(ШтрихкодBase64) Экспорт
	...
	//ПЕРЕОПРЕДЕЛЕНИЕ//
	//ЕРП, КА, УТ
	//Если ТипШтрихкода    = Перечисления.ТипыШтрихкодов.EAN8 
	//	Или ТипШтрихкода = Перечисления.ТипыШтрихкодов.EAN13 Тогда
...
Процедура ПроверитьНеобходимостьВводаМаркируемойПродукцииИСМПТК(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПроверитьНеобходимостьВводаМаркируемойПродукцииИСМПТК", СтруктураПараметровДействия) Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда 
			...
			Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "МаркируемаяПродукцияИСМПТК") Тогда
				Возврат;
			КонецЕсли;
			...
			Иначе 
				...
				ДанныеПродукции = РозничноеВыбытиеИСМПТКПереопределяемый.ДанныеМаркированнойПродукции(ТекущаяСтрока.Номенклатура);
...
Процедура УстановитьУсловноеОформлениеХарактеристикНоменклатурыВРабочемМесте(Форма, ДанныеНоменклатура, ДанныеХарактеристика) Экспорт
	...
	Если Не РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеХарактеристик() Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверитьОсобенностиУчетаНомИУчетТовГруппы(ОсобенностьУчетаНоменклатуры) Экспорт
	
	Возврат (ОсобенностьУчетаНоменклатуры = ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция")
			И Константы.ВестиУчетМаркируемогоТабакаИСМПТК.Получить())
		
			ИЛИ (ОсобенностьУчетаНоменклатуры = ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция")
			И Константы.ВестиУчетМаркируемойОбувиИСМПТК.Получить())
		
			ИЛИ (ОсобенностьУчетаНоменклатуры = ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры.ЛекарственныеПрепараты")
			И Константы.ВестиУчетМаркируемыхЛекарствИСМПТК.Получить());
...
Функция ПолучитьПараметрыУказанияСерийНаСервере(Объект) Экспорт
	...
	Возврат Новый ФиксированнаяСтруктура(ПараметрыУказанияСерий);
...
Функция ПроверитьИспользованиеСерийУНоменклатуры(Номенклатура) Экспорт
	...
	Возврат РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеСерийУНоменклатуры(Номенклатура);
...
Процедура ЗакрытиеФормыПроверкиКМНаСервере(ТаблицаПроверенныхСтрокаАдреса, ЧекИзменен, ДанныеТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаТоварНом Из ТаблицаНоменклатуры Цикл //Начинаем обход по товарам из формы редактирования Кодов маркировки
		...
		//БК
		БазоваяЕИНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТоварНом.Номенклатура, "БазоваяЕдиницаИзмерения");
		///////////////////
		...
		//Аналогичным образом получаем все коды макрировки, относящиеся к этой номенклатуре и уже лежащие в таблице КМ Чека
				...
				Если ОбъединятьПозицииСОдинаковымТоваром Тогда	
					...
					Иначе
						...
						Если ЗначениеЗаполнено(НайденнаяСтрокаТоваров[0].Упаковка) Тогда
							...
							//ПЕРЕОПРЕДЕЛЕНИЕ//
							//ЕРП, КА, УТ
							//КоэффициентПересчета = Справочники.УпаковкиЕдиницыИзмерения.КоэффициентУпаковки(НайденнаяСтрокаТоваров[0].Упаковка, НайденнаяСтрокаТоваров[0].Номенклатура);
							//Розница
...
Функция ПолучитьПереопределяемыеДанныеСпискаЧековККМ(ИсточникВызова = Неопределено) Экспорт
	...
	//ПЕРЕОПРЕДЕЛЕНИЕ//        
	//ЕРП, КА, УТ
	Если СтрНайти(ИсточникВызова, "ЖурналДокументов") = 0 Тогда
		ДанныеСпискаЧековККМ.Вставить("НаименованиеСписка",  Неопределено); //вызов из формы пробитого чека
	Иначе
		ДанныеСпискаЧековККМ.Вставить("НаименованиеСписка",  "ЧекиККМ"); //вызов из формы списка чеков
	КонецЕсли;
	ДанныеСпискаЧековККМ.Вставить("НаименованиеСтатусаЧека", "Статус");
	
	//Розница
	//ДанныеСпискаЧековККМ.Вставить("НаименованиеСписка", 	 "Список");
	//ДанныеСпискаЧековККМ.Вставить("НаименованиеСтатусаЧека", "СтатусЧекаККМ");
...
Функция ПроверитьНевозможностьРедактированияСпискаКМВЧеке(СтатусТекущегоЧека) Экспорт
	...
	//ПЕРЕОПРЕДЕЛЕНИЕ//            
	//ЕРП, КА, УТ
	//Если СтатусТекущегоЧека = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.Пробит") Тогда
	//	Возврат Истина;
	//Иначе
	//	Возврат Ложь;
	//КонецЕсли;
...
Функция ТекстЗапросаЧекККМДанныеТекущегоЧекаИзСписка(СсылкаЧек) Экспорт
	...
	|ИЗ
	|	Документ.%ИмяДокумента%.КодыМаркировкиИСМПТК КАК ЧекККМКодыМаркировкиИСМПТК
	|ГДЕ
	...
	|ИЗ
	|	Документ.%ИмяДокумента%.Товары КАК ЧекККМТовары
	|ГДЕ
	...
	Если ТипЗнч(СсылкаЧек) = Тип("ДокументСсылка.ЧекККМ") Тогда
		Возврат СтрЗаменить(Текст, "%ИмяДокумента%", "ЧекККМ");
	Иначе
		Возврат СтрЗаменить(Текст, "%ИмяДокумента%", "ЧекККМВозврат");
	КонецЕсли;
	#КонецОбласти
...
Функция ОпределениеПризнакаМаркируемаяПродукцияТекстаЗапроса(ПутьКПолюНоменклатура) Экспорт
	...
	#Область ЕРП_КА_УТ
	ОпределениеШаблон = "
	|	%1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция)
	|	ИЛИ %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция)
	|	ИЛИ %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ЛекарственныеПрепараты)";
