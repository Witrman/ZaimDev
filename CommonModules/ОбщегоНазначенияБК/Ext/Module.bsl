...
Процедура ПроверитьСоответствиеОрганизацииДоговоруВзаиморасчетов(Организация, ДоговорКонтрагента, 
	...
	Если ЗначениеЗаполнено(Организация) 
	   ...
	   И Организация <> ДоговорОрганизация Тогда
		...
		ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке("Выбран договор контрагента, не соответстветствующий организации, указанной в документе!", Отказ, Заголовок);
...
Процедура УстановитьПараметрыСеансаИспользоватьОграниченияПравДоступаНаУровнеЗаписей(ИмяПараметра, УстановленныеПараметры) Экспорт
	
	Если ИмяПараметра = "УдалитьИспользоватьОграниченияПравДоступаНаУровнеЗаписей" Тогда
				
		ПараметрыСеанса.УдалитьИспользоватьОграниченияПравДоступаНаУровнеЗаписей = Константы.УдалитьИспользоватьОграниченияПравДоступаНаУровнеЗаписей.Получить();
		
		УстановленныеПараметры.Добавить("УдалитьИспользоватьОграниченияПравДоступаНаУровнеЗаписей");
		
		УстановитьПараметрыМеханизмаОграниченияПравДоступа(, УстановленныеПараметры);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьПараметрыМеханизмаОграниченияПравДоступа(ИмяПараметра, УстановленныеПараметры) Экспорт

	Если Константы.УдалитьИспользоватьОграниченияПравДоступаНаУровнеЗаписей.Получить()
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ИЗ
		               |	Перечисление.УдалитьВидыОбъектовДоступа КАК ВидыОбъектовДоступа
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыПользователей.Состав КАК ПользователиГруппы
		               |		ПО (ПользователиГруппы.Пользователь = &ТекущийПользователь)
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УдалитьНазначениеВидовОбъектовДоступа КАК НазначениеВидовОбъектовДоступа
		               ...
		               |ГДЕ
		               |	НЕ ВидыОбъектовДоступа.Ссылка = ЗНАЧЕНИЕ(Перечисление.УдалитьВидыОбъектовДоступа.УдалитьВнешниеОбработки)
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВидыОбъектовДоступа.Ссылка";
		...
		Выборка = Запрос.Выполнить().Выбрать();
		ВидыОбъектовДоступаЗначения = Метаданные.Перечисления.УдалитьВидыОбъектовДоступа.ЗначенияПеречисления; 
		Пока Выборка.Следующий() Цикл
			ИмяПараметраСеанса = "УдалитьИспользоватьОграничениеПо" + ВидыОбъектовДоступаЗначения.Получить(Перечисления.УдалитьВидыОбъектовДоступа.Индекс(Выборка.Ссылка)).Имя;
			ПараметрыСеанса[ИмяПараметраСеанса] = Выборка.ЕстьОграничение;
	...
	Иначе
		...
		Для Каждого ВидОбъектаДоступа Из Метаданные.Перечисления.УдалитьВидыОбъектовДоступа.ЗначенияПеречисления Цикл
			Если НЕ ВидОбъектаДоступа = Метаданные.Перечисления.УдалитьВидыОбъектовДоступа.ЗначенияПеречисления.УдалитьВнешниеОбработки Тогда
				ИмяПараметраСеанса = "УдалитьИспользоватьОграничениеПо" + ВидОбъектаДоступа.Имя;
	...
	Если ПараметрыСеанса.УдалитьИспользоватьОграничениеПоСтруктурныеПодразделения = Истина И Константы.ПоддержкаРаботыСоСтруктурнымиПодразделениями.Получить() = Ложь Тогда
		ПараметрыСеанса.УдалитьИспользоватьОграничениеПоСтруктурныеПодразделения = Ложь; 
		УстановленныеПараметры.Добавить("УдалитьИспользоватьОграничениеПоСтруктурныеПодразделения");
	КонецЕсли;
...
Функция РолиПользователяНастроеныНекорректно()
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ПредставлениеРолиБазовыеПрава = "Базовые права внешнего пользователя";
	Иначе
		ИмяРолиБазовыеПрава = Метаданные.Роли.БазовыеПраваБСП.Имя;
		ПредставлениеРолиБазовыеПрава = "Базовые права";
	КонецЕсли;
