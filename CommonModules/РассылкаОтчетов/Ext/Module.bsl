...
Процедура ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки) Экспорт
	...
	Если Настройки.Свойство("ВидКИ") И Не Настройки.ВидКИ.ЭтоГруппа Тогда
		...
		СтрокаТаблицы.ГруппаКИ = Настройки.ВидКИ.ВидОбъектаКонтактнойИнформации;
	Иначе
		Если Настройки.Свойство("ВидКИ") Тогда
			СтрокаТаблицы.ГруппаКИ = Настройки.ВидКИ.ВидОбъектаКонтактнойИнформации;
		Иначе
			ИмяГруппыКИ = МетаданныеОсновногоТипа.Имя;
			СтрокаТаблицы.ГруппаКИ = Перечисления.ВидыОбъектовКонтактнойИнформации[ИмяГруппыКИ];
		КонецЕсли;
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 Ссылка ИЗ Справочник.ВидыКонтактнойИнформации ГДЕ ВидОбъектаКонтактнойИнформации = &Родитель И Тип = &Тип";
...
Функция СформироватьСписокПолучателейРассылки(Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	Если ТипПолучателей = Тип("СправочникСсылка.Пользователи") Тогда
		...
		|	ВЫРАЗИТЬ(Контакты.Представление КАК СТРОКА(100)) КАК EMail
		|ИЗ
		|	Получатели КАК Получатели
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК Контакты
		|		ПО Контакты.Объект = Получатели.Получатель
	...
	Иначе
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК Контакты
		|		ПО (Контакты.Объект = Получатели.Ссылка)
		|			И (Контакты.Вид = &ВидПочтовогоАдресаПолучателей)
		|ГДЕ
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	ПараметрыОтчета.Вставить("ЭтоВариант", ТипЗнч(ПараметрыОтчета.Отчет) = Тип("СправочникСсылка.ВариантыОтчетов") И ОтчетыВызовСервераБК.ПолучитьРежимВыполненияОтчета(ПараметрыОтчета.Отчет.Отчет.ПолноеИмя));
