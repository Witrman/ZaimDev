	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		Если ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.ЛинейныйПроцентом Тогда
			Если Не ЗначениеЗаполнено(ВыборкаПоОС.КоэффициентАмортизации) Тогда
				Если ВыдаватьСообщения = Истина Тогда
					ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(ТекстОшибки + НСтр("ru=' не указан процент годовой амотризации'"),
												ВыборкаПоОС.ОсновноеСредство, ,ДокОбъект, ВидРегламентнойОперации);
				КонецЕсли; 
				Продолжить;
			КонецЕсли;	
		...
		Иначе
			...
			Если Реквизиты.МетодПереоценки = Перечисления.МетодыПереоценкиВнеоборотныхАктивов.СписаниеАмортизацииПриПереоценке Тогда
				Если  (СтрокаОС.АмортизацияБУ + СтрокаОС.АмортизацияЗаМесяцБУ) <> 0 Тогда
					...
					Проводка.Содержание   = НСтр("ru='Списана амортизация при переоценке'");
					...
					Если (СуммаПереоценкиСтоимости > 0)  или (СуммаПереоценкиАмортизации > 0) Тогда
						...
						Если (СписокПереоценокОС.СуммаПереоценкиСтоимости < 0) или (СписокПереоценокОС.СуммаПереоценкиАмортизации > 0)  Тогда
							...
							Если СуммаЗакрытияПереоценкиСтоимости <> 0 Тогда
								...
								Проводка.Период       = Реквизиты.Период;
								Проводка.Организация  = Реквизиты.Организация;
								Проводка.Содержание   = НСтр("ru='Реверс убытка от  переоценки стоимости ОС'");
								Проводка.НомерЖурнала = НСтр("ru='ОС'");
							...
							Если СуммаЗакрытияПереоценкиАмортизации <> 0 И СуммаПереоценкиАмортизации <> 0 Тогда
								...
								Проводка.Период       = Реквизиты.Период;
								Проводка.Организация  = Реквизиты.Организация;
								Проводка.Содержание   = НСтр("ru='Реверс дохода от  переоценки амортизации ОС'");
								Проводка.НомерЖурнала = НСтр("ru='ОС'");
							 ...
							 Если СуммаЗакрытияПереоценкиСтоимости <> 0 Или СуммаЗакрытияПереоценкиАмортизации <> 0 Тогда
								...
								Если СуммаЗакрытияПереоценкиСтоимости <> 0 Тогда
									// по бух. учету реверс дохода/убытка
								 	...
								 	Проводка.Период       = Реквизиты.Период;
								 	Проводка.Организация  = Реквизиты.Организация;
								 	Проводка.Содержание   = НСтр("ru='Погашение резерва переоценки стоимости ОС'");
								 	Проводка.НомерЖурнала = НСтр("ru='ОС'");
								 	Проводка.Сумма        = СуммаЗакрытияПереоценкиСтоимости;
			...
			Если (СуммаПереоценкиСтоимости > 0) или (СуммаПереоценкиАмортизации > 0)Тогда
				...
				Если СуммаПереоценкиСтоимости <> 0 Тогда
					...
					Проводка.Период       = Реквизиты.Период;
					Проводка.Организация  = Реквизиты.Организация;
					Проводка.Содержание   = НСтр("ru='Переоценка стоимости ОС'");
					Проводка.НомерЖурнала = НСтр("ru='ОС'");
				...
				Если СуммаПереоценкиАмортизации <> 0 Тогда
					...
					Проводка.Период       = Реквизиты.Период;
					Проводка.Организация  = Реквизиты.Организация;
					Проводка.Содержание   = НСтр("ru='Переоценка амортизации ОС'");
					Проводка.НомерЖурнала = НСтр("ru='ОС'");
	...
	Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
		...
		Для Каждого СтрокаОС из  СписокПереоцененныхОC Цикл
			...
			Если СуммаПереоценкиСтоимости > 0 Тогда
				...
				ПроводкаНУ.Содержание   = НСтр("ru='Реверс убытка от  переоценки стоимости ОС'");
				ПроводкаНУ.НомерЖурнала = НСтр("ru='ОС'");
			...
			Если СуммаПереоценкиСтоимости < 0 Тогда
				...
				ПроводкаНУ.Период       = Реквизиты.Период;
				ПроводкаНУ.Организация  = Реквизиты.Организация;
				ПроводкаНУ.Содержание   = НСтр("ru='Переоценка стоимости ОС'");
				ПроводкаНУ.НомерЖурнала = НСтр("ru='ОС'");
...
Процедура СформироватьДвиженияАмортизацииПоНаправлениямРегл(ТаблицаВА, ТаблицаРеквизиты, Движения, Отказ) Экспорт	
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаВА Цикл
		...
		Проводка.Период					 = Реквизиты.Период;
		Проводка.Содержание				 = ?(НЕ ЗначениеЗаполнено(Реквизиты.Содержание), НСтр("ru = 'Начислена амортизация'"), Реквизиты.Содержание);
		...
		Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
			...
			ПроводкаНУ.Период				   = Реквизиты.Период;
			ПроводкаНУ.Содержание			   = ?(НЕ ЗначениеЗаполнено(Реквизиты.Содержание), НСтр("ru = 'Начислена амортизация по бухг. учету'"), Реквизиты.Содержание);
			...
			Если Реквизиты.ВедениеУчетаВременныхРазницБалансовымМетодом Тогда
				ПроводкаНУ.СчетКт	  = ?(СтрокаТаблицы.ПризнакФиксированногоАктива,ПланыСчетов.Налоговый.ВАВключенныеВСтоимостнойБаланс,ПланыСчетов.Налоговый.ВАНеПодлежащиеАмортизации);
			...
			Если СтрокаТаблицы.ПризнакПроизводственныхЗатрат  И Реквизиты.ВедениеУчетаВременныхРазницБалансовымМетодом Тогда
				...
				ПроводкаНУ.Период				   = Реквизиты.Период;
				ПроводкаНУ.Содержание			   = ?(НЕ ЗначениеЗаполнено(Реквизиты.Содержание), НСтр("ru = 'Амортизация (БУ) включена в производственные затраты'"), Реквизиты.Содержание);
				ПроводкаНУ.НомерЖурнала			   = Реквизиты.НомерЖурнала;
				ПроводкаНУ.ВидРегламентнойОперации = Реквизиты.ВидРегламентнойОперации;
				ПроводкаНУ.Организация			   = Реквизиты.Организация;
				ПроводкаНУ.Сумма				   = СтрокаТаблицы.Сумма;
...
Функция ТекстЗапросаРасчетСуммыСписываемогоРезерваПоПереоценке(СписокОС)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПоложительныеПереоценкиОС.ВнеоборотныйАктив КАК ОсновноеСредство,
		|	СУММА(ПоложительныеПереоценкиОС.СуммаПереоценкиАмортизацииПриход) КАК СуммаПереоценкиАмортизации1,
		|	СУММА(ПоложительныеПереоценкиОС.СуммаПереоценкиСтоимостиОборот) КАК СуммаПереоценкиАмортизации
		|ПОМЕСТИТЬ ВТ_ПоложительныеПереоценкиОС
		|ИЗ
		...
		|	СУММА(ВЫБОР
		|			КОГДА ЕСТЬNULL(АмортизацияОС_БУ.СуммаОборотКт, 0) <> ЕСТЬNULL(АмортизацияОС_БУ.СуммаОборотДт, 0)
		|				ТОГДА ЕСТЬNULL(АмортизацияОС_БУ.СуммаОборотДт, 0)
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК АмортизацияОборотПеремещения
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СчетаУчетаОсновныхСредствСрезПоследних.СчетНачисленияАмортизацииБУ КАК СчетНачисленияАмортизацииБУ,
		|	СчетаУчетаОсновныхСредствСрезПоследних.СчетУчетаБУ КАК СчетУчетаБУ,
		|	НачислениеАмортизацииБухгалтерскийУчетСрезПоследних.ОсновноеСредство КАК ОсновноеСредство,
		|	СУММА(ВЫБОР
		|			КОГДА АмортизацияОС_БУ.СуммаОборотКт <> АмортизацияОС_БУ.СуммаОборотДт
		|				ТОГДА ЕСТЬNULL(АмортизацияОС_БУ.СуммаОборотКт, 0)
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК СуммаАмортизации
		|ПОМЕСТИТЬ ПодготовительнаяТаблицаАмортизации
		|ИЗ
		|	РегистрСведений.НачислениеАмортизацииОСБухгалтерскийУчет.СрезПоследних(
		|			&НачалоПериода,
		|			Организация = &Организация
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПодготовительнаяТаблицаАмортизации.ОсновноеСредство КАК ОсновноеСредство,
		...
		|	ПодготовительнаяТаблицаАмортизации.СуммаАмортизации - ЕСТЬNULL(ВТ_ОборотПеремещения.АмортизацияОборотПеремещения, 0) - -ЕСТЬNULL(ПоложительныеПереоценкиОС.СуммаПереоценкиАмортизации, 0) КАК АмортизацияЗаМесяц
		|ПОМЕСТИТЬ ТаблицаАмортизации
		|ИЗ
		|	ПодготовительнаяТаблицаАмортизации КАК ПодготовительнаяТаблицаАмортизации
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Типовой.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Период, , , &СубконтоОС, Организация = &Организация) КАК СтоимостьОС_БУ
		|		ПО ПодготовительнаяТаблицаАмортизации.ОсновноеСредство = СтоимостьОС_БУ.Субконто1
		|			И ПодготовительнаяТаблицаАмортизации.СчетУчетаБУ = СтоимостьОС_БУ.Счет
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Типовой.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Период, , , &СубконтоОС, Организация = &Организация) КАК АмортизацияОС_БУ
		|		ПО ПодготовительнаяТаблицаАмортизации.ОсновноеСредство = АмортизацияОС_БУ.Субконто1
		|			И ПодготовительнаяТаблицаАмортизации.СчетНачисленияАмортизацииБУ = АмортизацияОС_БУ.Счет
		|		ЛЕВОЕ СОЕДИНЕНИЕ вт_ОборотПеремещения КАК ВТ_ОборотПеремещения
		|		ПО ПодготовительнаяТаблицаАмортизации.ОсновноеСредство = ВТ_ОборотПеремещения.ОсновноеСредство
		|			И ПодготовительнаяТаблицаАмортизации.СчетНачисленияАмортизацииБУ = ВТ_ОборотПеремещения.Счет
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПоложительныеПереоценкиОС КАК ПоложительныеПереоценкиОС
		|		ПО ПодготовительнаяТаблицаАмортизации.ОсновноеСредство = ПоложительныеПереоценкиОС.ОсновноеСредство
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
