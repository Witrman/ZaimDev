...
Процедура ЗаполнитьФормуДоговора(Знач ФормаАВР, Знач ИмяТаблицыКонтрагентов, Знач ИмяРеквизитаКонтрагента, ФормаДоговора, ДополнительныеПараметры = Неопределено)
	...
	Объект.Владелец = ФормаАВР.Объект[ИмяТаблицыКонтрагентов][0][ИмяРеквизитаКонтрагента];	
	...
	Объект.НомерДоговора = ФормаАВР.Объект.НомерДоговора;	
	Объект.ДатаДоговора  = ФормаАВР.Объект.ДатаДоговора;	
...
Функция ЗаполнитьПараметрыВыбораПоставщикаПолучателяДляСП(ВыбранноеЗначение, Элемент) Экспорт
	...
	Если НЕ ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Организации") И НЕ ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Контрагенты")
		И НЕ ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
		
		МассивПараметровВыбора = Новый Массив;
		Если ВыбранноеЗначение = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
			МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.ЯвляетсяСтруктурнымПодразделением", Истина));
		КонецЕсли;
