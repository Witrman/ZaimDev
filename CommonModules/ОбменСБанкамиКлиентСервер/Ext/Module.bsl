...
Функция ЗаполненыРеквизитыНастройкиОбмена(Объект, ЭтоТест = Ложь) Экспорт
	...
	Если Объект.ИспользуетсяКриптография И Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
...
//
// Параметры:
//  НастройкиОбмена - Структура - настройки обмена с банком
...
Функция ЕстьДоступныеСертификаты(ПараметрыОбмена, ВидЭД) Экспорт
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ПараметрыОбмена.ДоступныеСертификаты) ИЛИ НЕ ПараметрыОбмена.СертификатДоступен Тогда
