...
Процедура ПередатьЗначениеВМногострочнуюСтруктуру(Форма, ИмяГруппы, ИмяПоказателя, ЗначениеПоказателя, Расчет = Ложь, ПоказательРасшифровки  = Неопределено) Экспорт
	...
	Если СтрокаДанных = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Группа " + ИмяГруппы + " не найдена в структуре формы'");			
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	...
	Иначе
		...
		Если ТаблицаСтрокГруппы.Колонки.Найти(Колонка) = Неопределено Тогда
			// Графа не найдена
			ТекстСообщения = НСтр("ru = 'Графа " + Колонка + " не найдена в структуре формы'");			
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;	    			
...
Функция ПродолжитьПриНесоответствииВерсийФорм(Знач ВерсияФормы, Знач мВерсияФормы) Экспорт
	...
	Если ДатамВерсияФормы < ДатаВерсияФормы Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сохраненные данные не могут быть открыты текущей версией отчета!%1Версия отчета - от %2Версия данных - от %3'"), Символы.ПС, Формат(ДатамВерсияФормы, "ДЛФ=DD; ДП='Пустая дата'") + Символы.ПС, Формат(ДатаВерсияФормы,  "ДЛФ=DD; ДП='Пустая дата'"));			
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);										
