...
Процедура ДвижениеНЗП(ПараметрыПроведения, Движения, Отказ) Экспорт
	...
	Если Реквизиты.УбранФлажокПереносНЗПБУ  <> Неопределено 
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"),,, Реквизиты.Объект, Перечисления.ВидыРегламентныхОпераций.ПереносНЗПБУ, Ложь);	
...
Процедура ДобавитьЗаписиПоПеремещению(ПараметрыСостоянияИсточника, ПараметрыСостоянияПриемника, ИсходнаяСуммаКорректировки, СтруктураПараметров, Движения, Отказ)
	...
	Если ОбщаяСуммаКорректировки <> 0 Тогда
		...
		Проводка.Содержание    = НСтр("ru = 'Корректировка стоимости перемещения'");
					...
					Если ТипЗнч(ОперацияСписания.КорСубконто2) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
						...
						Иначе
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Корректировка по счету ""%1"" на сумму %2 не будет произведена, т.к. в договоре ""%3"" установлена валюта, отличная от валюты регламентированного учета'"),
...
Процедура ПроверкаПорядкаЗакрытияПодразделений(Реквизиты, ПланСчетов)
	...
	Для Каждого Результат Из РезультатЗапроса Цикл
		Если Результат.КонечныйПередел = 0 И Результат.СуммаОборот <> 0   И Не СтруктураДопПараметров.ПорядокПодразделенийСообщался Тогда
			...
			ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"),,, Реквизиты.Объект, РегОперация);	
...
Функция АнализДанныхПроизводства(Реквизиты)
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Источник = 1 Тогда // прямые затраты			
			...
			ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" + ТекстСообщения + "'"),,, Реквизиты.Объект, РегОперация, Ложь);	
		...
		ИначеЕсли Выборка.Источник = 2 Тогда // общие затраты		
			...
			ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" + ТекстСообщения + "'"),,, Реквизиты.Объект, РегОперация, ЭтоСообщениеОшибка);	
...
Функция ПолучитьПорядокПеределов(Реквизиты)
	...
	Если Не ПорядокПодразделенийСообщался Тогда
		...
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" + ТекстСообщения + "'"),,, Реквизиты.Объект, ВидРегламентнойОперации, Ложь);	
...
Процедура СформироватьЗапросПоКосвеннымЗатратам(Реквизиты, РезультатЗапроса)
	...
	Если СтруктураДопПараметров.Учет = "БУ" Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.Период КАК Период,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.Организация КАК Организация,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.СчетЗатратБУ КАК СчетЗатрат,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.СчетЗатратНУ КАК СчетЗатратНУ,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.СтатьяЗатрат КАК СтатьяЗатрат,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.СтруктурноеПодразделение КАК СтруктурноеПодразделение,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.Подразделение КАК Подразделение,
		               |	МетодыРаспределенияКосвенныхРасходов.БазаРаспределения КАК БазаРаспределения,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СчетЗакрытияБУ КАК СчетЗакрытияБУ,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СубконтоБУ1 КАК СубконтоБУ1,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СубконтоБУ2 КАК СубконтоБУ2,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СубконтоБУ3 КАК СубконтоБУ3,
		               |	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.МетодРаспределенияЗатрат.Ссылка КАК МетодРаспределенияЗатратСсылка,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.ПроцентРаспределения КАК ПроцентРаспределения,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СтруктурноеПодразделение КАК СтруктурноеПодразделениеЗакрытия,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СчетЗакрытияНУ КАК СчетЗакрытияНУ,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СубконтоНУ1 КАК СубконтоНУ1,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СубконтоНУ2 КАК СубконтоНУ2,
		               |	МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения.СубконтоНУ3 КАК СубконтоНУ3
		               |ПОМЕСТИТЬ МетодыРаспределения
		               |ИЗ
		               |	РегистрСведений.МетодыРаспределенияКосвенныхРасходовОрганизаций.СрезПоследних(&КонДата, Организация = &Организация) КАК МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МетодыРаспределенияКосвенныхРасходов.АналитикаРаспределения КАК МетодыРаспределенияКосвенныхРасходовАналитикаРаспределения
...
Функция РаспределитьОПРПоБазе(Реквизиты, Движения, ИмяРеквБазы, 
	...
	Если Обход.Метод = Перечисления.БазыРаспределенияКосвенныхРасходов.Процентом Тогда 
		...
		Если СуммаСписания <> 0 Тогда
			...
			Если НЕ НеРаспределилосьПоБазе Тогда
				Содержание  = "Распределение ОПР, " + Обход.МетодРаспределенияЗатратСсылка;
			Иначе
				Содержание  = "Распределение ОПР по всем подразделениям, " + Обход.МетодРаспределенияЗатратСсылка;
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(Обход.СчетЗакрытия) И НЕ Обход.СчетЗатрат.Забалансовый Тогда
				
				ТекстСообщения = НСтр("ru = '<%1>: не заполнен счет дебета'");
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(Обход.СчетЗатрат) И НЕ Обход.СчетЗакрытия.Забалансовый Тогда
				...
				ТекстСообщения = НСтр("ru = '<%1>: не заполнен счет кредита'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Содержание);			
...
Процедура ЕстьОстаткиНЗПНаНачалоМесяца(Реквизиты, ВидРегламентнойОперации)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока ВыборкаПоПодразделениям.Следующий() Цикл		
			ТекстСообщения = ТекстСообщения + "
			|Подразделение		: " + ВыборкаПоПодразделениям.Субконто1 + ". Сумма " + ВыборкаПоПодразделениям.КонОстаток + " ";
		...
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"),,, Реквизиты.Объект, ВидРегламентнойОперации);	
...
Процедура ДвижениеНЗППоБУиНУ(Реквизиты, Движения, Отказ)
	...
	Для Каждого ТекСтрокаДанныхНЗП Из ДанныеДляНЗП.Строки Цикл	
		...
		Если НайденаяСтрока.Количество() = 0 Тогда
			...
			ТекущийДокументДвижениеНЗП.Комментарий				 = "Отнесение на НЗП " + ПредставлениеПериода(Реквизиты.НачалоПериода, Реквизиты.КонецПериода, "ФП = Истина");
		...
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"),,, Неопределено, ВидРегламентнойОперации, Ложь);	
	...
	Для Каждого ТекущийДокумент Из СписокСформированныхДокументов Цикл
		...
		Иначе						
			...
			ТекущийДокументДвижениеНЗППеренос.Комментарий				= "Перенос с НЗП " + ПредставлениеПериода(НачалоДня(НачалоМесяца(ДобавитьМесяц(Реквизиты.Период, 1))), КонецДня(КонецМесяца(ДобавитьМесяц(Реквизиты.Период, 1))), "ФП = Истина");
	...
	Если НазваниеНачалаЭтапаСообщалось И  (НЕ НазваниеОкончанияЭтапаСообщалось)Тогда
		ТекстСообщения = "---Окончание переноса остатков производственных счетов на счета незавершенного производства----";
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" + ТекстСообщения + "'"),,, Неопределено, ВидРегламентнойОперации, Ложь);	
	КонецЕсли;
