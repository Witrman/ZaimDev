...
Функция ПодготовитьТаблицуРасчетСписанияРБП(ТаблицаРеквизиты, Отказ) Экспорт
	...
	Пока ВыборкаРБП.Следующий() Цикл		
		...
		Если  НЕ ЗначениеЗаполнено(ВыборкаРБП.ДатаНачалаСписания) ИЛИ
			...
			ТекстСообщения = НСтр("ru ='В справочнике ""Расходы будущих периодов"" для статьи """ + ПредставлениеРБП + """ (код "+ ВыборкаРБП.Код +") не заполнен период списания расходов!'");
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаРБП.СпособСписания) Тогда
			ТекстСообщения = НСтр("ru ='В справочнике ""Расходы будущих периодов"" для статьи """ + ПредставлениеРБП + """ (код "+ ВыборкаРБП.Код +") не заполнен способ списания расходов!'");
			СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект,  ВидРО);				
		...
		Если ВыборкаРБП.СпособСписания = Перечисления.СпособыСписанияРБП.Равномерно и НЕ ЗначениеЗаполнено(ВыборкаРБП.СуммаПервоначальногоПризнания) ТОГДА
			ТекстСообщения = НСтр("ru ='В справочнике ""Расходы будущих периодов"" для статьи """ + ПредставлениеРБП + """ (код "+ ВыборкаРБП.Код +") не указана сумма затрат!'");
			СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект,  ВидРО);				
		...
		Если ВыборкаРБП.СпособСписания = Перечисления.СпособыСписанияРБП.Равномерно и ВыборкаРБП.СуммаПервоначальногоПризнания < ВыборкаРБП.СуммаОстатокДтБУ ТОГДА
			...
			ТекстСообщения = НСтр("ru ='Сумма затрат ("+ВыборкаРБП.СуммаОстатокДтБУ+") в бухгалтерском учете "+ТекстСчетУчета+"для статьи """ + ПредставлениеРБП + """ (код "+ ВыборкаРБП.Код +") превышает сумму, указанную в справочнике ""Расходы будущих периодов"" ("+ ВыборкаРБП.СуммаПервоначальногоПризнания +")!'");
			СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект,  ВидРО);				
		...
		Пока ВыборкаПоСчетам.Следующий() Цикл					
			...
			Если ВыборкаПоСчетам.ВидДанных = "БУ" Тогда
				...
				Если НЕ ЗначениеЗаполнено(СчетБУ) Тогда					
					ТекстСообщения = НСтр("ru ='В справочнике ""Расходы будущих периодов"" для статьи """+ ПредставлениеРБП + """ (код "+ ВыборкаПоСчетам.Код +")  не заполнен реквизит ""Счет"" !'");
					СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект,  ВидРО);				
					Продолжить;
				КонецЕсли;
				...
				Если Реквизиты.ПоддержкаУчетаВременныхРазницПоНалогуНаПрибыль  и ИсточникОтнесенияВР = "БУ" Тогда
					...
					Если НЕ ЗначениеЗаполнено(СчетНУСоответствующийСчетуБУ) Тогда
						ТекстСообщения = НСтр("ru ='Для РБП """ + ПредставлениеРБП + """ (код "+ ВыборкаПоСчетам.Код +") не найдено соответствие для счета БУ " +  ВыборкаПоСчетам.СчетУчетаБУ + ", разницы между БУ и НУ не рассчитаны'");
						СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект, ВидРО);				
						Продолжить;
					КонецЕсли;
					
                    Если НЕ ЗначениеЗаполнено(ВыборкаПоСчетам.СчетЗатратНУ) Тогда
						ТекстСообщения = НСтр("ru ='Для РБП """ + ПредставлениеРБП + """ (код "+ ВыборкаПоСчетам.Код +") не указан счет отнесения затрат в НУ. Разницы между БУ и НУ не рассчитаны'");
						СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект, ВидРО);				
						Продолжить;
					КонецЕсли;
			...
			Если ВыборкаПоСчетам.ВидДанных = "НУ" и Реквизиты.УчитыватьКПН Тогда												
				...
				Если НЕ ЗначениеЗаполнено(ВыборкаПоСчетам.СчетЗатратНУ) Тогда
					ТекстСообщения = НСтр("ru ='Для РБП """ + ПредставлениеРБП + """ (код "+ ВыборкаПоСчетам.Код +") не указан счет отнесения затрат в НУ. Отражение в НУ невозможно.'");
					СообщитьОбОшибкеРегОперации(ТекстСообщения,ПредставлениеРБП, , Реквизиты.Объект, ВидРО);				
					Продолжить;
				КонецЕсли;
...
Процедура СформироватьДвиженияРасчетСписанияРБП(РасчетСписанияРБП, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаРасчетРБП Из Параметры.ТаблицаРасчетСписанияРБП Цикл
		...
		Если СтрокаРасчетРБП.ОтражатьВБухУчете Тогда
			...
			СодержаниеПроводки = "РБП за " + ПредставлениеПериода(НачалоМесяца(Реквизиты.Период), КонецМесяца(Реквизиты.Период), "ФП=Истина");
			Проводка.Содержание   = НСтр("ru = '"+СодержаниеПроводки+"'");
		...
		Если СтрокаРасчетРБП.ОтражатьВНалУчете Тогда
			...
			Если СтрокаРасчетРБП.ВидУчета = Справочники.ВидыУчетаНУ.ВР Тогда
				СодержаниеПроводки = "Временная разница по РБП (" + СтрокаРасчетРБП.РБП + ") за "+ ПредставлениеПериода(НачалоМесяца(Реквизиты.Период), КонецМесяца(Реквизиты.Период), "ФП=Истина");
			    Проводка.Содержание   = НСтр("ru = '"+СодержаниеПроводки+"'");
			Иначе
				СодержаниеПроводки = "РБП за " + ПредставлениеПериода(НачалоМесяца(Реквизиты.Период), КонецМесяца(Реквизиты.Период), "ФП=Истина");
			    Проводка.Содержание   = НСтр("ru = '"+СодержаниеПроводки+"'");
...
Процедура СформироватьДвиженияЗакрытияНДСПоСП(ТаблицаЗакрытияНДСПоСП, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаРасчетНДС Из Параметры.ТаблицаЗакрытияНДСПоСП Цикл
		...
		Проводка.Содержание               = НСтр("ru = 'Перенос начисленных сумм НДС на головное предприятие'");				
		...
		Если Реквизиты.УчитыватьКПН И Реквизиты.ПоддержкаУчетаВременныхРазницПоНалогуНаПрибыль Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаРасчетНДС.СчетНУ) Тогда
				ТекстСообщения = "Для счета " + СтрокаРасчетНДС.Счет.Код + " не указан соответствующий ему счет налогового учета.";			
				ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru = '"+ТекстСообщения), , Отказ, Реквизиты.Объект , Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету);						
				Продолжить;
			КонецЕсли;
			...
			Проводка.Содержание               = "Перенос начисленных сумм НДС на головное предприятие";				
			Проводка.ВидРегламентнойОперации  = Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету;	
			// По 1420 проводка в Дт 1420 головного подразделения НП
			Если СтрокаРасчетНДС.ДтПроводка = 1 Тогда
				Проводка.Сумма                    = СтрокаРасчетНДС.Сумма;
...
Функция ПолучитьДолюОблагаемогоОборота(Реквизиты, ДатаНачалаПериода) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" + ТекстКомментарияРасчета +"'"), , , Реквизиты.Объект , Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету, Ложь);	
	КонецЕсли;
...
Процедура СформироватьДвиженияПоНДССИзмененнымСрокомУплаты(ТаблицаЗакрытияНДССИзмененнымСрокомУплаты, ТаблицаРеквизиты, ТаблицаРеквизитыЗачетаНДС, Движения, Отказ) Экспорт
	...
	Если ТаблицаЗакрытияНДССИзмененнымСрокомУплаты.Количество() > 0 Тогда
		ТекстКомментарияРасчета = "Зачет по НДС. Существует непогашенная задолженность по НДС, с измененным сроком уплаты на сумму " + ТаблицаЗакрытияНДССИзмененнымСрокомУплаты.Итог("СуммаНДСОстаток") + "
		|Выполнен взаимозачет с бюджетом по НДС, с измененным сроком уплаты на сумму " + ТаблицаЗакрытияНДССИзмененнымСрокомУплаты.Итог("СуммаНДС");
...
Процедура СформироватьДвиженияПоНДСНеРазрешенномуВЗачет(ТаблицаРаспределенияНДСКВозмещению, ТаблицаРеквизиты, ПараметрыЗачетаНДС, Движения, Отказ)  Экспорт
	...
	Для Каждого СтрокаТаблицыНДСКВозмещению из ТаблицаРаспределенияНДСКВозмещению Цикл 
		...
		Если Не СуммаКСписанию = 0 Тогда
			...
			Проводка.Содержание               = НСтр("ru = 'Отнесение на затраты суммы НДС, не разрешенного к зачету'");
			Проводка.НомерЖурнала             = "НУ";	
			Проводка.ВидРегламентнойОперации  = Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету;
	...
	Для каждого СтрокаТаблицыНДСКВозмещению из ТаблицаРаспределенияНДСКВозмещению Цикл 
		...
		Для каждого СтрокаТаблицыНДС из ТаблицаРаспределенияНДС Цикл 
			...
			Если Не СуммаЗачета = 0 Тогда
				...
				Проводка.Содержание               = НСтр("ru = 'Зачет по НДС'");
				Проводка.НомерЖурнала          	  = "НУ";
				Проводка.ВидРегламентнойОперации  = Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету;
				...
				Если Реквизиты.УчитыватьКПН И Реквизиты.ПоддержкаУчетаВременныхРазницПоНалогуНаПрибыль Тогда
					...
					Если НЕ ЗначениеЗаполнено(СчетНДСНУ) Тогда
						...
						ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru = '"+ТекстСообщения+"'"),Реквизиты.Объект, Отказ, , Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету);
						Продолжить;
					КонецЕсли;
		...
		Если СуммаЗачетаПоСчетуВозмещения <> 0 Тогда
			...
			ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru = '"+ТекстСообщения+"'"),,,Реквизиты.Объект, Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету);
...
Процедура СформироватьДвиженияПоНДСВзаиморасчетыСБюджетом(Реквизиты, ТаблицаНДСВзаиморасчетыСБюджетом, Движения, Отказ) Экспорт
	...
	ТекстСообщения = "-------------------------------------------------------------------------------------
						|Зачет по НДС. Сумма НДС, переносимая в счет предстоящих платежей (переносимый зачет) = " + ТаблицаНДСВзаиморасчетыСБюджетом[0].СуммаНДС;
						
	ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"), , , Реквизиты[0].Объект , Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету, Ложь);	
...
Процедура СформироватьДвиженияПоПереносуОтрицательныхОстатковНДС(ТаблицаПереносаОтрицательныхОстатковНДС, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаЗакрытия ИЗ ТаблицаПереносаОтрицательныхОстатковНДС Цикл
		...
		Проводка.Содержание              = НСтр("ru='Зачет авансов и переплат по НДС'");
	...
	|Перенос авансов и переплат по НДС. Итого сумма : " + ОбщаяСумма;
			
	ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"), , , Реквизиты.Объект , Перечисления.ВидыРегламентныхОпераций.ЗачетАвансовИПереплатПоНДС, Ложь);	
...
Процедура СформироватьДвиженияПоУменьшениюСуммыНДС(ТаблицаПереносаУменьшенияСуммыНДС, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаЗакрытия ИЗ ТаблицаПереносаУменьшенияСуммыНДС Цикл
		...
		Если СтрокаЗакрытия.Сумма  <> 0 Тогда
			...
			Проводка.Сумма                   = СтрокаЗакрытия.Сумма;
			Проводка.Содержание              = НСтр("ru='Уменьшение суммы НДС за счет аванса'");
			Проводка.НомерЖурнала          	 = "БУ";
			Проводка.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету;
			ПроцедурыБухгалтерскогоУчета.УстановитьПодразделенияПроводки(Проводка, Реквизиты.СтруктурноеПодразделение, Реквизиты.СтруктурноеПодразделение);
	...
	ТекстСообщения = "-------------------------------------------------------------------------------------	
	|Зачет авансов и переплат по НДС. 
	|Уменьшение суммы НДС за счет аванса. Итого сумма : " + ОбщаяСумма;
...
Процедура СформироватьДвиженияПоЗакрытиюПодотчетныхСуммБУ(ТаблицаЗакрытияПодотчетныхСумм, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаЗакрытия ИЗ ТаблицаЗакрытияПодотчетныхСумм Цикл
		...
		Проводка.Сумма                   = -СтрокаЗакрытия.Сумма;
		Проводка.Содержание              = НСтр("ru='Закрытие подотчетных сумм'");
		Проводка.НомерЖурнала          	 = "БУ";
	...
	Для Каждого СтрокаНачислений ИЗ ТаблицаНачисленийПоНалогам Цикл
		...
		Для Каждого СтрокаАвансов Из ТаблицаАвансовыхПлатежейПоНалогам Цикл
			...
			Если (СтрокаНачислений.НалогСбор = СтрокаАвансов.НалогСбор) И 
				...
				Проводка.Сумма                   = СуммаЗачета;
				Проводка.Содержание              = НСтр("ru='Зачет авансовых платежей по налогам и сборам'");
	...
	Пока  ВыборкаГрупп.Следующий() Цикл				
		...
		Пока  ВыборкаФА.Следующий() Цикл	
			...
			Пока ЕстьЗаписи и Не ОсталосьСписать = 0 Цикл			
					...
					ПроводкаНУ.Содержание 	= НСтр("ru='Затраты по капитальному ремонту отнесены на увеличение стоимости(НУ)'");									
					...
					ПроводкаНУ.Сумма 		= СуммаВПроводку;
					ПроводкаНУ.Содержание 	= НСтр("ru='ВР по затратам на кап. ремонт, отнесенных на увеличение стоимости ФА(НУ)'");				
					ПроводкаНУ.ВидРегламентнойОперации 	= Перечисления.ВидыРегламентныхОпераций.РасчетВычетовПоРасходамНаРемонт;									
	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Пока ВыборкаФА.Следующий() Цикл
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ПроводкаНУ.Содержание 	= НСтр("ru='Активы, учитываемые отдельно, включены в стоимостный баланс групп (НУ)'");
...
Процедура РасчетВычетовПоПреференциямФАНУ_ВычетДоВводаВэксплуатацию(ТаблицаРеквизиты, Движения, Отказ)	
	...
	Пока ВыборкаФА.Следующий() Цикл				
		...
		ПроводкаНУ.Содержание 	 			= НСтр("ru='Отнесение на вычеты стоимости объекта преференций(НУ) до ввода в эксплуатацию'");	
	...
	Пока ВыборкаФА.Следующий() Цикл		
		...
		Пока ВыборкаСчетУчета.Следующий() Цикл
			...
			Пока ВыборкаДетали.Следующий() Цикл			
				...
				ПроводкаНУ.Содержание 				= НСтр("ru='Отнесение на вычеты стоимости объекта преференций(НУ)'");
				ПроводкаНУ.ВидРегламентнойОперации 	= Перечисления.ВидыРегламентныхОпераций.РасчетВычетовПоПреференциям;
			КонецЦикла;	
		КонецЦикла;	
	КонецЦикла;	
	...
	Пока  ВыборкаГрупп.Следующий() Цикл
		...
		Пока  ВыборкаФА.Следующий() Цикл		
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ПроводкаНУ.Содержание 			   = НСтр("ru='Признан доход от превышения доходов выбывших ФА над стоимостным балансом группы/подгруппы'");
	...
	Пока  ВыборкаГрупп.Следующий() Цикл		
		...
		Пока  ВыборкаФА.Следующий() Цикл		
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ПроводкаНУ.Сумма 				   = ВыборкаДетали.СтоимостьНУ;
				ПроводкаНУ.Содержание 			   = НСтр("ru='Стоимость группы, все активы которой выбыли, отнесена на вычеты'");
		...
		Пока  ВыборкаФА.Следующий() Цикл		
			...
			Пока ВыборкаДетали.Следующий() Цикл
				Если Не ВыборкаДетали.РасчетнаяАмортизацияФА = 0 Тогда				
					...
					ПроводкаНУ.Содержание 	= НСтр("ru='Начислена амортизация (НУ)'");	
	...
	Пока  ВыборкаГрупп.Следующий() Цикл		
		...
		Пока  ВыборкаФА.Следующий() Цикл		
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ПроводкаНУ.Сумма 				   = ВыборкаДетали.СтоимостьНУ;
				ПроводкаНУ.Содержание 			   = НСтр("ru='Стоимость группы менее минимума отнесена на вычеты'");
...
Процедура РасчетВычетаПоРеализованнымТоварам (ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Пока  Выборка.Следующий() Цикл
		...
		Проводка.Содержание   =  НСтр("ru='Признание ПР в отчетном периоде " + ПредставлениеПериода(НачалоГода(Реквизиты.Период),  КонецГода(Реквизиты.Период), "ФП=Истина")+"'");
		...
		Если НЕ Результат.Пустой() Тогда 				
			...
			Если НЕ ВычетВНУ = 0 Тогда
				...
				Для НомерСтроки = 0 по РаспределеннаСуммаПоПозициям.ВГраница() цикл
					...
					Проводка.Содержание   = НСтр("ru='Уменьшение вычета в отчетном периоде по ПР " + ПредставлениеПериода(НачалоГода(Реквизиты.Период),  КонецГода(Реквизиты.Период), "ФП=Истина")+"'");
					Проводка.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.РасчетВычетаПоРеализованнымТоварам;	
...
Процедура ЗакрытиеСчетовВА(ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Пока ВыборкаПоФА.Следующий() Цикл
		...
		Пока ВыборкаДетали.Следующий() Цикл
			...
			Проводка.Содержание = НСтр("Закрытие счетов налогового учета ВА'");
...
Процедура РасчетВычетовПоНалогамВБюджет(ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Не Реквизиты.ВедениеУчетаВременныхРазницБалансовымМетодом Тогда
		...
		ТекстСообщения = "Для предприятия, не ведущего учет временных разниц балансовым методом, расчет вычета по налогам не поддерживается. Операция не выполнена.";
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"),,, Реквизиты.Объект, Перечисления.ВидыРегламентныхОпераций.РасчетВычетовПоНалогам, Ложь);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СуммаНеРазрешеннаяКВычету <> 0 Тогда
			...
			Проводка.ВидУчетаДт 	= Справочники.ВидыУчетаНУ.НУ;
			Проводка.ВидУчетаКт 	= Справочники.ВидыУчетаНУ.НУ;
			Проводка.Содержание 	= НСтр("ru = 'Уменьшение вычета на сумму неоплаченной части налога'");
			...
			Проводка.ВидУчетаДт = Справочники.ВидыУчетаНУ.ВР;
			Проводка.ВидУчетаКт = Справочники.ВидыУчетаНУ.ВР;
			Проводка.Содержание = НСтр("ru = 'ВР на сумму неоплаченной части налога'");
...
Процедура РасчетВременныхРазницБалансовымМетодом(ТаблицаРеквизиты, Движения, СтавкаНалогаНаПрибыль, Отказ) Экспорт
	...
	Если НЕ Результат.Пустой() Тогда		
		...
		ТекстСообщения = "Полный расчет величины ВР по данным налогового баланса:";
		Пока ВыборкаПоСчетам.Следующий() Цикл
			ТекстСообщения = ТекстСообщения + "
			|				Счет " + ВыборкаПоСчетам.СчетПредставление + " = " + СокрЛП(ВыборкаПоСчетам.СуммаОНО);
		КонецЦикла;
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru ='"+ ТекстСообщения+"'"),,, Реквизиты.Объект, Перечисления.ВидыРегламентныхОпераций.РасчетВременныхРазниц, Ложь);	
		
		ТекстСообщения = "Итоговая величина ВР по данным налогового баланса: " + СокрЛП(ВыборкаИтогов.СуммаОНО);	
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru ='"+ ТекстСообщения+"'"),,, Реквизиты.Объект, Перечисления.ВидыРегламентныхОпераций.РасчетВременныхРазниц, Ложь);	
		РасчетнаяСумаОНА_ОНО = Окр(ВыборкаИтогов.СуммаОНО* СтавкаНалогаНаПрибыль/100, 2);
	КонецЕсли;
	
	ПризнакОНО = РасчетнаяСумаОНА_ОНО > 0;	
	ТекстСообщения = "Ставка налога на прибыль: " + СокрЛП(СтавкаНалогаНаПрибыль) + "%";
	...
	Запрос.Текст  = "ВЫБРАТЬ
	   ...
	   Если НЕ Результат.Пустой() Тогда
		   ...
		   Пока Выборка.Следующий() Цикл			
			   ...
			   Если Выборка.СуммаОстаток > 0 Тогда
				   ...
				   Проводка.Содержание   = НСтр("ru='Списан имеющийся отложенный налоговый актив " + ПредставлениеПериода(НачалоГода(Реквизиты.Период),  КонецГода(Реквизиты.Период), "ФП=Истина")+"'");
				   Проводка.НомерЖурнала = "ФР";	
			   ...
			   Иначе	
				   ...
				   Проводка.Содержание   =  НСтр("ru='Списано имеющееся отложенное налоговое обязательство " + ПредставлениеПериода(НачалоГода(Реквизиты.Период),  КонецГода(Реквизиты.Период), "ФП=Истина")+"'");
	   ...
	   Если РасчетнаяСумаОНА_ОНО <> 0 Тогда
		   ...
		   Если ПризнакОНО Тогда
			   ...
			   Проводка.Содержание   = НСтр("ru='Признано отложенное налоговое обязательство за период " + ПредставлениеПериода(НачалоГода(Реквизиты.Период),  КонецГода(Реквизиты.Период), "ФП=Истина")+"'");
		   ...
		   Иначе
			   ...
			   Проводка.Содержание   =  НСтр("ru='Признан отложенный налоговый актив за период " + ПредставлениеПериода(НачалоГода(Реквизиты.Период),  КонецГода(Реквизиты.Период), "ФП=Истина")+"'");
			   Проводка.НомерЖурнала = "ФР";	
			   Проводка.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.РасчетВременныхРазниц;	
		   КонецЕсли;			
	   КонецЕсли;					
...
Процедура РасчетНалогаНаПрибыль(ТаблицаРеквизиты, Движения, СтавкаНалогаНаПрибыль, Отказ) Экспорт
	...
	Если ВыборкаИтогов.ДоходНУ > 0 Тогда
		...
		ПроводкаБУ.Содержание   		   =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Начислен налог на прибыль за период %1'"), 
								ПредставлениеПериода(НачалоГода(Реквизиты.Период), КонецМесяца(Реквизиты.Период), "ФП=Истина"));
 	...
 	// Закрытие 6000 и 7000 счетов в БУ
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.СуммаДт <> 0 Тогда
				...
				Проводка.Содержание                = НСтр("ru = 'Расчет итогового дохода/убытка'");
...
Процедура ПроведениеРеформацииБаланса(ПараметрыПроведения, Движения, Отказ) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		Проводка.Сумма                    = Выборка.СуммаКТ;
		Проводка.Содержание               = НСтр("ru = 'Реформация баланса'");
		Проводка.НомерЖурнала             = "ФР";
...
Процедура СформироватьДвиженияПоСписаниюРезерваПоПереоценкеОС(ТаблицаРезервКСписанию, ТаблицаСписаниеРезерваПоПереоценкеОС, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаРезерва Из Параметры.ТаблицаРезервКСписанию Цикл
			...
			Если СуммаРезерваКСписанию <> 0 Тогда
				...
				Проводка.Содержание   = НСтр("ru = 'Списание резерва переоценки стоимости ОС'");
				Проводка.НомерЖурнала = НСтр("ru = 'ОС'");
