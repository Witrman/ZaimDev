...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода = '20200101', ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации")  Тогда
		...
		ИмяРеквизитаКодОрганаГосударственныхДоходов = "НалоговыйКомитет.КодОрганаГосударственныхДоходов";
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		ИмяРеквизитаКодОрганаГосударственныхДоходов = "КодОрганаГосударственныхДоходов";
	...
	Если НЕ ЗначениеЗаполнено(ТекБанковскийСчет) Тогда
		...
		|	Организации." + ИмяРеквизитаКодОрганаГосударственныхДоходов  + " 					        КАК КодОрганаГосударственныхДоходов,";
	...
	Если Данные <> Неопределено Тогда
		...
		Если РезультатЗапроса.Колонки.Найти("КодОрганаГосударственныхДоходов") <> Неопределено Тогда
			Сведения.Вставить("КодОрганаГосударственныхДоходов", Данные.КодОрганаГосударственныхДоходов);	
		КонецЕсли;
	...
	Для Счетчик = 1 по ЧислоПараметров Цикл
		...
		Если ПозЗапятой > 0  Тогда
			...
			Исключение
				...
				ОбщегоНазначения.СообщитьПользователю(НСтр("Не удалось определить значение параметра организации: " + ИмяПараметра));
...
Функция ОтветственныеЛицаОрганизаций(Организация, ДатаСреза, Исполнитель = Неопределено, Результат = Неопределено, ВыводитьФИОПолностью = Ложь) Экспорт
	...
	Результат.Вставить("РуководительФизЛицо"           , ПредопределенноеЗначение("Справочник.ФизическиеЛица.ПустаяСсылка"));
	Результат.Вставить("РуководительФизЛицоПодпись"    , ПредопределенноеЗначение("Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка"));
	Результат.Вставить("ГлавныйБухгалтерФизЛицо"       , ПредопределенноеЗначение("Справочник.ФизическиеЛица.ПустаяСсылка"));
	Результат.Вставить("ГлавныйБухгалтерФизЛицоПодпись", ПредопределенноеЗначение("Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка"));
	...
	Если Организация <> Неопределено Тогда
		...
		ЗапросПоЛицам.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                      ...
		                      |	ОтветственныеЛицаОрганизацииСрезПоследних.ФизическоеЛицо.ФайлПодписи КАК ФайлПодписи,
		                      |	ВЫБОР
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.Руководитель Тогда
				...
				Результат.РуководительФизЛицоПодпись	= Выборка.ФайлПодписи;
			...
			ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер Тогда
				...
				Результат.ГлавныйБухгалтерФизЛицоПодпись	= Выборка.ФайлПодписи;
...
// Возвращает информацию об ответственном лице структурной единицы, например, МОЛе склада
//
Функция ОтветственноеЛицо(СтруктурнаяЕдиница, ДатаСреза) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ФизическоеЛицо", ПредопределенноеЗначение("Справочник.ФизическиеЛица.ПустаяСсылка"));
	Результат.Вставить("Подпись"       , ПредопределенноеЗначение("Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка"));
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ОтветственныеЛицаСрезПоследних.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ЕСТЬNULL(ФизическиеЛица.ФайлПодписи, ЗНАЧЕНИЕ(Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка)) КАК Подпись
	|ИЗ
	|	РегистрСведений.ОтветственныеЛица.СрезПоследних(&ДатаСреза, СтруктурнаяЕдиница = &СтруктурнаяЕдиница) КАК ОтветственныеЛицаСрезПоследних
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Выборка.Следующий();
		Для Каждого КолонкаРезультата Из РезультатЗапроса.Колонки Цикл
			Результат.Вставить(КолонкаРезультата.Имя, Выборка[КолонкаРезультата.Имя]);
		КонецЦикла;
	КонецЕсли;
...
Функция ПроверитьНеобходимостьЗапускаСтартовогоПомощника() Экспорт 
	...
	Возврат ПолучитьФункциональнуюОпцию("НачалоРаботы");
...
Функция ДанныеФизЛицаТекущегоПользователя(Организация, ДатаСреза = Неопределено, ФИОКратко = Истина) Экспорт
	ФизическоеЛицо      = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Пользователи.ТекущийПользователь(), "ФизЛицо");
	...
	Если ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		...
		ДанныеФизЛица = ПроцедурыУправленияПерсоналомВызовСервера.ДанныеФизЛица(Организация, ФизическоеЛицо, ДатаПолученияДанных);
...
// Функция возвращает Истина, если в конфигурации ведется учет по типам цен номенклатуры.
//
Функция ИспользоватьТипыЦенНоменклатуры() Экспорт

	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьТипыЦенНоменклатуры");

КонецФункции // ИспользоватьТипыЦенНоменклатуры()

Функция СформироватьЗаголовокДокумента(ДокументОснование) Экспорт
	Возврат РаботаСДиалогамиКлиентСервер.СформироватьЗаголовокДокумента(ДокументОснование);
КонецФункции
...
Процедура УстановитьИнтерфейсНачалаРаботы(РежимНачалаРаботы) Экспорт
	...
	Если РежимНачалаРаботы = "БыстрыйСтарт" Тогда
...
Процедура УстановитьСтандартныйИнтерфейс(ИмяПользователя = Неопределено) Экспорт
	...
	Настройки = Новый НастройкиИнтерфейсаКлиентскогоПриложения;
	Настройки.УстановитьСостав(НастройкиСоставаИнтерфейсаКлиентскогоПриложения());
...
Функция НастройкиСоставаИнтерфейсаКлиентскогоПриложения() Экспорт
	...
	Если Ложь Тогда //ЭтоИнтерфейсИнтеграцииСБанком() Тогда
	...
	ИначеЕсли Ложь Тогда //ПолучитьФункциональнуюОпцию("ИнтерфейсТаксиПростой") Тогда
		Верх = Новый ГруппаНастройкиСоставаИнтерфейсаКлиентскогоПриложения;
		Верх.Добавить(Новый ЭлементНастройкиСоставаИнтерфейсаКлиентскогоПриложения("ПанельФункцийТекущегоРаздела"));
		Верх.Добавить(Новый ЭлементНастройкиСоставаИнтерфейсаКлиентскогоПриложения("ПанельИнструментов"));
		НастройкиСостава.Верх.Добавить(Верх);
