...
Процедура ОбновитьЭлементыФормыНастроек(Форма, УзелИерархииЭлементов, ПараметрыОбновления = Неопределено) Экспорт 
	...
	// Адаптация БК 3.0
	Если СтрНайти(Форма.ИмяФормы, "ФормаОтчетаБК") <> 0 Тогда
		РеквизитыФормыБК = Неопределено;
		Если ТипЗнч(ПараметрыОбновления) = Тип("Структура") Тогда 
			ПараметрыОбновления.Свойство("РеквизитыФормыБК", РеквизитыФормыБК);
			Если РеквизитыФормыБК <> Неопределено Тогда
				ИменаРеквизитовПредопределенных = ИменаРеквизитов.Предопределенных;
				ИменаЭлементовКУдалению = Новый Массив;
				Для Каждого СтилизованныеЭлементы Из ИменаРеквизитовПредопределенных Цикл
					ИменаЭлементовКУдалению.Очистить();
					МассивСтилизованныхЭлементов = СтилизованныеЭлементы.Значение;
					Для Каждого ИмяЭлемента Из МассивСтилизованныхЭлементов Цикл
						Если РеквизитыФормыБК.Найти(ИмяЭлемента) <> Неопределено Тогда
							ИменаЭлементовКУдалению.Добавить(ИмяЭлемента);
						КонецЕсли;
					КонецЦикла;
					Для Каждого ИмяКУдалению Из ИменаЭлементовКУдалению Цикл
						ИндексКУдалению = МассивСтилизованныхЭлементов.Найти(ИмяКУдалению);
						Если ИндексКУдалению <> Неопределено Тогда
							МассивСтилизованныхЭлементов.Удалить(ИндексКУдалению);
						КонецЕсли;
					КонецЦикла;
				КонецЦикла;
			КонецЕсли;
