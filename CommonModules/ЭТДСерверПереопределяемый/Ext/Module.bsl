...
Процедура ПриСозданииНаСервереСопоставлениеДолжностейЭТД(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Форма.Запись.Организация = ЭТДСервер.ОрганизацияПоУмолчанию();
...
Функция ПолучитьСотрудникаПоИИН(ИИН) Экспорт
	...
	Сотрудник = ПолучитьСотрудникаПоИИНБК(ИИН);
...
Функция ПолучитьДанныеДляАвтозаполненияЭТД(Организация, Период) Экспорт
	...
	СтруктураДанныхДляАвтозаполнения = Новый Структура("Организация, КодРегиона, МестоРаботы");
...
Процедура СформироватьЗапросОсновныеДоговорыБК(Запрос)
	...
	Запрос.Текст = 
		...
		|			И ВТ_СотрудникиДолжность.ДолжностьПриПриеме = ЭТД.Должность
...
Процедура СформироватьЗапросДопСоглашенияБК(Запрос)
	...
	Запрос.Текст =
		...
		|			И ВТ_ИзмененныеДолжности.Должность = ЭТДДопСоглашение.Должность
...
Функция ПолучитьСотрудникаПоИИНБК(ИИН)
