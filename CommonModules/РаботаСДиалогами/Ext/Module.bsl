...
//
Функция СформироватьЗаголовокДокумента(ДокументОбъект, НазваниеДокумента = "", СписокПрефиксовУзлов = Неопределено, ПереноситьДату = Ложь, ПредставлениеНомераОбъекта = Неопределено) Экспорт
	...
	СтрокаЗаголовка = НазваниеДокумента + " № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ДокументОбъект.Номер, ДокументОбъект, ПредставлениеНомераОбъекта, , , СписокПрефиксовУзлов)
...
Процедура ПриИзмененииЗначенияДатыДокумента(ДанныеОбъекта, ВалютаРегламентированногоУчета, ФлагИзмененияКурса = Неопределено, ФлагИзмененияУчетаНДС = Истина, СтруктураРезультатаВыполнения = Неопределено, ПроверкаПоВалютеДокумента = Ложь) Экспорт
	...
	ТекстВалюты     = ?(ПроверкаПоВалютеВзаиморасчетов, НСтр("ru = 'взаиморасчетов '"), НСтр("ru = 'документа '"));
	...
	ТекстВопроса    = НСтр("ru = 'На дату документа у валюты '") + ТекстВалюты + ТекстСтарыйКурс + НСтр("ru = ' был задан другой курс.'")
					+ Символы.ПС + НСтр("ru = 'Установить курс '") + ТекстВалюты + ТекстНовыйКурс + НСтр("ru = ' в соответствии с курсом валюты?'");
