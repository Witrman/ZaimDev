...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыЧерезWebСервис(Отказ,
	...
	ИначеЕсли СтруктураНастроекОбмена.ПроизводитьВыгрузкуДанных Тогда
		...
		Если ПараметрыОбмена.СообщениеДляСопоставленияДанных Тогда
			...
			//Исключение для обмена через универсальный обмен
			Если СтруктураНастроекОбмена.ИмяПланаОбмена = "СинхронизацияДанныхЧерезУниверсальныйФормат" Тогда
				ПараметрыПрокси.Вставить("МинимальнаяВерсия", "2.1.1.7");
			КонецЕсли;
		КонецЕсли;
