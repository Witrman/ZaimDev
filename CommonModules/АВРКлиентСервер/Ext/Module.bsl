...
Функция ТипРегистрацииНПАВРИСЭСФ(ТипРегистрации) Экспорт
	...
	ИначеЕсли ТипРегистрации = ПредопределенноеЗначение("Перечисление.ТипыРегистрацииНПАВР.УчастникСовместнойДеятельности") Тогда
		ТипРегистрацииНПАВРИСЭСФ = "JOINT_ACTIVITY_PARTICIPANT";
	ИначеЕсли ТипРегистрации = ПредопределенноеЗначение("Перечисление.ТипыРегистрацииНПАВР.УчастникСРП") Тогда
		ТипРегистрацииНПАВРИСЭСФ = "SHARING_AGREEMENT_PARTICIPANT";
	КонецЕсли;
...
Функция ТипРегистрацииНПАВРИБ(Знач ТипРегистрацииНПАВРИСЭСФ) Экспорт
	...
	ИначеЕсли ВРЕГ(ТипРегистрацииНПАВРИСЭСФ) = "JOINT_ACTIVITY_PARTICIPANT" Тогда
		ТипРегистрацииНПАВРИБ = ПредопределенноеЗначение("Перечисление.ТипыРегистрацииНПАВР.УчастникСовместнойДеятельности");
	ИначеЕсли ВРЕГ(ТипРегистрацииНПАВРИСЭСФ) = "SHARING_AGREEMENT_PARTICIPANT" Тогда
		ТипРегистрацииНПАВРИБ = ПредопределенноеЗначение("Перечисление.ТипыРегистрацииНПАВР.УчастникСРП");
	Иначе 
...
Функция ТипАВРИСЭСФ(Знач ТипАВРИБ) Экспорт
	
	Если ТипАВРИБ = ПредопределенноеЗначение("Перечисление.ТипыАВР.Первичный") Тогда
		Тип = "PRIMARY";
		
	ИначеЕсли ТипАВРИБ = ПредопределенноеЗначение("Перечисление.ТипыАВР.Исправленный") Тогда
		Тип = "FIXED";
...
Функция ТипАВРИБ(Знач ТипАВРИСЭСФ) Экспорт
	
	ВРегТипАВРИСЭСФ = ВРег(ТипАВРИСЭСФ);
	
	Если ВРегТипАВРИСЭСФ = "PRIMARY" Тогда
		Тип = ПредопределенноеЗначение("Перечисление.ТипыАВР.Первичный");
		
	ИначеЕсли ВРегТипАВРИСЭСФ = "FIXED" Тогда
		Тип = ПредопределенноеЗначение("Перечисление.ТипыАВР.Исправленный");
		
	Иначе
		Тип = ПредопределенноеЗначение("Перечисление.ТипыАВР.ПустаяСсылка");
...
Функция КонтейнерМетодов() Экспорт
	...
	#Иначе
		...
		АВРКлиентСерверПереопределяемый.КонтейнерМетодов(Контейнер, ИспользоватьВнешнююОбработку);
...
Функция СтатусАВР(Знач СтатусИСЭСФ) Экспорт
	...
	ИначеЕсли ВРегСтатусИСЭСФ = "TERMINATED" Тогда
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыАВР.Расторгнут");
		
	ИначеЕсли ВРегСтатусИСЭСФ = "CANCELED" Тогда
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыАВР.Аннулирован");
...
//Возвращает дату ввода выписки электронного акта выполненных работ версии 2.
//
//Выписка ЭАВР версии два были добавлены новые реквизиты в форму документа, добавлена возможность ввода исправленного ЭАВР, а так же новые условия при отправке/ подтверждении ЭАВР.
Функция ДатаВводаЭАВРВерсия2() Экспорт
	
	Возврат Дата(2023,2,3);
