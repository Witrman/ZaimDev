...
Функция СписокРазделовМонитораРуководителяПоУмолчанию() Экспорт
	...
	Для Каждого Раздел из Перечисления.РазделыМонитораРуководителя Цикл
		...
		Если Лев(ВРег(Представление), 7) = "УДАЛИТЬ"
			Или Представление = "ПоступлениеДенежныхСредств"
			Или Представление = "РасходДенежныхСредств"
			Или Представление = "ПродажиПоНоменклатурнымГруппам" 
			Или Представление = "НеоплаченныеСчетаПокупателям" 
			Или Представление = "НеоплаченныеСчетаПоставщиков" Тогда
			Продолжить;
		КонецЕсли;
...
Функция ШаблонЗапросаПоПросроченнойЗадолженнсти() Экспорт
	...
	Возврат "ВЫБРАТЬ
		...
		|				КОНЕЦ = &[СрокОплаты]) КАК ТиповойОстатки
		|
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	ТиповойОбороты.[СуммаОборот] КАК УменьшениеЗадолженности
		...
		|			КорСчет В
		|				(&СписокСчетов),
		|			&СубконтоКонтрагентДоговор) КАК ТиповойОбороты
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Контрагент,
		|	Договор
...
Функция СчетаРасчетовСКонтрагентами(Тип) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |ИЗ
	               |	ПланСчетов.Типовой.ВидыСубконто КАК ВидыСубконтоКонтрагенты
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.Типовой.ВидыСубконто КАК ВидыСубконтоДоговоры
	               |		ПО ВидыСубконтоКонтрагенты.Ссылка = ВидыСубконтоДоговоры.Ссылка
	               |ГДЕ
	               |	ВидыСубконтоКонтрагенты.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоТиповые.Контрагенты)
	               |	И НЕ ВидыСубконтоКонтрагенты.Ссылка.Забалансовый
	               |	И НЕ ВидыСубконтоКонтрагенты.Ссылка В ИЕРАРХИИ (&ИсключенныеСчета)
	               |	И ВидыСубконтоДоговоры.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоТиповые.Договоры)";
...
//
Функция ПолучитьДанныеРазделовМонитора(Организация, СписокРазделов, ВариантОкругления = 1000, ПолучатьПрошлыйПериод = Ложь) Экспорт
	...
	Запрос.Текст = ЗапросКДаннымРазделаМонитора(ПолучатьПрошлыйПериод, ЗначениеЗаполнено(Организация));
...
Функция ЗапросКДаннымразделаМонитора(ПолучатьПрошлыйПериод, ОтбиратьПоОрганизации)
...
Функция ДанныеОстаткиДенежныхСредств(Параметры) Экспорт
	...
	// Банк
	Отбор.Вставить("Порядок", Отчеты.ОстаткиДенежныхСредств.ПорядокТеньговыхСчетовВМониторе());
	НайденныеСтроки = ДанныеРазделовМонитора.НайтиСтроки(Отбор);
	Если НайденныеСтроки.Количество() > 0 Тогда
