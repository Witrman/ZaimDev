...
Функция ПолучитьКодыМаркировки(ИсходныеДанные) Экспорт
	...
	Если ИсходныеДанные.Свойство("ФорматBase64") 
	...
	Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПовтИсп.ЭтоEAN13(ШтрихкодСтрока)
		Или РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПовтИсп.ЭтоEAN8(ШтрихкодСтрока) Тогда
	...
	Иначе
		...
		Иначе
			...
			ДанныеНоменклатуры = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьНоменклатуруПоШтрихкоду(ДанныеРазбора.СоставКодаМаркировки.EAN);
			...
			Если Не НайденнаяНоменклатура.Пустая() Тогда
				ВидПродукции = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(НайденнаяНоменклатура);
			Иначе
				ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.ПустаяСсылка");
			КонецЕсли;
	...
	КодыМаркировки = Новый Структура("КодМаркировки, КодИдентификации, GTIN, EAN, ВидУпаковки, ВидПродукцииИС, Количество, GTINВерхнегоУровня");
	...
	Если ДанныеРазбора.ВидУпаковки = Перечисления.ВидыУпаковокИСМПТК.Групповая
		...
		КодыМаркировки.ВидПродукцииИС	  = ВидПродукции;
...
Функция ПолучитьДанныеРазбораКМ(ИсходныеДанные) Экспорт
	...
	НастройкиРазбораКодаМаркировки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.НастройкиРазбораКодаМаркировки();  
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Возврат РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ЗначенияРеквизитовОбъекта(Ссылка, Реквизиты, ВыбратьРазрешенные);
	
КонецФункции

Функция ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ВыбратьРазрешенные = Ложь) Экспорт
	
	Возврат РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ВыбратьРазрешенные);
...
Функция ПрочитатьСвойстваКлючейИдентификаторовПрименения() Экспорт
	...
	Макет = Обработки.РозничноеВыбытиеМаркированнойПродукцииИСМПТК.ПолучитьМакет("ИдентификаторыПримененияGS1");
	...
	Для НомерСтроки = 2 По ВысотаТаблицы Цикл
		...
		ПредставлениеИдентификатораСКодомВКонце = ПредставлениеИдентификатора + " " + РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку("(%1)", КлючИдентификатора);
		// Представление идентификатора в списках, упорядочиваемых по представлению
		ПредставлениеИдентификатораДляУпорядочивания = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку("(%1)", КлючИдентификатора) + " " + ПредставлениеИдентификатора;
...
Процедура ЗполнитьДопустимыеВидыПродукции(БазовыеПараметры) Экспорт
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ЗполнитьДопустимыеВидыПродукции(БазовыеПараметры);	
	
КонецПроцедуры

Процедура ПриСозданииНаСервере_НастроитьПодключаемоеОборудование(Форма) Экспорт
	
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПриСозданииНаСервере_НастроитьПодключаемоеОборудование(Форма);
...
Функция ПроверитьЗначениеКонстанты(ИмяКонстанты) Экспорт
	...
	Возврат РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ПроверитьЗначениеКонстанты(ИмяКонстанты);
