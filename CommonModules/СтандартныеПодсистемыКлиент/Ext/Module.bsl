...
Процедура ПоказатьРезультатВыполнения(Форма, Результат, ОбработчикЗавершения = Неопределено) Экспорт
	...
	ОбработчикЗавершенияБудетВыполнен = Ложь;
	...
	Если Результат.Свойство("ВыводПредупреждения") И Результат.ВыводПредупреждения.Использование Тогда
		...
		Если ЗначениеЗаполнено(ВыводПредупреждения.ТекстОшибок) Тогда
			...
			ОбработчикЗавершенияБудетВыполнен = Истина;
		Иначе
			ВернутьРезультатПослеПоказаПредупреждения(ВыводПредупреждения.Текст, ОбработчикЗавершения, Неопределено, ВыводПредупреждения.Заголовок);
			ОбработчикЗавершенияБудетВыполнен = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если Результат.Свойство("РазворачиваемыеУзлы") Тогда
		Для Каждого РазворачиваемыйУзел Из Результат.РазворачиваемыеУзлы Цикл
	...
	Если Не ОбработчикЗавершенияБудетВыполнен И ОбработчикЗавершения <> Неопределено Тогда
...
Процедура ПоказатьРезультатВыполненияЗавершение(Ответ, Результат) Экспорт
	...
	Если ТипЗнч(Ответ) = Тип("Число") Тогда
		Если Ответ = 1 Тогда
			ПолныйТекст = Строка(Результат.ВыводПредупреждения.Текст) + Символы.ПС + Символы.ПС + Результат.ВыводПредупреждения.ТекстОшибок;
			Заголовок = Результат.ВыводПредупреждения.Заголовок;
			Если ПустаяСтрока(Заголовок) Тогда
				Заголовок = НСтр("ru = 'Подробнее'");
			КонецЕсли;
			Обработчик = Новый ОписаниеОповещения("ПоказатьРезультатВыполненияЗавершение", ЭтотОбъект, Результат);
			ПоказатьВводСтроки(Обработчик, ПолныйТекст, Заголовок, , Истина);
			ОбработчикЗавершенияБудетВыполнен = Истина;
	...
	Если Не ОбработчикЗавершенияБудетВыполнен И Результат.ОбработчикЗавершения <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(Результат.ОбработчикЗавершения, Неопределено);
	КонецЕсли;
	
КонецПроцедуры

// Показывает окно предупреждение, а после его закрытия вызывает обработчик с заданным результатом.
Процедура ВернутьРезультатПослеПоказаПредупреждения(ТекстПредупреждения, Обработчик, Результат, Заголовок = Неопределено, Таймаут = 0) Экспорт
