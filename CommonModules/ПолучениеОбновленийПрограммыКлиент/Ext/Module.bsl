...
Процедура ПриНачалеРаботыСистемы() Экспорт
	Перем ПроверкаНаличияОбновления;
	...
	Если ПараметрыИПП.Свойство("ПоказатьОповещениеОВключенииАвтоматическойЗагрузкиИсправлений")
		И ПараметрыИПП.ПоказатьОповещениеОВключенииАвтоматическойЗагрузкиИсправлений Тогда
	...
	Если ПараметрыИПП.Свойство("ПроверкаНаличияОбновления", ПроверкаНаличияОбновления) Тогда
		
		// Сохранение признака запущенной проверки при старте системы
		ИнтернетПоддержкаПользователейКлиент.УстановитьЗначениеПараметраПриложения(
			"ПолучениеОбновленийПрограммы\ПроверкаПриЗапускеВыполнена",
			ПроверкаНаличияОбновления.ПроверкаЗапущена);
		
		Если ПроверкаНаличияОбновления.ОписаниеЗадания <> Неопределено Тогда
			// Задание запущено успешно. Вызвать обработку результата через 10 секунд.
			ИнтернетПоддержкаПользователейКлиент.УстановитьЗначениеПараметраПриложения(
				"ПолучениеОбновленийПрограммы\ОписаниеЗадания", ПроверкаНаличияОбновления.ОписаниеЗадания);
...
Процедура ПроверитьНаличиеОбновлений() Экспорт
	...
	НастройкиОбновления = ГлобальныеНастройкиОбновления();
	ОписаниеЗадания = ИнтернетПоддержкаПользователейКлиент.ЗначениеПараметраПриложения(
		"ПолучениеОбновленийПрограммы\ОписаниеЗадания");
	...
	Если ОписаниеЗадания = Неопределено Тогда
		...
		Если РезультатПроверки.ОписаниеЗадания <> Неопределено Тогда
			Если РезультатПроверки.ОписаниеЗадания.Выполнено Тогда
				ОбработатьЗавершениеПроверкиОбновления(
					НастройкиОбновления,
					РезультатПроверки.ПараметрыОповещения);
			Иначе
				ИнтернетПоддержкаПользователейКлиент.УстановитьЗначениеПараметраПриложения(
					"ПолучениеОбновленийПрограммы\ОписаниеЗадания", РезультатПроверки.ОписаниеЗадания);
				ПодключитьОбработкуРезультатаПроверки();
			КонецЕсли;
		ИначеЕсли НастройкиОбновления.РежимАвтоматическойПроверкиНаличияОбновленийПрограммы = 2 Тогда
			ПодключитьПроверкуПоРасписанию();
	...
	Иначе
		...
		ПараметрыОповещения = ПолучениеОбновленийПрограммыВызовСервера.РезультатЗаданияПроверкиНаличияОбновлений(
			ОписаниеЗадания);
