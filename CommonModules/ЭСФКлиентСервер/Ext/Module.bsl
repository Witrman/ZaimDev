...
Функция СвойстваСертификатаИзJSON_Kalkan(Знач СвойстваСертификатаБезОбработки) Экспорт 
	...
	#Если ТонкийКлиент ИЛИ ВебКлиент ИЛИ ТолстыйКлиент Тогда
...
Функция РазобратьСтрокуСертификата(СоставнаяСтрокаДанных) Экспорт
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ТекстСообщения = нСтр("ru='Не удалось разобрать некоторые данные сертификата. Подробности смотрите в журнале регистрации.'", "ru");
...
Функция ПолучитьДанныеСертификатаЧерезНЦА(БИН, Сервис, ДополнительныеПараметры = "") Экспорт
	...
	Попытка
		...
		Если ПустаяСтрока(ОтветСокета) Тогда
			ВызватьИсключение НСтр("ru = 'При получении данных о сертификате возникла ошибка: '") + Криптопровайдер.ТекстОшибки;
		КонецЕсли;
