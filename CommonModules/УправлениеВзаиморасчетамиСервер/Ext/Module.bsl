...
Процедура ПриИзмененииЗначенияДоговора(ДанныеОбъекта, СпособЗаполненияЦен, СтарыйДоговорКонтрагента = Неопределено, ИмяТабличнойЧасти = "Товары", СтруктураРезультатаВыполнения) Экспорт
	...
	Иначе
		...
		Если ((СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатурыКонтрагентов
			...
			СтрокаВозможныхВыборов = ?(СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры,
									 "покупателями или прочее",
									 "поставщиками или прочее");
		...
		Если НЕ МожноПродолжать Тогда
			...
			Если ЗначениеЗаполнено(СтарыйДоговорКонтрагента) 
			   ...
			   И (ЕстьОрганизация И РеквизитыСтарогоДоговора.Организация = ДанныеОбъекта.Организация ИЛИ НЕ ЕстьОрганизация) Тогда
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В документе можно выбирать только договора с %1. Был восстановлен старый договор.'"), СтрокаВозможныхВыборов);
				ДанныеОбъекта.ДоговорКонтрагента = СтарыйДоговорКонтрагента;
			Иначе
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В документе можно выбирать только договора с %1. Договор был очищен.'"), СтрокаВозможныхВыборов);
				ДанныеОбъекта.ДоговорКонтрагента = Неопределено;
			КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Договор)
		...
		Если ЗначениеЗаполнено(СтарыйДоговорКонтрагента) 
		   И РеквизитыСтарогоДоговора.Владелец = ДанныеОбъекта.Контрагент
		   И (ЕстьОрганизация И РеквизитыСтарогоДоговора.Организация = ДанныеОбъекта.Организация ИЛИ НЕ ЕстьОрганизация) Тогда
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выбранный договор принадлежит другому контрагенту. Был восстановлен старый договор.'"), СтрокаВозможныхВыборов);
			ДанныеОбъекта.ДоговорКонтрагента = СтарыйДоговорКонтрагента;
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выбранный договор принадлежит другому контрагенту. Договор был очищен.'"), СтрокаВозможныхВыборов);
			ДанныеОбъекта.ДоговорКонтрагента = Неопределено;
		КонецЕсли;
	...
	Иначе 
		...
		Если ЕстьВалютаДокумента И (ЗначениеЗаполнено(РеквизитыДоговора.ВалютаВзаиморасчетов))Тогда
			...
			Если ДанныеОбъекта.ВалютаДокумента <> НоваяВалютаДокумента Тогда
				...
				ТекстВопроса = ТекстВопроса + Символы.ПС + 
				                ...
				                ?(НЕ ЗначениеЗаполнено(ДанныеОбъекта.ВалютаДокумента), НСтр("ru = 'Не задана'"), СокрЛП(ДанныеОбъекта.ВалютаДокумента)),
		...
		Если ЕстьТипЦен Тогда
			...
			Если ДанныеОбъекта.ТипЦен <> НовыйТипЦен Тогда
				...
				ТекстВопроса 				= ТекстВопроса + Символы.ПС +
				                 				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Тип цен документа: ""%1"", договора: ""%2"".'"),
				                 				?(НЕ ЗначениеЗаполнено(ДанныеОбъекта.ТипЦен), НСтр("ru = 'Не задана'"), СокрЛП(ДанныеОбъекта.ТипЦен)),
				                   				СокрЛП(НовыйТипЦен));
			КонецЕсли;
...
Процедура СформироватьДвиженияПогашениеЗадолженности(ТаблицаВзаиморасчетов, ТаблицаРеквизиты, Движения, Отказ, СворачиватьПроводки = Истина) Экспорт
	...
	Для каждого СтрокаВзаиморасчетов Из Параметры.Взаиморасчеты Цикл
		...
		Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
			...
			ПроводкаНУ.Содержание = ?(Найти(СтрокаВзаиморасчетов.Содержание, "Оплата"), "Оплата", СтрокаВзаиморасчетов.Содержание);
...
Функция ТекстВопросаПересчитатьДокументПоДоговору(Знач ФормаДокумента, Знач РеквизитыДоговора)
	...
	Если ОтличаетсяТипЦен ИЛИ ОтличаетсяВалюта Тогда
		...
		Если ОтличаетсяТипЦен Тогда
			ТипЦенДокументаСтрока = ?(ЗначениеЗаполнено(Объект.ТипЦен), Объект.ТипЦен, НСтр("ru = 'Не задан'"));
			ТипЦенДоговораСтрока = ?(ЗначениеЗаполнено(РеквизитыДоговора.ТипЦен), РеквизитыДоговора.ТипЦен, НСтр("ru = 'Не задан'"));
			ОтличияТипЦен = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Тип цен документа: ""%1"", договора: ""%2"".'"),
				ТипЦенДокументаСтрока, ТипЦенДоговораСтрока);
		Иначе
			ОтличияТипЦен = "";	
		КонецЕсли;
		
		Если ОтличаетсяВалюта Тогда
			ВалютаДокументаСтрока = ?(ЗначениеЗаполнено(Объект.ВалютаДокумента), Объект.ВалютаДокумента, НСтр("ru = 'Не задана'"));
			ВалютаДоговораСтрока = ?(ЗначениеЗаполнено(РеквизитыДоговора.ВалютаВзаиморасчетов), РеквизитыДоговора.ВалютаВзаиморасчетов, НСтр("ru = 'Не задана'"));
			ОтличияВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Валюта документа: ""%1"", договора: ""%2"".'"),
				ВалютаДокументаСтрока, ВалютаДоговораСтрока);					
		Иначе	
			ОтличияВалюта = "";
		КонецЕсли;
		
		Отличия = ?(ОтличаетсяТипЦен И ОтличаетсяВалюта, ОтличияТипЦен + Символы.ПС + ОтличияВалюта, ОтличияТипЦен + ОтличияВалюта); 
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Отличия);
...
Функция СодержаниеПроводокПоЗачетуАвансов(НаправлениеДвижения, ЭтоВозврат, ВидДоговора)
	...
	Если НаправлениеДвижения = "Поступление" Тогда
		...
		Если ЭтоВозврат Тогда
			Содержание = НСтр("ru='Выделение аванса (оплаченная часть при возвратах)'");
		ИначеЕсли ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком Тогда
			Содержание = НСтр("ru='Зачет аванса поставщику'");
		Иначе
			Содержание = НСтр("ru='Зачет аванса'");
		КонецЕсли;
	...
	Иначе //"Выбытие"
		...
		Если ЭтоВозврат Тогда
			Если ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком Тогда
				Содержание = НСтр("ru='Возврат поставщику. Неоплаченная часть.'");
			Иначе
				Содержание = НСтр("ru='Возврат. Неоплаченная часть.'");
			КонецЕсли;
		ИначеЕсли ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем Тогда
			Содержание = НСтр("ru='Зачет аванса покупателя'");
		Иначе
			Содержание = НСтр("ru='Зачет аванса'");
		КонецЕсли;
...
Процедура ДобавитьСтрокиПогашенияЗадолженности(ТаблицаВзаиморасчетов, НераспределеннаяСумма, НераспределеннаяСуммаРегл, ОстаткиЗадолженности, СтрокаПлатежа, Реквизиты, Отказ)
	...
	Для каждого СтрокаОстатка Из ОстаткиЗадолженностиСОтбором Цикл
		...
		Иначе
			СодержаниеПроводки = ?(Реквизиты.ЭтоВозврат, 
										?(ТипЗнч(СтрокаОстатка.ДокументРасчетов) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя"),
...
Процедура ДобавитьСтрокуАванса(ТаблицаВзаиморасчетов, НераспределеннаяСумма, НераспределеннаяСуммаРегл, СтрокаДокумента, Реквизиты)
	...
	Иначе
		СодержаниеПроводки  = ?(Реквизиты.ЭтоВозврат, "Возврат", "Оплата (аванс)");
	КонецЕсли;
	СтрокаВзаиморасчетов.Содержание = СодержаниеПроводки;
...
Функция ВидыСубконтоРасчетов(Счет)
	...
	Для ИндексСубконто = 1 По 3 Цикл
		Если НЕ ЗначениеЗаполнено(СвойстваСчета["ВидСубконто" + ИндексСубконто]) ИЛИ СвойстваСчета["ВидСубконто" + ИндексСубконто + "ТолькоОбороты"] Тогда 
			Продолжить;
		КонецЕсли;
		ВидыСубконтоСчета.Добавить(СвойстваСчета["ВидСубконто" + ИндексСубконто]);
		Результат["ИспользоватьСубконто" + ИндексСубконто] = Истина;
	КонецЦикла;
	...
	Для Каждого ВидСубконто Из ШаблонВидыСубконтоРасчетов Цикл
		Если ВидыСубконтоСчета.Найти(ВидСубконто) <> Неопределено Тогда 
			ВидыСубконтоРасчетов.Добавить(ВидСубконто);
		КонецЕсли;
	КонецЦикла;
	
	Результат.ВидыСубконтоРасчетов = ВидыСубконтоРасчетов;
