...
// 
Функция ЗапускЗаданияОтправкиДокументовАсинхронныйОбмен(Знач НастройкаОбмена, Знач ИдентификаторСессии, Знач МассивЗапросовВыписок) Экспорт
...
//
Процедура ОтправитьЭДВБанк(Знач НастройкаОбмена, Знач ИдентификаторСессии, Знач МассивЗапросовВыписки, ДанныеВозврата) Экспорт
...
Процедура ПолучитьЗначенияРеквизитовНастройкиОбмена(Знач НастройкаОбмена, ЗапрашиваемыеРеквизиты) Экспорт
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена,
		...
		|ДополнительнаяОбработка, ОтправлятьДокументыБезПодписиSMS, АсинхронноЧерезДопОбработку");
...
//
Функция ПодписываетсяЗапросВыписки(Знач НастройкаОбмена) Экспорт
...
Функция ПоставляемыеНастройкиОбмена(Банк, ВключитьФО = Ложь, ЕстьПравоСозданияНастройкиЭДО = Ложь) Экспорт
	...
	БИКБанка = ЭлектронноеВзаимодействиеПереопределяемый.РеквизитыБанка(Банк).Код; 
	...
	Для Индекс = 2 По КоличествоЗаписей Цикл
		...
		ИначеЕсли ВариантПодключения = "ДополнительнаяОбработкаАсинхрон" Тогда
   			ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку;
   			АсинхронноЧерезДопОбработку = Истина;	
		КонецЕсли;
		...
		Настройки.Вставить("АсинхронноЧерезДопОбработку", АсинхронноЧерезДопОбработку);		
		Прервать
	КонецЦикла;
...
Функция ТипыПисемСбербанк(Знач ОтветБанка, Знач УникальныйИдентификатор, Знач НастройкаОбмена) Экспорт
	...
	Если ИспользуетсяТестовыйРежим Тогда
		АдресСервера = "https://eduhfsms.testsbi.sberbank.ru:9443";
	Иначе
...
Функция НазваниеБизнесСистемыСбербанк() Экспорт
	...
	Если ИспользуетсяТестовыйРежим Тогда
		НазваниеБизнесСистемы = "eduHF_UPG";
	КонецЕсли;
...
//
Функция ЗапускЗаданияАутентификацииПоЛогинуСбербанк(Знач НастройкаОбмена, Знач Логин, Знач Пароль, Знач ФродПараметры) Экспорт
...
Функция ДанныеДляОтправкиВБанкЧерезДопОбработку(Знач Ссылки) Экспорт
	...
	Пока ВыборкаНастройкиОбмена.Следующий() Цикл
		...
		КонецЦикла;
		Если ВыборкаНастройкиОбмена.НастройкаОбмена.ИспользуетсяКриптография Тогда 
			...
			КонецЕсли;
		Иначе 
			СтруктураДанных = Новый Структура("ДанныеПакетов, Сертификаты", ДанныеПакетов, Новый Массив);
			ДанныеВозврата.Вставить(ВыборкаНастройкиОбмена.НастройкаОбмена, СтруктураДанных);
		КонецЕсли;
	КонецЦикла;
...
Функция БазоваяАутентификация(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач ДанныеАутентификации, Знач ВерсияAPI, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	Заголовки.Вставить("APIVersion", ВерсияAPI);
	Заголовки.Вставить("AvailableAPIVersion", ОбменСБанкамиКлиентСервер.АктуальнаяВерсияФорматаАсинхронногоОбмена());
	Хеш = СтрокаBase64БезBOM(ДанныеАутентификации.Логин + ":" + ДанныеАутентификации.Пароль);
