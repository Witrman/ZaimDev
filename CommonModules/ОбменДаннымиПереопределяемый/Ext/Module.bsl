...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	...
	//ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменУправлениеТорговлей22Бухгалтерия30);
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменУправлениеТорговлейБухгалтерияПредприятия30);
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменУправлениеТорговлей22Бухгалтерия30);
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменЗарплатаИУправлениеПерсоналомБухгалтерияПредприятия);
...
//Для обмена данными с УНФ через универсальный формат используется ОМ МенеджерОбменаЧерезУниверсальныйФормат_УНФ 
Процедура ПолучитьВерсииФорматаОбмена(УзелИнформационнойБазы, ВерсииФормата) Экспорт
	
	Если ЗначениеЗаполнено(УзелИнформационнойБазы) И (УзелИнформационнойБазы.ВариантНастройки = "ОбменУНФБП" Или УзелИнформационнойБазы.ВариантНастройки = "ОбменБП30") Тогда	
		ВерсииФормата.Вставить(СокрЛП(УзелИнформационнойБазы.ВерсияФорматаОбмена), МенеджерОбменаЧерезУниверсальныйФормат_УНФ);	
	КонецЕсли;	
