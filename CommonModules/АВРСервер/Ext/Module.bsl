...
//
Процедура СоздатьИсходящиеAwp(Знач МассивАВР, Знач УстанавливатьПодпись, Знач ТипПодписиАВР, КоллекцияAwpXML, КоллекцияSignedContentXML, УполномоченныйСотрудник, ВерсияИСЭСФ, ДолжностьПодписывающего) Экспорт
	
	ЭСФСерверПовтИсп.ОбработкаОбменЭСФ().СоздатьИсходящиеAwp(МассивАВР, УстанавливатьПодпись, ТипПодписиАВР, КоллекцияAwpXML, КоллекцияSignedContentXML, УполномоченныйСотрудник, ВерсияИСЭСФ, ДолжностьПодписывающего);
...
Функция ПроверитьВозможностьОтправкиДокументовАВР(МассивИсходящихАВР, ДополнительныеПараметры) Экспорт
	...
	//проверка на заполненность документов АВР
	ЕстьОшибкиЗаполнения = Ложь;
	Для Каждого СсылкаАВР Из МассивИсходящихАВР Цикл
		
		РезультатПроверки = ОбработкаОбменЭСФ.ПроверитьАВР(СсылкаАВР);
		ОбработкаОбменЭСФ.СообщитьОбнаруженыОшибки(СсылкаАВР, РезультатПроверки);
...
// Заполняет документ АВР на основании документа.
//
// Параметры:
...
//  ДополнительныеПарметры - дополнительные данные которые необходимо передать в запросы.
//
Процедура ЗаполнитьИсходящийАВР(Знач СсылкаДокументОснование, ОбъектАВР, ДополнительныеПараметры) Экспорт
	
	ЭСФСерверПовтИсп.ОбработкаОбменЭСФ().ЗаполнитьИсходящийАВР(СсылкаДокументОснование, ОбъектАВР, ДополнительныеПараметры);
...
Процедура ПроверитьИсходящиеЭАВР(Знач МассивИсходящихЭАВР) Экспорт
	...
	ЭСФСерверПовтИсп.ОбработкаОбменЭСФ().ПроверитьИсходящиеЭАВР(МассивИсходящихЭАВР);
...
Функция СостояниеАВР(АВР) Экспорт
	...
	ИначеЕсли АВР.Статус = Перечисления.СтатусыАВР.Расторгнут Тогда
	...
	ИначеЕсли АВР.Статус = Перечисления.СтатусыАВР.Аннулирован Тогда
		
		Состояние = Перечисления.СостоянияАВР.Аннулирован;
...
Функция РазрешенныеДействияПоСтатусамАВР(Знач Направление) Экспорт
	...
	Если Направление = Перечисления.НаправленияЭСФ.Исходящий Тогда	
		...
		СоответствиеДоступностиДействия.Вставить(Перечисления.СтатусыАВР.Расторгнут			, Ложь);
		...
		СоответствиеДоступностиДействия.Вставить(Перечисления.СтатусыАВР.Аннулирован		, Ложь);
...
///////////////////////////////////////////////////////////////////////////////
// Работа с регистром сведений АктуальныеЭАВР

// Возвращает актуальный ЭАВР для документа основания
//
// Параметры:
//  ДокументОснование - ДокументСсылка.ДокументОснование -
//   ДокументОснование, для которого необходимо найти актуальный ЭАВР
//
// Возвращаемое значение:
//  Структура, Неопределено - Данные найденного актуального ЭАВР.
//   |- ПервичныеДокументыЭАВР - ДокументСсылка.ДокументОснование - ДокументОснование, для которого искался ЭАВР.
//   |- ЭАВР - ДокументСсылка.ЭАВР - ЭАВР, который является актуальным для документа основания.
//   |- СостоянияЭАВР - ПеречислениеСсылка.СостоянияАВР - Состояние актуального ЭАВР.
//  Если для ДокументОснование не задан актуальный ЭАВР, то возвращается Неопределено.
//
Функция ДанныеАктуальногоЭАВР(Знач ДокументОснование) Экспорт
	
	ДанныеАктуальногоЭАВР = Неопределено;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	АктуальныеЭАВР.ПервичныеДокументыЭАВР КАК ДокументОснование,
	|	АктуальныеЭАВР.ЭАВР КАК ЭАВР,
	|	АктуальныеЭАВР.СостоянияЭАВР КАК СостоянияЭАВР
	|ИЗ
	|	РегистрСведений.АктуальныеЭАВР КАК АктуальныеЭАВР
	|ГДЕ
	|	АктуальныеЭАВР.ПервичныеДокументыЭАВР = &ДокументОснование";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Если Выборка.Следующий() Тогда
			ДанныеАктуальногоЭАВР = Новый Структура;
			ДанныеАктуальногоЭАВР.Вставить("ДокументОснование", Выборка.ДокументОснование);
			ДанныеАктуальногоЭАВР.Вставить("ЭАВР", Выборка.ЭАВР);
			ДанныеАктуальногоЭАВР.Вставить("СостоянияЭАВР", Выборка.СостоянияЭАВР);
		КонецЕсли;
	КонецЕсли;
	
	Возврат ДанныеАктуальногоЭАВР;
...
Функция ПолучитьСтруктуруДанныхРеквизитов()
	...
	Реквизиты.Вставить("ТипАВР"); //нов
	Реквизиты.Вставить("ДатаОсновногоАВР"); //нов
	Реквизиты.Вставить("НомерОсновногоАВР"); //нов
	Реквизиты.Вставить("РегистрационныйНомерОсновногоАВР"); //нов
	Реквизиты.Вставить("ПереченьДокументации"); //нов
	Реквизиты.Вставить("ДополнительноеНаименование"); //нов
	Реквизиты.Вставить("ИспользованныеЗапасы"); //нов
	Реквизиты.Вставить("ДолжностьСдающего"); //нов
...
Функция ПолучитьСтруктуруДанныхПоставщикиПолучатели(ПоставщикПолучатель = "Поставщик")
	...
	СтруктураДанных.Вставить("ДатаДокументаПоверенного"); //нов
	СтруктураДанных.Вставить("НомерДокументаПоверенного"); //нов
...
Функция ПолучитьСтруктуруТаблицыУслуги()
	...
	Услуги.Колонки.Добавить("СоставнойКодГСВС",                   Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(115))); //нов
	Услуги.Колонки.Добавить("ДатаВыполнения",                     Новый ОписаниеТипов("Дата", , Новый КвалификаторыДаты(ЧастиДаты.Дата))); //нов
	Услуги.Колонки.Добавить("НаименованиеУслугиВСистеме",         Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(400))); //нов
...
Функция ПроверитьПервичныеДокументыНаНаличиеИсправленныхЭАВР(МассивОснований) Экспорт
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЭлектронныйАктВыполненныхРабот.Ссылка) КАК Ссылка,
	|	ЭлектронныйАктВыполненныхРабот.ДокументОснование КАК ДокументОснование,
	|	ЭлектронныйАктВыполненныхРабот.ТипАВР КАК ТипАВР
	|ПОМЕСТИТЬ ВТ_ЭлектронныйАктВыполненныхРабот
	|ИЗ
	|	Документ.ЭлектронныйАктВыполненныхРабот КАК ЭлектронныйАктВыполненныхРабот
	|ГДЕ
	|	ЭлектронныйАктВыполненныхРабот.ДокументОснование В(&Основание)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЭлектронныйАктВыполненныхРабот.ДокументОснование,
	|	ЭлектронныйАктВыполненныхРабот.ТипАВР
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_ЭлектронныйАктВыполненныхРабот.Ссылка) КАК Ссылка,
	|	ВТ_ЭлектронныйАктВыполненныхРабот.ДокументОснование КАК ДокументОснование
	|ИЗ
	|	ВТ_ЭлектронныйАктВыполненныхРабот КАК ВТ_ЭлектронныйАктВыполненныхРабот
	|ГДЕ
	|	ВТ_ЭлектронныйАктВыполненныхРабот.ТипАВР = &ТипАВР
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ЭлектронныйАктВыполненныхРабот.ДокументОснование";
	
	Запрос.УстановитьПараметр("Основание", МассивОснований);
	Запрос.УстановитьПараметр("ТипАВР", ПредопределенноеЗначение("Перечисление.ТипыАВР.Исправленный"));
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	МассивДокументовСИсправленнымЭАВР = Новый Массив();
	
	Пока Выборка.Следующий() Цикл
		
		ДокументСЭАВР = Новый Структура();
		
		ДокументСЭАВР.Вставить("ИсправленныйЭАВР", Выборка.Ссылка);
		ДокументСЭАВР.Вставить("ПервичныйДокумент", Выборка.ДокументОснование);
		
		МассивДокументовСИсправленнымЭАВР.Добавить(ДокументСЭАВР);
		
	КонецЦикла;
	
	Возврат МассивДокументовСИсправленнымЭАВР;
	
КонецФункции

#КонецОбласти

#Область РаботаССоответствиямиНаименованийИСсылок

// См. Обработки.ОбменЭСФ.СохранитьСоответствияНаименованийИСсылокЕдиницИзмеренийИУслуг()
Процедура СохранитьСоответствияНаименованийИСсылокЕдиницИзмеренийИУслуг(Знач КлючиАналитикиУчетаСоответствий, Знач НаправлениеАВР, Знач МассивАВР) Экспорт
	
	ЭСФСерверПовтИсп.ОбработкаОбменЭСФ().СохранитьСоответствияНаименованийИСсылокЕдиницИзмеренийИУслуг(КлючиАналитикиУчетаСоответствий, НаправлениеАВР, МассивАВР);
	
КонецПроцедуры

// См. Обработки.ОбменЭСФ.ИнициализироватьКлючиАналитикиУчетаУслуг()
Функция ИнициализироватьКлючиАналитикиУчетаУслуг(Знач МассивАВР, Знач СоздатьНовыеПриОтсутствии = Ложь) Экспорт
	
	Возврат ЭСФСерверПовтИсп.ОбработкаОбменЭСФ().ИнициализироватьКлючиАналитикиУчетаУслуг(МассивАВР, СоздатьНовыеПриОтсутствии);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура           = "АВРСервер.ЗаполнитьТипАВР";
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения     = "Отложенно";
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "1.0.0.12";
	Обработчик.Процедура           = "АВРСервер.ЗаполнитьСвязанныйАВР";
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения     = "Отложенно";
	
	// Заполнение статуса АВР Аннулирован
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "1.0.0.12";
	Обработчик.Процедура           = "АВРСервер.ЗаполнитьСтатусАннулирован";
	Обработчик.ОбщиеДанные         = Ложь;
...
Процедура ЗаполнитьТипАВР(Параметры) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ЭАВР.Ссылка КАК Ссылка,
	|	ЭАВР.ДатаОсновногоАВР КАК ДатаОсновногоАВР,
	|	ЭАВР.НомерОсновногоАВР КАК НомерОсновногоАВР,
	|	ЭАВР.РегистрационныйНомерОсновногоАВР КАК РегистрационныйНомерОсновногоАВР,
	|	ЭАВР.СвязанныйАВР КАК СвязанныйАВР
	|ИЗ
	|	Документ.ЭлектронныйАктВыполненныхРабот КАК ЭАВР
	|ГДЕ
	|	НЕ ЭАВР.Ссылка В (&МассивОбработанныхАВР)
	|	И ЭАВР.ТипАВР = ЗНАЧЕНИЕ(Перечисление.ТипыАВР.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЭАВР.Дата";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки = Блокировка.Добавить("Документ.ЭлектронныйАктВыполненныхРабот");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.Ссылка);
			Блокировка.Заблокировать();
			
			ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();
			Если ЗначениеЗаполнено(Выборка.ДатаОсновногоАВР)
				ИЛИ ЗначениеЗаполнено(Выборка.НомерОсновногоАВР)
				ИЛИ ЗначениеЗаполнено(Выборка.РегистрационныйНомерОсновногоАВР)
				ИЛИ ЗначениеЗаполнено(Выборка.СвязанныйАВР) Тогда
				
				ДокументОбъект.ТипАВР = Перечисления.ТипыАВР.Исправленный;
			Иначе
				ДокументОбъект.ТипАВР = Перечисления.ТипыАВР.Первичный;
			КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='При записи документа %1 для заполнения реквизита ""ТипАВР"" произошла ошибка: %2'");
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ДокументОбъект, ОписаниеОшибки());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗаполнитьСвязанныйАВР(Параметры) Экспорт
	
	Параметры.ОбработкаЗавершена = Ложь;
	
	Если Параметры.Свойство("МассивОбработанныхАВРСвязанныйАВР") Тогда
		МассивОбработанныхАВРСвязанныйАВР = Параметры.МассивОбработанныхАВРСвязанныйАВР;
	Иначе
		МассивОбработанныхАВРСвязанныйАВР = Новый Массив;
		Параметры.Вставить("МассивОбработанныхАВРСвязанныйАВР", МассивОбработанныхАВРСвязанныйАВР);
	КонецЕсли;
	...
	|	ЭАВР.Ссылка КАК Ссылка,
	|	ЭАВР.РегистрационныйНомерОсновногоАВР КАК РегистрационныйНомерОсновногоАВР,
	|	ЭАВР.Направление КАК Направление
	|ПОМЕСТИТЬ ВТ_ДокументыБезСвязанных
	|ИЗ
	|	Документ.ЭлектронныйАктВыполненныхРабот КАК ЭАВР
	|ГДЕ
	|	НЕ ЭАВР.Ссылка В (&МассивОбработанныхАВРСвязанныйАВР)
	|	И ЭАВР.СвязанныйАВР = ЗНАЧЕНИЕ(Документ.ЭлектронныйАктВыполненныхРабот.ПустаяСсылка)
	|	И (ЭАВР.ТипАВР = ЗНАЧЕНИЕ(Перечисление.ТипыАВР.ПустаяСсылка)
	|			ИЛИ ЭАВР.ТипАВР = ЗНАЧЕНИЕ(Перечисление.ТипыАВР.Исправленный))
	|	И ЭАВР.РегистрационныйНомерОсновногоАВР <> """"
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЭАВР.Дата
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РегистрационныйНомерОсновногоАВР,
	|	Направление
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_ДокументыБезСвязанных.Ссылка КАК Ссылка,
	|	ЭлектронныйАктВыполненныхРабот.Ссылка КАК СсылкаСвязанныйАВР
	|ИЗ
	|	ВТ_ДокументыБезСвязанных КАК ВТ_ДокументыБезСвязанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭлектронныйАктВыполненныхРабот КАК ЭлектронныйАктВыполненныхРабот
	|		ПО ВТ_ДокументыБезСвязанных.РегистрационныйНомерОсновногоАВР = ЭлектронныйАктВыполненныхРабот.РегистрационныйНомер
	|			И ВТ_ДокументыБезСвязанных.Направление = ЭлектронныйАктВыполненныхРабот.Направление";
	
	Запрос.УстановитьПараметр("МассивОбработанныхАВРСвязанныйАВР", МассивОбработанныхАВРСвязанныйАВР);
	РезультатЗапроса = Запрос.Выполнить();
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивОбработанныхАВРСвязанныйАВР.Добавить(Выборка.Ссылка);
		...
		Попытка
			...
			ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ДокументОбъект.СвязанныйАВР = Выборка.СсылкаСвязанныйАВР;
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='При записи документа %1 для заполнения реквизита ""СвязанныйАВР"" произошла ошибка: %2'");
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ДокументОбъект, ОписаниеОшибки());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//Заполняет документы с пустым статусом и заполненным рег.номером
//оформленные после даты перехода на версию 2 (февраль 2023). 
//Подобные документы могли быть получены, когда был патч, так как 
//значения в перечислении СтатусыАВР добавили только в сборке.
//
Процедура ЗаполнитьСтатусАннулирован(Параметры) Экспорт
	
	Параметры.ОбработкаЗавершена = Ложь;
	
	Если Параметры.Свойство("МассивОбработанныхАВРПустойСтатус") Тогда
		МассивОбработанныхАВРПустойСтатус = Параметры.МассивОбработанныхАВРПустойСтатус;
	Иначе
		МассивОбработанныхАВРПустойСтатус = Новый Массив;
		Параметры.Вставить("МассивОбработанныхАВРПустойСтатус", МассивОбработанныхАВРПустойСтатус);
	КонецЕсли;
	...
	|ГДЕ
	|	НЕ ЭАВР.Ссылка В (&МассивОбработанныхАВР)
	|	И ЭАВР.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыАВР.ПустаяСсылка)
	|	И ЭАВР.Дата >= &ДатаВведенияНовойВерсии
	|	И ЭАВР.РегистрационныйНомер <> """"
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЭАВР.Дата";
	
	Запрос.УстановитьПараметр("МассивОбработанныхАВР", МассивОбработанныхАВРПустойСтатус);
	Запрос.УстановитьПараметр("ДатаВведенияНовойВерсии", АВРКлиентСервер.ДатаВводаЭАВРВерсия2());
	РезультатЗапроса = Запрос.Выполнить();
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивОбработанныхАВРПустойСтатус.Добавить(Выборка.Ссылка);
		...
		Попытка
			...
			ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ДокументОбъект.Статус = Перечисления.СтатусыАВР.Аннулирован;
			ДокументОбъект.ОбменДанными.Загрузка = Истина;
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='При записи документа %1 для заполнения реквизита ""Статус"" произошла ошибка: %2'");
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ДокументОбъект, ОписаниеОшибки());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
