...
//
Процедура РассчитатьКоличествоПисемПоРазделам(СтруктураПараметров,
...
//
Процедура ОтправитьЗапросСтатусаПлатежногоДокумента(Параметры,
...
//  АдресРезультата - Строка - адрес временного хранилища для помещения результата.
//
Процедура ОтправитьЗапросНаПроверкуSMSСбербанк(Параметры,
		АдресРезультата) Экспорт
	...
	ТекстЗапроса = ТекстЗапросаПроверкиSMSСбербанк(Параметры.НастройкаОбмена, ВнешнийИдентификатор, Параметры.ИдентификаторКриптопрофиля, Параметры.SMSКод, Параметры.ТекущаяСессия);
...
//
Процедура ОтправитьЗапросНовыхДокументовСбербанк(Параметры,
		АдресРезультата) Экспорт

	ИдентификаторЗапроса = Строка(Новый УникальныйИдентификатор);
	ИдентификаторОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.НастройкаОбмена, "ИдентификаторОрганизации");
	ТекстЗапроса = ОбменСБанкамиСлужебныйВызовСервера.ТекстЗапросаНочнойВыписки(Параметры.НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации);
	Данные = Новый Массив;
	Данные.Добавить(ТекстЗапроса);
	Тикеты = Неопределено;
	ОтправитьДанныеВСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии, Данные, Тикеты, Неопределено);
...
//
Процедура ПолучитьИдентификаторКриптопрофиляСбербанк(Параметры,
		АдресРезультата) Экспорт
	...
	ПолучитьОтветыПоТикетамСбербанк(Параметры.НастройкаОбмена, МассивТикетов, Параметры.ТекущаяСессия.ИдентификаторСессии, ИдентификаторОрганизации, МассивОтветов);
...
//  АдресРезультата - Строка - адрес, по которому будут помещены результаты выполнения.
//
Процедура ПолучитьРезультатПроверкиSMSСбербанк(Параметры,
		АдресРезультата) Экспорт
...
//
Процедура ОпределитьИдентификаторВКСбербанк(Параметры,
		Адрес = Неопределено) Экспорт
...
Процедура ОтправитьЗапросыСтатусовЗапросовПоЛогинуСбербанк(Параметры, АдресРезультата) Экспорт
	...
	ТекстЗапроса = ТекстЗапросаСостоянияЗапросовВыписокСбербанк(Параметры.НастройкаОбмена, Параметры.МассивСообщенийОбмена, КоличествоИдентификаторов);
...
Процедура ПолучитьСтатусыЗапросовПоЛогинуСбербанк(Параметры, АдресРезультата) Экспорт
	...
	МассивОтветов = Новый Массив;
	ПолучитьОтветыПоТикетамСбербанк(Параметры.НастройкаОбмена, Параметры.МассивТикетов, Параметры.ТекущаяСессия.ИдентификаторСессии, ИдентификаторОрганизации, МассивОтветов, Параметры.КоличествоЗапросов);
	...
	Для Каждого Ответ Из МассивОтветов Цикл
		...
		ИначеЕсли Ответ = "<!--REQUEST NOT FOUND-->"
			...
			СохранитьТикетКакВнешнийИдентификаторСообщенияОбменаСбербанк(Параметры.НастройкаОбмена, Параметры.МассивТикетов.Получить(Индекс));
		...
		Иначе
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбработатьОтветСбербанка(Ответ, Параметры.НастройкаОбмена, МассивНовыхСообщенийОбмена, КоличествоТомов, Параметры.СоздаватьОперацииВыписки);
			Если КоличествоТомов > 0 Тогда
				ПолучитьБольшойПакетСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии, ИдентификаторОрганизации, ИдентификаторЗапроса, КоличествоТомов, МассивНовыхСообщенийОбмена);
			КонецЕсли;
		КонецЕсли;
		Индекс = Индекс + 1;
	КонецЦикла;
...
Процедура СформироватьЗапросыВыписокСбербанк(Параметры, АдресРезультата) Экспорт
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.НастройкаОбмена, "Организация, Банк, ПрограммаБанка, ИдентификаторОрганизации, ИспользуетсяКриптография");
	...
	Для Счетчик = 1 По КоличествоДней Цикл
		...
		Для Каждого НомерСчета Из Параметры.МассивБанковскихСчетов Цикл
			...
			ДанныеЗапроса = ДанныеЗапросаВыпискиСбербанк(Параметры.НастройкаОбмена, ДатаЗапроса, НомерСчета, ИдентификаторЗапроса);
			...
			Если РеквизитыНастройкиОбмена.ИспользуетсяКриптография Тогда
				ДайджестBase64 = ДайджестСбербанк(Перечисления.ВидыЭДОбменСБанками.ЗапросВыписки, АдресФайла, Параметры.НастройкаОбмена);
...
Процедура ЗагрузитьСписокБанковССайта() Экспорт
	...
	Возврат; // Список банков для Республики Казахстан пока не публикуется.
	УстановитьПривилегированныйРежим(Истина);
...
Функция ПространствоИменАсинхронногоОбмена(ВерсияФормата) Экспорт
	...
	ИначеЕсли ВерсияФормата = "2.05" Тогда
		URI = "http://bank.1c.ru/kz/XMLSchema";
	Иначе
...
Процедура ОбработатьЭлектронныеДокументы(Параметры, АдресРезультата) Экспорт
	...
	Если НЕ ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
		ТекстСообщения = ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ТекстСообщенияОНеобходимостиНастройкиСистемы("РАБОТАСБАНКАМИ");
	...
	// Основной запрос - выборка из временных таблиц:
		...
		ИначеЕсли МассивСсылокНаОбъект.Количество() = 1 И НастройкиОбмена.ПараметрыОбмена.Подписывать Тогда
			...
			Если НЕ ЗначениеЗаполнено(СообщениеОбмена)
				ИЛИ НЕ ОбменСБанкамиСлужебныйВызовСервера.ЭлектронныйДокументПолностьюПодписан(СообщениеОбмена) Тогда
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		...
		Если ТЗ_ЭД_КОтправке.Количество() > 0 Тогда
			...
			Для Каждого НастройкаОбмена Из МассивНастроекОбмена Цикл
				...
				Иначе
					СоотвНастроекОбменаИМассивовСообщенийОбменаКОтправкеСАвторизацией.Вставить(НастройкаОбмена, МассивСообщенийОбмена);
				КонецЕсли;
			КонецЦикла;
			...
			Для Каждого НастройкаОбмена Из МассивНастроекОбмена Цикл
				...
				МассивСообщенийОбмена = СоотвНастроекОбменаИМассивовСообщенийОбменаКОтправкеСАутентификациейПоСертификату.Получить(НастройкаОбмена);
				Если МассивСообщенийОбмена = Неопределено Тогда
					МассивСообщенийОбмена = Новый Массив;
				КонецЕсли;
				...
				СоотвНастроекОбменаИМассивовСообщенийОбменаКОтправкеСАутентификациейПоСертификату.Вставить(НастройкаОбмена, МассивСообщенийОбмена);
			КонецЦикла;
				...
				Для Каждого Элемент Из СоотвНастроекОбменаИМассивовСообщенийОбменаКОтправкеСАутентификациейПоСертификату Цикл
					...
					Для Каждого КлючИЗначение Из СоотвСертификатовИПараметров Цикл
						...
						Если ТипЗнч(СтруктураСертификата) = Тип("Структура")
								...
								И (ЗначениеЗаполнено(ИдентификаторСессии)
			...
			Если МассивСообщенийОбменаКОтправкеССервераБезПодписи.Количество() > 0 Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураВозврата.ОтправленныеДокументы, СтРезультата.ОтправленныеДокументы);
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураВозврата.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК, СтРезультата.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК, Истина);
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураВозврата.ПодтверждениеПлатежейВБанке.НастройкиОбмена, СтРезультата.ПодтверждениеПлатежейВБанке.НастройкиОбмена);
			КонецЕсли;
...
Функция ПодчиненноеСообщениеОбмена(ИсходноеСообщение, ВидЭД = Неопределено, ВыдаватьОшибку = Истина) Экспорт
	...
	Иначе
		Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ДополнительныеДанные
			...
			Дайджест = ДайджестСбербанк(РеквизитыИсходногоСообщенияОбмена.ВидЭД, ДанныеФайла.СсылкаНаДвоичныеДанныеФайла, РеквизитыИсходногоСообщенияОбмена.НастройкаОбмена);
...
Процедура СохранитьСообщениеОбмена(Знач ПараметрыСообщения, НовоеСообщениеСсылка = Неопределено) Экспорт
	...
	Если (Не ПараметрыСообщения.Свойство("Организация")
			ИЛИ Не ПараметрыСообщения.Свойство("Банк"))
			И ПараметрыСообщения.Свойство("НастройкаОбмена") Тогда
		РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыСообщения.НастройкаОбмена, "Организация, Банк");
...
Процедура ПолучитьНовыеДокументыИзБанка(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	АдресСервера = ?(Прав(РеквизитыНастройкиОбмена.АдресСервера, 1) = "/", РеквизитыНастройкиОбмена.АдресСервера, РеквизитыНастройкиОбмена.АдресСервера
		+ "/");
	...
	ПрочитатьИдентификаторыПакетов(Параметры.НастройкаОбмена, Результат.Путь, МассивИдентификаторов, ДанныеВозврата.ТребуетсяПовторнаяАутентификация);
	УдалитьИзВременногоХранилища(Результат.Путь);
	...
	Для Каждого Идентификатор Из МассивИдентификаторов Цикл
		...
		СохранитьПолученныйПакет(Параметры.НастройкаОбмена, Результат.Путь, СоздаватьОперацииВыписки, ДанныеВозврата, ДанныеВозврата.ТребуетсяПовторнаяАутентификация);
		УдалитьИзВременногоХранилища(Результат.Путь);
		...
		ДанныеВозврата.КоличествоПолученныхПакетов = ДанныеВозврата.КоличествоПолученныхПакетов
			+ 1;
...
Процедура ОтправитьЗапросСостоянияЭДВБанк(СтруктураПараметров,
		АдресХранилища) Экспорт
...
//
Процедура ПолучитьИзвещениеОСостоянииЭДАсинхронно(СтруктураПараметров,
		АдресХранилища) Экспорт
	...
	Пока Истина Цикл
		...
		ИзвещениеПолучено = ОбменСБанкамиСлужебныйВызовСервера.ПолученоИзвещениеПоЗапросу(СтруктураПараметров.СообщениеОбмена, ЕстьОшибка);
...
//
Функция ОтправкаПакетовВБанк(МассивПакетов, ИдентификаторСессии = Неопределено,
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетов Цикл
		...
		РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена, "АдресСервера, РесурсИсходящихДокументов, РесурсВходящихДокументов, ИспользуетсяКриптография, ПрограммаБанка,
		...
		Попытка
			...
			СтруктураВозврата.КоличествоОтправлено = СтруктураВозврата.КоличествоОтправлено
				+ 1;
...
Функция СоздатьДокументыПакетовЭД(МассивСообщенийОбмена) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	СообщениеОбменСБанками.НастройкаОбмена.АсинхронноЧерезДопОбработку КАК АсинхронноЧерезДопОбработку,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПЭД = Неопределено Тогда
			...
			Если Выборка.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку
				И НЕ Выборка.АсинхронноЧерезДопОбработку Тогда
	...
	Для Каждого ТекСтрока Из ТЗ_ПЭД Цикл
		...
		Если (ТекСтрока.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку И Не ТекСтрока.ПЭД.НастройкаОбмена.АсинхронноЧерезДопОбработку)
...
Процедура СформироватьИОтправитьПакетыВБанк(Параметры, АдресХранилища) Экспорт
	...
	Если МассивСтруктурПакетовЭД.Количество() > 0 Тогда
		Для Каждого СтруктураПЭД Из МассивСтруктурПакетовЭД Цикл
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураРезультата.ОтправленныеДокументы, Результат.ОтправленныеДокументы);

			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураРезультата.ОтправленныеСообщенияОбмена, Результат.ОтправленныеСообщенияОбмена);

			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураРезультата.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК, Результат.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК, Истина);

			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураРезультата.ПодтверждениеПлатежейВБанке.НастройкиОбмена, Результат.ПодтверждениеПлатежейВБанке.НастройкиОбмена, Истина);
...
//
Процедура РаспаковатьПакетОбменСБанками(ПакетОбменСБанками,
		СоздаватьОперацииВыписки, ДанныеВозврата) Экспорт
	...
	Попытка
		...
		Если ЗначениеСвойстваXDTO(ResultBank, "Success.GetPacketResponse.Document") <> Неопределено Тогда
			...
			Если ТипЗнч(Document) = Тип("ОбъектXDTO") Тогда
				ПрочитатьДокументИзПакета(НастройкаОбмена, ПакетОбменСБанками, Document, МассивВнешнихИдентификаторов, СоздаватьОперацииВыписки, ДанныеВозврата);
			Иначе
				Для Каждого Документ Из Document Цикл
					ПрочитатьДокументИзПакета(НастройкаОбмена, ПакетОбменСБанками, Документ, МассивВнешнихИдентификаторов, СоздаватьОперацииВыписки, ДанныеВозврата);
				КонецЦикла;
			КонецЕсли;
		...
		Если НЕ ДанныеВозврата.ЕстьОшибка Тогда
			ОбменСБанкамиСлужебныйВызовСервера.УстановитьСтатусПакета(ПакетОбменСБанками, Перечисления.СтатусыПакетовЭД.Распакован);
		КонецЕсли;
...
//
Процедура СохранитьПолученныйПакет(НастройкаОбмена, Путь,
...
Процедура ПрочитатьИдентификаторыПакетов(НастройкаОбмена, Путь,
	...
	Запрос.УстановитьПараметр("НастройкаОбмена", НастройкаОбмена);
	//// На серверных базах в некоторых случаях может изменяться сортировка идентификаторов,
	// // что может привести к некорректному обновлению статусов сообщений обмена
	// МассивИдентификаторов = ОбщегоНазначенияКлиентСервер.СократитьМассив(
...
Процедура СоздатьЭДПакетаAsync(Конверт, СвойстваСертификатов = Неопределено) Экспорт
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена, "Организация, Банк, ИдентификаторОрганизации, СжиматьДанныеПакетаЭД, ВерсияФормата");
	СведенияОбОрганизации = Неопределено;
	ЭлектронноеВзаимодействиеПереопределяемый.ПолучитьДанныеЮрФизЛица(РеквизитыНастройкиОбмена.Организация, СведенияОбОрганизации);
	РеквизитыБанка = ЭлектронноеВзаимодействиеПереопределяемый.РеквизитыБанка(РеквизитыНастройкиОбмена.Банк);
	...
	Попытка
		...
		ЗаполнитьСвойствоXDTO(CustomerPartyType, "id", РеквизитыНастройкиОбмена.ИдентификаторОрганизации, Истина, ТекстОшибки);
		...
		ЗаполнитьСвойствоXDTO(CustomerPartyType, "kbe", СведенияОбОрганизации.КБЕ, , ТекстОшибки);
		...
		Для каждого Строка Из Конверт.Сообщения Цикл
			...
			РеквизитыЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СообщениеОбмена, "ВидЭД, Идентификатор, ВидОперацииЭД");
			...
			Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение Тогда
				Если РеквизитыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеЗаработнойПлаты Тогда
					ВидЭДПакета = "41"
				ИначеЕсли РеквизитыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеПенсионныхВзносов Тогда
					ВидЭДПакета = "42"
				ИначеЕсли РеквизитыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеСоциальныхОтчислений Тогда
					ВидЭДПакета = "43"
				КонецЕсли;
			КонецЕсли;
...
Процедура СформироватьПлатежноеПоручение(
	...
	ВидОплаты = ЗначениеРеквизитаВДереве(ДеревоДокумента, "РеквизитыПлатежа.ВидОплаты");
	Если ВидОплаты = "41" Тогда
		ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеЗаработнойПлаты;
	ИначеЕсли ВидОплаты = "42" Тогда
		ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеПенсионныхВзносов;
	ИначеЕсли ВидОплаты = "43" Тогда
		ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеСоциальныхОтчислений;
	Иначе
		ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручение;
	КонецЕсли;
	СтруктураЭД.Вставить("ВидОперацииЭД", ВидОперацииЭД);
	...
	Иначе
		Если НастройкиОбменаЭД.НастройкаОбмена.АсинхронноЧерезДопОбработку Тогда 
			СформироватьПлатежноеПоручениеAsync(СсылкаНаОбъект, ДеревоДокумента, АдресФайлаВоВременномХранилище);
		Иначе 
...
Процедура СоздатьНастройкуОбмена(СтруктураПараметров, АдресХранилища) Экспорт
	...
	НастройкаОбменаСсылка = ОбменСБанкамиСлужебныйВызовСервера.НастройкаОбмена(СтруктураПараметров.Организация, СтруктураПараметров.Банк, Ложь, Истина);
	...
	ЗаполнитьЗначенияСвойств(НастройкаОбменаОбъект, СтруктураПараметров, "Организация, Банк, ИспользуетсяКриптография,
		...
		|НаименованиеБанка, НомерПодразделенияБанка, ДополнительнаяОбработка, АсинхронноЧерезДопОбработку");
	...
	Если СтруктураПараметров.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн
		...
		Для Каждого Элемент Из СтруктураПараметров.СертификатыБанка Цикл
			...
			Если ДвоичныеДанныеСертификата = Неопределено Тогда
				...
				ОбработатьОшибку(Операция, ТекстСообщения, ТекстСообщения, НастройкаОбменаСсылка);
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли СтруктураПараметров.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
			И НастройкаОбменаОбъект.АутентификацияПоСертификату Или СтруктураПараметров.АсинхронноЧерезДопОбработку  Тогда
	...
	Если СтруктураПараметров.ИспользуетсяКриптография Тогда
		...
		Для Каждого СтрокаВидаЭД Из НастройкаОбменаОбъект.ИсходящиеДокументы Цикл
			...
			Если ВидыДокументовСНесколькимиПодписями.Найти(ТекущийВидЭД) <> Неопределено Тогда
				...
				МаршрутОбъект.Наименование = СтрШаблон("%1 - %2 (%3)", НастройкаОбменаОбъект.Организация, НастройкаОбменаОбъект.Банк, ТекущийВидЭД);
				...
				МаршрутОбъект.КлючАвтоматическойНастройки = Справочники.НастройкиОбменСБанками.КлючАвтоматическойНастройки(НастройкаОбменаОбъект, ТекущийВидЭД);
	...
	НастройкаОбменаОбъект.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонНаименования, СтруктураПараметров.Организация, СтруктураПараметров.Банк);
...
Функция СоздатьНастройкуОбменаИзФайла(НастройкиЗаполнения, Организация,
		Недействительна, ЛокальныйФайл, МассивСертификатов = Неопределено,
		ЗагружатьВК = Истина, НастройкаОбмена = Неопределено) Экспорт
	...
	КБЕОрганизации = ЭД.Recipient.kbe;
 	Если ЗначениеЗаполнено(КБЕОрганизации) Тогда
  		СтруктураПоискаОрганизации.Вставить("КБЕ", КБЕОрганизации);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		...
		Если СтруктураПоискаОрганизации.Свойство("ИНН") Тогда
			ТекстИНН = Символы.ПС + НСтр("ru = 'ИНН/БИН: %1'");
			ТекстИНН = СтрШаблон(ТекстИНН, СтруктураПоискаОрганизации.ИНН);
		Иначе
			ТекстИНН = "";
		КонецЕсли;
		Если СтруктураПоискаОрганизации.Свойство("КБЕ") Тогда
			ТекстКБЕ = Символы.ПС + НСтр("ru = 'КБЕ: %1'");
			ТекстКБЕ = СтрШаблон(ТекстКБЕ, СтруктураПоискаОрганизации.КБЕ);
		Иначе
			ТекстКБЕ = "";
		КонецЕсли;
		...
		Иначе
			ТекстНаименование = "";
		КонецЕсли;
		ТекстСообщения = СтрШаблон(ТекстСообщения, ТекстИНН, ТекстКБЕ, ТекстНаименование);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	НаименованиеБанка = "";
	...
	ЗаполненыНаименования = ЗначениеЗаполнено(НаименованиеОрганизации)
		И ЗначениеЗаполнено(НаименованиеБанка);

	Объект.Наименование = Строка(НаименованиеОрганизации)
		+ ?(ЗаполненыНаименования, " - ", "") + Строка(НаименованиеБанка);
	...
	Если НЕ ЭД.Data.Свойства().Получить("CryptoParameters") = Неопределено
		...
		Если Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			...
			Если ЭД.Data.CryptoParameters.Encrypted <> Неопределено Тогда
				НайтиСоздатьПрограммуКриптографии(ЭД.Data.CryptoParameters.CSPName, ЭД.Data.CryptoParameters.CSPType, ЭД.Data.CryptoParameters.SignAlgorithm, ЭД.Data.CryptoParameters.HashAlgorithm, ЭД.Data.CryptoParameters.Encrypted.EncryptAlgorithm);
			Иначе
				НайтиСоздатьПрограммуКриптографии(ЭД.Data.CryptoParameters.CSPName, ЭД.Data.CryptoParameters.CSPType, ЭД.Data.CryptoParameters.SignAlgorithm, ЭД.Data.CryptoParameters.HashAlgorithm);
			КонецЕсли;
			...
			Для Каждого ГруппаПодписи Из ЭД.Data.CryptoParameters.CustomerSignature.GroupSignatures Цикл
				...
				Для Каждого Сертификат Из ГруппаПодписи.Certificate Цикл
					...
					Сертификат = ЭлектронноеВзаимодействиеСлужебныйВызовСервера.НайтиСоздатьСертификатЭП(ДанныеСертификата, Объект.Организация, ЭД.Data.CryptoParameters.CSPName);
	...
	Для Каждого Документ Из ЭД.Data.Document Цикл
		...
		Если ЗначениеЗаполнено(ТекущийВидЭД)
				И АктуальныеВидыЭД.Найти(ТекущийВидЭД) <> Неопределено Тогда
			...
			Если НовЗапись.ИспользоватьЭП Тогда
				...
				ИначеЕсли Документ.Signed.RuleSignatures = "(0)" Тогда
					...
					КонецЦикла;
					ЭлектронноеВзаимодействиеСлужебный.ЗаполнитьТаблицуТребованийКПодписаниюПоДереву(МаршрутОбъект.ТаблицаТребований, ДеревоТребований);
					МаршрутОбъект.КлючАвтоматическойНастройки = Справочники.НастройкиОбменСБанками.КлючАвтоматическойНастройки(Объект, ТекущийВидЭД);
					МаршрутОбъект.Записать();
				...
				// Заполнение сложных правил подписания.
					...
					ИначеЕсли ЧислоШифров = 1 Тогда
						...
						КонецЕсли;
						МаршрутОбъект = НайтиСоздатьМаршрутПодписания(Объект, ТекущийВидЭД);
						МаршрутОбъект.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 - %2 (%3)", Объект.Организация, Объект.Банк, ТекущийВидЭД);
						МаршрутОбъект.СхемаПодписания = Перечисления.СхемыПодписанияЭД.ПоПравилам;
						МаршрутОбъект.Организация = Объект.Организация;
...
//
Процедура СохранитьДатуПоследнейОперации(НастройкаОбмена,
		ДатаПоследнейОперации) Экспорт
...
//
Процедура ОтправитьТестовыйЗапросВыпискиАсинхронныйОбмен(СтруктураПараметров,
		АдресХранилища) Экспорт
...
Процедура ПолучитьИнформациюОВнешнейКомпоненте(Параметры, Адрес) Экспорт
	...
	Иначе
		...
		Если РезультатИнфоФайл.Свойство("КодСостояния") Тогда
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru = 'Код ошибки: %3'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатИнфоФайл.СообщениеОбОшибке, Параметры.URLИнфоФайла, РезультатИнфоФайл.КодСостояния);
		Иначе
...
//
Процедура ПрочитатьСобытияЖурналаРегистрации(ОтборЖурналаНаКлиенте,
		АдресХранилища) Экспорт
	...
	Для Каждого ТекЭлемент Из СтруктураВидовЭД Цикл
		...
		ИначеЕсли ТекЭлемент.Ключ = Перечисления.ВидыЭДОбменСБанками.Письмо Тогда
			Для Каждого ОбъектДляВыгрузки Из ТекЭлемент.Значение Цикл
				СтруктураВозврата = Новый Структура;
				СформироватьПисьмо(
					ОбъектДляВыгрузки, НастройкиОбъектов.Получить(ОбъектДляВыгрузки).ПараметрыОбмена, СтруктураВозврата);
				Если ЗначениеЗаполнено(СтруктураВозврата) Тогда
					МассивСтруктурВозврата.Добавить(СтруктураВозврата);
				КонецЕсли;
			КонецЦикла;
...
Процедура СформироватьПлатежноеТребованиеAsync(СсылкаНаДокумент, ДеревоДанных,
	...
	ПолучательКБЕ = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.КБЕ");
  	ЗаполнитьСвойствоXDTO(Отправитель, "kbe", ПолучательКБЕ, , ТекстОшибки);
	ЗаполнитьСвойствоXDTO(ЭД, "Sender", Отправитель, Истина, ТекстОшибки);
	...
	ПлательщикКБЕ = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.КБЕ");
 	ЗаполнитьСвойствоXDTO(РеквизитыПлательщика, "KBE", ПлательщикКБЕ, , ТекстОшибки);	
	...
	ЗаполнитьСвойствоXDTO(БанкПлательщика, "CorrespAcc", СокрЛП(ПлательщикКоррСчетБанка), , ТекстОшибки);
	...
	ЗаполнитьСвойствоXDTO(РеквизитыПолучателя, "KBE", ПолучательКБЕ, , ТекстОшибки);
	...
	ЗаполнитьСвойствоXDTO(БанкПолучателя, "CorrespAcc", СокрЛП(ПолучательКоррСчет), , ТекстОшибки);
	...
	КодНазначенияПлатежа = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.КодНазначенияПлатежа");
  	ЗаполнитьСвойствоXDTO(ДанныеПлатежа, "PaymentCode", КодНазначенияПлатежа, Истина, ТекстОшибки);
...
Процедура ПрочитатьОперациюВыписки(ЭД, Operation, НовыйЭД)
	...
	Если Payer <> Неопределено Тогда
		...
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(Payer, "KBE"), "ПлательщикКБЕ");
	...
	Если Payee <> Неопределено Тогда
		...
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(Payee, "KBE"), "ПолучательКБЕ");
	...
	КодНазначенияПлатежа = ЗначениеСвойстваXDTO(PayData, "PaymentCode");
	Если КодНазначенияПлатежа <> Неопределено Тогда
		РеквизитыПлатежа.Добавить(КодНазначенияПлатежа, "КодНазначенияПлатежа");
	КонецЕсли;
	...
	ДатаВалютирования = ЗначениеСвойстваXDTO(PayData, "ValueDate");
	Если ДатаВалютирования <> Неопределено Тогда
		РеквизитыПлатежа.Добавить(ДатаВалютирования, "ДатаВалютирования");
	КонецЕсли;
	
	ДатаПолученияТовара = ЗначениеСвойстваXDTO(PayData, "ReceiptGoodsDate");
	Если ДатаПолученияТовара <> Неопределено Тогда
		РеквизитыПлатежа.Добавить(ДатаПолученияТовара, "ДатаПолученияТовара");
	КонецЕсли;
	...
	Если ЭтоМемориальныйОрдер Тогда
		...
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Recipient, "inn"), СторонаКлиента
			+ "ИНН");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Recipient, "kbe"), "ПлательщикКБЕ");
...
Процедура ПрочитатьПлатежноеПоручениеAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ПлательщикКБЕ", Payer.KBE);
	...
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ПолучательКБЕ", Payee.KBE);
	...
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КодНазначенияПлатежа", PayData.PaymentCode);
	...
	Иначе
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ЭтоПлатежВБюджет", Ложь);
	КонецЕсли;
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДатаВалютирования", PayData.ValueDate);
 	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДатаПолученияТовара", PayData.ReceiptGoodsDate);
КонецПроцедуры
...
Процедура ПрочитатьЗапросОСостоянииAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КБЕ", ЭД.Sender.kbe);
...
Процедура ПрочитатьИзвещениеОСостоянииAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИдентификаторКлиента", ЭД.Recipient.Customer.id);
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "НаименованиеКлиента", ЭД.Recipient.Customer.name);
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИНН", ЭД.Recipient.Customer.inn);
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КБЕ", ЭД.Recipient.Customer.kbe);
...
Процедура СформироватьПлатежноеПоручениеCML(СсылкаНаДокумент, ДеревоДанных,
		АдресФайлаВоВременномХранилище)
...
Процедура СформироватьПлатежноеПоручениеAsync(СсылкаНаДокумент, ДеревоДанных,
		АдресФайлаВоВременномХранилище)
	...
	ПлательщикКБЕ = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.КБЕ");
	//Если ПлательщикКБЕ <> "0" Тогда
  		ЗаполнитьСвойствоXDTO(Отправитель, "kbe", ПлательщикКБЕ, , ТекстОшибки);	
	//КонецЕсли;
	ЗаполнитьСвойствоXDTO(ЭД, "Sender", Отправитель, Истина, ТекстОшибки);
	...
	//Если Не ЗначениеЗаполнено(ПлательщикКБЕ) И ЭтоПеречислениеВБюджет Тогда
	//	ПлательщикКБЕ = "0";
	//КонецЕсли;
	ЗаполнитьСвойствоXDTO(РеквизитыПлательщика, "KBE", ПлательщикКБЕ, ЭтоПеречислениеВБюджет, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(РеквизитыПлательщика, "Account", ПлательщикРасчСчет, , ТекстОшибки);
	...
	ПолучательКБЕ = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.КБЕ");
	//Если Не ЗначениеЗаполнено(ПолучательКБЕ) И ЭтоПеречислениеВБюджет Тогда
	//	ПолучательКБЕ = "0";
	//КонецЕсли;
  	ЗаполнитьСвойствоXDTO(РеквизитыПолучателя, "KBE", ПолучательКБЕ, ЭтоПеречислениеВБюджет, ТекстОшибки);	
	ПолучательРасчСчет = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.РасчСчет");
	ЗаполнитьСвойствоXDTO(РеквизитыПолучателя, "Account", ПолучательРасчСчет, , ТекстОшибки);
	...
	КонецЕсли;
	ДатаВалютирования = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.ДатаВалютирования");
	ЗаполнитьСвойствоXDTO(ДанныеПлатежа, "ValueDate", ДатаВалютирования, , ТекстОшибки);
	
	ДатаПолученияТовара = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.ДатаПолученияТовара");
	ЗаполнитьСвойствоXDTO(ДанныеПлатежа, "ReceiptGoodsDate", ДатаПолученияТовара, , ТекстОшибки);
	
	ТаблицаСотрудников = ЭлектронноеВзаимодействие.ДанныеДерева(ДеревоДанных, "СписокСотрудников");
	ПереданСписокСотрудников = ЗначениеРеквизитаВДереве(ДеревоДанных, "ПереданСписокСотрудников");
	
	Если ПереданСписокСотрудников И ТаблицаСотрудников.Количество() > 0 Тогда
		СписокСотрудников = ОбъектТипаCML(Фабрика, "EmployeesType", ПространствоИмен);
		Для Каждого Сотрудник Из ТаблицаСотрудников Цикл
			ЗаписьОСотруднике = ОбъектТипаCML(Фабрика, "EmployeesType.Employee", ПространствоИмен);
			
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "FirstName", Сотрудник.Имя, , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "LastName", Сотрудник.Фамилия, , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "Patronymic", Сотрудник.Отчество, , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "INN", Сотрудник.ИНН, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "BirthDate", Сотрудник.ДатаРождения, , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "PaymentPeriod", Сотрудник.ПериодПлатежа, , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "Account", Сотрудник.РасчСчет, , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(ЗаписьОСотруднике, "Sum", Сотрудник.Сумма, Истина, ТекстОшибки);
			
			СписокСотрудников.Employee.Добавить(ЗаписьОСотруднике);
		КонецЦикла;
		
		ЗаполнитьСвойствоXDTO(ДанныеПлатежа, "Employees", СписокСотрудников, , ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьВБанк(НастройкаОбмена, Настройки, Данные, Результат,
		ТекстОшибки)
...
Процедура ОбработатьОшибкуПередачиПакета(СообщенияОбмена, ВидОперации,
		ТекстОшибки, ПакетОбмена = Неопределено)
...
Процедура ПрочитатьДокументИзПакета(НастройкаОбмена, ПакетОбменСБанками,
		Document, МассивВнешнихИдентификаторов, СоздаватьОперацииВыписки,
		ДанныеВозврата)
	...
	Если КодДокумента = "02" Тогда
		ПрочитатьИзвещениеОСостоянииДокумента(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, НовоеСообщениеОбмена, ДанныеВозврата)
	...
	ИначеЕсли КодДокумента = "15" Тогда
		ПрочитатьВыпискуБанкаАсинхронныйОбмен(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, СоздаватьОперацииВыписки, НовоеСообщениеОбмена);
	...
	ИначеЕсли КодДокумента = "20" Тогда
		ПрочитатьПодтверждениеОткрытияСчетовПоЗарплатномуПроекту(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, Document, НовоеСообщениеОбмена, ДанныеВозврата)
	ИначеЕсли КодДокумента = "22" Тогда
		ПрочитатьПодтверждениеЗачисленияДенежныхСредствНаСчетаСотрудников(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, Document, НовоеСообщениеОбмена, ДанныеВозврата)
	ИначеЕсли КодДокумента = "35" Тогда
		ПрочитатьВалютнуюВыпискуБанка(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, СоздаватьОперацииВыписки, НовоеСообщениеОбмена)
	КонецЕсли;
...
Процедура ПрочитатьВыпискуБанкаАсинхронныйОбмен(НастройкаОбмена,
	...
	СтруктураРазбораФайла = СформироватьДеревоРазбора(Перечисления.ВидыЭДОбменСБанками.ВыпискаБанка, ДвоичныеДанныеФайла);
...
Процедура ПроверитьДанныеПорученияНаПереводВалюты(СсылкаНаОбъект, ДеревоДанных, Ошибки)
	...
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИННКонтрагента)
...
Процедура ПроверитьРеквизитыОрганизации(ДеревоДанных, Ошибки)
	...
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИННОрганизации)
		ИЛИ СтрДлина(ИННОрганизации) <> 12 Тогда
		ТекстОшибки = НСтр("ru = 'должен состоять из 12 цифр.'");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(
			Ошибки, НайденнаяСтрокаРеквизита, ТекстОшибки);
	КонецЕсли;
...
Процедура ПроверитьРеквизитыКонтрагента(ДеревоДанных, Ошибки)
	...
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИННКонтрагента)
		ИЛИ СтрДлина(ИННКонтрагента) <> 12 Тогда
		ТекстОшибки = НСтр("ru = 'должен состоять из 12 цифр.'");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(
			Ошибки, НайденнаяСтрокаРеквизита, ТекстОшибки);
	КонецЕсли;
...
Процедура ПрочитатьИзвещениеОСостоянииДокумента(НастройкаОбмена,
		ПакетОбменСБанками, ДвоичныеДанныеФайла, НовоеСообщениеОбмена,
		ДанныеВозврата)
	...
	Если ЗначениеЗаполнено(СообщениеЭДО) Тогда
		...
		РеквизитыРодителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураРеквизитов.СообщениеРодитель, "Статус, ДатаИзмененияВнешнегоСтатуса");
		...
		ИначеЕсли НЕ StatusDocNotice.Result.Error = Неопределено Тогда
			...
			ИначеЕсли StatusDocNotice.Result.Error.Code = "2207" Тогда
				...
				РеквизитыДляИзменения.Вставить("ПричинаОтклонения", НСтр("ru = 'Документ отсутствует в информационной базе банка.'"));
				...
				Иначе // запрос состояния
					ОбменСБанкамиСлужебныйВызовСервера.ИзменитьСообщениеОбмена(РеквизитыСообщения.СообщениеРодитель, РеквизитыДляИзменения);
...
Процедура ПрочитатьИзвещениеОСостоянииПакета(НастройкаОбмена,
		ПакетОбменСБанками, ДвоичныеДанныеФайла, ДанныеВозврата)
	...
	Если НЕ StatusPacketNotice.Result.Error = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ОтправленныйПакетЭД) Тогда
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ОтправленныйПакетЭД, Перечисления.СтатусыПакетовЭД.Отменен, СтруктураЭД, СообщенияОбмена);
			Для Каждого СообщениеОбмена Из СообщенияОбмена Цикл
	...
	ИначеЕсли НЕ StatusPacketNotice.Result.Status = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ОтправленныйПакетОбменСБанками) Тогда
			Если StatusPacketNotice.Result.Status.Code = "01" Тогда
				...
				СообщенияОбмена = Новый Массив;
				ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ОтправленныйПакетОбменСБанками, Перечисления.СтатусыПакетовЭД.Доставлен, СтруктураЭД, СообщенияОбмена);
				Для Каждого СообщениеОбмена Из СообщенияОбмена Цикл
					ДокументУчета = ОбменСБанкамиСлужебныйВызовСервера.ДокументУчета(СообщениеОбмена);
					Если ЗначениеЗаполнено(ДокументУчета) Тогда
						СостояниеЭДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СообщениеОбмена, "Состояние");
...
Процедура ПолучитьТекстЗапросаПараметровОбмена(ТекстЗапроса)
	...
	|	И НЕ ВТ_СертификатыИзНастроекОбмена.СертификатЭП.ПометкаУдаления
	|	И НЕ ВТ_СертификатыИзНастроекОбмена.СертификатЭП.Отозван";
...
Функция ДанныеЗапросаВыпискиСбербанк(НастройкаОбмена, ДатаЗапроса, НомерСчета,
	...
	БИК = ЭлектронноеВзаимодействиеПереопределяемый.РеквизитыБанка(Банк).Код;
...
Функция ДайджестПлатежногоПорученияСбербанк(НастройкаОбмена, ДеревоРазбора,
		СтрокаОбъекта)
...
Функция ТранзакцияISO(ДеревоДанных, Сумма, ЛокальныйИнструмент, ТекстОшибки)
	...
	Если ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.БанкПосредник") = Истина Тогда
		КорСчетБанкаПосредника = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.БанкПосредник.КорСчет");
...
Процедура СформироватьПисьмоСбербанк(СсылкаНаОбъект, Идентификатор,
		НастройкаОбмена, АдресФайлаВоВременномХранилище)

	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена, "ИспользуетсяКриптография, ИмяВнешнегоМодуля, НаименованиеБанка, НомерПодразделенияБанка");
	ИспользуетсяПодпись = РеквизитыНастройкиОбмена.ИспользуетсяКриптография;

	РеквизитыПисьма = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаОбъект, "Дата, Номер, Организация, ТипПисьма, Тема, Текст, СчетОрганизации, ДляВалютногоКонтроля, Основание");
	...
	КороткийНомер = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыПисьма.Номер);
	...
	РеквизитИНН = ИмяПрикладногоРеквизита("ИНН");
	...
	Если РеквизитыПисьма.ДляВалютногоКонтроля Тогда
		Если ТипЗнч(РеквизитыПисьма.Основание) = Тип("ДокументСсылка.ПисьмоОбменСБанками") Тогда
			РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыПисьма.Основание, "Направление, Номер, ВходящийНомер, Дата, ВходящаяДата");
			...
			Иначе
				...
				НомерДокументаОснования = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыОснования.Номер);
...
//  СтруктураПараметров - Структура - данные, возвращаемые процедурой.
//
Процедура СформироватьПисьмо(СсылкаНаОбъект, НастройкиОбменаЭД,
		СтруктураПараметров)
