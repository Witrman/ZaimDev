...
Функция ПересчитатьИзВалютыВВалюту(Сумма, ВалютаНач, ВалютаКон, ПоКурсуНач, ПоКурсуКон, 
	...
	Если ПоКурсуНач     = 0  Тогда
		ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(НСтр("ru = 'При пересчете валютных сумм обнаружен нулевой курс валюты: '") + СокрЛП(ВалютаНач));
		ПризнакОшибки = Истина;
	КонецЕсли;
	
	Если ПоКурсуКон     = 0  Тогда
		ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(НСтр("ru = 'При пересчете валютных сумм обнаружен нулевой курс валюты: '") + СокрЛП(ВалютаКон));
		ПризнакОшибки = Истина;
	КонецЕсли;
	
	Если ПоКратностьНач = 0  Тогда
		ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(НСтр("ru = 'При пересчете валютных сумм обнаружена нулевая кратность валюты: '") + СокрЛП(ВалютаНач));
		ПризнакОшибки = Истина;
	КонецЕсли;
	
	Если ПоКратностьКон = 0  Тогда
		ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(НСтр("ru = 'При пересчете валютных сумм обнаружена нулевая кратность валюты: '") + СокрЛП(ВалютаКон));
		ПризнакОшибки = Истина;
	КонецЕсли;
...
Функция СформироватьНадписьЦеныИВалюта(ДанныеДокумента, ЭтоДокументРозничнойТорговли = Ложь) Экспорт
	...
	ЕстьВалютныйУчет 	= ОбщегоНазначенияБКВызовСервера.ИспользоватьВалютныйУчет();
	ИспользоватьТипыЦенНоменклатуры = ОбщегоНазначенияБКВызовСервера.ИспользоватьТипыЦенНоменклатуры();
	
	Если ДанныеДокумента.Свойство("ВалютаДокумента") И ЕстьВалютныйУчет И Не ЭтоДокументРозничнойТорговли Тогда 	
	...
	Если ДанныеДокумента.Свойство("ТипЦен") И ИспользоватьТипыЦенНоменклатуры Тогда		
	...
	Если ДанныеДокумента.Свойство("УчитыватьНДС") Тогда				
		Если ДанныеДокумента.УчитыватьНДС Тогда
			Если ДанныеДокумента.Свойство("ОтложитьПринятиеНДСКЗачету") Тогда
				Если  ДанныеДокумента.ОтложитьПринятиеНДСКЗачету Тогда
					ТекстОтложенногоНДС = НСтр("ru = ', отложен '");
				Иначе
					ТекстОтложенногоНДС = "";
					ТекстОтложенногоНДСССимволом = ", ";
				КонецЕсли;
			КонецЕсли;
			...
			Если ДанныеДокумента.Свойство("СуммаВключаетНДС") Тогда
				ТекстВключениеНДС = ?(ДанныеДокумента.СуммаВключаетНДС,
					...
					НСтр("ru = 'сверху'"));
				ТекстНадписи = ТекстНадписи + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'НДС (%1%2), '"), ТекстВключениеНДС, ТекстОтложенногоНДС);
				Иначе
					ТекстНадписи = ТекстНадписи + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'НДС %1 '"), ТекстОтложенногоНДС);
			КонецЕсли;						
		Иначе
	...
	ТекстНадписи = ?(ПустаяСтрока(ТекстНадписи), "", Сред(ТекстНадписи, 1, СтрДлина(ТекстНадписи) - 2));
