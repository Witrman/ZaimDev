...
Функция ПолучитьРеквизитыКассира(ПараметрыФормы) Экспорт
	...
	ДанныеКассира = ОбщегоНазначенияБКВызовСервера.ДанныеФизЛицаТекущегоПользователя(ПараметрыФормы.Организация);
    Результат = Новый Структура("Наименование, ИНН");
	Результат.Наименование = ДанныеКассира.Представление;
...
Функция ЗначениеСтавкиНДС(СтавкаНДС) Экспорт
	...
	Возврат СтавкаНДСПоЗначению(СтавкаНДС);
...
Функция ЕдиницаИзмеренияПоКоду(КодЕдиницыИзмерения) Экспорт
	...
	Возврат Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду(КодЕдиницыИзмерения);
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, НомерСекции, ВерсияФФД) Экспорт
	...
	ОписаниеОшибки = "";
	ВерсияФФД = "1.0.0.0";	
	Результат = МенеджерОбъекта.СобратьДанныеЧеков(
		ДокументСсылка,
		НомерСекции,
		ВерсияФФД,
		ОписаниеОшибки);
