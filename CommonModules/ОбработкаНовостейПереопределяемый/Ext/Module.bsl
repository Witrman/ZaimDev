...
Процедура ДополнительноОбработатьНовостьПослеПолученияПослеЗаписи(НовостьСсылка) Экспорт
	...
	ПродуктОснование         = "AccountingKz"; // Продукт, на основании которого создан этот отраслевой продукт.
	...
	ПродуктыОснования.Добавить("AccountingKz");
	ПродуктыОснования.Добавить("AccountingBase");

	// Это отраслевой продукт, созданный на основании БК?
...
Процедура ДополнительноОбработатьТаблицуКонтекстныхНовостей(
	...
	Если ИдентификаторМетаданных = "ИнформационнаяПанель" Тогда
		...
		ЛентаНовостей_ЧтоНового = ИнтернетПоддержкаПользователейБК.ЛентаНовостейЧтоНового();
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиТекущемуПользователю() Тогда
			...
			ПараметрыПолученияНовостей.Вставить("КоличествоНовостей", ИнтернетПоддержкаПользователейБК.КоличествоКонтекстныхНовостей());
			ПараметрыПолученияНовостей.Вставить("ПорядокСортировки",  ИнтернетПоддержкаПользователейБК.ПорядокКонтекстныхНовостей());
...
Процедура ДополнительноОбработатьФормуПросмотраНовостейПриСозданииНаСервере(
	...
	ИнтернетПоддержкаПользователейБК.СнятиеВыделенияЭлементовДерева(Форма.Папки);
