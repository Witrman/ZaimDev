...
Функция РазобратьКодМаркировки(Знач ДанныеДляРазбора, ВидыПродукции = Неопределено, ПримечаниеКРезультатуРазбора = Неопределено, Знач Настройки = Неопределено, ПроверятьАлфавитЭлементов = Истина) Экспорт
	...
	Если Настройки = Неопределено Тогда
		Настройки = РазборКодаМаркировкиИСМПТКСлужебныйПовтИсп.НастройкиРазбораКодаМаркировки();
	КонецЕсли;
	
	Возврат РазборКодаМаркировкиИСМПТКСлужебныйКлиентСервер.РазобратьКодМаркировки(
		ДанныеДляРазбора, ВидыПродукции, ПримечаниеКРезультатуРазбора, Настройки, ПроверятьАлфавитЭлементов, РазборКодаМаркировкиИСМПТКСлужебный);
...
Функция ОбщиеНастройкиРазбораКодаМаркировки(ТолькоСервер = Ложь, ВидыПродукции = Неопределено, ТолькоУчитываемыеВидыПродукции = Истина, ВалидироватьШтрихкодаGS1БезРазделителей = Истина) Экспорт
	...
	НастройкиРазбораКодаМаркировки.Алфавит                                 = РазборКодаМаркировкиИСМПТКСлужебныйКлиентСервер.ДопустимыеСимволыВКодеМаркировки();
	НастройкиРазбораКодаМаркировки.ТолькоСервер                            = ТолькоСервер;
	
	УчитываемыеВидыМаркируемойПродукции = РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.УчитываемыеВидыМаркируемойПродукции();
	...
	Для Каждого ВидПродукции Из НастройкиРазбораКодаМаркировки.ДоступныеВидыПродукции Цикл
		...
		Если ВидПродукции = Перечисления.ВидыПродукцииИСМПТК.Алкогольная Тогда
		...
		Иначе
			...
			Если МодулиПодсистемыИСМП = Неопределено Тогда
				...
				//Если ОбщегоНазначения.ПодсистемаСуществует("МаркировкаИСМПТК") Тогда
					МодульНастройки = "РазборКодаМаркировкиДополнительныйИСМПТКСлужебный";
					МодульРазбораКМ = "РазборКодаМаркировкиДополнительныйИСМПТКСлужебныйКлиентСервер";
	...
	Для Каждого НастройкаОписанияКодаМаркировки Из ДанныеОписанийКодаМаркировки.ОписанияКодовМаркировки Цикл
		...
		Для Каждого ОписаниеЭлементовШаблонаКодаМаркировки Из НастройкаОписанияКодаМаркировки.СписокШаблонов Цикл
			...
			Если ОписаниеТекущегоШаблона = Неопределено Тогда
				...
				ПозицииЭлементовКодаМаркировки = РазборКодаМаркировкиИСМПТКСлужебныйКлиентСервер.ПозицииЭлементовВШаблонеКодаМаркировки(
