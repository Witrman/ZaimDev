...
Процедура ЗначениеДоступаНачалоВыбораЗавершение(Форма)
	...
	ИначеЕсли ТипЗнч(Форма.ТекущийВидДоступа) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
		...
		ПараметрыФормы.Вставить("РежимРаботы"					, "ВыборЗначенияДоступа");
		ОткрытьФорму("ОбщаяФорма.ФормаВыбораСтруктурногоПодразделения", ПараметрыФормы, Элемент);
		Возврат;
	КонецЕсли;
