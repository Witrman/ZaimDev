...
Функция СтрокаСоединенияДляЗапросаИнформацииОКонтактах(ВключатьEmail, ИмяСправочника)
	...
	Иначе
		...
		|			ПО Таблица.Ссылка = ТаблицаКонтактнаяИнформация.Объект
...
Функция ТекстЗапросаНаПолучениеИнформацииОКонтактах(ВключатьEmail)
	...
	// БК3. В метод ПолучитьСтрокуПроАдресДляЗапросаИнформацииОКонтактах
	// добавлена передача параметра Истина, т.к. без передачи этого параметра
	// в форме исходящего письма при открытии формы выбора получателя
	// возникала модульная ошибка: Поле объекта не обнаружено (АдресЭП).
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ",&ПолеАдресЭлектроннойПочты",  ПолучитьСтрокуПроАдресДляЗапросаИнформацииОКонтактах(ВключатьEmail,"Пользователи",Истина));
...
Функция СформироватьТекстЗапросаДляПоискаПоEmail(ПоискПоСписку, ИтогиПоEmail = Ложь)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТаблицаКонтактнойИнформации.Объект КАК Контакт,
	|	ВЫРАЗИТЬ(ТаблицаКонтактнойИнформации.Представление КАК СТРОКА(100)) КАК Представление,
	|	"""" КАК НаименованиеВладельца,
	|	ТаблицаКонтактнойИнформации.Объект.Наименование КАК Наименование
	|ИЗ
	|	РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнойИнформации
	|ГДЕ
	|	ТаблицаКонтактнойИнформации.Объект ССЫЛКА Справочник.Пользователи
	|	И ВЫРАЗИТЬ(ТаблицаКонтактнойИнформации.Представление КАК СТРОКА(100)) = &Адрес
	|	И (НЕ ТаблицаКонтактнойИнформации.Объект.ПометкаУдаления)
	...
	Для каждого ЭлементМассиваОписания Из МассивОписанияТиповКонтактов Цикл
		...
		Иначе
			...
			|
			|ВЫБРАТЬ
			|	ТаблицаКонтактнойИнформации.Объект,
			|	ВЫРАЗИТЬ(ТаблицаКонтактнойИнформации.Представление КАК СТРОКА(100)),
			|	" + ?(ЭлементМассиваОписания.ЕстьВладелец," ТаблицаКонтактнойИнформации.Объект.Владелец.Наименование","""""") + ",
			|	ТаблицаКонтактнойИнформации.Объект.Наименование
			|ИЗ
			|	РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнойИнформации
			|ГДЕ
			|	ТаблицаКонтактнойИнформации.Объект ССЫЛКА Справочник." + ЭлементМассиваОписания.Имя + "
			|	И ВЫРАЗИТЬ(ТаблицаКонтактнойИнформации.Представление КАК СТРОКА(100)) = &Адрес
			|   И (НЕ ТаблицаКонтактнойИнформации.Объект.ПометкаУдаления)
			|	И ТаблицаКонтактнойИнформации.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)
			|  "+?(ЭлементМассиваОписания.Иерархический," И (НЕ ТаблицаКонтактнойИнформации.Объект.ЭтоГруппа)","");
...
Функция НайтиКонтактыСАдресами(СтрокаПоиска) Экспорт
	...
	|	"""" КАК НаименованиеВладельца,
	|	ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) КАК Представление
	...
	|			И (ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) <> """")
	|ГДЕ
	|	НЕ Таблица.ПометкаУдаления
	|	И (Таблица.Наименование ПОДОБНО &ВведеннаяСтрока
	|			ИЛИ ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) ПОДОБНО &ВведеннаяСтрока)";
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
		|		ПО (ТаблицаКонтактнаяИнформация.Объект = СправочникКонтакт.Ссылка)
		|			И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты) И (ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) <> """"))
		|ГДЕ
		|	НЕ СправочникКонтакт.ПометкаУдаления 
		//|			ИЛИ ТаблицаКонтактнаяИнформация.ДоменноеИмяСервера ПОДОБНО &ВведеннаяСтрока
		|			И (ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) ПОДОБНО &ВведеннаяСтрока 
		|" + УсловиеПолейВводаПоСтроке + ") 
		|";
...
Функция КонтактыПоДоменномуАдресу(Знач ИмяДомена)
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если ОписаниеКонтакта.ИскатьПоДомену Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
			|ВЫБРАТЬ &ПостфиксВыбрать, 
			|	ТаблицаКонтактнаяИнформация.Объект КАК Контакт,
			|	&ПолеНаименование                   КАК Наименование,
			|	ВЫРАЗИТЬ(ТаблицаКонтактнаяИнформация.Представление КАК СТРОКА(100)) КАК Представление,
			|	&ПолеНаименованиеВладельца          КАК НаименованиеВладельца
			|ИЗ
			|	&ИмяТаблицы КАК ТаблицаКонтактнаяИнформация
			|ГДЕ
			|	&УсловиеТипКонтакта
			|	И ТаблицаКонтактнаяИнформация.Представление ПОДОБНО &СтрокаПоиска
			|	И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))";
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПостфиксВыбрать,", ?(ПерваяТаблица, "РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ ", ""));
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеНаименованиеВладельца", ?(ОписаниеКонтакта.ЕстьВладелец,"ТаблицаКонтактнаяИнформация.Объект.Владелец.Наименование ",""""""));
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеНаименование", "ТаблицаКонтактнаяИнформация.Объект.Наименование");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяТаблицы", "РегистрСведений.КонтактнаяИнформация");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеТипКонтакта", "ТаблицаКонтактнаяИнформация.Объект ССЫЛКА Справочник." + ОписаниеКонтакта.Имя + " ");
...
Функция ПолучитьТекстЗапросаПоискКонтактовПоСтрокеСEmail()
	...
	|	Справочник.Пользователи КАК ТаблицаСправочника
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
	|		ПО (ТаблицаКонтактнаяИнформация.Объект = ТаблицаСправочника.Ссылка)
	|			И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
	|ГДЕ
	|	ТаблицаСправочника.Ссылка В(&МассивСсылок)
	|	И (НЕ ТаблицаСправочника.ПометкаУдаления)";
	...
	Для каждого ЭлементМассиваОписания Из МассивОписанияТиповКонтактов Цикл
		...
		|	ВЫРАЗИТЬ(ТаблицаКонтактнаяИнформация.Представление КАК СТРОКА(100))
		|ИЗ
...
Функция АдресаЭлектроннойПочтыКонтактов(Контакты, ГруппаПолучателей = "") Экспорт
	...
	ТекстЗапроса = 
       ...
       |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
       |		ПО (ТаблицаКонтактнаяИнформация.Объект = ТаблицаКонтакт.Ссылка)
       |			И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
       |ГДЕ
	...
	|	КонтактыАдреса.АдресЭП КАК Адрес
	|ИЗ
	|	КонтактыАдреса КАК КонтактыАдреса
	|УПОРЯДОЧИТЬ ПО
	|	Контакт
...
Функция НайтиКонтактыПоТелефону(Знач Телефон, НайденныеКонтакты) Экспорт
	...
	|ГДЕ
	|	ТаблицаКонтактнойИнформации.Объект ССЫЛКА Справочник.Пользователи
	|	И ПОДСТРОКА(ТаблицаКонтактнойИнформации.Представление, 1, 100) = &Телефон
...
Функция НайтиКонтактыПоНачалуНаименования(Знач СтрокаПоиска) Экспорт
	...
	|	Таблица.Наименование КАК Наименование,
	|	"""" КАК НаименованиеВладельца,
	|	ТаблицаКонтактнаяИнформация.Представление КАК Представление
	|ИЗ
	|	Справочник.Пользователи КАК Таблица
	|ГДЕ
