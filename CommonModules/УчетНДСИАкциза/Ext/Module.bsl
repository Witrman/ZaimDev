...
Процедура СформироватьДвиженияНачисленияНДС(ТаблицаНДС, Реквизиты, Движения, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНДС Цикл
		...
		Проводка.Содержание   = НСтр("ru = 'Отложенное принятие НДС к зачету'");
		...
		Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
			...
			ПроводкаНУ.Содержание   = НСтр("ru = 'Отложенное принятие НДС к зачету'");
...
Процедура СформироватьДвиженияСписаниеТоваровНДС(ТаблицаСписанныеТоварыНДС, Реквизиты, Движения, Отказ) Экспорт
	...
	Если Реквизиты.КорректироватьНДС Тогда
		...
		Для Каждого СтрокаКорректировки Из ТаблицаСписанныеТоварыНДС Цикл		
			...
			ПроводкаБУ.Содержание  = НСтр("ru = 'Корректировка суммы НДС '");		
			...
			Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
				...
				ПроводкаНУ.Содержание  = НСтр("ru = 'Корректировка суммы НДС '");		
				ПроцедурыНалоговогоУчета.ВидУчетаНУ(ПроводкаНУ,  Реквизиты.ВидУчетаНУ);
		...
		Если  ТабличнаяЧасть <> Неопределено Тогда
			...
			Для Каждого СтрокаОснования Из ДокОснование[ИмяТабЧастиОснование] Цикл
				...
				Иначе 	//общий случай
					...
					Если ВидДокументаОснования = "РеализацияУслугПоПереработке" И ВидОбъекта = "СчетФактураВыданный"  Тогда																	
...
Функция ПолучитьТекстСчетаФактуры(ДанныеСчетаФактуры, ВидСчетаФактуры) Экспорт
	...
	СчетФактура = ?(ДанныеСчетаФактуры.Свойство("Ссылка"), ДанныеСчетаФактуры.Ссылка, Неопределено);
	
	Если НЕ ЗначениеЗаполнено(СчетФактура) Тогда
		Возврат НСтр("ru='Ввести счет-фактуру'");
	Иначе
		ДополнительныйТекст = "";
		...
		Иначе
			Если НЕ ДанныеСчетаФактуры.Проведен Тогда
				ДополнительныйТекст = НСтр("ru=' (не проведен)'");
			КонецЕсли;	
		КонецЕсли;				
		
		Если  ВидСчетаФактуры = "СчетФактураПолученный" Тогда
			Если  НЕ ЗначениеЗаполнено(ДанныеСчетаФактуры.НомерВходящегоДокумента) И НЕ ЗначениеЗаполнено(ДанныеСчетаФактуры.ДатаВходящегоДокумента) Тогда
				Возврат Строка(СчетФактура) + ДополнительныйТекст;
			Иначе
				Текст = Строка(СчетФактура) + ?(НЕ ЗначениеЗаполнено(ДанныеСчетаФактуры.НомерВходящегоДокумента), "" , " (№" + ДанныеСчетаФактуры.НомерВходящегоДокумента)
				+ ?(НЕ ЗначениеЗаполнено(ДанныеСчетаФактуры.ДатаВходящегоДокумента), "" , " от " + Формат(ДанныеСчетаФактуры.ДатаВходящегоДокумента,"ДФ=dd.MM.yyyy") + ")") + ДополнительныйТекст;		              		
				Возврат Текст;
			КонецЕсли;
		Иначе
			Текст = Строка(СчетФактура)+ ДополнительныйТекст;		              		
			Возврат Текст;
		КонецЕсли;
...
Процедура СформироватьДвиженияГТДИмпортНДС(ТаблицаНДС, Реквизиты, Движения, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНДС Цикл
		...
		Если Реквизиты.ВариантОтражения = 1 И  НЕ СтрокаТаблицы.ВидНалогаНДС = Перечисления.ВидыНДС.НДСМетодомЗачета Тогда
			...
			Проводка.Организация  = Реквизиты.Организация;
			Проводка.Сумма        = СтрокаТаблицы.СуммаНДС;
			Проводка.Содержание   =  НСтр("ru = 'Начисление НДС по импорту'");
			...
			Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
				...
				ПроводкаНУ.Содержание   =  НСтр("ru = 'Начисление НДС по импорту'");
	...
	//	КонецЕсли;
		...
		Если СтрокаТаблицы.ВидНалогаНДС = Перечисления.ВидыНДС.НДСМетодомЗачета Тогда
			...
			Проводка.Сумма       = СтрокаТаблицы.СуммаНДС;
			Проводка.Содержание  = НСтр("ru = 'Зачет '") + СтрокаТаблицы.Содержание;
			...
			Если Реквизиты.НеобходимостьОтраженияВНУ Тогда
				...
				ПроводкаНУ.Организация = Реквизиты.Организация;
				ПроводкаНУ.Сумма       = СтрокаТаблицы.СуммаНДС;
				ПроводкаНУ.Содержание  = НСтр("ru = 'Зачет '") + СтрокаТаблицы.Содержание;
...
Процедура СформироватьДвиженияЗаявлениеОВвозеТоваровНДС(ТаблицаНДС, Реквизиты, Движения, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНДС Цикл
		...
		Если СтрокаТаблицы.ВидНалогаНДС = Перечисления.ВидыНДС.НДСМетодомЗачета Тогда
			Содержание = НСтр("ru = 'НДС методом зачета начислен: '") + СокрЛП(Реквизиты.Комментарий);
		Иначе	
			Содержание = НСтр("ru = 'НДС по импорту: '") + СокрЛП(Реквизиты.Комментарий);			
		КонецЕсли; 
		...
		Если СтрокаТаблицы.ВидНалогаНДС = Перечисления.ВидыНДС.НДСМетодомЗачета Тогда
			...
			Проводка.Сумма       = СтрокаТаблицы.СуммаНДС;
			Проводка.Содержание  = НСтр("ru = 'НДС методом зачета принят к зачету: '") + СокрЛП(Реквизиты.Комментарий);		
			...
			Если Реквизиты.НеобходимостьОтраженияВНУ И ЗначениеЗаполнено(СтрокаТаблицы.КорСчетНУ) Тогда
				...
				ПроводкаНУ.Организация = Реквизиты.Организация;
				ПроводкаНУ.Сумма       = СтрокаТаблицы.СуммаНДС;
				ПроводкаНУ.Содержание  = НСтр("ru = 'НДС методом зачета принят к зачету: '") + СокрЛП(Реквизиты.Комментарий);					
...
Функция ПроверитьСоответствиеРеквизитовСчетаФактурыИДокументовОснований(ДокументСчетФактура, ДокументОснование, Отказ)
	...
	Если ЕстьРеквизитОрганизация И НЕ ДокументСчетФактура.Организация = ДокументОснование.Организация Тогда
		...
		ОрганизацияОснования    = НСтр("ru = '" + СокрЛП(ДокументОснование.Организация) + "'");
		ОрганизацияСчетаФактуры = НСтр("ru = '" + СокрЛП(ДокументСчетФактура.Организация) + "'");
	...
	Если ЕстьРеквизитСтруктурноеПодразделение И НЕ ДокументСчетФактура.СтруктурноеПодразделение = ДокументОснование.СтруктурноеПодразделение Тогда
		...
		СтруктурноеПодразделениеОснования    = НСтр("ru = '" + СокрЛП(?(ЗначениеЗаполнено(ДокументОснование.СтруктурноеПодразделение), ДокументОснование.СтруктурноеПодразделение, "головное подразделение")) + "'");
		СтруктурноеПодразделениеСчетаФактуры = НСтр("ru = '" + СокрЛП(?(ЗначениеЗаполнено(ДокументСчетФактура.СтруктурноеПодразделение), ДокументСчетФактура.СтруктурноеПодразделение, "головное подразделение")) + "'");
	...
	Если ЕстьРеквизитКонтрагент И НЕ ДокументСчетФактура.Контрагент = ДокументОснование.Контрагент Тогда
		...
		КонтрагентОснования    = НСтр("ru = '" + СокрЛП(ДокументОснование.Контрагент) + "'");
		КонтрагентСчетаФактуры = НСтр("ru = '" + СокрЛП(ДокументСчетФактура.Контрагент) + "'");
	...
	Если ЕстьРеквизитДоговор И НЕ ДокументСчетФактура.ДоговорКонтрагента = ДокументОснование.ДоговорКонтрагента Тогда
		...
		ДоговорОснования    = НСтр("ru = '" + СокрЛП(ДокументОснование.ДоговорКонтрагента) + "'");
		ДоговорСчетаФактуры = НСтр("ru = '" + СокрЛП(ДокументСчетФактура.ДоговорКонтрагента) + "'");
	...
	Если ЕстьРеквизитВалютаДокумента И НЕ ДокументСчетФактура.ВалютаДокумента = ДокументОснование.ВалютаДокумента Тогда
		...
		ВалютаОснования    = НСтр("ru = '" + СокрЛП(ДокументОснование.ВалютаДокумента) + "'");
		ВалютаСчетаФактуры = НСтр("ru = '" + СокрЛП(ДокументСчетФактура.ВалютаДокумента) + "'");
