...
Функция СтрокаGS1ВДату(Знач Значение, Знач ОписаниеТиповДаты, ТекстОшибки = "")
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		...
		ТекстОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, Значение);
		Возврат Неопределено;
	КонецЕсли;
	...
	Иначе
		...
		Для каждого СтрокаИдентификатора Из ИспользуемыеИдентификаторы Цикл
			...
			Если ЗначениеЗаполнено(СвойстваИдентификатора.ДополнительныйПараметрИмя) Тогда
				ПредставлениеИдентификатора = ПредставлениеИдентификатора + ПриведенноеКДлинеЗначение(ДопПараметр, СвойстваИдентификатора.ДлинаДопПараметра);
			КонецЕсли;
			Если УстанавливатьСкобки Тогда
			...
			ИначеЕсли ТипЗнч(Значение) = Тип("Число") Тогда
				...
				Если ЗначениеЗаполнено(СвойстваИдентификатора.ДополнительныйПараметрИмя)
				   И СвойстваИдентификатора.ДополнительныйПараметрИмя = ВРЕГ("ЧислоЗнаковПослеЗапятой") Тогда
					...
					ФорматнаяСтрокаЧисла = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ФорматнаяСтрокаЧисла, ДопПараметр);
				КонецЕсли;
...
Функция ПараметрыШтрихкодаSSCC(Знач Штрихкод) Экспорт
	...
	Если Не ОшибкаЧтения Тогда
		...
		Если ДлинаСтроки <> 18 Тогда
			...
			ТекстОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ДлинаСтроки, 18);
...
Функция ПараметрыШтрихкодаGS1СоСкобками(Знач Штрихкод, ПараметрыШтрихкода = Неопределено) Экспорт
	...
	Если ПараметрыШтрихкода = Неопределено Тогда
		...
		Если НомерПозиции <> 0 Тогда
			...
			ПараметрыШтрихкода.ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ПараметрыШтрихкода.ТекстОшибки, КлючИдентификатораSSCC);
	...
	Если ПозицияНачало <> 1
	 ИЛИ ПозицияКонец = 0 Тогда
		...
		ПараметрыШтрихкода.ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ПараметрыШтрихкода.ТекстОшибки, Штрихкод);
	...
	Если СвойстваИдентификатора = Неопределено Тогда
		...
		ПараметрыШтрихкода.ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ПараметрыШтрихкода.ТекстОшибки, КлючИдентификатора + ЗначениеДопПараметраСтрока);
	...
	Если ЗначениеДопПараметраСтрока <> ""
		...
		ПараметрыШтрихкода.ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ПараметрыШтрихкода.ТекстОшибки, ЗначениеДопПараметраСтрока, КлючИдентификатора);
	...
	Если СвойстваИдентификатора.ДлинаКода <> 0
		...
		ПараметрыШтрихкода.ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ПараметрыШтрихкода.ТекстОшибки, СтроковоеЗначениеПараметра, КлючИдентификатора + ЗначениеДопПараметраСтрока, СвойстваИдентификатора.ДлинаКода);
...
Функция ПараметрыШтрихкодаGS1(Знач Штрихкод, ПараметрыШтрихкода = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ПараметрыШтрихкода.ТекстОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
...
Функция ПараметрЗначенияИдентификатораПрименения(СвойстваИдентификатораПрименения, КлючИдентификатора, ДополнительныйПараметр, ЗначениеСтрокой, ТекстОшибки = "")
	...
	ИначеЕсли ТипЗначенияИдентификатора.СодержитТип(Тип("Число")) Тогда
		Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначениеСтрокой) Тогда
			...
			ТекстОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ЗначениеСтрокой);
	...
	Если НЕ ЗначениеЗаполнено(ЗначениеПараметра)
		...
		ТекстОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, СвойстваИдентификатораПрименения.ПредставлениеИдентификатора);
		ПараметрЗначенияИдентификатора = Неопределено;
		Возврат ПараметрЗначенияИдентификатора;
...
Функция ПараметрыШтрихкодаCode128(Знач Штрихкод) Экспорт
	...
	Иначе
		...
		ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрДлина(Штрихкод), НСтр("ru = '18,22 или 26, как установлено в формате'"));
		ПараметрыШтрихкода.Вставить("Результат", Неопределено);
		ПараметрыШтрихкода.Вставить("ТекстОшибки", ТекстОшибки);
	КонецЕсли;
...
Функция ПараметрыШтрихкодаCode128Формат1(Штрихкод)
	...
	Если ДлинаСтроки <> 22 Тогда
		...
		ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ДлинаСтроки, 22);
	...
	Иначе
		...
		Результат.Вставить("НомерПоПорядку", 		   СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Сред(Штрихкод, 19, 4)));
...
Функция ПараметрыШтрихкодаCode128Формат2(Штрихкод)
	...
	Если ДлинаСтроки <> 26 Тогда
		ТекстОшибки  = НСтр("ru = 'Неверный формат штрихкода Code-128: общая длина штрихкода равна %1 символам, вместо %2'");
		ТекстОшибки  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ДлинаСтроки, 26);
...
Функция ПараметрыШтрихкодаCode128Формат3(Штрихкод)
	...
	Иначе
		...
		Результат.Вставить("ИдентификаторОрганизации", Сред(Штрихкод, 1, 12));
		Результат.Вставить("НомерПоПорядку", 		   СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Сред(Штрихкод, 13, 6)));
...
Процедура РазобратьСтрокуШтрихкодаGS1СоСкобками(Штрихкод, РезультатРазбора, КодыGS1) Экспорт
	...
	Пока НомерСимвола <= ДлинаШтрихкода Цикл
		...
		Если Сред(Штрихкод, НомерСимвола, 1) <> "(" Тогда
			РезультатРазбора.ОписаниеОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
	...
	Пока НомерСимвола <= ДлинаШтрихкода Цикл
		...
		Если ОписаниеКода = Неопределено Тогда
			...
			Если ОписаниеКода = Неопределено Тогда
				...
				ОписаниеКода = КодыGS1[ИдентификаторПрименения];
				Если ОписаниеКода = Неопределено Тогда
					РезультатРазбора.ОписаниеОшибки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неизвестный идентификатор применения(AI) %1.';
...
Функция РазобратьКодМаркировки(ДанныеДляРазбораСтруктура, ВидыПродукции, ПримечаниеКРезультатуРазбора, Настройки, ПроверятьАлфавитЭлементов, МодульКонтекста) Экспорт
	...
	Если РезультатПоФильтру.Количество() = 0 Тогда
		...
		Если ЭтоНеформализованныйКодМаркировки(ПараметрыРазбораКодаМаркировки, Настройки, ДанныеРезультата, Новый ФиксированныйМассив(Результат)) Тогда
			РезультатПоФильтру = РезультатРазбораКодаМаркировкиПоФильтру(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеРезультата), ФильтрПоВидуПродукции);
		КонецЕсли;
		...
		Для Каждого ЧастьШтрихкода Из ЭлементСтека.ЧастиШтрихкода Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(РезультатРазбора.ЗначениеПеременное) И Не РезультатРазбора.ЗначениеПеременноеОбработано Тогда
				...
				Для ВариантДлины = 1 По МаксимальнаяДлинаЗначенияПеременнойДлины Цикл
					...
					РезультатРазбораПродолжение.Вставить("ДанныеШтрихкода", 	   СкопироватьРекурсивно(РезультатРазбора.ДанныеШтрихкода));
...
Функция ЭтоНеФормализованныйКодМаркировкиГрупповойУпаковкиТабака(ПараметрыРазбораКодаМаркировки, Настройки, ДанныеРезультата, РезультатБезФильтра)
	...
	Для Каждого ШаблонКодаМаркировки Из ШаблоныКодаМаркировкиСХвостом Цикл
		...
		ДанныеРезультата = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.КодМаркировкиСоответствуетШаблону(ПараметрыРазбораКодаМаркировки, Настройки, ШаблонКодаМаркировки);
...
Функция ЭтоНеФормализованныйКодМаркировкиЛогистическойУпаковкиGS1128(ПараметрыРазбораКодаМаркировки, Настройки, ДанныеРезультата, РезультатБезФильтра)
	...
	Если ПараметрыРазбораКодаМаркировки.НачинаетсяСоСкобки Тогда
		...
		Иначе
			...
			ИначеЕсли ИдентификаторGTIN = "(02)" Тогда
				РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(РезультатРазбора.ОписаниеОшибки);
			КонецЕсли;
	...
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		ДанныеРезультата.ВидыУпаковокПоВидамПродукции[ВидПродукции] = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТипШтрихкодаИВидУпаковки.ВидУпаковки);
	КонецЦикла;
...
Функция ЭтоНеФормализованныйКодМаркировкиЛогистическойУпаковкиТабакаGS1128(ПараметрыРазбораКодаМаркировки, Настройки, ДанныеРезультата)
	...
	Если ЗначениеЗаполнено(СоставКодаМаркировки.GTIN) Тогда
		СоставКодаМаркировки.EAN = ШтрихкодEANИзGTIN(СоставКодаМаркировки.GTIN);
	КонецЕсли;
...
Функция РезультатРазбораGS1МожетБытьОбработан(ДанныеДляРазбора, РезультатРазбора, ПримечаниеКРезультатуРазбора, ИдентификаторыОшибок, МодульКонтекста, СодержитРазделительGS, КодМаркировки) Экспорт
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
...
#Область ПроверкаНаличияИВидаМаркируемойПродукции
...
Функция ДопустимВидПродукции(ПараметрыСканирования, ВидПродукцииИС) Экспорт
	...
	Если ТипЗнч(ВидПродукцииИС) = Тип("Массив") Тогда   //Передается либо массив данных
		Для Каждого ТГДляПроверки Из ВидПродукцииИС Цикл
			Если ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ВидПродукцииИС) <> Неопределено Тогда
				ВидДопустим = Истина; //Хотя бы один допустим - этого хватит
				Прервать;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ПараметрыСканирования.ДопустимыеВидыПродукции.Найти(ВидПродукцииИС) <> Неопределено Тогда   //либо конкретное значение для проверки
		ВидДопустим = Истина; //Хотя бы один допустим - этого хватит
	КонецЕсли;
...
Процедура ЗаписьЖурналаРегистрацииОшибкаУдаленияФайла(ИмяФайла, ИнформацияОбОшибке)
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.СоздатьЗаписьЖурналаРегистрации("РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УдалитьВременныеФайлы", "Ошибка", , , Комментарий);
...
#Область ТекстСообщений

Функция ТекстСообщенияТребуетсяУказатьОрганизациюПриЗапросеДанныхУпаковокССервера() Экспорт
	
	Возврат НСтр("ru = 'Включена опция автоматической проверки состояния кодов маркировки по данным сервера. Для получения токена сеанса требуется указать Организацию!'");
	
КонецФункции

Функция ТекстСообщенияНевозможноВыполнитьПроверкуСтатусаКМ_ВФоне() Экспорт
	
	Возврат НСтр("ru = 'Не удалось получить токен сеанса для автоматической проверки состояния кодов маркировки по данным сервера. Статус кодов в документе неопределен и может быть ошибочным. Воспользуйтесь соответствующей командой табличной части, чтобы выполнить запрос повторно.'");
	
КонецФункции

Функция ТекстСообщенияНевозможноВыполнитьПроверкуСтатусаКМ_Вручную() Экспорт
	
	Возврат НСтр("ru = 'Не удалось получить токен сеанса для проверки состояния кодов маркировки по данным сервера. Возможные причины: возникли проблемы с подключением к сети Интернет, подписью ЭЦП, подпиской сервиса маркировки или доступностью серверов ИС МПТ.'");
	
КонецФункции

Функция ТекстСообщенияОтказДокументНеОтраженВУчете() Экспорт
	
	Возврат НСтр("ru = 'Невозможно выполнить, так как документ не отражен в учете.'");
	
КонецФункции

Функция ТекстСообщенияНеУдалосьЗаписатьДокументМаркировки() Экспорт
	
	Возврат НСтр("ru='Не удалось записать документ по причине: %1'");
	
КонецФункции

Функция ТекстСообщенияОшибкаЗапросаСоставаУпаковкиПоДаннымСервера() Экспорт
	
	Возврат НСтр("ru = 'Не удалось получить информацию по агрегированным кодам со стороны сервера. Подробная информация добавлена в Журнал регистрации.'");
	
КонецФункции

Функция ТекстСообщенияВДокументеОтсутствуетМаркируемаяПродукция() Экспорт
	
	Возврат НСтр("ru = 'В документе ""%ДокументОснование%"" отсутствует маркируемая продукция.'");
	
КонецФункции

Функция ТекстСообщенияНеВыбранДокументОснование() Экспорт
	
	Возврат НСтр("ru = 'Не выбран документ-основание!'");
	
КонецФункции

Функция ТекстСообщенияОтказДобавленияКодаНеОпределенаНоменклатура() Экспорт
	
	Возврат НСтр("ru = 'Не удалось определить элемент справочника Номенклатура, соответствующий штрихкоду кода маркировки.'")
		  + Символы.ПС 
		  + НСтр("ru = 'Данный код не может быть добавлен в список.'");
	
КонецФункции

Функция ТекстСообщенияЗапросСоставаУпаковокВОтправленномДокументе() Экспорт
	
	Возврат НСтр("ru = 'При выполнении команды в документе будут изменены данные полей ""Номенклатура"" и ""GTIN, вложенный"" на основании полученной с сервера информации.'")
		  + Символы.ПС
...
Функция ТекстСообщенияКодыДоступныДляИспользованияВДокументе(КодОдин = Ложь) Экспорт
	...
	Если КодОдин Тогда
		Возврат НСтр("ru = 'Выполнена проверка статуса кода по данным сервера ИС МПТ: замечаний не обнаружено. Введенный код может быть использован в документе.'");
	Иначе
		Возврат НСтр("ru = 'Выполнена проверка статусов кодов по данным сервера ИС МПТ: замечаний не обнаружено. Загруженные коды могут быть использованы в документе.'");
	КонецЕсли;
