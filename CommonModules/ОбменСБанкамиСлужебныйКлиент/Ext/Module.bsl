...
//
Процедура СинхронизироватьСБанком(Оповещение, НастройкаОбмена, ВыводитьОкноОжидания = Ложь, ОповещениеЗапускаДлительнойОперации = Неопределено) Экспорт
	...
	РеквизитыНастройкиОбмена = Новый Структура("ПрограммаБанка, АутентификацияПоСертификату, ИспользуетсяКриптография,
		|Недействительна, ИмяВнешнегоМодуля, АдресСервера, ИдентификаторОрганизации, ВерсияФормата, ДополнительнаяОбработка,
		|ПравоВыполненияОбмена, АсинхронноЧерезДопОбработку");
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		Если ОбменСБанкамиСлужебныйВызовСервера.ПодписываетсяЗапросВыписки(НастройкаОбмена) Тогда
			Оповещение = Новый ОписаниеОповещения("СформироватьЗапросыВыписокПослеПолученияОтпечатков", ЭтотОбъект, Параметры);
	...
	Иначе // АльфаБанк, обработка и компонента.
		Параметры.Вставить("ТребуетсяАвторизация", Ложь);
		ОтправитьДокументыВБанк(ДанныеАвторизации, Параметры);
		Если РеквизитыНастройкиОбмена.АсинхронноЧерезДопОбработку = Истина Тогда
			
			ВнешнийПодключаемыйМодуль = ВнешнийПодключаемыйМодульЧерезДополнительнуюОбработку(РеквизитыНастройкиОбмена.ИмяВнешнегоМодуля, Неопределено);
			
			ПараметрыВыписки = Новый Структура;
			ПараметрыВыписки.Вставить("НомерСчета",        "0");
			ПараметрыВыписки.Вставить("БИК",               "0");
			ПараметрыВыписки.Вставить("ДатаНачала"   ,     Дата("00010101000000"));
			ПараметрыВыписки.Вставить("ДатаОкончания",     Дата("00010101000000"));
			...
			ВнешнийПодключаемыйМодуль, Неопределено, 2, Новый Структура);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьВыпискуЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, Параметры) Экспорт
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		РеквизитыНастройкиОбмена = Новый Структура("АсинхронноЧерезДопОбработку");
		ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(
			Параметры.НастройкаОбмена, РеквизитыНастройкиОбмена);
	
		Если РеквизитыНастройкиОбмена.АсинхронноЧерезДопОбработку Тогда
			Параметры.Вставить("ДанныеСертификата", Новый Структура("ДвоичныеДанныеСертификата"));
			ПродолжитьПолучениеВыпискиПослеУстановкиСоединенияЧерезДопОбработку(Истина, Параметры);
		Иначе 
...
Процедура НачатьОтправкуПакетовЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, Параметры) Экспорт
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		Если ОбщегоНазначенияБКВызовСервера.ЗначениеРеквизитаОбъекта(Параметры.НастройкаОбмена, "АсинхронноЧерезДопОбработку") Тогда
			
			ПараметрыПакета = Новый Структура;
			ПараметрыПакета.Вставить("ОтправляемыеПакетыЧерезДопОбработку", Параметры.ДанныеДляОтправки.ДанныеПакетов);
			ПараметрыПакета.Вставить("ИмяПроцедуры", "ОтправитьПакетыЧерезДополнительнуюОбработку");
			ПараметрыПакета.Вставить("ОбработчикПродолжения", Параметры.ОбработчикПродолжения);
			ПараметрыПакета.Вставить("Модуль", ЭтотОбъект);
			ПараметрыПакета.Вставить("ПарольУстановлен", Истина);
			ПараметрыПакета.Вставить("ВнешнийПодключаемыйМодуль", Параметры.ВнешнийПодключаемыйМодуль);
			ПараметрыПакета.Вставить("ДанныеСертификата", Новый Структура("ДвоичныеДанныеСертификата", Неопределено));
			ПрерватьОбработку = Ложь;
		
			ОтправитьПакетыЧерезДополнительнуюОбработку(Истина, ПараметрыПакета);
	...
	Если АутентификацияВыполнена = Истина Тогда
		...
		Если Параметры.Свойство("ОтправляемыеПакетыЧерезДопОбработку") Тогда
			Для Каждого Документ Из Параметры.ОтправляемыеПакетыЧерезДопОбработку Цикл
				...
				Если Документ.Значение.Свойство("СлужебныеДанные") Тогда
					СтруктураОтправки.Вставить("СхемаДанных",         ПолучитьИзВременногоХранилища(Документ.Значение.СлужебныеДанные));
				Иначе 
					СтруктураОтправки.Вставить("СхемаДанных",         ОбменСБанкамиСлужебныйВызовСервера.ДвоичныеДанныеПрисоединенногоФайла(Документ.Ключ));
					СтруктураОтправки.Вставить("ДокументПакет",       Документ.Ключ);
				КонецЕсли;
			
				СтруктураОтправки.Вставить("Подписи",             Новый Массив);
			
				Для Каждого СтрокаДанныеПодписи Из Документ.Значение.Подписи Цикл
					ДанныеПодписи = Новый Структура("Сертификат, Подпись", СтрокаДанныеПодписи.Сертификат, СтрокаДанныеПодписи.Подпись);
...
Процедура ПодключитьИИнициализироватьВК(Оповещение, НастройкаОбмена) Экспорт
	...
	РеквизитыНастройкиОбмена = Новый Структура(
		"ИмяВнешнегоМодуля, ИспользоватьЖурналирование, КаталогДляЖурналирования, ИдентификаторОрганизации");
	...
	ДополнительныеПараметры.Вставить("ИдентификаторКлиента", РеквизитыНастройкиОбмена.ИдентификаторОрганизации);
	ДополнительныеПараметры.Вставить("НастройкаОбмена", НастройкаОбмена);
...
Процедура ПодготовитьЗапросыВыписокДляОтправкиЧерезВК(Оповещение, НастройкаОбмена, ПараметрыЗапроса, ЭтоСинхронизация = Ложь) Экспорт
	...
	МассивЗапросов = ОбменСБанкамиСлужебныйВызовСервера.ЗапросыВыписок(
		НастройкаОбмена, ПараметрыЗапроса, Новый Массив, НастройкиОбмена, ЭтоСинхронизация);
	...
	Иначе // выполнено
		...
		Если РезультатОперации.ТребуетсяАутентификация Тогда
			...
			Иначе
				...
				ВыполнитьАутентификациюПоЛогинуСбербанк(Обработчик, ДополнительныеПараметры.РеквизитыНастройкиОбмена.ИмяВнешнегоМодуля,
					ДополнительныеПараметры.НастройкаОбмена, ДополнительныеПараметры.НастройкаОбмена, , ,
					ДополнительныеПараметры.ВыводитьОкноОжидания);
			КонецЕсли
		Иначе
...
Процедура АутентификацияНаСервереПослеПолученияДанныхАутентификацииСбербанк(
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
	Иначе
		ПослеАутентификацииПоЛогинуСбербанк(Результат, ДополнительныеПараметры)
	КонецЕсли;
	...
	ИначеЕсли СтруктураКОтправке.САутентификациейПоСертификату.Количество() Тогда
		Для Каждого ТекЭл Из СтруктураКОтправке.САутентификациейПоСертификату Цикл
			...
			Для Каждого ОтправляемоеСообщениеОбмена Из ТекЭл.Значение Цикл
				Если ОбменСБанкамиСлужебныйВызовСервера.ЭлектронныйДокументПолностьюПодписан(ОтправляемоеСообщениеОбмена) Тогда
...
Процедура ПослеОтправкиДокументовВБанкАсинхронныйОбмен(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		ПолучитьНовыеДокументыИзБанка(Оповещение, ДополнительныеПараметры.НастройкаОбмена,
			ДополнительныеПараметры.ИдентификаторСессии, ДополнительныеПараметры.ВыводитьОкноОжидания, ДополнительныеПараметры.ОповещениеЗапускаДлительнойОперации);
	КонецЕсли;
...
Процедура СформироватьИОтправитьПакетыВБанк(Оповещение, МассивСообщенийОбмена, ИдентификаторыСессий = Неопределено)
	...
	Если РезультатОтправки.Статус = "Выполняется" Тогда
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатОтправки, Оповещение, ПараметрыОжидания);
	Иначе
		ПослеОтправкиПлатежныхДокументовВБанк(РезультатОтправки, ДополнительныеПараметры)
	КонецЕсли;
