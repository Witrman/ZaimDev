...
Функция КонтейнерМетодов() Экспорт
	...
	#Иначе
		...
		СНТКлиентСерверПереопределяемый.КонтейнерМетодов(Контейнер, ИспользоватьВнешнююОбработку);
...
Функция ДействиеАннуллироватьПодтверждение() Экспорт
	Возврат "АннуллироватьПодтверждение";
КонецФункции
...
Функция ЦветСостоянияСНТ(Знач СостояниеСНТ) Экспорт
	...
	ИначеЕсли СостояниеСНТ = ПредопределенноеЗначение("Перечисление.СостоянияСНТ.ПринятСервером")
		  ...
		  ИЛИ СостояниеСНТ = ПредопределенноеЗначение("Перечисление.СостоянияСНТ.ПринятОтПоставщика")
		  ИЛИ СостояниеСНТ = ПредопределенноеЗначение("Перечисление.СостоянияСНТ.ЗаблокированИСЭСФ") Тогда
...
Функция ИспользоватьВерсию2СопоставлениеСНТиФНО () Экспорт
...
Функция СтатусСНТ_ИБ(Знач СтатусСНТ) Экспорт
	...
	// Данный статус нужен и в документах СНТ так как при выписке такой ЭСФ блокируется вся цепочка СНТ->ЭСФ
	//  и соотвественно в документах СНТ нужно данный статус указывать.
	ИначеЕсли ВРегСтатусСНТ = "SEND_TO_ISGO" Тогда 
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыСНТ.ЗаблокированИСЭСФ");
...
Функция НаправлениеИСЭСФ(Знач НаправлениеИБ) Экспорт
	...
	Если НаправлениеИБ = ПредопределенноеЗначение("Перечисление.НаправленияСНТ.Входящий") Тогда
		Направление = "INBOUND";
	ИначеЕсли НаправлениеИБ = ПредопределенноеЗначение("Перечисление.НаправленияСНТ.Исходящий") Тогда
