...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Пользователь) И Метаданные.Справочники.Пользователи.ДлинаКода <> 0 Тогда
			...
			Пока ВыборкаПользователи.Следующий() Цикл
				Если СокрЛП(ВыборкаПользователи.Код) = ПользовательИБ.Имя Тогда
					Пользователь = ВыборкаПользователи.Ссылка;
					Прервать;
				КонецЕсли;
			КонецЦикла;
