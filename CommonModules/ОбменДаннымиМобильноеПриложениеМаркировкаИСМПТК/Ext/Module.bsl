...
Функция ПолучитьТекущиюВерсиюМобильногоПриложения() Экспорт
	...
	//БМ_ИСМПТ КОНТРОЛЬНАЯ_ТОЧКА	Версия мобильного приложения
	Возврат "1.0.1";
...
Функция ПолучитьКодыМаркировкиДокумента(Номер, Соединение) Экспорт
	...
	СписокСериализованный = Соединение.GiveDocumentMarkingCodes(ОбменДаннымиМобильноеПриложениеМаркировкаИСМПТК.Сериализовать(Номер));
	Список = ОбменДаннымиМобильноеПриложениеМаркировкаИСМПТК.Десериализовать(СписокСериализованный);
...
Функция ПолучитьКодыМаркировки(ИсходныеДанные, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если ДанныеРазбора.ВидУпаковки = Перечисления.ВидыУпаковокИСМПТК.Групповая
		...
		КодыМаркировки.ВидПродукцииИС	= ДанныеРазбора.ВидыПродукции[0];
...
функция ПолучитьНоменклатуруПоШтрихкоду(EAN, Объект) Экспорт
