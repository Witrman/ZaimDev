...
Процедура ОбновитьАдресЭлектроннойПочты(Знач ПользовательОбъект, Знач Адрес, Знач СтруктураАдресаЭП) Экспорт
	...
	ЗаписьКонтактнаяИнформация = РегистрыСведений.КонтактнаяИнформация.СоздатьМенеджерЗаписи();	
	ЗаписьКонтактнаяИнформация.Объект = ПользовательОбъект.Ссылка;
	ЗаписьКонтактнаяИнформация.Тип = ТипКИ;
	ЗаписьКонтактнаяИнформация.Вид = ВидКИ;
	ЗаписьКонтактнаяИнформация.Прочитать();
	
	Если НЕ ЗаписьКонтактнаяИнформация.Выбран() Тогда
		ЗаписьКонтактнаяИнформация = Неопределено;
	КонецЕсли;
	
	Если СтруктураАдресаЭП = Неопределено Тогда
		
		Если ЗаписьКонтактнаяИнформация <> Неопределено Тогда
			ЗаписьКонтактнаяИнформация.Удалить();
		КонецЕсли;
	...
	Иначе
		...
		Если ПользовательОбъект.Ссылка.Пустая() Тогда
			...
			СсылкаНового = ПользовательОбъект.ПолучитьСсылкуНового();
			
			Если СсылкаНового.Пустая() Тогда
				Ссылка = Справочники.Пользователи.ПолучитьСсылку();
				ПользовательОбъект.УстановитьСсылкуНового(Ссылка);
			Иначе
				Ссылка = СсылкаНового;
			КонецЕсли;
		...
		ЗаписьКонтактнаяИнформация.Объект = Ссылка;
		ЗаписьКонтактнаяИнформация.Вид = ВидКИ;
		ЗаписьКонтактнаяИнформация.Тип = ТипКИ;
		ЗаписьКонтактнаяИнформация.Представление = Адрес;
		
		// Запись выполняется вне транзакции записи элемента справочника Пользователи.
		// Если при записи пользователя произойдет ошибка, то в РС КонтактнаяИнформация
		// останется запись принадлежащая объекту <Объект не найден>. 
		ЗаписьКонтактнаяИнформация.Записать(Истина);
...
Процедура ОбновитьТелефон(Знач ПользовательОбъект, Знач Телефон) Экспорт
	...
	ВидКИ = Справочники.ВидыКонтактнойИнформации.СлужебныйТелефонПользователя;
	...
	ЗаписьКонтактнаяИнформация.Вид = ВидКИ;
	ЗаписьКонтактнаяИнформация.Тип = ТипКИ;
	ЗаписьКонтактнаяИнформация.Представление = Телефон;
	ЗаписьКонтактнаяИнформация.Поле3 = Телефон;
