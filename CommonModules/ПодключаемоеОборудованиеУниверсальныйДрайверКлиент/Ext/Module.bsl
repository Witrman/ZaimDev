...
Функция ВыполнитьКоманду(Команда, ВходныеПараметры = Неопределено, ВыходныеПараметры = Неопределено,
	...
	ИначеЕсли Команда = "OpenCheck" ИЛИ Команда = "ОткрытьЧек"  Тогда
		...
		ПолучательИИН = ВходныеПараметры[2];
		Результат = ОткрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ЧекВозврата, ФискальныйЧек, ВыходныеПараметры, ВходныеПараметры);
...
Функция Инкассация(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПараметрыПодключения.ТипОборудования = "ККТ" Или ПараметрыПодключения.ТипОборудования = "ФискальныйРегистратор" Тогда
		...
		Попытка
			Если ПараметрыПодключения.РевизияИнтерфейса > 2001 И ПараметрыПодключения.ТипОборудования = "ККТ" Тогда // Версия стандарта 2.2 (ОФД 1.0.5/1.1)
...
//
Функция ОткрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТипЧека, ФискальныйЧек, ВыходныеПараметры, ПараметрыОткрытияЧека = Неопределено) Экспорт
	...
	ПокупательEmail = "";
	ПокупательНомер = "";
	Если ПараметрыОткрытияЧека <> Неопределено И ПараметрыОткрытияЧека.Количество() > 0 Тогда
		ПолучательИИН = ПараметрыОткрытияЧека[2];
		ПокупательEmail = ПараметрыОткрытияЧека[3];
		ПокупательНомер = ПараметрыОткрытияЧека[4];
	ИначеЕсли ПараметрыОткрытияЧека = Неопределено Тогда
		ПолучательИИН = ?(Параметры.Свойство("ПолучательИИН"), Параметры.ПолучательИИН, "");
		ПокупательEmail = ?(Параметры.Свойство("ПокупательEmail"), Параметры.ПокупательEmail, "");
		ПокупательНомер = ?(Параметры.Свойство("ПокупательНомер"), Параметры.ПокупательНомер, "");
	КонецЕсли;

	Попытка
		
		Если ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса = 2008 Тогда
			Ответ = ОбъектДрайвера.ОткрытьЧек(ПараметрыПодключения.ИДУстройства, ФискальныйЧек, ТипЧека, Истина, НомерЧека, НомерСмены, ПолучательИИН, ПокупательEmail, ПокупательНомер);
		ИначеЕсли ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса = 2007 Тогда
			Ответ = ОбъектДрайвера.ОткрытьЧек(ПараметрыПодключения.ИДУстройства, ФискальныйЧек, ТипЧека, Истина, НомерЧека, НомерСмены, ПолучательИИН);
		Иначе 
...
Функция НапечататьФискальнуюСтроку(ОбъектДрайвера, Параметры, ПараметрыПодключения, Наименование, Количество, Цена, Сумма,
	...
	Результат = Истина;
	КодЕдиницыИзмерения    = Параметры.КодЕдиницыИзмерения;
	ОсвобожденныйОборотНДС = Параметры.ОсвобожденныйОборотНДС;
	КодМаркировки = ?(Параметры.Свойство("КодМаркировки"), Параметры.КодМаркировки, "");
	ИдентификаторТовара = "";
	ШтрихКод = Параметры.ШтрихКод;
	
	Попытка
		
		Если ПараметрыПодключения.Свойство("РевизияИнтерфейса") И (ПараметрыПодключения.РевизияИнтерфейса = 2007 Или ПараметрыПодключения.РевизияИнтерфейса = 2008) Тогда
			
			Ответ = ОбъектДрайвера.НапечататьФискСтроку(ПараметрыПодключения.ИДУстройства, Наименование, Количество, Цена,
										Сумма, НомерСекции, СтавкаНДС, ОсвобожденныйОборотНДС, КодЕдиницыИзмерения, КодМаркировки, ИдентификаторТовара, ШтрихКод);
			
		ИначеЕсли ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса = 2006 Тогда
			
			Ответ = ОбъектДрайвера.НапечататьФискСтроку(ПараметрыПодключения.ИДУстройства, Наименование, Количество, Цена,
														Сумма, НомерСекции, СтавкаНДС, ОсвобожденныйОборотНДС, КодЕдиницыИзмерения, КодМаркировки);
