...
Процедура ИзменитьПользовательскиеСпособыОповещенияЛентНовостей(ЛентыНовостей, НастройкиОповещения, Пользователь = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ЛентыНовостей) = ТипМассив Тогда
		...
		Для Каждого ТекущаяЛентаНовостей Из ЛентыНовостей Цикл
			Если ТипЗнч(ТекущаяЛентаНовостей) = ТипЛентыНовостей Тогда
				МассивЛентНовостей.Добавить(ТекущаяЛентаНовостей);
			КонецЕсли;
		КонецЦикла;
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Для Каждого ТекущаяЛентаНовостей Из МассивЛентНовостей Цикл
		// Проверку на пометку удаления или поле ОбязательныйКанал не производим, т.к. при чтении настроек это учитывается.
		Запись = РегистрыСведений.НастройкиЛентНовостейПользовательские.СоздатьМенеджерЗаписи();
			Запись.Пользователь                          = Пользователь;
			Запись.ЛентаНовостей                         = ТекущаяЛентаНовостей;
			Если ТипЗнч(НастройкиОповещения.СпособОповещения) = ТипСпособОповещения Тогда
				Запись.СпособОповещенияПользователяОНовостях = НастройкиОповещения.СпособОповещения;
			Иначе // Строка
				// {При добавлении способов оповещения исправлять здесь}.
				Если НастройкиОповещения.СпособОповещения = "Отключено" Тогда
					Запись.СпособОповещенияПользователяОНовостях = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено;
				ИначеЕсли НастройкиОповещения.СпособОповещения = "СистемаВзаимодействия" Тогда
					Запись.СпособОповещенияПользователяОНовостях = Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию;
				Иначе
					Запись.СпособОповещенияПользователяОНовостях = Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию;
				КонецЕсли;
			КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
			Обработчик.Версия                  = "*";
			Обработчик.Процедура               = "ОбработкаНовостей.ЗаполнитьОбработчикиРазделенныхДанных";
			Обработчик.ОбщиеДанные             = Истина;
...
//  чтобы потом получать их через СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ИнтернетПоддержкаПользователей.Новости.
// Не использовать этот метод для вызова из "ПриНачалеРаботыСистемы".
// Для вызова из "ПриНачалеРаботыСистемы" использовать ПриДобавленииПараметровРаботыКлиентаПриЗапуске.
...
Процедура ПриДобавленииПараметровРаботыКлиента(ПараметрыИПП) Экспорт
	...
	ПараметрыНовости = Новый Структура;
		ПараметрыНовости.Вставить("РазрешенаРаботаСНовостями"                        , РазрешенаРаботаСНовостями());
		ПараметрыНовости.Вставить("РазрешенаРаботаСНовостямиЧерезИнтернет"           , РазрешенаРаботаСНовостямиЧерезИнтернет());
		ПараметрыНовости.Вставить("РазрешенаРаботаСНовостямиТекущемуПользователю"    , РазрешенаРаботаСНовостямиТекущемуПользователю());
...
// Для минимизации серверных вызовов надо заполнять часто используемые параметры в этом методе,
//  чтобы потом получать их через СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиентаПриЗапуске().ИнтернетПоддержкаПользователей.Новости.
...
Процедура СбросДатыПоследнейУспешнойЗагрузкиНовостей(ЛентыНовостей, ПомечатьНовостиНаУдаление = Ложь) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр(
				"ru='ОбработкаНовостей.СбросДатыПоследнейУспешнойЗагрузкиНовостей: Должно запускаться в неразделенном сеансе.'",
				КодОсновногоЯзыка);
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЛокальныеЛентыНовостей.Количество() = 0 Тогда
		ТекстСообщения = НСтр(
			"ru='ОбработкаНовостей.СбросДатыПоследнейУспешнойЗагрузкиНовостей: Не удалось сформировать список лент новостей'",
			КодОсновногоЯзыка);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
