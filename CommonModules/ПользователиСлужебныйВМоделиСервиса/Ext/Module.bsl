...
Процедура ЗаписатьПользователяСервиса(Знач Пользователь, Знач СоздатьПользователяСервиса, Знач ПарольПользователяСервиса) Экспорт
	...
	// БК3. Начало.
	// В БК3 не используется подсистема КонтактнаяИнформация из БСП.
	
	КИ = Прокси.ФабрикаXDTO.Создать(
	...
	Запрос.УстановитьПараметр("Объект", ПользовательОбъект.Ссылка);
	
	МассивВидовКИ = Новый Массив;
	МассивВидовКИ.Добавить(Справочники.ВидыКонтактнойИнформации.СлужебныйАдресЭлектроннойПочтыПользователя);
	МассивВидовКИ.Добавить(Справочники.ВидыКонтактнойИнформации.СлужебныйТелефонПользователя);	
	...
	Пока Выборка.Следующий() Цикл
		...
		ВидКИXDTO = СоответствиеВидовКИ.Получить(Выборка.Вид);
		...
		ЗаписьКИ.Value = Выборка.Представление;
		ЗаписьКИ.Parts = "";
		
		ЗаписиКИ = КИ.Item; // СписокXDTO
		ЗаписиКИ.Добавить(ЗаписьКИ);
		
	КонецЦикла;
	
	ПользовательСервиса.Contacts = КИ;
