...
Функция РазделитьМассивСНТ(МассивСНТ) Экспорт
	Возврат СНТСервер.РазделитьМассивСНТ(МассивСНТ);	
КонецФункции
...
Функция ИзменитьСтатусыСНТ(Действие, КоллецияДляИзмененияСтатусов, ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ) Экспорт
	...
	Если ЭСФВызовСервера.ИспользоватьПодписьНовойКомпоненты() Тогда
		...
		Если ЭСФВызовСервера.ИспользоватьВнешнююКриптографиюДляКомпоненты() Тогда
			КоллекцияПодписейЭСФ = ЭСФКлиентСервер.НоваяКоллекцияПодписейЭСФ(Результат.МассивЧастейЗапроса, ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ);
		Иначе
		...
		Для Каждого ЭлементМассива из Результат.МассивЧастейЗапроса Цикл
			Подпись = КоллекцияПодписейЭСФ.Получить(ЭлементМассива.ИД);
