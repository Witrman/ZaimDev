...
// См. ЭСФСервер.ОткрытьСессиюСПодписью()
Функция ОткрытьСессиюСПодписью(Знач ПрофильИСЭСФ, ВерсияИСЭСФ = Неопределено, ПодписанныйАвторизационныйТикет) Экспорт
	
	Возврат ЭСФСервер.ОткрытьСессиюСПодписью(ПрофильИСЭСФ, ВерсияИСЭСФ, ПодписанныйАвторизационныйТикет);
...
Функция ПолучитьАвторизационныйТикет(Знач ПрофильИСЭСФ, ВерсияИСЭСФ = Неопределено) Экспорт
	...
	Возврат ЭСФСервер.ПолучитьАвторизационныйТикет(ПрофильИСЭСФ, ВерсияИСЭСФ);

КонецФункции

Функция ОткрытьСессиюСПодписьюПредварительно(Знач ПрофильИСЭСФ,ПараметрыОткрытияСессии) Экспорт
	
	Возврат ЭСФСервер.ОткрытьСессиюСПодписьюПредварительно(ПрофильИСЭСФ, ПараметрыОткрытияСессии);
...
Функция ИзменитьСтатусыЭСФ(Действие, КоллецияДляИзмененияСтатусов, ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ) Экспорт
	...
	Если ЭСФВызовСервера.ИспользоватьПодписьНовойКомпоненты() Тогда
		...
		Иначе
			ТекстОшибки = нСтр("ru='Использование подписи компонентой без внешней криптографии не поддерживается на сервере.'", "ru");
...
Функция МассивЭСФДругаяДата(МассивЭСФ, Дата, ИзмененГод = Ложь) Экспорт
	...
	|	ЕСТЬNULL(ГОД(СчетФактураВыданный.%СчетФактураДата) <> ГОД(&Дата), ЛОЖЬ) КАК ИзмененГодСФ,
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ИзмененГод ИЛИ Выборка.ИзмененГодСФ Тогда
				ИзмененГод = Истина;
			КонецЕсли;
		КонецЦикла;		
	КонецЕсли;
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НачалоДня(Выборка.ДатаЭСФ) <> ТекущаяДата Тогда
				...
				ОбъектЭСФ.Записать();
			ИначеЕсли ИзмененНомерСФ Тогда
...
Функция ЭСФ_ЗаписатьЗначениеJSON(Знач Значение) Экспорт

	Возврат ЭСФСервер.ЭСФ_ЗаписатьЗначениеJSON(Значение);

КонецФункции

Функция ЭСФ_ПрочитатьЗначениеJSON(Знач Значение, Знач СвойстваДаты = "") Экспорт

	Возврат ЭСФСервер.ЭСФ_ПрочитатьЗначениеJSON(Значение, СвойстваДаты);

КонецФункции

Функция ИспользоватьОткрытиеСессииСПодписью() Экспорт

	Возврат ЭСФСервер.ИспользоватьОткрытиеСессииСПодписью();

КонецФункции

Функция ИспользоватьПодписьНовойКомпоненты() Экспорт

	Возврат ЭСФСервер.ИспользоватьПодписьНовойКомпоненты();

КонецФункции

Функция ИспользоватьВнешнююКриптографиюДляКомпоненты() Экспорт

	Возврат ЭСФСервер.ИспользоватьВнешнююКриптографиюДляКомпоненты();

КонецФункции

// Получение сразу два параметра для новой компоненты: Возвращает используется ли подпись документов с помощью новой компоненты (НЦА, Калкан) 
Функция ПолучитьПараметрыИспользованияКомпоненты() Экспорт

	ДанныеНастройкиПодписиДляКомпоненты = Новый Структура;
	
	ДанныеНастройкиПодписиДляКомпоненты.Вставить("ИспользоватьПодписьНовойКомпоненты", ЭСФСервер.ИспользоватьПодписьНовойКомпоненты());
	ДанныеНастройкиПодписиДляКомпоненты.Вставить("ИспользоватьВнешнююКриптографиюДляКомпоненты", ЭСФСервер.ИспользоватьВнешнююКриптографиюДляКомпоненты());
	
	Возврат ДанныеНастройкиПодписиДляКомпоненты;
...
Функция СоздатьЭЦПXML(Знач ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры = Неопределено, ПредставлениеОрганизации = "", СлужебныеДанные = "") Экспорт
	...
	Возврат ЭСФКлиентСервер.СоздатьЭЦПXML(ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры, ПредставлениеОрганизации, СлужебныеДанные);
...
Функция СоздатьЭЦПXMLs(Знач ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры = Неопределено, ПредставлениеОрганизации = "", СлужебныеДанные = "") Экспорт
	...
	Возврат ЭСФКлиентСервер.СоздатьЭЦПXMLs(ДанныеДляПодписиJSON, БИН, Сервис, ДополнительныеПараметры, ПредставлениеОрганизации, СлужебныеДанные);
	
КонецФункции

Функция СохранитьКлючВФайл(ДанныеКлючаЭЦП) Экспорт
	
	Возврат ЭСФКлиентСервер.СохранитьКлючВФайл(ДанныеКлючаЭЦП);
	
КонецФункции

Процедура УдалитьФайлКлюча(ДанныеКлючаЭЦП) Экспорт
	
	ЭСФКлиентСервер.УдалитьФайлКлюча(ДанныеКлючаЭЦП);
...
Процедура УстановитьЗначениеКонстантЭСФПриИзмененииФункциональностиПрограммы() Экспорт
	...
	ЭСФСерверПереопределяемый.УстановитьЗначениеКонстантЭСФПриИзмененииФункциональностиПрограммы();
