...
Процедура ОбработатьИзменениеПоддержкиРаботыСоСтруктурнымиПодразделениями()
	...
	//КонецЕсли;
	//
	//ПараметрыСеанса.УдалитьИспользоватьОграничениеПоСтруктурныеПодразделения = НаборКонстант.ПоддержкаРаботыСоСтруктурнымиПодразделениями;
...
Функция ВозможноИзменениеПараметровУчетаПоПодразделениям(ТекущееЗначениеВестиУчетПоПодразделениям)
	...
	Если ЕстьОборотыНаСчетахДоходов Или ЕстьОборотыНаСчетахЗатрат Тогда
		...
		ТекстВопроса = НСтр("ru = 'На счетах доходов и затрат будет %1 аналитика ""Подразделения""!
		|В данный момент на этих счетах имеются обороты.
		|Вы хотите %2 аналитику?'");
		
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, ?(ТекущееЗначениеВестиУчетПоПодразделениям = Истина, "снята", "установлена"),
																			?(ТекущееЗначениеВестиУчетПоПодразделениям = Истина, "снять", "установить"));
...
Процедура ПрименитьПараметрыВеденияУчетаПоСотрудникам(Отказ)
	...
	Для Каждого Счет ИЗ СчетаЗП Цикл
		Если ТипЗнч(Счет.Значение) = Тип("ПланСчетовСсылка.Налоговый") Тогда
			ИмяПланаСчетов = НСтр("ru = 'Налоговый'");
		Иначе
			ИмяПланаСчетов = НСтр("ru = 'Типовой'");
		КонецЕсли;
		ПрименитьПараметрыСубконтоРаботникиОрганизации(Счет.Значение, ИмяПланаСчетов, ВестиУчетПоСотрудникам, Ложь);
	КонецЦикла;
	
	Если ВестиУчетПоСотрудникам Тогда
		ИмяСобытияЖурналаРегистрации    = НСтр("ru = 'Настройка параметров учета.Ведение учета по сотрудникам'");
		ШаблонСообщенияПроведенДокумент = НСтр("ru = 'Проведен документ ""%1"".'");
		ШаблонСообщенияОперацияБух      = НСтр("ru = 'Проведен документ ""%1"" по регистру бухгалтерии ""%2"", содержащий счет по заработной плате.'");
		ШаблонСообщенияНеПроведен       = НСтр("ru = 'Не удалось провести документ ""%1"" по причине: %2'");
...
Функция ОтменитьПроведениеДокументов(Отказ)
	...
	ИмяСобытияЖурналаРегистрации   = НСтр("ru = 'Настройка параметров учета.Ведение учета по сотрудникам'");
	ШаблонСообщенияПрочиеДокументы = НСтр("ru = 'Отменено проведение документа ""%1"".'");
	ШаблонСообщенияОперацияБух     = НСтр("ru = 'Отменено проведение документа ""%1"" по регистру бухгалтерии ""%2"", содержащий счет по заработной плате.'");
	ШаблонСообщенияНеУдалось       = НСтр("ru = 'Не удалось отменить проведение документа ""%1"" по причине: %2'");
