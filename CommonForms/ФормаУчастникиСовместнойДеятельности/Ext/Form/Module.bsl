...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Строка, НомерСтроки, Отказ)
	...
	Если Не ЗначениеЗаполнено(Строка.УчастникСовместнойДеятельности) Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не заполнена колонка """ + Элементы.УчастникиСовместнойДеятельностиУчастникиСовместнойДеятельностиУчастникСовместнойДеятельности.Заголовок 
				+ """ в строке ""%1"" списка """ + Элементы.УчастникиСовместнойДеятельности.Заголовок + """'"), НомерСтроки),
			, 
