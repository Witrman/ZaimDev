...
Функция ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек) Экспорт
					...
					Если ТипЗнч(ТекущийЭлемент.Значение) = ТипСтруктура Тогда
						Если (ТекущийЭлемент.Значение.Свойство("ВариантОтбора"))
							...
							Если (ТипЗнч(лкЗначениеОтбора) <> ТипСтрока
									И ТипЗнч(лкЗначениеОтбора) <> ТипЛентаНовостей
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейВсеНастройки" Тогда
			...
			Для Каждого ТекущиеНастройки Из Настройки Цикл
				Если ТипЗнч(ТекущиеНастройки) = ТипСтруктура Тогда
					Если (ОтборПоЛентеНовостей = Неопределено) ИЛИ (ОтборПоЛентеНовостей = ТекущиеНастройки.ЛентаНовостей) Тогда
						ПравильныеНастройки = Новый Структура;
							ПравильныеНастройки.Вставить("КодЛентыНовостей"      , "");
							ПравильныеНастройки.Вставить("ЛентаНовостей"         , Справочники.ЛентыНовостей.ПустаяСсылка());
							ПравильныеНастройки.Вставить("ОбязательныйКанал"     , Ложь);
							ПравильныеНастройки.Вставить("СпособОповещения"      , Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию);
							ПравильныеНастройки.Вставить("ПодпискаПринудительно" , "Неопределено");
							ПравильныеНастройки.Вставить("ПодпискаСамостоятельно", "Подписан");
						ЗаполнитьЗначенияСвойств(ПравильныеНастройки, ТекущиеНастройки);
		...
		Если (ОтборПоЛентеНовостей <> Неопределено) И (ПравильныйМассив.Количество() = 0) Тогда
			ПравильныеНастройки = Новый Структура;
				ПравильныеНастройки.Вставить("КодЛентыНовостей"      , ОтборПоЛентеНовостей.Код);
				ПравильныеНастройки.Вставить("ЛентаНовостей"         , ОтборПоЛентеНовостей);
				ПравильныеНастройки.Вставить("ОбязательныйКанал"     , ОтборПоЛентеНовостей.ОбязательныйКанал);
				ПравильныеНастройки.Вставить("СпособОповещения"      , Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию);
				ПравильныеНастройки.Вставить("ПодпискаПринудительно" , "Неопределено");
				ПравильныеНастройки.Вставить("ПодпискаСамостоятельно", "Подписан");
			ПравильныйМассив.Добавить(ПравильныеНастройки);
		КонецЕсли;
		Результат = ПравильныйМассив;
