...
Процедура ОткрытьАнализСчета(Команда)
	...
	ОткрытьФорму("Отчет.АнализСчетаТиповой.Форма.ФормаОтчета", ПараметрыАнализаСчета,, Истина);
...
Процедура ЗаполнитьНеиспользуемымиСчетами(Команда)
	...
	Если ТаблицаОбновляемыеСчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаЗаполнитьСчета", ЭтотОбъект), 
	...
	Если НЕ СчетаДляОбновления = Неопределено Тогда
		...
		Для НомерСтр = СчетаДляОбновления.Верх По СчетаДляОбновления.Низ Цикл
			...
			Если ЗначениеЗаполнено(ЗаменяемыйСчет) И ЗначениеЗаполнено(НовыйСчет) Тогда
				...
				СтрокаОбновляемыеСчета.НаименованиеИсходногоСчета = ПолучитьЗначениеРеквизита(ЗаменяемыйСчет, "Наименование");
				СтрокаОбновляемыеСчета.НовыйСчет = НовыйСчет;
				СтрокаОбновляемыеСчета.НаименованиеНовогоСчета = ПолучитьЗначениеРеквизита(НовыйСчет, "Наименование");
...
Функция ЗаменитьНаСервереВФоне()
	...
	ИмяПроцедуры = "ПланыСчетов.Типовой.ЗаменитьСчетаПриОбновленииПланаСчетов";
