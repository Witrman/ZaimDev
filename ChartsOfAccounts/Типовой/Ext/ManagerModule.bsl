...
Процедура ОчиститьНеиспользуемыеСубконто(Объект, Субконто, ЗаменяемаяСсылка, НоваяСсылка, ОбъектИзменен)
...
Процедура ЗаменаСсылкиНаОбъект(Объект, ИмяСвойства, ТипСсылки, ЗаменяемаяСсылка, НоваяСсылка, ОбъектИзменен, ОбъектыПоиска = Неопределено)
	...
	Для Каждого Ссылка Из ТаблицаОбъектов Цикл
					...
					Для каждого ЗаписьРегистра Из НаборЗаписей Цикл
						...
						Для Каждого СсылкаНаСчет Из Ссылка.Ссылка Цикл
							...
							Для каждого Реквизит из Реквизиты Цикл
								ЗаменаСсылкиНаОбъект(ЗаписьРегистра, Реквизит.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, ОбъектИзменен, Реквизиты);
							КонецЦикла;
						КонецЦикла;
						...
						Для Каждого СсылкаНаСчет Из Ссылка.Ссылка Цикл
							...
							Для каждого Ресурс из Ресурсы Цикл
								ЗаменаСсылкиНаОбъект(ЗаписьРегистра, Ресурс.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, ОбъектИзменен, Ресурсы);
							КонецЦикла;
						КонецЦикла;
			...
			Иначе  // если ОбъектИзмененияВИзмерении 
				...
				Если Объект.Выбран() Тогда
					...
					Для Каждого СсылкаНаСчет Из Ссылка.Ссылка Цикл
						ПолучениеСсылокНаНовыйОбъект(СписокЗаменяемыхОбъектов, СписокНовыхОбъектов, СсылкаНаСчет.Значение, ТипСсылки, НоваяСсылка);
						Для каждого Измерение из Измерения Цикл
							ЗаменаСсылкиНаОбъект(Объект, Измерение.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, ОбъектИзменен, Измерения);
						КонецЦикла;
					КонецЦикла;
					...
					Для Каждого СсылкаНаСчет Из Ссылка.Ссылка Цикл
						ПолучениеСсылокНаНовыйОбъект(СписокЗаменяемыхОбъектов, СписокНовыхОбъектов, СсылкаНаСчет.Значение, ТипСсылки, НоваяСсылка);
						Для каждого Реквизит из Реквизиты Цикл
							ЗаменаСсылкиНаОбъект(Объект, Реквизит.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, ОбъектИзменен, Реквизиты);
						КонецЦикла;
					КонецЦикла;
					...
					Для Каждого СсылкаНаСчет Из Ссылка.Ссылка Цикл
						ПолучениеСсылокНаНовыйОбъект(СписокЗаменяемыхОбъектов, СписокНовыхОбъектов, СсылкаНаСчет.Значение, ТипСсылки, НоваяСсылка);
						Для каждого Ресурс из Ресурсы Цикл
							ЗаменаСсылкиНаОбъект(Объект, Ресурс.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, ОбъектИзменен, Ресурсы);
						КонецЦикла;
					КонецЦикла;
			...
			Для Каждого СсылкаНаСчет Из Ссылка.Ссылка Цикл
				...
				Для каждого ТабличнаяЧасть из ТабличныеЧасти Цикл
					...
					Для каждого СтрокаТЧ из ТЧ Цикл
						Для каждого Реквизит из Реквизиты Цикл
							ЗаменаСсылкиНаОбъект(СтрокаТЧ, Реквизит.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, ОбъектИзменен, Реквизиты);
						КонецЦикла;
					КонецЦикла;
				КонецЦикла;
			КонецЦикла;
			...
			Для Каждого Регистр из Ссылка.Метаданные.Движения Цикл
					...
					Для каждого Запись из НаборЗаписей Цикл
						...
						Для каждого Измерение из Измерения Цикл
							...
							Если Балансовый Тогда
								ЗаменаСсылкиНаОбъект(Запись, Измерение.Имя, ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, НаборЗаписейИзменен, Измерения);
							Иначе
						...
						Если ИмяКоллекцииРегистров = "РегистрБухгалтерии" Тогда
							Если Регистр.Корреспонденция Тогда
								...
								ОчиститьНеиспользуемыеСубконто(Запись, "СубконтоДт", СсылкаНаСчет.Значение, НоваяСсылка, НаборЗаписейИзменен);
								ЗаменаСсылкиНаОбъект(Запись, "СчетКт", ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, НаборЗаписейИзменен);
								ОчиститьНеиспользуемыеСубконто(Запись, "СубконтоКт", СсылкаНаСчет.Значение, НоваяСсылка, НаборЗаписейИзменен);
								
							Иначе
								ЗаменаСсылкиНаОбъект(Запись, "Счет", ТипСсылки, СсылкаНаСчет.Значение, НоваяСсылка, НаборЗаписейИзменен);
								ОчиститьНеиспользуемыеСубконто(Запись, "Субконто", СсылкаНаСчет.Значение, НоваяСсылка, НаборЗаписейИзменен);
							КонецЕсли;
				...
				Если НаборЗаписейИзменен Тогда
					...
					Исключение
						...
						Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							Комментарий, Ссылка.Данные, ПолноеИмяРегистра, Ссылка.Ссылка);
		...
		Если ОбъектИзменен Тогда
			...
			Исключение
				...
				Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Комментарий, ИмяКоллекцииОбъектовМД, Ссылка.Метаданные.Синоним, Ссылка.Данные);
