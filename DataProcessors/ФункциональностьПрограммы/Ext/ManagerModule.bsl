...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюВедетсяУчетЗарплатыИКадры(Параметры, АдресХранилища) Экспорт
	...
	Если ИспользуетсяСинхронизацияДанныхЗУП Тогда
		...
		ОбластьШапкаИПояснение.Параметры.Пояснение = НСтр("ru = 'Существуют настройки обменов данными с конфигурациями ""Зарплата и управление персоналом для Казахстана"".
			|Для отключения функциональности ""Ведется учет по зарплате и кадрам"" удалите все настройки.'");
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюУчетПоддержкаСтруктурныхПодразделений(Параметры, АдресХранилища) Экспорт
	...
	Если Выборка.Количество() > 0 Тогда
		...
		ПараметрыШапки.Вставить("Пояснение",
			НСтр("ru = 'Для отключения функциональности ""Структурные подразделения"" в справочнике ""Организации"" необходимо удалить элементы, " 
				+ "для которых в реквизите ""Головная организация"" установлена иная организация (или изменить для них значение головной организации):'"));
	...
	Если Выборка.Количество() > 0 Тогда
		...
		ОбластьШапкаИПояснение.Параметры.Пояснение = НСтр("ru = 'Существуют бухгалтерские проводки в разрезе ""Структурных подразделений"".
			|Для отключения функциональности ""Структурные подразделения"" очистите ""Структурное подразделение"" в движениях документов.'");
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюВедетсяУчетТоваровПоТоварамОрганизацииБУ(Параметры, АдресХранилища) Экспорт
	...
	Если ЗначениеЗаполнено(Константы.ДатаПереходаНаУчетПоТоварамОрганизацийВРазрезеНомеровГТДБУ.Получить()) Тогда
		...
		ОбластьШапкаИПояснение.Параметры.Пояснение = НСтр("ru = 'Для отключения функциональности ""Источники происхождения"" очистите значение константы
			|""Дата перехода на учет по товарам организаций в разрезе источников происхождения"".'");
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюВестиУчетПоОрганизациям(Параметры, АдресХранилища) Экспорт
	...
	Если Выборка.Количество() > 0 Тогда
		...
		ПараметрыШапки.Вставить("Пояснение",
			НСтр("ru = 'Для отключения функциональности ""Учет по нескольким организациям"" необходимо, 
...
Процедура СформироватьСписокОбъектовПрепятствующихОтключениюУчетПоДоговорам(Параметры, АдресХранилища) Экспорт
	...
	Если УчетПоДоговорам.Количество() > 0 Тогда
		...
		ПарамерыШапки.Вставить("Пояснение",
			НСтр("ru = 'Для отключения функциональности ""Учет по договорам"" перенесите задолженность на один договор 
