								...
								Попытка
									...
									// Непосредственно загрузка файла, начало.
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											...
											ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(HTTPОтвет.Заголовки, "=", Символы.ПС),
											ТекущееХТТПСоединение.Протокол + "://" + ТекущаяЛентаНовостей.Сайт + "/" + лкИмяФайлаНаСервере,
										...
										Если ВременныйФайл.Существует() Тогда
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												...
												ТекущееХТТПСоединение.Протокол + "://" + ТекущаяЛентаНовостей.Сайт + "/" + лкИмяФайлаНаСервере,
												ОбработкаНовостей.СодержимоеТекстовогоФайла(ВременныйФайл.ПолноеИмя, 10));
						...
						Если Файл.Существует() Тогда
							...
							Если ЕстьОшибки = Ложь Тогда
								...
								Если ЕстьОшибки = Ложь Тогда
									...
									КонтекстВыполненияВложенныйВложенныйВложенный = ОбработкаНовостейКлиентСервер.НоваяЗаписьРезультатовВыполненияОпераций(); // Контекст "по шагам".
				...
				Если НЕ ПустаяСтрока(СокрЛП(Список.Код)) Тогда
					...
					Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							...
							errCode,
							errDescription,
							ОбработкаНовостей.ПредставлениеОбъектаХДТО(СписокОбновленныхДанных, Прокси.ФабрикаXDTO));
			...
			ИначеЕсли Список = "Список лент новостей" Тогда // Идентификатор.
				...
				Если ТипЗнч(СписокВсехДанных) = ТипОбъектXDTO Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						errCode,
						errDescription,
						ОбработкаНовостей.ПредставлениеОбъектаХДТО(СписокВсехДанных, Прокси.ФабрикаXDTO));
		...
		Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
			...
			Иначе
				...
				Если БылоОбновлениеКатегорийНовостей <> Истина Тогда
					БылаОшибка = НЕ ОбновитьСтандартныйСписокССервера(
						"Список категорий новостей", // Идентификатор.
						'00010101',
						Прокси,
						КонтекстВыполненияВложенный);
					Если БылаОшибка = Истина Тогда
...
Процедура ОбновитьСписокСправочниковИВерсииССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		ОбработкаНовостейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
			КонтекстВыполненияВложенный,
			...
			ОбработкаНовостейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					?(ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO,
						ОбработкаНовостей.ПредставлениеОбъектаХДТО(ДатыОбновленияСписков, Прокси.ФабрикаXDTO),
			...
			Если ЗаписыватьНовость = Истина Тогда
				...
				ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					Попытка
						...
						СтруктураНовостиПередЗаписью = ОбработкаНовостей.ПолучитьСсылкуНаНовость(ОбъектНовость.ЛентаНовостей, ОбъектНовость.УИННовости);
						Если (СтруктураНовости.Ссылка.Пустая()) И (НЕ СтруктураНовостиПередЗаписью.Ссылка.Пустая()) Тогда
							// В процессе заполнения (от ОбработкаНовостей.ПолучитьСсылкуНаНовость до текущего шага) какой-то поток
							//  успел записать новость с такими же ключами (ЛентаНовостей и УИННовости).
							// Не сохранять такую новость.

							ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Найдена новость с такими же значениями ключей (ЛентаНовостей и УИННовости). Запись отменена.'"));
			...
			Если ТипЗнч(СвойствоКатегория) = ТипСвойствоXDTO Тогда
				Если ТипЗнч(ОбъектХДТО.categoriesRefList.categoryRef) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Для каждого ТекущаяКатегорияХДТО Из ОбъектХДТО.categoriesRefList.categoryRef Цикл
						...
						лкКатегория = ПланыВидовХарактеристик.КатегорииНовостей.ПолучитьСсылку(Новый УникальныйИдентификатор(ТекущаяКатегорияХДТО.guid));
						// Для неправильного УИН будет "Объект не найден" с НЕпустой ссылкой. Сравнивать по "ВерсияДанных".
						Если ПустаяСтрока(лкКатегория.ВерсияДанных) Тогда
