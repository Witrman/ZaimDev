...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Поддержка двойного формата транспортного кода
	ПоддержкаДвойногоФормата = Константы.ПоддержкаДвойногоФорматаТранспортныхКодовИСМПТК.Получить();
	Если НЕ ПоддержкаДвойногоФормата Тогда 
		ДобавлятьAID = Истина;
	Иначе
		//Для обуви - 18 символов, для фармы - 20
		Если Параметры.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Обувная") Тогда
			ДобавлятьAID = Ложь;
		Иначе
			ДобавлятьAID = Истина;
		КонецЕсли;
...
Процедура СерийныйНомерSSCCПриИзменении(Элемент)
	...
	Иначе
		...
		Информация = НСтр("ru = 'Невозможно сгенерировать новый штрихкод:'") + Символы.ПС + НСтр("ru = 'серийный номер в разрезе организации и цифры расширения превышает допустимый интервал для этой ИБ.'")
					+ Символы.ПС + НСтр("ru = 'Подробнее см. в настройках обмена ИС МПТ.'");
		Штрихкод = "";
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавлятьAIDПриИзменении(Элемент)
	
	Если Не ДобавлятьAID Тогда
		ДобавлятьСкобочкиДляAID = Ложь;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаГотово", "Доступность", МожноСгенерироватьНовыйШтрихкод);	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДобавлятьСкобочкиДляAID", "Доступность", ДобавлятьAID);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПечатьЭтикетокШтрихкода", "Доступность", ПравоДоступа("Просмотр", Метаданные.Обработки.ПечатьКодовМаркировкиИСМПТК));
...
Процедура ГенерацияШтрихкода()
	...
	Если ЗначениеЗаполнено(ТипУпаковки)
		...
		Если ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодовИСМПТК.SSCC") Тогда
			...
			Штрихкод = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодSSCC(ПараметрыШтрихкода, ДобавлятьСкобочкиДляAID, ДобавлятьAID);
...
Функция ПроверкаКлиент()
	...
	Если НЕ ЗначениеЗаполнено(ТипУпаковки) Тогда
		...
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения,,"ТипУпаковки",,Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ТипШтрихкода) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Тип штрихкода""'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения,,"ТипШтрихкода",,Отказ);
	КонецЕсли;
	
	Если ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодовИСМПТК.SSCC") Тогда
		Если НЕ ЗначениеЗаполнено(ПрефиксКомпанииGS1) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Префикс компании GS1""'");
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения,,"ПрефиксКомпанииGS1",,Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СерийныйНомерSSCC) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Серийный номер""'");
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения,,"СерийныйНомерSSCC",,Отказ);
		КонецЕсли;
	КонецЕсли;
...
Функция ВыполнитьПроверкуИнтерваловГенерации(ПроверкаПриОткрытии)
	...
	Информация = НСтр("ru = 'Для генерации требуется указать Цифру расширения и Префикс компании GS1.'") + Символы.ПС + НСтр("ru = 'Серийный номер будет подобран автоматически.'");
...
Процедура СформироватьЗаписьВРегистреШтрихкодыSSCCИСМПТК() Экспорт
	...
	МенеджерЗаписи = РегистрыСведений.ШтрихкодыSSCCИСМПТК.СоздатьМенеджерЗаписи(); 
	МенеджерЗаписи.Организация 	  	  = Организация;
	МенеджерЗаписи.СерийныйНомерSSCC  = СерийныйНомерSSCC;
	МенеджерЗаписи.ПрефиксКомпанииGS1 = ПрефиксКомпанииGS1;
	МенеджерЗаписи.ЦифраРасширения    = ЦифраРасширения;
	МенеджерЗаписи.Штрихкод		  	  = Штрихкод;
	МенеджерЗаписи.ТипШтрихкода 	  = ТипШтрихкода;
	МенеджерЗаписи.ТипУпаковки 	   	  = ТипУпаковки;
	МенеджерЗаписи.ДатаГенерации	  = ДатаГенерации;
...
Процедура ПечатьЭтикетокШтрихкода(Команда)
	...
	ПоказатьВопрос(ОповещениеВопросПечати, 
		НСтр("ru = 'Штрихкод будет распечатан и записан в регистр. Продолжить?'"),
