...
Процедура ОК(Команда)
	...
	Если Количество = 0 Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено количество'"), , "Количество", , Отказ);
	КонецЕсли;
	...
	ПараметрЗакрытия.Вставить("Номенклатура"    , Номенклатура);
	ПараметрЗакрытия.Вставить("Валюта"          , Валюта);
	ПараметрЗакрытия.Вставить("Цена"            , Цена);
	ПараметрЗакрытия.Вставить("Количество"      , Количество);
	ПараметрЗакрытия.Вставить("Спецификация"    , Спецификация);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокСвойств = "Валюта, Цена, Количество, Номенклатура, ЕдиницаИзмерения, Спецификация";
