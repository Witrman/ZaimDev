...
Процедура ДокументыНаЭкспортПриАктивизацииСтроки(Элемент)
	...
	Если ДанныеСтроки <> Неопределено Тогда
		Если ДанныеСтроки.Готовность>0 Тогда
			Элементы.ДекорацияСтатуснаяСтрокаПриВыгрузке.Заголовок = "Ошибка: "+СтрЗаменить(ДанныеСтроки.ОписаниеОшибок, Символы.ПС, " ");
		Иначе
			Элементы.ДекорацияСтатуснаяСтрокаПриВыгрузке.Заголовок = "Документ готов к выгрузке!";
		КонецЕсли;
	КонецЕсли;
...
Процедура ДокументыКИмпортуВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ДокументыКИмпортуДокумент" И НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные.Документ) Тогда
		...
		ПоказатьВводЗначения(Оповещение, ПолученноеЗначение, "Выберите документ:", ОписаниеВозможныхТипов);
...
Процедура ДокументыКИмпортуПриАктивизацииСтроки(Элемент)
	...
	Если ДанныеСтроки <> Неопределено Тогда
		Если ДанныеСтроки.Готовность>0 Тогда
			Элементы.ДекорацияСтатуснаяСтрокаПриЗагрузке.Заголовок = "Ошибка: "+СтрЗаменить(ДанныеСтроки.ОписаниеОшибок, Символы.ПС, " ");
		Иначе
			Элементы.ДекорацияСтатуснаяСтрокаПриЗагрузке.Заголовок = "Документ готов к загрузке!";
		КонецЕсли;
	КонецЕсли;
...
Процедура ПрочитатьДанныеИзФайла() Экспорт
	...
	Иначе	
		...
		Исключение
			...
			ЗаписатьОшибкуВЖурнал(НСтр("ru = 'Клиент банка. Загрузка данных'"), ОписаниеОшибки);
		КонецПопытки;
