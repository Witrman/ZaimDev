...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаНоменклатура Из СписокКодовМаркировки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаНоменклатура.КоличествоПечать)
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'В строке %1 не заполнено поле ""Шаблон этикетки""';
			|en = 'В строке %1 не заполнено поле ""Шаблон этикетки""'"), НомерСтроки);
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("СписокКодовМаркировки", НомерСтроки, "ШаблонПотребительскойЭтикетки");
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения,, Поле,, Отказ);
...
Процедура СписокКодовМаркировкиШаблонЭтикеткиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ПроверитьЗначениеКонстанты("ВестиУчетМаркируемогоТабакаИСМПТК") Тогда
		...
		Если (ТекущиеДанныеПечатиШаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.ТабачнаяПачка") 
			...
			ТекстОшибки = НСтр("ru = 'Внимание! Выбранный шаблон %1 не соответствует требуемому формату печати штрихкода для текущей строки номенклатуры.'");
			ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ВыбранноеЗначение); 
...
Процедура Печать(Команда)
	...
	ПоказатьВопрос(ОповещениеВопросПечати, 
		...
		|Продолжить?';
			|en = 'Распечатать этикетки по выбранным данным можно только один раз. После выполнения команды коды будут отмечены как использованные.
		|Продолжить?'"),
...
Процедура ЗаполнитьПоGTIN(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Данные номенклатуры для печати будут получены из регистра ""Штрихкоды номенклатуры"", основываясь на указанных GTIN.
...
Процедура УстановитьУсловноеОформление()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокКодовМаркировкиОрганизация", "Видимость", ГрупповаяПечатьКМ);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокКодовМаркировкиЗаказКМ",     "Видимость", ГрупповаяПечатьКМ);
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	Иначе
		...
		Иначе
			...
			Выборка.Следующий();
			Если ЗначениеЗаполнено(Выборка.ДатаПечатиУниверсальная) Тогда
				...
				Иначе
					...
					|en = 'По документу ""%1"" все коды уже распечатаны. Для повторной печати воспользуйтесь командой сброса в Пуле кодов.'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(ТекстОшибки) Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Документ, "Организация") Тогда
				Организация = РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ЗначениеРеквизитаОбъекта(Документ, "Организация");
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьДанныеФормы(Отказ = Неопределено)
	...
	Запрос.УстановитьПараметр("ПустойШаблонЭтикетки", РозничноеВыбытиеИСМПТКПереопределяемый.ПустоеЗначениеОпределяемогоТипа("ШаблонЭтикеткиИСМПТК"));
	...
	Для Каждого СтрокаТаблицы Из ТаблицаСвободныхКодов Цикл
		СтрокаТаблицы.Содержимое = ИнтеграцияИСМПТК.ПредставлениеНоменклатуры(СтрокаТаблицы.Номенклатура, СтрокаТаблицы.Характеристика);
	КонецЦикла;
...
Процедура ЗаполнитьДанныеФормыДляТолькоВыбранныхИзГруппы(Отказ = Неопределено)
	...
	Для Каждого ЭлементОтбора Из ОтборДанныхИзПула Цикл
		...
		ТаблицаСвободныхКодовПромежуточная.Колонки.Добавить("Содержимое", ОбщегоНазначения.ОписаниеТипаСтрока(500));
...
Функция ВидимостьШаблонаЭтикетки()
	...
	Возврат РегистрыСведений.ПулКодовМаркировкиСУЗИСМПТК.ВидимостьШаблонаЭтикетки();
...
Процедура ОбработкаОтветаПользователяПриПечати(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеПечати.ОбъектыПечати.Количество() = 0 Тогда
		...
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения,,, "СписокКодовМаркировки.КоличествоПечать");
		Возврат;
	КонецЕсли;
