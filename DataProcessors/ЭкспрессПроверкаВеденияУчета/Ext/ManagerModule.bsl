...
Процедура ПроверитьПравилоОтражениеЗПРегУч(СтрокаПравила, ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("СчетОПВР", ?(НЕ ЗначениеЗаполнено(Налоги.ОбязательныеПенсионныеВзносыРаботодателя.СчетУчетаРасчетовСКонтрагентомБУ), Типовой.ОбязательстваПоПенсионнымОтчислениям, Налоги.ОбязательныеПенсионныеВзносыРаботодателя.СчетУчетаРасчетовСКонтрагентомБУ));
	...
	|				И ОтражениеВУчете.СубконтоКт1 = ЗНАЧЕНИЕ(Справочник.НалогиСборыОтчисления.ОбязательныеПенсионныеВзносыРаботодателя)
	|				И ОтражениеВУчете.СубконтоКт2 = ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВБюджетИФонды.Налог)
	|			ТОГДА ОтражениеВУчете.Сумма
	|		ИНАЧЕ 0
	|	КОНЕЦ,
...
Процедура ПроверитьПравилоСвоевременнаяВыпискаЭСФ(СтрокаПравила, ПараметрыОтчета)
	...
	Если СтрокаПравила.ОбнаруженыОшибки Тогда 	
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			НоваяСтрока.СчетФактура 	 	  = ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.СчетФактура), ВыборкаДетальныеЗаписи.СчетФактура, "Отсутствует");			
			
			ПредставлениеЭСФ = "" + ВыборкаДетальныеЗаписи.ЭСФ + " (" + ВыборкаДетальныеЗаписи.ЭСФСостояние + ")";   
			
			НоваяСтрока.ЭСФПредставление = ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.СчетФактура), ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ЭСФ),ПредставлениеЭСФ,""), "Отсутствует");
			НоваяСтрока.ЭСФ 	  = ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.СчетФактура), ВыборкаДетальныеЗаписи.ЭСФ, "Отсутствует");   
		...
		СтруктураЗаголовков = Новый Структура;
		СтруктураЗаголовков.Вставить("ДокументОтгрузки", 	  "Документ отгрузки");
		СтруктураЗаголовков.Вставить("СчетФактура",			  "Счет-фактура");			
		СтруктураЗаголовков.Вставить("ЭСФ", 				  "ЭСФ");
		СтруктураЗаголовков.Вставить("ДатаОборота", 		  "Дата оборота");
		СтруктураЗаголовков.Вставить("КрайняяДатаВыпискиЭСФ", "Крайняя дата выписки ЭСФ");      
		СтруктураЗаголовков.Вставить("ОсталосьДней", 		  "Осталось дней"); 
		СтруктураЗаголовков.Вставить("ТипКонтрагента", 		  "Тип контрагента"); 
...
Процедура ПроверитьПравилоПроведенныеДокументыСРасторгнутымиЭАВР(СтрокаПравила, ПараметрыОтчета)
	...
	Если СтрокаПравила.ОбнаруженыОшибки Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
			...
			НоваяСтрока.ЭАВР				  = "" + ВыборкаДетальныеЗаписи.ЭАВР + " (" + ВыборкаДетальныеЗаписи.Статус + ")";
		...
		СтруктураЗаголовков.Вставить("ДокументОснование", "Документ");
		СтруктураЗаголовков.Вставить("ДатаПодписанияГЗ", "Дата подписания"); 
        СтруктураЗаголовков.Вставить("ЭАВР", "ЭАВР");
		СтруктураЗаголовков.Вставить("Статус", "Статус"); 
...
Процедура ПроверитьПравилоНаличиеЗацикливанияПроизводства(СтрокаПравила, ПараметрыОтчета) 
	...
	Если Не ТаблицаОшибок.Количество() = 0 Тогда
		...
		СтруктураЗаголовков = Новый Структура;
		СтруктураЗаголовков.Вставить("ДокументСписания",              "Документ списания");
		СтруктураЗаголовков.Вставить("Подразделение",                     "Подразделение");
		СтруктураЗаголовков.Вставить("НоменклатурнаяГруппа",      "Номенклатурная группа");			
		...
		ПараметрыОтчета.ИсходящиеДанные.Вставить("НаличиеЗацикливанияПроизводстваМакетОформления", "ПроверкаНаличияЗацикливанияПроизводства");	
	Иначе
		СтрокаПравила.ОбнаруженыОшибки = Ложь;	
	КонецЕсли;
...
Процедура ЗаполнитьДеревоПроверокИзМакетаMXL(ДеревоПроверок, ДатаНач = Неопределено) Экспорт
	...
	Для НомСтроки = НомПервойСтрокиПравил По НомПоследнейСтрокиПравил Цикл
		...
		Если ЕстьИмяУСтроки Тогда
			...
			Если (ИмяОбластиСтроки = "МинусыПоТоварамОрганизацийБУ" ИЛИ ИмяОбластиСтроки = "ПроверкаВыпискиЭСФИИП") Тогда
				ДатаПереходаНаУчетПоТоварамОрганизацийНаВС = Константы.ДатаПереходаНаУчетПоТоварамОрганизацийНаВиртуальныхСкладах.Получить();
				Если ЗначениеЗаполнено(ДатаПереходаНаУчетПоТоварамОрганизацийНаВС)
						И ДатаПереходаНаУчетПоТоварамОрганизацийНаВС <= ДатаНач Тогда
					ЗначениеФО = Ложь;
				КонецЕсли;
			КонецЕсли;
