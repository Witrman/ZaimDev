...
Процедура ДоговорыУволенныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ДоговорыУволенныхСотрудник" Тогда
		ПоказатьЗначение( , Элемент.ТекущиеДанные.Сотрудник);
	Иначе
		ПоказатьЗначение( , Элемент.ТекущиеДанные.ЭТДСсылка);
	КонецЕсли;
...
Процедура ОтправитьГруппуДополнительно(Команда)
    ...
    Для каждого МассивСтрока Из Массив Цикл
        ...
        Если Не ПроверкаПройдена Тогда
            ...
            ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка(МассивСтрока));
            ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
            Продолжить;
        Иначе
...
Процедура СоздатьЭТДНаСервере()
	...
	Для Каждого Соглашение Из ДополнительныеСоглашения Цикл
		...
		//ПараметрыЗаполнения.ДатаЗаключения 				= Соглашение.ОсновнойДоговорСсылка.ДатаЗаключения;
...
&НаКлиенте
Функция ПроверитьЗаполнениеПриОтправкеЭТД(Объект)
	
	ДатаСеанса = ОбщегоНазначенияКлиент.ДатаСеанса();
	
	Возврат ПроверитьЗаполнениеПриОтправкеЭТДНаСервере(Объект,ДатаСеанса);
	
КонецФункции

&НаСервере
Функция ПроверитьЗаполнениеПриОтправкеЭТДНаСервере(Объект,ДатаСеанса)
