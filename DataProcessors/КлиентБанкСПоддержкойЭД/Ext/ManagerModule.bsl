	...
	Пока ВыборкаСотрудников.Следующий() Цикл
		...
		Если НЕ ПустаяСтрока(ВыборкаСотрудников.Сотрудник) Тогда
			...
			Иначе
				...
				Иначе
					ТекстВид = ?(ВидДанных = "ОПВ", НСтр("ru = 'пенсионных перечислений'"), НСтр("ru = 'социальных отчислений'"));
...
Функция СформироватьСтруктуруЗаголовковИмпорта()
	...
	СтруктураЗаголовков.Вставить("ЕстьОшибка",                "Есть ошибка");
...
Функция ПрочитатьФайл(Файл, Кодировка, СтекОповещений)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен с банком.Загрузка'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения + НСтр("ru = 'по причине: '") + ОписаниеОшибки.Описание);
...
Процедура ДополнитьСписокНеНайденных(СтрокаДокумента, ТаблицаКонтрагентов, СоздаватьКонтрагента = Ложь, Организация = Неопределено, ГруппаДляНовыхКонтрагентов = Неопределено, СтекОповещений = Неопределено)
	...
	Если НайденнаяЗаписьОКонтрагенте = Неопределено Тогда
		...
		ДобавитьОписаниеНовогоРеквизита("Наименование",
										...
										СтрокаДокумента);
		ДобавитьОписаниеНовогоРеквизита("БИН / ИИН",
										"БИН_ИИН",
										ТипКонтрагента,
										НовыйКонтрагент.Строки,
										СтрокаДокумента);
		ДобавитьОписаниеНовогоРеквизита("КБЕ",
										"КБЕ",
										ТипКонтрагента,
										НовыйКонтрагент.Строки,
										СтрокаДокумента);
		ДобавитьОписаниеНовогоРеквизита("РНН",
										"РНН",
										ТипКонтрагента,
										НовыйКонтрагент.Строки,
										СтрокаДокумента);
...
Функция ДобавитьОписаниеНовогоРеквизита(Представление, Реквизит, ТипКонтрагента, НовыйКонтрагент, СтрокаДокумента)
	...
	РеквизитыНовогоКонтрагента.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1'"), Представление);
...
Функция СоздатьКонтрагента(СтрокаКонтрагент, ТекОрганизация, СтекОповещений, ГруппаДляНовыхКонтрагентов = Неопределено) Экспорт //или СтрокаКонтрагент - строка, или ТаблицаКонтрагентов - ТЗ
	...
	Исключение
		...
		ТекстОповещения = НСтр("ru = 'Не удалось обработать %Ссылка% по причине: %Причина%'");
		ТекстОповещения = СтрЗаменить(ТекстОповещения, "%Ссылка%", Контрагент);
		ТекстОповещения = СтрЗаменить(ТекстОповещения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СоздатьНужныйДоговор(Владелец, ТекОрганизация, Плательщик, ВалютаСчета, СтекОповещений) Экспорт
	...
	Иначе
		...
		ДоговорОбъект.Наименование          = "Без договора";
...
Процедура ПреобразоватьСтрокуДереваВСтруктуру(СтрокаДерева, Структура)
	...
	Для Каждого Строка Из СтрокаДерева.Строки Цикл
		Если СтрНайти(Строка.Представление, "Р/счет") > 0 Тогда
