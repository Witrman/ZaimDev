...
Процедура ЗаполнитьДеревоДокументов()
	...
	Если СписокВыбранныхСтруктурныхПодразделений.Количество() = 0 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И ЭлектронныйДокументВС.СтруктурноеПодразделение В(&СписокВыбранныхСтруктурныхПодразделений)", " И ЭлектронныйДокументВС.СтруктурноеПодразделение = &ПодразделенияОрганизацийПустаяСсылка");
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ПодразделенияОрганизацийПустаяСсылка" , ПустоеПодразделение);	
...
Процедура ПодготовитьФормуНаСервере()
	...
	УчетПоИсточникамПроисхождения = ДатаПереходаНаУчетПоТоварамОрганизацийВРазрезеНомеровГТДБУ <> Дата(1,1,1);
	
	СпособЗаполнения = ?(УчетПоИсточникамПроисхождения, "ТоварамОрганизацииБУ", "ПоДаннымБУ");
...
Функция ПолучитьПараметрыЗаполненияЭДВС(ПараметрыСоздания)
	...
	ПараметрыСоздания.Вставить("ЗаполнятьПоТоварамОрганизации"	, СпособЗаполнения = "ТоварамОрганизацииБУ");
		...
		Для Каждого СтрокаУровень2 Из СтрокаУровень1.ПолучитьЭлементы() Цикл
			Для Каждого СтрокаУровень3 Из СтрокаУровень2.ПолучитьЭлементы() Цикл
				Если СтрокаУровень3.Пометка Тогда 
					...
					ИначеЕсли ТипЗнч(СтрокаУровень3.Организация) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
...
Функция ПолучитьПараметрыЗаполненияТабличныхЧастей()
	...
	ИначеЕсли ТипЗнч(ЭлементДерева) = Тип("СправочникСсылка.ВиртуальныеСклады") Тогда
		...
		Родитель 					= Родитель.ПолучитьРодителя();
		Организация					= Родитель.ЭлементДерева;
	ИначеЕсли ТипЗнч(ЭлементДерева) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
...
Функция ПолучитьМассивСкладов(ПараметрыПереоформления)
	Если ПараметрыПереоформления.Склад = Справочники.ВиртуальныеСклады.ПустаяСсылка() Тогда
		...
		|			И (ВЫБОР КОГДА &СтруктурноеПодразделение = &ПодразделенияОрганизацийПустаяСсылка
	...
	Иначе
		...
		|ГДЕ
		|	(ВЫБОР КОГДА &СтруктурноеПодразделение = &ПодразделенияОрганизацийПустаяСсылка
	...
	Запрос.УстановитьПараметр("ПодразделенияОрганизацийПустаяСсылка"	, ВСКлиентСерверПереопределяемый.ПолучитьПустуюСсылкуПодразделения());
...
&НаКлиенте
Функция ПолучитьКоличествоЭДВС()
	...
	КоличествоЭДВС = ПолучитьКоличествоЭДВСНаСервере(МассивЭДВС);
	
	Возврат КоличествоЭДВС;
	
КонецФункции

&НаСервере
Функция ПолучитьКоличествоЭДВСНаСервере(МассивЭДВС)
	...
	Запрос.Текст = 
		...
		|	Документ.ЭлектронныйДокументВС КАК ЭлектронныйДокументВС
		|ГДЕ
		|	НЕ ЭлектронныйДокументВС.ПометкаУдаления
...
Процедура Далее(Команда)
	...
	ИначеЕсли Элементы.ГруппаСтраницыПомощника.ТекущаяСтраница = Элементы.ГруппаПараметров Тогда
		...
		Если Не ЕстьОшибки Тогда
			...
			ЗаполнитьДеревоДокументов();
			КоличествоЭДВС = ПолучитьКоличествоЭДВС();
	
			Если КоличествоЭДВС = 0 Тогда
