...
Функция НовыйСписокКоманд(ИдентификаторСтрокиТаблицыОпераций)
	...
	Если РегламентнаяОперацияВыполнена(СтрокаТаблицыОпераций.Состояние) Тогда
		...
		СписокКоманд.Добавить("Проводки", "Показать проводки", , БиблиотекаКартинок.ЖурналПроводок);
		...
		Если СсылкаЗакрытиеМесяца <> Неопределено Тогда
			...
			Если МассивОшибок.Количество() > 0 Тогда
				СписокКоманд.Добавить("Ошибки", "Показать ошибки", , БиблиотекаКартинок.ОперацияВыполненаСОшибками);
			КонецЕсли; 
			...
			Если МассивКомментарий.Количество() > 0 Тогда
				СписокКоманд.Добавить("Комментарии", "Показать комментарии", , БиблиотекаКартинок.СообщениеИнформация);
			КонецЕсли;
	...
	Если МожноВыполнять Тогда
		...
		СписокКоманд.Добавить("Выполнить", "Выполнить операцию", , БиблиотекаКартинок.Провести);
		
		Если РегламентнаяОперацияВыполнена(СтрокаТаблицыОпераций.Состояние) Тогда
			СписокКоманд.Добавить("Отменить", "Отменить операцию", , БиблиотекаКартинок.ОтменаПроведения);
		КонецЕсли;
...
Функция СоздатьИлиПолучитьОбъектДокументаЗакрытиеМесяца(Знач ВидОперации, Знач СсылкаЗакрытияМесяца)
	...
	Иначе
		...
		ОбъектЗакрытиеМесяца.Комментарий = Строка(ВидОперации) + " за " + МесяцНачисленияСтрокой; 
...
Функция ТабДокОтчетОбОшибкахИКомментариях(Знач Ошибки)
	...
	Если Результат.Пустой() Тогда
		...
		ТекстСообщения = НСтр("ru = '[ОшибокКомментариев] при выполнении закрытия месяца не обнаружено.'");
		
		Если Ошибки Тогда
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "[ОшибокКомментариев]", НСтр("ru = 'Ошибок'"));
		Иначе
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "[ОшибокКомментариев]", НСтр("ru = 'Комментариев'"));
		КонецЕсли;
	...
	Если Ошибки Тогда 
		ОбластьШапка.Параметры.Заголовок = "Отчет об ошибках закрытия месяца";
	Иначе
		ОбластьШапка.Параметры.Заголовок = "Отчет по комментариям закрытия месяца";
	КонецЕсли; 		

	ОбластьШапка.Параметры.НазваниеОрганизации = НазваниеОрганизации;
	ОбластьШапка.Параметры.ДатаСоставления = РаботаСДиалогамиКлиентСервер.ДатаКакМесяцПредставление(Объект.ПериодРегистрации);
...
Функция ТабДокВыполненныеОперации()
	...
	ОбластьМакета.Параметры.Период         = Формат(Объект.ПериодРегистрации, "ДФ='MMММ yyyy ""г.""'");
