...
Процедура СоздатьТН(ДанныеОбъекта) Экспорт
	...
	Основание = "Отчет производства за смену №" + ДанныеОтчетаПроизводстваЗаСмену.Номер + " от " + Формат(ДанныеОтчетаПроизводстваЗаСмену.Дата, "ДФ=дд.ММ.гггг");
	...
	Для Каждого СтрокаТаблицы Из ДанныеОбъекта.ТребованияНакладные Цикл
		...
		Если СтрокаТаблицы.Отметка 
			...
			Если ДокументТН.Проведен Тогда
				...
				Исключение
					ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Дата документа %1 не изменена!", 
