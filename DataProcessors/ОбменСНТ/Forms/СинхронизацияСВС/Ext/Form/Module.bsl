...
Процедура Синхронизировать(Команда)
	...
	МассивПрофилейИСЭСФСДатойСинхронизации = Новый Массив; // Для получения входящих СНТ, в связи с вводом параметра LastEventDate
	КоллекцияСтруктурныхЕдиницИСЭСФ = Новый Соответствие;
	
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		Если СтрокаТаблицы.Пометка Тогда
			МассивПрофилейИСЭСФ.Добавить(СтрокаТаблицы.ПрофильИСЭСФ);
			МассивПрофилейИСЭСФСДатойСинхронизации.Добавить(Новый Структура("ПрофильИСЭСФ, ДатаНачалаСинхронизацииВходящихСНТ, ДатаНачалаСинхронизацииИсходящихСНТ", СтрокаТаблицы.ПрофильИСЭСФ, СтрокаТаблицы.ДатаНачалаСинхронизацииВходящихСНТ, СтрокаТаблицы.ДатаНачалаСинхронизацииИсходящихСНТ));
...
Функция СинхронизироватьНаСервере(МассивДанныхПрофилейИСЭСФ, МассивПрофилейИСЭСФСДатойСинхронизации)
	...
	Если ПараметрыЗадания.ЗапускатьФоновоеЗадание Тогда
		...
		Если РежимЗапуска = "СинхронизацияСВС" Тогда
			ИмяПроцедурыПолученияЭДВС = "СНТВызовСервера.ПолучитьНовыеСНТ";
			НаименованиеЗадания = НСтр("ru = 'Синхронизация СНТ'");
	...
	Иначе
		...
		Если РежимЗапуска = "СинхронизацияСВС" Тогда
			СНТВызовСервера.ПолучитьНовыеСНТ(ПараметрыЗадания);			
		Иначе
...
Процедура СинхронизироватьЗавершение(Результат = Неопределено, ДополнительныеПараметры) Экспорт
	...
	Если Параметры.ЗапускатьФоновоеЗадание Тогда
		...
		Если ТипЗнч(РезультатРаботыЗадания) = Тип("Структура") Тогда
			...
			СтруктураОповещений = Новый Структура(СНТКлиентСервер.ИмяСобытияСинхронизацияСНТ());
			РезультатРаботыЗадания.Вставить("ДополнительныеОповещения", СтруктураОповещений);
		КонецЕсли;
				
		СНТКлиентПереопределяемый.ОбработкаОповещенияСНТ_ПроверятьОповещенияФоновогоЗадания(ЭтаФорма, РезультатРаботыЗадания);
		
	Иначе
		СНТКлиент.ОповеститьФормы(СНТКлиентСервер.ИмяСобытияЗаписьСНТ());
		СНТКлиент.ОповеститьФормы(СНТКлиентСервер.ИмяСобытияСинхронизацияСНТ());
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПрофилейДляСинхронизации()
	...
	|	ЕСТЬNULL(ПараметрыМетодовИСЭСФВходящихСНТ.ЗначениеПараметра, &ДатаПоУмолчанию) КАК ДатаНачалаСинхронизацииВходящихСНТ,
	|	ЕСТЬNULL(ПараметрыМетодовИСЭСФИсходящихСНТ.ЗначениеПараметра, &ДатаПоУмолчанию) КАК ДатаНачалаСинхронизацииИсходящихСНТ
	|ИЗ
	|	ВТ_ПрофилиСинхронизации КАК ПрофилиСинхронизации
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыМетодовИСЭСФ КАК ПараметрыМетодовИСЭСФВходящихСНТ
	|		ПО (ПрофилиСинхронизации.СтруктурнаяЕдиница = ПараметрыМетодовИСЭСФВходящихСНТ.СтруктурнаяЕдиница
	|				И ПараметрыМетодовИСЭСФВходящихСНТ.ИмяМетода = &ИмяМетода     // дата начала синхронизации
	|				И ПараметрыМетодовИСЭСФВходящихСНТ.НаправлениеЭСФ = &Направление
	|				И ПараметрыМетодовИСЭСФВходящихСНТ.ИмяПараметра = &ИмяПараметра)
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыМетодовИСЭСФ КАК ПараметрыМетодовИСЭСФИсходящихСНТ
	|		ПО (ПрофилиСинхронизации.СтруктурнаяЕдиница = ПараметрыМетодовИСЭСФИсходящихСНТ.СтруктурнаяЕдиница
	|				И ПараметрыМетодовИСЭСФИсходящихСНТ.ИмяМетода = &ИмяМетода     // дата начала синхронизации
	|				И ПараметрыМетодовИСЭСФИсходящихСНТ.НаправлениеЭСФ = &Направление
	|				И ПараметрыМетодовИСЭСФИсходящихСНТ.ИмяПараметра = &ИмяПараметра)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если РежимЗапуска = "СинхронизацияСВС" Тогда
		Запрос.УстановитьПараметр("ИмяМетода",	"QUERYSNTUPDATES");
		Запрос.УстановитьПараметр("Направление", Перечисления.НаправленияЭСФ.ПустаяСсылка());
		Запрос.УстановитьПараметр("ДатаПоУмолчанию", Дата(2019, 12, 1, 0, 0, 0));
	Иначе
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = СНТКлиентСервер.ИмяСобытияСинхронизацияСНТ() Тогда
...
Процедура УправлениеФормой()
	...
	Если РежимЗапуска = "ПолучениеИДТоваров" Тогда
		...
		Элементы.ТаблицаДатаНачалаСинхронизацииСНТ.Заголовок = "Начальная дата получения ID товаров";
