...
Процедура ИзменитьСтатусСНТ (Команда)
	...
	ЭСФКлиент.ОпределитьНеобходимостьВводаДанныхИСЭСФ(КоллекцияСгруппированныхСНТ, ИзменитьСтатусСНТЗавершение);
...
Функция ВозможноВыполнитьДействие()
	...
	Если Параметры.Действие = "АннуллироватьПодтверждение" Тогда
		РезультатЗапросаАннулирования = СНТСервер.ПроверитьВозможностьАннулироватьПодтверждение(Параметры.МассивСНТ);
		
		Если РезультатЗапросаАннулирования.ЗапрещающиеОшибки.Количество() <> 0 Тогда
			
			Возможно = Ложь;
			ЭСФКлиентСервер.СообщитьПользователю(РезультатЗапросаАннулирования.ЗапрещающиеОшибки[0]);
			
		КонецЕсли;
		
	КонецЕсли;
	
	// Проверить количество СНТ.
	...
	// Проверить, что во всех СНТ указана одинаковая организация и подразделение организации. 
	Если Возможно Тогда
		
		Выборка = РезультатЗапроса.Выбрать();
		
		// Проверить, что во всех СНТ указана одинаковая организация.
		Выборка.Сбросить();
		Выборка.Следующий();
		...
		ТипПервогоСНТ = Выборка.ТипСНТ;
		ПоставщикМалаяТорговаяТочка = Выборка.ПоставщикМалаяТорговаяТочка;
		ПоставщикРозничныйРеализатор = Выборка.ПоставщикРозничныйРеализатор;
		ДатаСвязанногоСНТ = Выборка.ДатаСвязанногоСНТ;
		ЗаполненаТЧТоварыВС = Выборка.ТоварыВС;
		ВидПеремещенияСНТ = Выборка.ВидПеремещения;
		
		СоответствиеВозможностиДействий = СНТСервер.РазрешенныеДействияПоСтатусамСНТ(НаправлениеПервогоСНТ, ВидОперацииПервогоСНТ, Выборка, ВидПеремещенияСНТ);
		Выборка.Сбросить();
		Если СоответствиеВозможностиДействий.Количество() > 0 Тогда 
			Пока Выборка.Следующий() Цикл
				...
				//+
				// Проверить, что во всех СНТ указано одинаковое подразделение организации.
	...
	ТребуемоеНаправление = НаправлениеДляДействия(Действие, НаправлениеПервогоСНТ, ВидОперацииПервогоСНТ, ПоставщикМалаяТорговаяТочка, ПоставщикРозничныйРеализатор, ТипПервогоСНТ, ЗаполненаТЧТоварыВС, ДатаСвязанногоСНТ);
...
&НаСервере
Функция НаправлениеДляДействия(Знач Действие, Знач НаправлениеСНТ = Неопределено, ВидОперации = Неопределено, ПоставщикМалаяТорговаяТочка = Ложь, ПоставщикРозничныйРеализатор = Ложь, ТипСНТ = Неопределено, ЗаполненаТЧТоварыВС = Ложь, ДатаСвязанногоСНТ)
	...
	ИначеЕсли Действие = СНТКлиентСервер.ДействиеПодтверждение() Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ ВидОперации = Перечисления.ВидыОперацийСНТ.Перемещение ИЛИ ВидОперации = Перечисления.ВидыОперацийСНТ.ПеремещениеМеждуФилиалами
			//ИЛИ (ПоставщикМалаяТорговаяТочка И ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров)
			ИЛИ (ПоставщикМалаяТорговаяТочка И ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И ДатаСвязанногоСНТ >= СНТСерверПереопределяемый.ДатаИзмененийМТТ())
			ИЛИ (ПоставщикРозничныйРеализатор И ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И ЗаполненаТЧТоварыВС И ДатаСвязанногоСНТ < СНТСерверПереопределяемый.ДатаВводаМТТ())Тогда
			Направление = Перечисления.НаправленияСНТ.Исходящий;
		КонецЕсли;	
...
Функция ДанныеДляПроверкиВозможностиВыполненияДействия()
	...
	|	СНТ.ПоставщикРозничныйРеализатор КАК ПоставщикРозничныйРеализатор,
	|	СНТ.ПоставщикФизическоеЛицо КАК ПоставщикФизическоеЛицо,
	|	СНТ.ПолучательМалаяТорговаяТочка КАК ПолучательМалаяТорговаяТочка,
	|	СНТ.ПоставщикМалаяТорговаяТочка КАК ПоставщикМалаяТорговаяТочка,
	...
	//Дата нужна для СНТ на возврат Розничного реализатора у которых первичная СНТ датой выписки до 5.02.2022	
	|	ЕСТЬNULL(СНТ.СвязанныйСНТ.Дата, ДАТАВРЕМЯ(1,1,1)) КАК ДатаСвязанногоСНТ,
...
Процедура УстановитьЗаголовокФормы()
	...
	ИначеЕсли Действие = СНТКлиентСервер.ДействиеАннуллироватьПодтверждение() Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru = 'Аннулировать подтверждение сопроводительных накладных на товары'");	
...
Процедура УстановитьЗаголовокКнопкиВыполнитьДействие()
	...
	ИначеЕсли Действие = СНТКлиентСервер.ДействиеАннуллироватьПодтверждение() Тогда
		...
		Элементы.ФормаВыполнитьДействие.Заголовок = НСтр("ru = 'Аннулировать подтверждение СНТ в ИС ЭСФ'");	
