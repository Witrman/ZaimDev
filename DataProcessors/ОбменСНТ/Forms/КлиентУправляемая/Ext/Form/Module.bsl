...
Функция ИзменитьСтатусыСНТ(Знач Действие, Знач КоллецияДляИзмененияСтатусов, Знач ДанныеПрофиляИСЭСФ, Знач ДанныеКлючаЭЦП, Знач УникальныйИдентификаторФормыВладельца, Знач ЗапускатьФоновоеЗадание) Экспорт	
	...
	Если ДанныеПрофиляИСЭСФ.Свойство("ОписаниеСертификата") И Действие = СНТКлиентСервер.ДействиеПодтверждение() Тогда
		...
		КонецВхож = СтрНайти(ОписаниеСертификата, "ИИН владельца:");
		ТолькоФИО = Лев(ОписаниеСертификата, КонецВхож);
		ТолькоФИО = СтрЗаменить(ТолькоФИО, "ФИО владельца: ", "");
		ФИОСубъектаДляПодтверждения = Лев(ТолькоФИО, СтрДлина(ТолькоФИО) - 2);
