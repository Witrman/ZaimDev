	...
	Попытка
		...
		Для Каждого ДанныеРегистра Из СпособыСверткиОбъектовМетаданных Цикл
			...
			ИначеЕсли ДанныеРегистра.ТипОбъектаМД = "РегистрБухгалтерии" Тогда
				Если ДанныеРегистра.ОбъектМД = "Типовой" Тогда
					СформироватьДокументыВводаОстатковПоРегиструБухгалтерииТиповой(ДанныеРегистра, ПараметрыОбработки);
				ИначеЕсли ДанныеРегистра.ОбъектМД = "Налоговый" Тогда
					СформироватьДокументыВводаОстатковПоРегиструБухгалтерииНалоговый(ДанныеРегистра, ПараметрыОбработки);	
				КонецЕсли;
			КонецЕсли;
...
Процедура СформироватьДокументыВводаОстатковПоРегистрамСведений(СтрокаСписка, Параметры)
	...
	// По регистрам КоэффициентыИндексацииЗаработка и РаботникиОрганизаций необходимы записи за последний год.
	Если СтрокаСписка.ОбъектМД = "РаботникиОрганизаций"
	 ИЛИ СтрокаСписка.ОбъектМД = "КоэффициентыИндексацииЗаработка" Тогда
		ПериодСвертки = НачалоГода(Параметры.ПериодСвертки - 1);
	Иначе
		ПериодСвертки = Параметры.ПериодСвертки;	
	КонецЕсли;	
	
	Запрос.УстановитьПараметр("ДатаОст", ПериодСвертки-1);
	Запрос.УстановитьПараметр("ПериодЗаписи", НачалоДня(ПериодСвертки));
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("Запрос", Запрос);
	СтруктураПараметров.Вставить("ТекущийРегистр", ТекущийРегистр);
	СтруктураПараметров.Вставить("ПериодСвертки", ПериодСвертки);
...
// Процедура формирует документы ввода остатков по регистру бухгалтерии Типовой.
//
Процедура СформироватьДокументыВводаОстатковПоРегиструБухгалтерииТиповой(СтрокаСписка, Параметры)
	
	// При внесении изменений в данную процедуру нужно не забыть внести их в 
	// процедуру СформироватьДокументыВводаОстатковПоРегиструБухгалтерииНалоговый().
	
	Если НЕ СтрокаСписка.СпособСвертки = "Свернуть" Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ РегистрыБухгалтерии.Типовой.ПолучитьИспользованиеИтогов() Тогда
		Возврат;
	КонецЕсли;
	
	// Берем остатки только по счетам самого нижнего уровня.
	//
	// Например: Если есть счета 1300, 1340, 1341, 1342, 1343, то остатки берутся только по 1341, 1342, 1343.
	//
	// Для этого выбираем счета, у которые не являются родителями для других счетов,
	// т.к. если счет является родителем для другого счета, то он не является счетом самого нижнего уровня.
	
	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Типовой1.Ссылка КАК Счет
	|ПОМЕСТИТЬ СчетаНижнегоУровня
	|ИЗ
	|	ПланСчетов.Типовой КАК Типовой1
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Типовой КАК Типовой2
	|		ПО Типовой1.Ссылка = Типовой2.Родитель
	|ГДЕ
	|	Типовой2.Ссылка ЕСТЬ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет
	|;
	...
	|	ТиповойОстатки.СуммаРазвернутыйОстатокДт КАК СуммаРазвернутыйОстатокДт,
	|	ТиповойОстатки.СуммаРазвернутыйОстатокКт КАК СуммаРазвернутыйОстатокКт,
	|	ТиповойОстатки.ВалютнаяСуммаОстаток КАК ВалютнаяСуммаОстаток,
	|	ТиповойОстатки.ВалютнаяСуммаОстатокДт КАК ВалютнаяСуммаОстатокДт,
	|	ТиповойОстатки.ВалютнаяСуммаОстатокКт КАК ВалютнаяСуммаОстатокКт,
	|	ТиповойОстатки.ВалютнаяСуммаРазвернутыйОстатокДт КАК ВалютнаяСуммаРазвернутыйОстатокДт,
	|	ТиповойОстатки.ВалютнаяСуммаРазвернутыйОстатокКт КАК ВалютнаяСуммаРазвернутыйОстатокКт,
	|	ТиповойОстатки.КоличествоОстаток КАК КоличествоОстаток,
	|	ТиповойОстатки.КоличествоОстатокДт КАК КоличествоОстатокДт,
	|	ТиповойОстатки.КоличествоОстатокКт КАК КоличествоОстатокКт,
	|	ТиповойОстатки.КоличествоРазвернутыйОстатокДт КАК КоличествоРазвернутыйОстатокДт,
	|	ТиповойОстатки.КоличествоРазвернутыйОстатокКт КАК КоличествоРазвернутыйОстатокКт
	|ИЗ
	|	РегистрБухгалтерии.Типовой.Остатки(
	|			&ДатаСвертки,
	|			Счет В
	|				(ВЫБРАТЬ
	|					СчетаНижнегоУровня.Счет
	|				ИЗ
	|					СчетаНижнегоУровня),
	|			,
	|			&ТекстОтбораПоОрганизациям) КАК ТиповойОстатки
	|ИТОГИ
	|	СУММА(СуммаОстаток),
	...
	Если Параметры.ТолькоВыбранныеОрганизации Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстОтбораПоОрганизациям", "Организация В (&ВыбранныеОрганизации)");
	Иначе
		Запрос.УстановитьПараметр("ТекстОтбораПоОрганизациям", Истина); // Будут выбраны остатки по всем организациям.
	КонецЕсли;
	...
	Пока ВыборкаПоОрганизации.Следующий() Цикл
		...
		Пока ВыборкаПоСчетам.Следующий() Цикл
			...
			Если (НЕ ВыборкаПоСчетам.СуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.ВалютнаяСуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоОстаток = 0) 
				...
				ИЛИ (НЕ ВыборкаПоСчетам.СуммаРазвернутыйОстатокКт = 0) ИЛИ (НЕ ВыборкаПоСчетам.ВалютнаяСуммаРазвернутыйОстатокКт = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоРазвернутыйОстатокКт = 0) Тогда
				...
				ДокументОбъект.Содержание = "Ввод остатков по регистрам бухгалтерии: Типовой. Счет: " + ВыборкаПоСчетам.Счет.Код;
				...
				НаборЗаписей = ДвиженияДокумента.Типовой;
				...
				Пока ВыборкаДетали.Следующий() Цикл
					...
					Если ((ВыборкаПоСчетам.Счет.Вид = ВидСчета.Активный) ИЛИ (ВыборкаПоСчетам.Счет.Вид = ВидСчета.АктивноПассивный))
						...
						Если НЕ ВыборкаПоСчетам.Счет.Забалансовый Тогда
							НоваяЗапись.СчетКт = ПланыСчетов.Типовой.Вспомогательный;
						КонецЕсли;
						...
						НоваяЗапись.СтруктурноеПодразделениеДт = ВыборкаДетали.СтруктурноеПодразделение;
					...
					Если ((ВыборкаПоСчетам.Счет.Вид = ВидСчета.Пассивный) ИЛИ (ВыборкаПоСчетам.Счет.Вид = ВидСчета.АктивноПассивный))
						...
						Если НЕ ВыборкаПоСчетам.Счет.Забалансовый Тогда
							НоваяЗапись.СчетДт = ПланыСчетов.Типовой.Вспомогательный;
						КонецЕсли;
						...
						НоваяЗапись.СтруктурноеПодразделениеКт = ВыборкаДетали.СтруктурноеПодразделение;
				...
				ДокументОбъект.СуммаОперации = НаборЗаписей.Итог("Сумма");
				ЗаписатьДанные(ДокументОбъект);
...
//
Процедура СформироватьДокументыВводаОстатковПоРегиструБухгалтерииНалоговый(СтрокаСписка, Параметры)
		
	// При внесении изменений в данную процедуру нужно не забыть внести их в 
	// процедуру СформироватьДокументыВводаОстатковПоРегиструБухгалтерииТиповой().
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Налоговый1.Ссылка КАК Счет
	|ПОМЕСТИТЬ СчетаНижнегоУровня
	|ИЗ
	|	ПланСчетов.Налоговый КАК Налоговый1
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Налоговый КАК Налоговый2
	|		ПО Налоговый1.Ссылка = Налоговый2.Родитель
	|ГДЕ
	|	Налоговый2.Ссылка ЕСТЬ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет
	|;
	...
	|					СчетаНижнегоУровня),
	|			,
	|			&ТекстОтбораПоОрганизациям) КАК НалоговыйОстатки
	|ИТОГИ
	|	СУММА(СуммаОстаток),
	|	СУММА(СуммаОстатокДт),
	|	СУММА(СуммаОстатокКт),
	...
	Пока ВыборкаПоОрганизации.Следующий() Цикл
		...
		Пока ВыборкаПоСчетам.Следующий() Цикл
			...
			Если (НЕ ВыборкаПоСчетам.СуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоОстаток = 0) 
				ИЛИ (НЕ ВыборкаПоСчетам.СуммаРазвернутыйОстатокДт = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоРазвернутыйОстатокДт = 0)
				ИЛИ (НЕ ВыборкаПоСчетам.СуммаРазвернутыйОстатокКт = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоРазвернутыйОстатокКт = 0) Тогда
				...
				ДокументОбъект.СпособЗаполнения = "Вручную"; 
				ДокументОбъект.Комментарий = "*Сформирован обработкой свертки базы. " + Формат(ТекущаяДатаСеанса(),"ДФ=dd.MM.yyyy");
				ДокументОбъект.Содержание = "Ввод остатков по регистрам бухгалтерии: Налоговый. Счет: " + ВыборкаПоСчетам.Счет.Код;
				...
				Пока ВыборкаДетали.Следующий() Цикл
					...
					Если ((ВыборкаПоСчетам.Счет.Вид = ВидСчета.Активный) ИЛИ (ВыборкаПоСчетам.Счет.Вид = ВидСчета.АктивноПассивный))
						...
						Если НЕ ВыборкаПоСчетам.Счет.Забалансовый Тогда
							НоваяЗапись.СчетКт = ПланыСчетов.Налоговый.Вспомогательный;
						КонецЕсли;
						...
						НоваяЗапись.ВидУчетаДт = ВыборкаДетали.ВидУчета;
						НоваяЗапись.СтруктурноеПодразделениеДт = ВыборкаДетали.СтруктурноеПодразделение;
						НоваяЗапись.КоличествоДт = ВыборкаДетали.КоличествоОстатокДт;
						НоваяЗапись.Сумма = ВыборкаДетали.СуммаОстатокДт;
					...
					Если ((ВыборкаПоСчетам.Счет.Вид = ВидСчета.Пассивный) ИЛИ (ВыборкаПоСчетам.Счет.Вид = ВидСчета.АктивноПассивный))
						...
						Если НЕ ВыборкаПоСчетам.Счет.Забалансовый Тогда
							НоваяЗапись.СчетДт = ПланыСчетов.Налоговый.Вспомогательный;
						КонецЕсли;
						...
						НоваяЗапись.ВидУчетаКт = ВыборкаДетали.ВидУчета;
						НоваяЗапись.СтруктурноеПодразделениеКт = ВыборкаДетали.СтруктурноеПодразделение;
						НоваяЗапись.КоличествоКт = ВыборкаДетали.КоличествоОстатокКт;
						НоваяЗапись.Сумма = ВыборкаДетали.СуммаОстатокКт;
...
Функция СформироватьШапкуДокументаВводаОстатков(ТекущийРегистр, ДатаСвертки, ТипРегистра, Организация = Неопределено)
	...
	ДокументОстатков.Автор = Пользователи.ТекущийПользователь();
...
// 
Процедура АктивизироватьВводОстатков(ПериодСвертки, АдресХранилища, ЗаписатьСтатус) Экспорт
...
Процедура СвернутьИнформационнуюБазу(ПараметрыОбработки, АдресХранилища) Экспорт
	...
	Попытка
		...
		Для Каждого ТекСтрока из СпособыСверткиОбъектовМетаданных Цикл
			...
			// У регистров КоэффициентыИндексацииЗаработка и РаботникиОрганизаций
			// могут остаться движения, т.к. по ним сохранены записи за последний год. 
			Если ТекСтрока.ОбъектМД = "РаботникиОрганизаций"
			 ИЛИ ТекСтрока.ОбъектМД = "КоэффициентыИндексацииЗаработка" Тогда
				ТекСтрока.ОсталисьДвиженияПоРегистру = Истина;
			КонецЕсли;
		...
		АктивизироватьВводОстатков(ПараметрыОбработки.ПериодСвертки, АдресХранилища, Истина);
		
		// По регистрам КоэффициентыИндексацииЗаработка и РаботникиОрганизаций
		// документ ОперацияБух был создан прошлым годом,
		// т.к. по ним необходимы записи за последний год.		
		АктивизироватьВводОстатков(НачалоГода(ПараметрыОбработки.ПериодСвертки - 1), АдресХранилища, Ложь);
...
Процедура УдалитьДвиженияПоПодчиненномуРегистру(ПараметрыРегистра, ПараметрыОбработки, ПериодическийРегистрСведений = Истина)
	...
	Пока Истина Цикл
		...
		// По регистрам КоэффициентыИндексацииЗаработка и РаботникиОрганизаций необходимы записи за последний год.
		Если ПараметрыРегистра.ОбъектМД = "РаботникиОрганизаций" 
		 ИЛИ ПараметрыРегистра.ОбъектМД = "КоэффициентыИндексацииЗаработка" Тогда
			Запрос.УстановитьПараметр("ПериодСвертки", НачалоГода(ПараметрыОбработки.ПериодСвертки - 1));	
		Иначе
			Запрос.УстановитьПараметр("ПериодСвертки", ПараметрыОбработки.ПериодСвертки);	
		КонецЕсли;
...
Процедура ПометитьДокументыНаУдаление(ТипДокумента, ПараметрыОбработки)
	...
	// Документы НачислениеЗарплатыРаботникамОрганизаций необходимы за последние 12 месяцев.
	// Движения документов не нужны, нужны только данные самих документов.
	Если ТипДокумента = "НачислениеЗарплатыРаботникамОрганизаций" Тогда
		// Если свертка выполняется на начало 2016 года, то ДатаСвертки будет 31.12.2014 23:59:59. 
		ДатаСвертки = НачалоГода(ПараметрыОбработки.ПериодСвертки - 1) - 1;	
	Иначе
		// Если свертка выполняется на начало 2016 года, то ДатаСвертки будет 31.12.2015 23:59:59.
		ДатаСвертки = ПараметрыОбработки.ПериодСвертки - 1;	
	КонецЕсли;
	
	Запрос.УстановитьПараметр("Дата", ДатаСвертки);
	Запрос.УстановитьПараметр("ВыбранныеОрганизации", ПараметрыОбработки.Организации);
...
// Функция проверяет существование у регистра измерения "Организация" или "Налогоплательщик"
//
Функция ЕстьИзмерениеОрганизация(ДанныеРегистра)
	...
	Если ТекущийРегистр.Измерения.Найти("Организация") <> Неопределено Тогда
		СтруктураВозврата.НаличиеИзмерения = Истина;
		СтруктураВозврата.ИмяИзмерения = "Организация";
	ИначеЕсли ТекущийРегистр.Измерения.Найти("Налогоплательщик") <> Неопределено Тогда
		СтруктураВозврата.НаличиеИзмерения = Истина;
		СтруктураВозврата.ИмяИзмерения = "Налогоплательщик";
	КонецЕсли;
...
Функция ЕстьРеквизитОрганизация(ТипДокумента)
	...
	Если ТипДокумента.Реквизиты.Найти("Организация") <> Неопределено Тогда
		СтруктураВозврата.НаличиеИзмерения = Истина;
		СтруктураВозврата.ИмяИзмерения = "Организация";
	ИначеЕсли ТипДокумента.Реквизиты.Найти("Налогоплательщик") <> Неопределено Тогда
		СтруктураВозврата.НаличиеИзмерения = Истина;
		СтруктураВозврата.ИмяИзмерения = "Налогоплательщик";
	КонецЕсли;
...
Процедура СформироватьОтчетПроверкиОстатков(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ПараметрыОтчета.ТипДанных = СверткаИнформационнойБазыКлиентСервер.РегистрБухгалтерииТиповой() Тогда
		СформироватьОтчетПоРегиструБухгалтерииТиповой(ПараметрыОтчета, АдресХранилища);
	ИначеЕсли ПараметрыОтчета.ТипДанных = СверткаИнформационнойБазыКлиентСервер.РегистрБухгалтерииНалоговый() Тогда
		СформироватьОтчетПоРегиструБухгалтерииНалоговый(ПараметрыОтчета, АдресХранилища);
	Иначе
...
Процедура СформироватьЗапросПоМетаданным(ДанныеРегистра, ПараметрыОтчета)
	...
	Для Каждого ТекИзмерение Из ТекущийРегистр.Измерения Цикл
		Если ОтборПоОрганизациям Тогда
			Если ТекИзмерение.Имя = "Организация" или ТекИзмерение.Имя = "Налогоплательщик" Тогда
	...
	// По регистрам КоэффициентыИндексацииЗаработка и РаботникиОрганизаций необходимы записи за последний год.
	Если ДанныеРегистра.Наименование = "РаботникиОрганизаций"
	 ИЛИ ДанныеРегистра.Наименование = "КоэффициентыИндексацииЗаработка" Тогда
		ПериодСвертки = НачалоГода(ПараметрыОтчета.ПериодСвертки - 1);
	Иначе
		ПериодСвертки = ПараметрыОтчета.ПериодСвертки;	
	КонецЕсли;	
	
	Запрос.УстановитьПараметр("ПериодСвертки", ПериодСвертки);
	Запрос.УстановитьПараметр("ПериодСверткиВиртуальнойТаблицы", ПериодСвертки - 1);
...
Процедура УстановитьДатуЗапретаИзмененияДанных(ПараметрыОбработки)
	...
	Если Не ПараметрыОбработки.ТолькоВыбранныеОрганизации Тогда
			...
			СообщениеЖурналаРегистрации = НСтр("ru = 'Установлена новая общая дата запрета изменения данных: '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + ДатаЗапрета;
	...
	Иначе
		...
		Для Каждого Орг Из ПараметрыОбработки.Организации Цикл
			...
			Если Не ДатаЗапретаДляВсехПользователей Тогда 
				...
				НоваяЗапись.Раздел = ДатыЗапретаИзмененияБК.РазделБухгалтерскийУчет();
				НоваяЗапись.Пользователь = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей;
				НоваяЗапись.ДатаЗапрета = ДатаЗапрета;
				ОбновитьДатыЗапрета = Истина;
			КонецЕсли;
			...
			Если ОбновитьДатыЗапрета Тогда
				...
				СообщениеЖурналаРегистрации = НСтр("ru = 'Установлена новая дата запрета изменения данных для организации '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) 
...
Процедура СформироватьОтчетПоРегиструБухгалтерииТиповой(ПараметрыОтчета, АдресХранилища)
	
	СхемаКомпоновкиДанных = Обработки.СверткаИнформационнойБазы.ПолучитьМакет("ПроверкаРегистраБухгалтерииТиповой");
...
Процедура СформироватьОтчетПоРегиструБухгалтерииНалоговый(ПараметрыОтчета, АдресХранилища)
	
	СхемаКомпоновкиДанных = Обработки.СверткаИнформационнойБазы.ПолучитьМакет("ПроверкаРегистраБухгалтерииНалоговый");
	НК = Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновкиДанных);
	Настройки = СхемаКомпоновкиДанных.НастройкиПоУмолчанию;
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
