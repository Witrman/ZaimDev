...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СоздатьРезервнуюКопию.Видимость = Ложь;
	Элементы.ПанельАвторизацияИБ.Видимость = Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	Объект.СоздатьРезервнуюКопию           = Ложь;
...
Процедура ТипДанныхПриИзменении(Элемент)
	...
	Если Объект.ТипДанных = СверткаИнформационнойБазыКлиентСервер.РегистрБухгалтерииТиповой() Тогда
		Элементы.ГруппаРегистрБухгалтерииТиповой.Видимость = Истина;
		Элементы.ГруппаРегистрБухгалтерииНалоговый.Видимость = Ложь;
		Элементы.ГруппаРегистрыНакопления.Видимость = Ложь;
		Элементы.ГруппаРегистрыСведений.Видимость = Ложь;			
	ИначеЕсли Объект.ТипДанных = СверткаИнформационнойБазыКлиентСервер.РегистрБухгалтерииНалоговый() Тогда
		Элементы.ГруппаРегистрБухгалтерииТиповой.Видимость = Ложь;
		Элементы.ГруппаРегистрБухгалтерииНалоговый.Видимость = Истина;
		Элементы.ГруппаРегистрыНакопления.Видимость = Ложь;
		Элементы.ГруппаРегистрыСведений.Видимость = Ложь;					
	ИначеЕсли Объект.ТипДанных = СверткаИнформационнойБазыКлиентСервер.РегистрыСведенийИНакопления() Тогда
		Элементы.ГруппаРегистрБухгалтерииТиповой.Видимость = Ложь;
		Элементы.ГруппаРегистрБухгалтерииНалоговый.Видимость = Ложь;
		Элементы.ГруппаРегистрыНакопления.Видимость = Истина;
		Элементы.ГруппаРегистрыСведений.Видимость = Истина;		
	Иначе
		ВызватьИсключение НСтр("ru = 'Необрабатываемое значение переключателя ""Тип данных"".'");	
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ДобавитьОформляемоеПоле(ЭлементУО.Поля, "СпособыСверткиРегистровБухгалтерии");
	ДобавитьОформляемоеПоле(ЭлементУО.Поля, "СпособыСверткиРегистровНакопления");
	ДобавитьОформляемоеПоле(ЭлементУО.Поля, "СпособыСверткиРегистровСведений");
...
Процедура ПередОткрытиемСтраницы()
	...
	ИначеЕсли Объект.ТекущийЭтапСвертки = 3 Тогда
		...
		ЭлементыОтбора = ДокументыВводаОстатков.КомпоновщикНастроек.Настройки.Отбор.Элементы[1].ПравоеЗначение;
		ЭлементыОтбора.Очистить();
		ЭлементыОтбора.Добавить(Объект.ПериодСвертки);
		
		// Остатки по регистрам КоэффициентыИндексацииЗаработка и РаботникиОрганизаций сворачиваются прошлым годом,
		// т.к. по этим регистрам нужны записи за последние 12 месяцев.
		ЭлементыОтбора.Добавить(НачалоГода(Объект.ПериодСвертки - 1));
	...
	ИначеЕсли Объект.ТекущийЭтапСвертки = 4 Тогда
		...
		Объект.ПоказательВР = Истина;
		Объект.ПоказательНУ = Истина;
		Объект.ПоказательПР = Истина;
		Объект.ТипДанных = СверткаИнформационнойБазыКлиентСервер.РегистрБухгалтерииТиповой();
...
Функция ПроверитьВозможностьСозданияРезервнойКопии()
	...
	Если НЕ Файл.Существует() ИЛИ НЕ Файл.ЭтоКаталог() Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите существующий каталог для сохранения резервной копии ИБ.'"),,"Объект.ИмяКаталогаРезервнойКопииИБ");
		Возврат Ложь;
	КонецЕсли;
...
#Область ДополнительныеСлужебныеПроцедурыИФункции
