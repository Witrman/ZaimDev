...
Процедура  СформироватьПлатежныеПорученияНаСервере(РежимВвода = Истина, ДопПараметры)
	...
	//Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаСтраницы.ПодчиненныеЭлементы.ГруппаПлатежныеПоручения;
	
	//УстановитьОтборСтрокПлатежныхПоручений(ЭтаФорма);
...
Процедура ЗаполнитьДанныеПоПериодамНаСервере()
	...
	Если  ПолучениеОстатковНаДату = 1 Тогда
		Если НЕ (ЗначениеЗаполнено(НачПериода) И ЗначениеЗаполнено(КонПериода)) Тогда
