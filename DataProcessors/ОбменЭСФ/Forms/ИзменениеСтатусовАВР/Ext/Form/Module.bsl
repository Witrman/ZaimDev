...
Процедура ИзменитьСтатусАВР (Команда)
	...
	ЭСФКлиент.ОпределитьНеобходимостьВводаДанныхИСЭСФ(КоллекцияСгруппированныхАВР, ИзменитьСтатусАВРЗавершение,, Действие);
...
Процедура ИзменитьСтатусАВРЗавершение(ДанныеПрофилейИСЭСФ, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтрокаТаблицы Из СписокАВР Цикл
		...
		Если ДанныеКлючаЭЦП.Свойство("ДолжностьПодписывающего") Тогда
			ДанныеДляИзмененияСтатусовПоАВР.Вставить("ДолжностьПодписывающего", ДанныеКлючаЭЦП.ДолжностьПодписывающего);
		Иначе
			ДанныеДляИзмененияСтатусовПоАВР.Вставить("ДолжностьПодписывающего", Неопределено);
		КонецЕсли;
	...
	Если КоллекцияРезультатовИзмененияСтатусов <> Неопределено Тогда
		...
		Если Параметры.ЗапускатьФоновоеЗадание Тогда
			...
			Если Действие = "Подтверждение" Тогда
				
				Если ТипЗнч(КоллекцияРезультатовИзмененияСтатусов) = Тип("Структура") Тогда
					КоллекцияРезультатовИзмененияСтатусов.Вставить("ТекстСообщения", НСтр("ru = 'Подтверждение документов из ИС ЭСФ'"));
				КонецЕсли;
				
			ИначеЕсли Действие = "Отклонение" Тогда
				
				Если ТипЗнч(КоллекцияРезультатовИзмененияСтатусов) = Тип("Структура") Тогда
					КоллекцияРезультатовИзмененияСтатусов.Вставить("ТекстСообщения", НСтр("ru = 'Отклонение документов из ИС ЭСФ'"));
				КонецЕсли;
				
			ИначеЕсли Действие = "Расторжение" Тогда	
				
				Если ТипЗнч(КоллекцияРезультатовИзмененияСтатусов) = Тип("Структура") Тогда
					КоллекцияРезультатовИзмененияСтатусов.Вставить("ТекстСообщения", НСтр("ru = 'Расторжение документов из ИС ЭСФ'"));
				КонецЕсли;
				
			ИначеЕсли Действие = "Отзыв" Тогда
