...
Процедура СохранитьНастройки(Отказ = Ложь)
	...
	Если ИспользоватьОбменЭСФЧерезAPI И НЕ ЗначениеЗаполнено(ВерсияКриптобиблиотекиБД) 
		И НЕ (ЗначениеЗаполнено(ВерсияКомпонентыНЦА) И ОткрыватьСессиюСПодписью И ИспользоватьКомпонентуНЦАДляПодписиДокументов) Тогда
		Отказ = Истина;
		ТекстИсключения = НСтр("ru = 'Прямой обмен данными с ИС ЭСФ невозможен, так как не установлены средства криптографии.'") + Символы.ПС + 
			...
			| - установить компоненту для работы со средствами криптографии НУЦ РК и включить настройки открытия сессии и подписи документов при помощи данных средств;
