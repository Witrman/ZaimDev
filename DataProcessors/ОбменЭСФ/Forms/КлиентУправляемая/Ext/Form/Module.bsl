...
&НаКлиенте
Функция УдалосьПодключитьКриптопровайдерНЦАНаКлиенте(Версия = "", РежимТишины = Ложь, ТолькоПодключение = Ложь) Экспорт
...
&НаКлиенте
Функция УдалосьПодключитьКриптопровайдерНЦАНаСервере(Версия = "", РежимТишины = Ложь, ТолькоПодключение = Ложь) Экспорт
	
	Подключается = КриптографияВызовСервера.УдалосьПодключитьКриптопровайдерWSCryptoНаСервере(Версия, РежимТишины);
...
&НаКлиенте
Функция КриптопровайдерПодключается(Версия = "", РежимТишины = Ложь, МестоВыполненияОпераций = Неопределено, ТолькоПодключение = Ложь, ПроверятьНЦА = Неопределено) Экспорт
	...
	ИспользоватьПодписьНовойКомпоненты = ЭСФВызовСервера.ИспользоватьПодписьНовойКомпоненты();
	
	Если ПроверятьНЦА = Истина Тогда
		// Проверка подключения WSCrypto в требуемом контексте.
		Если ВыполнятьКриптографическиеОперацииНаКлиенте Тогда
			Подключается = УдалосьПодключитьКриптопровайдерНЦАНаКлиенте(Версия, РежимТишины, ТолькоПодключение);
		Иначе
			Подключается = УдалосьПодключитьКриптопровайдерНЦАНаСервере(Версия, РежимТишины);
		КонецЕсли;
	ИначеЕсли ПроверятьНЦА = Ложь Тогда
	...
	Иначе
		...
		Если ИспользоватьОткрытиеСессииСПодписью ИЛИ ИспользоватьПодписьНовойКомпоненты Тогда
			// Проверка подключения WSCrypto в требуемом контексте.
			Если ВыполнятьКриптографическиеОперацииНаКлиенте Тогда
				Подключается = УдалосьПодключитьКриптопровайдерНЦАНаКлиенте(Версия, РежимТишины, ТолькоПодключение);
			Иначе
		...
		Если НЕ (ИспользоватьОткрытиеСессииСПодписью И ИспользоватьПодписьНовойКомпоненты) Тогда
			// Проверка подключения в требуемом контексте.
			Если ВыполнятьКриптографическиеОперацииНаКлиенте Тогда
				Подключается = УдалосьПодключитьКриптопровайдерНаКлиенте(Версия, РежимТишины, ТолькоПодключение);
			Иначе
...
Функция ТипКлюча(Знач КлючBase64, Знач ПарольОтКлюча) Экспорт
	...
	Если ЭСФВызовСервера.ИспользоватьПодписьНовойКомпоненты() И ЭСФВызовСервера.ИспользоватьВнешнююКриптографиюДляКомпоненты() Тогда
		...
		Если ЭСФВызовСервераПовтИсп.ВыполнятьКриптографическиеОперацииНаКлиенте() Тогда
			ОтветСтруктура = ЭСФКлиентСервер.ПолучитьДанныеСертификатаВнешняяКриптография("000000000000", "1CService", ДополнительныеПараметры, ДанныеКлючаЭЦП);
		Иначе
			ОтветСтруктура = ЭСФВызовСервера.ПолучитьДанныеСертификатаВнешняяКриптография("000000000000", "1CService", ДополнительныеПараметры, ДанныеКлючаЭЦП);
		КонецЕсли;
...
Функция СвойстваСертификатаБезОбработки(Знач КлючBase64, Знач ПарольОтКлюча) Экспорт 
	...
	Возврат ЭСФКлиентСервер.СвойстваСертификатаБезОбработки(КлючBase64, ПарольОтКлюча);
...
Функция СвойстваСертификата(Знач КлючBase64, Знач ПарольОтКлюча) Экспорт 
	...
	Возврат ЭСФКлиентСервер.СвойстваСертификата(КлючBase64, ПарольОтКлюча);
	
КонецФункции

&НаКлиенте
Функция СвойстваСертификатаИзJSON(Знач СвойстваСертификатаБезОбработки) Экспорт 
	...
	СвойстваСертификата.СрокГодностиОт = МестноеВремя('19700101' + Лев(ПолучитьСвойствоСертификата(СвойстваСертификатаБезОбработки, "certNotBefore", ""),10));
	СвойстваСертификата.СрокГодностиДо = МестноеВремя('19700101' + Лев(ПолучитьСвойствоСертификата(СвойстваСертификатаБезОбработки, "certNotAfter", ""), 10));
	...
	Если СвойстваСертификатаБезОбработки.Свойство("subjectDn") И НЕ ПустаяСтрока(СвойстваСертификатаБезОбработки.subjectDn) Тогда
		ДанныеДн = ЭСФКлиент.РазобратьСтрокуСертификата(СвойстваСертификатаБезОбработки.subjectDn);
		СвойстваСертификата.ИИНСубъекта = Сред(ПолучитьСвойствоСертификата(ДанныеДн, "SERIALNUMBER", ""),4);
		...
		СвойстваСертификата.БИНОрганизации = Сред(ПолучитьСвойствоСертификата(ДанныеДн, "OU", ""),4);
	КонецЕсли;
	...
	СвойстваСертификата.ТипКлюча = ЭСФКлиент.Сертификат_ТипКлючаИзСвойствНовый(ПолучитьСвойствоСертификата(СвойстваСертификатаБезОбработки, "algorithm", ""));
...
Функция ОписаниеСертификата(Знач Свойства) Экспорт
	...
	Возврат ЭСФКлиентСервер.ОписаниеСертификата(Свойства);
...
Функция СвойстваСертификатаИзJSON_Kalkan(Знач СвойстваСертификатаБезОбработки) Экспорт 
	...
	Возврат ЭСФКлиентСервер.СвойстваСертификатаИзJSON_Kalkan(СвойстваСертификатаБезОбработки);
...
Функция ИнициализироватьХранилищеКлюча_ТипДанных_СтрокаBase64() Экспорт
	Возврат ЭСФКлиентСервер.ИнициализироватьХранилищеКлюча_ТипДанных_СтрокаBase64();	
КонецФункции
...
Функция Сертификат_ТипКлючаИзСвойств(Знач СвойстваСертификатаБезОбработки) Экспорт
	...
	Возврат ЭСФКлиентСервер.Сертификат_ТипКлючаИзСвойств(СвойстваСертификатаБезОбработки);
...
Функция Сертфикат_ИИНСубъектаИзСвойств(Знач СвойстваСертификатаБезОбработки) Экспорт
	...
	Возврат ЭСФКлиентСервер.Сертфикат_ИИНСубъектаИзСвойств(СвойстваСертификатаБезОбработки);
...
Функция Сертфикат_БИНОрганизацииИзСвойств(Знач СвойстваСертификатаБезОбработки) Экспорт
	...
	Возврат ЭСФКлиентСервер.Сертфикат_БИНОрганизацииИзСвойств(СвойстваСертификатаБезОбработки);
...
Функция НовыеСвойстваСертификата() 
	...
	Возврат ЭСФКлиентСервер.НовыеСвойстваСертификата();
...
Функция ДобавитьСтрокуОписания(Описание, Знач Наименование, Знач Значение) Экспорт
	...
	Возврат ЭСФКлиентСервер.ДобавитьСтрокуОписания(Описание, Наименование, Значение);
...
Функция ПолучитьСвойствоСертификата(СвойстваСертификата, ИмяСвойства, ПустоеЗначение)
	...
	Возврат ЭСФКлиентСервер.ПолучитьСвойствоСертификата(СвойстваСертификата, ИмяСвойства, ПустоеЗначение);
...
Процедура ОбновитьМодульКриптографии(ПараметрыАвторизации, ВыполняемоеОповещение = Неопределено, ТихийРежим = Ложь, Организация = Неопределено) Экспорт
	...
	Если ЭлектронноеВзаимодействиеССервисамиКлиент.ПроверитьНаличиеОбновлений(ПараметрыСервераИнтернета, СвойстваМодуляДляТекущегоРелиза, ШапкаОшибки) = Ложь Тогда
...
Процедура ОбновитьМодульКриптографииЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Результат = ЭлектронноеВзаимодействиеССервисамиКлиент.ЗакачатьИВключитьПризнакИспользованияКлиентУправляемая(ДополнительныеПараметры, 
		СвойстваМодуляДляТекущегоРелиза, ШапкаОшибки);
...
Процедура ОтправитьИсходящиеЭСФВыборПараметровИСЭСФ(Результат, ДополнительныеПараметры) Экспорт
	...
	ЭСФКлиент.ОпределитьНеобходимостьВводаДанныхИСЭСФ(КоллекцияСгруппированныхЭСФ, ОтправитьИсходящиеЭСФЗавершение);
...
Процедура ОтправитьИсходящиеЭСФЗавершение(ДанныеПрофилейИСЭСФ, ДополнительныеПараметры) Экспорт
		...
		Если ЭСФВызовСервераПовтИсп.ВыполнятьКриптографическиеОперацииНаКлиенте() Тогда
			...
			Для Каждого СгруппированныеЭСФ Из КоллекцияСгруппированныхЭСФ Цикл
					...
					Иначе
						...
						Иначе
							Если ДанныеПрофиляИСЭСФ.Свойство("ОписаниеСертификата") Тогда	
								...
								УполномоченныйСотрудник = СтрЗаменить(ТолькоФИО, "ФИО владельца: ", "");
							КонецЕсли;
						КонецЕсли;
				...
				Если ИспользоватьПодписьНовойКомпоненты Тогда
					...
					КоллекцияПодписейЭСФ2 = ЭСФКлиент.НоваяКоллекцияПодписей(КоллекцияSignedContentXML, ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ);
					КоллекцияПодписейЭСФ.Вставить(СтруктурнаяЕдиница, КоллекцияПодписейЭСФ2);
					Если КоллекцияПодписейЭСФ2.Количество() = 0 Тогда
					...
					КонецЕсли;
				Иначе
					КоллекцияПодписейЭСФ.Вставить(СтруктурнаяЕдиница, ЭСФКлиентСервер.НоваяКоллекцияПодписейЭСФ(КоллекцияSignedContentXML, ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ));
				КонецЕсли;
	...
	Если НЕ ОткрытьСессиюСПодписьюДляПрофилей(ДанныеПрофилейИСЭСФ, ПараметрыОткрытияСессии) Тогда
		// Не помещаем в очередь отправки, так как сведения для подписи мы не получили
		ЭСФВызовСервера.ОчиститьОчередьОтправкиЭСФ(МассивИсходящихЭСФ);
		ЭСФКлиент.ОповеститьФормы(ЭСФКлиентСервер.ИмяСобытияЗаписьЭСФ(), МассивИсходящихЭСФ);
		Возврат;
...
Процедура ОбновитьИОтправитьИсходящиеЭСФ(Знач МассивИсходящихЭСФ, Знач ДополнительныеПараметры) Экспорт
	...
	ЭСФКлиент.ОпределитьНеобходимостьВводаДанныхИСЭСФ(КоллекцияСгруппированныхЭСФ, ОбновитьПоНомеруИДатеИОтправитьИсходящиеЭСФ);
...
Функция ИзменитьСтатусыЭСФ(Знач Действие, Знач КоллецияДляИзмененияСтатусов, Знач ДанныеПрофиляИСЭСФ, Знач ДанныеКлючаЭЦП, Знач УникальныйИдентификаторФормыВладельца, Знач ЗапускатьФоновоеЗадание) Экспорт	
	...
	Если ЭСФВызовСервераПовтИсп.ВыполнятьКриптографическиеОперацииНаКлиенте() Тогда
		...
		Если ИспользоватьПодписьНовойКомпоненты Тогда
			...
			Иначе
				КоллекцияПодписейЭСФ = ЭСФКлиент.НоваяКоллекцияПодписей(СтрокаЗапросаВМассив, ДанныеКлючаЭЦП, ДанныеПрофиляИСЭСФ);
			КонецЕсли;
			...
			Если КоллекцияПодписейЭСФ.Количество() = 0 Тогда
				...
				ЭСФКлиент.ОповеститьФормы(ЭСФКлиентСервер.ИмяСобытияЗаписьЭСФ(), КоллецияДляИзмененияСтатусов);
				Возврат Неопределено;
			КонецЕсли;
...
Функция ПолучитьВерсиюКомпонентыНаСайте() Экспорт
	...
	ПараметрыСервераИнтернета = ПолучитьПараметрыИнтернета(Новый Структура);
