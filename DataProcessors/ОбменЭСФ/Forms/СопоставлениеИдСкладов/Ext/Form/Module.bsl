...
Процедура ПолучитьСкладыВС()
	...
	КонецЕсли;
	ПолучитьСкладыВыборПрофилейЗавершение = Новый ОписаниеОповещения("ПолучитьСкладыВыборПрофилейЗавершение", ЭтаФорма, ДополнительныеПараметры);
	...
	ТребуетсяСертификатАутентификации = ЭСФВызовСервера.ИспользоватьОткрытиеСессииСПодписью() И ЭСФВызовСервера.ИспользоватьВнешнююКриптографиюДляКомпоненты();
	...
	Если ЕстьНесколькоПрофилейДляОрганизации Тогда
		...
		Если ТребуетсяСертификатАутентификации Тогда
			ПараметрыФормы.Вставить("ТребуетсяВыборСертификатаВхода", Истина);
		КонецЕсли;
		ЭСФКлиент.ОткрытьФормуВводаДанныхИСЭСФ(ПолучитьСкладыВыборПрофилейЗавершение, ПараметрыФормы);

	Иначе
		Если ТребуетсяСертификатАутентификации Тогда
			ДополнительныеПараметры = Новый Структура("СписокСтруктурныхЕдиниц, ТребуетсяВыборСертификатаВхода",
				ПрофилиИСЭСФПоВыбраннымОрганизациям, Истина);
			ЭСФКлиент.ОткрытьФормуВводаДанныхИСЭСФ(ОбновитьДокументыВыборДанныхПрофилейИСЭСФ, ДополнительныеПараметры);
		Иначе
...
&НаКлиенте
Процедура ПолучитьСкладыВыборПрофилейЗавершение(Результат = Неопределено, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;

	МассивПрофилейИСЭСФ = ДанныеПрофилейДляПолученияСкладовНаСервереПоРезультатуВыбораПрофилей(Результат);
	ОчиститьСообщения(); 	
	//до перехода на сервер нужно получить ИД сессии на клиенте!
	//при этом учеть получение для каждого профиля
	Если НЕ ОткрытьСессиюСПодписьюДляПрофилей(МассивПрофилейИСЭСФ) Тогда
		Возврат;
	КонецЕсли;
	
	ПолучитьСкладыИзИСВС(Результат, ДополнительныеПараметры);
...
Функция ДанныеПрофилейДляПолученияСкладовНаСервереПоРезультатуВыбораПрофилей(Знач Результат)
	...
	Для Каждого Элемент Из Результат Цикл
		ДанныеПрофиляИСЭСФ = ЭСФСервер.ДанныеПрофиляИСЭСФ(Элемент.Значение.ПрофильИСЭСФ);
		Если ПустаяСтрока(ДанныеПрофиляИСЭСФ.ТекущийПарольАутентификации) Тогда
			ДанныеПрофиляИСЭСФ.ТекущийПарольАутентификации = Элемент.Значение.ПарольИСЭСФ;	
		КонецЕсли;
		Если МетодKalkan Тогда
			ДанныеПрофиляИСЭСФ.Вставить("КлючВходаBase64", Элемент.Значение.КлючВходаBase64);
			ДанныеПрофиляИСЭСФ.Вставить("ПарольВхода", Элемент.Значение.ПарольВхода);
		КонецЕсли;
...
Процедура ЗаполнитьСписокОрганизаций()
	...
	Если МассивПрофилейИСЭСФ.Количество() = 0 Тогда
		ЭСФКлиентСерверПереопределяемый.СообщитьПользователю(НСтр("ru='Для текущего пользователя не настроен профиль ИС ЭСФ. Для получения складов сначала необходимо настроить профиль ИСЭСФ и выбрать организации для работы'"));
		Возврат; 	
	КонецЕсли;
