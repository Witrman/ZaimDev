	...
	Для Каждого СтрокаДанных Из ДанныеИСЭСФ Цикл
		...
		Если ТребуетсяУказаниеДолжности Тогда
			Если ПустаяСтрока(СтрокаДанных.ДолжностьПодписывающего) Тогда
				ТекстСообщения = НСтр("ru = 'Укажите должность для документов ЭАВР %1'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", ?(ПереданСписокПользователей, ПользовательТекст, СтруктурнаяЕдиницаТекст));
				ЭСФКлиентСервер.СообщитьПользователю(ТекстСообщения, "Организация[" + СчетчикСтруктурныхЕдиниц + "].КлючИмя");
				ЕстьОшибки = Истина;
			КонецЕсли;
		...
		Если ТребуетсяВыборСертификата ИЛИ ТребуетсяВыборСертификатаВхода Тогда
			...
			Для Каждого СтрокаДанных Из ДанныеИСЭСФ Цикл
				...
				Если ТребуетсяВыборСертификатаВхода И НЕ ИспользуетсяЕдиныйКлюч Тогда
					...
					ОткрытыйСертификатBase64 = ОткрытыйСертификатBase64(СтрокаДанных.КлючВходаBase64, СтрокаДанных.ПарольВхода);
	...
	Иначе
		...
		Если ТребуетсяВыборСертификата Тогда
			...
			Если ТребуетсяВыборСертификата И ТребуетсяВыборСертификатаВхода Тогда
				...
				НовыйЭлемент.СписокВыбора.Вставить(0, 1, "Раздельные ключи (ЭЦП старого формата)");
				НовыйЭлемент.СписокВыбора.Вставить(1, 2, "Единый ключ (ЭЦП нового формата)");
...
Функция ОткрытыйСертификатBase64(Знач КлючBase64, Знач Пароль)
	...
	Если ЭСФВызовСервера.ИспользоватьОткрытиеСессииСПодписью() ИЛИ ЭСФВызовСервера.ИспользоватьПодписьНовойКомпоненты() Тогда
		Если ЭСФВызовСервера.ИспользоватьВнешнююКриптографиюДляКомпоненты() Тогда
			ОткрытыйСертификатBase64 = ИзвлечьСертификатИзФайла(КлючBase64, Пароль);
...
&НаКлиенте
Функция ИзвлечьСертификатИзФайла(Знач КлючBase64, Знач Пароль)
