...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	БИК = ЭлектронноеВзаимодействиеПереопределяемый.РеквизитыБанка(РеквизитыНастройкиОбмена.Банк).Код;
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияОтправкиЗапросовВыпискиВБанк(Знач ПараметрыЗапроса,
		Знач УникальныйИдентификатор, Знач СписокСообщенийОбмена)
	...
	Если ПараметрыЗапроса.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
		...
		Результат = ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ОтправитьЗапросВыпискиАсинхронныйОбмен", ПараметрыЗапроса, ПараметрыВыполнения);
	Иначе
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение выписки из банка.'");
		Результат = ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ПолучитьВыпискуСинхронныйОбмен", ПараметрыЗапроса, ПараметрыВыполнения);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ОтправитьЗапросСостоянияЭДНаСервере(Знач ПараметрыЗапроса,
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ОтправитьЗапросСостоянияЭДВБанк", ПараметрыЗапроса, ПараметрыВыполнения);
...
&НаКлиенте
Процедура ОбработатьРезультатЗапросаСостоянияЭД(РезультатЗадания,
		ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе // выполнено
		...
		Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПолученияИзвещенияОСостоянииЭДНаСервере(ПараметрыЗапроса, УникальныйИдентификатор);
...
Процедура ПослеОтправкиЗапросЗонда(Результат, ДополнительныеПараметры) Экспорт
	...
	Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПолученияИзвещенияОСостоянииЭДНаСервере(ДополнительныеПараметры.ПараметрыЗапроса, УникальныйИдентификатор);
...
Процедура ПослеСохраненияПолученныхПодписейВыписки(ЕстьОшибка, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыпискиБанка.Количество() Тогда
		...
		Если ВидОперации = "ПробноеПолучениеВыписки" Тогда
			Закрыть(Истина);
		Иначе
			ОповеститьОВыборе(РезультатПолученияВыписки(ВыпискиБанка));
...
Процедура ПолучитьВыпискуЧерезДополнительнуюОбработку()
	...
	Для Каждого НомерСчета Из МассивНомеровБанковскихСчетов Цикл
		...
		ПараметрыВыписки.Вставить("ДатаНачала", ДатаНачала);
		ПараметрыВыписки.Вставить("ДатаОкончания", ДатаОкончания);
		...
		Если ДанныеВыписки.Свойство("ПолучениеЗавершено") Тогда
			Если Не ДанныеВыписки.ЕстьОшибка Тогда 
				КолПолученных = ДанныеВыписки.КолПолученныхПакетов;
				СообщениеОбменаВыписка = ДанныеВыписки.СообщениеОбменаВыписка;
			КонецЕсли;
			Продолжить;
		КонецЕсли;
...
&НаКлиенте
Процедура ОтправитьЗапросНовыхДокументовСбербанк(Результат,
		ДополнительныеПараметры) Экспорт
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияЧтенияОтветаБанка(Знач НастройкаОбмена, Знач Данные,
		Знач Тикет, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ПрочитатьОтветСбербанка", ПараметрыСинхронизации, ПараметрыВыполнения);
...
Процедура Подключаемый_ПолучитьРезультатПроверкиSMSСбербанк()
	...
	Результат = ЗапускЗаданияПолученияРезультатовПроверкиСбербанк(НастройкаОбмена, СообщениеОбмена, ИсходныйТикетСбербанк, УникальныйИдентификатор);
...
Процедура ПослеПолученияРезультатовПроверкиSMSСбербанк(Результат,
	...
	Иначе // выполнено
		...
		Если Результат = Неопределено Тогда // банк еще не обработал запрос
			ЭлектронноеВзаимодействиеСлужебныйКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
			ПодключитьОбработчикОжидания("Подключаемый_ПолучитьРезультатПроверкиSMSСбербанк", ПараметрыОбработчикаОжидания.ТекущийИнтервал, Истина);
			Возврат;
...
&НаКлиенте
Процедура ПослеОтправкиЗапросаСтатусаПлатежаСбербанк(РезультатЗадания,
		ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		Если РезультатОперации.ТребуетсяАутентификация Тогда
			Если ДополнительныеПараметры.АутентификацияПроизводилась Тогда
				...
				Возврат;
			Иначе
				Обработчик = Новый ОписаниеОповещения("ОтправитьЗапросСтатусаПлатежаПослеБазовойАутентификацииСбербанк", ЭтотОбъект, ДополнительныеПараметры);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияОтправкиЗапросаСтатусаПлатежаСбербанк(Знач НастройкаОбмена,
		Знач СообщениеОбмена, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ОтправитьЗапросСтатусаПлатежногоДокумента", ПараметрыСинхронизации, ПараметрыВыполнения);

КонецФункции

&НаСервереБезКонтекста
Функция ЗапускЗаданияПолученияРезультатовПроверкиСбербанк(Знач НастройкаОбмена,
		Знач СообщениеОбмена, Знач Тикет, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ПолучитьРезультатПроверкиSMSСбербанк", ПараметрыОтправки, ПараметрыВыполнения);
...
Процедура Подключаемый_ПолучитьИдентификаторКриптопрофиляСбербанк()
	...
	Результат = ЗапускЗаданияПолученияИдентификатораКриптопрофиляСбербанк(НастройкаОбмена, ИсходныйТикетСбербанк, УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияПолученияИдентификатораКриптопрофиляСбербанк(Знач НастройкаОбмена,
		Знач Тикет, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ПолучитьИдентификаторКриптопрофиляСбербанк", ПараметрыОтправки, ПараметрыВыполнения);
...
Процедура ПослеОтправкиЗапросовСтатусовЗапросовВыписокСбербанк(Результат,
	...
	Иначе // выполнено
		...
		Если РезультатОтправки = Неопределено Тогда // все запросы имеют конечные статусы
			...
			ПерейтиНаСледующийШагСбербанк();
			ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОжиданияПолученияВыписки);
			ПодключитьОбработчикОжидания("Подключаемый_ОжидатьВыпискуСбербанк", ПараметрыОжиданияПолученияВыписки.ТекущийИнтервал, Истина);
		Иначе
...
&НаКлиенте
Процедура ПослеПолученияТикетаНаЗапросНовыхДокументовСбербанк(Результат,
		ДополнительныеПараметры) Экспорт
...
Функция ЗапускЗаданияОтправкиЗапросаНовыхДокументовСбербанк(Знач НастройкаОбмена,
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ОтправитьЗапросНовыхДокументовСбербанк", ПараметрыОтправки, ПараметрыВыполнения);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияЗапросаСтатусаЗапросовСбербанк(Знач НастройкаОбмена,
		Знач МассивСообщенийОбмена, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ОтправитьЗапросыСтатусовЗапросовПоЛогинуСбербанк", ПараметрыОтправки, ПараметрыВыполнения);
...
Процедура Подключаемый_ПолучитьИзвещениеОСостоянииПлатежногоДокумента()
	...
	Результат = ЗапускЗаданияПолученияСтатусаПлатежногоДокументаСбербанк(НастройкаОбмена, ИсходныйТикетСбербанк, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ПослеПолученияСтатусаПлатежаСбербанк(Результат,
		ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		РезультатОтправки = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		Если РезультатОтправки.ТребуетсяАутентификация Тогда
			Обработчик = Новый ОписаниеОповещения("ПолучитьСтатусПлатежаПослеАутентификацииПоЛогинуСбербанк", ЭтотОбъект, ДополнительныеПараметры);
			ОбменСБанкамиСлужебныйКлиент.ВыполнитьАутентификациюПоЛогинуСбербанк(Обработчик, ИмяВнешнегоМодуля, НастройкаОбмена, НастройкаОбмена);
		ИначеЕсли РезультатОтправки.МассивНеобработанныхТикетов.Количество() Тогда
			ЭлектронноеВзаимодействиеСлужебныйКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
			ПодключитьОбработчикОжидания("Подключаемый_ПолучитьИзвещениеОСостоянииПлатежногоДокумента", ПараметрыОбработчикаОжидания.ТекущийИнтервал, Истина);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияПолученияСтатусаПлатежногоДокументаСбербанк(Знач НастройкаОбмена,
		Знач Тикет, Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ПолучитьСтатусыЗапросовПоЛогинуСбербанк", ПараметрыОтправки, ПараметрыВыполнения);
...
Процедура НачатьФормированиеЗапросовВыписокСбербанк()
	...
	РезультатОтправки = ЗапускЗаданияФормированияЗапросовВыпискиСбербанк(НастройкаОбмена, ДатаНачала, ДатаОкончания, МассивБанковскихСчетовСсылка);
...
Процедура НачатьОтправкуЗапросовВыписокПоЛогинуСбербанк()
	...
	РезультатОтправки = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПоОтправкеДокументовПоЛогинуВСбербанк(ПараметрыОтправки);
	Если РезультатОтправки.Статус = "Выполняется" Тогда
		ИдентификаторЗадания = РезультатОтправки.ИдентификаторЗадания;
		ПараметрыОбработчикаОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		ПараметрыОбработчикаОжидания.ВыводитьОкноОжидания = Ложь;
		ПараметрыОбработчикаОжидания.ВыводитьПрогрессВыполнения = Истина;
		ПараметрыОбработчикаОжидания.ОповещениеОПрогрессеВыполнения = Новый ОписаниеОповещения("ВывестиПрогрессТекущейДлительнойОперацииСбербанк", ЭтотОбъект);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияФормированияЗапросовВыпискиСбербанк(Знач НастройкаОбмена,
		Знач ДатаНачала, Знач ДатаОкончания, Знач МассивБанковскихСчетовСсылка)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.СформироватьЗапросыВыписокСбербанк", ПараметрыОтправки, ПараметрыВыполнения);
...
&НаКлиенте
Процедура ПослеОтправкиЗапросовВыписокПоЛогинуСбербанк(Результат,
		ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		Если РезультатОтправки.ТребуетсяАутентификация Тогда // неверный идентификатор сессии.
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстСообщения, ТекстСообщения, "ОбменСБанками", НастройкаОбмена);
			ПриОшибкеТекущейОперацииСбербанк();
			ЗакрытьФормуНаКлиенте(Истина)
		Иначе
			КоличествоЗапросов = СписокСообщенийОбмена.Количество();
			ВывестиПрогрессТекущейОперацииСбербанк("" + КоличествоЗапросов + "/"
				+ КоличествоЗапросов);
			ПерейтиНаСледующийШагСбербанк();
			МассивТикетовСсылкаСбербанк = ПоместитьВоВременноеХранилище(РезультатОтправки.МассивТикетов, УникальныйИдентификатор);
			ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
			ПодключитьОбработчикОжидания("Подключаемый_ПолучитьВнешниеИдентификаторыЗапросовВыпискиПоЛогинуСбербанк", ПараметрыОбработчикаОжидания.ТекущийИнтервал, Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_ПолучитьКонечныеСтатусыЗапросовВыпискиПоЛогинуСбербанк()
	...
	РезультатОтправки = ЗапускЗаданияЗапросаСтатусаЗапросовСбербанк(НастройкаОбмена, СписокСообщенийОбмена.ВыгрузитьЗначения(), УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияПолученияСтатусовЗапросовПоЛогинуСбербанк(Знач НастройкаОбмена,
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияПробнойОтправкиЗапросаВыпискиВБанк(Знач ПараметрыЗапроса,
		Знач УникальныйИдентификатор)
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне("ОбменСБанкамиСлужебный.ОтправитьТестовыйЗапросВыпискиАсинхронныйОбмен", ПараметрыЗапроса, ПараметрыВыполнения);
	Возврат Результат
КонецФункции
