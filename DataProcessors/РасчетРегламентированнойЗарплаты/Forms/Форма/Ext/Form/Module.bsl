...
Процедура ЗаполнитьДеревоПодразделений()
	...
	КорневаяСтрока.Наименование = "Все подразделения";
	...
	Для Каждого СтрокаДок Из ТаблицаДокументов Цикл
		...
		Попытка
			...
			ИначеЕсли ТипЗнч(СтрокаДок.Документ) = Тип("ДокументСсылка.РасчетСНиСО") Тогда
				...
				ДокОбъект.Записать(РежимЗаписиДокумента.Запись);
				Документы.РасчетСНиСО.Рассчитать(ДокОбъект);
...
Процедура НастроитьВидимость(ОбновлениеДанных = Ложь)
	...
	//Если Объект.РассчитыватьЕдиныеПлатежи
	//	И НЕ ПорядокОбложенияДоходовРаботников = Перечисления.ПорядокОбложенияДоходовРаботников.ЕдиныйПлатеж Тогда
	//	
	//	Объект.РассчитыватьЕдиныеПлатежи = Ложь;
	//	
	//КонецЕсли;
