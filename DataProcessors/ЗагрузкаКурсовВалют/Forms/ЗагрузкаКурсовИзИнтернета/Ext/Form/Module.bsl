...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверитьВозможностьЗагрузкиКурсовИзИнтернета();
...
Процедура ДекорацияРекомендацияНажатие(Элемент)
	ОткрытьФорму("Обработка.ЗагрузкаКурсовВалют.Форма.ЗагрузкаКурсовИзФайлов");
КонецПроцедуры
...
&НаКлиенте
Процедура ЗагрузитьКурсыИзФайла(Команда)
	ОткрытьФорму("Обработка.ЗагрузкаКурсовВалют.Форма.ЗагрузкаКурсовИзФайлов");	
КонецПроцедуры

&НаКлиенте
Процедура НастроитьАвтоматическуюЗагрузкуКурсов(Команда)
	
	ОткрытьФорму("Обработка.ЗагрузкаКурсовВалют.Форма.НастройкаАвтоматическойЗагрузкиКурсовВалют");
...
////////////////////////////////////////////////////////////////////////////////
// ИЗМЕНЕНИЕ ВНЕШНЕГО ВИДА ФОМРЫ

&НаСервере
Процедура ПроверитьВозможностьЗагрузкиКурсовИзИнтернета()
	
	Валюта = Константы.ВалютаРегламентированногоУчета.Получить();
	
	Если Валюта.Наименование = "KZT" Тогда
		Элементы.ГруппаНекорректнаяВалюта.Видимость = Ложь;	
	Иначе
		Элементы.ДекорацияНекорректнаяВалюта.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Элементы.ДекорацияНекорректнаяВалюта.Заголовок, Валюта);
		Элементы.ГруппаНекорректнаяВалюта.Видимость = Истина;
	КонецЕсли;
...
Функция ЗагрузитьКурсыВРегистрКурсыВалют(
	...
	Если Результат.ВсеКурсыУспешноЗагружены Тогда
		...
		ОценкаПроизводительности.ЗакончитьЗамерВремени("Обработка ""Загрузка курсов валют"" (загрузка из интернета)", ЗначениеТаймера);
	КонецЕсли;
...
Функция СообщитьНеУдалосьЗагрузитьКурсыВалют(Знач Ошибки)
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Для Каждого ТекстОшибки Из Ошибки Цикл
			ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(ТекстОшибки + Символы.ПС);	
		КонецЦикла;
