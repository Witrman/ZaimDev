...
Функция ОповещенияПоДанномуСобытиюЕсть(ДанныеСтроки, РежимУплаты = Ложь, ВозвратитьТаблицу = Ложь)
	...
	Запрос.УстановитьПараметр("Периодичность",           ПредставлениеПериода(ДанныеСтроки.ЗаПериодС, КонецДня(ДанныеСтроки.ЗаПериодПо), "ФП=ИСТИНА"));
...
Процедура РазместитьСведения(НаДаты)
	...
	Для Каждого НаДатуКалендаря Из НаДаты Цикл
		...
		Для каждого Стр Из ТаблОтчетности Цикл
			...
			Если ВРег(НовСтр.Периодичность) = "ПОЛУГОДИЕ" И НачалоКвартала(НовСтр.ЗаПериодС) <> НачалоГода(НовСтр.ЗаПериодС) Тогда 
				СтроковоеПредставлениеПериода = ПредставлениеПериода(НачалоГода(НовСтр.ЗаПериодС), ДобавитьМесяц(КонецДня(НовСтр.ЗаПериодПо), -6), "ФП=Истина");
				СтроковоеПредставлениеПериода = СтрЗаменить(СтроковоеПредставлениеПериода, НСтр("ru = '1 '"), НСтр("ru = '2 '"));
			Иначе 
				СтроковоеПредставлениеПериода = ПредставлениеПериода(НачалоДня(НовСтр.ЗаПериодС), КонецДня(НовСтр.ЗаПериодПо), "ФП=Истина");
			КонецЕсли;
		...
		Если (Событие.НазваниеСобытия = "ОПВ") ИЛИ (Событие.НазваниеСобытия = "СО") ИЛИ (Событие.НазваниеСобытия = "ОППВ") ИЛИ (Событие.НазваниеСобытия = "ОПВР") Тогда
			...
			Если (Событие.НазваниеСобытия = "ОПВ")Тогда
				...
				Пока СрокиПеречисления.Следующий() Цикл
					...
					НоваяСтрока.Периодичность = "Месячная";
					...
					НоваяСтрока.Кто = "Агенты по уплате обязательных пенсионных взносов";
					НоваяСтрока.НалогПолноеНазв = "Перечисление обязательных пенсионных взносов";
				КонецЦикла;	
			ИначеЕсли (Событие.НазваниеСобытия = "ОППВ")Тогда
				...
				Пока СрокиПеречисления.Следующий() Цикл
					...
					НоваяСтрока.ЗаПериодПо = КонецМесяца(СрокиПеречисления.Месяц);
					НоваяСтрока.СрокУплаты = ПолучитьСледующуюРабочуюДату(СрокиПеречисления.СрокПеречисления);
					НоваяСтрока.Кто = "Вкладчики обязательных профессиональных пенсионных взносов";
					НоваяСтрока.НалогПолноеНазв = "Перечисление обязательных профессиональных пенсионных взносов";
				КонецЦикла;	
			ИначеЕсли (Событие.НазваниеСобытия = "ОПВР")Тогда
				...
				Пока СрокиПеречисления.Следующий() Цикл
					...
					НоваяСтрока.ЗаПериодПо = КонецМесяца(СрокиПеречисления.Месяц);
					НоваяСтрока.СрокУплаты = ПолучитьСледующуюРабочуюДату(СрокиПеречисления.СрокПеречисления);
					НоваяСтрока.Кто = "Вкладчики обязательных пенсионных взносов работодателя";
					НоваяСтрока.НалогПолноеНазв = "Перечисление обязательных пенсионных взносов работодателя";
				КонецЦикла;	
			Иначе
				...
				Пока СрокиПеречисления.Следующий() Цикл
					...
					НоваяСтрока.ЗаПериодПо = КонецМесяца(СрокиПеречисления.Месяц);
					НоваяСтрока.СрокУплаты = ПолучитьСледующуюРабочуюДату(СрокиПеречисления.СрокПеречисления);
					НоваяСтрока.Кто = "Юридические лица, являющиеся плательщиком социальных отчислений";
					НоваяСтрока.НалогПолноеНазв = "Перечисление социальных отчислений";
				КонецЦикла;	
...
Процедура ДобавитьУдалитьЗаписьВРегистрОповещения(ДанныеСтроки, ДобавитьСобытие, РежимУплаты = Ложь)
	...
	НовЗап.Периодичность         = ПредставлениеПериода(ДанныеСтроки.ЗаПериодС, КонецДня(ДанныеСтроки.ЗаПериодПо), "ФП=ИСТИНА");
...
Процедура ОткрытьПлатПоручениеИлиОтчет(Расшифровка, Статус)
	...
	Если (Статус = "1") Или (Статус = "3") Тогда
		...
		Если Найти(Расшифровка.Периодичность, "Месячная") Тогда
			Периодичность = ПеречислениеПериодичностьМесяц;
			мПериодичностьКалендарь = "Месячная";
		ИначеЕсли Найти(Расшифровка.Периодичность, "Квартальная") Тогда
