...
Процедура ОткрытьПоСтроке(ВыбраннаяСтрока)
	...
	Если Найти(ВыбраннаяСтрока.Периодичность, "Месячная") Тогда
		Периодичность = ПеречислениеПериодичностьМесяц;
	ИначеЕсли Найти(ВыбраннаяСтрока.Периодичность, "Квартальная") Тогда
...
Процедура ОтменитьСобытие(Команда)
	...
	ФормаЦУО = ПолучитьФорму("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью");
