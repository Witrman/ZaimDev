...
Процедура ВыборФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено Тогда
		...
		Иначе
			ВыбранныеФайлыПредставление = НСтр("ru = 'Выбрано файлов: '") + ВыбранныеФайлы.Количество();
		КонецЕсли;
...
Процедура ДанныеФайловНоменклатураПриИзменении(Элемент)
	...
	Если Не ТекущиеДанные = Неопределено Тогда
		ВидПродукцииИС = ИнтеграцияИСМПТКПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(ТекущиеДанные.Номенклатура);
		Шаблон = ПолучитьШаблонКМ(ВидПродукцииИС);
		
		ТекущиеДанные.Шаблон = Шаблон;
		ТекущиеДанные.ВидПродукции = ВидПродукцииИС;
...
Процедура УправлениеФормой() 
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДанныеФайловХарактеристика", "Видимость", ХарактеристикиИспользуются);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияПредупреждениеТаблицаИзменилась", "Заголовок", ЗаголовокПредупреждения);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Распечатать", "Доступность", ПравоДоступа("Просмотр", Метаданные.Обработки.ПечатьКодовМаркировкиИСМПТК));
...
Процедура ПрочитатьФайлыCSV(ОшибкаЗагрузки, ВыбранныйФайл)
	...
	НужноДобавлятьИдентификаторКодамSSCCПриПечатиИСМПТК = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ПроверитьЗначениеКонстанты("ДобавлятьИдентификаторКодамSSCCПриПечатиИСМПТК");
	...
	Для НомерСтроки = 1 По КоличествоСтрокВФайле Цикл
		...
		Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ЭтоКодSSCC(ДанныеСтроки) Тогда
		...
		Иначе
			...
			Если Не ЭтоКодМарки Тогда
				ТекстСообщения = НСтр("ru = 'Не удалось определить формат кода в строке № %1! Данная строка исключена из данных печати.'");
				ТекстСообщения = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, НомерСтроки); 
				ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПрочитатьФайлыExcel(ОшибкаЗагрузки, ВыбранныйФайл)
	...
	Для НомерСтроки = 1 По КоличествоСтрокВФайле Цикл
		...
		//Проверка транспортных кодов
		Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ЭтоКодSSCC(СтрокаФайла) Тогда
			Если СтрДлина(СтрокаФайла) = 18 Тогда
				ОбнаруженыНеполныеSSCC = Истина;
				ПолныйКодМаркировкиBase64 = РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.СтрокуВBase64(
											?(РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ПроверитьЗначениеКонстанты("ДобавлятьИдентификаторКодамSSCCПриПечатиИСМПТК"), 	
...
Процедура ЗаполнитьПоДаннымСтрокиФайла(ПолныйКодМаркировкиBase64, ОбнаруженыSSCC, ОбнаруженыDataMatrix)
	...
	КонецЕсли;
 	...
 	Если СтруктураКодовМаркировки.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИСМПТК.Логистическая") 
		И Не ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПроверкаФорматаТранспортногоКодаПройденаУспешно(ИсходныеДанные, РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ПроверитьЗначениеКонстанты("ПоддержкаДвойногоФорматаТранспортныхКодовИСМПТК")) Тогда
		Возврат;
	КонецЕсли;
	...
	НоваяСтрока.ВидПродукции = СтруктураКодовМаркировки.ВидПродукцииИС;
	
	//Заполняем шаблон
	Шаблон = ПолучитьШаблонКМ(СтруктураКодовМаркировки.ВидПродукцииИС);			
...
&НаСервере
Функция ПолучитьШаблонКМ(ВидПродукцииИС)
	
	Возврат ИнтеграцияИСМПТКПереопределяемый.ПолучитьШаблонКодаПоВидуПродукции(ВидПродукцииИС);
...
Процедура ВыполнитьПроверкиПередПечатью(ОтказПечати)
	...
	Если ОбнаруженыSSCC И ОбнаруженыDataMatrix Тогда
		ТекстСообщения = НСтр("ru = 'Таблица данных для печати содержит смешанную информацию по кодам DataMatrix и SSCC, печать невозможна!'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		ОтказПечати = Истина;	    
	КонецЕсли;
	...
	Если ОтказПечати Тогда
		ТекстСообщения = НСтр("ru = 'Печать прервана, т.к. были обнаружены ошибки заполнения данных!'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ВывестиСообщенияПослеЧтенияФайлов(ОбнаруженыSSCC, ОбнаруженыDataMatrix, ОбнаруженыНеполныеSSCC)
	...
	Если ОбнаруженыSSCC И ОбнаруженыDataMatrix Тогда
		ТекстСообщения = НСтр("ru = 'Внимание! В загруженных файлах обнаружены смешанные данные штрихкодов (DataMatrix и SSCC)! Такая печать не поддерживается, необходимо скорректировать таблицу предварительного просмотра данных печати и оставить только один формат.'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;		
	
	Если ОбнаруженыНеполныеSSCC Тогда
		Если Константы.ДобавлятьИдентификаторКодамSSCCПриПечатиИСМПТК.Получить() Тогда
			ТекстСообщения = НСтр("ru = 'Внимание! В загруженных файлах обнаружены 18-значные транспортные коды SSCC! При печати они будут преобразованы путем добавления идентификатора (00).'");
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		Иначе
			ТекстСообщения = НСтр("ru = 'Внимание! В загруженных файлах обнаружены 18-значные транспортные коды SSCC! При печати данные коды сгенерированы не будут!'");
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПоКоличествуГрупповыхУпаковок(Команда)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ТекстОшибки = НСтр("ru = 'Необходимо указать орагнизацию!'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
		Возврат;
	КонецЕсли;
...
Процедура ЗавершениеПолучитьДанныеПоГорупповымКодамМаркировки(РезультатВыполнения, Параметры) Экспорт
	...
	ДанныеПоГрупповомуКоду = ИнтерфейсИСМПТК.ЗапроситьДанныеОбАгрегацииСпискаКМ(СтруктураКодов, Организация, ТокенАвторизации);
	...
	Иначе
		...
		Для Каждого ОбновляемыйГрупповойКод Из СоответствиеГрупповыхКМ Цикл   
			...
			Если ЗначениеЗаполнено(GTIN) Тогда
				...
				СтрокаКМ.ВидПродукции   = ВидПродукцииИС;
				СтрокаКМ.Шаблон 		= ПолучитьШаблонКМ(ВидПродукцииИС);
...
Функция ДанныеДляПечатиСвободныеКоды()
	...
	Для Каждого СтрокаТаблицы Из ДанныеФайлов Цикл
		...
		Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ЭтоКодSSCC(СтрокаТаблицы.КодИдентификации) Тогда
		...
		Иначе
			...
			ПараметрыШтрихкода.КодМаркировки      = СтрокаТаблицы.КодИдентификации;
			ПараметрыШтрихкода.ХешСуммаКодаМаркировки = ПолучитьХэшСуммуКМ(СтрокаТаблицы.КодИдентификации);
			ПараметрыШтрихкода.ШаблонЭтикетки = ШаблонЭтикеткиПечати;
			ПараметрыШтрихкода.Штрихкод 	  = СтрокаТаблицы.КодМаркировки; 
			ПараметрыШтрихкода.Вставить("ПолныйКодМаркировки", СтрокаТаблицы.КодМаркировки);
		КонецЕсли;
...
Функция ПолучитьХэшСуммуКМ(КодМаркировки)
	...
	Возврат ИнтеграцияИСМПТК.ХэшСуммаСтроки(КодМаркировки);
...
Процедура  ЗаполнитьПоДаннымВыбранногоДокумента(НанесениеКМ)
	...
	Если ТаблицаДанныхРегистра.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru = 'По документу %1 в Пуле кодов не обнаружено готовых к печати кодов маркировки. Вероятно, они уже были распечатаны ранее. Данные этого документа не добавлены в таблицу печати.'");
		ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, НанесениеКМ);
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из НанесениеКМ.Товары Цикл
		...
		Если СтрокиИзПулаСОтбором.Количество() = 0 Тогда
			ТекстОшибки = НСтр("ru = 'Сведения о коде маркировки %1 из документа %2 не обнаружены в доступных для печати данных из Пула кодов! Вероятно, код уже был распечатан ранее. Он не был добавлен в таблицу печати.'");
			ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаТЧ.КодИдентификации, НанесениеКМ); 
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
			Продолжить;
		КонецЕсли;
		...
		ХешСуммаКодаМаркировки = СтрокаКодИзПула.ХешСуммаКодаМаркировки;
		...
		НоваяСтрока.Шаблон           = Шаблон;
		НоваяСтрока.EAN              = EAN;
		НоваяСтрока.ХешСуммаКодаМаркировки = ХешСуммаКодаМаркировки;
...
Процедура РаспечататьНанесения(Команда)
	...
	Если Не ДанныеПечати.ОбъектыПечати.Количество() = 0 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Выбранные коды маркировки были распечатаны, в Пуле кодов для них установлена дата печати.'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
		ДанныеДокументов.Очистить();
		ОбщееКоличествоПоДокументам = ДанныеДокументов.Количество();
	КонецЕсли;
...
Функция ДанныеДляПечатиРежимНанесения()
	...
	Для Каждого СтрокаТаблицы Из ДанныеДокументов Цикл
		...
		ПараметрыШтрихкода.GTIN               = СтрокаТаблицы.GTIN;
		ПараметрыШтрихкода.КодМаркировки      = СтрокаТаблицы.КодИдентификации;
		ПараметрыШтрихкода.ХешСуммаКодаМаркировки = СтрокаТаблицы.ХешСуммаКодаМаркировки;
		ПараметрыШтрихкода.ШаблонЭтикетки 	  = ШаблонЭтикеткиПечати;
		ПараметрыШтрихкода.Штрихкод 	  	  = СтрокаТаблицы.КодМаркировки; 
		ПараметрыШтрихкода.Вставить("ПолныйКодМаркировки", СтрокаТаблицы.КодМаркировки);
