...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Форма доступна только пользователю с ролью ""Полные права"".'"));
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ОбработкаиОбновления
...
Процедура ОткрытьОбработку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ЭтоСтрокаОбработки(ТекущиеДанные) Тогда
			ОткрытьФормуОбработчикаОбновления(ТекущиеДанные);	
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выберите обработку для открытия.'"));	
		КонецЕсли;
	КонецЕсли;
...
Процедура ТехническаяИнформация(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Действие доступно только для обработок обновления.'"));
		КонецЕсли;
...
Процедура ЗаполнитьКолонкиРеквизитаОбработчикиОбновленияИзКонстанты(ИзменятьПометку)
	...
	// Заполнить колонки реквизита ОбработчикиОбновления из константы СведенияОбОбновленииИБ_БК.
...
Функция ВыполнитьКоллекциюОбработчиковОбновленияНаСервере()
	...
	Если МассивПомеченныхОбработчиковОбновления.Количество() <> 0 Тогда
		...
		КоллекцияОбработчиковОбновления = Обработки.ОбновлениеИнформационнойБазыБК.ПустаяКоллекцияОбработчиковОбновления();
	...
	Иначе
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите хотя бы одну обработку для выполнения.'"));
...
Процедура ЗапуститьВыполнениеОбработчиковОбновления(КоллекцияОбработчиковОбновления)
	...
	Для Каждого ОписаниеОбработчика Из КоллекцияОбработчиковОбновления Цикл
		...
		МассивНайденныхСтрок = СведенияОбОбновленииИБ_БК.РучныеОбработчики.ТаблицаОбработчиков.НайтиСтроки(ПараметрыОтбора);
	...
	Результат = Обработки.ОбновлениеИнформационнойБазыБК.ЗапуститьФоновоеВыполнениеОбработчиковОбновления(КоллекцияОбработчиковОбновления, ЭтаФорма.УникальныйИдентификатор);
