...
Процедура ЗаполнитьРеквизитПредставлениеПериода(Параметры, ИмяСобытия, Результат) Экспорт
	...
	Пока Истина Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбъектДокумент.ПредставлениеПериода = ПредставлениеПериода(ОбъектДокумент.ДатаНачала, КонецДня(ОбъектДокумент.ДатаОкончания), "ФП = Истина");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				ОбъектОперация.Комментарий = СтрЗаменить(Строка(ОбъектКорректировка.Ссылка), НСтр("ru = '(не используется) '"), "");
