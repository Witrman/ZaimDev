...
#Область ОбщиеДанные
	...
	Если ТолькоПросмотр Тогда
		ЭтаФорма.Заголовок = "Проверка и редактирование списка кодов маркировки (только просмотр)";
	Иначе
		ЭтаФорма.Заголовок = "Проверка и редактирование списка кодов маркировки";
	КонецЕсли;
...
#Область ЗаполнениеДанныхКодыМарикровки
	...
	Для Каждого СтрокаКМ Из Параметры.СписокКМ Цикл		
		...
		Иначе
			НоваяСтрокаСписка.НомерСтрокиТоваров = "Не определен";
		КонецЕсли;
...
#Область ЗаполнениеДополнительныхДанных
	...
	Если Не Параметры.ДанныеВозвращаемогоКода = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Добавлен новый код маркировки: %1'");
		ТекстСообщения = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, СтруктураКодовМаркировки.КодИдентификации);
...
Процедура ОбработкаСобытияСканераОбщая(Параметр)
	...
	Если ЗначениеЗаполнено(Параметр) Тогда 
		...
		Если ЭтоОбычныйШтрихкод Тогда
			...
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли Не ОбъединятьСтрокиСОдинаковымиТоварами Тогда
			...
			ТекстСообщения = НСтр("ru = 'Это новый код маркировки! При отключенной опции объединения строк с одинаковыми товарами'")
						   + Символы.ПС
						   + НСтр("ru = 'в форме Проверки и редактирования кодов маркировки нельзя добавлять новые коды. Отсканируйте товар в форме заполняемого чека.'"); 
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли Не ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПроверкаФорматаТранспортногоКодаПройденаУспешно(РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиент.ПреобразоватьДанныеСоСканераВСтруктуру(Параметр)) Тогда
			Возврат;
		Иначе
...
Процедура ОбработатьКодМаркировки(ИсходныеДанные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ТекстСообщения = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, СтруктураКодовМаркировки.EAN);
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			//если отсканирован КМ и для него найдена номенклатура, но она не имеет особенностей учета по марикровке,
			...
			Если НайденнаяСтрокаТоваров.Количество() = 0 Тогда
				...
				ТекстСообщения = НСтр("ru = 'Отсканированный код маркировки принадлежит номенклатуре, не указанной в Чеке. Выполните его сканирование в форме Чека, чтобы добавить товар.'"); 
				РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;
...
Процедура ОбработатьКодМаркировкиПослеУточненияДанных(ДанныеНоменклатуры) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Добавлен новый код маркировки: %1'"); 
		ТекстСообщения = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, КодИдентификации);
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);		
...
Процедура УстановитьУсловноеОформление()
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроверкаЗавершена", "Видимость", Не ТолькоПросмотр);	
	Если ТолькоПросмотр Тогда
	...
	//Характеристики
	ХарактеристикиИспользуются = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеХарактеристик();
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыХарактеристика", "Видимость", ХарактеристикиИспользуются);
