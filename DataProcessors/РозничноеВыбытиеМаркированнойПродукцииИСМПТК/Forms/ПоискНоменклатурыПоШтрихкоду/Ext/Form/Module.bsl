...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьХарактеристикиНоменклатуры = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеХарактеристик();
		...
		Для Каждого МассивШтрихкодов Из Параметры.НеизвестныеШтрихкоды Цикл
			...
			Для Каждого СтрокаШтрихкода Из МассивШтрихкодов Цикл
				...
				Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура)
					...
					Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеХарактеристикУНоменклатуры(НоваяСтрока.Номенклатура) 
	...
	ОтображатьДанныеПоНоменклатуре = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьЗначениеКонстантыОтображенияДанныхНоменклатурыСУЗ();
	
	Если Не Параметры.Операция = "РаботаСТранспортнымиКодами" Тогда
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПоискНоменклатурыПоШтрихкодуПриСозданииНаСервере(ЭтаФорма);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПоискНоменклатурыПоШтрихкодуОбработкаПроверкиЗаполнения(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ВыполняетсяЗакрытие И Не ПроверятьТЧПередЗакрытием И НЕ ЗавершениеРаботы Тогда
		...
		ТекстВопроса = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентПереопределяемый.ПолучитьТекстВопросаПередЗакрытиемФормыПодбора(ДействияСНеизвестнымиШтрихкодами);					
...
Процедура ПеренестиВДокумент(Команда)
	...
	Если ИмяТаблицы = "ШтрихкодыНоменклатуры" Тогда
		...
		Если ПараметрЗакрытия.НайденыНезарегистрированныеТовары Тогда
			ТекстВопроса 	 = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентПереопределяемый.ПолучитьТекстВопросаПеренестиВДокументФормыПодбора(ИспользоватьХарактеристикиНоменклатуры);
			РезультатВопроса = Неопределено;
			ПоказатьВопрос(Новый ОписаниеОповещения("ПеренестиВДокументЗавершение", ЭтотОбъект, Новый Структура("ПараметрЗакрытия", ПараметрЗакрытия)), ТекстВопроса, РежимДиалогаВопрос.ОК);
			Возврат;
		КонецЕсли;
...
Процедура ЗарегистрироватьНовыйGTIN(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Будут зарегистрированы новые штрихкоды номенклатуры для всех заполненных позиций списка, по которым отсутствуют данные в регистре. Продолжить?'");
...
Процедура ЗарегистрироватьШтрихкодыНаСервереПродолжение(Отказ)
	...
	Если ШтрихкодыДляОтработки.Количество() > 0 Тогда
		...
		Попытка
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ЗарегистрироватьШтрихкоды(ШтрихкодыДляОтработки);
		...
		Исключение
			...
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ИнформацияОбОшибке().Описание));
...
Процедура УстановитьУсловноеОформление()
	...
	Если Параметры.Операция = "РаботаСТранспортнымиКодами" Тогда
		...
		ЭтаФорма.Заголовок = "Уточнение данных упаковок";
		...
		//Описание
		Элементы.ПояснениеКФорме.Заголовок = НСтр("ru = 'Заполните GTIN, Количество вложенных позиций и Вид продукции для каждой упаковки. Данные будут перенесены в документ для отображения в дереве кодов маркировки. Остальные поля справочные и заполняются при необходимости регистрации нового штрихкода.'");
		
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШтрихкодыУпаковокРаботаСНоменклатурой", "Доступность", Не ШтрихкодыУпаковок.Количество() = 0);
		...
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШтрихкодыУпаковокКоличество",   "Видимость", Не Входящий);
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШтрихкодыУпаковокВидПродукции", "Видимость", Не Входящий);
		
		ВыводитьКомандыПреобразования = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьПоддержкуДвойногоФорматаТранспортныхКодов();
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Идентификаторы", "Видимость", ВыводитьКомандыПреобразования);
		
	Иначе
		
		ЭтаФорма.Заголовок = "Подбор номенклатуры и регистрация штрихкодов для потребительских и групповых кодов";
...
Процедура УстановитьВидимостьПолейНоменклатуры()
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШтрихкодыУпаковокНоменклатура",   "Видимость", ОтображатьДанныеПоНоменклатуре);
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШтрихкодыУпаковокХарактеристика", "Видимость", ОтображатьДанныеПоНоменклатуре И ИспользоватьХарактеристикиНоменклатуры);
	
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШтрихкодыНоменклатурыХарактеристика", "Видимость", ИспользоватьХарактеристикиНоменклатуры);
...
Процедура ЗаполнитьНоменклатуру(СозданнаяНоменклатура)
	...
	Для каждого ЭлементКоллекции Из СозданнаяНоменклатура Цикл
		...
		Для каждого Штрихкод Из СтрокиИдентификатору Цикл
			...
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПоискНоменклатурыПоШтрихкодуПослеЗагрузкиНоменклатуры(Штрихкод, ЭлементКоллекции.Номенклатура);
...
Функция СформироватьДанныеДляПередачиНаСервере() Экспорт
	...
	Для Каждого Элемент Из ШтрихкодыУпаковок Цикл
		...
		EAN = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодEANИзGTIN(Элемент.GTIN);
...
Процедура ПеренестиВДокументТранспортныеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Отмена Тогда
		...
		Для Каждого Сообщение Из СписокОшибок Цикл
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(Сообщение);
		КонецЦикла;	
		Возврат;
	КонецЕсли;
...
Процедура ЗавершитьЗаполнитьДанныеВНесколькихСтроках(СтруктураКодовМаркировки = Неопределено, ПараметрКоманды = Неопределено) Экспорт
	...
	Если Не СтруктураКодовМаркировки = Неопределено Тогда
		Для Каждого НомерСтроки Из  Элементы.ШтрихкодыУпаковок.ВыделенныеСтроки Цикл
			...
			ВидПродукции = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(СтрокаРедактирования.Номенклатура);
			СтрокаРедактирования.ВидПродукции = ВидПродукции;
			
			Если РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеХарактеристикУНоменклатуры(СтрокаРедактирования.Номенклатура) 
...
Функция ПолучитьДоступныеВидыПродукции()
	...
	Возврат РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.МассивДоступныхВидовПродукции();
...
Процедура ПодобратьGTINИзРегистраДляНоменклатуры()
	...
	Если Не ТекущиеДанные = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура)
			...
			ВидПродукции = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(ТекущиеДанные.Номенклатура);
...
Функция ПолучитьХарактеристикуНоменклатуры(Штрихкоды, Номенклатура)
	...
	EAN = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодEANИзGTIN(Штрихкоды);
	Характеристика = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьСведенияОХарактеристикеНоменклатуры(Номенклатура, EAN);
	
	Возврат ?(Характеристика <> Неопределено, Характеристика.Характеристика, Характеристика);
...
Процедура ЗаполнитьПоGTINНаСервере()
	...
	ДанныеПоНоменклатуре = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьДанныеОНоменклатуреПоGTIN(ТаблицаКодовGTIN); 
		...
		Для Каждого СтрокаНоменклатуры Из ДанныеПоНоменклатуре Цикл 
			...
			Для Каждого СтрокаШтрихкод Из СтрокиТЧШтрихкодов Цикл
				...
				Если ЗначениеЗаполнено(СтрокаНоменклатуры.Номенклатура) 
					И Не НаправлениеИсходногоДокумента = ПредопределенноеЗначение("Перечисление.НаправленияДокументовИСМПТК.Входящий") Тогда //Во входящем документе вид продукции и количество сохраняются по данным документа 
					ВидПродукции = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(СтрокаНоменклатуры.Номенклатура);
...
Процедура ПослеЗакрытияВопросаЗаписатьШтрихкодНоменклатурыНаСервере()
	...
	Попытка
		...
		Для Каждого СтрокаТЧ Из ШтрихкодыУпаковок Цикл
			...
			EAN 			 = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодEANИзGTIN(СтрокаТЧ.GTIN);
			GTIN 			 = СтрокаТЧ.GTIN;
			...
			Если Не ЗначениеЗаполнено(Номенклатура) 
				...
				И РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьВедениеУчетаПоХарактеристикамУНоменклатуры(Номенклатура)
				И РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьИспользованиеХарактеристик()) Тогда
			...
			Если Не НоменклатураИзРегистра = Неопределено Тогда
				Если ЗначениеЗаполнено(НоменклатураИзРегистра.Номенклатура) Тогда //Может быть пустым в конфигурациях, поддерживающих неуникальные ШК номенклатуры
					...
					ТекстСообщенияУжеЗарегистрирован = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщенияУжеЗарегистрирован, EAN, НоменклатураИзРегистра.Номенклатура);
				Иначе
				    ТекстСообщенияУжеЗарегистрирован = НСтр("ru='Штрихкод ""%1"" уже зарегистрирован в базе!'");
					ТекстСообщенияУжеЗарегистрирован = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщенияУжеЗарегистрирован, EAN);
				КонецЕсли;
				РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщенияУжеЗарегистрирован);	
				Продолжить;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(GTIN) И Не РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ПроверитьКорректностьGTIN(GTIN) 
				...
				ТекстСообщенияНеФормат = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщенияНеФормат, EAN, GTIN);
				РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщенияНеФормат);	
				Продолжить;
			КонецЕсли;

			//Регистрируем новый ШК для позиции
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ЗаписатьШтрихкодНоменклатуры(Номенклатура, EAN, Неопределено, Характеристика);
		...
		Для Каждого СтрокаДанных Из ТаблицаДляСообщения Цикл
			...
			ТекстСообщения = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, СтрокаДанных.Номенклатура, СтрокаДанных.Штрихкод);
			РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		КонецЦикла;
	...
	Исключение
		...
		ТекстСообщения = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки(Информация));
		РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
...
Функция ПроверитьДанныеУпаковок() 
	...
	Для Каждого СтрокаТЧ Из ШтрихкодыУпаковок Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.GTIN) Тогда 
			ТекстСообщения = НСтр("ru='У штрихкода ""'") + СтрокаТЧ.Штрихкод + НСтр("ru='"" не указан GTIN'");
		Иначе
			//Проверяем формат
			...
			Если СтрДлина(СтрокаТЧ.GTIN) <> 14 Или Не GTINКорректногоФормата Тогда
				ТекстСообщения = НСтр("ru='У штрихкода ""'") + СтрокаТЧ.Штрихкод + НСтр("ru='"" указан не соответствующий формату GTIN'");	
			Иначе
				//Проверяем, связан ли ШК в регистре с номенклатурой
				НоменклатураИзРегистра = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПроверитьНаличиеШтрихкодаУноменклатуры(, РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодEANИзGTIN(СтрокаТЧ.GTIN));
				Если Не НоменклатураИзРегистра.Количество() = 0 Тогда
					//ШК связан, сверяем с данными ТЧ
					Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) 
						И Не СтрокаТЧ.Номенклатура = НоменклатураИзРегистра.Номенклатура Тогда
						ТекстСообщения = НСтр("ru='GTIN, указанный у штрихкода ""'") + СтрокаТЧ.Штрихкод + НСтр("ru='"", связан с другой номенклатурой в регистре Штрихкоды'");	
					КонецЕсли;
				Иначе
					//ШК не связан, проверяем номенклатуру
					Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) Тогда
						ТекстСообщения = НСтр("ru='GTIN, указанный у штрихкода ""'") + СтрокаТЧ.Штрихкод + НСтр("ru='"", не зарегистрирован в регистре Штрихкоды для номенклатуры ""'") 
										+ СтрокаТЧ.Номенклатура + НСтр("ru='""'");	
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Количество) Тогда 
			Если Не ТекстСообщения = "" Тогда
				ТекстСообщения = ТекстСообщения + НСтр("ru=' и не заполнено количество вложенных товаров в этой упаковке.'");
			Иначе
				//С GTIN все в порядке, не указано только количество
				ТекстСообщения = НСтр("ru='У штрихкода ""'") + СтрокаТЧ.Штрихкод + НСтр("ru='"" не заполнено количество вложенных товаров.'")
			КонецЕсли;
		Иначе
			Если Не ТекстСообщения = "" Тогда
				ТекстСообщения = ТекстСообщения + "."; //закрываем сообщение о проверке GTIN
			КонецЕсли;
		КонецЕсли;
