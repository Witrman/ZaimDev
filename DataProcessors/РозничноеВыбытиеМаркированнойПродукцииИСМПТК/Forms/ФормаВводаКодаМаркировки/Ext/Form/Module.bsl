...
Процедура ПриЗакрытии()
	...
	Если КодМаркировкиОбработан Тогда
		...
		Если ОбработатьБезВводаМарки Тогда
			РозничноеВыбытиеИСМПТККлиент.ОбработатьШтрихкодКакНемаркируемуюПродукцию(ВладелецФормы, ДанныеШтрихкодаБезМаркировки, ОткрытиеПослеДобавления);
		Иначе
			РозничноеВыбытиеИСМПТККлиент.ОбработатьШтрихкодКакМаркируемуюПродукциюИСМПТК(ВладелецФормы, ДанныеМаркировкиСоСканера, КлючСвязиСтроки, ОткрытиеПослеДобавления);
		КонецЕсли;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если ВводДоступен() Тогда
		Если (Событие = "Штрихкод" Или Событие = "ПолученШтрихкод") 
			И ВозможностьВводаПоШК() Тогда
			
			ОбработкаСобытияСканераОбщая(Параметр, Событие, Источник, Данные);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаСобытияСканераОбщая(Параметр, Событие, Источник, Данные = Неопределено)
	...
	НоменклатураПустаяСсылка   	   = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
	...
	Если Не ОтсканированныйШКСоответствует Тогда
		...
		Если Не Номенклатура = НоменклатураПоШК 
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Отсканированный штрихкод относится к другой номенклатуре: %1%2!'"), НоменклатураПоШК, СтрокаДляПодстановки);
			ВывестиСообщениеНаСтраницеВводаКодаМаркировки(ТекстСообщения);
			Возврат;
		КонецЕсли;
...
Процедура ДобавитьБезМаркировкиЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Штрихкод не соответствует формату кода маркировки.'");
		ВывестиСообщениеНаСтраницеВводаКодаМаркировки(ТекстСообщения);
...
Процедура ПоискПоШтрихкодуЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ТипШтрихкодаСоответствует = РозничноеВыбытиеИСМПТКВызовСервера.ПроверитьТипШтрихкодаВФормеВводаКМ(ДанныеШтрихкода.Штрихкод);
	//Проверка на введенный ЕАН8/ЕАН13 выполняется заранее, т.к. в "ПолучитьКодыМаркировки" в случае указания такого ШК
	//формируется сообщение в ином формате, не подходящим для данной розничной формы. 
	//Но менять его в общей процедуре нельзя, т.к. она нацелена на оптовый контур и документы ИС МПТ.
	Если Не ТипШтрихкодаСоответствует Тогда
		СообщитьОНесоответствииУказанногоШтрихкода(ДанныеШтрихкода.Штрихкод);
		Возврат;
	КонецЕсли;
		
	СтруктураКодовМаркировки = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйВызовСервера.ПолучитьКодыМаркировки(ДанныеШтрихкода);   
	...
	//Проверяем вид упаковки - в рознице транспортные коды упаковок указывать нельзя
	...
	Если ДополнительныеПараметры = Неопределено Тогда
		ДополнительныеПараметры = Новый Структура("СтруктураКодовМаркировки", СтруктураКодовМаркировки);
	КонецЕсли;
...
Процедура СообщитьОНесоответствииУказанногоШтрихкода(Штрихкод)
	...
	ТекстСообщения = НСтр("ru = 'Формат указанного штрихкода не соответствует требуемому формату кода маркировки.'"); 
	ВывестиСообщениеНаСтраницеВводаКодаМаркировки(ТекстСообщения);
