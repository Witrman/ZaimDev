...
Процедура РассчитатьСреднийЗаработокПосле_29_12_2007(СпособРасчета) Экспорт
	...
	КомпенсацияОтпуска = ?(ОсновнойВидРасчета = ПредопределенноеЗначение("ПланВидовРасчета.ОсновныеНачисленияОрганизаций.КомпенсацияОтпуска"), Истина, Ложь);

	ДатыРасчетногоПериода           = Обработки.РасчетПоСреднемуЗаработку.ПолучитьДатыРасчетногоПериода(Организация, Сотрудник, ДатаНачала, КомпенсацияОтпуска);
	ДатаНачалаРасчетногоПериода 	= ДатыРасчетногоПериода.ДатаНачалаРасчетногоПериода;
	ДатаОкончанияРасчетногоПериода 	= ДатыРасчетногоПериода.ДатаОкончанияРасчетногоПериода;
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Период > ДатаОкончанияРасчетногоПериода Тогда
			...
			Если УчитыватьКадровыеПерестановкиПриРасчетеСреднегоЗаработка И 
				(РассчитыватьКоэффициентИндексацииВПределахКадровыхПерестановок И Выборка.Период = ПериодПоследнегоПеремещения)
				ИЛИ (НЕ РассчитыватьКоэффициентИндексацииВПределахКадровыхПерестановок И Выборка.Период <= ПериодПоследнегоПеремещения) Тогда
				ТекущийКоэффициент = 1;
			Иначе 
				Если ДатаНачала < ДатаНачалаПримененияКоэффициентаПоМесяцуПовышения 
					...
					ТекущийКоэффициент = ТекущийКоэффициент * Выборка.Коэффициент;
				Иначе
					ТекущийКоэффициент = Выборка.Коэффициент;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			Иначе 
				Если ДатаНачала < ДатаНачалаПримененияКоэффициентаПоМесяцуПовышения 
					ИЛИ РасчетКоэффициентаНарастающимИтогом Тогда
					ТекущийКоэффициент = ?(Выборка.Коэффициент = 0, 1, ТекущийКоэффициент * Выборка.Коэффициент);
				Иначе
					ТекущийКоэффициент = ?(Выборка.Коэффициент = 0, 1, Выборка.Коэффициент);
				КонецЕсли;
			КонецЕсли;
	...
	Если ПервыйМесяцПремии < ПоследнийМесяцПремии Тогда
		...
		Пока ЕстьДанные Цикл
			...
			Пока ЕстьДанные 
				...
				Если УчитыватьКадровыеПерестановкиПриРасчетеСреднегоЗаработка 
					И ВыборкаКоэффициентовИндексацииПремий.Период < ПериодПоследнегоПеремещения Тогда
					ТекущийКоэффициент = 1;
				Иначе 
					Если ДатаНачала < ДатаНачалаПримененияКоэффициентаПоМесяцуПовышения 
						ИЛИ РасчетКоэффициентаНарастающимИтогом Тогда
