...
Процедура ЗапуститьВыгрузкуДанныхНаСервере()
	...
	Попытка
		...
		ИмяФайлаВыгрузки = ФайлыБТС.ОбщийКаталогВременныхФайлов() + Строка(Новый УникальныйИдентификатор()) + ".zip";
...
Процедура ЗаполнитьПользователейИБ()
	...
	Запрос.Текст = 
    	...
    	|	ПОДСТРОКА(ПользователиКонтактнаяИнформация.Представление, 1, 1000) КАК ЭлектроннаяПочта
		|ИЗ
        ...
        |		ПО (ПользователиКонтактнаяИнформация.Объект = Пользователи.Ссылка)
        |		И (ПользователиКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
       	|		И (ПОДСТРОКА(ПользователиКонтактнаяИнформация.Представление, 1, 1000) <> """")
		|ГДЕ
	...
	//Запрос.УстановитьПараметр("ВидАдресЭлектроннойПочты", Справочники.ВидыКонтактнойИнформации.СлужебныйАдресЭлектроннойПочтыПользователя);
