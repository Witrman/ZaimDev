...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначениеВДанныеФормы(ДеревоВсехИспользуемыхОтчетовПоКатегориям,   ДеревоОтчетов);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ДанныеНайденногоВидаОтчета) = Тип("Структура") И ДанныеНайденногоВидаОтчета.Свойство("Наименование") Тогда
		Если ЗначениеЗаполнено(ДанныеНайденногоВидаОтчета.Наименование) Тогда
			...
			НайденныеОтчеты = РанееСозданныеОтчеты.НайтиСтроки(Новый Структура("ПолноеИмя,Ссылка", ДанныеНайденногоВидаОтчета.ПолноеИмя, ДанныеНайденногоВидаОтчета.Ссылка));
			...
			ОткрытьФормуОтчетаНаКлиенте(ДанныеНайденногоВидаОтчета, ПараметрыФормы.Организация, ПараметрыФормы.ДатаНачалаПериодаОтчета, ПараметрыФормы.ДатаКонцаПериодаОтчета);
...
Процедура Выбрать(Команда)
	...
	Если ТекДеревоОтчетов.ТекущиеДанные <> Неопределено
		...
		ОткрытьФормуОтчетаНаКлиенте(ТекДеревоОтчетов.ТекущиеДанные, ПараметрыФормы.Организация, ПараметрыФормы.ДатаНачалаПериодаОтчета, ПараметрыФормы.ДатаКонцаПериодаОтчета);
	КонецЕсли;
...
Функция ДеревоВсехИспользуемыхОтчетов()
	...
	//Дерево предназначено только для регламентированных отчетов
...
&НаСервере
Процедура ЗаполнитьРанееСозданныеОтчеты()
	...
	ДобавитьРанееСозданныеВИзбранные(ДеревоОтчетов, ТаблицаСозданных);
...
&НаКлиенте
Процедура ОткрытьФормуОтчетаНаКлиенте(ДанныеОтчета, ОрганизацияОтчета = Неопределено, ДатаНачалаПериодаОтчета = Неопределено, ДатаОкончанияПериодаОтчета = Неопределено, ПериодичностьОтчета = Неопределено)
	...
	Если ДанныеОтчета.ПолноеИмя = "Документ.РегламентированныйОтчет" Тогда
		...
		РезультатОткрытияФормыНаСервере = ОткрытьФормуОтчетаНаСервере(ТекДанныеОтчета, ОрганизацияОтчета, ДатаНачалаПериодаОтчета, ДатаОкончанияПериодаОтчета, ПериодичностьОтчета, ИсточникОтчета);
		...
		ИначеЕсли РезультатОткрытияФормыНаСервере = "Открыть внутренний отчет-документ" Тогда
			ОткрытьФорму("Документ." + ИсточникОтчета + ".Форма.ОсновнаяФорма", , , ИсточникОтчета);		
		...
		ИначеЕсли ТипЗнч(РезультатОткрытияФормыНаСервере) = Тип("Структура") Тогда
			ОткрытьФорму("Отчет." + ИсточникОтчета + ".Форма.ОсновнаяФорма", РезультатОткрытияФормыНаСервере);			
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(ДанныеОтчета.Ссылка) Тогда
		
		ПараметрыОткрытияОтчета = СформироватьПараметрыОтчетаНаСервере(ТекДанныеОтчета, ОрганизацияОтчета, ДатаНачалаПериодаОтчета, ДатаОкончанияПериодаОтчета);
		ОткрытьФорму(ДанныеОтчета.ПолноеИмя + ".ФормаОбъекта", ПараметрыОткрытияОтчета);
		
	Иначе
		
		ОткрытьФорму(ДанныеОтчета.ПолноеИмя + ".ФормаОбъекта");
