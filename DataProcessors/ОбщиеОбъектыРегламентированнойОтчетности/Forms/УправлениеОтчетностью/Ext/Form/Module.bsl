...
Процедура ОтразитьВУчете(Команда)
	...
	Иначе
		Если ТекущиеДанные.ИсточникОтчета = "РегламентированныйОтчет910Форма" Тогда
			ДополнительныеПараметрыОтражения = СформироватьДополнительныеПараметрыОтраженияДанных(ТекущиеДанные.НалогоплательщикЯвляетсяИП);
	...
	Иначе
		Если Форма.Периодичность = "По итогам" Тогда
			...
			Если Форма.МассивНаименованийПравыхГраницПериода.Свойство("Месяц" + Месяц(Форма.ДатаКонцаПериодаОтчета), НайденноеЗначение) Тогда
				Подсказка = "Отчеты, представляемые по истечении " + НайденноеЗначение + " " + Формат(Форма.Год, "ЧГ=0") + " года";
			КонецЕсли;
		Иначе
			Если Форма.ДатаНачалаПериодаОтчета <= Форма.ДатаКонцаПериодаОтчета Тогда
				Подсказка = "Отчеты за период " + ПредставлениеПериода(Форма.ДатаНачалаПериодаОтчета, КонецДня(Форма.ДатаКонцаПериодаОтчета), "ФП = Истина");
			Иначе
				Подсказка = "";
			КонецЕсли;
		КонецЕсли;
...
Процедура УправлениеНадписямиМесяцев(Форма)
	...
	ИначеЕсли Месяц = 2 Тогда
		...
		Форма.Период = "Февраль";
		
	ИначеЕсли Месяц = 3 Тогда
		
		Форма.Период = "Март (I квартал)";
	...
	ИначеЕсли Месяц = 9 Тогда
		...
		Форма.Период = "Сентябрь (III квартал)";
		
	ИначеЕсли Месяц = 10 Тогда
		
		Форма.Период = "Октябрь";
	...
	ИначеЕсли Месяц = 12 Тогда
		...
		Форма.Период = "Декабрь (IV квартал)";
