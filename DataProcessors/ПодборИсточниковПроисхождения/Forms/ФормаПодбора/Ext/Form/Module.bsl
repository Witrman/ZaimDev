...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПодборПоРезервам") Тогда
		Список.ТекстЗапроса = ТекстЗапросаПодборПоРезервам();
	КонецЕсли;
...
Функция ВыбратьВсеНаСервере()
	...
	//скопируем отборы из пользовательских настройек
	ОтборНастроек = НастройкиКомпоновки.Отбор;
	Для Каждого ЭлементНастроек Из Список.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		
		Для Каждого ЭлементКомпоновщика Из Список.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			
			Если ЭлементНастроек.ИдентификаторПользовательскойНастройки = ЭлементКомпоновщика.ИдентификаторПользовательскойНастройки Тогда
				НовыйОтбор = ОтборНастроек.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				НовыйОтбор.ЛевоеЗначение  = ЭлементКомпоновщика.ЛевоеЗначение;
				
				НовыйОтбор.Использование = ЭлементНастроек.Использование;
				НовыйОтбор.ВидСравнения  = ЭлементНастроек.ВидСравнения;
				НовыйОтбор.ПравоеЗначение = ЭлементНастроек.ПравоеЗначение;
			КонецЕсли;
...
// Формирует текст запроса динамического списка подбора источников по резервам товаров ВС.
//
&НаСервере
Функция ТекстЗапросаПодборПоРезервам()
	...
	|	СправочникТовара.Ссылка КАК Товар,
	|	ТоварыВиртуальногоСкладаВРезерве.КоличествоОстаток КАК Остаток,
	|	ТоварыВиртуальногоСкладаВРезерве.ДокументПартии КАК ДокументПартии,
	...
	|	ЕСТЬNULL(СведенияОНоменклатуреГСВССрезПоследних.ПризнакАктивности, ЛОЖЬ) КАК ПризнакАктивности,
	|	ЕСТЬNULL(СправочникИсточникиПроисхождения.ГСВС.ПризнакВозможностиВыбора, ЛОЖЬ) КАК ПризнакВозможностиВыбора,
	|	ЕСТЬNULL(СведенияОНоменклатуреГСВССрезПоследних.ПризнакУникальногоТовара, ЛОЖЬ) КАК ПризнакУникальногоТовара,
	|	СправочникИсточникиПроисхождения.ГСВС.КодГСВС КАК КодГСВС,
	|	ЕСТЬNULL(СведенияОНоменклатуреГСВССрезПоследних.ПризнакПеречняИзьятий, ЛОЖЬ) КАК ПризнакПеречняИзьятий,
	|	ТоварыВиртуальногоСкладаВРезерве.ДокументПартии.Дата КАК ДатаДокументаПартии,
	|	ТоварыВиртуальногоСкладаВРезерве.ЕдиницаИзмерения,
	|	СправочникТовара.Наименование КАК НаименованиеТовара,
	|	СправочникИсточникиПроисхождения.МеткаТовара КАК ФизическаяМетка,
	|	СправочникИсточникиПроисхождения.ДополнительныйИдентификатор КАК ИДТовара,
	...
	|				И Склад = &Склад
	|				И ВЫБОР
	|					КОГДА &ПоддержкаУчетаСтруктурныхПодразделений
	|						ТОГДА СтруктурноеПодразделение = &СтруктурноеПодразделение
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ
	|				И Номенклатура.Ссылка ССЫЛКА Справочник.Номенклатура) КАК ТоварыВиртуальногоСкладаВРезерве
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникТовара
	|		ПО ТоварыВиртуальногоСкладаВРезерве.Номенклатура.Ссылка = СправочникТовара.Ссылка
	|ГДЕ
	|	ТоварыВиртуальногоСкладаВРезерве.КоличествоОстаток <> 0
	|	И ТоварыВиртуальногоСкладаВРезерве.СпособВыписки = ЗНАЧЕНИЕ(Перечисление.СпособВыпискиСНТ.ВЭлектронномВиде)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_Источники.Наименование,
	|	ВТ_Источники.Ссылка,
	|	ВТ_Источники.ПометкаУдаления,
	|	ВТ_Источники.КодТНВЭД,
	|	ВТ_Источники.ГСВС,
	|	ВТ_Источники.Товар,
	|	ВТ_Источники.Остаток,
	|	ВТ_Источники.ДокументПартии,
	|	ВТ_Источники.ПризнакУчетаНаВиртуальномСкладеГСВС,
	|	ВТ_Источники.ПризнакАктивности,
	|	ВТ_Источники.ПризнакВозможностиВыбора,
	|	ВТ_Источники.ПризнакУникальногоТовара,
	|	ВТ_Источники.КодГСВС,
	|	ВТ_Источники.ПризнакПеречняИзьятий,
	|	ВТ_Источники.ДатаДокументаПартии,
	|	ВТ_Источники.ЕдиницаИзмерения,
	|	ВТ_Источники.НаименованиеТовара,
	|	ВТ_Источники.ФизическаяМетка,
	|	ВТ_Источники.ИДТовара,
	|	ВТ_Источники.ПризнакУчетаНаВиртуальномСкладе
	|ИЗ
