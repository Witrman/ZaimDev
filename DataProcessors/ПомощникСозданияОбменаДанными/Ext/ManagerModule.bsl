...
Процедура ПроверитьПодключениеККорреспонденту(Параметры, АдресРезультата) Экспорт
	...
	Если НастройкиПодключения.ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.COM Тогда
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено
			ИЛИ НастройкиПодключения.ИмяПланаОбмена = "СинхронизацияДанныхЧерезУниверсальныйФормат" Тогда
			...
			Если НастройкиПодключения.ИмяПланаОбмена = "СинхронизацияДанныхЧерезУниверсальныйФормат" Тогда
				ПараметрыКорреспондента.Вставить("ИмяПланаОбмена", НастройкиПодключения.ИмяПланаОбмена);
			КонецЕсли;
		...
		Иначе
			...
			РезультатПроверки.СообщениеОбОшибке = НСтр("ru = 'Для подключения необходимо начать настройку из конфигурации корреспондента.'");
...
Процедура СохранитьНастройкиПодключения(Параметры, АдресРезультата) Экспорт
	...
	ИначеЕсли НастройкиПодключения.ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.WS Тогда
		...
		Если НастройкиПодключения.ИмяПланаОбменаКорреспондента = "СинхронизацияДанныхЧерезУниверсальныйФормат" Тогда
			WSПрокси = ОбменДаннымиСервер.ПолучитьWSПрокси_2_1_1_7(НастройкиПодключения);
		Иначе
...
Функция КодУзла(НастройкиПодключения)
	...
	Если НастройкиПодключения.ИспользоватьПрефиксыДляНастройкиОбмена
		...
		Или НастройкиПодключения.ИмяПланаОбмена = "ОбменЗарплатаИУправлениеПерсоналомБухгалтерияПредприятия" Тогда
...
Процедура ЗаполнитьНастройкиПодключенияИзXML(НастройкиПодключения,
	...
	Попытка
		ПрочитатьНастройкиПодключенияИзXMLВСтруктуру(СтруктураНастроек, ИмяФайлаСтрокаXML, ЭтоФайл, НастройкиПодключения);
	Исключение
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ПрочитатьНастройкиПодключенияИзXMLВСтруктуру(СтруктураНастроек, ИмяФайлаСтрокаXML, ЭтоФайл, НастройкиПодключения)
