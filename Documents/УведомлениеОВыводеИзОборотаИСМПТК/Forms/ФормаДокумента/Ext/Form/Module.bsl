	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") Тогда
		...
		Если ИзменитьДокументОснование Тогда
			...
			Иначе
				...
				"ru = 'На основании документа ""%ДокументОснование%"" уже введено Уведомление о выводе из оборота. Привязать текущее Уведомление к выбранному документу без отсоединения ранее связанного Уведомления?'");
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляВыводаИзОборота();
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВыводеИзОборотаИСМПТКОтправить", "Доступность", НеОтправлен);
...
&НаСервере
Процедура ЗаполнитьДополнительныеСведенияДокументаОснования() Экспорт
	
	Объект.ДокументОснованиеНаименование = нСтр("ru='Уведомление о выводе из оборота'", "ru");
	Объект.ДокументОснованиеДата  = Объект.Дата;
	Объект.ДокументОснованиеНомер = Объект.Номер;
...
Функция ПроверитьСвязанноеУведомление(ВыбранноеЗначение)
	...
	Возврат ИнтеграцияИСМПТК.ПроверитьНаличиеСвязанногоУведомленияОВыводеИзОборота(ВыбранноеЗначение, Истина);
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат ИнтеграцияИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияВыводИзОборотаНаСервере(Объект.ДокументОснование);
...
Процедура ДеревоМаркировкиПередУдалением(Элемент, Отказ)
	...
	Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияДокументовИСМПТК.Входящий") Тогда
		Отказ = Истина;
		Возврат;
	ИначеЕсли Не Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСМПТК.НеОпределен") Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьОтветСервераПоСостояниюКМ(МассивКМ, ОтветСервераСостояниеКМ)
	...
	ИнтеграцияИСМПТК.ОбработатьОтветСервераПоСостояниюКМ_Общая(ЭтаФорма, МассивКМ, ОтветСервераСостояниеКМ, "УведОВыводеИзВОборота", НовоеСоответствиеСтатусов);
