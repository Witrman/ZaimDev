...
Процедура СообщитьОПрекращенииПредоставленияВычета()
	Если ЗначениеЗаполнено(МесяцЗавершения) Тогда
		...
		Пока ВыборкаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='Прекращено предоставление вычета ""%1"", данный вычет должен был предоставляться %2. '");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ВыборкаЗапроса.ВидВычета, 
				?(ЗначениеЗаполнено(ВыборкаЗапроса.ДействуетПо), "по: " + Формат(ВыборкаЗапроса.ДействуетПо, "ДЛФ=DD"),"бессрочно"));
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СообщитьОПрекращенииПредоставленияВычета();
