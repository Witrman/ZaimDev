...
Функция ПечатьСчетаЗаказа(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		Если МакетНастроенДляВыводаKaspiQR Тогда
			ОбщиеДанныеНастроек = QRКод_ГенерацияКода.СохраненныеРеквизитыКода(Шапка.Поставщик, Шапка.СтруктурноеПодразделение);
			Если ОбщиеДанныеНастроек.Количество() > 0 Тогда
				...
				Если ДанныеКомпании.Свойство("ВыводитьQRКод") И ДанныеКомпании.ВыводитьQRКод = Истина Тогда
					...
					НомерДокументаДляПечати = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Шапка.Номер, Шапка.Ссылка);
					...
					Если ТипЗнч(ОтветСервиса) = Тип("ДвоичныеДанные") Тогда
						...
						ТабДокумент.Область(1, 7, 6, 33).Разъединить();
						ТабДокумент.Область(1, 7, 6, 28).Объединить();
