...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.СообщениеОбмена.Пустая() Тогда
		ОбъектДокумента = Параметры.СообщениеОбмена.ПолучитьОбъект();
...
Процедура ЗапроситьСостояниеЭД(Команда)
	...
	Если ТребуетсяПодпись Тогда
		...
		ИначеЕсли ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезВК") Тогда
			...
			КонецЕсли;
		ИначеЕсли ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезДопОбработку") Тогда 
			РеквизитыНастройкиОбмена = ПолучитьЗначенияРеквизитов(
			Объект.НастройкаОбмена, "ИмяВнешнегоМодуля,АсинхронноЧерезДопОбработку");
			Если РеквизитыНастройкиОбмена.АсинхронноЧерезДопОбработку Тогда 
				...
				ВнешнийПодключаемыйМодуль = ОбменСБанкамиСлужебныйКлиент.ВнешнийПодключаемыйМодульЧерезДополнительнуюОбработку(ИмяВнешнегоМодуля, Неопределено);
				ОбменСБанкамиСлужебныйКлиент.ОтправитьЗапросЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, Неопределено, 21, ЗапросСостоянияЭД)
			КонецЕсли;		
		КонецЕсли;
	Иначе
		Если ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
...
Процедура НачатьОтзывЭД(Строка, Параметры) Экспорт
	...
	Иначе
		ОбменСБанкамиСлужебныйКлиент.ОбработатьЭД(МассивСсылок, "Отправить", Отзыв);
	КонецЕсли;
