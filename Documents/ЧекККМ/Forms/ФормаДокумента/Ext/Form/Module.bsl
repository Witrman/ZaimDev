...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		Если ИмяСобытия = "ScanData" Тогда
			...
			ШтрихкодированиеИСМПТККлиентСервер.ЗакодироватьШтрихкодДанныхBase64(ТекущиеДанные);
...
Процедура ОповещениеОткрытьФормуАвторизацииЭТ(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатОткрытияФормы) = Тип("Структура") Тогда
		...
		Иначе
			...
			Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРозничнаяПродажа.Продажа") Тогда
				Если ДополнительныеПараметры.Свойство("ИзменитьТипТранзакции") Тогда
					НаименованиеОперации = "отмены оплаты";
				Иначе
					НаименованиеОперации = "оплаты";
				КонецЕсли;
			Иначе
				НаименованиеОперации = "отмены оплаты";
			КонецЕсли;
			
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Требуется выполнить операцию %1 на эквайринговом терминале.'"),НаименованиеОперации) + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru = 'Сумма операции:'") + " "+ РезультатОткрытияФормы.Сумма  + Символы.ПС;
			ТекстВопроса = ТекстВопроса + Символы.ПС;
			ТекстВопроса = ТекстВопроса + СтрШаблон(НСтр("ru = 'Операция %1 на эквайринговом терминале прошла успешно?'"),НаименованиеОперации);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, ИмяТаблицы, ЭтоИзФормыПодбора=Ложь)
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если НЕ ЭтоВставкаИзБуфера Тогда
			...
			Если ВестиУчетМаркируемойПродукцииИСМПТК И СтрокаТабличнойЧасти <> Неопределено И ИмяТаблицы = "Товары" Тогда
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта, СчетаУчетаКЗаполнению)
	...
	Если ДанныеОбъекта.ВестиУчетМаркируемойПродукцииИСМПТК Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	ВидыМаркируемойПродукции.ЗагрузитьЗначения(РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.УчитываемыеВидыМаркируемойПродукции());
...
Процедура СформироватьНадписьЦеныИВалюта(Форма)
	...
	Если Объект.СуммаВключаетНДС Тогда
		Форма.Элементы.ГруппаИтогиНДС.ПодчиненныеЭлементы.ИтогиВсегоНДС.Заголовок = "НДС (в т. ч.)";
	Иначе
		Форма.Элементы.ГруппаИтогиНДС.ПодчиненныеЭлементы.ИтогиВсегоНДС.Заголовок = "НДС (сверху)";
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьПоШтрихкодуНаСервере(Знач ДанныеШтрихкода)
	
	ШтрихкодированиеИСМПТККлиентСервер.ДекодироватьШтрихкодДанныхBase64(ДанныеШтрихкода);
	...
	Если ВестиУчетМаркируемойПродукцииИСМПТК Тогда
		...
		НастройкиРазбораКодаМаркировки = РазборКодаМаркировкиИСМПТКСлужебныйПовтИсп.НастройкиРазбораКодаМаркировки();
		РезультатРазбораКодаМаркировки = РазборКодаМаркировкиИСМПТКСлужебный.РазобратьКодМаркировки(
			Штрихкод, НастройкиРазбораКодаМаркировки.ДоступныеВидыПродукции, ПримечаниеКРезультатуРазбора, НастройкиРазбораКодаМаркировки);
		...
		Если РезультатРазбораКодаМаркировки <> Неопределено Тогда
			...
			РезультатРазбораКодаМаркировки.Свойство("СоставКодаМаркировки", СоставКодаМаркировки);
			РезультатРазбораКодаМаркировки.Свойство("ТипШтрихкода", ТипШтрихкода);
			
			Если СоставКодаМаркировки <> Неопределено Тогда
				Если СоставКодаМаркировки.Свойство("SSCC") Тогда
					ТекстСообщения = НСтр("ru = 'Введеный код маркировки соответстует транспортной упаковке и не может использоваться в документе!'");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
					Возврат;
				Иначе
					ШтрихкодНоменклатуры = СоставКодаМаркировки.EAN;
				КонецЕсли;
			КонецЕсли;
			
			Если ТипШтрихкода = Перечисления.ТипыШтрихкодовИСМПТК.DataMatrix
				ИЛИ ТипШтрихкода = Перечисления.ТипыШтрихкодовИСМПТК.GS1_DataMatrix Тогда
				КодМаркировки = РезультатРазбораКодаМаркировки.КодМаркировки;
			КонецЕсли;
	...
	Если ТаблицаНоменклатурыПоШтрихкоду.Количество() = 1 Тогда
		ДобавитьНоменклатуруПоШтрихкодуНаСервере(ТаблицаНоменклатурыПоШтрихкоду[0].Номенклатура, ШтрихкодНоменклатуры, КодМаркировки);
	Иначе
...
Процедура ЗаполнитьДанныеМаркировкиВСтроке(СтрокаТабличнойЧасти) Экспорт
	...
	ВидПродукцииИСМПТК      = ВидПродукцииИС(Новый Структура("ОсобенностьУчета", СтрокаТабличнойЧасти.Номенклатура.ОсобенностьУчета));
