...
Функция ПечатьТоварныйЧек(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ПредставлениеПоставщика = ОбщегоНазначенияБКВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, 
				"ПолноеНаименование,БИН_ИИН,ЮридическийАдрес,Телефоны,");
		...
		ДанныеПечатиСуммаПрописью.СуммаПрописью = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(Всего, Константы.ВалютаРегламентированногоУчета.Получить());
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТоварныйЧек") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьТоварныйЧек(МассивОбъектов, ОбъектыПечати), , "Документ.ЧекККМ.ПФ_MXL_ТоварныйЧек");
...
Процедура ПодготовитьКодМаркировкиBase64(СтруктураПечати, ДанныеШтрихКода)
	...
	СтруктураПечати.Вставить("КодМаркировкиBase64", ШтрихкодированиеИСМПТККлиентСервер.ШтрихкодВBase64(СтрокаШтрихКодBase64));
...
Процедура ДобавитьБлокКода(СтрокаКода, ДанныеШтрихКода, Блок, ДобавитьРазделитель = Ложь)
	...
	Если Не ДанныеШтрихКода.Получить(Блок) = Неопределено Тогда 
		...
		Если ДобавитьРазделитель Тогда 
			СтрокаКода 	= СтрокаКода + ПодключаемоеОборудованиеИСМПТККлиентСервер.РазделительGS1() + Блок + ЗначениеБлока;
		Иначе 
...
Функция СобратьДанныеЧеков(ДокументСсылка, НомерСекции, ВерсияФормата, СообщениеОбОшибке) Экспорт
	...
	Если Реквизиты.Следующий() Тогда
			...
			Если НЕ ПустаяСтрока(Позиция.КодМаркировки)
				...
				Если НЕ РезультатРазбораКодаМаркировки = Неопределено Тогда
					...
					Если СоставКодаМаркировки <> Неопределено Тогда
						...
						СтруктураКода = ПодключаемоеОборудованиеИСМПТККлиентСервер.РазобратьСтрокуШтрихкодаGS1(Позиция.КодМаркировки);
		...
		// Наличные
		Если Реквизиты.СуммаДокумента > СуммаБезналичнойОплаты Тогда
			
			СтрокаОплаты = Новый Структура();
			СтрокаОплаты.Вставить("ТипОплаты", Перечисления.ТипыОплатыККТ.Наличные);
			СтрокаОплаты.Вставить("Сумма", Реквизиты.СуммаДокумента-СуммаБезналичнойОплаты);
			ОбщиеПараметры.ТаблицаОплат.Добавить(СтрокаОплаты);
