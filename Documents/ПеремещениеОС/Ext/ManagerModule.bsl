...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Накладная на перемещение ОС'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			,
			"Документ.ПеремещениеОС.ПФ_MXL_ДА4");
	КонецЕсли;
...
Функция ПечатьНакладная(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	ТабДокумент.КлючПараметровПечати = "ПеремещениеОС_Накладная";
...
Функция ПечатьДА4(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПеремещениеОС.ПФ_MXL_ДА4");
	...
	Пока ШапкаДокумента.Следующий() Цикл
		...
		Если ВедетсяАналитическийУчет Тогда 
			...
			СчетаОС.Добавить(ПланыСчетов.Типовой.ОсновныеСредства);
			СчетаОС.Добавить(ПланыСчетов.Типовой.АрендованноеИмущество);
			Запрос.УстановитьПараметр("СчетаОС", СчетаОС);
			
			СчетаАмортизации = Новый Массив;
			СчетаАмортизации.Добавить(ПланыСчетов.Типовой.АмортизацияОсновныхСредств);
			СчетаАмортизации.Добавить(ПланыСчетов.Типовой.АмортизацияАрендованногоИмущества);
			...
			|ИЗ
			|	РегистрБухгалтерии.Типовой.Остатки(&Дата, Счет В ИЕРАРХИИ (&СчетаОС), &ВидыСубконтоОС, Субконто1 В (&МассивОС)) КАК ТиповойОстатки
			|
			|ОБЪЕДИНИТЬ
			|
			|ВЫБРАТЬ
			...
			|ИЗ
			|	РегистрБухгалтерии.Типовой.Остатки(&Дата, Счет В ИЕРАРХИИ (&СчетаАмортизации), &ВидыСубконтоОС, Субконто1 В (&МассивОС)) КАК ТиповойОстатки
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
