...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт 
	...
	Запрос.УстановитьПараметр("СодержаниеСписаныТМЗ", НСтр("ru='Возврат ТМЗ'"));
	...
	Запрос.УстановитьПараметр("СодержаниеЗачетАванса",     НСтр("ru = 'Зачет неоплаченной части'"));
...
Процедура ДобавитьКолонкуСодержание(ТаблицаЗначений) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Если СтрокаТаблицы.ЭтоУслуга Тогда 
			СтрокаТаблицы.Содержание = НСтр("ru = 'Возврат услуг'");
		ИначеЕсли СтрокаТаблицы.ЭтоОС Тогда 
			СтрокаТаблицы.Содержание = НСтр("ru = 'Возврат основных средств'");
		Иначе 
			СтрокаТаблицы.Содержание = НСтр("ru = 'Разница в стоимости товаров'");
		КонецЕсли;
		...
		Для Каждого ТекУчет Из СтруктураУчет Цикл
			...
			Для Каждого СтрокаПартии Из СтрокиСписанныхПартий Цикл
				...
				Если СтрокаПартии.Себестоимость <> СтрокаПартии["СуммаСписания" + СуффиксКолонкиСуммаСписания] Тогда
					...
					СтрокаРазницы.Содержание = НСтр("ru = 'Разница в стоимости товаров'");
				КонецЕсли;
		...
		Для Каждого СтрокаДокумента Из ТаблицаДокумента Цикл
			...
			Для Каждого СтрокаАванса Из ТаблицаЗачетАвансов Цикл
				...
				Если Суффикс = "НДС"  Тогда 
					...
					СтрокаРезультат.Содержание  = ?(НЕ Реквизиты.ОтложитьПринятиеНДСКЗачету, НСтр("ru = 'Возврат суммы НДС'") ,НСтр("ru = 'Отложенное принятие НДС к зачету'")) ;
					СтрокаРезультат.ЗнакПроводкиНУ = -1;
				...
				Иначе 
					Если СтрокаДокумента.ЭтоУслуга Тогда 
						СтрокаРезультат.Содержание = НСтр("ru = 'Возврат услуг'");
					ИначеЕсли СтрокаДокумента.ЭтоОС Тогда 
						СтрокаРезультат.Содержание = НСтр("ru = 'Возврат основных средств'");
					Иначе
						СтрокаРезультат.Содержание = НСтр("ru = 'Разница в стоимости товаров'");
					КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ВозвратПоставщику_Накладная") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьВозвратаПоставщику(МассивОбъектов, ОбъектыПечати),
			,
...
//
Функция ПечатьВозвратаПоставщику(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		ЗаголовокШапка.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(Шапка.Ссылка, "Возврат поставщику");
