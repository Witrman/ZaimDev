...
Процедура СформироватьНазначениеПлатежа()
	...
	ТекстСумма = "Сумма " + Формат(СуммаДляПлатежа, "ЧЦ=15;ЧДЦ=2;ЧРД=-;ЧН=0-00") + Символы.ПС;
...
Процедура ОбновитьПодвал(Форма)
	...
	СуммаИтого = "Сумма документа: " + ?(Объект.ВключатьКомиссиюБанка, Объект.СуммаДокумента + Объект.СуммаКомиссии, Объект.СуммаДокумента) + " " + Объект.ВалютаДокумента;
	...
	Если ПоказыватьВДокументахСчетаУчета Тогда
		...
		Если ЕстьРасшифровкаПлатежа Тогда
			...
			Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПокупателю Тогда
				Элементы.РасшифровкаПлатежаСчетУчетаРасчетовПоАвансам.Заголовок = "Счет возвратов (БУ)";								
			Иначе	
				Элементы.РасшифровкаПлатежаСчетУчетаРасчетовПоАвансам.Заголовок = "Счет авансов (БУ)";
			КонецЕсли;
