...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьЗаполнениеТабличнойЧастиПострочно(ЭтотОбъект.Комплектующие, "Комплектующие", Отказ, ПараметрыПроверки);
	...
	Для Каждого СтрокаТЧ Из ТаблицаСписания Цикл
		Если СтрокаТЧ.Количество < 0 Тогда
			...
			Если ИмяКорСубконто = Неопределено Тогда
				ТекстСообщения = НСтр("ru = 'В документе ""%1"" при распределении количества, из-за правил округления, в строке %2 образовалось отрицательное количество.'");
...
Процедура ПроверитьЗаполнениеТабличнойЧастиПострочно(ТабличнаяЧасть, ИмяТабличнойЧасти, Отказ, ПараметрыПроверки = Неопределено)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		Если  ЗначениеЗаполнено(СтрокаТабличнойЧасти.СчетУчетаБУ)
			...
			ТекстСообщения = ОбщегоНазначенияБККлиентСервер.ПолучитьТекстСообщения("Колонка", "Корректность", 
				НСтр("ru = 'Счет учета (БУ)'"), СтрокаТабличнойЧасти.НомерСтроки, ИмяТабличнойЧасти, ТекстСообщения);
