...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("СинонимТаблицаУчастникиСовместнойДеятельности", НСтр("ru = 'УчастникиСовместнойДеятельности'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактураВВалютеРеглУчета") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьСчетФактураВВалютеРеглУчета(МассивОбъектов, ОбъектыПечати),
			,
			"Документ.СчетФактураВыданный.ПФ_MXL_СчетФактура");
	КонецЕсли;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактураСНомерамиГТД") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, Истина),
			,
			"Документ.СчетФактураВыданный.ПФ_MXL_СчетФактура");
	КонецЕсли;
...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, ПоказыватьНомераГТД = Ложь) Экспорт
	...
	Если ПоказыватьНомераГТД Тогда
		ДанныеДляПечати = ПолучитьДанныеДляПечатиСчетФактурыСНомерамиГТД(МассивОбъектов);
	Иначе
		ДанныеДляПечати = ПолучитьДанныеДляПечатиСчетФактурыБезНомеровГТД(МассивОбъектов);
	КонецЕсли;	
		...
		Для Каждого ИмяОбласти Из МассивОбластейМакета Цикл
			...
			ИначеЕсли ИмяОбласти = "ШапкаТаблицыУчастникиСовместнойДеятельности" ИЛИ ИмяОбласти = "ИтогоУчастникиСовместнойДеятельности" Тогда
				...
				Если СтруктураДанныхПечати.УчастникСРП Тогда
					ОбластьМакета.Параметры.ЗаголовокСД = НСтр("ru ='СРП'");
				ИначеЕсли СтруктураДанныхПечати.УчастникСовместнойДеятельности Тогда
					ОбластьМакета.Параметры.ЗаголовокСД = НСтр("ru ='совместной деятельности'");					
				КонецЕсли;   				
...
Функция ПолучитьДанныеДляПечатиСчетФактурыБезНомеровГТД(МассивОбъектов, ВВалютеРеглУчета = Ложь)
	...
	Пока ВыборкаПоШапке.Следующий() Цикл 
		...
		ДанныеПечати.Вставить("Номер", РаботаСДиалогами.СформироватьЗаголовокДокумента(ВыборкаПоШапке.Ссылка, "Счет-фактура"));
		...
		Если ВыборкаПоШапке.ВидСчетаФактуры = Перечисления.ВидыСчетовФактур.Дополнительный 
			...
			Если ВыборкаПоШапке.ВидСчетаФактуры = Перечисления.ВидыСчетовФактур.Дополнительный Тогда
				ТекстДополнительныйИлиИсправленныйСчетФактура = НСтр("ru = 'Дополнительный счет-фактура %1'");
			ИначеЕсли ВыборкаПоШапке.ВидСчетаФактуры = Перечисления.ВидыСчетовФактур.Исправленный Тогда
				ТекстДополнительныйИлиИсправленныйСчетФактура = НСтр("ru = 'Исправленный счет-фактура %1'");
			Иначе
			...
			Если ЗначениеЗаполнено(ВыборкаПоШапке.ОсновнойСчетФактура) Тогда 
				...
				Формат(ВыборкаПоШапке.ОсновнойСчетФактураДата, "ДФ=dd.MM.yyyy"));	
			Иначе	
				ПредставлениеОсновногоСчетаФактуры = "";		
			КонецЕсли;
		...
		Если ВыборкаПоШапке.ВыводитьДатуОборотаПоРеализации Тогда 
			ДанныеПечати.Вставить("ДатаОборотаПоРеализации", Формат(ВыборкаПоШапке.ДатаОборотаПоРеализации, "ДФ=dd.MM.yyyy"));
		КонецЕсли;
		...
		ПредставлениеРНН_БИН = НСтр("ru='БИН / ИИН'; kz='БСН / ЖСН'", "ru");
		...
		Если Не ВыборкаПоШапке.Дата  = Неопределено Тогда 
			...
			Если ВыводитьРНН Тогда 
				СуффиксРНН_БИН = "РНН";
				ПредставлениеРНН_БИН = НСтр("ru='РНН'; kz='СТН'", "ru");
			КонецЕсли;
		КонецЕсли;
		...
		Если ВыборкаПоШапке.ДатаСвидетельстваПоНДС <> Дата(1,1,1) Тогда 
			ТекстСвидетельствоПоНДС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Свидетельство о постановке на регистрационный учет по НДС, серия %1, № %2, от %3'"),
			ВыборкаПоШапке.СерияСвидетельстваПоНДС, 
			ВыборкаПоШапке.НомерСвидетельстваПоНДС, 
			Формат(ВыборкаПоШапке.ДатаСвидетельстваПоНДС, "ДФ=dd.MM.yyyy"));
		...
		ПредставлениеГрузоотправителя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1, %2, %3'"),
		...
		ПредставлениеГрузополучателя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1, %2, %3'"),
		...
		Если НЕ ДанныеПечати.ДоговорСовместнойДеятельности Тогда 
			ДанныеПечати.Вставить("Получатель", НСтр("ru='Получатель'"));
		...
		Иначе 
			...
			Для Каждого СтрокаТаблицы Из ТаблицаПокупателей Цикл
				...
				Если СтрокаТаблицы.Покупатель = ВыборкаПоШапке.ПокупательДляПечати Тогда
					НоваяСтрока.Получатель = НСтр("ru='Получатель'");
				...
				Иначе 
					Если РеквизитыДоговора.ДоговорСовместнойДеятельности Тогда
						НоваяСтрока.Получатель = НСтр("ru='Участник совместной деятельности'");
					ИначеЕсли 
						РеквизитыДоговора.УчастникСРП Тогда
						НоваяСтрока.Получатель = НСтр("ru='Участник СРП'");
					КонецЕсли;
				КонецЕсли;
		...
		Иначе 
			...
			Для Каждого СтрокаПечати ИЗ ОбщаяТаблицаДляПечати Цикл
				...
				Если ЗначениеЗаполнено(СтрокаПечати.СтавкаНДС) Тогда
					Строчка.СтавкаНДС = ?(СтрокаПечати.СтавкаНДСДляОсвобожденногоОборота ИЛИ СтрокаПечати.СтавкаНДСМестоРеализацииНеРК, "Без НДС", "" + СтрокаПечати.СтавкаНДССтавка + "%");
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(Руководители.УполномоченныйПодписыватьСчетаФактурыЗаГлавногоБухгалтера) Тогда
			ДолжностьГлБухгалтера = ?(НЕ ЗначениеЗаполнено(Руководители.УполномоченныйПодписыватьСчетаФактурыЗаГлавногоБухгалтераДолжность), НСтр("ru='<Должность не указана>: '"), Руководители.УполномоченныйПодписыватьСчетаФактурыЗаГлавногоБухгалтераДолжность + НСтр("ru=': '"));
		...
		Иначе
			ДолжностьГлБухгалтера = НСтр("ru='Главный бухгалтер: '");	
		...
		Если ЗначениеЗаполнено(Руководители.УполномоченныйПодписыватьСчетаФактурыЗаРуководителя) Тогда
			ДолжностьРуководителя = ?(НЕ ЗначениеЗаполнено(Руководители.УполномоченныйПодписыватьСчетаФактурыЗаРуководителяДолжность), " ", Руководители.УполномоченныйПодписыватьСчетаФактурыЗаРуководителяДолжность + НСтр("ru=': '"));
		...
		Иначе
			ДолжностьРуководителя = НСтр("ru='Руководитель: '");
...
Функция ПолучитьДанныеДляПечатиСчетФактурыСНомерамиГТД(МассивОбъектов, ВВалютеРеглУчета = Ложь)
