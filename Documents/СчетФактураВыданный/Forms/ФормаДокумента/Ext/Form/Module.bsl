...
Процедура ПриОткрытии(Отказ)
	...
	// При создании исправленного счета-фактуры из документов реализации,
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Сторнирование"
		И ТипЗнч(Параметр.ДокументОснование) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		ОбновитьПредставлениеСостоянияСФ();
		ЭтаФорма.Прочитать();
	...
	Если ЭСФКлиентСервер.ЭтоОповещениеПодсистемыЭСФ(ИмяСобытия) Тогда
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "РегистрСведений.ПодтверждениеРеализацииВЕАЭС.Форма.ФормаРучногоВвода" Тогда
		
		ЗаполнитьЗначенияСвойств(ПараметрыРеализацииВЕАЭС, ВыбранноеЗначение);
		ОбновитьПредставлениеСостоянияРеализацииВЕАЭС(ЭтаФорма);
		
	ИначеЕсли ИсточникВыбора.ИмяФормы = "ОбщаяФорма.РеквизитыПечатиРеализации" Тогда
		
		Если ВыбранноеЗначение <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(Объект, ВыбранноеЗначение);
			ЗаполнитьПунктНазначенияНаСервере();
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.СчетФактураВыданный.Форма.ФормаРеквизиты" Тогда		
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = ПредопределенноеЗначение("РежимЗаписиДокумента.Проведение") Тогда
		КлючеваяОперация = "Документ ""счет-фактура (выданный)"" (проведение)";
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрыРеализацииВЕАЭС = ПолучитьПараметрыРеализацииВЕАЭС(ТекущийОбъект.Ссылка);
	Если Объект.СтранаНазначения <> Справочники.КлассификаторСтранМира.Казахстан Тогда
		СтранаЯвляетсяУчастникомЕАЭС = ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СтранаНазначения, "УчастникТаможенногоСоюза"));
	Иначе
		СтранаЯвляетсяУчастникомЕАЭС = Ложь;
	КонецЕсли;
	ОбновитьПредставлениеСостоянияРеализацииВЕАЭС(ЭтаФорма);
...
Процедура ДатаПриИзменении(Элемент)
	...
	// Заполнить дату совершения оборота по реализации.
...
Процедура ПокупательПриИзменении(Элемент)
	Объект.СтранаНазначения = ПолучитьСтрануРезидентстваПокупателя(Объект.Покупатель);
КонецПроцедуры
...
&НаКлиенте
Процедура СпособВыставленияПриИзменении(Элемент)
	СпособВыставленияПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура СпособВыставленияПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ПредставлениеСостоянияРеализацииВЕАЭСНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если ТипЗнч(ПараметрыРеализацииВЕАЭС) <> Тип("Структура") Тогда
		ПараметрыРеализацииВЕАЭС = ПолучитьПараметрыРеализацииВЕАЭС(Объект.Ссылка);
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Документ",            Объект.Ссылка);
	ПараметрыФормы.Вставить("Состояние",           ?(ЗначениеЗаполнено(ПараметрыРеализацииВЕАЭС.Состояние), ПараметрыРеализацииВЕАЭС.Состояние, ПредопределенноеЗначение("Перечисление.СостоянияРеализацийВЕАЭС.ОжидаетсяПодтверждение")));
	ПараметрыФормы.Вставить("ДатаПодтверждения",   ПараметрыРеализацииВЕАЭС.ДатаПодтверждения);
	ПараметрыФормы.Вставить("НомерПодтверждения",  ПараметрыРеализацииВЕАЭС.НомерПодтверждения);
	ПараметрыФормы.Вставить("Комментарий",         ПараметрыРеализацииВЕАЭС.Комментарий);
	ПараметрыФормы.Вставить("ТекущийПериод",       ТекущаяДата());
	ПараметрыФормы.Вставить("ЗаписыватьРезультат", Истина);
	
	ОткрытьФорму("РегистрСведений.ПодтверждениеРеализацииВЕАЭС.Форма.ФормаРучногоВвода", ПараметрыФормы, ЭтаФорма);
...
Процедура ОсновнойСчетФактураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ВидСчетаФактуры = ПредопределенноеЗначение("Перечисление.ВидыСчетовФактур.Дополнительный") Тогда
		...
		Результат = ПолучитьФорму("Документ.СчетФактураВыданный.ФормаВыбора",	ПараметрыФормыВыбора, Элемент);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|СтавкаНДС, СуммаНДС, СтавкаАкциза, СуммаАкциза, ОборотПоРеализации, КоэффициентАкциза");
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ РЕКВИЗИТОВ ТЧ НМА
...
Процедура ИзменитьНМА(Команда)
	...
	ИзменитьТабличнуюЧасть("НМА", "НМА");
...
Функция ПараметрыФормыРедактированияНомеровГТД()
	...
	ПараметрыФормы.АдресТовары = РаботаСДиалогами.АдресТаблицыЗначений(ЭтаФорма, "Товары");
	...
	Если ПараметрыФормы.ТолькоПросмотр ИЛИ Объект.ПодтвержденДокументамиОтгрузки Тогда
		ПараметрыФормы.ТолькоПросмотр = Истина;
	Иначе
...
Процедура УправлениеФормой(Форма)
	...
	СтруктураЭлементов = Новый Структура("ЦеныИВалюта, ФормаПерезаполнитьПоДокументамОснованиям, ФормаДобавитьИзДокументовОснований,
										 |ТоварыПодборТовары, ТоварыИзменитьТовары, УслугиПодборУслуги, ОСПодборОС, НМАПодборНМА");
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ВедетсяУчетПоТоварамОрганизацийБУ = НомераГТДСервер.ВедетсяУчетПоТоварамОрганизаций(Объект.ДатаСовершенияОборотаПоРеализации);
...
Процедура ОбновитьИтоги(Форма)
	...
	Форма.ИтогиВсего      = УчетНДСиАкцизаКлиентСервер.ПолучитьСуммуДокументаСНДС(Объект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(Объект, ЗаполнятьКоэффициентАкциза = Истина, СписокТабличныхЧастей = "Товары, Услуги, ОС, НМА")
...
Процедура ОбновитьПредставлениеСостоянияСФ()
	...
	Иначе
		...
		Элементы.ПредставлениеСостояниеДокумента.Заголовок = СокрЛП(СостояниеДокумента) + " документом: " + СокрЛП(РегистраторСостоянияДокумента);
	КонецЕсли;	
...
// Состояние реализации в страны ЕАЭС

&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьПредставлениеСостоянияРеализацииВЕАЭС(Форма)
	
	ПараметрыРеализацииВЕАЭС 	 = Форма.ПараметрыРеализацииВЕАЭС;
	СтранаНазначения 			 = Форма.Объект.СтранаНазначения;
	СтранаЯвляетсяУчастникомЕАЭС = Форма.СтранаЯвляетсяУчастникомЕАЭС;
	Ссылка = Форма.Объект.Ссылка;
	
	ЦветТекста = Новый Цвет();
	
	Если (НЕ ЗначениеЗаполнено(ПараметрыРеализацииВЕАЭС.Состояние) И 
			ЗначениеЗаполнено(СтранаНазначения) И НЕ СтранаЯвляетсяУчастникомЕАЭС)
		ИЛИ НЕ ЗначениеЗаполнено(Ссылка) 
		ИЛИ Форма.Объект.Товары.Количество() = 0  И Форма.Объект.ОС.Количество() = 0
		ИЛИ (НЕ ЗначениеЗаполнено(ПараметрыРеализацииВЕАЭС.Состояние) 
				И Форма.Объект.ДатаСовершенияОборотаПоРеализации < Дата('20150101')) Тогда
		
		ТекстЗаголовкаРеализацииВЕАЭС = "";
	ИначеЕсли ЗначениеЗаполнено(ПараметрыРеализацииВЕАЭС.Состояние) И 
		ЗначениеЗаполнено(СтранаНазначения) И НЕ СтранаЯвляетсяУчастникомЕАЭС Тогда
		
		ТекстЗаголовкаРеализацииВЕАЭС = НСтр("ru = 'Для страны назначения %1 не требуется указание даты и номера отметки Заявления о ввозе в налоговом органе'");
		ТекстЗаголовкаРеализацииВЕАЭС = СтрЗаменить(ТекстЗаголовкаРеализацииВЕАЭС, "%1", СтранаНазначения);
		ЦветТекста = WebЦвета.Красный;
	ИначеЕсли (ПараметрыРеализацииВЕАЭС.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияРеализацийВЕАЭС.ОжидаетсяПодтверждение") 
		ИЛИ НЕ ЗначениеЗаполнено(ПараметрыРеализацииВЕАЭС.Состояние)) И Форма.Объект.Проведен Тогда
		ТекстЗаголовкаРеализацииВЕАЭС = НСтр("ru = 'Ожидаются дата и номер отметки Заявления о ввозе в налоговом органе'");
		ЦветТекста = WebЦвета.Красный;
	ИначеЕсли ПараметрыРеализацииВЕАЭС.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияРеализацийВЕАЭС.НеПодтвержденаРеализация") Тогда
		ТекстЗаголовкаРеализацииВЕАЭС = НСтр("ru = 'Покупатель не оформил Заявление о ввозе в установленный срок'");
		ЦветТекста = WebЦвета.Красный;
	ИначеЕсли ПараметрыРеализацииВЕАЭС.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияРеализацийВЕАЭС.ПодтвержденаРеализация") Тогда
		ТекстЗаголовкаРеализацииВЕАЭС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Заявление о ввозе зарегистрировано в налоговом органе под №%1 от %2'"),
			ПараметрыРеализацииВЕАЭС.НомерПодтверждения,
			Формат(ПараметрыРеализацииВЕАЭС.ДатаПодтверждения, "ДФ=dd.MM.yyyy"));
	КонецЕсли;
	
	Форма.ПредставлениеСостоянияРеализацииВЕАЭС = ТекстЗаголовкаРеализацииВЕАЭС;
	Форма.Элементы.ПредставлениеСостоянияРеализацииВЕАЭС.ЦветТекста = ЦветТекста;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьПараметрыРеализацииВЕАЭС(ДокументРеализации)
	
	Результат = Новый Структура("Состояние, ДатаПодтверждения, НомерПодтверждения, Комментарий", 
								Перечисления.СостоянияРеализацийВЕАЭС.ПустаяСсылка(), '00010101', "", "");
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПодтверждениеРеализацииВЕАЭС.Состояние,
...
Процедура СформироватьНадписьЦеныИВалюта(Форма)
	...
	Если Объект.СуммаВключаетНДС Тогда
		Форма.Элементы.ИтогоВсегоНДС.Заголовок = "НДС (в т. ч.)";
	Иначе
		Форма.Элементы.ИтогоВсегоНДС.Заголовок = "НДС (сверху)";
	КонецЕсли;
	
	Если Объект.СуммаВключаетАкциз Тогда
		Форма.Элементы.ИтогоВсегоАкциз.Заголовок = "Акциз (в т. ч.)";		
	Иначе
		Форма.Элементы.ИтогоВсегоАкциз.Заголовок = "Акциз (сверху)";		
	КонецЕсли;
...
Процедура ДоговорКонтрагентаПриИзмененииНаСервере(СтруктураРезультатаВыполнения)
	...
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда 
		...
		Если НЕ ЗначениеЗаполнено(Объект.УсловияОплаты) И ЗначениеЗаполнено(Объект.ДоговорКонтрагента.УсловияОплаты) Тогда
			Объект.УсловияОплаты = Объект.ДоговорКонтрагента.УсловияОплаты;
		КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, ДанныеОбъекта)
	...
	Если ДанныеОбъекта.УчитыватьАкциз Тогда
...
Процедура ДокументыОснованияДокументОснованиеПриИзмененииНаСервере(СтруктураРезультатаВыполнения)
	...
	Документы.СчетФактураВыданный.ОпределениеПараметровСчетаФактуры(Объект);
	...
	Если Объект.ДокументыОснования.Количество() = 1 И ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументОснование) Тогда 
		...
		КонецЕсли;
		Документы.СчетФактураВыданный.ЗаполнитьРеквизитыДокумента(Объект);
	КонецЕсли;
...
Процедура ОбработкаОповещенияОбработкиТабличнойЧастиТоварыНаСервере(Параметры)
	...
	ЗаполнитьДобавленныеКолонкиТаблиц(Объект, Объект.УчитыватьАкциз И Объект.Товары.Количество() > 0, "Товары");
	ОбновитьИтоги(ЭтаФорма);
...
&НаСервереБезКонтекста
Функция ПолучитьСтрануРезидентстваПокупателя(Покупатель)
	
	СтранаРезидентства = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Покупатель, "СтранаРезидентства");
	Возврат ?(ЗначениеЗаполнено(СтранаРезидентства), СтранаРезидентства, Справочники.КлассификаторСтранМира.Казахстан);
...
Процедура ПерезаполнитьПоДокументамОснованиямНаСервере()
	...
	Документы.СчетФактураВыданный.СвернутьТабличныеЧасти(ДокументОбъект);
	ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
	
	ЗаполнитьДобавленныеКолонкиТаблиц(Объект, Объект.Товары.Количество() > 0);
...
Процедура ОпределитьОграничениеТипаДокументовОснования(Элемент, ДокументОснование = Неопределено)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")
	...
	Если ЭтоВозврат Тогда 
		Элемент.ОграничениеТипа = Новый ОписаниеТипов("ДокументСсылка.ВозвратТоваровОтПокупателя");
	Иначе 
		МассивДоступныхТипов = Элемент.ДоступныеТипы.Типы();
		МассивДоступныхТипов.Удалить(МассивДоступныхТипов.Найти(Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")));
...
Процедура ЗаполнитьОсновнойСчетФактуруНаСервере()
	Объект.ОсновнойСчетФактура = Документы.СчетФактураВыданный.НайтиОсновнойСчетФактуру(Объект);
КонецПроцедуры
...
Процедура РеквизитыПечати(Команда)
	...
	СтруктураПараметров.Вставить("УсловияОплаты",            		Объект.УсловияОплаты);
	СтруктураПараметров.Вставить("СпособОтправления",     		    Объект.СпособОтправления);
	СтруктураПараметров.Вставить("ПунктНазначения",                 Объект.ПунктНазначения);
	СтруктураПараметров.Вставить("ТипОбъекта",                      "СчетФактураВыданный");
	
	СтруктураПараметров.Вставить("УсловияПоставки",                   Объект.УсловияПоставки);
	СтруктураПараметров.Вставить("СпособОтправленияПоКлассификатору", Объект.СпособОтправленияПоКлассификатору);
...
Процедура ОбработкаОповещенияОбработкиТабличнойЧастиНаСервере(Параметры)
	...
	ЗаполнитьДобавленныеКолонкиТаблиц(Объект, Ложь, ИмяТабличнойЧасти);
...
&НаКлиенте
Процедура ДатаСовершенияОборотаПоРеализацииПриИзменении(Элемент)
	
	ДатаСовершенияОборотаПоРеализацииПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ДатаСовершенияОборотаПоРеализацииПриИзмененииНаСервере()
	ВедетсяУчетПоТоварамОрганизацийБУ = НомераГТДСервер.ВедетсяУчетПоТоварамОрганизаций(Объект.ДатаСовершенияОборотаПоРеализации);
...
&НаСервере
Процедура ЗаполнитьПунктНазначенияНаСервере()
	
	Документы.СчетФактураВыданный.ЗаполнитьПунктНазначения(Объект);
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьРеквизитыГосУчреждения(Команда)
	...
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("ГосучреждениеКодТоваров",  		Объект.ГосучреждениеКодТоваров);
	СтруктураПараметров.Вставить("ГосучреждениеНазначениеПлатежа",  Объект.ГосучреждениеНазначениеПлатежа);
	
	ОткрытьФорму("Документ.СчетФактураВыданный.Форма.ФормаРеквизиты", СтруктураПараметров, ЭтаФорма);
