...
Функция СформироватьСписокВыбораОснований()
	...
	Основания.Добавить(Строка(Объект.ВидОперации));
	...
	Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента) Тогда
			...
			ТекстВыбора = "по договору №" + ?(НЕ ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента.НомерДоговора), "_______", СтрокаПлатеж.ДоговорКонтрагента.НомерДоговора)
			+ " от " + ?(ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента.ДатаДоговора), Формат(СтрокаПлатеж.ДоговорКонтрагента.ДатаДоговора, "ДФ=dd.MM.yyyy"), "_______");		
			
			Основания.Добавить(ТекстВыбора);
			Основания.Добавить(Строка(Объект.ВидОперации) + " " + ТекстВыбора);
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Попытка
			ПредставлениеДок = РаботаСДиалогами.СформироватьЗаголовокДокумента(Объект.ДокументОснование);
		Исключение
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.НапечататьЧек.Доступность = НЕ Форма.ТолькоПросмотр;
	Элементы.НапечататьЧек.Видимость = (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ВозвратДенежныхСредствПокупателю"));
	...
	РаботаСДиалогами.УстановитьЗаголовокЭлементуУправленияУчитыватьКПН(Объект.Организация, Элементы.УчитыватьКПН);
	 ...
	 Если Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ВзносНаличнымиВБанк
		...
		Если ПоддержкаРаботыСоСтруктурнымиПодразделениями И ЗначениеЗаполнено(СтруктурноеПодразделениеОрганизацияПолучатель) И СтруктурноеПодразделениеОрганизацияПолучатель <> Объект.Организация Тогда
			СтруктурноеПодразделениеОрганизацияПолучатель = Объект.Организация;
			Объект.СтруктурноеПодразделениеПолучатель	  = Справочники.ПодразделенияОрганизаций.ПустаяСсылка();
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Значение в поле ""Структурная единица (получатель)"" не соответствует выбранной организации и было измнено.'"));
		КонецЕсли;
	КонецЕсли;
