...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаПоИсполнительнымЛистам Тогда
		...
		Если ВалютаДокумента <> ВалютаРегламентированногоУчета Тогда
			ТекстСообщения = НСтр("ru = 'Выплата по исполнительным листам может осуществляться только в валюте ""'") + ВалютаРегламентированногоУчета + """!";
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям ИЛИ
		...
		Если ВалютаДокумента <> ВалютаРегламентированногоУчета Тогда
			ТекстСообщения = НСтр("ru = 'Выплата заработной платы может осуществляться только в валюте ""'") + ВалютаРегламентированногоУчета + """!";
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ВалютаДокумента", "Объект", Отказ);
		КонецЕсли;
...
Процедура ПроверитьСоответствиеВедомостей(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.КонфликтныйДокумент) Тогда
			ТекстСообщения = НачалоСообщения + ?(ПоОдномуРаботнику, НСтр("ru = 'по выбранному сотруднику '"), "") + НСтр("ru = 'ведомость уже включена в '") + Строка(Выборка.КонфликтныйДокумент);
...
Процедура ПроверитьДостаточностьОстатковДС(Реквизиты, ТаблицаКонтрольОстатков, Отказ) экспорт
	...
	Если Реквизиты.СуммаДокумента > СуммаОстатка Тогда		
		ТекстОшибки = НСтр("ru='На счете %1 " + ?(ЗначениеЗаполнено(Реквизиты.СтруктурноеПодразделение), "структурное подразделение " 
				+ "%5", "") + "недостаточно средств для проведения операции. Сумма операции %2 %4, остаток счета %3  %4.'");
