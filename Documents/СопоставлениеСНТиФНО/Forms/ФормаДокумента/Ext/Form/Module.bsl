...
&НаКлиенте
Процедура ТоварыСНТСкладНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтруктурноеПодразделениеОрганизация) Тогда
		...
		ОтборОрганизация.Добавить(СтруктурноеПодразделениеОрганизация);
	Иначе
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановкаПредставленияДокумента();
...
&НаСервере
Процедура УстановкаПредставленияДокумента()
	
	Если Параметры.Ключ.Пустая() Тогда
		Представление = "";
	Иначе
		Представление = Объект.Номер + " от " + Формат(Объект.Дата, "ДФ = dd.MM.yyyy HH:mm:ss") + "";
	КонецЕсли;
	
	ВСОбщегоНазначения.УстановитьЗаголовокФормыДокумента(Представление, Объект.Ссылка, ЭтаФорма);
...
Процедура ТоварыСНТПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ПараметрыПеретаскивания.Значение[0].Свойство("РегистрационныйНомерФНО", РегистрационныйНомерФНО)
		 ...
		 И ПараметрыПеретаскивания.Значение[0].Свойство("ТоварНаименование", ТоварНаименование)
		 И ПараметрыПеретаскивания.Значение[0].Свойство("КодТНВЭД", КодТНВЭД) Тогда
		
		ТекущаяСтрокаТоварыСНТ = Объект.ТоварыСНТ.НайтиПоИдентификатору(Строка);
		
		Если ТекущаяСтрокаТоварыСНТ.КодТНВЭД <> КодТНВЭД Тогда
			ТекущаяСтрокаТоварыСНТ.КодТНВЭД = КодТНВЭД;
		КонецЕсли;
...
Процедура ЗаполнитьРеквизитыТоваровСНТПоРеквизитамТоваровФНО(МассивСтрокФНО,МассивСтрокСНТ)
	...
	СоответсвиеДубликатов = Новый Соответствие();
	
	Для Каждого СтрокаФНО Из МассивСтрокФНО Цикл
		
		КлючПоиска = Строка(СтрокаФНО.Товар.УникальныйИдентификатор()) +
								Строка(СтрокаФНО.КодТНВЭД)+
								Строка(СтрокаФНО.ЕдиницаИзмерения.УникальныйИдентификатор())+
								Строка(СтрокаФНО.ЕдиницаИзмеренияКод);
					
		ИндексыДубликата = СоответсвиеДубликатов.Получить(КлючПоиска);
		
		Если ИндексыДубликата <> Неопределено Тогда
			СообщениеОбОшибке =
				НСтр("ru='В строке %1 табличной части ""Товары из ФНО 328.00"" ключевые поля дублируются со строкой %2.
				|Данные строки %1 не обработаны'");
			
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				СообщениеОбОшибке,
				СтрокаФНО.НомерСтроки,
				ИндексыДубликата.НомерСтрокиФНО);
			
			ЭСФКлиентСерверПереопределяемый.СообщитьПользователю(СообщениеОбОшибке);
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтрокаСНТ Из МассивСтрокСНТ Цикл
			...
			Если СтрокаФНО.Товар = СтрокаСНТ.Товар
				...
				КлючПоиска = Строка(СтрокаСНТ.Товар.УникальныйИдентификатор()) +
								Строка(СтрокаСНТ.КодТНВЭД)+
								Строка(СтрокаСНТ.ЕдиницаИзмерения.УникальныйИдентификатор())+
								Строка(СтрокаСНТ.ЕдиницаИзмеренияКод);
					
				ИндексыДубликата = СоответсвиеДубликатов.Получить(КлючПоиска);
				Если ИндексыДубликата = Неопределено Тогда
					
					СоответсвиеДубликатов.Вставить(КлючПоиска,
						Новый Структура("НомерСтрокиФНО,НомерСтрокиСНТ",
							СтрокаФНО.НомерСтроки,
							СтрокаСНТ.НомерСтроки));
				...
				Иначе
					СообщениеОбОшибке =
						НСтр("ru='В строке %1 табличной части ""Товары из СНТ"" ключевые поля дублируются со строкой %2.
						|Данные строки %1 не заполнены'");
					
					СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						СообщениеОбОшибке,
						СтрокаСНТ.НомерСтроки,
						ИндексыДубликата.НомерСтрокиСНТ);
