...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("СодержаниеВзаимозачет", НСтр("ru = 'Взаимозачет'"));
	Запрос.УстановитьПараметр("СодержаниеПереносЗадолженности", НСтр("ru = 'Перенос задолженности'"));
	Запрос.УстановитьПараметр("СодержаниеСписаниеЗадолженности", НСтр("ru = 'Списание задолженности'"));
	Запрос.УстановитьПараметр("СодержаниеСписаниеДтЗадолженностиНУ", НСтр("ru = 'Списание дебиторской задолженности'"));
	Запрос.УстановитьПараметр("СодержаниеСписаниеКтЗадолженностиНУ", НСтр("ru = 'Списание кредиторской задолженности'"));
...
Функция ПечатьАктаВзаимозачета(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл 
		...
		СтрокаШапки = ТекстШапки + " № "+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаПоДокументам.Номер, ВыборкаПоДокументам.Ссылка) + Символы.ПС + " от " + Формат(ВыборкаПоДокументам.Дата, "ДЛФ=DD"); 
		...
		СтрокаКредиторки  = СтрокаКредиторки + Формат(ВыборкаПоДокументам[КолонкаСуммы + "Кредиторки"], "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты + " по следующим договорам:";
		...
		Пока ВыборкаПоДаннымДокумента.Следующий() Цикл
			...
			ОбластьМакета.Параметры.Сумма   = Формат(ВыборкаПоДаннымДокумента[КолонкаСуммы + "Кредиторки"], "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты;
		...
		+ Формат(ВыборкаПоДокументам[КолонкаСуммы + "Дебиторки"], "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты + " по следующим договорам:";
		...
		Пока ВыборкаПоДаннымДокумента.Следующий() Цикл
			...
			ОбластьМакета.Параметры.Договор = СокрЛП(ВыборкаПоДаннымДокумента.ДоговорКонтрагента);
			ОбластьМакета.Параметры.Счет    = ВыборкаПоДаннымДокумента.СчетУчетаРасчетов;
			ОбластьМакета.Параметры.Сумма   = Формат(ВыборкаПоДаннымДокумента[КолонкаСуммы+"Дебиторки"], "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты;
			ТабДокумент.Вывести(ОбластьМакета);
		КонецЦикла;
