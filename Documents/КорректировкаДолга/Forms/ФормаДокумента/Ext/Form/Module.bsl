...
Процедура ВывестиНадписи(Форма) 
	...
	Форма.НадписьДебиторскаяЗадолженность  = "Дебиторская задолженность: " + Формат(СуммаДеб, "ЧЦ=15; ЧДЦ=2; ЧН=-") + " " + Строка(Объект.ВалютаДокумента);
	Форма.НадписьКредиторскаяЗадолженность = "Кредиторская задолженность: " + Формат(СуммаКред, "ЧЦ=15; ЧДЦ=2; ЧН=-") + " " + Строка(Объект.ВалютаДокумента);
	...
	Если Разница = 0 И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкаДолга.ПроведениеВзаимозачета") Тогда
		Форма.НадписьВзаимозачет = "ВЗАИМОЗАЧЕТ";
	Иначе
		Форма.НадписьВзаимозачет = "Разница: " + Формат(Разница, "ЧЦ=15;ЧДЦ=2") + " " + Строка(Объект.ВалютаДокумента);
	КонецЕсли;
