...
Процедура ВыполнитьПроведениеДокументовСНТнаВозврат(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СНТ.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияСНТ.Исходящий)
		|	И НЕ СНТ.ПредоставленВБумажномВиде
		|	И СНТ.Проведен
		|	И СНТ.ТипСНТ = ЗНАЧЕНИЕ(Перечисление.ТипыСНТ.ВозвратТоваров)
		|	И (СНТ.ПоставщикРозничныйРеализатор
		|			ИЛИ СНТ.ПоставщикРозничнаяРеализация
		|			ИЛИ СНТ.ПоставщикФизическоеЛицо
		|				И НЕ СНТ.ПоставщикНерезидент)
		|	И СНТ.Дата >= &Дата
		|	И НЕ СНТ.Ссылка В (&МассивОбработанныхСНТ)
		|
		|УПОРЯДОЧИТЬ ПО
		|	СНТ.Дата
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	вт_ОтобранныеДокументы.Ссылка КАК Регистратор
		|ИЗ
		|	вт_ОтобранныеДокументы КАК вт_ОтобранныеДокументы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаВиртуальныхСкладах.Обороты(&Дата, , Регистратор, ) КАК ТоварыНаВиртуальныхСкладахОбороты
		|		ПО вт_ОтобранныеДокументы.Ссылка = ТоварыНаВиртуальныхСкладахОбороты.Регистратор
		|ГДЕ
		|	ТоварыНаВиртуальныхСкладахОбороты.КоличествоОборот ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		ПО вт_ОтобранныеДокументы.Ссылка = ТоварыНаВиртуальныхСкладахОбороты.Регистратор
		|ГДЕ
		|	НЕ ТоварыНаВиртуальныхСкладахОбороты.КоличествоОборот ЕСТЬ NULL
		|	И вт_ОтобранныеДокументы.ПоставщикРозничныйРеализатор";
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивОбработанныхСНТ.Добавить(Выборка.Регистратор);
		СНТ = Выборка.Регистратор.ПолучитьОбъект();
		...
		Исключение
			...
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, Выборка.Регистратор, ОписаниеОшибки());
