...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "ГруппаРазделыГТД" Тогда
		...
		Элементы.РазделыТаможеннаяСтоимость.Заголовок = СформироватьЗаголовокПоВалюте("Таможенная стоимость", СтрокаВалютыДокумента);
		Элементы.РазделыТаможенныйСбор.Заголовок = СформироватьЗаголовокПоВалюте("Таможенный сбор", СтрокаВалютыДокумента);
		Элементы.РазделыТаможенныйСборВал.Заголовок = СформироватьЗаголовокПоВалюте("Таможенный сбор", СтрокаВалютыВзаиморасчетов);
...
Процедура РазделыПриАктивизацииСтроки(Элемент)
	...
	Элементы.ТоварыСуммаПошлины.Заголовок = СформироватьЗаголовокПоВалюте("Пошлина", ?(Не ЗначениеЗаполнено(ВалютаПошлины), СтрокаВалютыДокумента, ВалютаПошлины));
	Элементы.ТоварыСуммаНДС.Заголовок = СформироватьЗаголовокПоВалюте("НДС", ?(Не ЗначениеЗаполнено(ВалютаНДС), СтрокаВалютыДокумента, ВалютаНДС));
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.ВедетсяУчетПоТоварамОрганизацийБУ Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНомерГТД", "Заголовок", "Источник происхождения");
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНомерГТД", "Заголовок", "Номер ГТД");		
	КонецЕсли;
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.ЗначениеКопирования = Неопределено 
			...
			Если ПустаяСтрока(Объект.ВидВходящегоДокумента) Тогда 
				Объект.ВидВходящегоДокумента = "Грузовая таможенная декларация";
			КонецЕсли;	
...
Функция СформироватьЗаголовокПоВалюте(Знач ТекстЗаголовка, Знач Валюта)
	...
	Если НЕ ЗначениеЗаполнено(Валюта) Тогда
		ТекстВозврата  = ТекстЗаголовка + " (<>)";
	Иначе
		ТекстВозврата  = ТекстЗаголовка + " (" + Валюта + ")";
	КонецЕсли;
	
	Возврат НСтр("ru='" + ТекстВозврата + "'");
...
Процедура УстановитьЗначенияКолонокВПодвалеТабличнойЧасти(ЭлементТабличнаяЧасть, ИмяТабличнойЧасти, НомерРаздела)
	...
	Если ИмяТабличнойЧасти = "Товары" Тогда
		Элементы.ГруппаТМЗ.Заголовок = "ТМЗ" + ?(КоличествоСтрокРаздела > 0, " (" + КоличествоСтрокРаздела + ")", "");
	Иначе
		Элементы.ГруппаОС.Заголовок = "ОС" + ?(КоличествоСтрокРаздела > 0, " (" + КоличествоСтрокРаздела + ")", "");
	КонецЕсли;  	
