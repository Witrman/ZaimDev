...
Процедура ПроведениеСпискаДокументовВФоне(СтруктураПараметров, АдресХранилища, МассивРегОпераций = Неопределено) Экспорт
	...
	Для Каждого ДокументЗакрытия Из МассивРегОпераций Цикл
		...
		ИначеЕсли ТипЗнч(ДокументЗакрытия) = Тип("Структура") Тогда   // Выполняется закрытие месяца из Помощника
			...
			ВидОперации = ДокументЗакрытия.ВидОперации;
			МесяцНачисленияСтрокой = ДокументЗакрытия.МесяцНачисленияСтрокой;
			...
			ТекстСообщения = НСтр("ru='Начато выполнение операции: " + ВидОперации + " за " + МесяцНачисленияСтрокой + "'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			
			ОбъектЗакрытиеМесяца.ВидРегламентнойОперации = ВидОперации;
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось записать документ ""[ЗакрытиеМесяца]"".'");
				Если ЗначениеЗаполнено(ОбъектЗакрытиеМесяца.Ссылка) Тогда
					ТекстЗакрытияМесяца = "" + ОбъектЗакрытиеМесяца.Ссылка;
				Иначе
					ТекстЗакрытияМесяца = НСтр("ru='с видом операции: " + ВидОперации + "'");
				КонецЕсли;
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "[ЗакрытиеМесяца]", ТекстЗакрытияМесяца);		
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось провести документ ""[ЗакрытиеМесяца]"".
			...
			ТекстСообщения = НСтр("ru='Закончено выполнение операции: " + ВидОперации + " за " + МесяцНачисленияСтрокой + "'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Функция ПодготовитьПараметрыПроведения(ДокументОбъект, УбранФлажокПереносНЗПБУ, УбранФлажокПереносНЗПНУ, Отказ) Экспорт 
	...
	Запрос.УстановитьПараметр("Содержание",     НСтр("ru='Переоценка валютных средств'"));
	...
	Запрос.УстановитьПараметр("НомерЖурналаНМА"	,  НСтр("ru='НА'"));
	Запрос.УстановитьПараметр("НомерЖурналаОС"	,  НСтр("ru='ОС'"));
