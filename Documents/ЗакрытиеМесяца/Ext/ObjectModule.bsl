		...
		Для Каждого СтрокаВалют Из КурсыВалют Цикл
			Если Не ЗначениеЗаполнено(СтрокаВалют.Валюта) Тогда
				ТекстСообщения = ОбщегоНазначенияБККлиентСервер.ПолучитьТекстСообщения("Колонка",, НСтр("ru = 'Валюта'"),
					СтрокаВалют.НомерСтроки, "КурсыВалют");
...
Процедура РасчетСуммНДСКЗачету(ПараметрыПроведения, ПараметрыЗачетаНДС, Отказ)
	...
	ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='" +ТекстСообщения+"'"), , , Реквизиты.Объект , Перечисления.ВидыРегламентныхОпераций.РасчетНДСКЗачету, Ложь);	
...
Процедура РасчетВременныхРазниц(ПараметрыПроведения, Отказ)
	...
	Если СтавкаНалогаНаПрибыль = 0 Тогда
		ЗакрытиеМесяца.СообщитьОбОшибкеРегОперации(НСтр("ru='Не указана ставка налога на прибыль. Расчет временных разниц не возможен.'"), , ,ЭтотОбъект, Перечисления.ВидыРегламентныхОпераций.РасчетВременныхРазниц);
		Возврат;
	КонецЕсли;	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЗначениеЗаполнено(СообщенияОПовторахВидовОпераций) Тогда
		ТекстСообщения = НСтр("ru='"+СообщенияОПовторахВидовОпераций+"
		|Документ не может быть проведен!'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
	...
	Если НЕ ЗначениеЗаполнено(ВидРегламентнойОперации) Тогда
		ТекстСообщения = НСтр("ru='"+"-----" + "Начало проведения документа " + Ссылка + "-----"+"'" );
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВидРегламентнойОперации) Тогда	
		ТекстСообщения = НСтр("ru='"+"-----" + "Окончание проведения документа " + Ссылка + "-----"+"'" );
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	...
	Если ПереносНЗПБУ  Тогда
		...
		Если ПометкаУдаления  <> Ссылка.ПометкаУдаления  Тогда
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(ТекстСообщения,, ?(Ссылка.ПометкаУдаления = Ложь, "Удаление документа ", "Снятие пометки удаления документа ") + Ссылка + """", Ложь);			
	...
	Если ЗачетАвансовИПереплатПоНДС  Тогда
			...
			Если НЕ РасчетНДСКЗачету  Тогда
				...
				Если Результат.Количество() = 0 Тогда
					...
					ТекстСообщения = "В текущем месяце не был выполнен Зачет НДС к возмещению.
					|Необходимо провести документ ""Закрытие месяца"" с таким флажком!";
					ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(ТекстСообщения,,, Ложь);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если Ссылка.ПереносНЗПБУ ИЛИ Ссылка.ПереносНЗПНУ Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			ОбщегоНазначенияБККлиентСервер.СообщитьОбОшибке(НСтр("ru='" +ТекстСообщения+"'"),,, Ложь);
