...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|ПОМЕСТИТЬ Реквизиты
	|ИЗ
	|	Документ.ВводНачальныхОстатковОС КАК Реквизиты
	|ГДЕ
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|ПОМЕСТИТЬ ТаблицаОС
	|ИЗ
	|	Документ.ВводНачальныхОстатковОС.ОС КАК ТаблицаОС
	|ГДЕ
...
Процедура СформироватьДвиженияПроводокБух(ТаблицаОС, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Параметры.ТаблицаОС  Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчетаБУ) И НЕ (СтрокаТаблицы.ТекущаяСтоимостьБУ = 0 )Тогда
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			,
			"Документ.ВводНачальныхОстатковОС.ПФ_MXL_ОписьОС");
	КонецЕсли;
...
Функция ПечатьОпись(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	ТабДокумент.КлючПараметровПечати = "ВводНачальныхОстатковОС_Опись";
	ТабДокумент.ТолькоПросмотр       = Истина;
	
	Макет  = УправлениеПечатью.МакетПечатнойФормы("Документ.ВводНачальныхОстатковОС.ПФ_MXL_ОписьОС");		
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл 
		...
		Область.Параметры.Заголовок = РаботаСДиалогами.СформироватьЗаголовокДокумента(Ссылка, НСтр("ru = 'Ввод начальных остатков по ОС'"));
...
Функция ДанныеДляПечатиШтрихкодовОС(МассивДокументов) Экспорт
	...
	Возврат ПоместитьВоВременноеХранилище(ПодключаемоеОборудованиеБКВызовСервера.ПолучитьДанныеДляПечатиОС("ВводНачальныхОстатковОС", МассивДокументов));
