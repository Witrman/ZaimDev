...
Функция ПолучитьДанныеСпискаПоОборотам(СтруктураОтбора)
	...
	ВидыСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоТиповые.Партии);
	...
	|ИЗ
	|	РегистрБухгалтерии.Типовой.Обороты(&НачПериода, &КонПериода, , " + УсловиеВыбораСчетаВТекстеЗапроса + ", &ВидыСубконто, " + УсловиеВТекстеЗапроса + ", , ) КАК ОсновнойОбороты";
...
Функция ПолучитьДанныеСпискаПоОстаткам(СтруктураОтбора)
	...
	|	КОНЕЦ КАК Цена
	|ИЗ  
	|	РегистрБухгалтерии.Типовой.Остатки(&КонПериода, "+ УсловиеВыбораСчетаВТекстеЗапроса + ", &ВидыСубконто, " + УсловиеВТекстеЗапроса + ") КАК ОсновнойОстатки";
...
Процедура СчетУчетаПриИзменении(Элемент)
	...
	НаСчетеВедетсяУчетПоСкладам = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ВедетсяУчетПоСкладам(СчетУчета);
	УправлениеФормой(ЭтаФорма);
