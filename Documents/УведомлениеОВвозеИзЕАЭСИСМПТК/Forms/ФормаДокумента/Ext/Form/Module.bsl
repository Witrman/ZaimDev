...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОВвозеИзЕАЭС() Тогда		
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВыпискаБумажногоУведомления И Не ЗначениеЗаполнено(Объект.ДатаВыпискиНаБумажномНосителе) Тогда
		ТекстСообщения = НСтр("ru = 'A. Не указана дата Уведомления, выписанного на бумаге.'");
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияУведомленияОВвозе();
...
Процедура ВыборДокументаОснованияЗавершение(ВыбранныйДокумент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйДокумент <> "" И ТипЗнч(ВыбранныйДокумент) <> Тип("Неопределено") Тогда
		...
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.ДополнитьЗначенияОтбораПриВыбореДокументаОснованияДляВвозаТоваров(СтруктураОтбора, ИмяДокумента, Объект.Направление, "ВвозИзЕАЭС");
...
&НаКлиенте
Процедура СвязанноеУведомлениеПриИзменении(Элемент)
	
	СвязанноеУведомлениеПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура СвязанноеУведомлениеПриИзмененииНаСервере()
	
	Объект.РегистрационныйНомерСвязанногоУведомления = РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ЗначениеРеквизитаОбъекта(Объект.СвязанноеУведомление, "НомерИСМПТ");
	Объект.ДатаСвязанногоУведомления = РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.ЗначениеРеквизитаОбъекта(Объект.СвязанноеУведомление, "ДатаИСМПТ");
		
КонецПроцедуры

&НаКлиенте
Процедура СвязанноеУведомлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Отбор.Вставить("ТипУведомления", ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.Исходный"));
	Отбор.Вставить("Получатель",     Объект.Организация);
	Отбор.Вставить("Поставщик",  Объект.Поставщик);
	
	Если Отбор.Количество() <> 0 Тогда
		
		ПараметрыФормыВыбора.Вставить("Отбор", Отбор);
		
		ОткрытьФорму("Документ.УведомлениеОВвозеИзЕАЭСИСМПТК.ФормаВыбора", ПараметрыФормыВыбора, Элемент);
		СтандартнаяОбработка = Ложь;
...
Процедура ТипУведомленияПриИзменении(Элемент)
	...
	ТипУведомленияПриИзмененииНаСервере();	
	
КонецПроцедуры

&НаСервере
Процедура ТипУведомленияПриИзменениинаСервере()
	
	Объект.ЭтоКорректировка = ?(Объект.ТипУведомления = ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.Исправленный"), Истина, Ложь);
	УправлениеФормой();
...
Процедура ПоставщикПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Поставщик) Тогда
		СтранаОтправления = ИнтеграцияИСМПТКПереопределяемый.ПолучитьСтрануОтправленияВвозИзЕАЭС(Объект.Поставщик);
...
Процедура СопоставитьСДокументомОснованием()
	...
	Для Каждого Товар Из ПодобранныеМарки Цикл
		...
		Если Товар.КоличествоОснование = 0 Тогда
			...
			ТекстОшибки = НСтр("ru = 'В Уведомлении имеется номенклатура %Ном, отсутствующая в документе-основании.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Ном", Товар.Номенклатура);
			
		ИначеЕсли Товар.Количество = 0 Тогда
			
			ТекстОшибки = НСтр("ru = 'В Уведомлении отсутствуют данные по номенклатуре %Ном, присутствующей в документе-основании.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Ном", Товар.Номенклатура);
...
Процедура УправлениеФормой()
	...
	Если Исходящий И НеОтправлен Тогда
		ВыводитьКомандыИсходящегоДокумента = Истина;
	Иначе
		ВыводитьКомандыИсходящегоДокумента = Ложь;
	КонецЕсли;
	
	//Перезаполнение по основанию
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда 
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОбщаяКомандаПерезаполнитьПоПервичномуДокументуИСМПТК", "Видимость", ВыводитьКомандыИсходящегоДокумента);
	Иначе
	...
	//Связанное уведомление (корректировка)
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСвязанноеУведомление", "Видимость", Объект.ЭтоКорректировка);
	//Реквизиты бумажного документа
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаБумажныйДокумент", "Видимость", ВыпискаБумажногоУведомления);
	//Заголовок
	ИнтерфейсИСМПТК.УстановитьЗаголовокФормыДокумента(, Объект.Ссылка, ЭтаФорма);
	////Команда Проверить документ
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОбщаяКомандаПроверитьИСМПТК", "Доступность", НеОтправлен);
	//Ручной ввод ШК	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоискПоШтрихкоду", "Доступность", ВыводитьКомандыИсходящегоДокумента);
	...
	КонецЕсли;
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииПолучитьДанныеПоКоличеству", "Доступность", Исходящий); //команда используется и для отправленных документов тоже
	//Команды загрузки 
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандыЗагрузкиТЧ", "Доступность", ВыводитьКомандыИсходящегоДокумента);
	...
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.НастройкиПодключенияМобильноеПриложениеИСМПТК) Тогда
		...
		Если Не ИспользоватьПриложение Тогда
			ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииЗагрузитьКодыМаркировкиИзМобильногоПриложения", "Видимость", Ложь);
		Иначе
			ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииЗагрузитьКодыМаркировкиИзМобильногоПриложения", "Доступность", ВыводитьКомандыИсходящегоДокумента);
		КонецЕсли;
	Иначе
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииЗагрузитьКодыМаркировкиИзМобильногоПриложения", "Видимость", Ложь);
	КонецЕсли;
	...
	Иначе
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОбщаяКомандаРазбитьНаНесколькоДокументовИСМПТК", "Видимость", ВыводитьКомандыИсходящегоДокумента);
	КонецЕсли;
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДокументыСоответствия", "ТолькоПросмотр", НЕ НеОтправлен);
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТипУведомления", "Доступность", НеОтправлен);
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Объект.ТипУведомления = Неопределено ИЛИ Объект.ТипУведомления = ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.ПустаяСсылка") Тогда
...
Процедура ЗаполнитьПоДокументуОснованию()
	...
	Объект.СтранаОтправления = РеквизитыШапки.СтранаРегистрации;
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат ИнтеграцияИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияВвозТоваровИзЕАЭСНаСервере(Объект.ДокументОснование);
...
Процедура УстановитьВидимостьКнопокИзмененияСтатуса()
	...
	СответвствиеСтатусов = ИнтеграцияИСМПТК.РазрешенныеДействияПоСтатусамУведомленияОВвозеИзЕАЭС(Объект.Направление, Объект.ВидОперации);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзЕАЭСИСМПТКОтозвать",    "Видимость", СответвствиеСтатусов[ИнтеграцияИСМПТККлиентСервер.ДействиеОтзыв()][Объект.Статус]);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзЕАЭСИСМПТКПодтвердить", "Видимость", СответвствиеСтатусов[ИнтеграцияИСМПТККлиентСервер.ДействиеПодтверждение()][Объект.Статус]);
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	НеОтправлен = Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСМПТК.НеОпределен");
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзЕАЭСИСМПТКОтправить", "Доступность", НеОтправлен);
...
Процедура ЗапретитьРедактированиеПолей()
	...
	Если Не Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСМПТК.НеОпределен") Тогда
		...
		Элементы.РегистрационныйНомерСвязанногоУведомления.ТолькоПросмотр = Истина;
		Элементы.ДатаСвязанногоУведомления.ТолькоПросмотр = Истина;
		...
		Элементы.НомерКвитанцииАкциза.ТолькоПросмотр = Истина;
		Элементы.ДатаУплатыАкциза.ТолькоПросмотр = Истина;
		Элементы.СуммаАкциза.ТолькоПросмотр = Истина;
...
Процедура ОбработатьОтветСервераПоСостояниюКМ(МассивКМ, ОтветСервераСостояниеКМ)
	...
	ИнтеграцияИСМПТК.ОбработатьОтветСервераПоСостояниюКМ_Общая(ЭтаФорма, МассивКМ, ОтветСервераСостояниеКМ, "УведОВвозеЕАЭС", НовоеСоответствиеСтатусов);
