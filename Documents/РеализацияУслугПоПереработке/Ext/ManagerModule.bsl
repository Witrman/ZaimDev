...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОПродукции") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьОтчета("Услуги",МассивОбъектов, ОбъектыПечати),
			,
			"Документ.РеализацияУслугПоПереработке.ПФ_MXL_Акт");
	КонецЕсли;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОМатериалах") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьОтчета("МатериалыЗаказчика",МассивОбъектов, ОбъектыПечати),
			,
			"Документ.РеализацияУслугПоПереработке.ПФ_MXL_Акт");
	КонецЕсли;
...
//
Функция ПечатьАктаОбОказанииУслуг(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ДанныеДокументов.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ДанныеДокументов.Ссылка, "Акт");
		...
		Если ДанныеДокументов.УчитыватьНДС Тогда
			...
			ОбластьМакета.Параметры.НДС      = ?(ДанныеДокументов.СуммаВключаетНДС, "В том числе НДС", " Сумма НДС");
			ТабДокумент.Вывести(ОбластьМакета);
		...
		ОбластьМакета.Параметры.ИтоговаяСтрока ="Всего оказано услуг " + НомерСтроки
			+ ", на сумму " + ОбщегоНазначенияБКВызовСервера.ФорматСумм(СуммаКПрописи, ДанныеДокументов.ВалютаДокумента);
		ОбластьМакета.Параметры.СуммаПрописью  = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(СуммаКПрописи, ДанныеДокументов.ВалютаДокумента);
		ТабДокумент.Вывести(ОбластьМакета);
...
//
Функция ПечатьОтчета(ТабЧасть,МассивОбъектов, ОбъектыПечати)  Экспорт
	...
	Пока ДанныеДокументов.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ДанныеДокументов.Ссылка, "Отчет о " + ?(ТабЧасть = "Услуги", "продукции, произведенной из сырья заказчика", "переработанном сырье "));	
		ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПечатьР1(МассивОбъектов, ОбъектыПечати, КолонкиНДС = Ложь) Экспорт
	...
	Пока Док.Следующий() Цикл
		...
		Подвал.Параметры.ДатаПринятия             = Формат(Док.ДатаПодписанияГЗ, "ДЛФ=Д");
...
Функция ПечатьСчетаФактуры(МассивОбъектов, ОбъектыПечати, ВВалютеРеглУчета = Ложь) Экспорт
	...
	Возврат ?(ВВалютеРеглУчета, 
				...
				Документы.СчетФактураВыданный.ПечатьСчетФактура(МассивСчетовФактур, ОбъектыПечати, Ложь));
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт 
	...
	Запрос.УстановитьПараметр("СодержаниеВыделенНДС",     				НСтр("ru = 'НДС'"));
	Запрос.УстановитьПараметр("СодержаниеУслуги",     	   				НСтр("ru = 'Затраты от реализации произв. услуг в план.ценах'"));
	Запрос.УстановитьПараметр("СодержаниеПроводкиПлановойСебестоимости",НСтр("ru = 'Затраты от реализации услуг перераб.дав.сырья в план.ценах'"));
...
Процедура ДобавитьКолонкуСодержание(ТаблицаЗначений, ТекстСодержания = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекстСодержания) Тогда 
		ТекстСодержания = НСтр("ru='Выручка от реализации'");
	КонецЕсли;
		...
		Попытка
			...
			Иначе
				...
				Иначе
					...
					ЗаписьЖурналаРегистрации("Обновление ИБ. Документ ""Реализация услуг по переработке"". Заполнение даты подписания ГЗ", УровеньЖурналаРегистрации.Информация,,,ТекстСообщения);
