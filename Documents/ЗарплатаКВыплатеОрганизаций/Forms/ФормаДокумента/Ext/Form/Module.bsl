...
Процедура ПроверитьДокументыВведенныеНаОсновании()
	...
	Если УжеОплачивали Тогда
		...
		КоличествоФизЛиц = Мин(ПлатежныеДокументы.Итог("КоличествоФизлиц"), Объект.Зарплата.Количество());
		
		МаксДоковДляПоказа = 3;
		
		ОплатаТекст = ?(КоличествоДоктов = 1,НСтр("ru = 'документом '"), НСтр("ru = 'документами '"));
		Для Н = 1 По Мин(КоличествоДоктов, МаксДоковДляПоказа) Цикл
			ОплатаТекст = ОплатаТекст + ?(Н=1,"",", ") + ПлатежныеДокументы[Н-1].Представление;
		КонецЦикла;
		
		Если КоличествоДоктов >= МаксДоковДляПоказа Тогда
			ОплатаТекст = ОплатаТекст + "...";
		КонецЕсли;
		
		Если Объект.Зарплата.Количество() = 0 Тогда
			ОплатаСтатус = 1;
		Иначе
			ОплатаСтатус = КоличествоФизЛиц / Объект.Зарплата.Количество();
		КонецЕсли;
	Иначе
		ОплатаСтатус = 0;
		ОплатаТекст = "";
...
Функция ПолучитьОписаниеСостоянияДокумента(Заполнен, ЗаполненСОшибками, ЕстьНулевыеСуммы) 
	...
	Если ЗначениеЗаполнено(Объект.ПериодНачисленияДепонированнойЗарплаты)  
		...
		ТекстПоПериодам = ТекстПоПериодам + Символы.ВК + НСтр("ru = 'Период начисления зарплаты:  "+ Формат(Объект.ПериодНачисленияЗарплаты,"ДФ=dd.MM.yyyy") + ". '")
						+ НСтр("ru = 'Период начисления депонированной зарплаты:" + Формат(Объект.ПериодНачисленияДепонированнойЗарплаты,"ДФ=dd.MM.yyyy") + ".'") + Символы.ВК;
...
Процедура ПодготовитьСправкуФормы(Форма) 
	...
	//вариант справки 1
	ТекстСпособаВыплат = "<B>Способ заполнения документа</B> <A id=АктивизироватьЭУ href=""V8:СпособРасчетаСуммКВыплате""><P id=СпособРасчетаСуммКВыплате style=""DISPLAY:inline"">" + Объект.СпособРасчетаСуммКВыплате;
	...
	Если Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ЗарплатаЗаМесяц") Тогда
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(зарплата за <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>).";
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ДепонированнаяЗарплатаЗаМесяц") Тогда		
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(депонированная зарплата в <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>).";
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ЗарплатаИДепонированнаяЗарплата") Тогда		
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(зарплата за <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>&nbsp;и депонированная зарплата в <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>).";
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ОстаткиПоЗарплатеНаКонецМесяца") Тогда		
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(остатки по зарплате по <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>&nbsp;(включительно)).";		
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ОстаткиПоДепонированнойЗарплатеНаКонецМесяца") Тогда		
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(остатки по депонированной зарплате по <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>&nbsp;(включительно)).";
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ЗарплатаИОстаткиПоДепонированнойЗарплате") Тогда		
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(зарплата за <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>&nbsp;и остатки по депонированной зарплате по <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>&nbsp;(включительно)).";
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ВсяСуммаЗадолженности") Тогда		
		...
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(остатки по зарплате и депонированной зарплате по <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>).";
	...
	ИначеЕсли Объект.СпособРасчетаСуммКВыплате = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммКВыплате.ПоВидуРасчета") Тогда		
		...
		Если Объект.ВидРасчета = Неопределено Тогда
			ВидРасчетаСправка = "<не указан>";
		ИначеЕсли Объект.ВидРасчета = ПредопределенноеЗначение("ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка") Тогда
			ВидРасчетаСправка = "<не указан>";
		Иначе
			ВидРасчетаСправка = Объект.ВидРасчета;		
		КонецЕсли;
		
		ТекстСпособаВыплат = ТекстСпособаВыплат + "</P></A>&nbsp;(зарплата за <A id=АктивизироватьЭУ href=""V8:МесяцНачисленияСтрокой""><P id=ПериодНачисленияЗарплаты style=""DISPLAY:inline"">" + ИППериодЗарплаты + "</P></A>&nbsp;по виду расчета <A id=АктивизироватьЭУ href=""V8:ВидРасчета""><P id=ВидРасчета style=""DISPLAY:inline"">" + ВидРасчетаСправка + "</P></A>).";
	...
	Если Объект.СпособВыплаты = ПредопределенноеЗначение("Перечисление.СпособыВыплатыЗарплаты.ЧерезКассу") Тогда
		СпособВыплатыТекст = "(список сотрудников, не имеющих карт-счета в банке).";	
	ИначеЕсли Объект.СпособВыплаты = ПредопределенноеЗначение("Перечисление.СпособыВыплатыЗарплаты.ЧерезБанк") Тогда
		СпособВыплатыТекст = "(список сотрудников, имеющих карт-счета в банке <A id=АктивизироватьЭУ href=""V8:Банк""><P id=Банк style=""DISPLAY:inline"">" + ?(ЗначениеЗаполнено(Объект.Банк), Объект.Банк, "<не указан>") + "</P></A>).";
	Иначе
		СпособВыплатыТекст = "(при заполнении наличие карт-счета не учитывается).";
	КонецЕсли;
	...
	|		<DIV>
	|		<B>Способ выплаты</B> <A id=АктивизироватьЭУ href=""V8:СпособВыплаты""><P id=СпособВыплаты style=""DISPLAY:inline"">" + ?(ЗначениеЗаполнено(Объект.СпособВыплаты), Объект.СпособВыплаты, "<не указан>") + "</P></A>&nbsp;" + СпособВыплатыТекст + "</DIV>
	|       <DIV>
	|	    <B>Процент выплаты </B>  " + Строка(Объект.ПроцентВыплаты) + ".
	|       </DIV>
	|       <DIV>
	|	    <B>Округлять суммы до </B> " + Строка(Объект.РазмерОкругления) + ".
	|       </DIV>
	|		<DIV>
	|		Для изменения порядка округления сумм, указания процента выплаты, необходимо воспользоваться 
	|		гиперссылкой <A id=АктивизироватьЭУ href=""V8:ДекорацияПараметрыЗаполнения""><P id=НадписьПараметрыЗаполнения style=""DISPLAY:inline"">Настройка расчета сумм к выплате...</P></A>.</DIV>
	|		<BR>
	|	</FONT></BODY>
	|</HTML>";
