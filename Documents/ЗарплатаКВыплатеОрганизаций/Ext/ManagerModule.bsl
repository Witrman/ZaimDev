...
Функция ПечатьТ2(МассивОбъектов, ОбъектыПечати, СортироватьПоТабельномуНомеру = Истина) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьМакетаШапкаДокумента.Параметры.ФИОГлБуха 			   = ?(НЕ ЗначениеЗаполнено(ОтветЛица.ГлавныйБухгалтер), НСтр("ru = 'Не предусмотрен'"), ОтветЛица.ГлавныйБухгалтер);
		...
		ОбластьМакетаШапкаДокумента.Параметры.ДатаДок = Формат(Шапка.Дата, "ДФ=""дд ММММ гггг ' года.'""");
		...
		Если Шапка.СпособРасчетаСуммКВыплате = Перечисления.СпособыРасчетаСуммКВыплате.ЗарплатаЗаМесяц 
			...
			ОбластьМакетаШапкаДокумента.Параметры.ПериодВыплаты = "за "+Формат(Шапка.ПериодНачисленияЗарплаты, "ДФ=ММММ") + " " + Формат(Шапка.ПериодНачисленияЗарплаты, "ДФ=гггг") + " года.";
		...
		ИначеЕсли Шапка.СпособРасчетаСуммКВыплате = Перечисления.СпособыРасчетаСуммКВыплате.ДепонированнаяЗарплатаЗаМесяц 
			...
			ОбластьМакетаШапкаДокумента.Параметры.ПериодВыплаты = "за "+Формат(?(Шапка.ПериодНачисленияДепонированнойЗарплаты = Дата("00010101"), Шапка.ПериодНачисленияЗарплаты, Шапка.ПериодНачисленияДепонированнойЗарплаты), "ДФ=ММММ") + " " + Формат(Шапка.ПериодНачисленияДепонированнойЗарплаты, "ДФ=гггг") + " года.";
		...
		Иначе
			Если Шапка.ПериодНачисленияЗарплаты = Шапка.ПериодНачисленияДепонированнойЗарплаты ИЛИ Шапка.ПериодНачисленияДепонированнойЗарплаты = Дата("00010101") Тогда
				ОбластьМакетаШапкаДокумента.Параметры.ПериодВыплаты = "за "+Формат(Шапка.ПериодНачисленияЗарплаты, "ДФ=ММММ") + " " + Формат(Шапка.ПериодНачисленияЗарплаты, "ДФ=гггг") + " года.";
			Иначе
				ОбластьМакетаШапкаДокумента.Параметры.ПериодВыплаты = "за "+Формат(Мин(Шапка.ПериодНачисленияЗарплаты,Шапка.ПериодНачисленияДепонированнойЗарплаты), "ДЛФ=дд") + " - " + Формат(Макс(Шапка.ПериодНачисленияЗарплаты,Шапка.ПериодНачисленияДепонированнойЗарплаты), "ДЛФ=дд");		
			КонецЕсли;
		КонецЕсли;
