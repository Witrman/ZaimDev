...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.НастройкиПодключенияМобильноеПриложениеИСМПТК) Тогда
		...
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоКодовМаркировкиПолученныхЗагрузитьКодыМаркировкиИзМобильногоПриложения", "Видимость", ИспользоватьПриложение);
	КонецЕсли;
...
Процедура ВыбратьВсе(Команда)
	...
	ЭелементыДерева = ДеревоКодовМаркировкиПолученных.ПолучитьЭлементы();
	
	Для Каждого СтрокаТЧ Из ЭелементыДерева Цикл
		...
		Если СтрокаТЧ.СтатусКодаПослеСверки = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.НеНайден")
			Или СтрокаТЧ.СтатусКодаПослеСверки = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.Исключен") Тогда
...
Процедура ЗадатьВопросИПродолжитьДобавлениеКМ(СтруктураКодовМаркировки) Экспорт
	...
	ТекстВопроса = "Указанный код идентификации «" + СтруктураКодовМаркировки.КодИдентификации + "» отсутствует в сверяемом Акте приема/передачи.
	|Он будет добавлен в список со статусом сверки ""Добавлен"".";
...
Функция НайтиПоКодуИдентификации(КодИдентификации)
	...
	Если Не НайденныйЭлемент = Неопределено Тогда
		...
		ИначеЕсли НайденныйЭлемент.СтатусКодаПослеСверки = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.Найден") Тогда
			НайденныйЭлемент.СтатусКодаПослеСверки = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.Исключен"); //код подтвержден, но не нужен (возврат поставщику)
		КонецЕсли;
...
Функция ОбработатьРезультатЗагрузкиКодов(АдресРезультата)
	...
	Если РезультатЗадания.Успешно И НЕ РезультатЗадания.ТаблицаПолученныхКодов.Количество() = 0 Тогда
		Возврат ИнтеграцияИСМПТКПереопределяемый.ТаблицаЗначенийВМассив(РезультатЗадания.ТаблицаПолученныхКодов);
	Иначе 
...
Процедура ПолучитьКлючАвторизации(СобытиеПослеАвторизации) Экспорт
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиент.ПолучитьКлючАвторизацииОбщая(СобытиеПослеАвторизации, "");
...
Процедура ЗавершениеПолучитьДанныеПоГорупповымКодамМаркировки(РезультатВыполнения, Параметры) Экспорт
	...
	Для Каждого ЭлементДерева Из ЭелементыДерева Цикл
		...
		Если ЭлементДерева.ВидУпаковки = Перечисления.ВидыУпаковокИСМПТК.Логистическая Тогда
			...
			СтруктураЗапрос = Новый Структура("Штрихкод, ВидУпаковки, ВидПродукции, ФорматBase64", ИнформацияПоКоду, ЭлементДерева.ВидУпаковки, ЭлементДерева.ВидПродукцииИС, Ложь);
			...
			Если ЗначениеЗаполнено(ДанныеПоГрупповомуКоду.ТекстОшибки) Тогда
				ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ДанныеПоГрупповомуКоду.ТекстОшибки);
			Иначе
...
Процедура ЗавершениеПолучитьДанныеПоГрупповомуКодуМаркировки(РезультатВыполнения, Параметры) Экспорт
	...
	Если Не ТекущийЭлементДерева.ВидУпаковки = Перечисления.ВидыУпаковокИСМПТК.Логистическая Тогда 
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю("Получить вложенные коды можно только для транспортной упаковки!");
	Иначе
		...
		СтруктураЗапрос = Новый Структура("Штрихкод, ВидУпаковки, ВидПродукции, ФорматBase64", ИнформацияПоКоду, ТекущийЭлементДерева.ВидУпаковки, ТекущийЭлементДерева.ВидПродукцииИС, Ложь);
	...
	Для Каждого ВложенныйКод Из ВложенныеКоды Цикл
		Если ТипЗнч(ВложенныйКод) = Тип("Строка") Тогда
			...
			Если ТипЗнч(ДанныеКода) = Тип("Строка") Тогда
				...
				ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ВложенныйКод);
				ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
				Продолжить;
			КонецЕсли;
		...
		Иначе
			...
			Если ТипЗнч(ДанныеКода) = Тип("Строка") Тогда
				ТекстОшибки = НСтр("ru = 'Не удалось разобрать код маркировки, %1 он не был добавлен в документ.'");
				ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ВложенныйКод.Ключ);
				ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
				Продолжить;
			КонецЕсли;
...
Процедура ДобавитьНовуюВеткуДерева(ТекущаяВетка, ВложенныйКод, ДанныеКода, ИспользоватьВложенный, НоваяВетка)
	...
	НоваяВетка.ВидПродукцииИС			   = ДанныеКода.ВидПродукцииИС;
...
Функция ЗаполнитьТаблицуКодовМаркировки(МассивКодов)
	...
	Для Каждого ВложенныйКод Из МассивКодов Цикл
		...
		НоваяСтрока.ВидПродукцииИС	 = ДанныеКода.ВидПродукцииИС;
...
Функция ЗаполнитьДеревоКодовМаркировки()
	...
	Для Каждого СтрокаКодаМаркировки Из ТаблицаКодовМаркировки Цикл
		...
		Если ТипЗнч(ДанныеКода) = Тип("Строка") Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось разобрать код маркировки %1, он не был добавлен в документ.'");
			ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаКодаМаркировки.КодИдентификации);
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
			Продолжить;
		КонецЕсли;
		...
		Если ДанныеКода.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИСМПТК.Групповая") Тогда
			//Определяем ном-ру для блоков по верхнему уровню. Если ведется учет через одну ном-ру и упаковки, товар будет тот же, что для нижнего уровня.
			//Но если это разные ном-ры (блок и пачка), то подобрать нужно именно блок.
			EANВерхнегоУровня  = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодEANИзGTIN(ДанныеКода.GTINВерхнегоУровня);
			ДанныеНоменклатуры = ПолучитьНоменклатуруПоШтрихкоду(EANВерхнегоУровня);
		Иначе
		...
		НоваяВетка.ВидУпаковки				   = ДанныеКода.ВидУпаковки;
		НоваяВетка.Количество		 		   = 1;
		НоваяВетка.ВидПродукцииИС		 	   = СтрокаКодаМаркировки.ВидПродукцииИС;
		НоваяВетка.СтатусКодаПослеСверки 	   = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.НеНайден");
...
Функция ДобавитьКодИдентификацииВДерево(СтруктураКодовМаркировки)
	...
	НовыйЭлемент.ВидПродукцииИС	 			 = СтруктураКодовМаркировки.ВидПродукцииИС;
...
Процедура ДеревоКодовМаркировкиПолученныхПередУдалением(Элемент, Отказ)
	...
	Если Не Элементы.ДеревоКодовМаркировкиПолученных.ТекущиеДанные.СтатусКодаПослеСверки = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.Добавлен") Тогда
		...
		//Все остальные коды проверяем на наличие вышестоящих - "удалять" разрешается только код верхнего уровня.
		//При этом из таблицы его не удаляем, только меняем статус на Исключен.
	
		КодыПервогоУровня = ДеревоКодовМаркировкиПолученных.ПолучитьЭлементы();
		МассивТекущихКодовВерхнегоУровня = Новый Массив();
		Для Каждого СтрокаВерхнегоУровня Из КодыПервогоУровня Цикл
			МассивТекущихКодовВерхнегоУровня.Добавить(СтрокаВерхнегоУровня.КодИдентификации);
		КонецЦикла;
		                          
		Если НЕ МассивТекущихКодовВерхнегоУровня.Найти(Элементы.ДеревоКодовМаркировкиПолученных.ТекущиеДанные.КодИдентификации) = Неопределено Тогда
			Элементы.ДеревоКодовМаркировкиПолученных.ТекущиеДанные.СтатусКодаПослеСверки = ПредопределенноеЗначение("Перечисление.СтатусыСверкиКодаИСМПТК.Исключен");
			Элементы.ДеревоКодовМаркировкиПолученных.ТекущиеДанные.Подобран = Истина;
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
...
Процедура ОбработатьПолученныеДанныеТСД()
	...
	//Если ИнтеграцияИСМПТКВызовСервера.ИспользоватьАвтоОпределениеЛогистическихКодовИСМПТК() Тогда 
	//	ИнтеграцияИСМПТККлиент.ЗагрузитьКодыМаркировкиИзФайлаЗавершение(ИнтеграцияИСМПТККлиент.ПолучитьТокенАвторизации(Организация), СтруктураПараметров);
	//Иначе
