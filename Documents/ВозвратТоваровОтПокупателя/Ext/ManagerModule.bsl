...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт 
	...
	Запрос.УстановитьПараметр("СодержаниеСписаныТМЗ", НСтр("ru='Возврат ТМЗ от покупателя'"));
	...
	Запрос.УстановитьПараметр("СодержаниеВыделенНДС",      НСтр("ru='Возврат суммы НДС'"));
	Запрос.УстановитьПараметр("СодержаниеВыделенАкциз",    НСтр("ru='Возврат суммы акциза'"));
	Запрос.УстановитьПараметр("СодержаниеЗачетВыручки",    НСтр("ru='Зачет сумм возврата: выручка'"));
	Запрос.УстановитьПараметр("СодержаниеЗачетНДС",        НСтр("ru='Зачет сумм возврата: НДС'"));
	Запрос.УстановитьПараметр("СодержаниеЗачетАкциза",     НСтр("ru='Зачет сумм возврата: акциз'"));
...
Процедура ДобавитьКолонкуСодержание(ТаблицаЗначений, ТекстСодержания = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекстСодержания) Тогда 
		ТекстСодержания = НСтр("ru='Возврат выручки от реализации'");
	КонецЕсли;
		...
		Для Каждого СтрокаДокумента Из ТаблицаТоварыУслуги Цикл
			...
			Для Каждого СтрокаАванса Из ТаблицаЗачетАвансов Цикл
				...
				Если Суффикс = "НДС" Тогда 
					СтрокаРезультат.Содержание     = НСтр("ru = 'Зачет сумм возврата: НДС'");
					СтрокаРезультат.СодержаниеНУ   = НСтр("ru = 'Возврат суммы НДС'");
					...
					Если НайденныеСтроки.Количество() <> 0 Тогда 
						СтрокаРезультат.СчетДоходовБУ = НайденныеСтроки[0].СчетУчетаНДС;
						СтрокаРезультат.СубконтоДоходовБУ1 = Справочники.НалогиСборыОтчисления.НалогНаДобавленнуюСтоимость;
						...
						СтрокаРезультат.СчетДоходовНУ = ПроцедурыНалоговогоУчетаВызовСервераПовтИсп.ПолучитьСчетУчетаНДСНачисленногоНУ(НайденныеСтроки[0].СчетУчетаНДС, Реквизиты.Период);
						СтрокаРезультат.СубконтоДоходовНУ1 = Справочники.НалогиСборыОтчисления.НалогНаДобавленнуюСтоимость;
				...
				ИначеЕсли Суффикс = "Акциза" Тогда 
					СтрокаРезультат.Содержание     = НСтр("ru = 'Зачет сумм возврата: акциз'");
					СтрокаРезультат.СодержаниеНУ   = НСтр("ru = 'Возврат суммы акциза'");
					СтрокаРезультат.ЗнакПроводкиНУ = -1;
					...
					КонецЕсли;
				Иначе 
					СтрокаРезультат.Содержание = НСтр("ru = 'Зачет сумм возврата: выручка'");
			...
			Если ОсталосьПогаситьВал <> 0 ИЛИ ОсталосьПогаситьРегл <> 0 Тогда 
				...
				Суффикс = Сред(Показатель.Значение, 6);
				Если Суффикс = "НДС" Тогда 
					СтрокаРезультат.Содержание = НСтр("ru = 'Возврат суммы НДС'");
					СтрокаРезультат.ЗнакПроводкиНУ = -1;
					...
					КонецЕсли;
				ИначеЕсли Суффикс = "Акциза" Тогда 
					СтрокаРезультат.Содержание = НСтр("ru = 'Возврат суммы акциза'");
					СтрокаРезультат.ЗнакПроводкиНУ = -1;
				...
				Иначе 
					...
					СтрокаРезультат.Содержание = НСтр("ru = 'Возврат выручки от реализации'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактураВыданныйВВалютеРеглУчета") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьСчетаФактуры(МассивОбъектов, ОбъектыПечати, Истина),
			,
			"Документ.СчетФактураВыданный.ПФ_MXL_СчетФактура");
	КонецЕсли;
...
// Функция формирует табличный документ с печатной формой накладной,
	...
	//
Функция ПечатьВозвратаОтПокупателя(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		ОбластьМакетаЗаголовок.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ВыборкаПоДокументам.Ссылка, "Возврат от покупателя");
		...
		Если ВыборкаПоДокументам.УчитыватьНДС Тогда
			...
			ОбластьИтогоНДС.Параметры.НДС = ?(ВыборкаПоДокументам.СуммаВключаетНДС, "В том числе НДС:", "Сумма НДС:");
		...
		ОбластьМакетаСуммаПрописью.Параметры.ИтоговаяСтрока = "Всего наименований " + ВсегоНаименований
		+ ", на сумму " + ОбщегоНазначенияБКВызовСервера.ФорматСумм(СуммаКПрописи, ВыборкаПоДокументам.ВалютаДокумента);
		
		ОбластьМакетаСуммаПрописью.Параметры.СуммаПрописью = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(СуммаКПрописи, ВыборкаПоДокументам.ВалютаДокумента);
...
Функция ПечатьСчетаФактуры(МассивОбъектов, ОбъектыПечати, ВВалютеРеглУчета = Ложь) Экспорт
	...
	Возврат ?(ВВалютеРеглУчета, 
				Документы.СчетФактураВыданный.ПечатьСчетФактураВВалютеРеглУчета(МассивСчетовФактур, ОбъектыПечати), 
				Документы.СчетФактураВыданный.ПечатьСчетФактура(МассивСчетовФактур, ОбъектыПечати));
