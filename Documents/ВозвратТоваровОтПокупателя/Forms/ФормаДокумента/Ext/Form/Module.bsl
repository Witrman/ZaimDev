...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		РаботаСДиалогами.УстановитьЗаголовокФормыДокумента("продажа", Объект.Ссылка, ЭтаФорма);
...
Процедура СформироватьНадписьЦеныИВалюта(Форма)
	...
	Если Форма.Объект.СуммаВключаетНДС Тогда
		Форма.Элементы.ИтогиВсегоНДС.Заголовок = "НДС (в т. ч.)";
	Иначе
		Форма.Элементы.ИтогиВсегоНДС.Заголовок = "НДС (сверху)";
	КонецЕсли;
	
	Если Форма.Объект.СуммаВключаетАкциз Тогда
		Форма.Элементы.ИтогиВсегоАкциз.Заголовок = "Акциз (в т. ч.)";
	Иначе
		Форма.Элементы.ИтогиВсегоАкциз.Заголовок = "Акциз (сверху)";
	КонецЕсли;
...
Процедура ПроверкаСозданияДополнительныхДокументов(ПараметрыЗаписи)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВозвратТоваровОтПокупателя.ВозвратРозничнойПродажи")
		...
		Если ЕстьПодходящиеДокументыРКО() Тогда
			...
			ТекстВыбораДокумента = нСтр("ru=', или выбрать существующий документ'")
		КонецЕсли;
		...
		ТекстВопроса = ТекстВопроса + НСтр("ru='необходимо оформить документ Расходный кассовый ордер и пробить чек ККМ.'");
		ТекстВопроса = ТекстВопроса + Символы.ПС + Символы.ПС;
		ТекстВопроса = ТекстВопроса + СтрШаблон(НСтр("ru='Создать документ РКО и пробить чек%1?'"), ТекстВыбораДокумента);
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаСозданиеРозничныхДокументов", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, СписокКнопок, 0,, "Создание связанных документов");
	КонецЕсли;
...
Функция СоздатьДокументРКО()
	...
	ДокументРКО.Комментарий = НСтр("ru='Создан автоматически при проведении документа Возврат товаров от покупателя.'") ;
...
Функция ЗаписатьДокументРКО(ДокументСсылка, РеквизитыДляЗаписи)
	...
	Попытка
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Создан документ: %1'"), ДокументОбъект.Ссылка);
