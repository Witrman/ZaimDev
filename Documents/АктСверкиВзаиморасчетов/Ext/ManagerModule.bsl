...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если НужноПечататьМакет Тогда
		...
		"АктСверки",
		НСтр("ru = 'Акт сверки'"),
		ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати),
		,
...
Функция ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		ОписаниеПериода = НСтр("ru = 'Период: %1'");
		ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ОписаниеПериода,  ПредставлениеПериода(НачалоДня(Шапка.ДатаНачала), КонецДня(Шапка.ДатаОкончания), "ФП = Истина"));
		
		ИмяПредставителяОрганизации = Шапка.ПредставительОрганизации;
		ИмяПредсатвителяКонтрагента = Шапка.ПредставительКонтрагента;
		
		ТекстЗаголовка = НСтр("ru = 'взаимных расчетов за период с %1 по %2 между %3 и %4'");
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗаголовка, Формат(Шапка.ДатаНачала, "ДФ=dd.MM.yyyy"), Формат(Шапка.ДатаОкончания, "ДФ=dd.MM.yyyy"), НазваниеОрганизации, НаименованиеКонтрагента);
		
		Если ЗначениеЗаполнено(Шапка.Договор) Тогда
			Если Шапка.Договор.ЭтоГруппа Тогда
				ПодтекстЗаголовка = НСтр("ru = '%1 по группе договоров %2'");
			Иначе
				ПодтекстЗаголовка = НСтр("ru = '%1 по договору %2'");
			КонецЕсли;
		...
		ДанныеПредставителяОрганизации = ПроцедурыУправленияПерсоналомВызовСервера.ДанныеФизЛица(Шапка.Организация, Шапка.ПредставительОрганизации, Шапка.ДатаОкончания);
		...
		ОбластьНачОстатки.Параметры.ДатаНачала = Формат(Шапка.ДатаНачала, "ДЛФ=DD");
		...
		Пока ОбходПоДоговорамСКонтрагентом.Следующий() Цикл
			...
			Пока ОбходПоДоговорам.Следующий() Цикл
				Если ОбходПоДоговорам.Дата <> NULL Тогда
					СписокДокументов.Добавить(Новый Структура("ДатаДокумента, РегистраторПредставление, Документ, СуммаОборотДт, СуммаОборотКт, ОтложитьПринятиеНДСКЗачету",
					ОбходПоДоговорам.Дата, ОбходПоДоговорам.Представление, ОбходПоДоговорам.Документ, ОбходПоДоговорам.СуммаДогДт, ОбходПоДоговорам.СуммаДогКт, ОбходПоДоговорам.ОтложитьПринятиеНДСКЗачету));
				КонецЕсли;
				
				Если ОбходПоДоговорам.ДатаК <> NULL Тогда
					СписокДокументовК.Добавить(Новый Структура("ДатаДокумента, РегистраторПредставление, Документ, СуммаОборотДт, СуммаОборотКт, ОтложитьПринятиеНДСКЗачету",
					ОбходПоДоговорам.ДатаК, ОбходПоДоговорам.ПредставлениеК, ОбходПоДоговорам.Документ, ОбходПоДоговорам.СуммаДогДтКонтр, ОбходПоДоговорам.СуммаДогКтКонтр, ОбходПоДоговорам.ОтложитьПринятиеНДСКЗачету));
				КонецЕсли;
			КонецЦикла; 
			...
			Для Инд = 0 По МаксИндекс Цикл
				Если Инд < СписокДокументов.Количество() Тогда
					...
					Если НЕ СтрокаОрганизации.Документ = Неопределено Тогда
						...
						+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаОрганизации.Документ.Номер, СтрокаОрганизации.Документ) + НСтр("ru =' от '") 
						...
						Если СтрокаОрганизации.Документ.Метаданные().Реквизиты.Найти("ВидВходящегоДокумента") <> Неопределено Тогда
							Если ЗначениеЗаполнено(СтрокаОрганизации.Документ.НомерВходящегоДокумента) И ЗначениеЗаполнено(СтрокаОрганизации.Документ.ДатаВходящегоДокумента) Тогда
								...
								НСтр("ru = ' № вх. '") + СтрокаОрганизации.Документ.НомерВходящегоДокумента + НСтр("ru = ' от '") + Формат(СтрокаОрганизации.Документ.ДатаВходящегоДокумента, "ДФ=dd.MM.yyyy");  
						...
						Иначе
							...
							РегистраторПредставление =  РегистраторПредставление + ?(ЗначениеЗаполнено(НомерДокументаГЗ),", № вх. " + НомерДокументаГЗ 
					...
					ОбластьОбороты.Параметры.ДатаДокумента = Формат(СтрокаОрганизации.ДатаДокумента,"ЧЦ=21; ЧДЦ=2; ДФ=dd.MM.yy");
				Иначе
					Если СтрокаОрганизации <> Неопределено Тогда 
				...
				Если Инд < СписокДокументовК.Количество() Тогда
					...
					Если НЕ СтрокаОрганизацииКонтр.Документ = Неопределено Тогда
						...
						+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаОрганизацииКонтр.Документ.Номер, СтрокаОрганизацииКонтр.Документ) + НСтр("ru = ' от '") 
						...
						Если СтрокаОрганизацииКонтр.Документ.Метаданные().Реквизиты.Найти("ВидВходящегоДокумента") <> Неопределено Тогда
							Если ЗначениеЗаполнено(СтрокаОрганизацииКонтр.Документ.НомерВходящегоДокумента)  
								...
								НСтр("ru = ' № вх. '") + СтрокаОрганизацииКонтр.Документ.НомерВходящегоДокумента + НСтр("ru = ' от '") + Формат(СтрокаОрганизацииКонтр.Документ.ДатаВходящегоДокумента,"ДФ=dd.MM.yyyy");  
							КонецЕсли;
						...
						Иначе
							...
							РегистраторПредставлениеКонтр =  РегистраторПредставлениеКонтр +  ?(ЗначениеЗаполнено(НомерДокументаГЗ),", № вх. " + НомерДокументаГЗ 
							+ ?(ЗначениеЗаполнено(ДатаДокументаГЗ)," от " + Формат(ДатаДокументаГЗ,"ДФ=dd.MM.yyyy"),""),"");
						КонецЕсли;
					...
					ОбластьОбороты.Параметры.ДатаДокументаКонтр = Формат(СтрокаОрганизацииКонтр.ДатаДокумента,"ЧЦ=21; ЧДЦ=2; ДФ=dd.MM.yy");
				Иначе
					Если СтрокаОрганизацииКонтр <> Неопределено Тогда 
				...
				Если ДокументСФКонтр <> Неопределено Тогда
					...
					Если ТипЗнч(ДокументСФКонтр) = Тип("ДокументСсылка.СчетФактураПолученный") Тогда
						...
						Если ЗначениеЗаполнено(ДокументСФКонтр.НомерВходящегоДокумента) Тогда
							Документ = Документ + НСтр("ru = ' № вх. '") + ДокументСФКонтр.НомерВходящегоДокумента;
						КонецЕсли;
						Если ЗначениеЗаполнено(ДокументСФКонтр.ДатаВходящегоДокумента) Тогда
							Документ = Документ + НСтр("ru = ' от '") + Формат(ДокументСФКонтр.ДатаВходящегоДокумента, "ДФ=dd.MM.yyyy");
						КонецЕсли;
		...
		ОбластьКонОстатки.Параметры.ДатаОкончания = Формат(КонецДня(Шапка.ДатаОкончания), "ДЛФ=DD");
		...
		Если СписокДокументов.Количество() = 0 И ОстатокНаКонец = 0 Тогда
			РезультатыСверки = "";
		ИначеЕсли НЕ ЗначениеЗаполнено(Шапка.ДатаОкончания) Тогда
		...
		Иначе
			РезультатыСверки = НСтр("ru = 'на '") + Формат(Шапка.ДатаОкончания, "ДФ=dd.MM.yyyy") + НСтр("ru = ' задолженность '");
			
			Если ОстатокНаКонец > 0 Тогда
				ТекстСверки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в пользу %1  %2  %3 (%4)'"),
				СокрЛП(НазваниеОрганизации),
				Формат(ОстатокНаКонец, "ЧЦ=21; ЧДЦ=2"),
				Строка(ВалютаРезультата),
				СокрЛП(ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(ОстатокНаКонец, ВалютаРезультата))
				);
		...
		Если Шапка.СверкаСогласована Тогда
			ОбластьПодвал.Параметры.ПоДаннымКонтрагента = НСтр("ru = 'По данным '") + СокрЛП(НаименованиеКонтрагента);
			...
			Иначе
				...
				Если ОстатокНаКонецК < 0 Тогда
					...
					Строка(ВалютаРезультата),
					СокрЛП(ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(-ОстатокНаКонецК, ВалютаРезультата))
					);
			...
			Если ОстатокНаКонец+ОстатокНаКонецК<>0 Тогда
				...
				НСтр("ru = 'В результате сверки выявлено расхождение информации о состоянии расчетов в размере %1 %2 (%3).'"),
				Формат(СуммаРасхождений, "ЧЦ=21; ЧДЦ=2"),
				Строка(ВалютаРезультата),
				СокрЛП(ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(СуммаРасхождений, ВалютаРезультата))
				);
		...
		ОбластьПодвал.Параметры.ПредставлениеРНН_БИНОрганизации = ОбщегоНазначенияБК.ПолучитьРегистрационныйНомерОрганизацииКонтрагентаВПечатнуюФорму(СведенияОбОрганизации, Шапка.Ссылка.Дата);
		
		СведенияОПокупателе = ОбщегоНазначенияБКВызовСервера.СведенияОЮрФизЛице(Шапка.Контрагент, Шапка.Дата);
		
		ОбластьПодвал.Параметры.НаименованиеКонтрагента = СокрЛП(НаименованиеКонтрагента);
		ОбластьПодвал.Параметры.ПредставлениеРНН_БИНКонтрагента = ОбщегоНазначенияБК.ПолучитьРегистрационныйНомерОрганизацииКонтрагентаВПечатнуюФорму(СведенияОПокупателе, Шапка.Ссылка.Дата);
...
Функция ПолучитьТесктовоеПредставлениеСФ(ДокументЭСФ)
	...
	Если ДокументЭСФ <> Неопределено Тогда  		
		ТекстовоеПредставлениеДокумента = ДокументЭСФ.Метаданные().Синоним +" " 
		+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ДокументЭСФ.Номер, ДокументЭСФ) + НСтр("ru = ' от '") 
