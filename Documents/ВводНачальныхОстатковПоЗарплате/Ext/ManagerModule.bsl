...
Функция ПодготовитьПараметрыПроведения(Ссылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("СтрокаСодержаниеЗП", 	НСтр("ru = 'Нач. сальдо по з/п за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеДепЗП", 	НСтр("ru = 'Нач. сальдо по деп. з/п за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеОПВ", 	НСтр("ru = 'Сальдо ОПВ за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеОППВ", 	НСтр("ru = 'Сальдо ОППВ за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеСО", 	НСтр("ru = 'Сальдо СО за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеВОСМС", 	НСтр("ru = 'Сальдо взносов ОСМС за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеООСМС", 	НСтр("ru = 'Сальдо отчислений ОСМС за '"));
	Запрос.УстановитьПараметр("СтрокаСодержаниеОПВР", 	НСтр("ru = 'Сальдо ОПВР за '"));
...
Функция ТекстЗапросаПроводкиБУ(НомераТаблиц, Реквизиты, ПараметрыПроведения)
	...
	Иначе
		...
		|	ЗНАЧЕНИЕ(ПланСчетов.Типовой.Вспомогательный),
		|	ЗНАЧЕНИЕ(ПланСчетов.Типовой.ОбязательстваПоПенсионнымОтчислениям),
		|	ЗНАЧЕНИЕ(Справочник.НалогиСборыОтчисления.ОбязательныеПенсионныеВзносыРаботодателя) КАК СубконтоКт1,
		|	ОПВР.ВидПлатежа КАК СубконтоКт2,
		|	ОПВР.ФизЛицо КАК СубконтоКт3,
		|	ЗНАЧЕНИЕ(ПланСчетов.Налоговый.Вспомогательный) КАК СчетДтНУ,
		|	ЗНАЧЕНИЕ(ПланСчетов.Налоговый.ПрочиеНалоги) КАК СчетКтНУПоУмолчанию,
...
Процедура СформироватьПроводкиБУ_НУ(Реквизиты, ТаблицаПроводок, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаПроводки Из ТаблицаПроводок Цикл 
		...
		Содержание 						= СтрокаПроводки.Содержание + Формат(СтрокаПроводки.ПериодРегистрации, "ДФ='МММ гг'");
