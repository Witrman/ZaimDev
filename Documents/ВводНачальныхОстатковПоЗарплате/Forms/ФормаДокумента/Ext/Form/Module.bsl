...
Процедура ВзносыЗаполнитьПоРаботникам(Команда)
	...
	ПередЗаполнениемТабличныйЧастей("ПоРаботникам", "ВзносыИОтчисления", "заполнить");
...
Процедура ВзносыЗаполнитьПоФизЛицам(Команда)
	...
	ПередЗаполнениемТабличныйЧастей("ПоФизЛицам", "ВзносыИОтчисления", "заполнить");
...
Процедура ЗарплатаЗаполнитьПоРаботникам(Команда)
	...
	ПередЗаполнениемТабличныйЧастей("ПоРаботникам", "ЗарплатаИНалоги", "заполнить");
...
Процедура ЗарплатаЗаполнитьПоФизЛицам(Команда)
	...
	ПередЗаполнениемТабличныйЧастей("ПоФизЛицам", "ЗарплатаИНалоги", "заполнить");
...
Процедура ОПВЗаполнитьПоРаботникам(Команда)
	...
	ПередЗаполнениемТабличныйЧастей("ПоРаботникам", "ОПВПодлежитПеречислению", "заполнить");
...
Процедура ОПВЗаполнитьПоФизЛицам(Команда)
	...
	ПередЗаполнениемТабличныйЧастей("ПоФизЛицам", "ОПВПодлежитПеречислению", "заполнить");
...
&НаКлиенте
Процедура ПередЗаполнениемТабличныйЧастей(СпособЗаполнения, ИмяТабличнойЧасти, Действие = "рассчитать")

	ДопПараметры = Новый Структура("СпособЗаполнения, ИмяТабличнойЧасти", СпособЗаполнения, ИмяТабличнойЧасти);

	Если Объект.Проведен Тогда
		
		ТекстВопроса = НСтр("ru='Автоматически %1 документ можно только после отмены его проведения. Выполнить отмену проведения документа?'");
	...
	ИначеЕсли Модифицированность ИЛИ Объект.Ссылка.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru='Автоматически %1 документ можно только после его записи. Записать?'");
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Действие);
		Режим = РежимДиалогаВопрос.ДаНет;
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаИзменениеВидаОперацииОчиститьТабЧасти", ЭтотОбъект, ДопПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, Режим, 0);
...
КонецПроцедуры // ПередЗаполнениемТабличныйЧастей()
