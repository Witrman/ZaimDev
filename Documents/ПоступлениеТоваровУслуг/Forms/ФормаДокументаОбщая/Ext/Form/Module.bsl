...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	ТекстВопроса = "";
	Если КоличествоОС > 0 ИЛИ КоличествоУслуги > 0 Тогда
		Если  Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку") Тогда
			Если КоличествоОС > 0 И КоличествоУслуги > 0 Тогда
				ТекстВопроса = НСтр("ru = 'После смены операции при записи документа будут очищены табличные части ""ОС"" и ""Услуги"", Продолжить?'");
			ИначеЕсли  КоличествоОС > 0 Тогда
				ТекстВопроса = НСтр("ru = 'После смены операции при записи документа будет очищена табличная часть ""ОС"". Продолжить?'");				
			ИначеЕсли  КоличествоУслуги > 0 Тогда
				ТекстВопроса = НСтр("ru = 'После смены операции при записи документа будет очищена табличная часть ""Услуги"". Продолжить?'");
			КонецЕсли;				
		ИначеЕсли  Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.Импорт") 
			И КоличествоУслуги > 0 Тогда
			ТекстВопроса = НСтр("ru = 'После смены операции при записи документа будет очищена табличная часть ""Услуги"". Продолжить?'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ТекстВопроса <> "" Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаИзменениеВидаОперацииОчиститьТабЧасти", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ПодготовитьФормуНаСервере() Экспорт
	...
	Если НЕ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Импорт Тогда
...
Процедура ОбновитьИтоги(Форма) Экспорт
	...
	УчитыватьОС     = Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку");
	УчитыватьУслуги = Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку")
	                  И Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.Импорт");
...
&НаСервере
Процедура ВидОперацииПриИзмененииНаСервере() Экспорт
...
Процедура ВидОперацииОбработатьИзменение()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку Тогда
		...
		// Очистим реквизиты в табличных частях.
		ПустойИП = Справочники.НомераГТД.ПустаяСсылка();
		Для каждого Строка Из Объект.Товары Цикл
			Строка.НомерГТД     = ПустойИП;
		КонецЦикла;
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Импорт Тогда			
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) И НЕ ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		СтруктураПараметровДляПолученияДоговора = Новый Структура;
		СтруктураПараметровДляПолученияДоговора.Вставить("СписокДопустимыхВидовДоговоров",СтруктураДоступныхВидовДоговоров(Объект.ВидОперации).СписокДопустимыхВидовДоговоров); 
		СтруктураПараметровДляПолученияДоговора.Вставить("УчетАгентскогоНДС",Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПоступлениеОтНерезидента);
		...
		Если Объект.ДоговорКонтрагента <> ТекущийДоговорКонтрагента Тогда
			ДоговорКонтрагентаОбработатьИзменение(Новый Структура, ПричиныИзмененияСчетовУчета);
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОтСтруктурногоПодразделения
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Импорт 
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПоступлениеОтНерезидента Тогда
		...
		Если Объект.УчитыватьНДС Тогда
			...
			ПараметрыОбъекта = Новый Структура("Организация, Дата, Ссылка, ТипЦен, УчитыватьНДС, СуммаВключаетНДС,
				|ВалютаДокумента, КурсВзаиморасчетов, КратностьВзаиморасчетов, Склад, ДоговорКонтрагента");
			ЗаполнитьЗначенияСвойств(ПараметрыОбъекта, Объект);
			...
			Для Каждого СтрокаТабличнойЧасти Из Объект.ОС Цикл
				СтрокаТабличнойЧасти.Сумма = ОбработкаТабличныхЧастейКлиентСервер.ПересчитатьЦенуПриИзмененииФлаговНалогов(
	...
	ПоступлениеТоваровУслугФормы.ЗаполнитьСчетаУчета(ЭтаФорма, ПричиныИзмененияСчетовУчета, "ВидОперации");
...
Процедура ЗаполнитьТабличнуюЧастьИзФайла(АдресЗагруженныхДанных, ИмяТЧ)
	...
	ТекстСообщения = СтрШаблон("В табличную часть %1 добавлено %2: %3, исключено %4: %5",  ИмяТЧ, ?(ИмяТЧ="Товары", "товаров", "услуг"), ЗагруженныеДанные.Количество(), ?(ИмяТЧ="Товары", "услуг", "товаров"), ИсключаемыеДанные.Количество());
...
Процедура ЗагрузитьИзФайлаУслуги(Команда)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "ПоступлениеТоваровУслуг.Услуги";
