...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("СодержаниеТовары",     НСтр("ru = 'Доп. расходы при поступлении товаров'"));
	Запрос.УстановитьПараметр("СодержаниеОС",         НСтр("ru = 'Доп. расходы при поступлении ОС'"));
	Запрос.УстановитьПараметр("СодержаниеНМА",        НСтр("ru = 'Доп. расходы при поступлении НМА'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ДопРасходыНакладная") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"ДопРасходыНакладная",
			НСтр("ru = 'Доп. расходы'"),
			ПечатьДопРасходов(МассивОбъектов, ОбъектыПечати),
			,
...
//
Функция ПечатьДопРасходов(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(Шапка.Ссылка, "Доп. расходы");
		...
		ОбластьМакета.Параметры.ИтоговаяСтрока ="Всего наименований " + 1 + ", на сумму " + ОбщегоНазначенияБКВызовСервера.ФорматСумм(СуммаКПрописи, Шапка.ВалютаДокумента);
		ОбластьМакета.Параметры.СуммаПрописью  = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(СуммаКПрописи, Шапка.ВалютаДокумента);
...
Функция НайтиСоздатьНоменклатуруПоУмолчанию(ГруппаПоУмолчанию)
	...
	Содержание = НСтр("ru = 'Услуга сторонней организации (создано при переходе с редакции 2.0)'");
...
Функция НайтиСоздатьГруппуПоУмолчанию()
	...
	Содержание = НСтр("ru = 'Услуги (переход с редакции 2.0)'");
