...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//БМ_ИСМПТ НА_РАЗВИТИЕ	Товарные группы при формировании заказа КМ в СУЗ
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.КонтактноеЛицо) Тогда
			Объект.КонтактноеЛицо = ИнтеграцияИСМПТКПереопределяемый.ПолучитьКонтактноеЛицо(Объект.Ответственный);
		КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьНанесениеКодовМаркировки() 
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	УправлениеФормой();
	ИнтеграцияИСМПТККлиент.ОповеститьФормы(ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьЗаказНаЭмиссию());
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ИсточникВыбора) = Тип("ФормаКлиентскогоПриложения") 
		И ИсточникВыбора.ИмяФормы = ИнтеграцияИСМПТККлиентПереопределяемый.ПолучитьИмяФормыПодбораНоменклатуры() Тогда
			...
			Если ИзменитьДокументОснование Тогда
				...
				Если Структура_СвязанныйЗаказ = Неопределено Тогда
					...
					Если (Не ЗначениеЗаполнено(Объект.OrderID)
						...
						ТекстВопроса = НСтр("ru = 'Перезаполнить Заказ по выбранному основанию?'");
				...
				Иначе
					...
					ТекстВопросаВыборОснования = НСтр("ru = 'Документ ""%ДокументОснование%"" уже связан с Заказом кодов маркировки. Привязать текущий Заказ к выбранному документу без отсоединения ранее связанного Заказа?'");
...
Функция ПроверитьСвязанныйЗаказ(ВыбранноеЗначение)
	...
	Возврат ИнтеграцияИСМПТК.ПроверитьНаличиеСвязанногоЗаказаКодов(ВыбранноеЗначение);
...
Процедура ТоварыУпаковкаПриИзменении(Элемент)
	...
	Если Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Табачная") Тогда
		...
		Если Не ЗаполненаБазоваяЕИ Тогда
			...
			УпаковкаНеШтучная = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.УпаковкаНеШтучная(ТекущиеДанные.Номенклатура, ТекущиеДанные.Упаковка);
		КонецЕсли;
...
Процедура ВыполнитьПересчетКоличества()
	...
	Иначе 
		...
		Если ТекущиеДанные.Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.БлокТабачныхПачек") Тогда
			ТекущиеДанные.Количество = ТекущиеДанные.КоличествоУпаковок;
		Иначе
			ТекущиеДанные.Количество = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.ПересчитатьКоличествоУпаковокВКоличество(ТекущиеДанные.КоличествоУпаковок, ТекущиеДанные.Упаковка, ТекущиеДанные.Номенклатура);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПредставлениеДокументНажатие(Элемент, СтандартнаяОбработка)
...
Процедура НанесениеКМНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДокументИСМПТ) = Тип("СписокЗначений") И ЗначениеЗаполнено(ДокументИСМПТ) Тогда
		...
		Если ДокументИСМПТ.Количество() > 1 Тогда
			СтандартнаяОбработка = Ложь;
			ИнтеграцияИСМПТККлиент.ОткрытьФормуПросмотраСпискаДокументов(ЭтаФорма, ЭтаФорма.НанесениеКМ);
		КонецЕсли;
...
&НаКлиенте
Процедура ПодобратьИзКаталога(Команда)
	
	Если Не ЗначениеЗаполнено(Объект.ВидПродукции) Тогда
		
		ТекстСообщения = НСтр( "ru = 'Необходимо заполнить Вид продукции на вкладке Основное!'");
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	СтруктураОтбора.Вставить("СтатусКарточки", ПредопределенноеЗначение("Перечисление.СтатусыКарточекНацКаталогаИСМПТК.Опубликована"));
	...
	ПараметрыОткрытия.Вставить("Заголовок", 	  "Выбор товара из карточек Национального каталога");
	ПараметрыОткрытия.Вставить("ЭтоВыборКарточки", Истина);
	
	ОткрытьФорму("РегистрСведений.ТоварыНацКаталогаИСМПТК.ФормаСписка", ПараметрыОткрытия, ЭтотОбъект,,,, Новый ОписаниеОповещения("ПодобратьИзКаталогаЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляЗаказаКМ();
	...
	Если ДокументыВыбора.Количество() > 1 Тогда
		...
		ОткрытьФорму("Обработка.ОбменИСМПТК.Форма.ФормаВыбораДокументаОснования", ПараметрыФормыВыбора,,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОткрытьПодбор(Команда)
	...
	ОчиститьСообщения();
	ИнтеграцияИСМПТККлиентПереопределяемый.ПодобратьНоменклатуруВЗаказ(ЭтаФорма, УникальныйИдентификатор);
...
Процедура ОбработкаПодбораТоваровИзПодбора(АдресТоваровВХранилище)
	...
	ИнтеграцияИСМПТКПереопределяемый.ЗаполнитьЗаказИзПодбора(Объект.Товары, Объект.ВидПродукции, АдресТоваровВХранилище);
...
Процедура УстановитьУсловноеОформление()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПолучитьКМПовторно", "Доступность", ЕстьОшибкиПолученияКМ());
...
Процедура УправлениеФормой()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗаполнитьПоGTIN", 	  	  "Видимость", ОтображатьДанныеПоНоменклатуре);
	...
	ОтразитьГруппуИдентификаторовПроизводства = Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.ЛекарственныеПрепараты") 
												ИЛИ Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Табачная");
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаЛекарственныеТовары","Видимость", ОтразитьГруппуИдентификаторовПроизводства);
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗаполнитьПоGTIN", "Доступность",    Не РазрешенТолькоПросмотр);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОткрытьПодбор", 	 "Доступность",    Не РазрешенТолькоПросмотр);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПечать", 	 "Видимость", 	   ДоступностьПечать);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПолучитьКМПовторно", 		"Видимость", ДоступностьПовторногоПолучения);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПолучениеКодовМаркировки", "Видимость", ДоступностьГруппыПолучения);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументЗаказКодовМаркировкиСУЗИСМПТКПроверить", "Доступность", Не РазрешенТолькоПросмотр);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументЗаказКодовМаркировкиСУЗИСМПТКРазбитьНаНесколькоДокументов", "Видимость", Не РазрешенТолькоПросмотр);
	//////////
	...
	Если Не Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Обувная") 
		...
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОформитьНанесениеКМ", 	"Видимость", Не ЗначениеЗаполнено(НанесениеКМ));
		//Ссылка с предупреждением о не оформленном отчете по нанесению
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНанесениеНеОформлено", "Видимость", Не ЗначениеЗаполнено(НанесениеКМ));		
		//Ссылка на оформленный документ Нанесения 
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНанесениеОформлено", 	"Видимость", ЗначениеЗаполнено(НанесениеКМ));
		...
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПолучениеКодовМаркировки", "Видимость", Не КодыПолученыУспешно);
		
	ИначеЕсли Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Обувная") 
		И ДанныеИзПула.КоличествоВПуле = Объект.Товары.Итог("Количество") Тогда
		...
		//Ссылка с предупреждением о не оформленном отчете по нанесению
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНанесениеНеОформлено", "Видимость", Ложь);		
		//Ссылка на оформленный документ Нанесения 
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНанесениеОформлено", 	"Видимость", Истина);
		Элементы.ДекорацияОформленоУведомление.Заголовок = НСтр("ru='Выполнено автоматическое нансение кодов маркировки в СУЗ.'", "ru");
		
	Иначе
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОформитьНанесениеКМ",   "Видимость", Ложь);
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНанесениеНеОформлено", "Видимость", Ложь);
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНанесениеОформлено", 	"Видимость", Ложь);
	КонецЕсли;
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НанесениеКМ", "Видимость", ЗначениеЗаполнено(НанесениеКМ));
	//////////
	
	//Причина отклонения
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПричинаОтклонения", "Видимость", ЗначениеЗаполнено(Объект.ПричинаОтклонения));
	
	ВидимостьДокументаОснования = ИнтеграцияИСМПТКПереопределяемый.ПолучитьВидимостьСсылкиДокументОснованиеВЗаказе();
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПредставлениеДокумент", "Видимость", ВидимостьДокументаОснования);	 
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаГруппаОтражение",  "Видимость", ВидимостьДокументаОснования);	 
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КонтактноеЛицо", 	   "Видимость", Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Обувная"));
	
	ВидимостьПодбораНоменклатуры = ИнтеграцияИСМПТКПереопределяемый.ПолучитьВидимостьПодбораНоменклатурыВЗаказе();
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОткрытьПодбор", "Видимость", ВидимостьПодбораНоменклатуры);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыШаблон",  "ТолькоПросмотр", Не Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Табачная"));
	СформироватьСписокВыбораШаблона();
...
Процедура СформироватьПредставлениеДокументаОснования(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Объект.ДокументОснование) Тогда
		Форма.ПредставлениеДокумент = Форма.Объект.ДокументОснование;
		Форма.Элементы.ПредставлениеДокумент.ЦветТекста = ИнтеграцияИСМПТККлиентСервер.ЦветСиний();
	Иначе
		Форма.ПредставлениеДокумент = НСтр("ru = 'Документ не отражен в учете'");
		Форма.Элементы.ПредставлениеДокумент.ЦветТекста = ИнтеграцияИСМПТККлиентСервер.ЦветКрасный();
	КонецЕсли;
...
Функция ЗаполнитьGTINПоНоменклатуре(Номенклатура, Упаковка, Характеристика, ХарактеристикиИспользуются, ЭтоТабак = Ложь)
	...
	КонецЕсли;
	ДанныеШтрихкодов = ИнтеграцияИСМПТКПереопределяемый.ПодобратьGTINДляЗаказа(Номенклатура, Упаковка, Характеристика, ХарактеристикиИспользуются, ЭтоТабак);
...
Процедура ЗаполнитьПоGTINНаСервере()
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.GTIN) Тогда
			...
			Штрихкод = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ШтрихкодEANИзGTIN(СтрокаТЧ.GTIN);
	...
	Если ДанныеПоНоменклатуре.Количество() <> 0 Тогда
		...
		Для Каждого Строка Из ДанныеПоНоменклатуре Цикл
			...
			УпаковкаНеШтучная = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйПереопределяемый.УпаковкаНеШтучная(Строка.Номенклатура, Строка.Упаковка);
...
Процедура УстановитьВидимостьКнопокИзмененияСтатуса()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументЗаказКодовМаркировкиСУЗИСМПТКПроверить", "Видимость", НеОтправлен);
	
	Если ЗначениеЗаполнено(Объект.ПричинаОтклонения) Тогда
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументЗаказКодовМаркировкиСУЗИСМПТКПолучитьКМ", "Видимость", Ложь); //При ошибке Получать КМ нельзя
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПорционноеПолучениеКодовМаркировки", 			  "Видимость", Ложь);
	Иначе
		...
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПорционноеПолучениеКодовМаркировки", "Видимость", ЭтоОбувь И КодыГотовы);
		
		ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСохранениеВФайл", 		 "Видимость", ЭтоОбувь И КодыПолучены);
	КонецЕсли;
...
Процедура УстановитьДоступностьКомандыВводаСерий()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыУказатьСерийныеНомера",  "Доступность", ФлагДоступности);
	...
	Если Не ВыбранныеФайлы = Неопределено Тогда
		...
		Если Не СписокКодов = Неопределено Тогда
			...
			Для Каждого СтрокаТЧ Из СписокКодов Цикл
				...
				КодДляЗаписи = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.Base64ВШтрихкод(СтрокаТЧ.ПолныйКодМаркировки);
...
&НаКлиенте
Процедура ПодобратьИзКаталогаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из Результат Цикл
		...
		НоваяСтрокаТовары.СпособФормированияСерийногоНомера = ПредопределенноеЗначение("Перечисление.СпособыФормированияСерийногоНомераСУЗИСМПТК.Автоматически");
		
		//БМ_ИСМПТ НА_РАЗВИТИЕ	Товарные группы
		Если Элемент.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.МолочнаяПродукция") Тогда
			НоваяСтрокаТовары.Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.МолочнаяПродукция");
			
		ИначеЕсли Элемент.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Обувная") Тогда
			НоваяСтрокаТовары.Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.Обувь");
			
		ИначеЕсли Элемент.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.ЛекарственныеПрепараты") Тогда
			НоваяСтрокаТовары.Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.ЛекарственныеПрепараты");
			
		ИначеЕсли Элемент.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.ЛегкаяПромышленность") Тогда
			НоваяСтрокаТовары.Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.ЛегкаяПромышленность");
			
		ИначеЕсли Элемент.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.Табачная") Тогда
			НоваяСтрокаТовары.Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗИСМПТК.ПустаяСсылка"); //Значение по умолчанию
		КонецЕсли;
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат ИнтеграцияИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияЗаказаКМНаСервере(Объект.ДокументОснование);
...
Процедура ВыборДокументаОснованияЗавершение(ВыбранныйДокумент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйДокумент <> "" И ТипЗнч(ВыбранныйДокумент) <> Тип("Неопределено") Тогда
		...
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.ДополнитьЗначенияОтбораПриВыбореДокументаОснованияДляЗаказаКМ(СтруктураОтбора, ИмяДокумента);
...
Процедура ОформитьНанесениеКМ(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ЭтаФорма.Параметры.Ключ) ИЛИ ЭтаФорма.Модифицированность Тогда
		ТекстВопроса = НСтр("ru = 'Отчет о нанесении нельзя вводить на основании не записанного документа. Записать Заказ?'");
	КонецЕсли;
	...
	Иначе
		...
		Если ЕстьКодыТребующиеНанесения Тогда
			...
			Иначе
				...
				ИнтеграцияИСМПТККлиент.ОповеститьФормы(ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьНанесениеКодовМаркировки(), Объект.Ссылка);
...
Функция ПроверитьНаличиеКодовВПуле() 
	 ...
	 Возврат ИнтеграцияИСМПТК.ПроверитьНаличиеКодовВПуле(Объект.Ссылка);
...
Функция СформироватьГруппуНанесений() 
	  ...
	  Возврат ИнтеграцияИСМПТК.СформироватьГруппуНанесений(Объект.Ссылка);
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ЗначениеЗаполнено(Объект.ВидПродукции) И ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		
		ВидНоменклатуры = ИнтеграцияИСМПТКПереопределяемый.ПолучитьВидПродукцииПоНоменклатуре(ТекущиеДанные.Номенклатура);
		Если Объект.ВидПродукции <> ВидНоменклатуры Тогда
			...
			ТекстСообщения = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ТекущиеДанные.Номенклатура, ТекущиеДанные.НомерСтроки);
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения, , , "Форма.Объект");
