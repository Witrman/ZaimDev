...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если  ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратЗаработнойПлаты
		  ...
		  ИЛИ ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратЕдиногоПлатежа Тогда
		  ...
		  Если ВалютаДокумента <> ВалютаРегламентированногоУчета Тогда
			  Если ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратЗаработнойПлаты Тогда
				  НачалоТекстСообщения = НСтр("ru = 'Возврат заработной платы'");
			  ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратПенсионныхВзносов Тогда
				  НачалоТекстСообщения = НСтр("ru = 'Возврат пенсионных взносов'");			
			  ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратЕдиногоПлатежа Тогда
				  НачалоТекстСообщения = НСтр("ru = 'Возврат единого платежа'");			
			  Иначе
				  НачалоТекстСообщения = НСтр("ru = 'Возврат взносов и отчислений соц. страхования'");			
			  КонецЕсли;         
			ТекстСообщения = НСтр("ru = ' %1 возможно оформлять только в %2'");

			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, НачалоТекстСообщения, ВалютаРегламентированногоУчета), ЭтотОбъект,,, Отказ);			
		КонецЕсли;
