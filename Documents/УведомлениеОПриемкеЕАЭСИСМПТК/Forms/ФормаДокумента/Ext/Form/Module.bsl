...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ИнтеграцияИСМПТККлиент.ОповеститьФормы(ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОПриемкеЕАЭС(), МассивДокумент);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОПриемкеЕАЭС()
		Или ИмяСобытия = "УведомлениеОПриемкеОтвязать" Тогда		
...
Процедура ЗагрузитьКодыМаркировкиИзФайлаЗавершение(ДанныеДляРазбора) Экспорт
	...
	Для Каждого КодМаркировкиBase64 Из ДанныеДляРазбора Цикл
		...
		Если ДанныеРазбора = Неопределено Тогда 
			Если Не ЗначениеЗаполнено(КодМаркировкиBase64.ШтрихКод) Тогда
				...
				ТекстОшибки     = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, ДанныеДляРазбора.Найти(КодМаркировкиBase64)+1);
			Иначе
				...
				ТекстОшибки     = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, НеКорректныйКод, ДанныеДляРазбора.Найти(КодМаркировкиBase64)+1);
			КонецЕсли;
		...
		Если СтрокаДляПриемки.Количество() = 0 Тогда
			...
			НоваяСтрокаТЧ.ВидПродукцииИС	 = СтруктураКодовМаркировки.ВидПродукцииИС;
			...
			ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, СтруктураКодовМаркировки.КодИдентификации, ДанныеДляРазбора.Найти(КодМаркировкиBase64)+1);
			МассивОшибок.Добавить(ТекстОшибки);
	...
	Для Каждого Ошибка Из МассивОшибок Цикл
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(Ошибка);
	КонецЦикла;
...
Процедура УправлениеФормой()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Ошибки", "Видимость", Не Объект.Ошибки.Количество() = 0);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "МаркиНеучтенные", "Видимость", Не Объект.МаркиНеучтенные.Количество() = 0);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РежимМассовойПриемки", "Доступность", НеОтправлен);
	
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандыЗагрузкиТЧ",   "Доступность", НеОтправлен); 
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "МаркиГруппаЭкспорт1", "Видимость",   Не Объект.МаркиНеучтенные.Количество() = 0); 
		
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОПриемкеЕАЭСИСМПТКОтправить", "Доступность", НеОтправлен);
...
Процедура ОбработатьКодМаркировки(ИсходныеДанные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не НайденныеСтрокиВТЧ = Неопределено Тогда 
		...
		Если НайденныеСтрокиВТЧ[0].СтатусПриемки Тогда
			...
			ТекстВопроса = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстВопроса, СтруктураКодовМаркировки.КодИдентификации);
		...
		Иначе
			...
			ТекстСообщения = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, СтруктураКодовМаркировки.КодИдентификации);
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;			
	Иначе
	...
	Если РезультатЗадания.Успешно И НЕ РезультатЗадания.ТаблицаПолученныхКодов.Количество() = 0 Тогда
		Для Каждого Строка Из РезультатЗадания.ТаблицаПолученныхКодов Цикл
			...
			Если Не НайденныеСтроки.Количество() = 0 Тогда
				...
				ТекстОшибки = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.КодИдентификации);
				МассивОшибок.Добавить(ТекстОшибки);
			...
			Иначе
				...
				НоваяСтрокаТЧ.ВидПродукцииИС	 = Строка.ВидПродукцииИС;
