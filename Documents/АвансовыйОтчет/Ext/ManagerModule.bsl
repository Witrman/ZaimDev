...
Функция ТекстЗапросаПогашениеЗадолженности(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если  Реквизиты.ЕстьОплатаПоставщикам  Тогда
		...
		|	ЕСТЬNULL(ТаблицаОплатаПоставщикам.Содержание, ""Оплата"") КАК Содержание
		|ИЗ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрНаПеречислениеОПВ") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьРеестра(МассивОбъектов, ОбъектыПечати, "ПеречислениеПенсионныхВзносов"),
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрНаПеречислениеСО") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьРеестра(МассивОбъектов, ОбъектыПечати, "ПеречислениеСоциальныхОтчислений"),
			,
			"Документ.АвансовыйОтчет.ПФ_MXL_Реестр");
	КонецЕсли;
...
Функция ПечатьРеестра(МассивОбъектов, ОбъектыПечати, ВидРеестра) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.АвансовыйОтчет.ПФ_MXL_Реестр");
		...
		Пока ВыборкаПоВидуВзносов.Следующий() Цикл
			...
			Если ВидРеестра = "ПеречислениеПенсионныхВзносов" Тогда
				...
				ИначеЕсли ВыборкаПоВидуВзносов.ВидОперации = Перечисления.ВидыОперацийОПВПеречислениеВФонды.ПеречислениеОбязательныхПенсионныхВзносовРаботодателя Тогда
					ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление обязательных взносов работодателя'");
				Иначе
			...
			ИначеЕсли ВидРеестра = "ПеречислениеСоциальныхОтчислений" Тогда
				Если ВыборкаПоВидуВзносов.ВидОперации = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОбязательныхСоциальныхОтчислений Тогда
					ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление обязательных социальных отчислений'");
...
Функция ПечатьТ3(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("МассивДокументов", Шапка.ВыданныеАвансы.Выгрузить().ВыгрузитьКолонку("Документ"));
		...
		ОбластьТитул1.Параметры.ОрганизацияРНН_БИН = ОбщегоНазначенияБКВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "БИН_ИИН,", Ложь, Шапка.ДатаДокумента, "ru");
		...
		|	""Пенсионные взносы перечисление в фонды"",
		...
		|	АвансовыйОтчет.Документ.Номер,
		|	АвансовыйОтчет.Документ.Дата,
		|	""Социальное страхование перечисление в фонды"",
		|	СУММА(АвансовыйОтчет.Сумма),
		|	0,
		|	0,
		|	&СчетСО,
		|	НЕОПРЕДЕЛЕНО,
		|	"""",
		|	""Перечисление взносов и отчислений соц. страхования""
		|ИЗ
