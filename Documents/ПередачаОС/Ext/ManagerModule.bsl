...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'ДА-1 (Акт премки-передачи)'");
	КомандаПечати.Обработчик    = "УправлениеПечатьюБККлиент.ВыполнитьКомандуПечати";
	КомандаПечати.ПроверкаПроведенияПередПечатью = НЕ ПользователиБКВызовСервераПовтИсп.РазрешитьПечатьНепроведенныхДокументов();
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПечатьДА1") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"ПечатьДА1",
			НСтр("ru = 'ДА-1 (Акт премки-передачи)'"),
...
Функция ПечатьНакладная(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ШапкаДокумента.Следующий() Цикл
		...
		ТекстРасходнаяНакладная = НСтр("ru = 'Расходная накладная'");
		ОбластьМакетаЗаголовок.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ШапкаДокумента.Ссылка, ТекстРасходнаяНакладная);
		...
		Если ШапкаДокумента.УчитыватьНДС Тогда
			ОбластьМакетаИтогоНДС.Параметры.НДС = ?(ШапкаДокумента.СуммаВключаетНДС, НСтр("ru = 'В том числе НДС:'"), НСтр("ru = 'Сумма НДС:'"));
		...
		ОбластьМакетаСуммаПрописью.Параметры.ИтоговаяСтрока = НСтр("ru = 'Всего наименований '") + ВыборкаСтрокОС.Количество()
															+ НСтр("ru = ', на сумму '") + ОбщегоНазначенияБКВызовСервера.ФорматСумм(СуммаКПрописи, ШапкаДокумента.ВалютаДокумента);
...
Процедура ПрисоединитьСекцию(Макет, ТабДокумент, ВыводитьПринял, ВыводитьДатаПринял, ВыводитьПодвалВерх, ВыводитьПодвалНиз, ВыведенПодвал, ШапкаДокумента)
	...
	ИначеЕсли ВыводитьПодвалВерх Тогда
		...
		ДанныеДоверенности = Новый Структура("ДоверенностьНомер,ДоверенностьДата,УдалитьДоверенность,ДоверенностьЛицо,ДоверенностьВыдана");
		ЗаполнитьЗначенияСвойств(ДанныеДоверенности,ШапкаДокумента);
		ОбщегоНазначенияБК.ЗаполнитьДанныеДоверенностиВПечатнойФорме(ДанныеДоверенности, ТекущаяСекция,,,"ДоверенностьЛицоВыдана", Истина);	
	...
	Пока ШапкаДокумента.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ШапкаДокумента.ДокументОснованиеВид) Тогда
			Если ШапкаДокумента.ДокументОснованиеВид = "приказа" Тогда
				Приказ.Параметры.ДокументОснованиеВид = НСтр("ru = 'приказа'");
			ИначеЕсли ШапкаДокумента.ДокументОснованиеВид = "постановления" Тогда
				Приказ.Параметры.ДокументОснованиеВид = НСтр("ru = 'постановления'");
			ИначеЕсли ШапкаДокумента.ДокументОснованиеВид = "распоряжения" Тогда
				Приказ.Параметры.ДокументОснованиеВид = НСтр("ru = 'распоряжения'");
			КонецЕсли;	
		Иначе
		...
		Для Каждого СтрокаМОЛ Из ДанныеМОЛ Цикл
			...
			ОССдал.Параметры.ОбъектОсновныхСредств = ?(ВыводитьОС, НСтр("ru = 'Долгосрочный актив'"), "");
			ОССдал.Параметры.Сдал				   = ?(ВыводитьОС, НСтр("ru = 'Сдал'"), "");
			...
			ПрисоединитьСекцию(Макет, ТабДокумент, ВыводитьПринял, ВыводитьДатаПринял, ВыводитьПодвалВерх, ВыводитьПодвалНиз, ВыведенПодвал, ШапкаДокумента);
...
Процедура ДобавитьКолонкуСодержание(ТаблицаЗначений, ТекстСодержания = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекстСодержания) Тогда 
		ТекстСодержания = НСтр("ru='Выручка от реализации ОС'");
	КонецЕсли;
