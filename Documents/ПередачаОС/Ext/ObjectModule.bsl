...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ОбщегоНазначенияБКВызовСервера.ДобавитьКолонкуВТаблицуЗначений(ТаблицыРеализация.СобственныеТоварыУслуги, "НомерЖурнала", НСтр("ru = 'ОС'"));   	
	...
	Если ПараметрыПроведения.Реквизиты[0].ВидОперации = Перечисления.ВидыОперацийПередачиОС.ПередачаСтруктурномуПодразделению Тогда 
		Документы.ПередачаОС.ДобавитьКолонкуСодержание(ТаблицыРеализация.СобственныеТоварыУслуги, НСтр("ru='Положит. отклонение ст-ти реал-ции от себест-ти'"));
...
Процедура ПроверкаЗАполненияТабличнойЧастиОС(Отказ, ЭтотОбъект)
	...
	Для каждого СтрокаТабличнойЧасти Из ЭтотОбъект.ОС Цикл
		...
		Если ЭтотОбъект.УчитыватьНДС 
			...
			СтрокаТабличнойЧасти.НомерСтроки, "ОС");
