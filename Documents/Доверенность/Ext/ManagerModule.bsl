		...
		Если НЕ ВыборкаСтрокТовары = Неопределено Тогда
			...
			Пока ВыборкаСтрокТовары.Следующий() Цикл
				...
				Если ВыборкаСтрокТовары.Количество <> 0 Тогда
					...
					СокрЛП(ОбщегоНазначенияБКВызовСервера.КоличествоПрописью(ВыборкаСтрокТовары.Количество)) + ")";
...
Функция ПечатьЖурналДоверенностей(МассивОбъектов, ОбъектыПечати, ПараметрыПечати = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ОтборПоОрганизации) Тогда
		...
		ДанныеДляЗаполнения.Вставить("ТекстРНН_БИН",             НСтр("ru = 'ИИН/БИН'; kz = 'ЖСН/БСН'"));
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		ДанныеДляЗаполнения.Вставить("ДатаДокумента",      Формат(ВыборкаДокументов.Дата,"ДФ=dd.MM.yyyy"));
		ДанныеДляЗаполнения.Вставить("СрокДействия",       Формат(ВыборкаДокументов.ДатаДействия, "ДФ=dd.MM.yyyy"));
