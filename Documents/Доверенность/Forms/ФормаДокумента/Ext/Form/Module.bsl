...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийДоговор, "ВедениеВзаиморасчетов");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ПоступлениеТоваровУслуг" 
		...
		Если ТипЗнч(Параметр) = Тип("Структура") И Параметр.Свойство("ДокументОснование")
			...
			НадписьДокументИсполнения = "Документ исполнения: " + Источник;
...
Процедура СделкаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ПоДокументу) Тогда
		...
		ТекстВопроса = НСтр("ru = '%1 дополнительный реквизит ""По документу""?'");
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, 
							?(НЕ ЗначениеЗаполнено(Объект.Сделка), "Очистить", "Изменить"));
...
Процедура Подбор(Команда)
	...
	ИначеЕсли Команда.Имя = "ПодборОС" Тогда
		ПараметрыПодбора = ПолучитьПараметрыПодбора("ОС");
		Если ПараметрыПодбора <> Неопределено Тогда
	...
	ИначеЕсли Команда.Имя = "ПодборНМА" Тогда
		ПараметрыПодбора = ПолучитьПараметрыПодбора("НМА");
		Если ПараметрыПодбора <> Неопределено Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ЗначениеЗаполнено(ДокументИсполнения) Тогда
		НадписьДокументИсполнения = "Документ исполнения: " + ДокументИсполнения;
	Иначе
...
Процедура ПриИзмененииЗначенияОрганизацииСервер(СтруктураПараметров, СтруктураРезультатаВыполнения)
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) 
		...
		Объект.СтруктурнаяЕдиница = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурноеПодразделениеОрганизация, "ОсновнойБанковскийСчет");
	КонецЕсли;
...
Процедура СделкаПриИзмененииНаСервере()
	...
	Если НЕ ТекущаяСделка = Объект.Сделка Тогда
		...
		ПредставлениеДокумента = ?(НЕ ЗначениеЗаполнено(Объект.Сделка), "", РаботаСДиалогами.СформироватьЗаголовокДокумента(Объект.Сделка, СокрЛП(Объект.Сделка.Метаданные().Представление())) + " г.");
