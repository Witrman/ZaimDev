...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОВвозеИзТретьихСтран() Тогда		
...
Процедура ВыборДокументаОснованияЗавершение(ВыбранныйДокумент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйДокумент <> "" И ТипЗнч(ВыбранныйДокумент) <> Тип("Неопределено") Тогда
		...
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.ДополнитьЗначенияОтбораПриВыбореДокументаОснованияДляВвозаТоваров(СтруктураОтбора, ИмяДокумента, Объект.Направление, "ВвозИзТретьихСтран");
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если Не ТекДанные = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ТекДанные.ТНВЭД)
			...
			ТекДанные.ТНВЭД = ИнтеграцияИСМПТКПереопределяемый.ПолучитьКодТНВЭДПоНоменклатуре(ТекДанные.Номенклатура)
		КонецЕсли;
	КонецЕсли;
...
Процедура РассчитатьИтогиПоМаркам(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Рассчет итогов следует выполнять только после полного завершения редактирования таблицы Марки.'") + Символы.ПС + НСтр("ru = 'При этом будет предварительно очищена таблица Товары. Выполнить рассчет?'");
	Режим = РежимДиалогаВопрос.ДаНет;
...
Процедура УправлениеФормой()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТипДокументаСоответствия",   "ТолькоПросмотр", НЕ НеОтправлен);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НомерДокументаСоответствия", "ТолькоПросмотр", НЕ НеОтправлен);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДатаДокументаСоответствия",  "ТолькоПросмотр", НЕ НеОтправлен);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Товары", "ТолькоПросмотр", НЕ НеОтправлен);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыРассчитатьИтогиПоМаркам", "Доступность", НеОтправлен);
...
&НаСервере
Процедура ДобавитьОбновитьДанныеПоТоварам()
	...
	Для Каждого СтрокаТЧ Из Объект.Марки Цикл
		...
		Иначе
			...
			НоваяСтрока.ВидПродукцииИС = СтрокаТЧ.ВидПродукцииИС;
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат ИнтеграцияИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияВвозТоваровИзТретьихСтранНаСервере(Объект.ДокументОснование);
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	НеОтправлен  = Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСМПТК.НеОпределен");
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзТретьихСтранИСМПТКОтправить", "Доступность", НеОтправлен);
...
Процедура ПослеЗакрытияВопросаЗаписатьШтрихкодНоменклатуры(Результат, Параметры) Экспорт
	...
	Попытка
		...
		ИнтеграцияИСМПТКПереопределяемый.ЗаписатьШтрихкодНоменклатуры(Номенклатура, EAN,, Характеристика);
