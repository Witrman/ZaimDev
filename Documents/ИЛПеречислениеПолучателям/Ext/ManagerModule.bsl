...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СписокНаПеречислениеПоИЛ") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьСписокНаПеречислениеПоИЛ(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ПечатьСписокНаПеречислениеПоИЛ(МассивОбъектов, ОбъектыПечати)
	...
	Макет = ПолучитьМакет("ПФ_MXL_СписокНаПеречислениеПоИЛ");
	...
	Пока ВыборкаПоШапкеДокумента.Следующий() Цикл
		...
		ОбластьШапка.Параметры.ОписаниеПериода 	= "за " + Формат(ВыборкаПоШапкеДокумента.ПериодРегистрации, "ДФ='ММММ гггг'");
		
		ВключатьСборы = Ложь;
		Если ВыборкаПоШапкеДокумента.СпособПеречисления = Перечисления.СпособыПеречисленияПоИсполнительномуЛисту.ЧерезКассу Тогда
			ОбластьШапка.Параметры.НазначениеПлатежа 	= "на выдачу денежных средств";
		Иначе 
			ОбластьШапка.Параметры.НазначениеПлатежа 	= "на перечисление денежных средств";
			ВключатьСборы = Истина;
		КонецЕсли;
		
		ОбластьШапка.Параметры.СпособПеречисления = НРег(ВыборкаПоШапкеДокумента.СпособПеречисления);
		...
		ОбластьШапкаСтрОбщий.Параметры.ТекстРНН_БИН = ?(ВыводитьРНН, "РНН", "БИН / ИИН");
		...
		ОбластьПодписи.Параметры.ИтогоСуммаПлатежаПрописью = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(ИтогоСуммаПлатежа, Константы.ВалютаРегламентированногоУчета.Получить());
