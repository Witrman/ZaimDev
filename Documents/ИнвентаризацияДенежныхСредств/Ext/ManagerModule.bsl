...
Функция ПечатьИНВ6(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока РегистраторыШапка.Следующий() Цикл
		...
		ДокОснованиеДата 										 = Формат(РегистраторыШапка.ДокументОснованиеДата, "ДФ='дд ММММ гггг ''года'''");
		ОбластьМакетаШапка.Параметры.ДокументОснованиеДата       = ?(ЗначениеЗаполнено(ДокОснованиеДата), ДокОснованиеДата, НСтр("ru = '«____»_______________20____года'"));
		...
		ОбластьМакетаШапка.Параметры.ДатаОкончанияИнвентаризации = Формат(РегистраторыШапка.ДатаОкончанияИнвентаризации, "ДФ='дд ММММ гггг ''года'''");
		
		Если РегистраторыШапка.ВидОперации = Перечисления.ВидыОперацийИнвентаризацияДенежныхСредств.ПоКассе Тогда
			ТекстСообщения = НСтр("ru = 'в кассе: ""%1""'");
			...
			Пока ВыборкаСтрокДенежныеСредстваСсылка.Следующий() Цикл
				...
				Если ВыборкаСтрокДенежныеСредстваСсылка.Валюта <> ОбщегоНазначенияБКВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета() Тогда 
					...
					Если НомерПП = 2 Тогда
						ОбластьМакетаСтрокаВВалюте.Параметры.Описание = НСтр("ru = 'Наличные денежные средства и эквиваленты денежных средств в иностранной валюте'");
					Иначе
