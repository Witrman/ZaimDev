...
Процедура СвернутьСтрокиРасчетаОтОбратного(ТЗНачисления,
	...
	Пока Н >= 0 Цикл
		...
		ИначеЕсли ПредыдущийВидРасчета <> Строка.ВидРасчета ИЛИ ПредыдущаяДатаНачала <> Строка.ДатаНачала ИЛИ ПредыдущаяДатаОкончания <> Строка.ДатаОкончания Тогда
			...
			Сообщить("" + Строка.Сотрудник + ": в одном документе нельзя рассчитывать более одного расчета ""от обратного"" для каждого отдельного сотрудника документа!", СтатусСообщения.Важное);
		КонецЕсли;
	...
	Для Н = 0 По СписокСтрокРасчетаОтОбратного.Количество() -1 Цикл
		...
		Если ТекущийРазмер <> 0 И Строка.Размер <> ТекущийРазмер Тогда
			...
			Сообщить("" + Строка.Сотрудник + ": недопустимо изменять размер на руки для расчета ""от обратного"" в течение месяца (с " + Строка(Строка.Размер) + " на " + Строка(ТекущийРазмер) + ")", СтатусСообщения.Важное);
		КонецЕсли;
...
Процедура ПроверитьНаличиеКадровыхИзмененийПоСотрудникам(Объект, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ПричинаИзмененияСостояния = ?(Выборка.ПричинаИзмененияСостояния = ПредопределенноеЗначение("Перечисление.ПричиныИзмененияСостояния.ПриемНаРаботу"), " принят на работу ", " перемещен ");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Сотрудник ""%1"" был %3 позже даты начисления, %2 '"),
		Выборка.Сотрудник,
		Формат(Выборка.Период, "ДФ=dd.MM.yyyy"), ПричинаИзмененияСостояния);
