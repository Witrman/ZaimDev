		...
		Если ЗначениеЗаполнено(КонтрагентДокумента) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДоговорКонтрагентаДокумента) Тогда
				...
				Иначе
					...
					ТекстСообщения  = НСтр("ru = 'Нет договора с поставщиком, соответсвтующего контрагенту, организации и валюте. Будет создан новый договор.'");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ВозвратДенежныхСредствПодотчетником", НСтр("ru = 'Возврат средств от подотчетника'"));
	...
	Пока ДанныеДокументов.Следующий() Цикл
		...
		Если ИмяМакета = "ПКОНДС" Тогда
			СуммаПрописью = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 В т.ч. НДС %2.'"), СуммаПрописью, Формат(ДанныеДокументов.СуммаНДС,"ЧЦ=15; ЧДЦ=2"));
		КонецЕсли;
		...
		ОбластьМакетаШапка.Параметры.ДатаДокумента      = Формат(ДанныеДокументов.ДатаДокумента, "ДФ = 'дд.ММ.гггг'")+" года";
		...
		ОбластьМакетаШапка.Параметры.ФИОГлавногоБухгалтера = ?(НЕ ЗначениеЗаполнено(Бухгалтер), НСтр("ru = 'Не предусмотрен'"), Бухгалтер);
