...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДатаПереходаНаВС = Константы.ДатаПереходаНаУчетПоТоварамОрганизацийНаВиртуальныхСкладах.Получить();
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		Если ВыбранноеЗначение.Свойство("КодСтроки") Тогда
			Если ИсточникВыбора.ИмяМакета = "УсловияПоставки" Тогда
			...
			ИначеЕсли ИсточникВыбора.ИмяМакета = "СпособыОтправления" Тогда
...
Процедура ОбработкаВыбораСчетаФактуры(СчетФактура)
	...
	ИначеЕсли НайденныеДокументыЭДВС.Количество() > 0 Тогда	
		...
		Для Каждого НайденныйДокумент Из НайденныедокументыЭДВС Цикл
			ТекстОшибки = НСтр("ru='На основании учетного документа уже введен %1. Рекомендуем отвязать ЭДВС от документа ""Поступление товаров и услуг"", в противном случае образуются некорректные остатки!'");
...
Процедура ОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Номенклатура")
		ИЛИ ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.ОсновныеСредства")
		ИЛИ ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.НематериальныеАктивы") Тогда		
		
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		
		Если ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Номенклатура")
			ИЛИ ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.ОсновныеСредства")
			ИЛИ ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.НематериальныеАктивы") Тогда
...
&НаКлиенте
Процедура ПоставщикЭкспортерИлиУчастникСРППриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.Поставщики.ТекущиеДанные;
	СтрокаТабличнойЧасти.Экспортер = СтрокаТабличнойЧасти.ЭкспортерИлиУчастникСРП;
	СтрокаТабличнойЧасти.УчастникСРП = Ложь;
	
КонецПроцедуры

&НаКлиенте
Процедура ПоставщикУчастникСРППриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.Поставщики.ТекущиеДанные;
	СтрокаТабличнойЧасти.ЭкспортерИлиУчастникСРП = СтрокаТабличнойЧасти.УчастникСРП ИЛИ СтрокаТабличнойЧасти.Экспортер;
	
КонецПроцедуры

&НаКлиенте
Процедура ПоставщикЭкспортерПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПолучателиНерезидентПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.Получатели.ТекущиеДанные;
	СтрокаТабличнойЧасти.НерезидентВерсия2 = СтрокаТабличнойЧасти.Нерезидент;
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучателиНерезидентВерсия2ПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.Получатели.ТекущиеДанные;
	СтрокаТабличнойЧасти.Нерезидент = СтрокаТабличнойЧасти.НерезидентВерсия2;
	
КонецПроцедуры

&НаСервере
Процедура ПолучательРозничнаяРеализацияПриИзмененииНаСервере(ИзменитьНомерДокументаПоставки = Ложь)
	
	Если ИзменитьНомерДокументаПоставки Тогда
		
		СтруктураНомерИДатаПоставки = ЭСФСерверПереопределяемый.ПолучитьНомерДатуДокументаПоставки(Объект.СчетФактура.ДокументОснование);
		Объект.ДокументПодтверждающийПоставкуНомер = СтруктураНомерИДатаПоставки.ДокументПодтверждающийПоставкуНомер;
		
		Если НЕ ЗначениеЗаполнено(СтруктураНомерИДатаПоставки.ДокументПодтверждающийПоставкуНомер) Тогда
			
			ТекстСообщения = НСтр("ru = 'Поле ""Номер"" раздела ""F. Документы поставки"" не заполнено, так как данный документ не отражен в учете'");
	...
	Дата  = Объект.ДатаОборота;
	Склад = Объект.Склад;
	ЭтоЭСФсСНТ = ?((СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "KZ-SNT-") > 0 ИЛИ СтрНайти(Объект.ДокументПодтверждающийПоставкуАВРНомер, "KZ-SNT-") > 0), Истина, Ложь);
	
	//++ НЕ ЭСФ
	//так же перезаполним ИД товаров с 1 на нужное значение по формату (ГСВС + ИД товара + ИД склада), иначе товары не будут списаны с ВС ИС ЭСФ
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		ИсточникПроисхождения = СтрокаТовары.ИсточникПроисхождения;
		
		Если ЗначениеЗаполнено(ИсточникПроисхождения) И ИсточникПроисхождения.ПризнакУчетаНаВиртуальномСкладе И СтрокаТовары.ИдентификаторТовара = "1" Тогда
			СтруктураЗначенийИП = ПолучитьИсточникПроисхожденияТовараНаСервере(Дата, ИсточникПроисхождения, Склад, , ЭтоЭСФсСНТ);
			СтрокаТовары.ИдентификаторТовара = СтруктураЗначенийИП.ИдентификаторТовара;
			
			ТекстСообщения = НСтр("ru = 'Скорректировано значение графы ""Идентификатор товара, работ, услуг"" в строке №%1 таблицы товаров.'");
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, СтрокаТовары.НомерСтроки);
			ЭСФКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
...
Процедура ПолучательРозничнаяРеализацияПриИзменении(Элемент)
	...
	Если СтрокаПолучатели.РозничнаяРеализация И НЕ Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияЭСФ.Входящий") Тогда
		
		Если Объект.ДокументПодтверждающийПоставкуНомер = "NON SNT" Тогда
			ТекстВопроса = НСтр("ru = 'При выписке ЭСФ в розницу разделе ""F. Документы поставки"" рекомендуется указать номер документа поставки вместо ""NON SNT"". Перезаполнить номер?'");
			
			ПерезаполнитьНомерДокументПоставкиЗавершение = Новый ОписаниеОповещения("ПерезаполнитьНомерДокументаПоставкиЗавершение", ЭтаФорма);
			ПоказатьВопрос(ПерезаполнитьНомерДокументПоставкиЗавершение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
			ТекстВопроса = НСтр("ru = 'При выписке ЭСФ в розницу будет скорректировано значение графы ""Идентификатор товара, работ, услуг"" в таблице товаров. Скорректировать значение?'");
			
			ПерезаполнитьНомерДокументПоставкиЗавершение = Новый ОписаниеОповещения("ПерезаполнитьИдентификаторыЗавершение", ЭтаФорма);
			ПоказатьВопрос(ПерезаполнитьНомерДокументПоставкиЗавершение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
...
&НаКлиенте
Процедура ПерезаполнитьНомерДокументаПоставкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПолучательРозничнаяРеализацияПриИзмененииНаСервере(Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПерезаполнитьИдентификаторыЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПолучательРозничнаяРеализацияПриИзмененииНаСервере(Ложь);
	КонецЕсли;
...
Процедура ПерезаполнитьПоляИПНаКлиенте() Экспорт
	...
	Номенклатура = СтрокаТабличнойЧасти.Товар;
    СтруктураЗначенийИП = ПолучитьИсточникиИзНомераГТДНаСервере(НомерГТД, Номенклатура, Объект.ДатаОборота);
...
Функция ПолучитьИсточникиИзНомераГТДНаСервере(НомерГТД, Номенклатура, Дата)
	Возврат ЭСФСерверПереопределяемый.ПолучитьИсточникиИзНомераГТД(НомерГТД,Номенклатура, Дата);
КонецФункции
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Копирование Тогда
		...
		ЭСФКлиентПереопределяемый.УстановитьОграничениеТипаЕдиницыИзмерения(ЭтаФорма, Элементы.Товары.ТекущиеДанные);
	КонецЕсли;
...
&НаКлиенте
Процедура ТоварыБезНДСПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.Товары.ТекущиеДанные;
...
Процедура ТоварыИсточникПроисхожденияПриИзменении(Элемент)
	...
	Дата  = Объект.ДатаОборота;
	Склад = Объект.Склад;
	СтруктураЗначенийИП = ПолучитьИсточникПроисхожденияТовараНаСервере(Дата, ИсточникПроисхождения, Склад, Истина, ЭтоЭСФсСНТ);
	ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти,СтруктураЗначенийИП,"НомерЗаявленияВРамкахТС, НомерПозицииВДекларацииИлиЗаявлении, ПризнакПроисхождения, ТоварНаименованиеВРамкахТС, ИдентификаторТовара, КодТНВЭД, ДополнительныйИдентификатор, ПризнакУчетаНаВиртуальномСкладе");
	Если (СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "NON SNT") > 0 ИЛИ СтрНайти(Объект.ДокументПодтверждающийПоставкуАВРНомер, "KZ-SNT-") > 0)  Тогда
		СтрокаТабличнойЧасти.ИдентификаторТовара = "1";
	КонецЕсли;
	//-- НЕ ЭСФ
...
Процедура ТоварыТоварПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти = Элементы.Товары.ТекущиеДанные;
	ТоварПриИзмененииНаСервере(СтрокаТабличнойЧасти);
	//++ НЕ ЭСФ
...
Процедура ПерезаполнитьПоСНТНаСервере()
	...
	ОбъектЭСФ = РеквизитФормыВЗначение("Объект");
	ЭСФСервер.ОчиститьОбъект(ОбъектЭСФ, "Номер, ДатаОборота, СчетФактура, Вид");
	МассивСНТ = Новый Массив;
	МассивСНТ.Добавить(Объект.СНТ);
	СуществующиеАктуальныеЭСФ = ЭСФСерверПереопределяемый.ПроверитьАктуальныйЭСФДляСНТ(МассивСНТ, ОбъектЭСФ.Ссылка);
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ДокументОснованиеСНТ", Объект.СНТ);
	Если СуществующиеАктуальныеЭСФ.Количество() <> 0 Тогда
		ПараметрыЗаполнения.Вставить("ДополнительныеПараметры", СуществующиеАктуальныеЭСФ[0]);
	КонецЕсли;
	ПараметрыЗаполнения.Вставить("ПерезаполнениеЭСФ", Истина);
	ЭСФСерверПереопределяемый.ЗаполнитьИсходящийЭСФИзСНТ(ПараметрыЗаполнения, ОбъектЭСФ);	
	ЗначениеВРеквизитФормы(ОбъектЭСФ, "Объект");
	ЗаполнитьДопРеквизитыТаблицТоваровПоУчастникамСД();
...
&НаКлиенте
Процедура СоздатьСчетФактуруНаКлиентеПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.ОК Тогда
		
		Если ЭСФКлиент.ОбъектЗаписан(ЭтаФорма) Тогда
			
			Если НЕ СсылочныеРеквизитыДляСозданияСчетаФактурыЗаполнены() Тогда
				
			 	ТекстВопроса = НСтр(
					"ru = 'Документ имеет незаполненные ссылочные реквизиты.
					|Продолжить?'");	
					
				ДокументИмеетНезаполненныеСсылочныеРеквизитыЗавершение = Новый ОписаниеОповещения("ДокументИмеетНезаполненныеСсылочныеРеквизитыЗавершение", ЭтаФорма);
				ПоказатьВопрос(ДокументИмеетНезаполненныеСсылочныеРеквизитыЗавершение, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
				
			Иначе
				
				ДокументИмеетНезаполненныеСсылочныеРеквизитыЗавершение(КодВозвратаДиалога.ОК, Неопределено);
...
Процедура ТоварыНайтиСоздатьОбновитьНомераГТД(Команда)
	...
	Если ВедетсяУчетПоТоварамОрганизацийБУ Тогда
		...
		ТоварыНайтиСоздатьОбновитьНомераГТДНаСервере(ВыделенныеСтроки);
	...
	//++ НЕ ЭСФ	
	ИначеЕсли ВедетсяУчетПоТоварамОрганизацийНаВС Тогда
...
Процедура ЗаполнитьИсточникиПроисхожденияПоОстаткам(ОчищатьИсточники = Ложь)
	...
	Если (СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "NON SNT") > 0 ИЛИ СтрНайти(Объект.ДокументПодтверждающийПоставкуАВРНомер, "NON SNT") > 0)  Тогда
		Для Каждого СтрокаТовары из Объект.Товары Цикл
			Если СтрокаТовары.ИдентификаторТовара <> "1" Тогда
	...
	Если НЕ ЭтоЭСФсСНТ И НЕ (Объект.Получатели.Количество() > 0 И Объект.Получатели[0].РозничнаяРеализация)Тогда
		Для Каждого СтрокаТовары из Объект.Товары Цикл
			Если СтрокаТовары.ИдентификаторТовара <> "1" Тогда
				СтрокаТовары.ИдентификаторТовара = "1";
			КонецЕсли;
...
Процедура ТоварыНайтиСоздатьОбновитьНомераГТДНаСервере(МассивИдентификаторовСтрок)
	...
	// Создать МассивВыделенныхСтрок из МассивИдентификаторовСтрок.
	...
	// На данный момент номера ГТД имеет смысл создавать
	// только для номенклатуры у которой снят признак "Услуга",
	// т.к. по другим объектам учет по ГТД сейчас не ведется.	
	МассивСтрокТоваров = Новый Массив;
	Для Каждого ВыделеннаяСтрока Из МассивВыделенныхСтрок Цикл
		
		СтрокаПодходитДляЗаполненияГТД = Ложь;
		
		Если ТипЗнч(ВыделеннаяСтрока.Товар) = Тип("СправочникСсылка.Номенклатура") Тогда
			Если НЕ ВыделеннаяСтрока.Товар.Услуга Тогда
				СтрокаПодходитДляЗаполненияГТД = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаПодходитДляЗаполненияГТД Тогда
			МассивСтрокТоваров.Добавить(ВыделеннаяСтрока);
		Иначе
			ТекстСообщения = НСтр(
			"ru = 'Невозможно заполнить поле ""Источник происхождения"" в строке номер %1.
			...
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ВыделеннаяСтрока.НомерСтроки);
			ЭСФКлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.ВерсияБланкаЭСФ) Или Объект.ВерсияБланкаЭСФ = 1 Тогда 
		ТаблицаТоваровСПониженнойСтавкой = ЭСФСервер.ПолучитьТаблицуТоваровСПониженнойСтавкой(МассивСтрокТоваров,Объект.Дата);
		
		// Заполнить ТаблицаНомеровГТД, которая будет использоваться 
		// для поиска, создания, обновления номеров ГТД.
		Для Каждого СтрокаТовар Из МассивСтрокТоваров Цикл
			
			СтрокаНомерГТД = ТаблицаНомеровГТД.Добавить();
			
			СтрокаНомерГТД.Идентификатор = Формат(СтрокаТовар.ПолучитьИдентификатор(), "ЧН=0; ЧГ=");
			СтрокаНомерГТД.НомерСтроки = Формат(СтрокаТовар.НомерСтроки, "ЧН=0; ЧГ=");
			СтрокаНомерГТД.НомерГТД = СтрокаТовар.НомерЗаявленияВРамкахТС;
			СтрокаНомерГТД.НомерСтрокиГТД = СтрокаТовар.ДополнительныеДанные;		
			СтрокаНомерГТД.КодТНВЭД = СтрокаТовар.КодТНВЭД;
			СтрокаНомерГТД.НаименованиеТовара = СтрокаТовар.ТоварНаименование;
			СтрокаНомерГТД.СтранаПроисхожденияТовара = Неопределено;
			
			СпособыПроисхожденияТоваровИСЭСФ = Объект.Поставщики[0].ДополнительныеСведения;
			СпособыПроисхожденияТоваровИБ = ЭСФКлиентСервер.СпособыПроисхожденияТоваровИБ(СпособыПроисхожденияТоваровИСЭСФ);	
			СтрокаНомерГТД.СпособПроисхожденияТовара = СпособыПроисхожденияТоваровИБ;
			
			СтрокаНомерГТД.РегистрационныйНомерЭСФ = Объект.РегистрационныйНомер;
			
			СтрокаНомерГТД.СсылкаНомерГТД = СтрокаТовар.НомерГТД;		
			ПониженнаяСтавка = ТаблицаТоваровСПониженнойСтавкой.Найти(СтрокаТовар.НомерСтроки,"НомерСтроки") <> Неопределено;
			СтрокаНомерГТД.ПризнакПроисхождения = ЭСФСервер.ПолучитьПризнакПроисхождения(ПониженнаяСтавка,СпособыПроисхожденияТоваровИБ);
		КонецЦикла;
		
	ИначеЕсли Объект.ВерсияБланкаЭСФ = 2 Тогда
		...
		Для Каждого СтрокаТовар Из МассивСтрокТоваров Цикл
			...
			СтрокаНомерГТД.НомерСтроки = Формат(СтрокаТовар.НомерСтроки, "ЧН=0; ЧГ=");
			СтрокаНомерГТД.НомерГТД = СтрокаТовар.НомерЗаявленияВРамкахТС;
			СтрокаНомерГТД.НомерСтрокиГТД = СтрокаТовар.НомерПозицииВДекларацииИлиЗаявлении;
			СтрокаНомерГТД.КодТНВЭД = СтрокаТовар.КодТНВЭД;
			СтрокаНомерГТД.ГСВС = ЭСФСерверПереопределяемый.ПолучитьГСВС(,СтрокаТовар.КодТНВЭД);
			СтрокаНомерГТД.НаименованиеТовара = СтрокаТовар.ТоварНаименованиеВРамкахТС;
			СтрокаНомерГТД.СтранаПроисхожденияТовара = Неопределено;
	...
	Справочники.НомераГТД.НайтиСоздатьОбновитьНомераГТД(ТаблицаНомеровГТД, "Товары", Объект.Ссылка);
	
	// Заполнить в таблице Товары ссылки на найденные и созданные элементы справочника НомераГТД. 
	Для Каждого СтрокаТовар Из Объект.Товары Цикл
		СтрокаНомерГТД = ТаблицаНомеровГТД.Найти(Формат(СтрокаТовар.ПолучитьИдентификатор(), "ЧН=0; ЧГ="), "Идентификатор");
...
Процедура УстановитьДоступностьРеквизитов()
	...
	Если Элементы.ФормаРедактироватьНедоступныеРеквизиты.Пометка Тогда
		...
		ТолькоПросмотрРеквизитовЭСФ = НЕ ЭСФСерверПереопределяемый.ДоступнаРольБезусловногоРедактированияЭСФ()
										...
										ИЛИ Объект.Состояние = Перечисления.СостоянияЭСФ.АннулированИСЭСФДляОтнесенияВЗачетИНаВычеты
										ИЛИ Объект.Состояние = Перечисления.СостоянияЭСФ.АннулированПриОтзывеСНТ
										ИЛИ Объект.Состояние = Перечисления.СостоянияЭСФ.АннулированПриОтклоненииСНТ
		...
		Иначе
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодменюНомераГТД", "Видимость", ВедетсяУчетПоТоварамОрганизацийБУ);
		КонецЕсли;
		
		//установим для колонки Товар виртуального склада, для исходящих ЭСФ, только просмотр,
		//так как признак будет определяться по данным источника происхождения
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПризнакУчетаНаВиртуальномСкладе", "ТолькоПросмотр", (Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияЭСФ.Исходящий")));
...
Функция НовыйМассивИменНедоступныхЭлементов()
	...
	Массив.Добавить("ГруппаПолучательРозничнаяРеализация");
	Массив.Добавить("ГруппаПолучательФизическоеЛицо");
	...
	Массив.Добавить("ДокументПодтверждающийПоставкуАВРНомер");
	Массив.Добавить("ДокументПодтверждающийПоставкуАВРДата");
...
Процедура СформироватьПредставлениеСостояния()
	...
	ИначеЕсли Объект.Состояние = Перечисления.СостоянияЭСФ.Отозван
		ИЛИ Объект.Состояние = Перечисления.СостоянияЭСФ.Аннулирован
		ИЛИ Объект.Состояние = Перечисления.СостоянияЭСФ.ОтклоненПолучателем Тогда
	...
	ИначеЕсли Объект.Состояние = Перечисления.СостоянияЭСФ.ЗаблокированИСЭСФ Тогда
...
Процедура ВыбратьСНТНаКлиенте()
	...
	ИначеЕсли Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияЭСФ.Входящий")// эсф входящая дополнительная от поставщика 
		И Объект.Вид = ПредопределенноеЗначение("Перечисление.ВидыЭСФ.Дополнительный") Тогда
		ТипСНТ = ПредопределенноеЗначение("Перечисление.НаправленияСНТ.Исходящий"); //возврат тмз поставщику
...
Процедура УправлениеФормой()
	...
	Если ЭтаФорма.РежимТолькоПросмотр Тогда 
		...
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Грузоотправитель", "Видимость", Ложь);
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Грузополучатель", "Видимость", Ложь);
	...
	Если ВедетсяУчетПоТоварамОрганизацийНаВС Тогда
		...
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодменюНомераГТД", "Видимость", (Объект.Направление = Перечисления.НаправленияЭСФ.Исходящий И НЕ Объект.Вид = Перечисления.ВидыЭСФ.Дополнительный));
	Иначе
	...
	ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНомерГТД", "Видимость", ВедетсяУчетПоТоварамОрганизацийБУ);	
	...
	Если Объект.Направление = Перечисления.НаправленияЭСФ.Исходящий Тогда
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыИсточникПроисхождения", "ТолькоПросмотр", Ложь);
	Иначе
		Если Объект.Вид = ПредопределенноеЗначение("Перечисление.ВидыЭСФ.Дополнительный") И Объект.СвязанныйЭСФДата <= ДатаПереходаНаВС Тогда
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыИсточникПроисхождения", "ТолькоПросмотр", Ложь);
		ИначеЕсли Объект.Вид = ПредопределенноеЗначение("Перечисление.ВидыЭСФ.Дополнительный") И Объект.Дата >= ЭСФСерверПереопределяемый.ДатаСнятияВСвГСВС() Тогда
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыИсточникПроисхождения", "ТолькоПросмотр", Ложь);
		Иначе
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыИсточникПроисхождения", "ТолькоПросмотр", Истина);
		КонецЕсли;
	КонецЕсли;
	...
	//++ НЕ ЭСФ
	Если Не (СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "KZ-SNT-") > 0) Тогда
		Если Объект.Вид = Перечисления.ВидыЭСФ.Дополнительный И ЗначениеЗаполнено(Объект.СвязанныйЭСФ) Тогда
			Если (СтрНайти(Объект.СвязанныйЭСФ.ДокументПодтверждающийПоставкуНомер, "KZ-SNT-") > 0) Тогда
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНомерИзСНТ", "Видимость", Истина);
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоПолучателямНомерИзСНТ", "Видимость", Истина);
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоПоставщикамНомерИзСНТ", "Видимость", Истина);
			Иначе
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНомерИзСНТ", "Видимость", Ложь);
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоПолучателямНомерИзСНТ", "Видимость", Ложь);
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоПоставщикамНомерИзСНТ", "Видимость", Ложь);
			КонецЕсли;
		Иначе
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНомерИзСНТ", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоПолучателямНомерИзСНТ", "Видимость", Ложь);
	...
	ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПредставлениеТоварыНеПодлежащиеВС", "Видимость", ЕстьОстаткиТоваровНеВС);
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументЭлектронныйДокументВССоздатьЭлектронныйДокументВССписание", "Видимость", ЕстьОстаткиТоваровНеВС);
	//-- НЕ ЭСФ
...
&НаКлиенте
Процедура ТоварПриИзмененииНаСервере(СтрокаТабличнойЧасти)
	
	Товар = СтрокаТабличнойЧасти.Товар; 
	СтрокаТабличнойЧасти.ЕдиницаИзмеренияХраненияОстатков = ПолучитьЕдиницуИзмеренияХраненияОстатков(Товар);
		
Конецпроцедуры

&НаСервере
Функция ПолучитьЕдиницуИзмеренияХраненияОстатков(Товар)
	
	ЕдиницаИзмеренияХраненияОстатков = Неопределено;
	
	Если ЗначениеЗаполнено(Товар) И ТипЗнч(Товар) = Тип("СправочникСсылка.Номенклатура") Тогда
		ЕдиницаИзмеренияХраненияОстатков = Товар.БазоваяЕдиницаИзмерения;
		
	КонецЕсли;
	
	Возврат ЕдиницаИзмеренияХраненияОстатков; 
...
Процедура НастроитьВерсиюЭСФ(Форма, ВерсияБланка)
	...
	|ГруппаПоставщикУчастникСРП, 
	|ГруппаПоставщикЭкспортер,
	|ГруппаПоставщикМеждународныйПеревозчик,
	|ГруппаПоставщикДоверитель,
	...
	|ПолучательКодСтраны,
	|ГруппаПолучательУчастникСРП,
	|ГруппаПолучательДоверитель,
	|ГруппаПолучательРозничнаяРеализация,
	|ГруппаПолучательНерезидентВерсия2,
	|ГруппаПолучательФизическоеЛицо,
	...
	|ГруппаПоставщикЭкспортерИлиУчастникСРП, 
	|ПоставщикиЭкспортерИлиУчастникСРП,
	|ГруппаПолучательНерезидент,
...
&НаСервереБезКонтекста
Функция ПолучитьИсточникПроисхожденияТовараНаСервере(Дата, ИсточникПроисхождения, Склад, ДляДокументаЭСФ, ЭтоЭСФсСНТ)
	Возврат ВССерверПереопределяемый.ПолучитьДанныеИсточникаПроисхождения (Дата, ИсточникПроисхождения, Склад, Истина, ЭтоЭСФсСНТ);
КонецФункции
//-- НЕ ЭСФ
...
Процедура ПроверитьНаличиеКоэффициентаПересчета()
	...
	|ГДЕ
	|	вт_ТоварыВсе.ПризнакУчетаНаВиртуальномСкладе = ЛОЖЬ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
...
#Область СозданиеЭДВССписаниеТоваровНеВСпоКодТНВЭД
...
Процедура ПредставлениеТоварыНеПодлежащиеВСНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		ОформитьЭДВСНаСписаниеЗавершение = Новый ОписаниеОповещения("ОформитьЭДВСНаСписаниеЗавершение", ЭтаФорма);
		...
		Если (НЕ Объект.Проведен) И (НЕ Объект.ПометкаУдаления) Тогда
			...
			ПослеЗакрытияВопросаВводаНаОснованииЭДВССписание = Новый ОписаниеОповещения("ПослеЗакрытияВопросаВводаНаОснованииЭДВССписание", ЭтаФорма);
			ПоказатьВопрос(ПослеЗакрытияВопросаВводаНаОснованииЭДВССписание, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0); 
		...
		Иначе
			ОформитьЭДВСНаСписаниеЗавершение(КодВозвратаДиалога.ОК, Неопределено);
		КонецЕсли;
...
&НаКлиенте
Процедура ОформитьЭДВСНаСписаниеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда
		...
		Если ЭСФКлиент.ОбъектЗаписан(ЭтаФорма) Тогда
			...
			ЗначенияЗаполнения = Новый Структура();
			ЗначенияЗаполнения.Вставить("ДокументОснование", Объект.Ссылка);
			ЗначенияЗаполнения.Вставить("ЭтоВводСписаниеИзВходящегоЭСФ", Истина);
...
Процедура ПроверитьСписокТоваровНеПодлежащихУчетуНаВС()
	...
	Если Объект.Проведен И (Объект.Направление = Перечисления.НаправленияЭСФ.Входящий И Объект.Получатели[0].РозничнаяРеализация) Тогда
	...
	СписокЭДВССписаниеТоваровНеВС.ЗагрузитьЗначения(ПолучитьМассивЭДВССписаниеТоваровНеВС(Объект.Ссылка));
...
Функция ПолучитьТаблицуТоваровНеВС()
	...
	Запрос.УстановитьПараметр("ТекущийЭСФ", Объект.Ссылка);
	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДатаСеанса());
	
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыВС.Товар КАК Номенклатура,
	|	ТоварыВС.ИсточникПроисхождения КАК ИсточникПроисхождения
	|ПОМЕСТИТЬ ТоварыВС
	|ИЗ
	|	Документ.ЭСФ.Товары КАК ТоварыВС
	|ГДЕ
	|	ТоварыВС.Ссылка = &ТекущийЭСФ
	|	И ТоварыВС.Ссылка.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭСФ.Входящий)
	|	И ТоварыВС.Ссылка.Вид <> ЗНАЧЕНИЕ(Перечисление.ВидыЭСФ.Дополнительный)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					ИЗ
	|						ТоварыВС КАК ТоварыВС)) КАК ТоварыНаВиртуальныхСкладахОстатки
...
&НаСервереБезКонтекста
Функция ПолучитьМассивЭДВССписаниеТоваровНеВС(ДокументЭСФ)
