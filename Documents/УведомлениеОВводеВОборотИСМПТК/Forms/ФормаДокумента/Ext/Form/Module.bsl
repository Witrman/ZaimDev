...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Объект.НомерИСМПТ = "";
		Объект.ПричинаВводаВОборот = ПредопределенноеЗначение("Перечисление.ПричиныПовторногоВводаКМВОборотИСМПТК.Излишки");
	КонецЕсли;
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") Тогда
		...
		Если ИзменитьДокументОснование Тогда
			...
			Иначе
				...
				"ru = 'На основании документа ""%ДокументОснование%"" уже введено Уведомление о вводе в оборот. Привязать текущее Уведомление к выбранному документу без отсоединения ранее связанного Уведомления?'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ИнтеграцияИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОВводеВОборот() Тогда		
		...
		Если Не ЗначениеЗаполнено(Объект.ДокументОснованиеНаименование) Тогда
			Объект.ДокументОснованиеНаименование = нСтр("ru='Уведомление о вводе в оборот'");
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляВводаВОборот();
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВводеВОборотИСМПТКОтправить", "Доступность", НеОтправлен);
...
Процедура ПредставлениеСостоянияНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Объект.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияДокументовИСМПТК.Сформирован") Тогда
		...
		ПоказатьВопрос(ОтправитьПолучателюЗаписьФормы, НСтр("ru = 'Отправить документ в ИС МПТ?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура УправлениеФормой()
	...
	УправлениеВидимостьюПолейДанныеДокументаОснования(НеОтправлен);
...
Функция ПроверитьСвязанноеУведомление(ВыбранноеЗначение)
	...
	Возврат ИнтеграцияИСМПТК.ПроверитьНаличиеСвязанногоУведомленияОВводеВОборот(ВыбранноеЗначение, Истина);
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат ИнтеграцияИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияВводВОборотНаСервере(Объект.ДокументОснование);
...
Процедура ЗапретитьРедактированиеПолей()
	...
	Если Не Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСМПТК.НеОпределен") Тогда
		...
		Элементы.ПричинаВводаВОборот.ТолькоПросмотр = Истина;
...
Процедура ПослеЗакрытияВопросаРедактироватьСтрокиТранспортныеКоды(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Исключение
			...
			Информация = ИнформацияОбОшибке();
			ТекстСообщения = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.ТекстСообщенияНеУдалосьЗаписатьДокументМаркировки();
			ТекстСообщения = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки(Информация));
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения, , , "Форма.Объект");
...
Процедура ОбработатьОтветСервераПоСостояниюКМ(МассивКМ, ОтветСервераСостояниеКМ)
	...
	ИнтеграцияИСМПТК.ОбработатьОтветСервераПоСостояниюКМ_Общая(ЭтаФорма, МассивКМ, ОтветСервераСостояниеКМ, "УведОВводеВОборот", НовоеСоответствиеСтатусов);
...
&НаСервере
Процедура УправлениеВидимостьюПолейДанныеДокументаОснования(НеОтправлен)
	
	//Данные документа-основания в Прочее
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДокументОснованиеНаименование", "ТолькоПросмотр", Не НеОтправлен);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДокументОснованиеДата", 		   "ТолькоПросмотр", Не НеОтправлен);
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДокументОснованиеНомер", 	   "ТолькоПросмотр", Не НеОтправлен);
	//Команда их заполнения по основанию
	ИнтерфейсИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗаполнитьСлужебныеПоляПоДокументуОснования", "Доступность", НеОтправлен И ЗначениеЗаполнено(Объект.ДокументОснование));
...
Процедура ВвыполнитьЗаполнениеСлужебныеПоляПоДокументуОснования()
	...
	ДанныеУчетногоДокумента = ИнтеграцияИСМПТКВызовСервера.ПолучитьДопСведенияДокументаОснованияВводВыводИзОборота(Объект.ДокументОснование);
