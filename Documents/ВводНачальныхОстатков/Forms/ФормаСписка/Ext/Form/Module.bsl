...
Процедура СписокПередЗагрузкойПользовательскихНастроекНаСервере(Элемент, Настройки)
	...
	Если ОткрытИзПомощника Тогда
		...
		Отборы = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.КомпоновщикНастроек.Настройки.Отбор, "РазделУчета");
		
		Если Отборы.Количество() > 0 Тогда
			ОтборРазделУчета = Отборы[0];
		КонецЕсли;
		...
		Если ОтборОрганизация = Неопределено И ОтборРазделУчета = Неопределено И ОтборДата = Неопределено Тогда
			Возврат;
		КонецЕсли;
		...
		Для Каждого ЭлементНастроек Из Настройки.Элементы Цикл
			Если ТипЗнч(ЭлементНастроек) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
				...
				ИначеЕсли ЭлементНастроек.ИдентификаторПользовательскойНастройки = "РазделУчета" И ОтборРазделУчета <> Неопределено Тогда
					ЭлементНастроек.Использование  = ОтборРазделУчета.Использование;
					ЭлементНастроек.ВидСравнения   = ОтборРазделУчета.ВидСравнения;
					ЭлементНастроек.ПравоеЗначение = ОтборРазделУчета.ПравоеЗначение;
