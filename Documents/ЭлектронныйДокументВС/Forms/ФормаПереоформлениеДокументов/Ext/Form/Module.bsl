...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Параметры.Свойство("ПомощникПерехода") И Параметры.ПомощникПерехода Тогда
		...
		ДатаОформления				= НачалоДня(ДатаПереходаНаУчетТоваровВС);
...
Функция ПолучитьТекстЗапросаВводОстатков()
	...
	|ИЗ
	|	Документ.ЭлектронныйДокументВС.Товары КАК ЭДВСТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ вт_ДокументЭДВС
	|		ПО ЭДВСТовары.Ссылка = вт_ДокументЭДВС.Ссылка
	|		ГДЕ вт_ДокументЭДВС.Проведен 
	|		И НЕ вт_ДокументЭДВС.ПометкаУдаления
	|
	|СГРУППИРОВАТЬ ПО
	|	ЭДВСТовары.Товар,
	|	ЭДВСТовары.ЕдиницаИзмерения,
	...
	|	Документ.ЭлектронныйДокументВС.ТоварыВС КАК ЭДВСТоварыВС
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ вт_ДокументЭДВС
	|		ПО ЭДВСТоварыВС.Ссылка = вт_ДокументЭДВС.Ссылка
	|		ГДЕ вт_ДокументЭДВС.Проведен 
	|		И НЕ вт_ДокументЭДВС.ПометкаУдаления
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПереоформитьНаСервере(ПараметрыПереоформления)
	...
	Если НЕ ПараметрыПереоформления.СтруктурноеПодразделение = Неопределено Тогда
		СписокСтруктурныхПодразделений.Добавить(ПараметрыПереоформления.СтруктурноеПодразделение);
	КонецЕсли;
	...
	Если ВызванаИзПомощникаПерехода Тогда
		...
		Запрос.УстановитьПараметр("ДатаНачала"		, НачалоДня(ДатаПереходаНаУчетТоваровВС) );
		Запрос.УстановитьПараметр("ДатаОкончания"	, КонецДня(ДатаПереходаНаУчетТоваровВС));
		Запрос.УстановитьПараметр("МассивЭДВС"		, ПараметрыПереоформления.МассивЭДВС);
		
		ПараметрыСоздания.Вставить("ДатаНачала"		, НачалоДня(ДатаПереходаНаУчетТоваровВС));
		ПараметрыСоздания.Вставить("ДатаОкончания"	, КонецДня(ДатаПереходаНаУчетТоваровВС));
