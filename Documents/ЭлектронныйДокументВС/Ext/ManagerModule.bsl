...
Функция ПолучитьЗначенияПризнаковПроисхождения(ТаблицаТовары,Дата)Экспорт
	...
	ТаблицаСтрок.Колонки.Добавить("ТипПошлины", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыПошлинВС"));
	ТаблицаСтрок.Колонки.Добавить("НомерЗаявленияВРамкахТС", Новый ОписаниеТипов("Строка"));
	ТаблицаСтрок.Колонки.Добавить("ТипПроисхождения", Новый ОписаниеТипов("ПеречислениеСсылка.ТипПроисхожденияТоваровВС"));

	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаТовары.ГСВС КАК ГСВС,
	|	ТаблицаТовары.ТипПроисхождения КАК ТипПроисхождения,
	|	ТаблицаТовары.НомерСтроки КАК НомерСтроки,
	|	ТаблицаТовары.НомерЗаявленияВРамкахТС КАК НомерЗаявленияВРамкахТС
	|ПОМЕСТИТЬ вт_Товары
	|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ТипПроисхождения) Тогда
			
			СтруктураДанных = ВСКлиентСервер.ОпределитьТипПроисхождения(ВыборкаДетальныеЗаписи.НомерЗаявленияВРамкахТС);
			
			НоваяСтрока.ТипПроисхождения = СтруктураДанных.ТипПроисхождения;
			НоваяСтрока.ТипПошлины = СтруктураДанных.ТипПошлины;
		КонецЕсли;
		
		ТипПроисхождения = ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ТипПроисхождения), ВыборкаДетальныеЗаписи.ТипПроисхождения, НоваяСтрока.ТипПроисхождения);
		НоваяСтрока.ПризнакПроисхождения = ВСКлиентСервер.ОпределитьПризнакПроисхожденияТовара(ТипПроисхождения, ВыборкаДетальныеЗаписи.ПризнакПеречняИзьятий);
