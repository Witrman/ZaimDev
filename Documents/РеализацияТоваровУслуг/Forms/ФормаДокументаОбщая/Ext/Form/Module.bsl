...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		Если ИмяСобытия = "ScanData" Тогда
			...
			КонецЕсли;
			Результат = Новый Структура("Штрихкод", ТекущийКод);
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	ВидОперацииПриИзмененииНаСервере(Новый Структура);
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			...
			Если ВидОперацииДокументаОснования <> Объект.ВидОперации Или ВидОперацииДокументаОснования <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализацияТоваров.Услуги") Тогда
				...
				Сообщение.Текст = СтрШаблон("Нет данных для заполнения. Ввод на основании реализации ТМЗ и услуг доступен только для документов с видом операции ""%1""", ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализацияТоваров.Услуги"));
				Сообщение.Сообщить();
				Возврат
			КонецЕсли;
		КонецЕсли;
...
Процедура ПодборТовары(Команда)
	...
	РеализацияТоваровУслугФормыКлиент.Подбор(ЭтаФорма,"Товары", УникальныйИдентификатор);
...
Процедура ПодборУслуги(Команда)
	...
	РеализацияТоваровУслугФормыКлиент.Подбор(ЭтаФорма,"Услуги", УникальныйИдентификатор);
...
Процедура ИзменитьУслуги(Команда)
	...
	РеализацияТоваровУслугФормыКлиент.ИзменитьТабличнуюЧасть(ЭтаФорма,"Услуги", "Услуги",ПараметрыФормы, УникальныйИдентификатор);
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			...
			Если ЗначениеЗаполнено(Объект.СпособВыпискиАктовВыполненныхРабот)
				...
				ТекстСообщения = НСтр("ru = 'Способ выписки <%1> не соответствует виду операции документа <%2>." + Символы.ПС + "Способ выписки изменен на <%3>.'");
...
&НаСервере
Процедура ВидОперацииПриИзмененииНаСервере(СтруктураПараметров) Экспорт
	
	ПричиныИзмененияСчетовУчета = Новый Массив;
	ПричиныИзмененияСчетовУчета.Добавить("ВидОперации");
	
	РеализацияТоваровУслугФормы.ПроверитьСоответствиеКонтрагентаВидуОперации(Объект.ВидОперации, Объект.Контрагент, Объект.ДоговорКонтрагента);
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ТекущийКонтрагент = Неопределено;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
	...
	РеализацияТоваровУслугФормы.ЗаполнитьСчетаУчета(ЭтаФорма,ПричиныИзмененияСчетовУчета, "ВидОперации");
	
	Если СтруктураПараметров.Свойство("ОчиститьТабЧасти") И СтруктураПараметров.ОчиститьТабЧасти Тогда 
		
		Объект[СтруктураПараметров.ИмяТабЧасти].Очистить();
		
		Если СтруктураПараметров.ИмяТабЧасти = "Товары" Тогда
...
Процедура ОткрытьФормуВыбораСчетаНаОплату(ТабличнаяЧасть)
	...
	РеализацияТоваровУслугФормыКлиент.ОткрытьФормуВыбораСчетаНаОплату(ЭтаФорма,ТабличнаяЧасть);
