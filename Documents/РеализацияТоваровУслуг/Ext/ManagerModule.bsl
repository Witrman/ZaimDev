...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт 
	...
	Запрос.УстановитьПараметр("СодержаниеВыделенАкциз",    НСтр("ru = 'Акциз'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктОбОказанииУслуг") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ПечатьНакладная(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ДанныеДокументов.Следующий() Цикл 
		...
		ОбластьЗаголовок.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ДанныеДокументов.Ссылка, НСтр("ru = 'Расходная накладная'"));
		...
		Если ДанныеДокументов.УчитыватьНДС Тогда
			ОбластьИтогоНДС.Параметры.НДС 	   = ?(ДанныеДокументов.СуммаВключаетНДС, НСтр("ru='В том числе НДС:'"), НСтр("ru='Сумма НДС:'"));
		...
		ОбластьСуммаПрописью.Параметры.СуммаПрописью  = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(СуммаКПрописи, ДанныеДокументов.ВалютаДокумента);
...
Функция ПечатьАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ДанныеДокументов.Следующий() Цикл 
		...
		ОбластьЗаголовок.Параметры.ТекстЗаголовка = РаботаСДиалогами.СформироватьЗаголовокДокумента(ДанныеДокументов.Ссылка, "Акт");
		ТабДокумент.Вывести(ОбластьЗаголовок);
		...
		ОбластьПодписи.Параметры.РасчетныйСчетОрганизации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 в %2, %3'"),
		...
		ОбластьПодписи.Параметры.РасчетныйСчетКонтрагента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 в %2, %3'"),
...
Функция ПечатьР1(МассивОбъектов, ОбъектыПечати, КолонкиНДС = Ложь) Экспорт
	...
	Пока ДанныеДокументов.Следующий() Цикл 
		...
		Подвал.Параметры.ДатаПринятия            = Формат(ДанныеДокументов.ДатаПодписанияГЗ, "ДЛФ=Д");
			...
			Если ДокументОбъект.ДатаПодписанияГЗ = Дата (1,1,1) Тогда
				...
				Если Выборка.ЕстьАВР Тогда
					...
					Иначе
						...
						Выборка.ДокументРТУ);
						ЗаписьЖурналаРегистрации("Обновление ИБ. Документ ""Реализация ТМЗ и услуг"". Заполнение даты подписания ГЗ", УровеньЖурналаРегистрации.Информация,,,ТекстСообщения);
