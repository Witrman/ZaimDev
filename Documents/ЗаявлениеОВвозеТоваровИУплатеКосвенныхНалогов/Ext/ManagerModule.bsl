...
Процедура ПроверитьСоответствиеНалоговомуПериоду(ДокументСсылка) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'В документе <%1>'"), ДокументСсылка);
		ТекстСообщения = ТекстСообщения + Символы.ПС
			+ СтрШаблон(НСтр("ru = 'выявлены несоответствия ""Даты принятия к учету"" налоговому периоду документа ""%1"":'"),
						Формат(ДокументСсылка.НалоговыйПериод, "ДФ='MMMM yyyy'"));
		
		ВыборкаНесоответствия = РезультатЗапроса.Выбрать();
		
		Пока ВыборкаНесоответствия.Следующий() Цикл
			ТекстСообщения = ТекстСообщения + Символы.ПС
				+ СтрШаблон(НСтр("ru = '- ""%1"": строка <%2> по документу <%3> не относится к выбранному налоговому периоду'"),
...
Функция ПечатьЗаявлениеНаВвоз(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ВыборкаПоШапке.Следующий() Цикл
		...
		ОбластьМакета.Параметры.Дата  = Формат(ВыборкаПоШапке.Дата, "ДЛФ=DD");
