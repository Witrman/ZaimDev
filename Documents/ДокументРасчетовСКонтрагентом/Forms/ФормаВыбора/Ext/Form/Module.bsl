...
Функция ПроверитьВедениеУчетаПоДокументамРасчетов(СчетУчета)
	...
	СвойстваСчета = ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.ПолучитьСвойстваСчета(СчетУчета);
	
	Для Н = 1 По СвойстваСчета.КоличествоСубконто Цикл

		Если НЕ СвойстваСчета["ВидСубконто" + Н + "ТолькоОбороты"]
		   И СвойстваСчета["ВидСубконто" + Н] = ПланыВидовХарактеристик.ВидыСубконтоТиповые.ДокументыРасчетовСКонтрагентами Тогда
...
Процедура ОграничениеСпискаДокументовПоТипу(Результат, ТипыДокументов)
	...
	Для каждого СтрокаВыборки Из Результат Цикл
		...
		Иначе
			...
			Иначе
				...
				ЕстьСуммаДокумента	 = ОбщегоНазначенияБК.ЕстьРеквизитДокумента("СуммаДокумента", 	МД_Документа);
				ЕстьВидОперации		 = ОбщегоНазначенияБК.ЕстьРеквизитДокумента("ВидОперации",		МД_Документа);
				ЕстьВалютаДокумента  = ОбщегоНазначенияБК.ЕстьРеквизитДокумента("ВалютаДокумента",	МД_Документа);
				ЕстьОтветственный	 = ОбщегоНазначенияБК.ЕстьРеквизитДокумента("Ответственный",	МД_Документа);
	...
	Для каждого СтрокаВыборки Из Результат Цикл
		...
		Если СтрокаДокумента <> Неопределено Тогда
			...
			СтрокаВыборки.СостояниеДокумента = ?(СтрокаДокумента.ПометкаУдаления, 1, ?(СтрокаДокумента.Проведен, 0, 2));
...
Функция ПолучитьДанныеСпискаПоРеквизитам(СтруктураОтбора)
	...
	Для Каждого ТипДокумента Из ТипыДокументов.Типы() Цикл
		...
		+?(ОбщегоНазначенияБК.ЕстьРеквизитДокумента("СуммаДокумента",МД_Документа),", Док.СуммаДокумента КАК Сумма",", Null КАК Сумма")
		+ ", Док.Дата КАК Дата"
		+ ", Док.Номер КАК Номер"
		...
		+ ?(ОбщегоНазначенияБК.ЕстьРеквизитДокумента("ВидОперации",МД_Документа),", Док.ВидОперации КАК ВидОперации",", Null КАК ВидОперации")
		+ ?(ОбщегоНазначенияБК.ЕстьРеквизитДокумента("ВалютаДокумента",МД_Документа),", Док.ВалютаДокумента КАК Валюта",", Null КАК Валюта")
		+ ?(ОбщегоНазначенияБК.ЕстьРеквизитДокумента("Ответственный",МД_Документа),", Док.Ответственный КАК Ответственный",", Null КАК Ответственный")
		+ " 
		| ИЗ 	
		|	Документ." + МД_Документа.Имя + " КАК Док 
		| Где
...
Функция ПолучитьДанныеСпискаПоОстаткам(СтруктураОтбора)
	...
	Для Н = 1 По СвойстваСчета.КоличествоСубконто Цикл
		...
		Если НЕ СвойстваСчета["ВидСубконто" + Н + "ТолькоОбороты"] Тогда
			...
			Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Контрагенты Тогда
				УсловиеЗапроса = УсловиеЗапроса + " И Субконто" + Н + " = &субконто" + Н;
				Запрос.УстановитьПараметр("Субконто" + Н, Контрагент);
			ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.Договоры Тогда
				УсловиеЗапроса = УсловиеЗапроса + " И Субконто" + Н + " = &субконто" + Н;
				Запрос.УстановитьПараметр("Субконто" + Н, ДоговорКонтрагента);
			ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоТиповые.ДокументыРасчетовСКонтрагентами Тогда
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		Запрос.УстановитьПараметр("Счет", ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.СчетаВИерархии(Счет));
...
Функция ПолучитьДанныеСпискаПоОборотам(СтруктураОтбора)
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Счет) Тогда
		Если ПроцедурыБухгалтерскогоУчетаВызовСервераПовтИсп.НаСчетеВедетсяУчетПоДокументамРасчетов(Счет) Тогда
			ИспользованоСубконто = ИспользованоСубконто + 1;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СчетУчета = Неопределено Тогда
		СчетУчета = ПланыСчетов.Типовой.ПустаяСсылка();		
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли РежимОтбораДокументов = Перечисления.РежимОтбораДокументов.ПоОстаткам
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, "СчетУчета",, Отказ);	
