...
Процедура УстановитьКнопкиПодменюЗаполненияНазначенияПлатежа()	
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеПенсионныхВзносов ИЛИ
		...
		Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеДенежныхСредствПодотчетнику Тогда
...
Процедура ОбновитьПодвал(Форма)
	...
	СуммаИтого = "Всего: " + ?(Объект.ВключатьКомиссиюБанка, Объект.СуммаДокумента + Объект.СуммаКомиссии, Объект.СуммаДокумента) + " " + Объект.ВалютаДокумента;
	...
	Если Объект.ВключатьКомиссиюБанка И Объект.СуммаКомиссии <> 0 Тогда
		Форма.ИнформационнаяНадписьИтого = Форма.ИнформационнаяНадписьИтого + НСтр("ru = ', в т.ч. комиссия '") + Объект.СуммаКомиссии + " " + Объект.ВалютаДокумента;
	КонецЕсли;
...
Процедура СформироватьНазначениеПлатежа(АвтоподстановкаСуммыНДС = "СуммаИНДС") Экспорт
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеПенсионныхВзносов
	 ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеСоциальныхОтчислений Тогда
				...
				Если ЗначениеЗаполнено(ДокументДляПеречисления) Тогда
					...
					Если ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениАкт
					 ИЛИ ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениСам Тогда
						СтрокаТекста = НСтр("ru = 'Пеня за несвоевременное перечисление %1'"); 
						Если ВидОПВ = "ОПВ" Тогда
							СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаТекста, НСтр("ru = 'обязательных пенсионных взносов'"));
							Объект.КодНазначенияПлатежа = "019";
						ИначеЕсли ВидОПВ = "ОПВР" Тогда 
							СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаТекста, НСтр("ru = 'обязательных пенсионных взносов работодателя'"));
							Объект.КодНазначенияПлатежа = "098";
						Иначе
							СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаТекста, НСтр("ru = 'обязательных профессиональных пенсионных взносов'"));
					...
					Если СтрокаТекста = "" Тогда
						...
						ИначеЕсли ВидОПВ = "ОПВР" Тогда
							...
							СтрокаТекста = НСтр("ru = '%1% обязательные пенсионные взносы работодателя'"); 
			...
			Если (Объект.ПеречислениеСоциальныхОтчислений.Количество() > 0) Тогда
				...
				Если ЗначениеЗаполнено(ДокументДляПеречисления) Тогда
					...
					Если ДокументДляПеречисления.ВидОперации = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОбязательныхСоциальныхОтчислений Тогда
						...
						Если ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениАкт
					 		ИЛИ ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениСам Тогда
							СтрокаТекста = НСтр("ru = 'Пеня за несвоевременное перечисление социальных отчислений'");
							Объект.КодНазначенияПлатежа = "017";
						Иначе
							СтрокаТекста = НСтр("ru = 'Социальные отчисления'");
					...
					ИначеЕсли ДокументДляПеречисления.ВидОперации = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеВзносовОСМС Тогда	
						Если ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениАкт
					 		ИЛИ ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениСам Тогда
							СтрокаТекста = НСтр("ru = 'Пеня за несвоевременное перечисление взносов на обязательное социальное медицинское страхование'");
							Объект.КодНазначенияПлатежа = "124";
						Иначе 
							СтрокаТекста = НСтр("ru = 'Взносы ОСМС'");
					...
					ИначеЕсли ДокументДляПеречисления.ВидОперации = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОтчисленийОСМС Тогда	 
						Если ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениАкт
					 		ИЛИ ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениСам Тогда
							СтрокаТекста = НСтр("ru = 'Пеня за несвоевременное перечисление отчислений на обязательное социальное медицинское страхование'");
							Объект.КодНазначенияПлатежа = "123";
						Иначе 
							СтрокаТекста = НСтр("ru = 'Отчисления ОСМС'");
		...
		ПодстрокаТекста = НСтр("ru = '%1 за период %2 г. cогласно прилагаемого списка.'"); 
		...
		Если Объект.ВключатьКомиссиюБанка Тогда
			...
			ПодстрокаТекста = НСтр("ru = '%1 В том числе комиссия банка %2%. Комиссионное вознаграждение банка %3.'");
			СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодстрокаТекста, СтрокаТекста, Объект.ПроцентКомиссии, ОбщегоНазначенияБКВызовСервера.ФорматСумм(Объект.СуммаКомиссии, Объект.ВалютаДокумента, "0-00","", "-", Истина));
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеЕдиногоПлатежа Тогда
		...
		Если (Объект.ПеречислениеЕдиныхПлатежей.Количество() > 0) Тогда
			...
			Если ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениАкт
		 		ИЛИ ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениСам Тогда
				СтрокаТекста = НСтр("ru = 'Пеня за несвоевременную и (или) неполную уплату единого платежа'");
				Объект.КодНазначенияПлатежа = "187";
			Иначе 
				СтрокаТекста = НСтр("ru = 'Единый платеж'");
				Объект.КодНазначенияПлатежа = Справочники.НалогиСборыОтчисления.ЕдиныйПлатеж.КодНазначенияПлатежа;					
			КонецЕсли;
			
		КонецЕсли;
		
		ПодстрокаТекста = НСтр("ru = '%1 за период %2.'"); 
		СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодстрокаТекста, СтрокаТекста, НРег(Формат(ПериодРегистрацииПеречислений, "ДФ='ММММ гггг'")));
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеЗаработнойПлаты
		...
		Если Объект.ВключатьКомиссиюБанка Тогда
			...
			ПодстрокаТекста = НСтр("ru = '%1 %2. В том числе комиссия банка %3%. Комиссионное вознаграждение банка %4.'");  
			СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодстрокаТекста, СтрокаТекста, ОбщегоНазначенияБКВызовСервера.ФорматСумм(СуммаПоВедомостям + Объект.СуммаКомиссии, Объект.ВалютаДокумента, "0-00","", "-", Истина), Объект.ПроцентКомиссии, ОбщегоНазначенияБКВызовСервера.ФорматСумм(Объект.СуммаКомиссии, Объект.ВалютаДокумента, "0-00","", "-", Истина));
		
		Иначе
			ПодстрокаТекста = НСтр("ru = '%1 %2.'");  
			СтрокаТекста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодстрокаТекста, СтрокаТекста, ОбщегоНазначенияБКВызовСервера.ФорматСумм(Объект.СуммаДокумента, Объект.ВалютаДокумента, "0-00","", "-", Истина));
		КонецЕсли;
	...
	Иначе
		...
		АдресБезНДС = Найти(СтрокаТекста, НСтр("ru = 'без НДС'"));
		...
		АдресБезНалога = Найти(СтрокаТекста, НСтр("ru = 'без налога'"));
		...
		Если АвтоподстановкаСуммыНДС = "СуммаИНДС" ИЛИ АвтоподстановкаСуммыНДС = "НДС" Тогда
			...
			Для Каждого СтрокаНДС Из ТабНДС Цикл
				Если Не ЗначениеЗаполнено(СтрокаНДС.СтавкаНДС) Тогда
					ПодтекстШаблон = НСтр("ru = '%1 без налога'");
				...
				ИначеЕсли СтрокаНДС.СтавкаНДС.ДляОсвобожденногоОборота ИЛИ СтрокаНДС.СтавкаНДС.МестоРеализацииНеРК	Тогда
					ПодтекстШаблон = НСтр("ru = '%1 без НДС'");
					ТекстСумма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодтекстШаблон, ТекстСумма);
				Иначе	
...
Процедура ЗаполнитьТекстПолучателя(ЗаполнитьРНН = Истина, ЗаполнитьТекстПолучателя = Истина)
	...
	Если ЗаполнитьРНН Тогда
		...
		ВыводитьРНН = НЕ ЗначениеЗаполнено(ДатаПрекращенияВыводаРНН) ИЛИ Объект.ДатаВыписки < ДатаПрекращенияВыводаРНН;
		
		Если ЗначениеЗаполнено(КонтрагентДляПечати) Тогда
			Если ВыводитьРНН Тогда 
				Объект.РННПолучателя = СведенияОКонтрагентДляПечати.РНН;
			Иначе 
...
Процедура ЗаполнитьТекстОтправителя(ЗаполнитьРНН = Истина, ЗаполнитьТекстОтправителя = Истина)
	...
	Если ЗаполнитьРНН Тогда
		...
		Если ЗначениеЗаполнено(ОрганизацияДляПечати) Тогда
			Если ВыводитьРНН Тогда 
				Объект.РННПлательщика = СведенияОбОрганизацииДляПечати.РНН;
			Иначе 
...
Процедура ВидОперацииПриИзмененииНаСервере()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога ИЛИ
		...
		Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНДССИзмененнымСрокомУплаты Тогда
...
Процедура ОрганизацияПриИзмененииНаСервере(СтруктураПараметров, СтруктураРезультатаВыполнения)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПереводНаДругойСчет
		...
		Если ПоддержкаРаботыСоСтруктурнымиПодразделениями И ЗначениеЗаполнено(СтруктурноеПодразделениеОрганизацияПолучатель) И СтруктурноеПодразделениеОрганизацияПолучатель.Владелец <> Объект.Организация Тогда
			...
			ТекстСообщения  = НСтр("ru = 'Значение в поле ""Структурная единица - получатель"" не соответствует выбранной организации и было измнено'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Функция ПолучитьСтруктуруПриИзмененииДокументаСОНаСервере(Документ)
	...
	Если ВидОперацииВедомости = ПредопределенноеЗначение("Перечисление.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОбязательныхСоциальныхОтчислений") Тогда
		СтруктураВозврата.ВидВзноса = "СО";
	ИначеЕсли ВидОперацииВедомости = ПредопределенноеЗначение("Перечисление.ВидыОперацийСОПеречислениеВФонды.ПеречислениеВзносовОСМС") Тогда
		СтруктураВозврата.ВидВзноса = "ВОСМС";
	ИначеЕсли ВидОперацииВедомости = ПредопределенноеЗначение("Перечисление.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОтчисленийОСМС") Тогда
		СтруктураВозврата.ВидВзноса = "ООСМС";
	КонецЕсли;
