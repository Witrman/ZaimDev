	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.ФормаВыбораИзКлассификатора") Тогда
		...
		Если ВРег(ИсточникВыбора.ИмяМакета) = ВРег("КодыБюджетнойКлассификации") Тогда 
			...
			Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") Тогда
				ЭтотОбъект.КодБК = ВыбранноеЗначение;
			Иначе
				ЭтотОбъект.КодБК = ВыбранноеЗначение.КодСтроки;
			КонецЕсли;
...
Процедура УстановитьЗаголовокНадписиРНН(Форма)
	...
	Если ВыводитьРНН Тогда 
		Элементы.НадписьРННПлательщика.Заголовок = "РНН:";
		Элементы.НадписьРННПолучателя.Заголовок  = "РНН:";
	Иначе 
		Элементы.НадписьРННПлательщика.Заголовок = "БИН / ИИН:";
		Элементы.НадписьРННПолучателя.Заголовок  = "БИН / ИИН:";
	КонецЕсли;
