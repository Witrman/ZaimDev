...
Процедура ЗаполнитьПоДокументуОснованию(Объект, ДокументОснование) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗарплатаКВыплатеОрганизаций")
			  ...
			  ИЛИ ТипЗНЧ(ДокументОснование) = Тип("ДокументСсылка.РасчетНалоговПриПоступленииАктивовУслуг") Тогда
...
Процедура ЗаполнитьДокументПоОснованиюРегл(Объект, ДокументОснование)
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РасчетПениОПВиСО") Тогда
		...
		Пока ВыборкаМесяцНалоговогоПериода.Следующий() Цикл
			...
			НовыйДок.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Создан автоматически при перечислении пени по документу %1'"), Строка(ДокументОснование));
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РасчетНалоговПриПоступленииАктивовУслуг") Тогда
		...
		Объект.СуммаДокумента = ДокументОснование.СуммаДокумента - СуммаОПВ - СуммаИПН - СуммаВОСМС;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("СодержаниеКомиссия"     , НСтр("ru = 'Комиссия банка'"));
	Запрос.УстановитьПараметр("СодержаниеНДС"          , НСтр("ru = 'НДС, уплаченный в бюджет принят к зачету'"));
	Запрос.УстановитьПараметр("СодержаниеПрочее"       , НСтр("ru = 'Прочее списание денежных средств'"));
	Запрос.УстановитьПараметр("СодержаниеНалог"        , НСтр("ru = 'Перечисление налога'"));
	Запрос.УстановитьПараметр("СодержаниеПеревод"      , НСтр("ru = 'Перевод на другой счет организации'"));
	Запрос.УстановитьПараметр("СодержаниеПрочиеРасчеты", НСтр("ru = 'Расчеты с контрагентом'"));
	Запрос.УстановитьПараметр("СодержаниеРасчетыКредит", НСтр("ru = 'Расчеты по кредитам и займам'"));
	Запрос.УстановитьПараметр("СодержаниеПодотчет"     , НСтр("ru = 'Перечисление средств подотчетнику'"));
	...
	Если Реквизиты.ЕстьПеречислениеЗаработнойПлаты ИЛИ
		 ...
		 Реквизиты.ЕстьПеречислениеЕдиногоПлатежа Тогда
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьРеестра(МассивОбъектов, ОбъектыПечати),
			,
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьСпискаНаКартСчета(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ПечатьПлатежногоПоручения(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ДанныеДокументов.Следующий() Цикл
		...
		Если МесяцПрописью = 1 Тогда 
			ДатаДокумента = СтрЗаменить(Формат(ДанныеДокументов.Дата, ФорматДаты), "г.", "года"); 
		Иначе 
			ДатаДокумента = Формат(ДанныеДокументов.Дата, ФорматДаты) + " года"; 
		КонецЕсли;
		...
		ОбластьМакета.Параметры.ДатаВалютирования	  		= Формат(ДанныеДокументов.ДатаВалютирования, "ДФ=dd.MM.yyyy");
		ОбластьМакета.Параметры.ДатаПолученияТовара  		= ?(ЗначениеЗаполнено(ДанныеДокументов.ДатаПолученияТовара), СтрЗаменить(Формат(ДанныеДокументов.ДатаПолученияТовара, "ДЛФ=DD"),"г.", "года"),"");
...
Функция ПечатьРеестра(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПлатежноеПоручениеИсходящее.ПФ_MXL_Реестр");
	...
	Пока ВыборкаПоРегистратору.Следующий() Цикл
		...
		Если ВыборкаПоРегистратору.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеПенсионныхВзносов Тогда
			Если ВыборкаПоРегистратору.ВидОперацииОПВ = Перечисления.ВидыОперацийОПВПеречислениеВФонды.ПеречислениеОбязательныхПрофессиональныхПенсионныхВзносов Тогда
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление обязательных профессиональных пенсионных взносов'");
			ИначеЕсли ВыборкаПоРегистратору.ВидОперацииОПВ = Перечисления.ВидыОперацийОПВПеречислениеВФонды.ПеречислениеОбязательныхПенсионныхВзносовРаботодателя Тогда
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление обязательных пенсионных взносов работодателя'");
			Иначе
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление обязательных пенсионных взносов'");
			КонецЕсли;
		...
		Иначе
			Если ВыборкаПоРегистратору.ВидОперацииСО = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОбязательныхСоциальныхОтчислений Тогда
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление социальных отчислений'");
			ИначеЕсли ВыборкаПоРегистратору.ВидОперацииСО = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеВзносовОСМС Тогда
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление взносов обязательного социального медицинского страхования'");
			ИначеЕсли ВыборкаПоРегистратору.ВидОперацииСО = Перечисления.ВидыОперацийСОПеречислениеВФонды.ПеречислениеОтчисленийОСМС Тогда
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление отчислений обязательного социального медицинского страхования'");
			ИначеЕсли ВыборкаПоРегистратору.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеЕдиногоПлатежа Тогда
				ОбластьШапки.Параметры.Наименование  = НСтр("ru = 'Список на перечисление единого платежа'");
			КонецЕсли;
		...
		ОбластьПодвала.Параметры.СуммаПрописью   = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(ВыборкаПоДокументам.Сумма, ОбщегоНазначенияБКВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета());
		...
		ОбластьПодвала.Параметры.ГлавныйБухгалтер = ?(ПустаяСтрока(ОтветЛица.ГлавныйБухгалтерДляПодписиБанковскихДокументов),  НСтр("ru = 'Не предусмотрен'"), ОтветЛица.ГлавныйБухгалтерДляПодписиБанковскихДокументов);
...
Функция ПечатьСпискаНаКартСчета(МассивОбъектов, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПлатежноеПоручениеИсходящее.ПФ_MXL_СписокПеречисленияНаКартСчета");
	...
	Пока ВыборкаПоРегистратору.Следующий() Цикл
		...
		Если ВыборкаПоРегистратору.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеЗаработнойПлаты Тогда
			...
			ОбластьШапка.Параметры.НазначениеПлатежа 	= НСтр("ru = 'на перечисление заработной платы'");
			ОбластьШапка.Параметры.ОписаниеПериода 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'за %1'"),
		...
		ИначеЕсли ВыборкаПоРегистратору.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеДенежныхСредствПодотчетнику Тогда
			...
			ОбластьШапка.Параметры.НазначениеПлатежа 	= НСтр("ru = 'на перечисление денежных средств в подотчет'");
		...
		ОбластьПодписи.Параметры.ИтогоСуммаКПеречислениюПрописью = ОбщегоНазначенияБКВызовСервера.СформироватьСуммуПрописью(ИтогоСуммаКПеречислению, ВыборкаПоРегистратору.ВалютаДенежныхСредств);
...
Функция СформироватьЗапросДляПечатиОПВиСО(Режим, МассивСсылок)
	...
	ИначеЕсли Режим = "ПоТабличнойЧасти" Тогда
		...
		ТекстЗапроса = 
			...
			|	НАЧАЛОПЕРИОДА(СОПеречислениеВФонды.Ссылка.ПериодРегистрации, МЕСЯЦ) КАК ПериодРегистрации,
...
Функция СформироватьЗапросДляПечатиПеречислениеЗарплатыПодотчет(Режим, МассивСсылок)
	...
	ИначеЕсли Режим = "ПоТабличнойЧасти" Тогда
		...
		ТекстЗапроса = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ППИ.Ссылка,
			|	ППИ.Ссылка.Дата КАК ДатаДокумента,
			...
			|		ИНАЧЕ """"
			|	КОНЕЦ КАК ФИОРаботника,
			|	ЗарплатаКВыплате.Физлицо.Наименование КАК ФизЛицоНаименование,
			|	ЗарплатаКВыплате.НомерКартСчета.НомерСчета КАК НомерКартСчета,
			|	СУММА(ЗарплатаКВыплате.Сумма + ЗарплатаКВыплате.ДепонированнаяСумма) КАК СуммаКВыплате
			|ПОМЕСТИТЬ ВТ_ЗарплатаПеречисление
			|ИЗ
			...
			|ГДЕ
			|	ППИ.Ссылка В(&МассивОбъектов)
			|	И ППИ.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийППИсходящее.ПеречислениеЗаработнойПлаты)
			|
			|СГРУППИРОВАТЬ ПО
			|	ППИ.Ссылка,
			|	ЗарплатаКВыплате.Физлицо,
			...
			|		ИНАЧЕ """"
			|	КОНЕЦ,
			|	ЗарплатаКВыплате.Физлицо.Наименование,
			...
			|	ПеречислениеВПодотчет.Ссылка.Дата КАК ДатаДокумента,
			...
			|		ИНАЧЕ """"
			|	КОНЕЦ КАК ФИОРаботника,
			|	ПеречислениеВПодотчет.ФизЛицо.Наименование КАК ФизЛицоНаименование,
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			|			МАКСИМУМ(ФИОФизЛиц.Период),
			|			ЗарплатаПеречисление.Ссылка,
			|			ЗарплатаПеречисление.Физлицо
			|		ИЗ
			|			ВТ_ЗарплатаПеречисление КАК ЗарплатаПеречисление
			|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц КАК ФИОФизЛиц
			|				ПО ЗарплатаПеречисление.Физлицо = ФИОФизЛиц.ФизЛицо
			|					И ЗарплатаПеречисление.ДатаДокумента >= ФИОФизЛиц.Период
			|		
			|		СГРУППИРОВАТЬ ПО
			|			ЗарплатаПеречисление.Ссылка,
			|			ЗарплатаПеречисление.Физлицо
			...
			|			ПеречислениеВПодотчет.ФизЛицо
			|		ИЗ
			|			ВТ_ПеречислениеВПодотчет КАК ПеречислениеВПодотчет
			|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц КАК ФИОФизЛиц
			|				ПО ПеречислениеВПодотчет.ФизЛицо = ФИОФизЛиц.ФизЛицо
			|					И ПеречислениеВПодотчет.ДатаДокумента >= ФИОФизЛиц.Период
			|		
			|		СГРУППИРОВАТЬ ПО
			|			ПеречислениеВПодотчет.Ссылка,
			|			ПеречислениеВПодотчет.ФизЛицо) КАК ДанныеДокументовПоПериодам
			|		ПО ФИОФизЛиц.ФизЛицо = ДанныеДокументовПоПериодам.ФизЛицо
			|			И ФИОФизЛиц.Период = ДанныеДокументовПоПериодам.Период
			|
			|ИНДЕКСИРОВАТЬ ПО
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ЗарплатаПеречисление.Ссылка КАК Ссылка,
			|	ЗарплатаПеречисление.Физлицо,
			|	ЗарплатаПеречисление.РННРаботника,
			|	ЗарплатаПеречисление.ИИНРаботника,
			|	ЗарплатаПеречисление.СуммаКВыплате,
			|	ВЫБОР
			|		КОГДА ЗарплатаПеречисление.ФИОРаботника = """"
			|			ТОГДА ЕСТЬNULL(ФИОФизЛиц.Фамилия + "" "" + ФИОФизЛиц.Имя + "" "" + ФИОФизЛиц.Отчество, ЗарплатаПеречисление.ФизЛицоНаименование)
			|		ИНАЧЕ ЗарплатаПеречисление.ФИОРаботника
			|	КОНЕЦ КАК ФИОРаботника,
			|	ЗарплатаПеречисление.НомерКартСчета
			|ИЗ
			|	ВТ_ЗарплатаПеречисление КАК ЗарплатаПеречисление
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ФИОФизЛиц КАК ФИОФизЛиц
			|		ПО (ФИОФизЛиц.Ссылка = ЗарплатаПеречисление.Ссылка)
			|			И (ФИОФизЛиц.ФизЛицо = ЗарплатаПеречисление.Физлицо)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ПеречислениеВПодотчет.Ссылка,
			|	ПеречислениеВПодотчет.ФизЛицо,
			|	ПеречислениеВПодотчет.РННРаботника,
			|	ПеречислениеВПодотчет.ИИНРаботника,
			|	ПеречислениеВПодотчет.СуммаКВыплате,
			|	ВЫБОР
			|		КОГДА ПеречислениеВПодотчет.ФИОРаботника = """"
			|			ТОГДА ЕСТЬNULL(ФИОФизЛиц.Фамилия + "" "" + ФИОФизЛиц.Имя + "" "" + ФИОФизЛиц.Отчество, ПеречислениеВПодотчет.ФизЛицоНаименование)
			|		ИНАЧЕ ПеречислениеВПодотчет.ФИОРаботника
			|	КОНЕЦ,
			|	ПеречислениеВПодотчет.НомерКартСчета
			|ИЗ
			|	ВТ_ПеречислениеВПодотчет КАК ПеречислениеВПодотчет
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ФИОФизЛиц КАК ФИОФизЛиц
			|		ПО (ФИОФизЛиц.Ссылка = ПеречислениеВПодотчет.Ссылка)
			|			И (ФИОФизЛиц.ФизЛицо = ПеречислениеВПодотчет.ФизЛицо)
			|
			|УПОРЯДОЧИТЬ ПО
			|	Ссылка,
			|	ФИОРаботника
