...
Процедура ПроверитьКорректностьЗначенияФОИспользоватьНесколькоБанковскихСчетовОрганизации(Организация)
	...
	Если ИсправитьЗначениеФункциональнойОпции Тогда 
		...
		ТекстСообщения = НСтр("ru = 'Скорректирована запись для %Организация% в регистре сведений ""Использовать несколько банковских счетов организации""'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Организация%", Организация);
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка состояния функциональной опции'"), УровеньЖурналаРегистрации.Предупреждение,
