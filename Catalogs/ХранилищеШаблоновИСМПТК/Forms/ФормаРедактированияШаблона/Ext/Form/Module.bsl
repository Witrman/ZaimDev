...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СхемаКомпоновкиДанных = Неопределено Тогда
		СхемаКомпоновкиДанных = Обработки.ПечатьКодовМаркировкиИСМПТК.ПолучитьМакет("ПоляШаблонаИСМП");
	КонецЕсли;
	
	ЭтаФорма.Заголовок = НСтр("ru = 'Редактирование шаблона этикетки'");
	Элементы.ГруппаШаблоныПоУмолчанию.Видимость = Ложь;
	Элементы.ФормаЗагрузитьШаблонИСМППоУмолчанию.Видимость = Истина;
	...
	НеGS1 = Параметры.Ссылка.НеGS1;
	
	Если СтруктураШаблона <> Неопределено Тогда
		// Загрузка ранее сохраненного шаблона
		...
		СтруктураШаблона.Свойство("ТипКода"						, ?(НеGS1, 18, 24)); //18 - DataMatrix, 24 - GS1 DataMatrix
		...
		ТипКода = ?(ЭтаФорма.НеGS1, 18, 24);
	Иначе
		
		//Создание нового со значениями по умолчанию
		ЗаполнитьНовыйШаблонПоУмолчанию();
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если СохранятьИзменения Тогда
		Если ПолеТабличногоДокумента.ОбластьПечати = Неопределено Тогда
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(НСтр("ru = 'Не установлена область печати'"),
																				   ЭтаФорма,
		...
		Если РазрешитьАвтовысотуСтрок ИЛИ НЕ НаличиеСтрокСАвтовысотой Тогда
			...
			Иначе
				ДополнительныеПараметры = Новый Структура("АдресШаблона", АдресШаблона);
				ОбработчикОповещения 	= Новый ОписаниеОповещения("ОповещениеВопросаОНеСоответствииКоличества",
																   ЭтотОбъект,
																   ДополнительныеПараметры);
					
				ПоказатьВопрос(ОбработчикОповещения,
							   НСтр("ru = 'Превышено максимальное количество на странице. Сохранить изменения?'"),
	...
	ИначеЕсли Модифицированность Тогда
		...
		ОбработчикОповещения = Новый ОписаниеОповещения("ОповещениеВопросаОЗакрытии",
													    ЭтотОбъект,
														ДополнительныеПараметры);
		ПоказатьВопрос(ОбработчикОповещения,
...
Процедура ПриОткрытии(Отказ)
	...
	ЗаполнитьПредставлениеТипаКода();
...
&НаКлиенте
Процедура ОповещениеЗаполнитьШаблонИСМППоУмолчанию(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		ОповещениеОВыбореШаблона = Новый ОписаниеОповещения("ОбработатьВыборШаблонаЭтикетки", ЭтаФорма);
		ОткрытьФорму("Справочник.ХранилищеШаблоновИСМПТК.Форма.ФормаВыбораШаблона",,ЭтотОбъект, Истина,,,ОповещениеОВыбореШаблона, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ОповещениеВопросаОНеСоответствииКоличества(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.ОК Тогда
		Оповестить("ИзмененШаблон", ДополнительныеПараметры.АдресШаблона, ЭтаФорма);
		Модифицированность = Ложь;
		Закрыть();
	КонецЕсли;
...
Процедура ТипКодаПриИзменении(Элемент)
	...
	Элементы.УровеньКоррекцииQR.Видимость = (ТипКода = 16 ИЛИ ТипКода = 99 ИЛИ ТипКода = 17 ИЛИ ТипКода = 18);
...
&НаКлиенте
Процедура ЗагрузитьШаблонИСМППоУмолчанию(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполнитьШаблонИСМППоУмолчанию", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Редактируемый шаблон будет заменен на выбранный стандартный шаблон, продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура СохранитьЗакрыть(Команда)
	...
	СохранятьИзменения = Модифицированность;
	Закрыть();
...
Процедура ПроверитьРазмещение(Отказ)
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Лево = 0 
		 И ПолеТабличногоДокумента.ОбластьПечати.Право = 0) Тогда
		...
		Если КоличествоПоГоризонтали <> МассивТаблиц.Количество() Тогда
			...
			СообщениеПоГоризонтали = НСтр("ru = 'Максимальное количество по горизонтали:'") + " " + МассивТаблиц.Количество();
			
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(СообщениеПоГоризонтали,,
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Верх = 0 
		 И ПолеТабличногоДокумента.ОбластьПечати.Низ = 0) Тогда
		...
		Если КоличествоПоВертикали <> МассивТаблиц.Количество() Тогда
			...
			СообщениеПоВертикали = НСтр("ru = 'Максимальное количество по вертикали:'") +" " + МассивТаблиц.Количество();
			
			ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(СообщениеПоВертикали,,
...
Процедура УстановитьКартинкуШтрихкода(РисунокШтрихкода)
	...
	Эталон = Справочники.ХранилищеШаблоновИСМПТК.ПолучитьМакет("ЭталонШтрихкода");
	КоличествоМиллиметровВПикселе = Эталон.Рисунки.Квадрат100Пикселей.Высота / 100;
	
	ПримерШтрихкода = Неопределено;
	Коды = ПолучитьКоды();
	Для Каждого СтрокаТЧ Из Коды Цикл
		Если СтрокаТЧ.ТипКода = 24 Тогда
	...
	РазборИОбработкаКодовМаркировкиИСМПТКСлужебный.КодGS1ДляКомпонентыПечати(ЗначениеШтрихкодаДляКомпоненты, 24);
	...
	ПараметрыШтрихкода.Вставить("ТипКода", 24); //По умолчанию картинка во всех шаблонах генерируется в формате GS1-DataMatrix
	...
	РезультатГенерацииШтрихкода = ПечатьКодовМаркировкиИСМПТК.ИзображениеШтрихкода(ПараметрыШтрихкода);
	...
	Иначе
		ИнтеграцияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(НСтр("ru = 'При генерации картинки штрихкода произошла ошибка.'"));
	КонецЕсли;
...
Функция ПолучитьКоды()
	...
	Возврат Справочники.ХранилищеШаблоновИСМПТК.ПолучитьОбразцыШтрихкодов();
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяПараметраШтрихАвтопроверки()
...
Процедура ВыбратьДоступнуюСтрокуВШаблон(ВыбраннаяСтрока)
	...
	Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод() Или ИмяПоляВШаблоне = ИмяПараметраШтрихАвтопроверки() Тогда
...
Процедура ПоместитьВТабличныйДокументШаблонПоУмолчанию(ИмяШаблона)
	...
	ШаблонПоУмолчанию = Справочники.ХранилищеШаблоновИСМПТК.ПолучитьМакет(ИмяШаблона);
...
Процедура ЗагрузкаШаблонаИзФайлаЗавершениеСервер(АдресФайла)
	...
	ПолеТабличногоДокумента.Прочитать(ИмяПромежуточногоФайла);
...
&НаКлиенте
Процедура ОбработатьВыборШаблонаЭтикетки(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПоместитьВТабличныйДокументШаблонПоУмолчанию(Результат);
	ТипКода = ?(ЭтаФорма.НеGS1, 18, 24);
	Модифицированность = Истина;
...
&НаСервере
Процедура ЗаполнитьНовыйШаблонПоУмолчанию() 
	
	ПоместитьВТабличныйДокументШаблонПоУмолчанию("ШаблонЭтикетки_58х40_Пустой");
	ТипКода = ?(ЭтаФорма.НеGS1, 18, 24);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПредставлениеТипаКода()
	
	ТипКодаПредставление = ПечатьКодовМаркировкиИСМПТККлиент.ЗаполнитьПредставлениеТипаКода(ТипКода);
