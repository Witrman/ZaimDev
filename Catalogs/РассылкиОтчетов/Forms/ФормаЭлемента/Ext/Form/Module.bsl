...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Рассылки_ЗакрытиеОтчета" Тогда
		Если Параметр.ИдентификаторФормыРассылки = УникальныйИдентификатор Тогда
...
Процедура АвторПриИзменении(Элемент)
	...
	ПолучитьСписокПочтовыхАдресов(Объект.Автор, НовыйСписок);
...
Процедура ПредварительныйПросмотрОтчетаПродолжение(РезультатВыбора, ПараметрыОтчета) Экспорт
	...
	Если Объект.Персонализирована Тогда
		...
		Для Каждого ПользовательскаяНастройкаКД Из ПерсонализированныеНастройки Цикл
			...
			ИначеЕсли ТипЗнч(ПользовательскаяНастройкаКД) = Тип("Структура") Тогда
				ПользовательскаяНастройкаКД.Значение = Получатель;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	ПараметрыФормы.Вставить("ОткрытИзРассылки", Истина);
	ПараметрыФормы.Вставить("РассылкаОтчетов", Объект.Ссылка);
	ПараметрыФормы.Вставить("АдресНастроек", "");
	ПараметрыФормы.Вставить("ИдентификаторСтрокиОтчета", ИдентификаторТекущейСтрокиТаблицыОтчетов);
	ПараметрыФормы.Вставить("ИдентификаторФормыРассылки", УникальныйИдентификатор);
	ПараметрыФормы.Вставить("ПропуститьПроверкуЗаполнения", Истина);
...
Функция ПочтовыеАдресаПолучателя(Получатель, СписокЗначений)
	...
	Возврат ПолучитьСписокПочтовыхАдресов(Получатель, СписокЗначений);
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	РежимВариантаОтчета = Неопределено;
	ПараметрыОтчета.Свойство("РежимВариантаОтчета", РежимВариантаОтчета);
	Если РежимВариантаОтчета = Неопределено Тогда
		РежимВариантаОтчета = Истина;
	КонецЕсли;
	
	// Восстановление настроек
	Если СтрокаОтчеты.СКД Тогда
		ПараметрПериодРассылки = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПараметр(ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки, "ПериодОтчета");
		Если НЕ РежимВариантаОтчета Тогда
			Для Каждого ЭлементНастроек Из ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки.Элементы Цикл
				Если ЭлементНастроек = ПараметрПериодРассылки Тогда
					Продолжить;
				КонецЕсли;
				ЭлементНастроек.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
			КонецЦикла;

			Если ПараметрПериодРассылки <> Неопределено Тогда
				ПараметрПериодРассылки.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
			КонецЕсли;
			Элементы.ПользовательскиеНастройки.РежимОтображения = РежимОтображенияНастроекКомпоновкиДанных.БыстрыйДоступ;
		Иначе
...
Функция ПолучитьСписокПочтовыхАдресов(Получатель, СписокЗначений = Неопределено)
	...
	|	ВЫРАЗИТЬ(тчКонтактнаяИнформация.Представление КАК СТРОКА(100)) КАК Представление,
	|	тчКонтактнаяИнформация.Вид КАК Значение
	|ИЗ
	|	РегистрСведений.КонтактнаяИнформация КАК тчКонтактнаяИнформация
	|ГДЕ
	|	тчКонтактнаяИнформация.Объект = &Получатель
	|	И тчКонтактнаяИнформация.Тип = &ТипКИ";
...
Процедура ОбработкаОповещенияНаСервере(Параметр)
	...
	РассылкаОбъект = РеквизитФормыВЗначение("Объект");
	
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		Если СтрокаОтчеты.ПолноеИмя = Параметр.ПолноеИмя Тогда
			РассылкаОбъектСтрока = РассылкаОбъект.Отчеты.Найти(СтрокаОтчеты.Отчет, "Отчет");
			Если РассылкаОбъектСтрока <> Неопределено Тогда
				НовыеНастройки  = ПолучитьИзВременногоХранилища(Параметр.АдресНастроек);
				СтарыеНастройки = РассылкаОбъектСтрока.Настройки.Получить();
				Если Объект.Персонализирована И ПолучателиУказаны(Объект.Получатели) Тогда
					Если НЕ СтрокаОтчеты.СКД И (ТипЗнч(СтарыеНастройки) = Тип("ТаблицаЗначений")) Тогда
						СтарыеНастройкиМассив = Новый Массив;
						Найденные = СтарыеНастройки.НайтиСтроки(Новый Структура("Использование", Истина));
						Для Каждого СтрокаНастройка Из Найденные Цикл
							ДобавляемаяНастройка = Новый Структура("Реквизит, Значение", СтрокаНастройка.Реквизит, СтрокаНастройка.Значение);
							СтарыеНастройкиМассив.Добавить(ДобавляемаяНастройка);
						КонецЦикла;
						Фильтр = Новый Структура("Использование, Значение", Истина, "[Получатель]");
						ПерсонализированныеНастройки = ОтчетыКлиентСервер.ЭлементыНастроекОтобранные(СтарыеНастройкиМассив, Фильтр);
						Для Каждого ПерсонНастройка Из ПерсонализированныеНастройки Цикл
							НайденнаяНоваяНастройка = НовыеНастройки.Найти(ПерсонНастройка.Реквизит, "Реквизит");
							Если НайденнаяНоваяНастройка <> Неопределено Тогда
								НайденнаяНоваяНастройка.Значение = "[Получатель]";
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
				РассылкаОбъектСтрока.Настройки = Новый ХранилищеЗначения(НовыеНастройки, Новый СжатиеДанных(9));
				СтрокаОтчеты.АдресНастроек = Параметр.АдресНастроек;		

				СтрокаПредупреждения = ОтчетыПриАктивизацииСтрокиНаСервере(СтрокаОтчеты.НомерСтроки-1, Истина);
				Если СтрокаПредупреждения <> "" Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаПредупреждения, , "Объект.Отчеты["+ (СтрокаОтчеты.НомерСтроки-1) +"].Представление");
				Иначе
					Модифицированность = Истина;
	...
	ИдентификаторСтроки = Параметр.ИдентификаторСтрокиОтчета;
	//НовыеПользовательскиеНастройки = ПолучитьИзВременногоХранилища(Параметр.АдресНастроекХ);
	СтрокаОтчеты = Объект.Отчеты.НайтиПоИдентификатору(ИдентификаторСтроки);
	ИндексСтроки = Объект.Отчеты.Индекс(СтрокаОтчеты);
	Элементы.Отчеты.ТекущаяСтрока = ИдентификаторСтроки;
	
	//Если СтрокаОтчеты <> Неопределено Тогда
	//	СтрокаОтчеты.АдресНастроек = Параметр.АдресНастроекХ;
	//КонецЕсли;
	
	//ОбъектСтрокаОтчеты = РеквизитФормыВЗначение("Объект").Отчеты.Получить(ИндексСтроки);
	//
	//Если ОбъектСтрокаОтчеты <> Неопределено Тогда
	//	ОбъектСтрокаОтчеты.АдресНастроек = Параметр.АдресНастроекХ;
	//КонецЕсли;
	
	//КомпоновщикНастроекКД.ЗагрузитьПользовательскиеНастройки(ПолучитьИзВременногоХранилища(Параметр.АдресНастроекХ));
	
	//СтрокаПредупреждения = ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, Истина, НовыеПользовательскиеНастройки);
	СтрокаПредупреждения = ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, Истина);
	Если СтрокаПредупреждения <> "" Тогда
