...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		// Получатели рассылки хранятся в справочнике "Пользователи".
		Объект.ТипПолучателейРассылки = ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Справочник.Пользователи");
		...
		СтрокаТипПолучателей = ТаблицаТиповПолучателей.НайтиСтроки(Новый Структура("ИдентификаторОбъектаМетаданных", Объект.ТипПолучателейРассылки));
		Если СтрокаТипПолучателей <> Неопределено Тогда
			ТипПолучателейРассылки = СтрокаТипПолучателей[0].ТипПолучателей;
			Объект.ВидПочтовогоАдресаПолучателей = СтрокаТипПолучателей[0].ОсновнойВидКИ;
		КонецЕсли;
		...
		ПолучательПоУмолчанию = Объект.Получатели.Добавить();
		ПолучательПоУмолчанию.Получатель = Пользователи.ТекущийПользователь();
		
		ПредставлениеПолучателей = ПредставлениеПолучателей(ЭтотОбъект);
		ПолучателиРассылки = ПредставлениеПолучателей.Краткое;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтоНовый Или ИзмененПериод Или ОбновитьНастройкиОтчетаВРассылке Тогда
		...
		Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
			...
			Если НЕ ЭтоНовый И (ИзмененПериод Или ОбновитьНастройкиОтчетаВРассылке) Тогда
			...
			Если ПользовательскиеНастройки <> Неопределено Тогда
				...
				ИначеЕсли ТипЗнч(ПользовательскиеНастройки) = Тип("ХранилищеЗначения") Тогда
					ПользовательскиеНастройки = ПользовательскиеНастройки.Получить();
					Если ТипЗнч(ПользовательскиеНастройки) = Тип("Структура") Тогда
						ПользовательскиеНастройкиТаблицаЗначений = Новый ТаблицаЗначений;
						ПользовательскиеНастройкиТаблицаЗначений.Колонки.Добавить("Значение");
						ПользовательскиеНастройкиТаблицаЗначений.Колонки.Добавить("Использование");
						ПользовательскиеНастройкиТаблицаЗначений.Колонки.Добавить("Представление");
						ПользовательскиеНастройкиТаблицаЗначений.Колонки.Добавить("Реквизит");
						Для Каждого КлючИЗначение Из ПользовательскиеНастройки Цикл
							ПользовательскиеНастройкиСтрокаТЗ = ПользовательскиеНастройкиТаблицаЗначений.Добавить();
							ПользовательскиеНастройкиСтрокаТЗ.Реквизит = КлючИЗначение.Ключ;
							ПользовательскиеНастройкиСтрокаТЗ.Значение = КлючИЗначение.Значение;
							ПользовательскиеНастройкиСтрокаТЗ.Представление = КлючИЗначение.Ключ;
							ПользовательскиеНастройкиСтрокаТЗ.Использование = Истина;
						КонецЦикла;
						ПользовательскиеНастройки = ПользовательскиеНастройкиТаблицаЗначений;
					КонецЕсли;
				КонецЕсли;
			...
			КлючОбъекта = ОбъектСтрокаОтчеты.Отчет.Отчет.ПолноеИмя + "/" + ОбъектСтрокаОтчеты.Отчет.КлючВарианта;
			ПользовательскиеНастройки_ = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить(КлючОбъекта + "/ТекущиеПользовательскиеНастройки",, Неопределено);
			
			ПользовательскиеНастройкиРассылка_ = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить(КлючОбъекта, "НастройкиРассылки", Неопределено);
			
			Если ПользовательскиеНастройки_ <> Неопределено Тогда
				ДополнительныеСвойства = ПользовательскиеНастройки_.ДополнительныеСвойства;
			...
			Если ПользовательскиеНастройкиРассылка_ <> Неопределено Тогда
				ДополнительныеСвойстваРассылка = ПользовательскиеНастройкиРассылка_.ДополнительныеСвойства;
				Если ДополнительныеСвойстваРассылка.Свойство("ДанныеОтчетаРассылка") И ТипЗнч(ДополнительныеСвойстваРассылка.ДанныеОтчетаРассылка) = Тип("ХранилищеЗначения") Тогда
					НастройкиОтчета_БК = ДополнительныеСвойстваРассылка.ДанныеОтчетаРассылка.Получить();
				КонецЕсли;
			КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если СтрокаОтчеты <> Неопределено Тогда
		ПериодОтчетаИзНастроек = ПериодОтчетаИзПользовательскихНастроек(СтрокаОтчеты, Элементы.ВидПериодаОтчета.СписокВыбора);
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Иначе
			...
			Если ОбъектСтрокаОтчеты <> Неопределено Тогда
				КлючОбъектаПроверка = ОбъектСтрокаОтчеты.Отчет.Отчет.ПолноеИмя + "/" + ОбъектСтрокаОтчеты.Отчет.КлючВарианта;
				ПользовательскиеНастройкиРассылка_ = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить(КлючОбъектаПроверка, "НастройкиРассылки", Неопределено);
			КонецЕсли;
			...
			Если ПараметрНастройкиОтчета = Неопределено Тогда
				Если ТипЗнч(НастройкиОтчета) = Тип("ПользовательскиеНастройкиКомпоновкиДанных") Тогда
					НастройкиОтчета.ДополнительныеСвойства.Свойство("НастройкиОтчета", ПараметрНастройкиОтчета);
				ИначеЕсли ТипЗнч(НастройкиОтчета) = Тип("Структура") Тогда
					ПараметрНастройкиОтчета = Новый ХранилищеЗначения(НастройкиОтчета, Новый СжатиеДанных(9));
				КонецЕсли;
			КонецЕсли;

			Если ПараметрНастройкиОтчета <> Неопределено Тогда
				Если ТипЗнч(ПараметрНастройкиОтчета) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
					СтрокаОтчеты.АдресНастроек = ПоместитьВоВременноеХранилище(ПараметрНастройкиОтчета.Значение, Адрес);
				Иначе
					СтрокаОтчеты.АдресНастроек = ПоместитьВоВременноеХранилище(ПараметрНастройкиОтчета, Адрес);
				КонецЕсли;
			КонецЕсли;
...
&НаСервере
Функция ПериодОтчетаИзПользовательскихНастроек(СтрокаОтчеты, СписокПериодов)
	...
	Если ТипЗнч(ПользовательскиеНастройкиКД) = Тип("ХранилищеЗначения") Тогда
		ПользовательскиеНастройкиКД = ПользовательскиеНастройкиКД.Получить();
	ИначеЕсли ТипЗнч(ПользовательскиеНастройкиКД) = Тип("ТаблицаЗначений") Тогда
		Если ПользовательскиеНастройкиКД.Колонки.Найти("Реквизит") <> Неопределено
			И ПользовательскиеНастройкиКД.Колонки.Найти("Значение") <> Неопределено Тогда
			НастройкиСтруктура = Новый Структура;
			Для Каждого НастройкиСтрока Из ПользовательскиеНастройкиКД Цикл
				НастройкиСтруктура.Вставить(НастройкиСтрока.Реквизит, НастройкиСтрока.Значение);
			КонецЦикла;
			ПользовательскиеНастройкиКД = НастройкиСтруктура;
		КонецЕсли;
	КонецЕсли;

	ЗначениеПараметра = Неопределено;
	Если ТипЗнч(ПользовательскиеНастройкиКД) = Тип("Структура") И ПользовательскиеНастройкиКД.Свойство("ПериодОтчета") Тогда
		ЗначениеПараметра = Новый Структура("Значение", ПользовательскиеНастройкиКД.ПериодОтчета);
	Иначе
...
Функция НастройкиВРассылкеОтличаютсяОтНастроекОтчета(АдресНастроекРассылки, АдресНастроекОтчета)
	...
	Если НЕ ЭтоАдресВременногоХранилища(АдресНастроекОтчета) ИЛИ НЕ ЭтоАдресВременногоХранилища(АдресНастроекРассылки) Тогда
	...
	Возврат НЕ РассылкаОтчетовБК.КоллекцииНастроекОтчетаИдентичны(ДанныеОтчетаВРассылке, ДанныеОтчетаВОтчете);
	//Возврат Ложь;
