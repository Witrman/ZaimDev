...
Процедура ПолучитьСписокНовостей(
	...
	Перем ЛентаНовостей, СписокНайденныхНовостей, Тег, ИмяТемы, ИмяОтбора, КоличествоНовостей, Источник;
	...
	Если ВариантОтбора = 0 Тогда // Все
		Если ПараметрыПолученияНовостей.Свойство("КоличествоНовостей") Тогда
			КоличествоНовостей = ПараметрыПолученияНовостей.КоличествоНовостей;
			Если (ТипЗнч(КоличествоНовостей) <> ТипЧисло) ИЛИ (КоличествоНовостей < 0) Тогда
				КоличествоНовостей = 0; // Все новости
			КонецЕсли;
		КонецЕсли;
		Если ПараметрыПолученияНовостей.Свойство("Источник") Тогда
			Источник = ПараметрыПолученияНовостей.Источник;
			Если (ТипЗнч(Источник) <> ТипСтрока) Тогда
				Источник = "";
			КонецЕсли;
		КонецЕсли;
	...
	Если (ВариантОтбора = 0) ИЛИ (ВариантОтбора = 1) Тогда
	...
	Если ВариантОтбора = 0 // Все
		...
		Если (ВРег(ОтображаемыеНовости_Вариант) = ВРег("Количество"))
				ИЛИ ((Источник = "ФормаПросмотраНовостейРабочийСтол") И (ОтображаемыеНовости_Количество > 0)) Тогда // По количеству
	...
	Если (ВариантОтбора = 0)
			И (Источник = "ФормаПросмотраНовостейРабочийСтол")
