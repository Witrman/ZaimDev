	...
	Если СтрДлина(Параметры.АдресМассиваНовостей) > 0 Тогда
		...
		Если ТипЗнч(МассивНовостей) = ТипМассив Тогда
			Для Каждого ТекущаяНовость Из МассивНовостей Цикл
				Если ТипЗнч(ТекущаяНовость) = ТипСтруктура
						И ТекущаяНовость.Свойство("Важность")
						И ТекущаяНовость.Важность = 1
						И ТекущаяНовость.Свойство("Форма")
						И (
							(ПустаяСтрока(Параметры.ИдентификаторФормы) = Истина)
							ИЛИ (СтрСравнить(ТекущаяНовость.Форма, Параметры.ИдентификаторФормы) = 0))
						И (
							(ТекущаяНовость.Свойство("Событие") = Истина)
