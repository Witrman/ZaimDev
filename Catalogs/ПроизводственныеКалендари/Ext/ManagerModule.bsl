...
Функция ДанныеПроизводственныхКалендарейИзМакета(КодыКалендарей = Неопределено, ФормироватьПолныйНабор = Истина) Экспорт
	...
	МодульЗаполнениеКалендарныхГрафиков = ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ДанныеПроизводственногоКалендаря");
...
Функция ПроизводственныеКалендариИзМакета() Экспорт
	...
	МодульКалендарныеГрафики = ОбщегоНазначения.ОбщийМодуль("Справочники.ПроизводственныеКалендари");
	ТекстовыйДокумент = МодульКалендарныеГрафики.ПолучитьМакет("ОписаниеКалендарей");
...
Функция СписокВидовДня() Экспорт
	...
	Для Каждого МетаданныеВидаДней Из Метаданные.Перечисления.ВидыДнейПроизводственногоКалендаря.ЗначенияПеречисления Цикл
		Если МетаданныеВидаДней <> Метаданные.Перечисления.ВидыДнейПроизводственногоКалендаря.ЗначенияПеречисления.Предпраздничный
			И МетаданныеВидаДней <> Метаданные.Перечисления.ВидыДнейПроизводственногоКалендаря.ЗначенияПеречисления.Нерабочий Тогда
...
//
Функция ПраздничныеДниПроизводственногоКалендаря(КодПроизводственногоКалендаря, НомерГода) Экспорт
	...
	Если КодПроизводственногоКалендаря = "РК" Тогда	
		...
		ДобавитьПраздничныйДень(ПраздничныеДни, 2, 1, НомерГода);
		...
		Если НомерГода > 2009 Тогда
		    ...
		    ДобавитьПраздничныйДень(ПраздничныеДни, 21, 3, НомерГода);
			ДобавитьПраздничныйДень(ПраздничныеДни, 22, 3, НомерГода);
		...
		ДобавитьПраздничныйДень(ПраздничныеДни, 30, 8, НомерГода);	
		Если НомерГода <= 2008 Тогда
			// Законом РК № 152-IV ЗРК от 22 апреля 2009 года исключен День Республики
			ДобавитьПраздничныйДень(ПраздничныеДни, 25, 10, НомерГода);	
		КонецЕсли;
		ДобавитьПраздничныйДень(ПраздничныеДни, 16, 12, НомерГода);		
		ДобавитьПраздничныйДень(ПраздничныеДни, 17, 12, НомерГода);			
		...
		Если НомерГода >= 2006 Тогда
			...
			Если НомерГода = 2006 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 10,  1, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2007 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 20, 12, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2008 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни,  8, 12, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2009 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 27, 11, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2010 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 16, 11, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2011 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни,  6, 11, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2012 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 26, 10, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2013 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 15, 10, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2014 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни,  4, 10, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2015 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 24,  9, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2016 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 12,  9, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2017 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни,  1,  9, НомерГода, Ложь);
			ИначеЕсли НомерГода = 2018 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни, 20,  8, НомерГода, Ложь);
			КонецЕсли;
		КонецЕсли;
		
		Если НомерГода >= 2008 Тогда
			ДобавитьПраздничныйДень(ПраздничныеДни, 6,  7, НомерГода);
		КонецЕсли;
		
		Если НомерГода >= 2012 Тогда
			ДобавитьПраздничныйДень(ПраздничныеДни, 1, 12, НомерГода);
		КонецЕсли;
		
		Если НомерГода >= 2013 Тогда
			ДобавитьПраздничныйДень(ПраздничныеДни, 7,  5, НомерГода);
		КонецЕсли;
...
Процедура ДобавитьПраздничныйДень(ПраздничныеДни, День, Месяц, Год, ПереноситьВыходной = Истина, ДобавлятьПредпраздничный = Ложь, ТолькоНерабочий = Ложь) 
