...
Функция НаименованиеНастройки(Источник)
    ...
    Если НачалоДня(Источник.НачалоПериода) = НачалоГода(Источник.НачалоПериода)
		...
		ПредставлениеПериодаНастройки = "на " + Формат(Источник.НачалоПериода, "ДФ=yyyy") + " г.";
	Иначе
		ПредставлениеПериодаНастройки = "на период " + Формат(Источник.НачалоПериода, "ДФ='MMMM yyyy'") + 
			" - " + Формат(Источник.КонецПериода, "ДФ='MMMM yyyy'");
	КонецЕсли;
