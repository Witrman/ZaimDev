...
Процедура ПодписиУстановитьЗаголовок(Форма)
	...
	Для Индекс = 0 По СписокСтрок.Количество() - 1 Цикл
		ПодписиЗаголовок = ПодписиЗаголовок + ?(ПустаяСтрока(ПодписиЗаголовок), "", ", ") + Строка(Форма["ФизЛицо" + СписокСтрок[Индекс]]);
		Если ЗначениеЗаполнено(Форма["Должность" + СписокСтрок[Индекс]]) Тогда
			ПодписиЗаголовок = ПодписиЗаголовок + " (" + Строка(Форма["Должность" + СписокСтрок[Индекс]]) + ")";
		КонецЕсли;
	КонецЦикла;
	
	ПодписиЗаголовок = "Подписи" + ?(ПустаяСтрока(ПодписиЗаголовок), "", ": ") + ПодписиЗаголовок;
