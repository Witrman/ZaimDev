...
Процедура ОбновитьСписокХО()
	...
	Элементы.СтраницаВсех.Заголовок = "Все ("+?(КоличествоОтображаемых = ОбщееКоличествоКорреспонденций, "", "" + КоличествоОтображаемых + " из ") + ОбщееКоличествоКорреспонденций + ")";
...
&НаСервереБезКонтекста
Функция ЗаполнитьВидОперацииНаСервере(Ссылка, ИмяВидаОперации)
	
	ВидОперации = Перечисления[ИмяВидаОперации].ПустаяСсылка();
	
	Для Каждого Перечень Из Перечисления[ИмяВидаОперации] Цикл
		Если Строка(Перечень) = Ссылка.ВидОперацииДокумента Тогда
			ВидОперации = Перечень;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПолучитьСтрукутруПараметровДляЗакрытияМесяца(Ссылка, ДанныеОбъекта)
	
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("ВидОперацииДокумента", Ссылка.ВидОперацииДокумента);
...
Процедура ВвестиОперациюНаКлиенте(ВыбраннаяСтрока)
	...
	Если ЗначениеЗаполнено(ВыбраннаяСтрока.ВидОперацииДокумента) Тогда
		...
		Для Каждого СтрокаСоответствия Из ТаблицаСоответствий.НайтиСтроки(Новый Структура("ИмяДокумента", ИмяДокумента)) Цикл
			...
			Если ЗначениеЗаполнено(СтрокаСоответствия.ИмяВидаОперации) Тогда
				ВидОперацииДокумента = ЗаполнитьВидОперацииНаСервере(ВыбраннаяСтрока.Ссылка, СтрокаСоответствия.ИмяВидаОперации);
	...
	ИначеЕсли ИмяДокумента = "ЗакрытиеМесяца" Тогда
		...
		ПараметрыДляДокумента = ПолучитьСтрукутруПараметровДляЗакрытияМесяца(ВыбраннаяСтрока.Ссылка, ДанныеОбъекта);
