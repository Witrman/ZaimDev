...
Процедура ОбработчикОповщенияВыбораКлючаВхода(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда	
		...
		Если ДанныеКлючаЭЦП = Неопределено Тогда
			ТекстОшибки = нСтр("ru='Не удалось получить данные для входа в ИС ЭСФ'", "ru");
	...
	Иначе
		...
		Для Каждого ПрофильИСЭСФ Из ВыборкаПрофилиИСЭСФ Цикл
			...
			Попытка
				...
				ИдентификаторСессии = ЭСФКлиент.ОткрытьСессиюСПодписьюПользователя(ПрофильИСЭСФ, Неопределено, ДанныеКлючаЭЦП);
