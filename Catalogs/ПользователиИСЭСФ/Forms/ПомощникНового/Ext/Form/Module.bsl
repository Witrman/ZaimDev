...
Процедура ВыбратьСертификат(Команда)
	...
	Если ЭСФВызовСервера.ИспользоватьОткрытиеСессииСПодписью() И НЕ ЭСФВызовСервера.ИспользоватьВнешнююКриптографиюДляКомпоненты() Тогда
		...
		Если ОтветСтруктура <> Неопределено Тогда
			...
			Если ОтветСтруктура.responseObject.Свойство("pem") Тогда
				СертификатBase64 = Сред(ОтветСтруктура.responseObject.pem,30,СтрДлина(ОтветСтруктура.responseObject.pem)-58);
			КонецЕсли;
			ОбновитьПанельКнопокНазадДалееЗакрыть();
		КонецЕсли;
...
Процедура ИзвлечьСертификатИзФайла(Знач КлючBase64, Знач Пароль)
	...
	Если ОтветСтруктура <> Неопределено Тогда
		...
		ИмяАутентификации = ЭСФКлиентСервер.ИмяАутентификации(СвойстваСертификата.ИИНСубъекта);
		ОписаниеСертификата = СвойстваСертификата.ОписаниеСертификата;
		Если ОтветСтруктура.responseObject.Свойство("pem") Тогда
			СертификатBase64 = СокрЛП(ОтветСтруктура.responseObject.pem);
		КонецЕсли;
		ОбновитьПанельКнопокНазадДалееЗакрыть();
	КонецЕсли;
