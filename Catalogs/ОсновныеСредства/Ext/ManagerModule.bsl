		...
		Если ТекущиеСведенияОС.Следующий() Тогда
			...
			Если ЗначениеЗаполнено(ДокументВыбытия) Тогда	
				Если ИмяДокументаВыбытия = "Передача ОС" Тогда
					ПричинаВыбытия		 = НСтр("ru='передача ОС'");
				ИначеЕсли ИмяДокументаВыбытия = "Списание ОС" Тогда
					ПричинаВыбытия = НСтр("ru='списание ОС'");
				КонецЕсли;
				...
				ТекстСообщения = НСтр("ru='№%1 от %2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ИмяДокументаВыбытияНомер, Формат(ИмяДокументаВыбытияДата, "ДФ=dd.MM.yyyy"));
