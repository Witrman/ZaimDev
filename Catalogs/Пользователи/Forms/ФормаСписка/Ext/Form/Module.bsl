...
Функция КонтактнаяИнформацияПользователя(Пользователь, АдресФотографииПоУмолчанию, УникальныйИдентификатор)
	СтруктураКИ    = ПользователиСлужебный.ОписаниеПользователя(Пользователь);
	СтруктураКИ_БК = ПользователиБК.КонтактнаяИнформацияПользователяБК(Пользователь, СтруктураКИ);
	ЗаполнитьЗначенияСвойств(СтруктураКИ, СтруктураКИ_БК);
	Если СтруктураКИ.Фотография <> Неопределено Тогда
		СтруктураКИ.Фотография = ПоместитьВоВременноеХранилище(СтруктураКИ.Фотография, УникальныйИдентификатор);
	Иначе
		СтруктураКИ.Фотография = АдресФотографииПоУмолчанию;
	КонецЕсли;
