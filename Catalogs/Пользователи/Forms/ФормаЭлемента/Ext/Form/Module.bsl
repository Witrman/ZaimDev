...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			...
			ПользовательИБАдресЭлектроннойПочтыБК = "";
		Иначе
			// Добавление элемента.
	...
	Иначе
		...
		// В БК 3.0 используется своя "Контактная информация"
		ЗаполнитьКонтактнуюИнформацию(ЭтотОбъект, Объект.Ссылка);
	...
	КонецЕсли;
	// В БК 3.0 используется своя "Контактная информация"
	ИмяРеквизитаСПочтойДляВосстановленияПароля = "ПользовательИБАдресЭлектроннойПочтыБК";
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("Запись_КонтактнаяИнформация") Тогда
		
		ПодключитьОбработчикОжидания("ОбновитьКонтактнуюИнформациюПользователяБК", 0.1, Истина);
...
Процедура НастроитьФорму(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.КонтактнаяИнформацияБК.Видимость = НЕ ПользовательИБЗапрещеноВосстанавливатьПароль;	
...
Процедура ПереопределитьРедактированиеКонтактнойИнформацииВСервисе()
	...
	КИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов;
	
	СтрокаEMail = КИ.НайтиСтроки(Новый Структура("Вид", Справочники["ВидыКонтактнойИнформации"].СлужебныйАдресЭлектроннойПочтыПользователя))[0];
	...
	СтрокаТелефон = КИ.НайтиСтроки(Новый Структура("Вид", Справочники["ВидыКонтактнойИнформации"].СлужебныйТелефонПользователя))[0];
...
Процедура ОбновитьСпособИзмененияEMailВСервисе()
	...
	СтрокаEMail = ТаблицаКИ.НайтиСтроки(Новый Структура("Вид", Справочники["ВидыКонтактнойИнформации"].СлужебныйАдресЭлектроннойПочтыПользователя))[0];
	ЭлементEMail = Элементы[СтрокаEMail.ИмяРеквизита];
...
Функция ВидКонтактнойИнформацииEmailПользователя()
	...
	ИмяПредопределенногоЗначения = "Справочник." + "ВидыКонтактнойИнформации" + ".СлужебныйАдресЭлектроннойПочтыПользователя";
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Если ДействияСПользователемСервиса <> Неопределено Тогда
		...
		// В БК3 не используется подсистема КонтактнаяИнофрмация из БСП.

		Если ОбщегоНазначенияБКВызовСервера.СлужебныйАдресЭлектроннойПочтыПользователя(Объект.Ссылка) = Неопределено Тогда
			ЗаполненEMail = Ложь;

		Иначе
			ЗаполненEMail = Истина;	
		КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьКонтактнуюИнформациюПользователяБК()
	ЗаполнитьКонтактнуюИнформацию(ЭтотОбъект, Объект.Ссылка);
КонецПроцедуры
...
Процедура ЗаполнитьКонтактнуюИнформацию(Форма, ТекущийПользователь)
	...
	Если НЕ ЗначениеЗаполнено(ТекущийПользователь) Тогда
		Форма.ПользовательИБАдресЭлектроннойПочтыБК = "";
		Форма.ПользовательИБАдресЭлектроннойПочты   = "";
		Возврат;
	КонецЕсли;
	
	КонтактнаяИнформацияПользователяБК = КонтактнаяИнформацияПользователя(ТекущийПользователь);
	
	Форма.ПользовательИБАдресЭлектроннойПочтыБК = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КонтактнаяИнформацияПользователяБК, "АдресЭлектроннойПочты", "");
	Форма.ПользовательИБАдресЭлектроннойПочты   = Форма.ПользовательИБАдресЭлектроннойПочтыБК;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция КонтактнаяИнформацияПользователя(Пользователь)
	Возврат ПользователиБК.КонтактнаяИнформацияПользователяБК(Пользователь, Неопределено);
КонецФункции
