...
Процедура ВыполнитьДенежнуюОперацию(Команда)
	...
	Если Сумма = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Сумма по денежной операции не должна быть равна 0.");
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьДенежнуюОперациюЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения <> Неопределено Тогда
		...
		Если РезультатВыполнения.Результат Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Денежная операция выполнена успешно.");
		Иначе
