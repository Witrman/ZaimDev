...
Процедура ИПНПрочиеВычетыДо2018Года(Команда)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ТекстВопроса = "Перед вводом сведений необходимо записать элемент. Записать?";
	...
	Если НЕ ОтменаРедактирования Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			...
			ИначеЕсли Элемент.ТекущиеДанные.Период <= ДатаЗапретаИзмененияДанных Тогда
				СообщениеОбОшибке = НСтр("ru = 'Период закрыт для редактирования, укажите период позже даты '") + Формат(ДатаЗапретаИзмененияДанных, "ДФ='дд ММММ гггг'");
				ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке,, "КартСчетаФизЛицНаборЗаписей.Период", , Отказ);
			Иначе
...
Процедура УстановитьОчиститьИзображение(Форма, ИмяЭлементаСКартинкой, Изображение)
	...
	Форма["ЗагрузкаОчистка" + ИмяЭлементаСКартинкой] = ?(ЗначениеЗаполнено(Изображение), "Очистить ", "Загрузить ")
		+ СтрРазделить(Форма.Элементы["Адрес" + ИмяЭлементаСКартинкой].ТекстНевыбраннойКартинки, " ")[1];
...
Процедура ОбновитьПредставлениеЭлемента(Форма, ИмяОбновляемогоЭлемента)
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "ГражданствоФизЛиц" Тогда
		...
		Иначе
			ГражданствоФизЛицПредставлениеЗаписи = "" + ГражданствоФизЛиц.Страна + ?(ГражданствоФизЛиц.НеИмеетПравоНаПенсию, НСтр("ru = ', не '"),", " ) + НСтр("ru = 'имеет право на пенсионное обеспечение'") +  
					  ?(ГражданствоФизЛиц.НеПодлежитСоциальномуСтрахованию, НСтр("ru = ', не '"),", " ) + НСтр("ru = 'подлежит социальному страхованию'") +
					  ?(ГражданствоФизЛиц.НеПодлежитСоциальномуМедицинскомуСтрахованию, НСтр("ru = ', не '"),", " ) + НСтр("ru = 'подлежит социальному медицинскому страхованию'") +
					  ?(ГражданствоФизЛиц.НеЯвляетсяНалоговымРезидентомРК, НСтр("ru = ', не '"),", " ) + НСтр("ru = 'является налоговым резидентом РК'") + 
					  ?(ГражданствоФизЛиц.ИностранныйСпециалист,", ", НСтр("ru = ', не '") ) + НСтр("ru = 'является иностранным специалистом'");
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "ПаспортныеДанныеФизЛиц" Тогда
		...
		Иначе
			ПаспортныеДанныеФизЛицПредставлениеЗаписи = "" + ПаспортныеДанныеФизЛиц.ДокументВид + ", "+
						?(ЗначениеЗаполнено(ПаспортныеДанныеФизЛиц.ДокументСерия), НСтр("ru = 'Серия: '") + ПаспортныеДанныеФизЛиц.ДокументСерия + ", ","")+ 
						"№ " + ПаспортныеДанныеФизЛиц.ДокументНомер + 
						НСтр("ru = ', выдан: '") + Формат(ПаспортныеДанныеФизЛиц.ДокументДатаВыдачи,"ДФ='дд ММММ гггг'; ДП='...'") + 
						НСтр("ru = ' года, '") + ПаспортныеДанныеФизЛиц.ДокументКемВыдан;
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "СведенияОПенсионномОбеспеченииФизЛиц" Тогда
		...
		СведенияОПенсионномОбеспеченииФизЛицПредставлениеЗаписи = ?(СведенияОПенсионномОбеспеченииФизЛиц.Пенсионер,НСтр("ru = 'Является'"), НСтр("ru = 'Не является'")) + НСтр("ru = ' пенсионером'") +
								НСтр("ru = '; Пенсионный фонд: '") + ?(НЕ ЗначениеЗаполнено(СведенияОПенсионномОбеспеченииФизЛиц.ПенсионныйФонд), НСтр("ru = 'не указан'"), СведенияОПенсионномОбеспеченииФизЛиц.ПенсионныйФонд);
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "СведенияОбИнвалидностиФизлиц" Тогда
		...
		Если ЗначениеЗаполнено(СведенияОбИнвалидностиФизлиц.ОтношениеКИнвалидности) Тогда
			СведенияОбИнвалидностиФизлицПредставлениеЗаписи = Строка(СведенияОбИнвалидностиФизлиц.ОтношениеКИнвалидности) + ": " + СведенияОбИнвалидностиФизлиц.ГруппаИнвалидности + НСтр("ru = ' группы'") 
						+ НСтр("ru = ', справка серия: '") + СведенияОбИнвалидностиФизлиц.СерияСправки + НСтр("ru = ', номер: '") + СведенияОбИнвалидностиФизлиц.НомерСправки 
						+ НСтр("ru = ', выдана: '")  + Формат(СведенияОбИнвалидностиФизлиц.ДатаВыдачи,"ДФ='дд ММММ гггг';ДП='...'") + НСтр("ru = ' года '")
						+ НСтр("ru = ', срок действия: '") + Формат(СведенияОбИнвалидностиФизлиц.СрокДействияСправки,"ДФ='дд ММММ гггг';ДП='...'") + НСтр("ru = ' года '");
		Иначе
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "СведенияОФизлицахУчастникахВОВ" Тогда
		...
		Если ЗначениеЗаполнено(СведенияОФизлицахУчастникахВОВ.Статус) Тогда
			СведенияОФизлицахУчастникахВОВПредставлениеЗаписи = НСтр("ru = 'Статус: '") + СведенияОФизлицахУчастникахВОВ.Статус + НСтр("ru = ', удостоверение серия: '") + СведенияОФизлицахУчастникахВОВ.СерияУдостоверения
								 + НСтр("ru = ', номер: '") + СведенияОФизлицахУчастникахВОВ.НомерУдостоверения + НСтр("ru = ', выдано: '")  + Формат(СведенияОФизлицахУчастникахВОВ.ДатаВыдачи,"ДФ='дд ММММ гггг';ДП='...'") + НСтр("ru = ' года '");
		Иначе
...
Процедура ПослеЗакрытияОкнаРедактированияФИО(Результат, Параметры) Экспорт
	...
	Если ВРег(ФИОДоИзменения) <> ВРег(НовоеФИО) Тогда
		Режим = РежимДиалогаВопрос.ДаНет;
		ТекстВопроса = НСтр("ru = 'Изменить наименование элемента справочника на '") + НовоеФИО + "?";
...
Процедура ПрочитатьСведенияПрочиеВычетыНаСервере()
	...
	Если Выборка.Количество() > 0 Тогда
		Пока Выборка.Следующий() Цикл
			ИПНПрочиеВычеты = ИПНПрочиеВычеты + ?(ИПНПрочиеВычеты <> "", "; ", НСтр("ru = 'Предоставляется '")) + 
							НСтр("ru = 'вычет ""'") + Выборка.ВычетИПН + 
							НСтр("ru = '"" в сумме '") + Формат(Выборка.СуммаВычета, "ЧЦ=15; ЧДЦ=2; ЧРГ=") + 
							НСтр("ru = ', налогоплательщик '") + Выборка.Налогоплательщик;
		КонецЦикла;
	Иначе 
