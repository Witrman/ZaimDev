...
Процедура УправлениеФормой(Элементы, Год)
	...
	Если Год < 2007 Тогда
		Элементы.СведенияДляИсчисленияИПНПримененнаяЛьгота.Заголовок = "в т.ч. льгота";
	ИначеЕсли Год < 2018 Тогда	
		Элементы.СведенияДляИсчисленияИПНПримененнаяЛьгота.Заголовок = "Примененная льгота";
	Иначе 
		Элементы.СведенияДляИсчисленияИПНПримененнаяЛьгота.Заголовок = "Примененный вычет";	
	КонецЕсли;	
...
Процедура ПриИзмененииОтборов()
	...
	Если Модифицированность И ЗначениеЗаполнено(ТекущийНалогоплательщик) Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Данные были изменены, учитывать введенные сведения по налогоплательщику '") + ТекущийНалогоплательщик + 
									НСтр("ru = '  за '") + Формат(ТекущийГод, "ЧГ=") + НСтр("ru = ' год?'"), Режим, 0);
	Иначе 
